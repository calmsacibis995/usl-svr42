...
#ident	"@(#)ucb:i386at/ucbcmd/mt/mt.c	1.1"
#ident	"$Header: $"
...
#define	MTASF		100	/* Phoney ioctl for abs. file positioning */

	...
	char **argv;
{
	...
	if (comp->c_code == MTASF) {
		 ...
		 * space forward.
		 */
		if (mt_asf  ||  !MT_TYPE_SCSI(mt_status.mt_type)) {
			/*printf("mt: rewind\n");*/

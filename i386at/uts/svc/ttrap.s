...
/	actual or intended publication of such source code.
	...
	.ident	"@(#)uts-x86at:svc/ttrap.s	1.16"
	.ident	"$Header: $"

include(../util/symbols.m4)

	.set    SEL_RPL, 0x03           / selector priority level mask
...
.kentry_check:
	...
	jne	.kentry_done	/    dont call hooks
	movl	$io_kenter, %esi
...
ifdef(`NODEBUGGER',`',`
	...
	movl	%db6, %eax		/ Save users DR6
	movl	%eax, u+u_debugreg+24
	xorl	%eax, %eax		/ Clear DR6 for kernel
	movl	%eax, %db6
	movl	save_kernel_db0, %eax	/ Restore kernels DR0
	movl	%eax, %db0
	movl	save_kernel_db1, %eax	/ Restore kernels DR1
	movl	%eax, %db1
	movl	save_kernel_db2, %eax	/ Restore kernels DR2
	movl	%eax, %db2
	movl	save_kernel_db3, %eax	/ Restore kernels DR3
	movl	%eax, %db3
	movl	save_kernel_db7, %eax	/ Restore kernels DR7
	movl	%eax, %db7
	xorl	%eax, %eax
...
.kexit_check:
	...
	js	.kexit_neg
	jnz	.kexit_done	/ If not last exit, dont call hooks
	movl	$io_kexit, %esi
...
/       various kernel entry points.
	...
	.align	4
	.globl	cmntrap
	.type	cmntrap,@function
cmntrap:
ifdef(`VPIX',`
	lidt    %cs:idtdsc1     / Note: dont care about DS..pure linear
')
...
ifdef(`MERGE386',`
	...
	jnz     .user_trap              / IS_LDT_SEL test is meaningless
')')
	testw	$SEL_RPL, %ax		/ check for kernel mode
	jz	.kern_trap
...
ifdef(`MERGE386',`
	...
	je	.notvm86trap
	cmpl	$0, vmonstate		/ have we come from a V86 process ?
	je	.notvm86trap		/ if not, continue with the unix
	call	vm86_trap		/ call the vm86 trap handler
	cmpl	$0, %eax		/ We took it
	je	.vm86_ret_u
	jmp	.notvm86trap		/ We pass on 1 and all others

.vm86_ret_u:
	addl	$4, %esp		/ otherwise
	jmp	ret_user		/ return

/	Here we depend on the hook being kept in sync with what is done
/	at the system call call gate entry point.  That is we want to be in
/	the same state when we call systrap as we are when set up in misc.s.
/
.vm86_sys_call:				 / from uni.s sys_call
	movl	[UESP\*4](%ebp), %eax
	movl	[ECX\*4](%ebp), %ebx
	movl	%ebx, [UESP\*4](%ebp) 	/ Put flattened stack in for UESP
	movl	%eax, [ECX\*4](%ebp)	/ Save original SP for later restore
	movb	$0, u+u_sigfault   / donot catch signal processing GP faults
	call	systrap
	addl	$4, %esp
	movl	[UESP\*4](%ebp), %eax
	movl	[ECX\*4](%ebp), %ebx
	movl	%ebx, [UESP\*4](%ebp) 	/ Put everything back the way it came
	movl	%eax, [ECX\*4](%ebp)
	jmp     ret_user        	/ jump to common return to user code
	
.notvm86trap:
')
	movb	$0, u+u_sigfault    / dont catch signal processing GP faults
...
ifdef(`VPIX',`
	lidt    %cs:idtdsc1     / Note: dont care about DS..pure linear
')
	/ Use pushl instead of a subl because it preserves the psw flags
	pushl	$0		/ pad the stack with dummy ERRCODE, TRAPNO
	pushl	$0
...
.clock:
	pushl	%eax
	movl	sysenvmtp, %eax / sys environment structure location
                                / test for micro-channel bus
	testl	$MC_BUS, [machflags\*4](%eax)
	jz	clock_nomca
	inb	$0x61		/ clear clock latch on MCA bus only
	orb	$0x80, %al
	outb	$0x61
...
ifdef(`VPIX',`
	...
	jz      .real_cs
	pushl   $USER_CS    / User CS ... dont push a v86 mode CS
	jmp     .push_ip
...
')
	...
	/ We have to decrement the interrupt count because
	/ k_trap() could be called to resolve a user page-fault due
	/ to addupc() referencing user data-structures.
	/ k_trap() will not resolve a page-fault if we are servicing 
	/ an interrupt. We are done with the clock interrupt handler
	/ and we are temporarily decrementing the interrupt count
	/ represented by the clock interrupt.

	decl	intr_entered	/ un-nest the clock interrupt indicator
	sti                     / enable interrupts
	pushl   $1              / one tick
        pushl   [EIP\*4](%ebp)
	...
	incl	intr_entered	/ Bump up the clock interrupt indicator

.cmnintret:
	cmpl    $0, ipl         / if not at ipl zero ..
	jnz     .notimein        /   .. dont run timeouts
...
ifdef(`NODEBUGGER',`',`
	...
	jnz	.skip_dbreg_save
	movl	%db0, %eax		/ Save kernels debug registers
...
.itis_v86:
	...
	testw   $SEL_RPL, [CS\*4](%ebp)  / Did we interrupt kernel mode?
	jz      .not_v86                 / if yes, then leave, otherwise,
...
.was_v86:
	...
	popfl                           / wont switch to V86 task
			    / OK to clear VM since it cant be on anyway!
	popl	%gs
	popl	%fs
	popl	%es
	popl	%ds
...
/ Entry point for cleanup after user signal handling
	...
	.align	4
	.globl  sigclean	/extern sigclean
	.globl	sig_clean
	.type	sig_clean,@function
sig_clean:
	/ set up the stack to look as in reg.h
	...
	cli                     / Dont allow interrupts that could clear the
	...
	jmp     .user_ret        / Cant do regular return-to-user processing
	.size	sig_clean,.-sig_clean

...
#ident	"@(#)uts-x86at:svc/tables2.c	1.10"
#ident "$Header: $"
...
#include	<svc/brgmgr.h>		/* Added for common boot support */
#ifdef VPIX
...
extern overrun(), mcetrap(), resvtrap();
...
/*
** The folowing was added to support the common boot from GDI
*/

extern char kpt0, kptn, stext[], sdata[], sbss;
extern int uprt_start();

struct sysenvmt k_sysenvmt = {0};
struct sysenvmt *sysenvmtp = &k_sysenvmt;

struct brg_ctl_blk _start = {
	BCBmagic,
	(paddr_t)KVBASE,
	(paddr_t)KVSBASE,
	(paddr_t)&kpt0,
	(paddr_t)&kptn,
	(paddr_t)stext, (paddr_t)sdata, (paddr_t)&sbss,
	(paddr_t)&df_stack, 0x0ffcL,
	(paddr_t)gdt, sizeof(gdt) / sizeof(struct seg_desc),
	(paddr_t)idt, sizeof(idt) / sizeof(struct gate_desc),
	(paddr_t)&scall_dscr,
	(paddr_t)&sigret_dscr,
#ifdef VPIX
	(paddr_t)idt2, sizeof(idt2) / sizeof(struct gate_desc),
#else
	NULL,NULL,
#endif
	BCBversion,	/* version number   */
	0L,	    /* control flag	*/
	0L,	    /* status flag	*/
	JTSSSEL,
	KTSSSEL,
	NULL,		/* bootinfo address */
	(paddr_t)&k_sysenvmt,	    /* sysenvmt address */
	NULL,		/* bps address	    */
	NULL,		/* uprt start	    */
	NULL,		/* kernel start	    */
	0L, 0L, 0L, 0L, 0L,

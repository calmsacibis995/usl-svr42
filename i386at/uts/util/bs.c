...
/*
 * bs.c:  Parse the bootstring and set rootdev, swapdev, and dumpdev.
...
extern	dev_t rootdev, swapdev, dumpdev;

/* boot string variables */
#define	MAXPARMS	2		/* number of device parms */

/*
 * this will be used to switch rootdev, 
 * swapdev and pipedev to ramd when we
 * boot from the floppy.
 */
char	bf_argv[BC_MAXARGS][B_STRSIZ] = { 
		"",
		"rootdev=ramd(0,0)",
		"swapdev=ramd(1,0)",
		"pipedev=ramd(0,0)"
...
bootarg_parse()
{
	...
	for (i = 1; i < bootinfo.bargc; i++) {	/* parse args */
		if (bootinfo.bootflags & BF_FLOPPY)
			bcopy(&bf_argv[i][0], hold_arg, B_STRSIZ);
		else
			bcopy(&bootinfo.bargv[i][0],
					hold_arg, B_STRSIZ);
...
char *s;
{
	...
	if ((p = strchr(s, '=')) == (char *)0)  
		return(oem_doarg(hold_arg));
...
int *parm, base;
{
	...
	digits = digit[base];
	*parm = 0;
	while ((q = strchr(digits, (*p>='A') ? *p|0x20 : *p)) != (char *)0) {

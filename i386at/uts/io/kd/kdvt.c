...
#ident	"@(#)uts-x86at:io/kd/kdvt.c	1.12"
#ident	"$Header: $"
...
int	force;
{
	...
	struct proc	*procp;
	unchar	omode,tmp;
	extern	unsigned short ws_ckbstate, ws_lkbstate;
	...
	case VT_SETMODE:
		...
		if (vtmode.mode == VT_PROCESS) {
			...
			if (!ws_procmode(&Kdws,chp)) {
				drv_getparm(UPROCP, (ulong*)&chp->ch_procp);
				drv_getparm(PPID, (ulong*)&pid);
...
Switch2VT0Text()
{
	channel_t *vt0 = ws_getchan(&Kdws, 0);

	if(vt0 == NULL)
		return;
	ws_activate(&Kdws, vt0, VT_FORCE);
	if(vt0->ch_dmode == KD_GRAPHICS)
			kdv_textmode(vt0);
}

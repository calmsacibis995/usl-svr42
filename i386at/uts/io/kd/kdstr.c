...
#ident	"@(#)uts-x86at:io/kd/kdstr.c	1.17"
#ident	"$Header: $"
...
#ifdef	BLTCONS
#include <io/kd/kd_btc.h>
#endif

#include <io/ddi.h>

STATIC int kdopen(), kdclose();
int	kdwput(), kdwsrv();
...
kdinit()
{
	...
	if (ws_alloc_attrs(&Kdws, &Kd0chan, KM_NOSLEEP))
		cmn_err(CE_PANIC, "kdinit: no room for attrs info");
...
cred_t	*credp;
{
	...
	if ((chp = ws_getchan(&Kdws, indx)) == (channel_t *)NULL) {
		...
		if (ws_alloc_attrs(&Kdws, chp, KM_SLEEP)) {
			cmn_err(CE_WARN, "kdopen: out of memory for t_attrs");
			return(ENOMEM);
		}
...
#ifdef MERGE386
		else	if (kdppi_ioctl(qp, mp, iocp, chp)){
			break;
		}
#endif /* MERGE386 */
		else	ws_iocnack(qp, mp, iocp, EINVAL);
#ifdef DEBUG1
	...
	case CH_CHAN:
		...
		case CH_CHANCLOSE:
			...
			if (chp->ch_id != 0) {
			   ...
			   /* reset attributes for VT 0. (ws_chinit will do it for other
			    * VTs). Note that I can call ws_alloc_attrs without
			    * checking the return val since it won't need to do
			    * a kmem_zalloc(). Therefore, this call will always
			    * succeed.
			    */
	...
	case K_RBT:
		{
			...
			if (inrtnfirm) {
				softreset();
              			...
              			btcreset();
#endif
				Write8042CommandByte(KB_RESETCPU);
			} else if((chp = ws_activechan(&Kdws)) != NULL && 
				chp->ch_rawmode == 0) {
					if (proc_init)
						psignal(proc_init, SIGFPE);
			}
		}
		return(0);
	case K_SRQ:

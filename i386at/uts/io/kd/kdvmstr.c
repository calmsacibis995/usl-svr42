...
#ident	"@(#)uts-x86at:io/kd/kdvmstr.c	1.5"
#ident	"$Header: $"
...
channel_t	*chp;
{
	...
	else	kvmp->kdvm_arg = -1;
	strioccall(do_kdvmstr_ioctl, kvmp, iocbp->ioc_id, qp);
	ws_iocack(qp, mp, iocbp);
}

int	
do_kdvmstr_ioctl(kvmp, iocid, qp, rvalp)
struct kdvm_iocarg  *kvmp;
long	iocid;
queue_t	*qp;
int	*rvalp;
...
#ifdef DEBUG
		cmn_err(CE_NOTE, "Calling ws_queuemode(chp,LDEV_ATTACHQ, %x)",
			arg);
#endif
...
int	cmd;
{
	...
	case SW_ENHB80x43:
	case SW_ENHC80x43:
		if ((VTYPE(V600)) || (!DTYPE(Kdws, KD_EGA) && !DTYPE(Kdws, KD_VGA))) {
			rv = ENXIO;
			break;
		}
		cmd -= OFFSET_80x43;
...
kdvm_xenixioctl(chp, cmd, arg, rvalp, ioc_hit)
channel_t	*chp;
int	cmd,
	arg;
int	*rvalp,

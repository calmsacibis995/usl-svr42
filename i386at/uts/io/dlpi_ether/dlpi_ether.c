...
#ident	"@(#)uts-x86at:io/dlpi_ether/dlpi_ether.c	1.16"
#ident	"$Header: $"
...
mblk_t *
DLform_snap(mp,dst_addr)
mblk_t *mp;
unsigned char *dst_addr;
{
dl_unitdata_req_t       *data_req = (dl_unitdata_req_t *)mp->b_rptr;

	struct llcb *llcbp = (struct llcb *)((caddr_t)data_req + data_req->dl_dest_addr_offset);
	mblk_t *nmp;
	DL_mac_hdr_t *hdr;
	...
	bcopy((caddr_t)dst_addr,(caddr_t)hdr->src.bytes,DL_MAC_ADDR_LEN);
...
ushort dsap,ssap,control;
	...
	*/
	...
	freeb(mp);
        ...
        } else if (DLxmit_packet(bd, nmp) == -1)
		freemsg(nmp);
        splx(old);
}
...
int oldlevel;
	...
	bcopy((caddr_t)&subs_sap->snap_global,(caddr_t)&snap_global,SNAP_GLOBAL_SIZE);
...
mblk_t	*mp;
{
	...
	case DL_CSMACD:
		if (sap->sap_addr == SNAPSAP) {
			if ( (!(sap->flags & SNAPCSMACD)) || 
			      (!(nmp = DLform_snap(mp,bd->eaddr.bytes)))){
			        	DLuderror_ind(q, mp, DL_UNDELIVERABLE);
					return;
			}
		} else {
	...
	if (bd->flags & TX_BUSY) {
		...
		DLuderror_ind(q, nmp, DL_UNDELIVERABLE);
	}
	splx(old);
	return;
}
		...
		default:
        	...
        	if (bd->flags & TX_BUSY) {
 			bd->flags |= TX_QUEUED;
       			...
       			freemsg(nmp); 	/* Update the appropriated SNMP stat */
        	}
        	splx(old);
		return NULL;
	}

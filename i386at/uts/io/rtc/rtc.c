...
#ident	"@(#)uts-x86at:io/rtc/rtc.c	1.3"
#ident	"$Header: $"
...
int *rval_p;
{
	...
	case RTCRTIME:
		...
		if (copyout((caddr_t)rtcbuf, addr, RTC_NREG) != 0)
			ecode = EFAULT;
		break;
	case RTCSTIME:
		...
		if (copyin(addr, (caddr_t)rtcbuf, RTC_NREGP) != 0) {
			ecode = EFAULT;
			break;
		}
		rtcput(rtcbuf);

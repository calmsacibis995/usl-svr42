...
#ident	"@(#)uts-x86at:io/kbd/kbd.c	1.5"
#ident	"$Header: $"
 ...
 *	giant infinite loop in the same function (kbdproc).  The unreachable
...
extern int  spltty(), splx(), strcpy(), timeout();
extern void untimeout();
static int  kbdopen(), kbdrput(), kbdrsrv(), kbdwput(),
	    kbdwsrv(), kbdclose(), kbdunload(), kbdproc(),
	    kbd2big(), zzcompare(), kbdresult(), frisk(), kbd_ttype(),
	    kbdlist();
static void kbdioc(), tabfill(), kbdunlink(), freetable(), kbdload(),
	    kbd_publink(), kbd_prvlink(), kbdverb(), hotsw(), kbdlaux(),
	    pay_the_piper(), kbd_deref(), kbd_timeout(), kbd_runtime(),
	    kbd_setime();
static struct kbd_tab *kbdfind(), *kbdlinktab(), *newtable(),
		      *kbdnewcomp(), *kbd_newalp();
static struct tablink *kbd_attached(), *grabtl();
static struct kbdusr *new_side();
static mblk_t *buf_to_msg();
...
extern struct user u;	/* for setting u_error during open */
extern clock_t lbolt;	/* timer needs this */
	...
	int	sflag;	/* stream open flag */
{
	...
	uu->zuid = u.u_uid; /* our security blanket */
	uu->zumem = uu->lsize = 0;
	uu->lalsz = 0;
	...
	register struct kbdusr *d;
{
	...
	register int rval, i; /* rval= kbdproc ret; i= relative position */
...
chew_it_again:
	...
	if (d->u_current && (d->u_current->t_flag == KBD_COT)) {
		...
		out = (mblk_t *) 0;
		while (cur && mp) {
			if (! (rval = kbdproc(q, mp, d, cur, &out, i))) {
	 ...
	 */
		...
		out = (mblk_t *) 0;
		if (! (rval = kbdproc(q, mp, d, d->u_cp, &out, 0))) {
			if (out)
				putnext(q, out);
			out = (mblk_t *) 0;
			if ((mp->b_rptr < mp->b_wptr) || mp->b_cont)
 ...
 */
				...
				++i;
				while (cur && mp) {
					if (! (rval = kbdproc(d->u_q, mp, d, cur, &out, i))) {
						if (out)
							putnext(d->u_q, out);
						freemsg(mp);
						return;
 ...
 */
static void
kbdioc(q, mp, iocp, d)		/* q is write queue pointer */

	queue_t *q;
	mblk_t *mp;
	register struct iocblk *iocp;
	register struct kbdusr *d;	/* d is uu->oside to start */
{
 ...
 */
				...
				if (t->t_flag == KBD_COT) {
					...
					t->t_alpfunc = (unsigned char *) alp_con(t->t_name, &(tl->l_alpid));
				}
				tl->l_next = d->u_list;
				d->u_list = tl;
				++(t->t_ref);
	...
	case KBD_DETACH:
			...
			}
			{
				...
				if ((t->t_flag != KBD_COT) &&
				    (t->t_flag & KBD_ALP)) {
					if (svp = alp_discon(t->t_name, tl->l_alpid))
						putnext(RD(q), svp);
	...
	register struct iocblk *iocp;
{
	...
	case KBD_UNLOAD:
		 ...
		 * nak it.  kbdunload did the error setting.
		 */
...
again_sam:
	rp = mp->b_rptr;
	while (1) {		/* Snidely Whiplash, heh heh heh */
#if 0
  ++ntimes;
  if (ntimes > 4097) {
...
static int
kbdresult(off, textp, where)
	int off;	/* offset in table text */
	unsigned char *textp; /* text for this table */
	mblk_t **where;
{
...
static mblk_t *
buf_to_msg(base, top)
	unsigned char *base, *top; /* top-1 points to last byte */
{
	register mblk_t *mp;

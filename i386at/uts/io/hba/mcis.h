...
#ifndef _IO_HBA_MCIS_H	/* wrapper symbol for kernel use */
#define _IO_HBA_MCIS_H	/* subject to change without notice */

#ident	"@(#)uts-x86at:io/hba/mcis.h	1.8"
#ident	"$Header: $"
...
struct mcis_scb
{
	...
	uint		s_hostcnt;	/* system buffer count		*/
	paddr_t 	s_tsbp;		/* tsb address			*/
	paddr_t 	s_link;		/* link pointer			*/
	unchar		s_cdb[12];	/* scsi cmd descriptor block	*/
...
struct mcis_fcscb	
{
	unchar		fc_cmdop;
	unchar		fc_cmdsup;
	ushort		fc_gtv   : 13;		/* Global time value */
	ushort		fc_drate : 3;		/* Transfer rate */
};

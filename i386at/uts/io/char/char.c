...
#ident	"@(#)uts-x86at:io/char/char.c	1.10"
#ident	"$Header: $"
	...
	while ((mp = getq(qp)) != NULL) {
	   ...
	   if (cp->c_state & C_RAWMODE) {
	   	if (!validproc(cp->c_rawprocp,cp->c_rawpid)) {
			...
			cp->c_rawprocp = NULL;
			cp->c_stashed_p_v86 = NULL;
			cp->c_rawpid = 0;
			setchanmode(OTHERQ(qp), 0);
	   	}
	   }
...
int cnt, flush;
{
	 ...
	 */
	if (cp->c_state & C_XQUEMDE && cp->c_xqinfo->xq_devices&QUE_KEYBOARD) {
	...
	register charstat_t *cp;
{
	...
	case GIO_KEYMAP: {
		...
		else	bcopy(cmp->cr_keymap_p, kmp, size);
		nmp->b_wptr += size;
	...
	register charstat_t *cp;
{
	...
	case KDDFLTKEYMAP: {
		...
		if (kp->key_direction == KD_DFLTGET) {
		   ...
		   oldpri = splstr();
#ifdef DEBUG
		cmn_err(CE_NOTE, "DFFLTKEYMAP get map of type %s",
				type == SCO_FORMAT ? "SCO FORMAT" :
						"USL FORMAT");	
#endif
		   if(type == SCO_FORMAT)
			xlate_keymap(cmp->cr_defltp->cr_keymap_p,
							&kp->key_map, type);
		   else bcopy(cmp->cr_defltp->cr_keymap_p,
		...
		oldpri = splstr();
#ifdef DEBUG
		cmn_err(CE_NOTE, "DFFLTKEYMAP set map of type %s",
				type == SCO_FORMAT ? "SCO FORMAT" :
						"USL FORMAT");	
#endif
		if(type == SCO_FORMAT)
		...
		else	bcopy(&kp->key_map,cmp->cr_defltp->cr_keymap_p,
					sizeof(keymap_t));
		splx(oldpri);
		chr_iocack(qp,mp,iocp,0);
		break;
...
int rawmode;
{
	...
	if((mp = allocb(sizeof(ch_proto_t), BPRI_HI)) == NULL){
		cmn_err(CE_NOTE, "setchanmode: cannot allocate buf");
		return;
	}

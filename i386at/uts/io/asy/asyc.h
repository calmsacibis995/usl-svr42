...
#ident	"@(#)uts-x86at:io/asy/asyc.h	1.6"
#ident	"$Header: $"
...
#define 	FIFOEN550	0xc7	/* 550 fifo enabled, Rx fifo level at 14 */
...
struct asyc_aux {
	...
	uchar_t obuf[OSIZE];
	uchar_t ierrs[ISIZE];
	ushort	oput, oget; /* Offset into obuf */
	ushort	iput, iget; /* Offset into ibuf and ierrs */
	int		asyc_state;
	ushort	fifo_size;
	ushort	asyc_iflag;	
};
...
struct asyc{
	...
	minor_t		asyc_dev;
#ifdef MERGE386
	int (*asyc_ppi_func)();		/* Merge asyc func pointer */
	unsigned char *asyc_ppi_data;	/* merge data */
#endif /* MERGE386 */
};

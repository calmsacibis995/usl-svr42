...
#ifndef _IO_TARGET_SD01_H	/* wrapper symbol for kernel use */
#define _IO_TARGET_SD01_H	/* subject to change without notice */

#ident	"@(#)uts-x86at:io/target/sd01.h	1.10"
#ident	"$Header: $"
...
struct job{
	...
	unsigned int	j_seccnt;	/* sector count			*/
};
...
struct disk {
	...
	struct dev_spec *dk_spec;	/* Device helper strucutre	*/
	...
	char *blkbuf;			/* Marginal bad block data area	*/
	...
	buf_t  *dk_bbh_rbuf;             /* buffer header for BBH (reads)*/
	buf_t  *dk_bbh_wbuf;             /* buffer header for BBH (wrts) */
 	buf_t  *dk_bbh_mbuf;             /* buffer header for BBH (misc) */
	struct job *dk_bbh_rjob;         /* job struct for BBH (reads)   */
	struct job *dk_bbh_wjob;         /* job struct for BBH (writes)  */
	struct job *dk_bbh_mjob;         /* job struct for BBH (misc)    */
	struct alts_mempart dk_amp;	/* incore alts partition info	*/
	struct alts_mempart *dk_wkamp_p;/* working cpy alts part info	*/
	struct alt_info *dk_alttbl;	/* AT&T alternate sector table	*/
	struct altsectbl *dk_ast_p;	/* ptr to working cpy of alttbl	*/
	...
	struct job *dk_syncq_fw;	/* 1st entry on sync queue 	*/
	struct job *dk_syncq_bk;	/* Last entry on sync queue 	*/
	int	dk_syncq_maxl;		/* max length of sync queue	*/
	int	dk_syncq_curl;		/* current length of sync queue */
	struct	disk *dk_waitp;		/* list of waiting devices	*/
	int	dk_outsz_lo;		/* low water mark for send job	*/
	int	dk_outsz_hi;		/* hi water mark for send job	*/
};
#define DK_AMP		(dk->dk_amp)
#define DK_WKAMP_P	(dk->dk_wkamp_p)
#define DK_AST_P	(dk->dk_ast_p)
...
/*
 * Conversion macros for converting hz to/from microsecs.
 * Not using the DDI function hztousec() and usectohz() because
 * the extensive overflow checking eventually causes only 0x7fffffff
 * to be returned and we aren't concerned with overflow for the
 * performance data.
 * The integer macros assume HZ (defined in param.h) is <= 1000
 * The integer version of MSEC2HZ(x) has trouble rolling over
 * when x is in the range [(unsigned long)(0-(HZ-1)), 1000/HZ];
 * but since that is not a problem in our usage, we won't do
 * the arithmetic handstands necessary to solve the problem.
 */
#define MSPERS	1000		/* millsecs per second */

#if (MSPERS % HZ == 0)
/* Integer versions for HZ a multiple of MSPERS */
#define HZ2MSEC(x)	(unsigned long)(x) * (unsigned long)(MSPERS / HZ)
#define MSEC2HZ(x)	((unsigned long)(x) * (unsigned long)HZ)	  \
                                / (unsigned long)MSPERS
#else
/* Double versions */
#define HZ2MSEC(x)	(unsigned long)((double)(x)			\
                                * ((double)MSPERS / (double)HZ))
#define MSEC2HZ(x)	(unsigned long)((double)(x)			\
                                * ((double)HZ / (double)MSPERS))
#endif
...
#define sd01_blk2sec(BLK,DKP,PART) \
	(((BLK) >> BLKSEC(DKP)) + ((DKP)->dk_vtoc.v_part[(PART)].p_start))
#define sd01_sec2blk(SEC,DKP,PART) \
	(((SEC)< ((DKP)->dk_vtoc.v_part[(PART)].p_start)) ? \
	((SEC) << BLKSEC(DKP)) : \
	((((SEC) - ((DKP)->dk_vtoc.v_part[(PART)].p_start))) << BLKSZ(DKP)))

struct sd01dyn_bbh {
	uint	db_flag;
	struct  job *db_badjob;
	struct  job *db_fq;
	struct	disk *db_waitdk;
	int	db_fill[4];
};

#define	DBBH_BUSY	0x1
#define DBBH_ARM	0x2
#define DBBH_ARMING	0x4
#define DBBH_SEND	0x8

#endif /* _IO_TARGET_SD01_H */

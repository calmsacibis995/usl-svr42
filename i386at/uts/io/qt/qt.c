...
#ident	"@(#)uts-x86at:io/qt/qt.c	1.8"
#ident	"$Header: $"
...
#include <io/qt/qt.h>
#include <io/qt/qt_globals.h>
#include <io/qt/tape.h>
...
qtinit ()
{
	...
	zeroq();
	qt_init_drive();	/* ul90-34517 fix * see if we can identify the tape
			 * ul90-34517 fix * device plugged in right now 
			 */
	}               /* ul90-34517 fix */

qt_init_drive()        /* ul90-34517 fix */
{
	...
	sel_wangtek();		/* Try Wangtek PC-36/EV-811 first	*/
	if (qt_check ()) 
		cmn_err(CE_NOTE, "!Wangtek PC-36/EV-811 cartridge tape controller was found at address %xH.\n", qt_0_sioa);
	else {
		sel_archive();	/* No PC-36 or equivalent, try Archive SC499R*/
		if (qt_check ()) {
			msoutb(RSTDMA,01);
			cmn_err(CE_NOTE, "!Archive SC499-R/VP402 cartridge tape controller was found at address %xH.\n", qt_0_sioa);
		}
		else {
			board_type = BT_UNKNOWN;
			cmn_err(CE_WARN, "Cartridge controller was not found at address %xH.\n\t Tape driver disabled.\n", qt_0_sioa);
		}
	}
	RETURN(1, board_type);
}
...
qt_timeout ()
{
	...
	if (isset(F_PROGRESS)) {
		...
		if (kill_it) {	/* Abort the current operation */
			if (topq() == A_RDDATA || topq() == A_WRDATA || topq() == A_FLUSH || topq() == A_SBF)
				msoutb (DMA_STR, 05);
...
int	otyp;
{
	...
	if (board_type == BT_UNKNOWN) {
		/* ul90-34517 fix * try again if no board type was
		 * known at init time
		 */
		if (qt_init_drive() == BT_UNKNOWN) { /* ul90-34517 fix */
			u.u_error = ENXIO;
			return;
		}                   /* ul90-34517 fix */
	}
...
register  short	dev;
{
	...
	if (isset(F_LOPW)) {
		...
		/* wangtek controllers don't respond to the rewind request
		 * when the last command was a read (ul90-31212 (PR 2135))
		 */
		if (board_type == BT_WANGTEK) {
			if (isset(F_LCMDR))
				deassert(CTL_ONLINE);
...
{ 
	...
	case T_RWD:
		...
		break;
	case T_WRFILEM:
		/* check to ensure not reading dm90-306775 -  PR 1887 */
		if (isset(F_READ)) {
			iqt->error = EIO;
			setflags(F_ERROCC);
			goto done;
	...
	case T_SFF:
	case T_SBF:
		/* Fix for (dm90-30675) (PR1887).  Old code called seq_check
		 * which returned without clearing F_EXCLUSIVE.
		 */
		if (isset(F_LOPW)) {
			iqt->error = EIO;
			setflags(F_ERROCC);
...
register short	dev;
{
	...
	if (debug_flag & DB_STRUCT)
		prtstr ();
#endif
	/* Fix for dm90-31981 (PR2034) -- seq_check macro must be executed before
	 * excl_check, because excl_check sets F_EXCLUSIVE, which is not true
	 * when seq_check detects an error and returns.
	 */
	seq_check();
	excl_check();
	qt_inprog (F_EXCLUSIVE);
...
readfm_proc ()
{
	...
	case A_RDDATA:
		if (isset(F_DMAWAIT)) {
			/* b_resid can never be bigger than b_bcount -- dm90-30576 PR1990 */
			if (iqt->b->b_bcount < iqt->io_count + BLOCKSIZE )
				iqt->b->b_resid = iqt->b->b_bcount;
			else
...
qt_rddata ()
{
	...
	if (iqt->io_count < BLOCKSIZE) {
		...
		clrflags(F_WRMODE);
		iqt->vbufferp = iqt->vbuffer;
		RETURN(19, (qt_setdma (iqt->pbuffer, DMA_READ)));
	}
	if (qt_setdma (iqt->pio_addr, DMA_READ)) {
...
qt_wrdata ()
{
	...
	if (isset(F_ACTIVE) && isset(F_WRMODE)) {
		...
		RETURN(20, (qt_setdma (iqt->pbuffer, DMA_WRITE)));
	}
	if (qt_setdma (iqt->pio_addr, DMA_WRITE)) {
		iqt->io_count -= BLOCKSIZE;
		iqt->pio_addr += BLOCKSIZE;
		iqt->vio_addr += BLOCKSIZE;
		RETURN(20, (SUCCESS));
...
qt_flush ()
{
	...
	iqt->flags &= ~F_ACTIVE;
	RETURN(21, (qt_sndcmd (C_WRDATA) && qt_setdma (iqt->pbuffer, DMA_WRITE)));
}
...
qt_sbf ()
{
	...
	if (iqt->sfarg--) {
		if (isset(F_ISSRDWR) && !qt_sndcmd (C_RDDATA))
			RETURN(24, (FAIL));
		RETURN(24, (qt_setdma (iqt->pbuffer, DMA_READ)));
	} else {
		iqt->sfarg = 0;
		clrflags(F_PROGRESS);
	}
 ...
 * transfer from/to the floppy disk controller.
 */
qt_ldma(addr, rw)
long	addr;
short	rw;
{
	...
	asm("cli");
	outb(DMA1CBPFF, 0); /* set the first/last F/F */
	outb(DMA1WMR, rw); /* output R/W mode byte */
	outb(DMA1BCA1, addr ); /* set low byte of address */
	outb(DMA1BCA1, (addr >> 8)); /* set 2nd byte of address */
	outb(DMACH1PG, (addr >> 16)); /* set page register */
	outb(DMA1BCWC1, (512-1)); /* set low byte of count */
	outb(DMA1BCWC1, ((512-1) >> 8)); /* set high byte of count */
	asm("sti");
}

sel_wangtek ()                                                  
{									
	board_type	= BT_WANGTEK;

	/* Set constant addresses and flags */				
	DATAPORT = CMDPORT = (CTLPORT = STATPORT = qt_0_sioa) + 1;	
...
sel_archive () 							
{									
	board_type	= BT_ARCHIVE;

	/* Set constant addresses and flags */				
	CTLPORT = STATPORT = (CMDPORT = DATAPORT = qt_0_sioa) + 1;	
	RSTDMA = (DMAGO = qt_0_sioa + 2) + 1;				

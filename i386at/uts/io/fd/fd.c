...
#ident	"@(#)uts-x86at:io/fd/fd.c	1.16"
#ident	"$Header: $"
...
#ifdef MERGE386
	if( mrg_in_use( 0 )) {
		/* Do not unload the floppy driver if merge386 is running */
		return(-1);
	}
#endif /* MERGE386 */
...
#endif /* MERGE386 */
	if(drv_getparm(UPROCP, (ulong*)&fdproc_t) == -1 )
	{
		f->fd_status &= ~OPENING;
		wakeup(&f->fd_status);
		return(EINVAL);
...
#endif
	f = &fd[UNIT(dev)];
	return(physiock((void (*)())fdbreakup, NULL, dev, B_READ, (daddr_t)(f->fd_n512b),uio_p));
}
...
#endif
	f = &fd[UNIT(dev)];
	return(physiock((void (*)())fdbreakup, NULL, dev, B_WRITE, (daddr_t)(f->fd_n512b),uio_p));
}
...
#endif
	if(drv_getparm(LBOLT, (ulong*)&lbolt_val) == -1)
	{
		return(EINVAL);
	}
	bp->av_forw = NULL;
...
register struct buf *bp;
{
	...
	if (fd_mtimer[n] == 0) { /* start floppy motor first */
		...
		timeout((void (*)())fdwait, (caddr_t)bp, f->fd_mst);
		if (fdt_running == 0)
			timeout((void (*)())fdtimer, (caddr_t)0, MTIME);
		retval = 0;
		goto done;
	}
...
seek:
	...
	case SEEK_DONE: /* come here for SEEK completion */
		...
		fdcst.fd_state = XFER_BEG;
		timeout((void (*)())fdwait, (caddr_t)bp, f->fd_hst);
		retval = 0;
		goto done;
	}
	retval = 1;
	...
	if (++fdtab.b_errcnt > f->fd_maxerr) {
		if (type == 1) {
			...
			fdreset();
                        ...
                        if (type > ctrmax + 2)
                        cmn_err(CE_WARN,"FD controller %s",fd_ctrerr[ctrmax]);
                        else
...
register struct fdstate *f;
{
	...
	if (--fdcst.fd_etimer == 0) {
		...
		timeout((void (*)())fdnoflop, (caddr_t)f, ETIMOUT);
}
...
register struct buf *bp;
{
	...
	if (fd_mtimer[n] == 0) {
		...
		fd_curmotors |= (ENABMOTOR << n);
		outb(FDCTRL, fd_curmotors | enabint | NORESET | n);
		timeout((void(*)())fdrawio, (caddr_t)bp, fd[n].fd_mst);	/* wait for motor */
		if (fdt_running == 0)
			timeout((void(*)())fdtimer, (caddr_t)0, MTIME);
		return(0);
	}
...
register struct buf *bp;
{
	...
	if (fd_mtimer[n] == 0) {
		...
		fd_curmotors |= (ENABMOTOR << n);
		outb(FDCTRL, fd_curmotors | enabint | NORESET | n);
		timeout((void (*)())fdmtnio, (caddr_t)bp, fd[n].fd_mst);	/* wait for motor */
		if (fdt_running == 0)
			timeout((void (*)())fdtimer, (caddr_t)0, MTIME);
		return;
	}

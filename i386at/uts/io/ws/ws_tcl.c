...
#ident	"@(#)uts-x86at:io/ws/ws_tcl.c	1.4"
#ident	"$Header: $"
...
/* Enhanced Application Compatibility Support */
	...
	case TCL_SWITCH_VT:
		{
		   channel_t *newchp;
		   int	newvt = protop->chp_stype_arg;
		   ...
		   if(newvt >= 0)
		      (void)ws_activate(wsp,ws_getchan(wsp,newvt), VT_NOFORCE);
		}
		break;
	...
	case TCL_PRINT_FONTCHAR:
  		if (chp->ch_dmode != KD_GRAPHICS)
		{
			...
			tsp->t_pnum = 1;
			tcl_sfont(wsp, chp, tsp);
			tcl_norm(wsp, chp, tsp, protop->chp_stype_arg);
	...
	case TCL_SET_OVERSCAN_COLOR:
		(void ) (*wsp->w_setborder)(chp, protop->chp_stype_arg);
		break;
	...
	case TCL_NOBACKBRITE:
		...
		else	tsp->t_flags &= ~T_NOBACKBRITE;
		break;
...
termstate_t	*tsp;
{
	register int	tabindex = tsp->t_ntabs;
	register ushort	tabval;
...
register termstate_t	*tsp;
{
	...
	for (cnt1 = 0; cnt1 < tsp->t_ntabs; cnt1++)
		if (tsp->t_tabsp[cnt1] >= tsp->t_col)
			break;
	if(cnt1 < tsp->t_ntabs && tsp->t_tabsp[cnt1] == tsp->t_col)

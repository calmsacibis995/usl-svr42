...
#ident	"@(#)uts-x86at:io/ws/ws_subr.c	1.11"
#ident	"$Header: $"
...
int
ws_alloc_attrs(wsp, chp, km_flag)
wstation_t	*wsp;
channel_t	*chp;
int	km_flag;
{
...
int	chan;
{
	...
	bcopy (&kb_attrmask[0],tsp->t_attrmskp,sizeof(struct attrmask)*nattrmsks);
...
int	force;
{
	...
	if ((*wsp->w_activate)(chp, force)) {
		...
		drv_getparm(LBOLT, (ulong*)&protop->chp_tstmp);
...
int	 cmd, arg;
{
	...
	switch(cmd)
	{
		...
		case LDEV_ATTACHQ:
			...
			error =  event_check_que(xqp, arg, chp, cmd);
			if (error || !ws_notify(chp, XQENAB)) {
				chp->ch_flags &= ~CHN_QRSV;
				wakeup(&chp->ch_flags);
	...
	struct cred *cr;
{
	...
	if (cdevsw[getmajor(dev)].d_str) {
		...
		if (vp == (vnode_t *) NULL)
			return (EINVAL);
		error = stropen(vp, &dev, flag, cr);
		VN_RELE(vp); /* lower reference count */
	} else error =  (*cdevsw[getmajor(dev)].d_open) (&dev, flag, otyp, cr);
        ...
        struct cred *cr;
{
	...
	if (cdevsw[getmajor(dev)].d_str) {
		...
		VN_RELE(vp); /* lower reference count */
	} else error =  (*cdevsw[getmajor(dev)].d_read) (dev, uiop, cr);
        ...
        struct cred *cr;
{
	...
	if (cdevsw[getmajor(dev)].d_str) {
		...
		VN_RELE(vp); /* lower reference count */
	} else error =  (*cdevsw[getmajor(dev)].d_write) (dev, uiop, cr);
...
dev_t dev;
{
	void ws_sysv_clrcompatflgs();

	ws_compatflgs[getminor(dev)/8] |= 1 << (getminor(dev) % 8);
	ws_sysv_clrcompatflgs(dev, 3);
	ws_sysv_clrcompatflgs(dev, 4);
}
...
/* The following routines support COFF/ELF-based USL applications that
 * use KD driver ioctls that overlap with STREAMS ioctls.
 */

extern unchar ws_svr3_compatflgs[];
extern unchar ws_svr4_compatflgs[];
...
{
	if(arg == 3)
		ws_svr3_compatflgs[getminor(dev)/8] |= 1 << (getminor(dev) % 8);
	else	ws_svr4_compatflgs[getminor(dev)/8] |= 1 << (getminor(dev) % 8);
...
int   arg;
{
	if(arg == 3)
 	    ...
 	    ws_svr4_compatflgs[getminor(dev)/8] &= ~(1 << (getminor(dev) % 8));
}

int
ws_sysv_compatset(dev, arg)
dev_t dev;
int   arg;
{
  if(arg == 3)
      return (ws_svr3_compatflgs[getminor(dev)/8] & (1 << (getminor(dev) % 8)));

  return (ws_svr4_compatflgs[getminor(dev)/8] & (1 << (getminor(dev) % 8)));
}

extern int maxminor;

void
ws_sysv_initcompatflgs(dev)
dev_t dev;
{
	bzero(&ws_svr3_compatflgs[0],maxminor/8 + 1);
	bzero(&ws_svr4_compatflgs[0],maxminor/8 + 1);
}

extern struct kdvdc_proc kdvdc_vt[];

void
ws_set_vt_proc_info(index)
int	index;
{
	kdvdc_vt[index].kdvdc_pid = u.u_procp->p_opid;
	kdvdc_vt[index].kdvdc_procp = (char *)u.u_procp;
}
...
dev_t dev;
{
	...
	if((p = (proc_t *)kdvdc_vt[getminor(dev)].kdvdc_procp) != NULL) {
		if(p->p_stat == SZOMB) {
			kdvdc_vt[getminor(dev)].kdvdc_pid = -1;
			kdvdc_vt[getminor(dev)].kdvdc_procp = (char *) NULL;
			return(0);
		}	
		if(kdvdc_vt[getminor(dev)].kdvdc_pid == u.u_procp->p_opid &&
		   (proc_t *)kdvdc_vt[getminor(dev)].kdvdc_procp == u.u_procp )
			return(1);
	}
	return(0);
}
...
void
ws_initvdc_compatflgs(dev)
dev_t dev;
{
	bzero(&kdvdc_vt[0],(maxminor + 1)*sizeof(struct kdvdc_proc));
}

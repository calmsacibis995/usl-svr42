...
#ident	"@(#)uts-x86at:io/ws/ws_tables.c	1.5"
#ident	"$Header: $"
...
/* 79*/ {{A_DEL, A_DEL, A_DEL, A_DEL, A_DEL, A_DEL, K_RBT, A_DEL },0x02, L_O },
...
xlate_keymap(src_kmap, dst_kmap, format)
keymap_t *src_kmap, *dst_kmap;
int	format;
{

	register keyinfo_t *src_key, *dst_key;
	register int	i, usl_index, sco_index;

	src_key = src_kmap->key;
	dst_key = dst_kmap->key;

	for(i=0; i<USL_TABLE_SIZE; i++)
   	     bcopy(&src_key[i], &dst_key[i], sizeof(keyinfo_t));

	switch(format)
	{
		case  USL_FORMAT:
			dst_kmap->n_keys = USL_TABLE_SIZE;
			...
			for(i=0; usl_sco_extkeys[i].escape_code >= 0; i++) {
			  ...
			  if(usl_index >= 0 && sco_index >= 0 )
	   		    bcopy(&src_key[sco_index], &dst_key[usl_index], 
							sizeof(keyinfo_t));
			}
			break;
		case  SCO_FORMAT:
			dst_kmap->n_keys = SCO_TABLE_SIZE;

			for(i=USL_TABLE_SIZE; i<SCO_TABLE_SIZE; i++)
	   			bcopy(&nop_key, &dst_key[i], sizeof(keyinfo_t));

...
#ident	"@(#)oamintf:i386at/cmd/oamintf/machinemgmt/prtconf.c	1.6"
...
/* check for devices in edt - no error msg if we fail */
	if ((fd_edt = open("/etc/scsi/pdi_edt", O_RDONLY)) != -1) {
		...
		while (read(fd_edt, &sedt, sizeof(struct scsi_edt)) != NULL)
			if (sedt.c_maj){
				/* skip redundant device info, if present */
				for (sp = (char *) sedt.tc_inquiry; *sp; sp++)
					if (*sp == ' ')
						break;
				if (! *sp) /* no spaces found */
					sp = (char *) sedt.tc_inquiry;
				else
					for ( ; *sp == ' '; sp++)
						; /* doesn't skip all spaces */

...
#ident	"@(#)scsi-cmds:i386at/cmd/scsi-cmds/DISK.c	1.1"
#ident  "$Header: $"
...
char	**argv;
{
	...
	bsfd = open(BADSECFILE,O_WRONLY|O_CREAT|O_TRUNC);	
	if (bsfd==-1)
		error("Opening of the badsec file failed\n");

	for (blc_p=badsl_chain; blc_p; blc_p=blc_p->bl_nxt) {
		if (write(bsfd,blc_p,BADSLSZ) == -1)
			error("Writing to the badsec file failed\n");
	}
...
#ifdef DK_DEBUG
	bsfd = open(BADSECFILE,O_RDONLY);
	if (bsfd==-1)
		error("Opening of the badsec file for read failed\n");

	printf("main: found badsec after disk format\n");
	blc_p = (struct badsec_lst *)malloc(BADSLSZ);
	status=read(bsfd,blc_p,BADSLSZ);
	while (status== BADSLSZ) {
		for (i=0; i<blc_p->bl_cnt; i++)
			printf(" badsec=0x%x ", blc_p->bl_sec[i]);

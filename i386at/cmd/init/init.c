...
#ident	"@(#)init:i386at/cmd/init/init.c	1.32.10.14"
#ident  "$Header: init.c 1.2 91/07/24 $"
...
debug("%d:single():waitproc() returned (CASE: NOT BY MY CHILD):RETURN %s\n",getpid(), istate);
								...
								console("No TP on console remaining in SINGLE USER\n");
							}
						}else{
							kill(su_process->p_pid,SIGKILL);
							prev_state = cur_state;
...
char				*shcmd;
{
	...
	if(!itabopened) {
		...
		if ((fptmp_inittab = fopen(inittabp, "r")) != NULL) {
			...
			console("Warning: Cannot open %s errno: %d\n",
			 inittabp, errno);
			console("Using %s that was previously opened.\n",
			 previnittabp);
			console("It may be different or out of date.\n");
			itabopened = 1;
		}
	}
...
debug("%d:initialize():ENTER\n",getpid());
	...
	if (initprivs() == REBOOT) {
		console("Cannot access %s, returning to firmware mode.\r\n", PDF);
		firmware();	/* No Return */
...
failed to execute.  errno = %d (exec of shell failed)\n", cmd.c_command, errno);
	...
	if ((ret == FAILURE) ||
	 (tpmajornum != (major_t)NODEV) && (tponconsole != TRUE)){
		console("Remaining at run level %c\n",level(SINGLE_USER));
...
realcon386()
{
	...
	if (consolebuf.st_rdev == consdevdsfbuf.st_rdev){
		...
		if (flags.rcef && flags.dynamic_rcmf){
			/* Force correct baud rate -- ioctl.syscon could
			 * contain the settings from another port.
			 */
			;
		}
	}else{
...
initprivs()
{
	...
	/* If the "exec" fails print an error message. */
			console("WARNING: Could not initialize privilege data.\r\n");
			exit(1);
		} else while (waitproc(process) == FAILURE);
	#ifdef	ACCTDEBUG
		debug("SYSINIT- id: %.4s term: %o exit: %o\n",
	...
	if ((pid=fork()) != FAILURE) {
		if (pid==0) {
			...
			if ((auditctl(ASTATUS, &actl, sizeof(actl_t)))==FAILURE) {
		                console("UX:auditctl: ERROR: unable to get status of auditing\n");
				exit(1);
			}
				...
				if (((alog.ppathp=(char *)
				  ...
				  ||((alog.progp=(char *)
					calloc(MAXPATHLEN,sizeof(char)))==NULL)) {
					console("UX:adt_runprog: ERROR: unable to malloc() space.\n");
					exit(ADT_MALLOC);
				}
	                	if ((auditlog(ALOGGET,&alog,sizeof(alog_t)))==FAILURE) {
	                        	console("UX:auditlog: ERROR: failed to get pathname to AUDIT switch program.\n");
					exit(1);
				}
							...
							if ((alog.defpgmp=(char *)
								...
								sizeof(char)))==NULL) {
				    					...
				    					exit(ADT_MALLOC);
                    				    	...
                    				    	strncpy(alog.defpgmp,dfltpgmp,
								MAXPATHLEN);
				    			alog.onfull |= APROG;
		            				if ((auditlog(ALOGSET,&alog,sizeof(alog_t)))==FAILURE) {
								console("UX:auditlog: ERROR: ERROR: failed ALOGSET.\n");
								exit(1);
				    			}
				   		}
					}
				...
				if (execl(savedprogp, savedprogp, (char *)NULL) == FAILURE)  {
					if (errno!=ENOEXEC)
						console("UX:init: ERROR: execl of \"%s\" failed; errno = %d\n",savedprogp,errno);
					else
		...
		console("UX:adt_runprog: ERROR: fork failed; errno = %d\n",errno);
}
...
errno = saveerrno;
#endif
			console("Cannot set security attributes on %s errno == %d\n", dsfpath, errno);
			return (FAILURE);
		}
...
firmware()
{
	...
	if (pid == 0){
		console("Reboot from Assurance Media\n\n");
	...
	if ((tpmajornum != (major_t)NODEV) && (tponconsole == TRUE)){
		if ((ctrlfd = opentpdev(SYSCONREAL, 0, &tpinf, errbuf,
		 ERRBUFSZ)) == -1){
			...
			reset_syscon();
			console("%s", errbuf);
			console("%s not accessible, resetting device to default\n",
			 sttyname(&statbuf));
			/*
...
errno = saveerrno;
#endif
			console("Failed to re-open data channel during setting up a Trusted Path\n\t%s\n",
			 strerror(errno)); 
			(void)tp_datadisconnect(ctrlfd, &tpinf);
			(void)close(ctrlfd);
			(void)close(tmpfd);
...
errno = saveerrno;
#endif
			console("Allocating %s during setting up a Trusted Path Failed\n",
			 datadsfname);
			(void)tp_datadisconnect(ctrlfd, &tpinf);
			(void)close(ctrlfd);
			(void)close(datafd);
...
errno = saveerrno;
#endif
			console("Pushing STREAMS module(s) %s failed:%s\n",
...
debug("%d:set_autopush():ENTER\n",getpid());
	...
	if (tpmajornum != (major_t)NODEV){
		...
		if (ioctl(sadfd, SAD_SAP, &push) < 0) {
			retcode = errno;
			console("Could not configure autopush for line %s:%s\n",
			 devtab[DTTAG_TP_CONSDEV].dt_dsfname, strerror(errno));
			switch (retcode) {
	
			case EINVAL:
				console("\tInvalid major device number or invalid module name or too many modules\n");
				break;

			case ENOSTR:
				console("Major device is not a STREAMS driver\n");
				break;

			case EEXIST:
				console("Major/minor already configured\n");
				break;

			case ENOSR:
				console("Ran out of autopush structures");
				break;

			case ERANGE:
				console("lastminor must be greater than minor\n");
				break;

...
#ident	"@(#)disksetup:i386at/cmd/disksetup/ix_altsctr.c	1.6"
...
read_alts()
{
	...
	if ((status =absdsk_io(*alts_fd, alts_sec, (char *)&alttbl,
	    ...
	    perror("Unable to read alternate table: ");
	    exit(90);
	}
...
assign_alts()
{
	...
	if (alttbl.alt_sec.alt_reserved == 0) {
		fprintf(stdout, "Warning: No alternates partition in VTOC \n");
		fprintf(stdout, "         Bad sectors will not be marked!\n");
		return;
	}
...
/* 		check for used alternate is bad				*/
		for (i=0; i < (int)alttbl.alt_sec.alt_used; i++) {
			if (alttbl.alt_sec.alt_base + i == curbad) {
				fprintf(stderr,"Bad sector %ld is an assigned alternate!\n",
				    curbad);
				growbadp[cnt].bad_start = ALTS_ENT_EMPTY;
				break;
			}
/* 			check if bad block already mapped (already in list) */
	...
	for (growbadp=ap->ap_gbadp, cnt=0; cnt<ap->ap_gbadcnt; cnt++) {
		...
		if (alttbl.alt_sec.alt_used >= alttbl.alt_sec.alt_reserved) {
			fprintf(stderr, "Insufficient alternates available for bad sector %ld!\n",
				    curbad);
			break;
		}
...
wr_alts()
{
	...
	if ((status =absdsk_io(*alts_fd, alts_sec, (char *)&alttbl,
	    ...
	    perror("Unable to write alternate table: ");
	    exit(90);
	}
}
...
get_unix_srtsec()
{
	...
	if (ioctl(*alts_fd, V_RDABS, &alts_absio) < 0) {
		perror("Unable to read fdisk table: ");
		exit(70);
	}
	...
	if (unix_part == NULL) {
		fprintf(stderr, "No active UNIX System partition in partition table!\n");
		exit(71);
	}

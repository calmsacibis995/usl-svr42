...
#ident	"@(#)disksetup:i386at/cmd/disksetup/scl.c	1.7"
...
printf("send_scb: op(VERIFY) badsec match= 0x%x return SC_IDERR\n", badsec[i]);
		...
		if (smx_op == SS_REASGN) {
			...
			smx_len = 0;
printf("send_scb: op(SS_REASGN) addr= 0x%x \n", smx_addr);
			for (i=0; i<badsec_cnt; i++) {
				if ((badsec[i] >= smx_addr) && 
			    	    (badsec[i] <= (smx_addr + smx_len))) {
printf("send_scb: badsec reassign match= 0x%x return SC_NODFCT\n", badsec[i]);
	...
	case (SDI_CKSTAT & MASK_FF) :	/* Target returned check status     */
		...
		case S_CKCON :	/* Error executing command */
			...
			case SD_NOSENSE :
				break;
			case SD_VENUNI :
				(void)(*errfunc)("Vendor Unique (0x%X)\n", sense_data->sd_sencode);
				break;
			case SD_RESERV :
				(void)(*errfunc)("Reserved (0x%X)\n", sense_data->sd_sencode);
				break;
			default :
				...
				case ID_RANDOM :
					...
					case SC_NOSENSE :
						break;
					case SC_NOSGNL :
						(void)(*errfunc)("No Index/Sector Signal\n");
						break;
					case SC_NOSEEK :
						(void)(*errfunc)("No Seek Complete\n");
						break;
					case SC_WRFLT :	
						(void)(*errfunc)("Write Fault\n");
						break;
					case SC_DRVNTRDY :
						(void)(*errfunc)("Drive Not Ready\n");
						break;
					case SC_DRVNTSEL :
						(void)(*errfunc)("Drive Not Selected\n");
						break;
					case SC_NOTRKZERO :
						(void)(*errfunc)("No Track Zero found\n");
						break;
					case SC_MULTDRV :
						(void)(*errfunc)("Multiple Drives Selected\n");
						break;
					case SC_LUCOMM :
						(void)(*errfunc)("Logical Unit Communication Failure\n");
						break;
					case SC_TRACKERR :
						(void)(*errfunc)("Track Following error\n");
						break;
					case SC_IDERR :	
					case SC_UNRECOVRRD :
					case SC_NOADDRID :
					...
					case SC_RESET :	
						return(sense_data->sd_sencode);
					case SC_SEEKERR :
						(void)(*errfunc)("Seek Positioning error\n");
						break;
					case SC_DFCTLSTERR :
						(void)(*errfunc)("Defect List error\n");
						break;
					case SC_PARAMOVER :
						(void)(*errfunc)("Paramater Overrun\n");
						break;
					case SC_SYNCTRAN :
						(void)(*errfunc)("Synchronous Transfer error\n");
						break;
					case SC_NODFCTLST :
						(void)(*errfunc)("Primary Defect List not found\n");
						break;
					case SC_INVOPCODE :
						(void)(*errfunc)("Invalid Command Operation Code\n");
						break;
					case SC_ILLBLCK :
						(void)(*errfunc)("Illegal Logical Block Address.\nAddress greater than the LBA returned by the READ CAPACITY data with PMI not set.\n");
						break;
					case SC_ILLFUNC :
						(void)(*errfunc)("Illegal function for device type\n");
						break;
					case SC_ILLCDB :
						(void)(*errfunc)("Illegal Field in CDB\n");
						break;
					case SC_INVLUN :
						(void)(*errfunc)("Invalid LUN\n");
						break;
					case SC_INVPARAM :
						(void)(*errfunc)("Invalid field in Parameter List\n");
						break;
					case SC_WRPROT :
						(void)(*errfunc)("Write Protected\n");
						break;
					case SC_MDSELCHNG :
						(void)(*errfunc)("Mode Select Parameters changed.\n");
						break;
					case SC_INCOMP :
						(void)(*errfunc)("Incompatible Cartridge\n");
						break;
					case SC_FMTFAIL :
						(void)(*errfunc)("Medium Format Corrupted\n");
						break;
					case SC_NODFCT :
						return(sense_data->sd_sencode);

					case SC_RAMFAIL :
						(void)(*errfunc)("RAM Failure\n");
						break;
					case SC_DATADIAG :
						(void)(*errfunc)("Data Path Diagnostic Failure\n");
						break;
					case SC_POWFAIL :
						(void)(*errfunc)("Power On Diagnostic Failure\n");
						break;
					case SC_MSGREJCT :
						(void)(*errfunc)("Message Reject Error\n");
						break;
					case SC_CONTRERR :
						(void)(*errfunc)("Internal Controller Error\n");
						break;
					case SC_SELFAIL :
						(void)(*errfunc)("Select/Reselect Failed\n");
						break;
					case SC_SOFTRESET :
						(void)(*errfunc)("Unsuccessfule Soft Reset\n");
						break;
					case SC_PARITY :
						(void)(*errfunc)("SCSI Interface Parity Error\n");
						break;
					case SC_INITERR :
						(void)(*errfunc)("Initiator Detected Error\n");
						break;
					case SC_ILLMSG :
						(void)(*errfunc)("Inappropriate/Illegal Message\n");
						break;
					default :
						(void)(*errfunc)("Unknown sense code (0x%X)\n", sense_data->sd_sencode);
						break;
					}
					break;
				default :
					(void)(*errfunc)("Unknown device type (0x%X)\n", Inquiry_data.id_type);
					break;
				}
				break;
			}
		...
		case S_BUSY :		/* Controller busy	   */
			(void)(*errfunc)("Controller busy\n");
			break;
		case S_RESER :		/* LUN Reserved		   */
			(void)(*errfunc)("LUN Reserved\n");
			break;
		default :
			(void)(*errfunc)("Unknown status (0x%X)\n", scb->SCB.sc_status);
			break;
		}
		break;
			...
			case SC_NORECORD :
				if (sense_data.sd_valid) {
					...
					if (!no_map) {
						...
						scsi_write((long) block[1], bufpt, 512);
						free(bufpt);
						verify_len -= (block[1] - verify_start + 1);
						verify_start = block[1] + 1;
					}
					else {
...
struct ident	*inquiry_data;
{
	...
	if ( inquiry_data->id_len < (unsigned char)(IDENT_SZ - 5))
...
long		rabl_bufsz;
{
	...
	if (!badsl_chain) {
		...
		blc_p = badsl_chain;
		if (!blc_p) 
			error("scsi_nodfct: bad sector chain list broken.\n");

...
#ident	"@(#)ucb:common/ucblib/libc/port/gen/double_decim.c	1.1"
#ident	"$Header: $"
...

{
	...
	if (e <= 0)
		...
		int	ipd = (int)pd;	/* kludge: some compiler does not allow
				 	 * pd in switch */
...
{
	_big_float     *pb, b, d;
	int		ipb = (int)pb;		/* kludge: some compiler does no
						 * allow 'pb' to be used in switch */

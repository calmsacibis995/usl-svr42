#ident	"@(#)ucb:common/ucblib/librpcsoc/svc_tcp.c	1.2"
#ident	"$Header: $"
...
static char sccsid[] = "@(#)svc_tcp.c 1.25 88/06/11 Copyr 1984 Sun Micro";
#endif
	...
	u_int recvsize;
{
	...
	if (sock == RPC_ANYSOCK) {
		if ((sock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0) {
			(void) syslog(LOG_ERR, "svctcp_create - tcp socket creation problem: %m");
			return ((SVCXPRT *)NULL);
		}
		madesock = TRUE;
	}
	...
	if ((getsockname(sock, (struct sockaddr *)&addr, &len) != 0)  ||
	    (listen(sock, 2) != 0)) {
		(void) syslog(LOG_ERR, "svctcp_create - cannot getsockname or listen: %m");
		if (madesock)
		       (void)close(sock);
		return ((SVCXPRT *)NULL);
	}
	...
	register int len;
{
	...
	do {
		readfds = mask;
		if (select(_rpc_dtbsize(), &readfds, (int*)NULL, (int*)NULL, 
			   &wait_per_try) <= 0) {
			if (errno == EINTR) {
				continue;
			}

#ident	"@(#)ucb:common/ucblib/librpcsoc/svc_udp.c	1.2"
#ident	"$Header: $"
...
static char sccsid[] = "@(#)svc_udp.c 1.26 88/06/11 Copyr 1984 Sun Micro";
#endif
	...
	u_int sendsz, recvsz;
{
	...
	if (sock == RPC_ANYSOCK) {
		if ((sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0) {
			(void) syslog(LOG_ERR, "svcudp_create: socket creation problem: %m");
			return ((SVCXPRT *)NULL);
		}
		madesock = TRUE;
	}
	...
	if (getsockname(sock, (struct sockaddr *)&addr, &len) != 0) {
		(void) syslog(LOG_ERR, "svcudp_create - cannot getsockname: %m");
		if (madesock)
			(void)close(sock);
		return ((SVCXPRT *)NULL);
	}
...
#	define EQADDR(a1, a2)	(memcmp((char*)&a1, (char*)&a2, sizeof(a1)) == 0)
	...
	memcpy((char *)&uc->uc_addr, (char *)&xprt->xp_raddr, sizeof(struct sockaddr_in));
	return(0);
}

...
#ident	"@(#)ucb:common/ucbcmd/stty/sttyparse.c	1.2"
#ident	"$Header: $"
...
struct	winsize	*winsize;
{
	...
	while(--argc > 0) {
		...
		if ((term & ASYNC) || term == -1) {
			...
			if (eq("ek")) {
			...
			else if (eq("line") && (!(term & TERMIOS) || term == -1)) {
				if (--argc) {
			...
			else if((term & TERMIOS) && eq("ospeed")) {
				if (--argc) { 
					s_arg = *++argv;
					match = 0;
				...
				else arg_error("ospeed");
			}
			else if((term & TERMIOS) && eq("ispeed")) {
				if (--argc) { 
					s_arg = *++argv;
					match = 0;
					for(i=0; speeds[i].string; i++)
		...
		if((term & WINDOW) || term == -1) {
			...
			else if((eq("columns") || eq("cols"))) {
				if (--argc)
					winsize->ws_col = atoi(*++argv);
				else if (eq("cols")) arg_error("cols");
					else arg_error("columns");
			}
			else if(eq("xpixels")) {
				if (--argc)
					winsize->ws_xpixel = atoi(*++argv);
				else arg_error("xpixels");
			}
			else if(eq("ypixels")) {
				if (--argc)
					winsize->ws_ypixel = atoi(*++argv);
				else arg_error("ypixels");
			}
		}
		if(!match)
			if(!encode(cb, term)) {
...
/* if an argument is not provided to an option that needs an argument */
/* then print an error message and exit with 1.			      */
arg_error(no_arg_to)
const char *no_arg_to;
{
	fprintf(stderr, "stty: No argument for \"%s\"\n", no_arg_to);
	exit(1);
}

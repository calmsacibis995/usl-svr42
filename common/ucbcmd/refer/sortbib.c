...
#ident	"@(#)ucb:common/ucbcmd/refer/sortbib.c	1.3"
#ident	"$Header: $"
/*
...
char *validkeys = "ABCDEFGHIJKLMNOPQRSTVXYZ";	/* list of valid sort keys */
...
char keystr[];
{
	...
	for (i = 0, j = 0; keystr[i]; i++, j++)
	{
		...
		if (keystr[i] == NULL)
			break;
		keystr[j] = keystr[i];
		if (strchr(validkeys,keystr[j]) == NULL) {
			fprintf(stderr,"Invalid sort key specified: %c\n",keystr[j]);
			exit(1);
		}
	}
	keystr[j] = NULL;
...
void onintr()		/* remove tempfile in case of interrupt */
{
	fprintf(stderr, "\nInterrupt\n");
	unlink(tempfile);
	exit(1);

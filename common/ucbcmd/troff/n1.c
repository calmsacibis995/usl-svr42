#ident	"@(#)ucb:common/ucbcmd/troff/n1.c	1.2"
#ident	"$Header: $"
	...
	while (--argc > 0 && (++argv)[0][0] == '-')
		...
		case 'F':	/* switch font tables from default */
			if (argv[0][2] != '\0') {
				...
				errprint("Font directory not specified");
			continue;
		case 0:
			goto start;
		case 'i':
		...
		case 's':
			...
			continue;
		case 'r':
			if (argv[0][2] != '\0' && strlen(&argv[0][2]) >= 2 &&
				argv[0][3] != '\0')
				eibuf = sprintf(ibuf+strlen(ibuf), 
				      ".nr %c %s\n", argv[0][2], &argv[0][3]);
			else errprint("Incorrect option specified");
			continue;
		case 'c':
		case 'm':
			if (mflg++ >= NMF) {
			...
			if (getenv("TROFFMACS") != '\0') {
				...
				nmfi++;
			} else if (tryfile("/usr/ucblib/doctools/tmac/", &argv[0][2], nmfi)
			  || tryfile("/usr/ucblib/doctools/tmac/tmac.", &argv[0][2], nmfi))
				nmfi++;
			else {

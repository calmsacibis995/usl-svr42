...
#ident	"@(#)ucb:common/ucbcmd/install/install.c	1.3"
#ident	"$Header: $"
...
#define	DEF_GROUP	"sys"		/* default group */
	...
	char	**argv;
{
	...
	if (dflag) {		/* install a directory */
		...
		if (oflag && getuid() == 0 && chown(dirname, pp->pw_uid, -1) && errno != EPERM) {
...
nocompare:
	...
	if (getuid() == 0 && chown(path, pp->pw_uid, gp->gr_gid) && errno != EPERM) {
		fprintf(stderr, "install: chown: %s: %s\n", path, sys_errlist[errno]);
		status = 1;
	}

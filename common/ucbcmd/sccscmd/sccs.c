...
#ident	"@(#)ucb:common/ucbcmd/sccscmd/sccs.c	1.3"
#ident	"$Header: $"
...
# define TOUCH		9	/* touch a g-file to update its time */
...
{
	"admin",	PROG,	REALUSER,		"/usr/ccs/bin/admin",
	"cdc",		PROG,	0,			"/usr/ccs/bin/rmdel",
	"comb",		PROG,	0,			"/usr/ccs/bin/comb",
	"delta",	PROG,	0,			"/usr/ccs/bin/delta",
	"get",		PROG,	0,			"/usr/ccs/bin/get",
	"help",		PROG,	NO_SDOT,		"/usr/ccs/bin/help",
	"prs",		PROG,	0,			"/usr/ccs/bin/prs",
	"prt",		PROG,	0,			"/usr/ucb/prt",
	"rmdel",	PROG,	REALUSER,		"/usr/ccs/bin/rmdel",
	"val",		PROG,	0,			"/usr/ccs/bin/val",
	"what",		PROG,	NO_SDOT,		"/usr/ccs/bin/what",
	"sccsdiff",	SHELL,	REALUSER,		"/usr/ccs/bin/sccsdiff",
	"edit",		CMACRO,	NO_SDOT,		"get -e",
	"delget",	CMACRO,	NO_SDOT,		"delta:mysrp/get:ixbeskcl -t",
	"deledit",	CMACRO,	NO_SDOT,		"delta:mysrp -n/get:ixbskcl -e -t -g/touch",
	...
	*/
	...
	switch (cmd->sccsoper)
	{
	  case SHELL:		/* call a shell file */
		*ap = cmd->sccspath;
		*--ap = "sh";
		rval = callprog("/usr/bin/sh", cmd->sccsflags, ap, forkflag);
		break;
	  ...
	  case ENTER:		/* enter new sccs files */
		 ...
		 * complain.
		 */
		if (stat(SccsPath, &statb) == -1) 
		{
			if (mkdir(SccsPath, 0777) == -1) {
				syserr("Cannot mkdir %s", SccsPath);
				exit(EX_SOFTWARE);
			}
		} 
		else if (!(statb.st_mode & S_IFDIR)) 
		{
			usrerr("File `%s' exists, but is not a directory", 
			    SccsPath);
			exit(EX_SOFTWARE);
		}
	  ...
	  case TOUCH:		
		/* 
		 * Update the mtime of a file with the mtime of the
		 * corresponding s. file.  We are given the clear file 
		 * here.  The goal is to make the clear file more recent
		 * than the s. file so that the "make" program won't
		 * get confused.  Because of the NFS it is not safe
		 * to just set it to the current time as this may
		 * be in the past with respect to the s. file whose
		 * time is set by the machine serving it.
		 */
		for (np = &ap[1]; *np != NULL; np++)
		{
			...
			timep.actime =  statb.st_mtime + 1;
			timep.modtime = timep.actime;
			rval = utime(*np, &timep);
			if (rval != 0) 
			{
				break;

#ident	"@(#)ucb:common/ucbcmd/sendmail/aux/vacation.c	1.5"
#ident	"$Header: $"
...
#ifndef lint
static char	SccsId[] = "@(#)vacation.c 1.7 87/10/14 SMI"; /* from UCB 5.3 7/1/85 */
#endif not lint
	...
	char *myname;
{
	...
	if (f == NULL)
	{
		f = fopen("/usr/ucblib/vacation.def", "r");
		if (f == NULL)
			syserr("No message to send");
	}

	(void) sprintf(line, "/usr/ucblib/sendmail -eq -f %s %s", myname, user);
	pipe = popen(line, "w");
	if (pipe == NULL)
		syserr("Cannot exec /usr/ucblib/sendmail");
...
if ((myname = getenv("USER")) == NULL)
	if ((myname = getenv("LOGNAME")) == NULL)
		if ((myname = cuserid(NULL)) == NULL)
			syserr("User name unknown");
homedir = getenv("HOME");
if (homedir == NULL)
...
printf("Press return when ready to edit this file.\n");
		fflush(stdout);
		gets(line);
		system(cmd);
	}

#ident	"@(#)ucb:common/ucbcmd/sendmail/src/parseaddr.c	1.2"
#ident	"$Header: $"
...
SCCSID(@(#)parseaddr.c 1.19 88/09/09 SMI); /* from UCB 5.6 4/2/86 */
...
# endif DEBUG
	...
	toolong:
					errno = ENOMEM;
syserr("expansion too long, avp=%x (%s%s%s...), npvp=%x", avp,
		avp[0], avp[1], avp[2], npvp);
					return;
				}
				*avp++ = rp;
			}

#ident	"@(#)ucb:common/ucbcmd/sendmail/src/queue.c	1.3"
#ident	"$Header: $"
...
# ifndef QUEUE
SCCSID(@(#)queue.c 1.25 88/07/01 SMI (no queueing)); /* from UCB 5.21 4/17/86 */
# else QUEUE

SCCSID(@(#)queue.c 1.25 88/07/01 SMI); /* from UCB 5.21 4/17/86 */
...
# endif QUEUE
		...
		if (c1 >= 'B' && c2 >= 'Z')
		{
			errno = 0;
			if (type != '\0') {
			     syserr("queuename: Cannot create files in \"%s\"",
				    QueueDir);
			}
			exit(EX_NOPERM);
		}

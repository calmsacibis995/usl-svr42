#ident	"@(#)ucb:common/ucbcmd/sendmail/src/daemon.c	1.2"
#ident	"$Header: $"
...
SCCSID(@(#)daemon.c 1.31 88/09/20 SMI (with daemon mode)); /* from UCB 5.21 7/21/86 */
...
# if BSD >= 43
    int i;
    for (i= 0; i < MAXMXHOSTS && hp->h_addr_list[i]; i++)
	/*sp->s_value.sv_host.h_addrlist[ i ] = 
		*(struct in_addr *)(*hp->h_addr_list++);*/
		memcpy((char *)& (sp->s_value.sv_host.h_addrlist[i]),
		(char *) (hp->h_addr_list[i]),sizeof(struct in_addr));
...
# ifdef DEBUG
	if (tTd(16, 1))
		printf("openhost (%x)\n", ntohl(hp->h_addrlist[hp->h_index].s_addr));
# endif DEBUG
...
# endif DEBUG
	...
	hp->h_tried = 1;		/* We are trying to connect */
		if (Verbose)
	printf("addr = %x port = %d family = %d zero = %d\n",

...
#ident	"@(#)ucb:common/ucbcmd/man/man.c	1.2"
#ident	"$Header: $"
...
/* Paths for directory structure. 
 * System V sometimes may use a different directory structure as
 * follows:
 *    /usr/man/u_man:/usr/man/p_man:/usr/man/a_man:/usr/man/local
 * and MANDIR can be set to this string.
 */
...
#define TCAT	"lp -T troff"	/* command to "display" troff output */


/*
 * These are the possible subsections of all known 4bsd-derived systems.
 * SVr4 may use upper case letters and pare down the number of
 * subsections and speed things up a bit. 
 */
#define	ALLSECT	"1nl68234570po"	/* order to look through sections */
...
usage:
		(void) fprintf(stderr,
		    "Usage:\tman [-] [-t] [-M path] [-T man] [ section ] name ...\n");
...
sectin:
		...
		mrc = manual(section, subsec, argv[0]);
		argc--, argv++;
	} while (argc > 0);
	exit(mrc);
	/* NOTREACHED */
...
search:
		...
		title[last+2] = 0;
		if (!checkpage()) {
			if ((section >= '0' && section <= '8') && subsec == 0) {
				sp = "\0";
				goto search;
			}
			else if (section == 'o') {
...
#undef	checkpage
	...
	if (!catonly && stat(manpname, &mansb) >= 0 &&
		...
		char *cbp = cmdbuf;
                register FILE *md;
		char *datestring;	/* date man page last changed */
...
!= '/')                                continue;
		...
		/* 
		 * May need to determine date of last access and
		 * pass this to the macro package, but this
		 * information is not known for SVr4.  Keeping
		 * datestring in to ease adding it in later.
		 */

...
#ident	"@(#)libadm:common/lib/libadm/putdgrp.c	1.2.6.2"
#ident	"$Header: putdgrp.c 1.3 91/06/25 $"
...
#define ETC_DIR		"/etc"
#define DACPERM		S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH
	...
	char  **members;		/* Members for that devgrp */
{
	...
	/* get uid/gid from /etc and used these to set the 
	 * set the ownership of the dgroup.tab file 
	 * This is done here in case the file didn't exist
	 * and it was created by the call to lkdgrptab()
	 */
	if (stat(ETC_DIR,&sbuf) == -1)
		return(FALSE);

	if (fchown(fileno(oam_dgroup),sbuf.st_uid, sbuf.st_gid) == -1)
		return(FALSE);

	if (fchmod(fileno(oam_dgroup),DACPERM) == -1)
		return(FALSE);

...
#ident	"@(#)libadm:common/lib/libadm/devreserv.c	1.2.10.2"
#ident  "$Header: devreserv.c 1.2 91/06/25 $"
/*LINTLIBRARY*/
...
struct devlks {
	long	lk_key;
	char	lk_alias[((DDB_MAXALIAS+2)/2)*2];
};
...
_openlkfile()
{
	...
	if (lockname = _rsvtabpath()) {
	    ...
	    /* Open it */
	    lkfilefd = open(lockname, O_RDWR|O_CREAT, 0666);
	    free(lockname);
	...
	char   *device;
{
	...
	if (alias = devattr(device, DDB_ALIAS)) {
		...
		if (strncmp(alias, plk->lk_alias, DDB_MAXALIAS) == 0)
		    locked = TRUE;
		else plk++;
	    }
	...
	char   *device;
{
	...
	olderrno = errno;
	if (alias = devattr(device, DDB_ALIAS)) srchalias = alias;
	else {
	    errno = olderrno;
	    srchalias = device;
	}

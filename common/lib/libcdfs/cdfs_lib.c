...
#ident	"@(#)libcdfs:common/lib/libcdfs/cdfs_lib.c	1.9"
#ident	"$Header: $"
...
int
cd_pvd (path, pvd)
	char				*path;				/* CD-ROM device or file/dir	*/
...
cd_xar (path, fsec, xar, applen, esclen)
	char				*path;				/* Ptr to file/directory name	*/
	int					fsec;				/* File section					*/
	struct iso9660_xar	*xar;				/* Ptr to XAR structure			*/
	int					applen;				/* Len of buf for App Use field	*/
	int					esclen;				/* Len of buf for Esc Seq field	*/
{
	int					RetVal;				/* Return value					*/
...
int
cd_ptrec (path, ptrec)
	char				*path;				/* Ptr to directory name		*/
...
int
cd_type (path)
	char				*path;				/* CD-ROM dev or file/directory	*/
{
	int					RetVal;				/* Return value					*/
	int					fd;					/* File descriptor				*/
...
cd_suf (path, fsec, signature, index, buf, buflen)
	char				*path;				/* Ptr to file/directory		*/
	int					fsec;				/* File section					*/
	...
	int					buflen;				/* Length of buf				*/
{
	...
	if (signature == NULL) {
		...
		(void) strncpy (Args.signature, signature, 2);
	}
	Args.index = index;
...
cd_GetDevName (path, tmppath)
	char			*path;					/* Path to turn into a dev name	*/
	...
	enum cdfs_type		fs_type;			/* CDFS file system type (9660/HS)*/
{
	...
	if (TmpTime != (time_t) 0) {
		...
		RetVal = strftime ((char *) DateString, sizeof (DateString), "%C",
					TimeStruct);
		if (RetVal == 0) {
			return (-1);
		}

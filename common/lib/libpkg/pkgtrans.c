...
/*LINTLIBRARY*/
#ident	"@(#)libpkg:common/lib/libpkg/pkgtrans.c	1.15.20.19"
#ident "$Header: $"
...
int	options;
{
	...
	if((srcdev.bdevice && dstdev.bdevice)  && !strcmp(srcdev.bdevice, dstdev.bdevice)) {
		progerr(ERR_TRANSFER);
		logerr(MSG_SAMEDEV);
		return(1);
	}
	...
	ids_name = NULL;
	if(srcdev.bdevice) {
		if(n = _getvol(srcdev.bdevice, NULL, NULL, "Insert %v into %p.", srcdev.norewind, 0)) {
			cleanup();
			if(n == 3)
				return(3);
			progerr(ERR_TRANSFER);
...
int	options;
{
	...
	if(ids_name) {	/* unpack the datastream into a directory */
		...
		for(part=1; part <= nparts;) {
			...
			/*
			 * If the datastream media is diskette, this process
			 * will hang when attempting to close the pipe descriptor
			 * from ds_close().  To prevent this, kill the dd process
			 * reading from the datastream diskette device now.
			 */
			else if (part > 1 && srcdev.bdevice) {
				(void) sigsend(P_PID, dd_pid, SIGKILL);
				if(waitid(P_PID, dd_pid, &infop, WSTOPPED||WEXITED) < 0) {
					/* If dd process no longer around disregard */
					if (errno != ECHILD) {
...
cleanup()
{
	...
	if(dstdev.mount && !ods_name)
	 ...
	 /*
	  * The following line is needed in order to keep pkgtrans and
	  * pkginfo from hanging with pipe implementation of ds_ginit()
	  * when reading package from datastream device.
	  */
	if (dd_pid > 0) {
		(void) sigsend(P_PID, dd_pid, SIGKILL); 
		if (waitid(P_PID, dd_pid, &infop, WSTOPPED||WEXITED) < 0) {
			/* 
			 * If dd_pid child process is no longer around
			 * disregard the error from waitid.
			 */
			if (errno != ECHILD) {
				progerr("could not terminate dd process");
				exit(1); 

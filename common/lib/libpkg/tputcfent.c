...
/*LINTLIBRARY*/
#ident	"@(#)libpkg:common/lib/libpkg/tputcfent.c	1.7.6.2"
...
FILE *fp;
{
	...
	if(!strchr("lsin", ept->ftype)) {
		...
		if(ept->ainfo.macid < 0)
			(void) fprintf(fp, "Expected MAC level (security field): %d\n", user_pub);
		else
			(void) fprintf(fp, "Expected MAC level (security field): %d\n", ept->ainfo.macid);
		(void) fprintf(fp, "Expected fixed privileges (security field): %s\n", ept->ainfo.priv_fix);
		(void) fprintf(fp, "Expected inheritable privileges (security field): %s\n", ept->ainfo.priv_inh);
	}

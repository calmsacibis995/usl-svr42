...
#ident	"@(#)libns:common/lib/libns/uidmap.c	1.7.11.3"
#ident  "$Header: uidmap.c 1.2 91/06/26 $"
#include <stdio.h>
#include "idload.h"
...
char	    *token;
{
	...
	if (last != NULL) {
		...
		nmlist->next->idmap->i_loc = UID_NOBODY;
...
struct mach	*curr_mach;
{
	...
	while (curr_mach != NULL) {
		...
		while (name != NULL) {
			 ...
			 *	as long as the local value is UID_NOBODY
			...
			list = curr_mach->m_nmlist;
			while (list != NULL) {
				if (excludes && list->idmap->i_loc != UID_NOBODY)
					excludes = 0;

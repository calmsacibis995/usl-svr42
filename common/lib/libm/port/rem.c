...
double	x, y;
{
	...
	if ((LOWORD(y) == 0 && HIWORD(y) == 0) || (xexp == MAXEXP) || 
	   ((yexp == MAXEXP) && (LOWORD(y) || HIFRACTION(y)))) {
		/* IEEE specifies exception for y == 0 or
		 * signalling NaN or x ==
		 * inf or signalling NaN
		 */
		...
		exc.type = DOMAIN;
		exc.name = "remainder";
		if (((xexp == MAXEXP) && !QNANBIT(x)) 
			|| ((LOWORD(y) == 0) && (HIWORD(y) == 0))
			|| ((yexp == MAXEXP) && !QNANBIT(y))) {
			x1 /= x2; /* raise invalid-op exception */
			HIQNAN(exc.retval);
			LOQNAN(exc.retval);
		}
		else { /* x or y was quiet NaN */
			exc.retval = (xexp == MAXEXP) ? x : y;
		}

...
#ident	"@(#)curses:common/lib/xlibcurses/screen/wrefresh.c	1.41.2.4"
#ident  "$Header: wrefresh.c 1.2 91/06/27 $"
...
#ifdef __STDC__
extern	int	_outch(int), _outwch(chtype);
#else
extern	int	_outch(), _outwch();
#endif
...
/* SS: colors */
	    if (back_color_erase)
		_turn_off_background();
	    _PUTS(clr_eos ? (char *)clr_eos : tparm(parm_delete_line, scrli - topy),
		    scrli - topy);

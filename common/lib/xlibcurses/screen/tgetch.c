...
#ident	"@(#)curses:common/lib/xlibcurses/screen/tgetch.c	1.15.2.4"
#ident  "$Header: tgetch.c 1.3 91/08/08 $"
...
#define ENVNAME		"CKEYPAD"	/* Name of env variable */
#define TICKSEC		1000000L	/* ticks per second */
...
long 	_alarm_time = 3L; 	/* set alarmtime, default 0.3 sec. */
...
#define WAIT3		333
	...
	if (*chars_onQ == 0)
	{
	    ...
	    inp = inputQ[0];
	/* inp &= 0xff;  Not good; EOF not being recognised */
...
_fpk()
{
    ...
    /* set alarm */
    tick_time = _alarm_time*100000L;	/* 1/10 sec */
    timer.it_interval.tv_sec = 0L;
    timer.it_interval.tv_usec = 0L;
    timer.it_value.tv_sec = tick_time/TICKSEC;
    timer.it_value.tv_usec = tick_time-(tick_time/TICKSEC)*TICKSEC;

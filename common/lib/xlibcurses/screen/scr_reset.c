...
#ident	"@(#)curses:common/lib/xlibcurses/screen/scr_reset.c	1.12.2.3"
#ident  "$Header: scr_reset.c 1.2 91/06/26 $"
...
int	type;
{
	...
	/* soft labels */
	(fread((char *) &magic, sizeof(short), 1, filep) != 1))
	goto err;

	fread((char *) &abc , sizeof(short), 1, filep); /* This just to
		skip the 2 bytes of data*/

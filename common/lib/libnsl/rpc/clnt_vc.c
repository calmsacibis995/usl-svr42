...
#ident	"@(#)libnsl:common/lib/libnsl/rpc/clnt_vc.c	1.5.9.2"
#ident  "$Header: clnt_vc.c 1.3 91/09/20 $"
	...
	u_int recvsz;			/* buffer send size */
{
	...
	ct->ct_addr.buf = NULL; /* SNI: initialize to 0, so noone tries to free */
	...
	case T_IDLE:
		...
		if (t_connect(fd, sndcall, rcvcall) == -1) {
			...
			/* SNI: we didn't allocate the address, so we don't free it! */

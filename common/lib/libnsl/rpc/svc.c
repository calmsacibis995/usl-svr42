...
#ident	"@(#)libnsl:common/lib/libnsl/rpc/svc.c	1.4.9.4"
#ident  "$Header: svc.c 1.3 91/09/20 $"
	...
	SVCXPRT *xprt;
{
	...
	if (xports == NULL) {
		 ...
		 */
/*
 * This is commented out because rpc callback support is broken,
 * causing the lock manager to dump core.
	...
	fd_set *readfds;
{
	...
	for (fd = 0; fd < setsize; fd += NFDBITS) {
		...
		/*
		 * avoid hanging in SVC_RECV if
		 * transport handle is dead (SNI).
		 */
		if (SVC_STAT(xprt) == XPRT_DIED){
			SVC_DESTROY(xprt);
			continue;
		}

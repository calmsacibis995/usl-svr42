...
#ident	"@(#)libnsl:common/lib/libnsl/rpc/clnt_bcast.c	1.6.7.3"
#ident  "$Header: clnt_bcast.c 1.2 91/06/26 $"
	...
	char		*nettype;	/* transport type */
{
	...
	char	*optbuf = (char *)NULL;		/* pointer to area used to return
						 * protocol-specific options */
	long	maxopts = 0;			/* maximum number of bytes of
						 * protocol specific options */
	...
	if (maxopts > 0) {
		optbuf = malloc((unsigned)maxopts);
		if (optbuf == (char *)NULL) {
			stat = RPC_SYSTEMERROR;
			goto done_broad;
		}
		t_rdata.opt.buf = optbuf;
		t_rdata.opt.maxlen = maxopts;
	}

...
#ident	"@(#)libnsl:common/lib/libnsl/rpc/svc_vc.c	1.8.9.2"
#ident  "$Header: svc_vc.c 1.3 91/09/20 $"
...
/*
 * Thist routine is called by rendezvouz_request when a listen is
 * pending on a filedescriptor.
 * The listen is consumed and appended to a linked list.
 *
 * We (mis-) use call->udata.buf as a pointer for the linked list.
 */

static struct t_call *
queue_listen(listenqueue, fd)
	struct t_call *listenqueue;
	int fd;
{
	struct t_call *t_call;

	t_call = (struct t_call *) t_alloc(fd, T_CALL, T_ADDR | T_OPT);
	if (t_call == NULL) {
		(void) syslog(LOG_ERR, "queue_listen: no memory");
		return (listenqueue);
	}
	while (t_listen(fd, t_call) == -1) {
		if (errno == EINTR)
			continue;
		(void) t_free((char *)t_call, T_CALL);
		return (listenqueue);
	}
	t_call->udata.buf = (char *) listenqueue;
	return(t_call);
}
	...
	register SVCXPRT *xprt;
{
	char *tpname = NULL;
	char devbuf[256];
	struct t_call *listenqueue = NULL;
	...
	/*
	 * Consume all pending events for a transport endpoint.
	 */

	while (event = t_look(xprt->xp_fd)) {
		switch (event) {
		case T_DISCONNECT:
		...
		case T_LISTEN:
			listenqueue = queue_listen(listenqueue, xprt->xp_fd);
			break;
		}
	}

	/*
	 * Nothing to do, if no outstanding listen requests
	 */
	if (listenqueue == NULL)
		return(FALSE);
...
err1:
			while (listenqueue) {
				struct t_call *t_call = listenqueue;
				listenqueue = (struct t_call *) t_call->udata.buf;
				t_call->udata.buf = NULL;
	...
	while (listenqueue) {
		...
		while (t_accept(xprt->xp_fd, fd, &t_call2) == -1) {
			if (t_errno == TLOOK) {

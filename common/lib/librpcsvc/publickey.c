...
#ident	"@(#)librpcsvc:common/lib/librpcsvc/publickey.c	1.2.6.1"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/lib/librpcsvc/publickey.c,v 1.1 91/02/28 20:58:45 ccs Exp $"
	...
	char *publickey;
{
	...
	*p = 0;
	/* Assuming that publickey has enough space */
	...
	char *ret;
{
	...
	for (;;) {
		...
		if (res[0] == '#')
			...
			lookup = NULL;
			err = yp_match(domain, PKMAP, key, strlen(key),
					&lookup, &len);
			if (err) {
#ifdef DEBUG

...
#ident	"@(#)nametoaddr:common/lib/nametoaddr/resolv/libsocket/inet/rcmd.c	1.1.7.3"
	...
	*p = '\0';
	...
	if (!superuser) {
		if ((hostf = fopen("/etc/hosts.equiv", "r")) != NULL) {
			if (!_rs__validuser(hostf, fhost, luser, ruser, baselen, 0)) {
				(void) fclose(hostf);
				return(0);
		        }
			(void) fclose(hostf);
	...
	if (!_rs__validuser(hostf, fhost, luser, ruser, baselen, 1)) {
	  	(void) fclose(hostf);
		if (euid != (uid_t)-1)
			(void) seteuid (euid);
		return(0);
...
static int
_rs__validuser(hostf, rhost, luser, ruser, baselen, isrhosts)

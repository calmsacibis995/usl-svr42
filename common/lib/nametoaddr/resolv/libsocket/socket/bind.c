...
#ident	"@(#)nametoaddr:common/lib/nametoaddr/resolv/libsocket/socket/bind.c	1.1.6.3"
#ident  "$Header: bind.c 1.2 91/06/26 $"
...
extern int	errno;
static int	_rs__unbind();

int
_rs_bind(s, name, namelen)
	register int			s;
	...
	register int			namelen;
{
	...
	return (_rs__bind(siptr, name, namelen, NULL, NULL));
}

_rs__bind(siptr, name, namelen, raddr, raddrlen)
	register struct _si_user	*siptr;
	register struct sockaddr	*name;
	register int			namelen;
	register char			*raddr;
	register int			*raddrlen;
{
	...
	} else	namelen = _rs__s_min(namelen, siptr->udata.addrsize);
	...
	if (bind_req->ADDR_length) {
		_rs__s_aligned_copy(buf, bind_req->ADDR_length, size,
				(caddr_t)name,
	...
	if (!_rs__s_do_ioctl(siptr->fd, buf, size, TI_BIND, NULL)) {
	 ...
	 */
	if (raddr != NULL)
		*raddrlen = _rs__s_cpaddr(siptr, raddr, *raddrlen,
	...
	register struct _si_user	*siptr;
{
	...
	sigsave = sigset(SIGPOLL, SIG_HOLD);
	if (!_rs__s_do_ioctl(siptr->fd, siptr->ctlbuf,

...
#ident	"@(#)libeti:form/utility.c	1.10"
...
*/
	char * vend = v + n;
	while (vend > v && (*(vend-1) == ' ' || *(vend-1) == NULL)) --vend;
	return vend;
}
...
*/
	...
	*/
	...
	else /* non public field */
	{
 		(void)wmove(Sub(f), c->rows, c->cols);
 		wsyncup( Sub(f));
	}
	(void)untouchwin (w);

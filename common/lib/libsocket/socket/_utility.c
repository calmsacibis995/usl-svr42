...
#ident	"@(#)libsocket:common/lib/libsocket/socket/_utility.c	1.20.12.5"
#ident  "$Header: _utility.c 1.3 91/09/20 $"
	...
	register int		s;
{
	...
	if ((siptr = find_silink(s)) != NULL)
		return (siptr);
	if (s < 0)		/* errno will be set in find_silink, */
		return(NULL);	/* (in an equivalent test), but we must */
				/* return now and not try to re-constitute */
				/* from a obviously invalid pointer */
	...
	register int		flags;
{
	...
	return(recvmsg(siptr, msg, flags));

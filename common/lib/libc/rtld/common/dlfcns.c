...
#ident	"@(#)rtld:common/dlfcns.c	1.14"
...
int mode;
{
	...
	if (!dl_head) {  /* set up DLLIB structure for main */
		...
		if ((dl_head = (DLLIB *)_rtmalloc(sizeof(DLLIB))) == 0) 
			return 0;
		dl_head->dl_group = dlgroup;
		dl_head->dl_name = (char *)0; /* main has null name */
		dl_head->dl_status = DL_CLOSED;
		dl_tail = dl_head;
		if ((dl_head->dl_list = dl_makelist(_ld_loaded, dlgroup)) ==
			(DLLIST *)0) {
			dl_head = 0;
			return 0;
		}
	...
	if (!dlptr || (dlptr->dl_status != DL_OPEN && dlptr->dl_name)) {
		...
		if ((dlptr->dl_list = dl_makelist((struct rt_private_map *)(retval->d_un.d_ptr), dlgroup)) == (DLLIST *)0) {
			goto error_unhook;
		}
	}
...
** appendLm(lptr, lm)
**
** append lm to the end of DLLIST lptr, return 1 if successful
** 0 otherwise. Only way this can fail is if _rtmalloc fails.
*/

static int appendLm(lptr,lm)
	DLLIST *lptr;
	struct rt_private_map *lm;
{
	/* lptr cannot be NULL to start off */
	DLLIST *lprev;

	for(; lptr; lprev=lptr, lptr = lptr->l_next){
		int index;
		for(index = 0; index < DL_LISTSIZE; index++){
			if(!lptr->l_list[index]){
				/* found an empty slot; insert and
				** return */
				lptr->l_list[index] = lm;
				return 1;
			}
		}
	}
	if ((lptr = (DLLIST *)_rtmalloc(sizeof(DLLIST))) == 0) 
		return 0;
	/* link the new chunk in and insert lm */
	lprev->l_next = lptr;
	lptr->l_list[0] = lm;
	return 1;
}
...
** lmExists(lstart,lm)
**
** Returns 1 if lm exists in DLLIST lstart, 0 otherwise.
*/

static int lmExists(lstart, lm)
	DLLIST *lstart;
	struct rt_private_map *lm;
{
	DLLIST *lptr;
	...
	for (lptr = lstart; lptr; lptr = lptr->l_next) {
 ...
 */
static DLLIST *dl_makelist(lm, group)
struct rt_private_map *lm;
unsigned long group;
{
	DLLIST *lhead, *lread, *lptr;
	Elf32_Dyn *lneed;
	register int i_rd;
	char *name;
	struct rt_private_map *curlm;

	DPRINTF(LIST,(2,"rtld: dl_makelist(0x%x, %d)\n",lm, group));
	...
	lread = lhead;
	i_rd = 0;
	while (curlm = lread->l_list[i_rd]) {
		/* process each object on needed list */
		for( lneed = (Elf32_Dyn *)DYN(curlm);
				lneed->d_tag!=DT_NULL; lneed++) {

			struct rt_private_map *tmpLm;
			int fileCnt, index;
			char **fnames;
			...
			name = (char *)STRTAB(curlm) + lneed->d_un.d_val;

			if(tmpLm = _so_loaded(name)){
				/*
				** Add to set if not already there
				*/
				if (!lmExists(lhead, tmpLm)) {
				   _rt_addset(group, tmpLm);
				   if(!appendLm(lhead,tmpLm)) return 0;
				}
			} else  {
				/* The only we can get here is if
				** the library has not been loaded or 
				** its been loaded through an alias
				** name. */

				fileCnt = _so_find(name, &fnames);

				for(index=0; index<fileCnt; index++){
				   tmpLm = _so_loaded(fnames[index]);
				   if(tmpLm == 0){
					_rt_lasterr("%s: %s: error: unable to load %s",
...
VOID *handle;
{
	...
	if (COUNT(((DLLIB *)handle)->dl_list->l_list[0]) <= 0)
	{
		...
		/* remove this group from each member's group set */
		for (lptr = ((DLLIB *)handle)->dl_list; lptr; 
			lptr = lptr->l_next) {
			for (i = 0; i < DL_LISTSIZE; i++) {
				if (!lptr->l_list[i])
					break;
				else {
					_rt_delset(group, lptr->l_list[i]);
				}
			}
		}
	}

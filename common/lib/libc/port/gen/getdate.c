...
#ident	"@(#)libc-port:gen/getdate.c	1.12"
...
struct tm *t;
{
	...
	nct = *ct;
	tv = mktime(ct);
	if (t->tm_isdst > 0)  /* saw a %Z */
		if (t->tm_isdst -1 != ct->tm_isdst)
		{
   			getdate_err = 8;
                	return(NULL);
		}

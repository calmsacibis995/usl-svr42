...
#ident	"@(#)libc-port:gen/hrtctime.c	1.7"
...
hrtime_t *tdp;
{
	...
	return(_hrtasctime(tmp, (ulong)tdp->hrt_rem, tdp->hrt_res));
}
...
ulong res;
{
	...
	(void) sprintf(cbuf, "%.3s %.3s %2d %02d:%02d:%02d",
	...
	p = cbuf + strlen(cbuf);
	if (res == 1)
		(void) sprintf(p, " %4d", tmp->tm_year + 1900);
	else
		(void) sprintf(p, ".%0*ld %4d", (int)llog.dl_lop, lfrac.dl_lop,
					tmp->tm_year + 1900);
	return(cbuf);
}

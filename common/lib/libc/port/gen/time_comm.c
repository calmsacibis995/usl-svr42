...
#ident	"@(#)libc-port:gen/time_comm.c	1.38"
/*
...
int addend;
{
	...
	if (*sum > 0) {
		if (INT_MAX - *sum < addend) 
			return -1;
	}
	else if (INT_MIN - *sum > addend)
			return -1;
...
long addend;
{
	...
	if (*sum > 0) {
		if (LONG_MAX - *sum < addend) 
			return -1;
	}
	else if (LONG_MIN - *sum > addend)
			return -1;
...
static long
multadd(sum, value, scale)    /*  scale should not be non-positive */
long *sum;
int value;
int scale;
{
	...
	return(add(sum, value * (long) scale));
}
...
{
	struct tm	*tptr;
	long	secs = timeptr->tm_sec;
	int	temp;
	int	isdst = 0;
	long	daybegin, dayend;
	struct tm	stm;

	temp = timeptr->tm_mday;
	if (addint(&temp, -1) != 0)
		goto err;

	if (multadd(&secs, temp, SEC_PER_DAY) != 0)
		goto err;

	if (multadd(&secs, timeptr->tm_hour, SEC_PER_HOUR) != 0)
		goto err;

	if (multadd(&secs, timeptr->tm_min, SEC_PER_MIN) != 0)
		goto err;

	if (timeptr->tm_mon >= 12) {
		if (addint(&timeptr->tm_year, timeptr->tm_mon / 12) != 0)
			goto err;
		timeptr->tm_mon %= 12;
	} else if (timeptr->tm_mon < 0) {
		int n;
		if (timeptr->tm_mon <= LONG_MIN)
			goto err;
		temp = -timeptr->tm_mon;
		{
			n = temp / 12;
			if ( n * 12 != temp)
				n++;
			if (addint(&timeptr->tm_year, -n ) != 0)
				goto err;
		}
		timeptr->tm_mon = ((n = temp % 12) ? 12 - n : 0);
	}
	...
	if (multadd(&secs, temp, SEC_PER_YEAR) != 0)
		goto err;
	...
	if (multadd(&secs, temp/4 - 1 - LEAP_TO_70, SEC_PER_DAY) != 0)
		goto err;

	if (timeptr->tm_year % 4 == 0) {
		if (multadd(&secs, __lyday_to_month[timeptr->tm_mon], SEC_PER_DAY) != 0)
			goto err;
	}
	else {

		if (multadd(&secs, __yday_to_month[timeptr->tm_mon], SEC_PER_DAY) != 0)
			goto err;
	}

	_ltzset(secs);
	if (timeptr->tm_isdst > 0) {
		if (add(&secs, altzone) != 0)
			goto err;
	}
	else {
		if (timeptr->tm_isdst < 0)
			isdst = -1;
		if (add(&secs, timezone) != 0)
			goto err;
	}
...
char **tz;
{
	...
	if (!isgraph(*q) || isdigit(*q) || *q == ':' || *q == ','
		|| *q == '-' || *q == '+') 
		return(0);
	while (isgraph(*++q) && !isdigit(*q) && *q != ',' && *q != '-' 
		&& *q != '+')  ;

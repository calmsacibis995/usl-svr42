...
#ident	"@(#)libc-port:gen/getutx.c	1.12"
...
static void	unlockutx(), sendpid(), _setlvl();
static	idcmp(), allocid(), lockutx();
	...
	struct utmpx *utx;
{
	...
	} else if ((fdx < 0) && ((fdx = open(filex, O_WRONLY)) < 0)) {
		close(fd);
		return;
	}

 ...
 */
extern char *fcvt(), *ecvt(), *fcvtl(), *ecvtl();
...
/*LONG DOUBLE*/		if(((flagword & LONGD) /*&& ldval != 0*/) 
			     || (dval != 0)) {
			...
			if(flagword & LONGD)
/*LONG DOUBLE*/			*--suffix=(decpt > 0 /*|| ldval == 0*/) ? '+' : '-';
			else
		...
		case 'f':
			...
			if(flagword & LONGD) {	/* dealing with long double */
				/* Establish default precision */
				if (!(flagword & DOTSEEN))
					prec = 6;
	...
	while (found)
	{
		...
		again:;
			...
			case 'G':
				...
				else (void)va_arg((*pargs).ap, double);
				break;
			case 's':

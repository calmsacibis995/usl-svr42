...
#ident	"@(#)libc-port:stdio/fopen.c	1.20.2.4"
/*LINTLIBRARY*/
...
#ifdef	__STDC__
	#pragma weak _realfopen = fopen
	#pragma weak _realfreopen = freopen
#endif
	...
	register FILE *iop;
{
	...
	iop->_file = (unsigned char)fd;	/* assume that fd fits in unsigned char */
	if (plus == '+')
		iop->_flag = _IORW;
	else if (type[0] == 'r')
		iop->_flag = _IOREAD;

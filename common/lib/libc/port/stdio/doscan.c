...
va_list va_alist;
{
	...
	charswitch:
		...
		switch(ch)
		{
		 ...
		 case 'C':
		 case 'S':
			 size = wstring(stow,ch,len,iop,&args.ap);
			 break;
#endif /*NO_MSE*/
		 case 'c':
		 case 's':
...
va_list *listp;
{
	...
	if (stow && digitseen)
		if(floater) 
		{
			...
			*np = '\0';
			if (size == 'L')
				ldval = strtold(numbuf, (char **)NULL);
			else 
				dval = atof(numbuf);
			if (negflg) {
				if (size == 'L') ldval = -ldval;
				else dval = -dval;
			}
			if (size == 'L')
...
wchar_t *wp;
{
	...
	while (--len >= 0)
	{
		if ((ch = locgetc()) < 0200)	/* includes EOF */
		{
			if (ch != EOF)
				goto backup;
			return 0;
...
stva_list arglst[];
{
	...
	while ((fmt = strchr(fmt, '%')) != 0)
	{
		...
		if (*fmt == '*' || *fmt == '%')
			continue;
		if (fmt[n = strspn(fmt, "0123456789")] == '$')
		{

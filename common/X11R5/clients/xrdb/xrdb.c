...
/*
 * 	Define the remove flag to be used with -n -edit option, for removing the file 
 *	after displaying.
 */
Bool remove_file_flag = False;
    ...
    Bool execute;
{
	...
	if (!input) {
		...
		/*
 		* 	If the -n option is given then set the remove flag to high.
 		*/
		else
			if (dont_execute)
	...
	if (dont_execute) {		/* then write to standard out */
		...
		/*
 		* 	Unlink the edit file name given, as it is created by xrdb.
 		*/
		if (dont_execute)
			unlink (editFile);
	    unlink (template);
...
#define CPP_DEFAULT "/lib/cpp"
#define CPP_DEFAULT1 "/usr/ccs/lib/cpp"

        strcpy(path,cpp_program);
        strtok(path," ");
	result = strcspn(cpp_program, " ");
	strcpy(defines, cpp_program + result);	
        result=stat(path, &statbuf);
	if (result >= 0) return;
	strcpy(path,CPP_DEFAULT);
        result=stat(path, &statbuf);
	if (result >= 0) {
		sprintf(path, "%s %s",CPP_DEFAULT, defines);
		strcpy(cpp_program,path);
		return;
	}
	
	strcpy(path,CPP_DEFAULT1);
        result=stat(path, &statbuf);
	if (result >= 0) {
		sprintf(path,"%s %s", CPP_DEFAULT1, defines);
		strcpy(cpp_program,path);
		return;
	}

                fprintf(stderr,"cannot find C Preprocessor %s\n",path);
                fprintf(stderr,"Use -cpp commandline option\n");
                Syntax();
}

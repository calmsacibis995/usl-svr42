...
#ident	"@(#)r5xfontsel:xfontsel.c	1.3"
...
#define _memutil_h
#undef  MEMUTIL
	/* Turn off MEMUTIL memory utilities because of unusual use of (our
	/* redefined) XtFree here (passed as argument to another function). */
 ...
 *	CHANGE # UNKNOWN
 *	FILE # xfontsel.c
 *  temp_menu is an array of type FieldMenuRec of dimension equal to the number
 *  of fields. 
 *	ENDCHANGE # UNKNOWN
 */
	FieldMenuRec temp_menu[FIELD_COUNT];
 ...
 *	CHANGE # UNKNOWN
 *	FILE # xfontsel.c
 *  The function EnableFieldItems is called with a priority of 2 .
 *  temp_menu , an array of FieldMenuRec  is passed for closure .
 *  This is used for the field buttons .
 *	EnableFieldItems(closure);
 *	ENDCHANGE # UNKNOWN
 */
	ScheduleWork(EnableFieldItems, (XtPointer)temp_menu, 2);
 ...
 *	CHANGE # UNKNOWN
 *	FILE # xfontsel.c
 *  Instead of unmapping the sampleText label widget, we should just set the 
 *  label to blanks. Otherwise the viewport does not get mapped.
 *
 ...
 */
 	XtUnmapWidget(sampleText);
	XtSetArg (args[0], XtNlabel, " ");
	XtSetValues (sampleText, args, 1);
 	XtMapWidget(sampleText);
#endif
    }
}
 ...
 *	CHANGE # UNKNOWN
 *	FILE # xfontsel.c
 *  The function was added to be able to set the buttons to sensitive after 
 *  the parsing procedure is over. It is called through ScheduleWork .
 *
 *	EnableFieldItems(closure);
 *	ENDCHANGE # UNKNOWN
 */
void EnableFieldItems(closure)
	XtPointer closure;
{
    FieldMenuRec *temp_menu = (FieldMenuRec*) closure ;
	int f ;

		for (f = 0 ;f <= FIELD_COUNT; f++) {
			XtSetSensitive(temp_menu[f].button, True);
	}

...
/*
 * 		Since the -spy option was hanging around indefinitely even after the
 *	named window is deleted, we added StructureNotify to the list of events
 *	selected. This allows us to check for DestroyNotify events.
 *	Of course, we also get a lot of other events like UnmapWindow etc to 
 *	handle this we use a switch statement.
 */

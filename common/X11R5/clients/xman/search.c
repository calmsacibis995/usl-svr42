 ...
 *	CHANGE # UNKNOWN
 *	FILE # search.c
 *  If manpages are given at the command line then search_string
 *  is the given manpage's name.
 *	ENDCHANGE # UNKNOWN
 */
  if (man_file)
	search_string = man_file_name;
  else
  	search_string = SearchString(man_globals);

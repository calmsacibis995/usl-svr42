...
void MySaveFormattedPage(Widget);
...
/*
 * 	We need to popup down the standby popup in every case. We also
 * 	quit the application as xman freezes here. Till this bug is solved ...
 */
		XtPopdown( XtParent(man_globals->standby) );

		/*
		 * The INTRINSICS get confused at this point : so print an
		 * error message and quit
		 */
		fprintf(stderr,"xman : %s\n", error_buf);
		exit(1);

		PopupWarning(man_globals, error_buf);
		printf("*** OUT OF POPUPWARNING ***\n");
		file = NULL;
	}
  	else
	{
		...
		/*
		 * 	We need to popup down the standby popup in every case.
		 */
			XtPopdown( XtParent(man_globals->standby) );
			PopupWarning(man_globals, error_buf);
		}
		else
		{
			XtPopdown( XtParent(man_globals->standby) );
			/*
			*  Added the resource saveAlways, so that no questions are asked when a new
			*  manpage is displayed.
			*/
			if (resources.save_always)
			{
				MySaveFormattedPage(man_globals->This_Manpage);
			}
			else
			{
				/*
				 *  Save the manpage even if we are coming through TopBox.
				 *
				 *    if ( (man_globals->save == NULL) ||
				 *	   (man_globals->manpagewidgets.manpage == NULL) ) 
				 */
				if ( (man_globals->save == NULL) ||
				   (man_globals->This_Manpage == NULL) ) 
				{
				...
				else
				{
					...
					if ( (ptr = rindex(catdir, '/')) != NULL) 
					{
					  *ptr = '\0';
					  ...
					  {
					/*
					*	If manpage is already created, then get x,y from manpage. Otherwise from
					*  get it from TopBox widget, saved in man_globals->This_Manpage.
					*/
						if (man_globals->manpagewidgets.manpage)
						{
						...
						{
								x = (Position) Width(man_globals->This_Manpage)/2;
								y = (Position) Height(man_globals->This_Manpage)/2;
								XtTranslateCoords(man_globals->This_Manpage, x, y, &x, &y);
						}
...
/*
 *  MySaveFormattedPage is to save the manpage (same as SaveFormattedPage),
 *	but it is called if the resource saveAlways is true. Then teh likeToSave
 *	widget is not popped up.
 */
void
MySaveFormattedPage(w)
Widget w;
{
  ManpageGlobals * man_globals;

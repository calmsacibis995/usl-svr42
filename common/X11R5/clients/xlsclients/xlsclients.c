...
#ident	"@(#)r5xlsclients:xlsclients.c	1.2"
 ...
 *	CHANGE # UNKNOWN
 *	FILE # buttons.c
 *  The options should be display and max , not d  or m followed by any other
 *  junk 
 *	ENDCHANGE # UNKNOWN
 */
	if (arg[0] == '-') {
		...
		if (++i >= argc) usage ();
		if  (!strcmp(arg,"-d") || !strcmp(arg,"-display"))
			displayname = argv[i];
		else  
			usage ();
			continue;
		...
		if (++i >= argc) usage ();
		if  (!strcmp(arg,"-m") || !strcmp(arg,"-maxcmdlen"))

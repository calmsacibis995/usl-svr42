...
/*copyright "%c%"*/
#ifndef NOIDENT
#ident	"@(#)nameserver:xdaemon.c	1.14"
#endif
...
int	sig;
{
    fprintf(stderr, "xdaemon: received signal %d\n", sig);
...
int	fd;
{
...
#ifdef DEBUG
                      	fprintf(stderr, "XDAEMON: read error; errno %d\n",
                                                 errno);
#endif
	...
	if(ptr[n] != '\0')
	{
		fprintf(stderr, "XDAEMON: net name error\n");
                SendNull(fd);
	}	
	else if(pipe(fds) < 0){
		fprintf(stderr, "XDAEMON: pipe failed error\n");
                SendNull(fd);
		}
	else if((pid = fork()) == 0)
	{
		...
		close(fds[0]);
		close(fds[1]);
		sprintf(path, GetXWINHome ("lib/net/%s/nameserver"), net);
		...
		if(pid < 0)
		{
			fprintf(stderr, "XDAEMON: fork failed error\n");
                	SendNull(fd);
			close(fds[1]);
		}
		else { 
		 	...
		 	if(write(fds[1], iop->inputbuf, iop->msglen) != iop->msglen){
				SendNull(fd);
				fprintf(stderr,"XDAEMON: write failed error\n");
				}
			close(fds[1]);
		}
	}

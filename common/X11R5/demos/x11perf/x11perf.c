...
/*
 * USL November 1991
 *	With constant HEIGHT and WIDTH, we cannot run in the 800x600 modes
 */
#ifndef FIXED_WINDOW
int HEIGHT, WIDTH;
#endif
...

{
    ...
    /*
     * USL November 1991
     *	The original MIT source has WIDTH and HEIGHT set to 600
     *  If we want to run the server in any lesser res modes 
     *  (ex: 800x600 or 640x480),  this will not work.
     *  So, if the width or height is more than 600, clip it to 600,
     *  else take the max width and height of the frame buffer and 
     *  reduce them by a few pixels for the text display.
     */

    WIDTH=  XDisplayWidth(xparms.d,screen);
    /* allow 30 pixels for text display */
    HEIGHT= XDisplayHeight(xparms.d,screen) - 30;
    printf("screen width=%d screen height=%d\n", WIDTH, HEIGHT+30);

    if (WIDTH>600) WIDTH = 600;
    if (HEIGHT>600) HEIGHT = 600;

    printf("x11perf test window, width=%d height=%d\n", WIDTH, HEIGHT);
#endif

...
#ident	"@(#)r5fontutil:clients/mkfontdir/mkfontdir.c	1.6"
/***********************************************************
    ...
    FontTablePtr    table;
{
	...
	fprintf(stderr,"dirName=%s FontDirFile=%s\n",dirName,FontDirFile);
	return FALSE;
    }
    fprintf(file, "%d\n", table->used);
    ...
    char    *font_name;
{
    ...
    if (snfGetInfoBitmap ((FontPathElementPtr) 0, &info, (FontEntryPtr) 0, file_name) != Successful) {
	fprintf(stderr,"Error getting bitmap info file_name=%s\n",file_name);
	return FALSE;
	}
	...
	if (atom_name && strcmp (atom_name, "FONT") == 0 && info.isStringProp[i])
	{
		...
		fprintf(stderr,"no name atom for file=%s\n",file_name);
		return FALSE;
	}
 ...
 fprintf(stderr,"no name atom at end file=%s\n",file_name);
    return FALSE;
}
    ...
    FontTablePtr	table;
{
	...
	fprintf(stderr,"error GetFontName for %s\n",font_name);
	return FALSE;
    ...
    FontTablePtr    table;
{
    ...
    memset((char *)hashTable,0, HASH_SIZE * sizeof(NameBucketPtr));
	...
	if (renderer)
	{
		...
		if (!(bucket->name = MakeName (file->d_name))) {
			fprintf(stderr,"can't make name \n");
		    return FALSE;
	}
		bucket->next = 0;
		bucket->renderer = renderer;
	...
	for (bucket = hashTable[hash]; bucket; bucket = next)
	{
	    ...
	    if (status)
	    {
		   ...
		   fprintf(stderr,"err from ProcessFile\n");
		    status = FALSE;
	    	}
	}
	    free (bucket->name);
...
int fntopts;
char *renderer_options[3];
char *fontconfig_file;

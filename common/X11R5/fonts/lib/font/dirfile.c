#ident	"@(#)r5fontlib:font/dirfile.c	1.4"
    ...
    FontDirectoryPtr	*pdir;
{
    ...
    if (!file) {
#ifdef WARNING
		ErrorF("\nWARNING:  The fonts.dir file for %s is missing.\n",directory);
		ErrorF("The fonts in this directory are not accessible ");
		ErrorF("to the server.\ncd to %s and run:",directory);
		ErrorF("\tmkfontdir . \n");
		ErrorF("This will create a fonts.dir file.\n");
#endif
	...
	if (fstat (fileno(file), &statb) == -1) {
		ErrorF("\nWARNING: Can't stat fonts.dir file for %s.\n",directory);
		ErrorF("This will cause problems if you want to use fonts");
		ErrorF(" from this directory.\n ");
		ErrorF("Change protections on files and directories in this path.\n");
		dir = FontFileMakeDir(directory, 0);
		*pdir = dir;
		return Successful;
		}
	...
	if ((count == EOF) || (count != 1)) {
	    fclose(file);
	        ErrorF("\nWARNING: Error encountered reading fonts.dir file for %s\n",directory);
	        ErrorF("Bad format encountered in file!\n");
		ErrorF("The fonts in this directory are not accessible ");
		ErrorF("to the server.\ncd to %s and run:",directory);
		ErrorF("\tmkfontdir . \n");
		ErrorF("This will create a valid fonts.dir file.\n");
		dir = FontFileMakeDir(directory, 0);
		*pdir = dir;
	        return  Successful;
	}
	...
	while ((count = fscanf(file, "%s %[^\n]\n", file_name, font_name)) != EOF) {
		...
		ErrorF("\tmkfontdir . \n");
		FontFileFreeDir(dir);
		FontFileMakeDir(directory, 0);
			/* free first one so any invalid names do
				not try to be used, and just
				create a null entry */
		*pdir = dir;
		return Successful;
	    }
	    ...
	    {
		ErrorF("Warning: bad format for %s in %s\n", font_name,file_name);
		continue;
	    }
	}
	fclose(file);
    status = ReadFontAlias(directory, FALSE, &dir);
    if (dir) FontFileSortDir(dir);

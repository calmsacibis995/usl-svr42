...
#ident	"@(#)r5fontlib:font/bitscale.c	1.2"
/*
    ...
    char	*isStringProp;	/* booleans per new property */
{
    ...
    pf->get_bitmaps = /*bitmapGetBitmaps;*/ 0;
    pf->get_extents = /*bitmapGetExtents;*/ 0;
...
#undef MINMAX
	...
	goto bail;
    memset(bitmapFont->bitmaps,0,  bytestoalloc);
	...
	for (i = 0; i < nchars; i++)
	{
		...
		memcpy (pci->bits, opci->bits, size);
	    	glyphBytes += size;
	    }
	}
    }
	...
	if (pfi->firstRow <=  (unsigned int)r 
		&& r <= (unsigned int)pfi->lastRow &&
		pfi->firstCol <= (unsigned int) c 
		&& c <= (unsigned int) pfi->lastCol) {
    ...
    xCharInfo  *pink;
{
    ...
    int         newBit;		/* newBitmap column strchr, by bits */
	...
	llcounter = ll;
	while (llcounter) {
	    int         bit;	/* strchr into glyph row, indicating a bit */

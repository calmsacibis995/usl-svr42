...
#ident	"@(#)libspeedo:speedo/spglyph.c	1.5"
...
#ifdef DEBUG
fprintf(stderr,"BitmapFormatImageRecMin\n");
#endif
	cfv->bpr = 0;
...
#ifdef DEBUG2
fprintf(stderr,"char=%d size=%d\n",(ch-firstChar), charsize);
#endif
	    size += charsize;
	}
	break;
...
#ifdef DEBUG
fprintf(stderr,"BitmapFormatImageRecMaxWidth\n");
#endif
...
#ifdef DEBUG
fprintf(stderr,"BitmapFormatImageRectMax\n");
#endif
	bpr = GLWIDTHBYTESPADDED(FONT_MAX_WIDTH(pinfo), scanlinepad);
	cfv->bpr = bpr;
    ...
    fsBitmapFormatMask fmask;
{
    ...
    int         start,
                ...
                i;
#ifdef DEBUG
fprintf(stderr,"in build_all_sp_bitmaps\n");
#endif
...
fprintf(stderr,"size of bitmaps=%d\n" , glyph_size);
#endif
    pfont->info.glyphsizeSet = 1;
    pfont->info.tot_glyphsize = glyph_size;
    
    if (pfont->rendererPtr->caller == XSERVER_TYPE)
	bitmaps = (pointer) CacheGlyphs(pfont, glyph_size,spmf->num_chars, 0);
        else
        bitmaps = (pointer) xalloc(glyph_size);
    if (!bitmaps)
	return AllocError;
    memset((char *) bitmaps, 0, glyph_size);

    pfont->info.freeWhen = FONT_DONT_FREE;
    	/* set to do not free until rerender code is done */
	/* set up some state */
    cur_spf = spf;
    spf->bitmaps = bitmaps;
    cfv->format = format;

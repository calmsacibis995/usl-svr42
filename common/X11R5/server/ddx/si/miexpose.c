...
#ident	"@(#)siserver:ddx/si/miexpose.c	1.3"
...
/*
 * 	to compile with -Xc
 *      (unsigned short)pSrcDrawable->width & pSrcDrawable->height 
 *	typecasted as (short) to compare with (short) srcBox.x2 & y2.
 */
	if ((srcBox.x1 >= 0) && (srcBox.y1 >= 0) &&
	    (srcBox.x2 <= (short) pSrcDrawable->width) &&
 	    (srcBox.y2 <= (short) pSrcDrawable->height))
	    return NULL;
    ...
    register RegionPtr prgn, other_exposed;
{
	...
	/*
	 * If a save-under image is being restored, subtract
	 * pImageClip from prgn and expose the resulting region.
	 * If the resulting prgn is empty, simply return.
	 */ 
	pSUPriv = SUPrivate(pWin->drawable.pScreen); 
        if (SUTestNoExpose(pWin, pSUPriv))
        {
	    (* pWin->drawable.pScreen->Subtract)(prgn, prgn, pSUPriv->pImageClip);
    	    if (REGION_NIL(prgn))
	        return;
#ifdef SAVETRACE
	    else
	    {
	        siSUPrintRegion(prgn, "----EXPOSING");
	    }
#endif
        }

        /*
         * Check to see if the drawable conflicts with any (non-inferior)
         * save-under windows.
         */
...
#define COUNT_BITS	8
    ...
    /*
     * If a save-under image is being restored, subtract
     * pImageClip from prgn and paint the border/background in
     * the resulting region.  If the resulting prgn is empty,
     * simply return.
     */ 
    pSUPriv = SUPrivate(pScreen); 
    if (prgn && !REGION_NIL(prgn) && SUTestNoExpose(pWin, pSUPriv))
    {
        (* pScreen->Subtract)(prgn, prgn, pSUPriv->pImageClip);
        if (REGION_NIL(prgn))
            return;
#ifdef SAVETRACE
	else
	{
	    siSUPrintRegion(prgn, "----PAINTING");
	}
#endif
    }
#endif

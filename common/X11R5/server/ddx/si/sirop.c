...
#ident	"@(#)siserver:ddx/si/sirop.c	1.2"
...
void
si_rop (pSrc, pDest, srcx, srcy, destx, desty, width, height, alu, planeMask)
    ...
    unsigned	planeMask;
{
    ...
    {
	ERROR_MSG("si_rop: src, dest depths don't match\n");
	return;
    }
    ...
    if (height < 0 || width < 0)
    {
	ERROR_MSG("si_rop:  width and/or height less than zero\n");
	return;
    }

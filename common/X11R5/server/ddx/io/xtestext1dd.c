#ident	"@(#)siserver:ddx/io/xtestext1dd.c	1.3"
...
#include "misc.h"
#include "dixstruct.h"
#include "keynames.h"			/* USL */
#include "xwin_io.h"			/* USL */
#include <sys/xque.h>			/* USL */
...
static ushort	XTestMask[ 8 ] = { 0, 0x4, 0x2, 0x1, 0, 0, 0, 0 };

XTestQueueNode	*XTestQueueNodeFreeList = (XTestQueueNode *)0;

static Bool	command_key_flag = FALSE;

extern XTestQueue	xTestQ;
...
#define SetMostSigBit(x)	((x) |= 0x80)

/*
** End of AT&T Additions
*/
 ...
 *
 * AT&T: the following two variables names are being changed
 *       from XTest... to xTest... because of double defined
 *       errors when linking the server using libX11_s.a.
 */
 ...
 */
KeyCode			xtest_command_key = K_F12 + TABLE_OFFSET;
...
CARD32		mode;
{
	 ...
	 * AT&T Bug Fix: added "& ~(XTestEXCLUSIVE)"
	 */
...
{	
	...
	/* AT&T: added to contstrain logical mouse to physical screen */

	/* TEMP attConstrainXY( (short *)&xtest_mousex, (short *)&xtest_mousey ); */

        constrainXY2Scr ((short *)&xtest_mousex, (short *)&xtest_mousey);
...
short	my;
{
	...
	{ 
		...
		/* attConstrainXY( (short *)&xtest_mousex, (short *)&xtest_mousey ); */
...
/* Added by AT&T */
	...
	if ( keycode == xtest_command_key )
	{
		if ( keystate == KeyPress )
			command_key_flag = !command_key_flag;
	}
	else
		command_key_flag = FALSE;

/* End of AT&T addition */

	/* return TRUE if the event should be passed on to DIX */
	if (exclusive_steal)
		return FALSE;
	else
		return keycode != xtest_command_key || !command_key_flag;
}
...
XTestGenerateEvent( dev_type, keycode, keystate, mousex, mousey )
	int	dev_type;	/* IGNORED: == device_id in hi-level calls */
	int	keycode;	/* keycode or button_num in hi-level calls */
	int	keystate;	/* XTestKEY_DOWN/_UP */
	int	mousex;		/* IGNORED */
	int	mousey;		/* IGNORED */

{
	...
	node = XTestQueueNodeFreeList ? XTestQueueNodeFreeList :
			(XTestQueueNode *)Xalloc( sizeof( XTestQueueNode ));

	if ( XTestQueueNodeFreeList )
		XTestQueueNodeFreeList = XTestQueueNodeFreeList->next;

	node->next = (XTestQueueNode *)0;
	event = &node->xqevent;

	/*
	** Initialize xqEvent structure depending on keycode.
	*/

	if ( keycode < 8 )	/* Mouse Button */
	{
		event->xq_type = XQ_BUTTON;
		node->variant1 = keystate;
		node->variant2 = XTestMask[ keycode ];
	}
	else	/* Keyboard */
	{
		/*
		** Initialize xqEvent structure.  I must subtract
		** TABLE_OFFSET from keycode because the server
		** calls ddx/i386/kbd.c:ProcessI386Input(), after
		** removing xqEvent from queue, which adds TABLE_OFFSET
		** to keycode.
		*/

		if ( keystate == XTestKEY_UP )
			SetMostSigBit( keycode );

		event->xq_type = XQ_KEY;
		event->xq_code = keycode;
...
#endif
			event->xq_code -= TABLE_OFFSET;
	}

	/*
	** Finish initializing xqEvent structure.
	**
	** xq_x/y are relative motions, NOT absolute.
	*/

	event->xq_x = 0;
	event->xq_y = 0;
	event->xq_time = queue->xq_curtime;

	/*
	** Enqueue the simulated input event in the xTestQueue.
	*/

	if ( XTestQueueEmpty( xTestQ ))
		xTestQ.head = xTestQ.tail = node;
	else
		xTestQ.tail = xTestQ.tail->next = node;
	...
	int	dev_type;	/* IGNORED: == device_id in hi-level calls */
{
	...
	/*
	** I should change XTestMovePointer and all subsequent
	** calls to take short's rather than int's.
	*/

	/* attConstrainXY( (short *)&x, (short *)&y ); */

         constrainXY2Scr ((short *)&x, (short *)&y);

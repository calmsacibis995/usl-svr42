...
#ident	"@(#)siserver:dix/dixfonts.c	1.8"
/*copyright	"%c%*/
/************************************************************************
...
extern pointer cmdline_fontpath;
...
#ifdef DEBUG
fprintf(stderr,"OpenFont pfontname=%s\n",pfontname);
#endif
    ...
    int         nProtoCCIStructs;
{
    ...
    for (r = pFont->info.firstRow;
	    ninfos < (unsigned) nProtoCCIStructs && r <= (unsigned short) pFont->info.lastRow;
	    r++) {
	i = 0;
	for (c = (unsigned short) pFont->info.firstCol; c <= (unsigned short) pFont->info.lastCol; c++) {
	    chars[i++] = r;
	    chars[i++] = c;
	}
	ret = (*pFont->get_metrics) (pFont, ncols, chars, TwoD16Bit,
			       &count, charInfos);
...
InitFonts ()
{
    static long mincache, maxcache=0;
    static int logcachestats=0;
    int result=0; 

    patternCache = MakeFontPatternCache();
    XFontFileRegisterFpeFunctions(XSERVER_TYPE);
    if (cmdline_fontpath != NULL) {
		defaultFontPath = (char *) cmdline_fontpath;
			/* server commandline takes precedences */
    } else {
       defaultFontPath = (char *) GetFontDefaults(defaultFontPath);
    }
    GetCachesize(&mincache, &maxcache, &logcachestats);
#ifdef DEBUG
	fprintf(stderr,"cachesize=%ld lowwater=%ld\n",maxcache,mincache);
#endif
    if ((mincache <= 0) && (maxcache <= 0)) {
        mincache = DEFAULT_MINCACHE;
	maxcache = DEFAULT_MAXCACHE;
	} 
    if (maxcache <= 0) maxcache = DEFAULT_MAXCACHE;
    if (maxcache <  MIN_CACHESIZE) maxcache = MIN_CACHESIZE;
    if (mincache <= 0) mincache = maxcache * LOWWATER_PERCENT;
    if (mincache > maxcache) mincache = maxcache * LOWWATER_PERCENT;

#ifdef DEBUG
	fprintf(stderr,"cachesize=%ld lowwater=%ld\n",maxcache,mincache);
#endif

    if (!InitXwinFontCache(mincache, maxcache, logcachestats)) return AllocError;
    ...
    int        *num;
{
     ...
     */
#ifdef DEBUG
fprintf(stderr,"res.x_resolution =%d\n",res.x_resolution);
#endif
#ifndef USL

#ident	"@(#)siserver:dix/xtestext1di.c	1.2"
...
#include "dixstruct.h"
#include <sys/types.h>			/* LES */
#include <sys/xque.h>			/* LES */
...
int 		XTestSimulationFlag = FALSE;
ClientPtr	XTestSimulationClient;
...
/*
 * holds the resource types assigned to this extension
 */
static RESTYPE		XTestType1;
static RESTYPE		XTestType2;
/*
...
static int ProcTestStartSimulation();
static int ProcTestStopSimulation();
static int SProcTestStartSimulation();
static int SProcTestStopSimulation();
...
static void	XTestSimulationClientGone();
...
XTestExtension1Init()
{
	...
	if (extEntry)
	{
		...
		/*
		 * get the resource types for this extension
		 */
		XTestType1 = CreateNewResourceType(XTestCurrentClientGone);
		if (XTestType1 == 0)
		{
			FatalError("XTestExtension1Init: CreateNewResourceType failed\n");
		}
		XTestType2 = CreateNewResourceType(XTestSimulationClientGone);
		if (XTestType2 == 0)
		{
			FatalError("XTestExtension1Init: CreateNewResourceType failed\n");
		}
	...
	register ClientPtr	client;
{
	...
	else if (stuff->data == X_TestStartSimulation)
	{
		return(ProcTestStartSimulation(client));
	}
	else if (stuff->data == X_TestStopSimulation)
	{
		return(ProcTestStopSimulation(client));
	}
	else
	{
	...
	register ClientPtr	client;
{
	...
	{
		return(SProcTestStartSimulation(client));
	}
	else if (stuff->data == X_TestStopSimulation)
	{
		return(SProcTestStopSimulation(client));
	}
	else
	{
		SendErrorToClient(client,
	...
	register ClientPtr	client;
{
	...
	/*
	** AT&T: Add && XTestSimulationFlag to condition
	*/
	if (XTestSimulationFlag &&
		(playback_client == NULL || playback_client == client))
	{
		/*
	...
	register ClientPtr	client;
{
	...
	** AT&T
	*/
	xTestQ.head = xTestQ.tail = (XTestQueueNode *)0;
	XTestSimulationFlag = FALSE;
	/*
	 * defined in xtest1dd.c
	 */
	stop_stealing_input();	
...
/*
** The following two procedures have been added to solve the
** deadlock created when clients grab the server during
** simulation by tmon.
*/

/*****************************************************************************
 *
 *	ProcTestStartSimulation
 *
 *
 */
static int
ProcTestStartSimulation(client)
	register ClientPtr	client;
{
	REQUEST(xTestGetInputReq);
	REQUEST_SIZE_MATCH(xTestGetInputReq);
	...
	if ( !XTestSimulationFlag )
	{
		XTestSimulationFlag = TRUE;
		XTestSimulationClient = client;
		/*
		 * Set up a resource associated with the client using this
		 * function so that this extension gets called when the 
		 * client "goes away".  This allows this extension to
		 * clean up the server state.
		 */
		AddResource( FakeClientID(client->index),
			    XTestType2,
			    0);
		return(Success);
	}
...
/*****************************************************************************
 *
 *	ProcTestStopSimulation
 *
 *
 */
static int
ProcTestStopSimulation(client)
	register ClientPtr	client;
{
	REQUEST(xTestGetInputReq);
	REQUEST_SIZE_MATCH(xTestGetInputReq);
	...
	{
		XTestSimulationFlag = FALSE;
		FreeResource(FakeClientID(client->index), RT_NONE);
		return(Success);
	}
	else
	{
...
/*****************************************************************************
 *
 *	SProcTestStartSimulation
 *
 *
 */
static int
SProcTestStartSimulation(client)
	register ClientPtr	client;
{
	register char	n;
...
/*****************************************************************************
 *
 *	SProcTestStopSimulation
 *
 *
 */
static int
SProcTestStopSimulation(client)
	register ClientPtr	client;
{
	register char	n;
...
/*ARGSUSED*/
static void
XTestSimulationClientGone(value, id)
	int	value;
	XID	id;
{
	extern int	XTestacknowledge;

	XTestSimulationFlag = FALSE;
	XTestacknowledge = FALSE;
}

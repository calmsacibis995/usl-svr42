...
#ident	"@(#)siserver:dix/window.c	1.5"
...
/* #if (defined(SYSV) && defined(NO_BACK_SAVE)) */
    ...
    Window wid;
{
    ...
    CrushTree(pWin);
    pWin->firstChild = pWin->lastChild = (WindowPtr) NULL;
	...
	switch (index)
	  ...
	  case CWCursor:
		...
		if (pCursor == (CursorPtr) None)
		{
		    ...
		    if (pWin->optional)
		    {
			oldCursor = pWin->optional->cursor;
...
#ifdef XWIN_SAVE_UNDERS
    /*                                                                               * Added: Nov. 1989
     * Unfortunately there are no hooks into DDX via pScreen here ...
     * Must check to see if change in stacking order affects save-under
     * windows
     */
     siSUCheckStack(pWin, pFirstChange);
#endif
    ...
    WindowPtr pSib;
{
	...
	if (pWin->valdata)
 	{
	    pWin->valdata->before.resized = shrunk;
    ...
    unsigned short width;
{
    ...
    pScreen = pWin->drawable.pScreen;
    pParent = pWin->parent;
    if (WasViewable && (unsigned) width < oldwidth)
	anyMarked = MarkOverlappedWindows(pWin, pWin);
    ...
    int smode;
{
    ...
    if (!pSib) {
	    ...
	    box.x1 = x + pWin->parent->borderSize.extents.x1;
	    box.y1 = y + pWin->parent->borderSize.extents.y1;
	    box.x2 = x + (int)w + pWin->parent->borderSize.extents.x1;
	    box.y2 = y + (int)h + pWin->parent->borderSize.extents.y2;
    }

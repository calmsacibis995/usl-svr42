#ident	"@(#)siserver:dix/dispatch.c	1.8"
...
extern void		waitForSiguser2();
extern int		waiting_for_acquire;

#ifdef TIMING

#include <sys/times.h>
char		ThisTheFirstClient;
long		starting, ending;
struct tms	begintiming, endtiming;
...
#ifdef i386
#ifdef TIMING
    ThisTheFirstClient = 1;
#endif /* TIMING */

    setjmp(startagain);         /* set a jump point for longjmp */
#endif

    while (!dispatchException)
    {
#ifdef i386
	if (waiting_for_acquire > 0)	/* Is a VT switch request pending? */
            waitForSiguser2();
...
#ifdef XTESTEXT1
	if(queue->xq_head != queue->xq_tail || XTestQueueNotEmpty( xTestQ ))
#else
	...
	*****************/
	...
	while (!dispatchException && (--nready >= 0))
	{
...
#ifdef XTESTEXT1
	    if (grabbingClient && (client != onlyClient) && !XTestSimulationFlag)
#else
	        ...
	        if (result <= 0) 
		{
		    ...
		    else 	/* if length == 0, return BadLength */
			result = BadLength;
...
#ifdef TIMING
   if (starting > 0) {
        if ((ending = times (&endtiming)) < 0)
            ErrorF ("times() failed\n");
        else {
            ErrorF ("Server timing: Real (%ld) Sys (%ld) User (%ld)\n",
                        ending - starting,
                        endtiming.tms_utime - begintiming.tms_utime,
                        endtiming.tms_stime - begintiming.tms_stime);
        }
   }
#endif /* TIMING */
...
/* see comments in server/include/dix.h */
#ifdef OLDR4
    ...
    register ClientPtr client;
{
	...
	reply->sequenceNumber = client->sequence;
	ret = QueryFont( pFont, reply, nprotoxcistructs);
	if (ret != Successful) return(BadFontName);
    ...
    register ClientPtr client;
{
    ...
    {
        if ((stuff->depth != 1) ||
	    ((unsigned int) stuff->leftPad
		 >= (unsigned int ) screenInfo.bitmapScanlinePad))
            return BadMatch;
...
extern int GetHosts();
    ...
    /* result = GetHosts(&pdata, &nHosts, &len, &reply.enabled); rjk */
    result = GetHosts(client, &pdata, &nHosts, &len, &reply.enabled);
    if (result != Success)
	return(result);
    reply.type = X_Reply;
    reply.sequenceNumber = client->sequence;
    ...
    register ClientPtr client;
{
    ...
    stuff->reqType = 2;
    stuff->length += (((unsigned) prefix->nbytesAuthProto + 3) >> 2) +
		     (((unsigned) prefix->nbytesAuthString + 3) >> 2);
    if (client->swapped)
    {
    ...
    register ClientPtr client;
{
	...
	csp.length = ((unsigned) csp.lengthReason + 3) >> 2;

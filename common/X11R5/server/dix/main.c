...
#include "opaque.h"
#include "servermd.h"
extern void SetSchedulerClass();
extern char *getenv();	/* rjk */
    ...
    char	*argv[];
{
    ...
    InitMiscellaneous (argc, argv);	/* see ../os/sysV/utils.c */
    SetSchedulerClass(); 	/* Set scheduler class */
	...
	if (InitFonts () != Success)
		ErrorF("\nFailed to allocate XwinFontCache\n");
	if (SetDefaultFontPath(defaultFontPath, &i) != Success)
	    ErrorF("\nfailed to set default font path\n '%s'", defaultFontPath);
	/*if (!SetDefaultFont(defaultTextFont)) {
	    ErrorF("\nCould not open Server's default font '%s' \n", defaultTextFont);
	    ErrorF("Server cannot run without this font.\n");
	    ErrorF("Check the default font path for this path, \n");
	    ErrorF("check that a valid fonts.dir file exists, and \n");
	    ErrorF("that the font exists and is readable.\n");
		ddxGiveUp (); exit(1); 
	}
*/
         if (!(rootCursor = CreateRootCursor(defaultCursorFont, 0))) {
	    ErrorF("\ncould not open default cursor font '%s'\n",
		       defaultCursorFont);

	    ErrorF("Server cannot run without this font.\n");
            ErrorF("Check the default font path for this path, and\n");
            ErrorF("check that a valid fonts.dir file exists, and that \n");    
            ErrorF("the font exists, and is readable.\n");                                  	ddxGiveUp (); exit(1); /* TMK attexit(1); */
	}
	if (!SetDefaultFont(defaultTextFont)) {
	    ErrorF("\nCould not open Server's default font '%s' \n", defaultTextFont);
	    ErrorF("Server cannot run without this font.\n");

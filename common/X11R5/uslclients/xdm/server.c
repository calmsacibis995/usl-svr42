...
struct display	*d;
{
	...
	return FALSE;
    if (oldpid = StoreServerPid (pid)) {
	if (oldpid == -1)
		LogError ("Can't create server pid file\n");
	else
		LogError ("Can't lock server pid file, another xdm is running (pid %d)\n", oldpid);
	exit (EXIT_XDM);
    }
    return TRUE;
}

...
Screen *screen;
{
	...
	if (gp = (DmGlyphPtr)DtGetData(screen, DM_CACHE_PIXMAP, NULL, 0)) {
		gp->count++;
		return(gp);
	}

	if (gp = (DmGlyphPtr)malloc(sizeof(DmGlyphRec))) {
		gp->pix = XCreateBitmapFromData(DisplayOfScreen(screen),
		...
		DtPutData(screen, DM_CACHE_PIXMAP, NULL, 0, (void *)gp);
	}
...
char *masktype;
{
	...
	if (gp = DtGetData(screen, DM_CACHE_PIXMAP, (void *)fullpath, len)) {
	...
	if ((*type == 'b') || XReadPixmapFile(dpy, RootWindowOfScreen(screen),
	...
	if (foundname) {
		if (XReadBitmapFile(dpy, RootWindowOfScreen(screen),
			...
			if ((width != gp->width) || (height != gp->height)) {
				fprintf(stderr, "icon mask dimension (%dx%d)"
				" does not match icon dimension (%dx%d)\n",
	...
	DtPutData(screen, DM_CACHE_PIXMAP, gp->path, len, gp);
bye:
...
unsigned int height;
{
	...
	if (gp = DtGetData(screen, DM_CACHE_PIXMAP, (void *)name, len)) {
		gp->count++; /* bump usage count */
		return(gp);
	}
...
DmGlyphPtr gp;
{
	if (gp && --(gp->count) == 0) {
		...
		if (gp->path) {
			DtDelData(screen, DM_CACHE_PIXMAP, gp->path,
					strlen(gp->path) + 1);
			free(gp->path);
		}
		else
			DtDelData(screen, DM_CACHE_PIXMAP, NULL, 0);
		XFreePixmap(DisplayOfScreen(screen), gp->pix);

...
char *argv[];
{
	...
	if (argc < 4) {
		Toplevel = XtInitialize((char *)str_wksh, (char *)str_wksh, NULL, 0, 0, NULL);
	} else {
		int moargc = argc - 3;

		Toplevel = XtInitialize(argv[2], argv[3], NULL, 0, &moargc, &argv[3]);
	...
	XtAddConverter(XtRString, XtRPixmap, 
		WkshCvtStringToPixmap, screenConvertArg, 1);
	XtAddConverter(XtRString, XtRWidget, 
		WkshCvtStringToWidget, NULL, 0);
	...
	XtAddConverter(XtRString, XtRCallback, 
		WkshCvtStringToCallback, NULL, 0);
...
char *arg0;
{
	printerr(arg0, usagemsg(CONSTCHAR "usage: %s [-gcsrGCS] Widgetid arg ...", arg0), NULL);
	return(1);
}
...
char **argv;
{
	wtab_t *w;
	char *arg0 = argv[0];
	int ind, next, indices[MAXARGS], numindices, usrdatamode;
	Arg args[2];
	char *mode;
	register int i, j;
	int pos, num;
	...
	case 'p':
		/*
		 * put data at a position args: position string
		 */
	...
	case 'g':
		 ...
		 */
		{
			...
			for (i = 0; i < numindices; i++) {
				...
				if (items[indices[i]] != NULL && str != NULL) {
					altprintf("%s\n", str);
				} else {
					printerr(arg0, CONSTCHAR "listget: an item is NULL in list:", w->envar);
				}
...
Boolean flag;	/* if TRUE, then set mnemonics, else just find existing */
{
	...
	if (XtIsSubclass(wid, xmManagerWidgetClass) || XtIsShell(wid)) {
		...
		if (flag && resource != NULL) {
			...
			XtGetValues(wid, kidargs, 1);
			XtSetArg(kidargs[0], XmNmnemonic, (XtArgVal)&m);
			XtGetValues(wid, kidargs, 1);
			 ...
			 */
			scoretab = XtMalloc(n = strlen(label));
...
Boolean shortflag;
{
	...
	if (class == xmTextWidgetClass) {
		...
		XtGetValues(wid, args, 1);
		if (str != NULL) {
			cvt = XtMalloc(strlen(str)*4 + 1);
#if defined(SVR4) || defined(SVR4_0)
			strecpy(cvt, str, NULL);
#else
			strcpy(cvt, str);
#endif
			XFree(str);

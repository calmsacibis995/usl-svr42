...
char *v;
{
	...
	if (v == NULL || v[0] != 'W' || (len = strlen(v)) < 2 ||
	...
	if (W[index]->type == TAB_WIDGET && W[index]->w == NULL) {
		if (arg0)
...
char *argv[];
{
	...
	for (i = 1; i < argc; i++) {
		...
		if ((address = fsym(classrec, -1)) == NULL) {
			printerr(argv[0], usagemsg(CONSTCHAR "could not find class record called: %s ", classrec), dlerror());
		} else {
			classtab = (classtab_t *)XtMalloc(sizeof(classtab_t));
			memset(classtab, '\0', sizeof(classtab_t));
			classtab->cname = strdup(argv[i]);

...
XrmValuePtr fval, toval;
{
	 ...
	 */
	if (XReadBitmapFile(display, root, path, &width, &height, &pixmap, &hotx, &hoty) != BitmapSuccess)  {
		format = XYBitmap;
		/*
...
void
WkshCvtStringToWidget(args, nargs, fval, toval)
XrmValuePtr args;
Cardinal *nargs;
XrmValuePtr fval, toval;
{
	...
	if (strcmp(wname, CONSTCHAR "NULL") == 0) {
		toval->addr = NULL;
		toval->size = 0;
		return;
	}
	...
	XtWarningMsg(CONSTCHAR "cvtStringToWidget", CONSTCHAR "badname", CONSTCHAR "XtToolkitError",
		CONSTCHAR "cannot find a widget with that name", NULL, 0);
	toval->addr = NULL;
	toval->size = 0;
}

void
WkshCvtStringToCallback(args, nargs, fval, toval)
XrmValuePtr args;
Cardinal *nargs;
XrmValuePtr fval, toval;
{

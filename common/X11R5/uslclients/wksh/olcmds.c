...
#ident	"@(#)wksh:olcmds.c	1.7"

/* X/OL includes */

#include <stdio.h>
#include <signal.h>
#define malloc do_not_redefine_malloc
#include "name.h"
#undef malloc
...
char *argv[];
{
	...
	XtAddConverter(str_XtRString, str_XtRWidget, 
	...
	XtAddConverter(str_XtRBool, str_XtRString, 
		WkshCvtBooleanToString, NULL, 0);
	XtAddConverter(str_XtRString, str_XtRCallback, 
...
char **argv;
{
	...
	default:
		printerr(arg0, usagemsg(CONSTCHAR "usage: %s [-c | -e | -r ] Widgetid arg ...", arg0), NULL);
		return(1);
	}
	...
	case 'm':
		{
			...
			if (t.size && t.addr) {
				...
				if (var) {
					if ((wtab = widget_to_wtab(wid)) != NULL) {
...
Boolean shortflag;
{
	...
	if (class == textFieldWidgetClass) {
		...
		if (buf != NULL) {
			cvt = XtMalloc(strlen(buf)*4 + 1);
#if defined(SVR4) || defined(SVR4_0)
			strecpy(cvt, buf, NULL);
#else
			strcpy(cvt, buf);
#endif
			XFree(buf);
...
OlTextMotionCallDataPointer mods;
{
	...
	if (ops == NULL || ops->active == FALSE)
		return;
	if (ops->autotraversal && ops->maxcursor > 0 && mods->new_cursor >= ops->maxcursor) {
		OlCallCallbacks(ops->textfield, XtNverification, ops);
		OlMoveFocus(ops->textfield, OL_NEXTFIELD, 0);
		OlTextEditSetCursorPosition(ops->textedit, 0, 0, 0);
		mods->ok = FALSE;
	}

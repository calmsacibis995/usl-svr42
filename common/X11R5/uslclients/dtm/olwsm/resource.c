...
/* ident	"@(#)dtm:olwsm/resource.c	1.15" */
	...
	List *			buffer;
{
	XChangeProperty(DISPLAY, ROOT, XA_RESOURCE_MANAGER, XA_STRING, 8,
		PropModeReplace, (unsigned char *)buffer->entry, buffer->count);
}
	...
	DNODE **		programs;
{
	...
	while (p = (char *)list_next(&I)) {
		...
		case COLLECTING_NAME:
			...
			else if (COLON(c)) {  /* no mnemonics delimiter */
				name_size = (p - name) - space;
   				value_mnemonic = "";
                                value_mnemonic_size = 0;
				state = LOOKING_FOR_VALUE;
			}
			else if (POUND(c)) {  /* found mnemonics delimiter */
				name_size = (p - name) - space;

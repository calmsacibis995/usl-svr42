...
#ifndef NOIDENT
#pragma ident	"@(#)dtadmin:nfs/verify.c	1.10"
#endif
...
verifyLabel(char * label, Gizmo popup)
{
    ...
    {
	SetMessage(popup, TXT_WhiteInLabel, Popup);
	return INVALID;
    }
    return VALID;
}
...
verifyRemotePath(char * path, Gizmo popup)
{
    ...
    if ( *path != '/')
    {
	SetMessage(popup, TXT_BadRemotePath, Popup);
	return INVALID;
    }
    return VALID;
}
...
verifyShareOptions(char *options, Gizmo popup)
{
    ...
    {
	SetMessage(popup, TXT_SpaceNotAllowed, Popup);
	return INVALID;
    }
...
verifyMountOptions(char *options, Gizmo popup)
{
	...
	SetMessage(popup, TXT_OptionsNotVerified, Popup);
	return VALID;
    }
    ...
    {
	SetMessage(popup, TXT_BadMountOptions, Popup);
	return INVALID;
    }
}

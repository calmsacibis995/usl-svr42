...
#ifndef NOIDENT
#pragma ident	"@(#)dtadmin:nfs/share.c	1.11"
#endif
...
isShared(struct share * sharep)
{
    ...
    {
	if (errno != ENOENT)
	    SetMessage(MainWindow, TXT_sharetabOpenError, Base);
	return False;
    }
...
ShareIt(struct share *sharep, DmObjectPtr op)
{
	...
	SetMessage(MainWindow, TXT_ShareFailed, Base);
    }
    return status;
}
...
UnShareIt(struct share *sharep, DmObjectPtr op)
{
	...
	SetMessage(MainWindow, TXT_UnShareFailed, Base);
    }
    return status;
}

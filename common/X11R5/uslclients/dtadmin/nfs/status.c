...
#ifndef NOIDENT
#pragma ident	"@(#)dtadmin:nfs/status.c	1.22"
#endif
...
static MenuGizmo  statusMenu = { NULL, "statusMenu", "statusMenu",
...
{ NULL, "statusPopup", TXT_StatusTitle, &statusMenu, statusGizmos,
...
isNFSRunning()
{
    ...
    if (! WIFEXITED(retval))
    {
	SetMessage(nfsw-> statusPopup, TXT_NFSstatusFailed, Popup);
...
dfmounts()
{
	...
	if (NFSserver == q_Yes)
	    SetMessage(nfsw-> statusPopup, TXT_dfmountsFailed, Popup);
...
PostStart(XtPointer client_data, int *source, XtInputId *id)
{
    ...
    if (! WIFEXITED(status))
    {
	SetMessage(nfsw-> statusPopup, TXT_NFSstartFailed, Popup);
...
PostStop(XtPointer client_data, int *source, XtInputId *id)
{
    ...
    if (! WIFEXITED(status))
    {
	SetMessage(nfsw-> statusPopup, TXT_NFSstopFailed, Popup);
...
startrpc()
{
    ...
    if (! WIFEXITED(status))
    {
	SetMessage(nfsw-> statusPopup, TXT_RPCstartFailed, Popup);

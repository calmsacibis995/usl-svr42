...
#ifndef NOIDENT
#pragma ident	"@(#)dtadmin:nfs/localAdv.c	1.29"
#endif
...
static MenuItems getHostItems[] = {{ True, TXT_GetHost, CHR_GetHost }, {0}};
...
bringDownPopups(Widget parent, Boolean force)
{
	...
	if ((shell = GetPopupGizmoShell(nfsw-> hostPopup)) != NULL)
	{
	    SetWMPushpinState(XtDisplay(shell), XtWindow(shell),
			      WMPushpinIsOut); 
	    XtPopdown(shell);
...
addException(defaultAccessIndex type)
{
    ...
    if (verifyHost((char *)localSetting.host.current_value,
		   nfsw-> localPropertiesPrompt) == INVALID)
	return;
    insertException(headPtr, localSetting.host.current_value, type);

    DEBUG0("addException: before manipulate\n");
    ManipulateGizmo(InputGizmoClass, &hostG, ReinitializeGizmoValue);
...
updateData()
{
    ...
    DeleteLocalCB((Widget)NULL, (XtPointer)NULL, (XtPointer)flags);
    newLocal(shared);
    return;
}
...
getHostCB(Widget wid, XtPointer client_data, XtPointer call_data)
{
    ...
    SetMessage(nfsw-> localPropertiesPrompt, "", Popup);
    HostCB(wid, (XtPointer)&hostG, call_data);
    return;
}
...
postVerifyCB(Widget wid, XtPointer client_data, XtPointer call_data)
{
	...
	if (result < 0)
	{
	    SetMessage(popup, TXT_DirNotCreated, Popup);
	    break;
	}
	setLabel();

...
#ifndef NOIDENT
#pragma ident	"@(#)dtadmin:nfs/remoteProp.c	1.34"
#endif
...
DEBUG1("mount opts = \"%s\"\n", vfs_entry.vfs_mntopts);
    ...
    {
        SetMessage(nfsw-> remotePropertiesPrompt, TXT_vfstabOpenError, Popup);
	return;
    }    
...
DEBUG4("bootmount=\"%s\", %x, access=\"%s\", %x\n",
    ...
    {
        SetMessage(nfsw-> remotePropertiesPrompt, TXT_dtvfstabOpenError, Popup);
	return;
    }
...
RemotePropertyCB(Widget w, Boolean displayPopup, XtPointer call_data)
{
    ...
    OlVaFlatSetValues(RemoteMenu.child, PropApply,
		      XtNlabel, GetGizmoText(TXT_Apply),
		      XtNmnemonic, *(GetGizmoText(CHR_Apply)), NULL);
...
OlTextFieldVerify       *tf_verify;
{
    ...
    if (verifyHost(host, nfsw-> remotePropertiesPrompt) == INVALID)
    {
	MoveFocus(remoteHostG.textFieldWidget);
	...
	SetMessage(nfsw-> remotePropertiesPrompt, TXT_dfsharesFailed, Popup);
	standardCursor();
	return;
    }
	...
	case 34:
	    ...
	    SetMessage(nfsw-> remotePropertiesPrompt, buf, Popup);
	    break;
	case 35:
...
verifyAllFields()
{
    ...
    if (verifyHost((char *)remoteSetting.host.current_value, popup) == INVALID) 
	return INVALID;
...
updateData()
{
    ...
    DeleteRemoteCB((Widget)NULL, (XtPointer)NULL, (XtPointer)flags);
    newRemote(mounted);
    return;
}
...
getHostCB(Widget wid, XtPointer client_data, XtPointer call_data)
{
    ...
    SetMessage(nfsw-> remotePropertiesPrompt, " ", Popup);
    HostCB(wid, (XtPointer)&remoteHostG, call_data);
    return;
}
...
findFolderCB(Widget wid, XtPointer client_data, XtPointer call_data)
{
	...
	fileG-> directory = STRDUP(getenv("HOME"));
...
postVerifyCB(Widget wid, XtPointer client_data, XtPointer call_data)
{
	...
	if (result < 0)
	{
	    SetMessage(nfsw-> remotePropertiesPrompt, TXT_DirNotCreated, Popup);
	}
	else
	{

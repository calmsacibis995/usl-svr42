...
#ifndef NOIDENT
#pragma ident	"@(#)dtadmin:nfs/file.c	1.9"
#endif
/*
		...
		XtCallbackProc postVerifyCB)
{
	...
	*value = path;
	...
	case EINTR:
	    break;		/* try again */
	case EACCES:
	    SetMessage(popup, TXT_PathAccess, Popup);
	    return INVALID;
	    break;
	case ENOLINK:
	    /* FALL THRU */
	case EMULTIHOP:
	    SetMessage(popup, TXT_PathMustBeLocal, Popup);
	    return INVALID;
	    break;
	case ENOENT:
	    ...
	    break;
	default:
	    SetMessage(popup, TXT_BadPath, Popup);
	    return INVALID;
	    break;
	}
    }

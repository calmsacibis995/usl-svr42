...
#ifndef NOIDENT
#pragma	ident	"@(#)dtadmin:dashboard/dashboard.c	1.35"
#endif
/*
...
#define	HELP_PATH	"dtadmin/dashbrd.hlp"
...
/*
	...
	*	using the first 12 lines of usr/lib/locale/$LANG/LC_TIME
	*/
...
void	ResetDate()
{
	...
	if (strcmp(cur_date, date_buf)) {
		...
		XtSetArg(arg[0], XtNstring, date_buf);
...
ClockReset()
{
	...
	sprintf(buf, "%s %02d%02d%02d%02d%02d", getuid()==0? DATECMD0: DATECMD1,
				n[1], n[2], n[3], n[4], n[0]);
	...
	XtIntervalId	interval;
{
	...
	if ((cmdpipe = popen(ptr+1,"r")) == NULL)
		FooterMsg(GetGizmoText(string_cantUpdate));
	else {
	...
	char	*name, *flag, *mntpt;
{
	...
	XtSetArg(arg[0], XtNspan,		((DASH_WIDTH-inch/2)/2));
	...
	XtSetArg(arg[8], XtNshowValue,		TRUE);	/* for Motif mode */
	w_g = XtCreateManagedWidget("gauge", gaugeWidgetClass, w_cap, arg, 9);
	...
	XtSetArg(arg[7], XtNheight,		(0.75*inch));
	...
	char	*device;
{
	...
	sprintf(cmdbuf, "LANG=C /sbin/dfspace %s", device);
	if ((cmdpipe = popen(cmdbuf,"r")) == NULL) {
		FooterMsg(GetGizmoText(string_cantGauge));
		return 0;
	}
	else {
	...
	while (fgets(buf, 80, cmdpipe) != NULL) {
		if (strstr(buf,"Memory") != NULL) {
			for (ptr=buf; *ptr == ' '; ptr++)
				;
			strcpy(memline,ptr);
			memline[strlen(memline)-1] = 0;
		}
		else if (strstr(buf,"Math") != NULL) {
			for (ptr=buf; *ptr == ' '; ptr++)
				;
			strcpy(mathline,ptr);
			mathline[strlen(mathline)-1] = 0;
		}
	}
	pclose(cmdpipe);
	cmdpipe = (FILE *)-1;
	...
	char	*name, *flag;
{
	...
	for (c = '1'; ; c++) {
		sprintf(buf, "ctape%c", c);
top:		if ((ptr = DtamGetDev(buf, FIRST)) == NULL)
			break;
...
{   
	...
	InitializeMemutil();
#endif
	if ((ptr=getenv("LC_TIME")) != NULL || (ptr=getenv("LANG")) != NULL) {
		FILE	*monfile;
		int	n;
	...
	w_toplevel = XtInitialize("dash","System_Status",NULL,0,&argc,argv);
	...
	XtSetValues(base.scroller, arg, 1); 
	XtSetArg(arg[0], XtNheight, 9*DASH_WIDTH/10);

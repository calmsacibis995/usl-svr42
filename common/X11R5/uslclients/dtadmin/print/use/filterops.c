...
#ifndef NOIDENT
#ident	"@(#)dtadmin:print/use/filterops.c	1.12"
#endif
...
static char	**ParseFilterOpts (FilterOpts *filterOpts, int numOpts);
...
static HelpText FilterHelp = {
    TXT_filterHelp, HELP_FILE, TXT_filterHelpSect,
};
...
FilterOptsCB (Widget widget, XtPointer client_data, XtPointer call_data)
{
    ...
    page->filterOptions = ParseFilterOpts (&page->workFilter,
					   page->numOptions);
		...
		FilterOpts *filterOpts)
{
    ...
    {
	/* Get the defaults */
	for (dflts=properties->printer->dflts; dflts; dflts->next)
	{
	 ...
	 * the modes string from the request.
	 */
...
{
    *pDst = *pSrc;
    pDst->inputType = strdup (pDst->inputType);
    pDst->opts = strdup (pDst->opts);
    pDst->modes = strdup (pDst->modes);
 ...
 */
static char **
ParseFilterOpts (FilterOpts *filterOpts, int numOpts)
{
    char	**optionList;
    char	**pOpt;
    char	**modeList;

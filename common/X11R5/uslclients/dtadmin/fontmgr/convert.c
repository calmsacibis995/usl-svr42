...
#ident	"@(#)dtadmin:fontmgr/convert.c	1.2"
  ...
  */
{
  ...
  if (RESULT == EOF) {
    fprintf (stderr, "Fontmgr: Error:  Premature end of input.\n");
    return -1;
  } else
    return (RESULT);
}

BinaryToAscii(in_file_name, out_file_name)
  ...
  */
{
  ...
  do {
    ...
    if (Header.Check != CHECK_BYTE) {
      fprintf (stderr, "Fontmgr: Error: Invalid segment header.  (Check byte 0x%02x)\n", Header.Check);
      fclose( in_file);
      fclose( out_file);
      success = FALSE;
      return success;
	...
	putc ('\n', out_file);
     ...
     default:
      fprintf (stderr, "Fontmgr: Error: Invalid segment type %d.\n", Header.Type);
      success = FALSE;      
    }

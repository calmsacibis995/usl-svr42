...
#ident	"@(#)dtadmin:fontmgr/font_util.c	1.12"
...
ModalGizmo prompt = {0, "dc", TXT_PROMPT_TITLE, (Gizmo) &prompt_menu};

static MenuItems notice_menu_item[] = {
{ TRUE, TXT_OK,     ACCEL_NOTICE_OK, 0, NoticeCB},
{ NULL }
};
static MenuGizmo notice_menu = {0, "cm", "cm", notice_menu_item };
static ModalGizmo notice = {0, "ng", TXT_NOTICE_TITLE, (Gizmo) &notice_menu};
...
static void
NoticeCB(w, client_data, call_data)
     Widget w;
     XtPointer client_data;
     XtPointer call_data;
{
    XtPopdown( notice.shell);
...
{
    if (!notice.shell)
	CreateGizmo(app_shellW, ModalGizmoClass, &notice, NULL, 0);

    SetModalGizmoMessage(&notice, GetGizmoText(text));
...
void
PromptUser( text, ok_CB, ok_data, cancel_CB, cancel_data, help_index)
    char *text;
...
#define HELP_INDEX   2
    ...
    if (!prompt.shell) {
	prompt.title = GetGizmoText(prompt.title);
    ...
    OlVaFlatSetValues( prompt_menu.child, HELP_INDEX,
...
char *cmd, *arg0, *arg1, *arg2, *arg3, *arg4;
{
	...
	if (pipe(fildes) != 0) {
		...
		sprintf(errbuf, "unable to open pipe for %s", cmd);
		perror(errbuf);
		exit(0);
	}
	switch ((*pid) = fork()) {
	case -1:
		{
			...
			sprintf(errbuf, "unable to fork for %s\n", cmd);
			perror(errbuf);
			return(NULL);
		}
	case 0:		/* We are the child */
		if (close(1) == -1) {
			perror("could not close stdout in child\n");
			exit(1);
		}
		if (dup(fildes[1]) == -1) {
		...
		if (execl(cmd, cmd, arg0, arg1, arg2, arg3, arg4, NULL) == -1) {
			char errbuf[512];
			sprintf(errbuf, "unable to exec %s", cmd);
			perror(errbuf);
			exit(0);
		}
		break;
	default:	/* We are the parent */
		...
		fd = fildes[0];
		/*******************************************************
		if (fcntl(fd, F_SETFL, O_RDONLY|O_NDELAY) == -1) {
			char errbuf[512];
			sprintf(errbuf, "could not set O_NDELAY mode on pipe of %s", cmd);
			perror(errbuf);
			exit(0);
		}
		********************************************************/
		...
		if ((fp = fdopen(fd, "r")) == NULL) {
			char errbuf[512];
			sprintf(errbuf, "could not fdopen() pipe of command %s", cmd);
			perror(errbuf);
		}
		return(fp);
	}
...
OpenProgress( max)
    int max;
{
    XtVaSetValues( view->gauge, XtNsliderMax, max, XtNsliderValue, 0, 0);
    XtManageChild( view->gauge);
...
UpdateProgress( value)
    int value;
{
    XtVaSetValues( view->gauge, XtNsliderValue, value, 0);
    XFlush(XtDisplay(view->gauge));

} /* end of UpdateProgess */


void
CloseProgress()
{
    XtUnmanageChild( view->gauge);

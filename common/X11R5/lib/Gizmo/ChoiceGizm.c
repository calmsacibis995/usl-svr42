...
#ifndef NOIDENT
#pragma ident	"@(#)Gizmo:ChoiceGizm.c	1.21"
#endif
...
static char *    ChoiceUnconverter(ChoiceGizmo *);
...
CreateAbbrevChoice(Widget promptArea, ChoiceGizmo * gizmo, ArgList args, int num_args)
{
   ...
   abbreviated = True;
   CreateChoice(promptArea, gizmo, args, num_args);
   abbreviated = False;
...
CreateChoice (Widget promptArea, ChoiceGizmo * gizmo, ArgList args, int num_args)
{
   ...
   switch (menu->buttonType)
   {
      ...
      case NNS:
      case NON:
         gizmo->settings->current_value = STRDUP(gizmo->settings->previous_value);
         for (i = 0; menu->items[i].label; i++)
         {
            menu->items[i].set = (previous_value[i] == 'x');
            if (abbreviated && !menu->function && !menu->items[i].function)
            {
               menu->items[i].function = SetPreview;
               menu->items[i].client_data = (XtPointer)gizmo;
...
/*
 * FIX: presumes exclusive!!!
 */
      XtSetArg(arg[0], XtNstring, 
         menu->items[(int)gizmo->settings->previous_value].label);
      XtSetArg(arg[1], XtNwidth,      100);
      XtSetArg(arg[2], XtNbackground,   5); /* FIX: for debugging */
...
BuildChoice()
{
   ...
   fprintf(stdout, "static ChoiceGizmo %s = { ", SCAN(label));
   fprintf(stdout, "\"%s\", ", SCAN(label));   /* name     */
   fprintf(stdout, "%s, ", SCAN(label));       /* menu     */
   fprintf(stdout, "%s ", SCAN(label));        /* verify   */
   fprintf(stdout, "};\n");

...
#ifndef NOIDENT
#pragma ident	"@(#)Gizmo:BaseWGizmo.c	1.38"
#endif
 ...
 * CopyBaseWindow
 *
 * The \fICopyBaseWindow\fP fuction is used to create a copy 
 * of an existing BaseWindowGizmo \fIold\fP.  This function makes
 ...
 * BaseWindowGizmo structure) using the \fICopyBaseWindow\fP function
 * .LI
 * creates the widget tree using the \fICreateBaseWindow\fP function
 * .LI
 * realizes the new independent base window using the \fIRealizeBaseWindow\fP
 * procedure.
 * .LE
 * 
...
Gizmo gizmo;
{
   ...
   new->icon_name    = CSTRDUP(old->icon_name);
   new->icon_pixmap  = CSTRDUP(old->icon_pixmap);
   new->icon_shell   = NULL;
   new->shell        = NULL;
   new->form         = NULL;
   new->message      = NULL;
   ...
   new->error        = CSTRDUP (old->error);
   new->status       = CSTRDUP (old->status);
 ...
 * FreeBaseWindow
 *
 * The \fIFreeBaseWindow\fP procedure is used to free a previously
 * allocated BaseWindowGizmo \fIold\fP.
 *
 * See also:
 *
 ...
 * CreateBaseWindow
 *
 * The \fICreateBaseWindow\fP function is used to create an
 * entire, \fIstandard\fP base window shell.  This shell is created
 * on the display associated with the \fIparent\fP Widget
 * using the elements of the \fIgizmo\fP BaseWindowGizmo.  \fIargs\fP
 * and \fInum\fP, if non-NULL, are used as Args in the creation
 * of the application shell.  The function returns the Widget
 * handle of the application shell that is created.
 *
 * Standard Appearance:
 *
 * The \fICreateBaseWindow\fP function creates a standard base
 * window composed of an applicationShell Widget \fIshell\fP containing
 * a form Widget \fIform\fP containing a flatButtons Widget (the
 * menu bar), a scrolledWindow * \fIscroller\fP Widget, and a staticText 
 * Widget \fImessage\fP.  A typical window appears as:~
 * .sp
 * .BP /r4/richs/desktop/reqmnts/main.ps 3.5i 6.0i
 * .EP
 *
 * Example:
 *
 ...
 * the menu bar.  Then the application invokes the \fICreateBaseWindow\fP
   ...
   (Widget parent, BaseWindowGizmo * gizmo, Arg * args, int num)
{
   ...
   if (gizmo->num_gizmos == 0)
   {
      ...
      XtSetArg(arg[5], XtNyRefWidget,    menu->child);
      XtSetArg(arg[6], XtNyAttachOffset, footerHeight);
      XtSetArg(arg[7], XtNweight, 1);
      gizmo->scroller =
         XtCreateManagedWidget("_X_", scrolledWindowWidgetClass, gizmo->form, arg, 8);
   }
   else
   {
 ...
 * GetBaseWindowShell
 *
 * The \fIGetBaseWindowShell\fP function is used to retrieve the application
 * shell widget stored in the \fIgizmo\fP BaseWindowGizmo data structure.
 *
 * See also:
 *
 ...
 *
 * The \fIGetBaseWindowScroller\fP function is used to retrieve the
 * scrolled window widget stored in the \fIgizmo\fP BaseWindowGizmo data 
 ...
 * RealizeBaseWindow
 *
 * The \fIRealizeBaseWindow\fP procedure is used to map (realize)
 * the application shell in the \fIgizmo\fP BaseWindowGizmo.
 *
 * See also:
 *
 * CopyBaseWindow(3), FreeBaseWindow(3), CreateBaseWindow(3)
 ...
 * SetBaseWindowError
 *
 * The \fISetBaseWindowError\fP procedure is set the message
 * appearing in the left footer message are of the \fIgizmo\fP BaseWindowGizmo 
 * to the \fImessage\fP string.
 *
 * See also:
 *
 * CopyBaseWindow(3), FreeBaseWindow(3), CreateBaseWindow(3)
 ...
 * SetBaseWindowStatus
 *
 * The \fISetBaseWindowStatus\fP procedure is set the message
 * appearing in the right footer message are of the \fIgizmo\fP BaseWindowGizmo 
 * to the \fImessage\fP string.
 *
 * See also:
 *
...
BuildBaseWindow()
{
   ...
   fprintf(stdout, "static BaseWindowGizmo %s = ", Scan());
   fprintf(stdout, "{ \"%s\", ", Scan());
   fprintf(stdout, "&%s };\n", Scan());

} /* end of BuildBaseWindow */
/*
 * ManipulateBaseWindow
 *
 */
...
CreateCenteredPixmap(Widget w, int icon_width, int icon_height, int width, int height, Boolean is_color, Pixmap pixmap, Pixmap pixmask, char * icon_name)
{
   ...
   values.font       = fs->fid;
   gc = XtGetGC(w, GCForeground | GCFont, &values);

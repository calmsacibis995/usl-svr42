...
#ifndef NOIDENT
#pragma ident	"@(#)Gizmo:TimeGizmo.c	1.8"
#endif
...
CreateTime(Widget promptArea, TimeGizmo * gizmo, ArgList args, int num_args)
{
   ...
   XtSetArg(arg[1], XtNcharsVisible, 8); /* FIX: 8 is enough? */
...
SetCurrentTime(TimeGizmo * gizmo, int hour, int min)
{
   ...
   XtSetArg(arg[3], XtNselectEnd,      0 /* FIX? 5 */);
 ...
 * GetTime
 *
 * The \fIGetTime\fP function is used to retrieve the text
 * stored in a TimeGizmo which is in the PopupGizmo \fIshell\fP.
 ...
 * SetTime
 *
 * The \fISetTime\fP procedure is used to replace the text
 * stored in a TimeGizmo which is in the PopupGizmo \fIshell\fP.
 * The \fIitem\fP index is used to determine which of the MiscGizmos
...
BuildTime()
{
   ...
   fprintf(stdout, "static TimeGizmo %s = ", Scan());

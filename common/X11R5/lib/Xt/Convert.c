    ...
    XtCacheRef			*cache_ref_return;
{
    ...
    Cardinal		num_args;
    XrmValue		*args;
    char locBuf[16]; static char locBufInUse = 0; /* PERF */
	...
	if (from_type == p->from && to_type == p->to) {
	    ...
	    num_args = p->num_args;
	    if (num_args != 0) {
		/* args = (XrmValue*) 	/** PERF: see above */
		    /* ALLOCATE_LOCAL( num_args * sizeof (XrmValue)); /* PERF */
		args = (XrmValue*) ALLOCATE_MAYBE		    /* PERF */
		    (num_args*sizeof(XrmValue),locBuf,locBufInUse); /* PERF */
	    ...
	    /* if (args) DEALLOCATE_LOCAL((XtPointer)args); /** PERF: above */
	    if (args) FREE_MAYBE((XtPointer)args,locBuf,locBufInUse); /* PERF */
	    return retval;
	}
    }

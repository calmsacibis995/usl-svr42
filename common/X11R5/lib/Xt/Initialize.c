...
#endif
{
	...
	char localBuf[SEARCH_LIST_SIZE*sizeof(XrmHashTable)];	/* PERF */
	static char localBufInUse = 0; 				/* PERF */
	...
	/* if (!(search_list = (XrmHashTable*) 	/** PERF: see above */
	/* ALLOCATE_LOCAL( SEARCH_LIST_SIZE*sizeof(XrmHashTable)))) /* PERF */
	if (!(search_list = (XrmHashTable*) ALLOCATE_MAYBE(	/* PERF */
	  SEARCH_LIST_SIZE*sizeof(XrmHashTable),localBuf,localBufInUse)))
	    _XtAllocError(NULL);
	name_list[0] = pd->name;
	class_list[0] = pd->class;
	...
	while (!XrmQGetSearchList(db, name_list, class_list,
	    ...
	    /* if (!(search_list = (XrmHashTable*)ALLOCATE_LOCAL(size)))/*PERF*/
	    if (!(search_list = (XrmHashTable*)			/* PERF */
	      ALLOCATE_MAYBE(size,localBuf,localBufInUse)))	/* PERF */
		_XtAllocError(NULL);
	    bcopy( (char*)old, (char*)search_list, (size>>1) );
	    /* DEALLOCATE_LOCAL(old); 	/** PERF: see above */
	    FREE_MAYBE(old,localBuf,localBufInUse); 	/* PERF */
	}
	...
	/* DEALLOCATE_LOCAL( search_list ); 	/** PERF: see above */
	FREE_MAYBE(search_list,localBuf,localBufInUse); 	/* PERF */
}

...
#endif
{
    /* char *buf, *buf1, *buf2, *colon, *start;		/** PERF: see below */
    char *buf, *colon, *start;				/** PERF: see below */
    int len;
    Boolean firstTime = TRUE;

    /* PERF: (Performance improvement work, 4/92)                        */
    /*       To cut down on number of malloc/free's of the same storage, */
    /*       I'm making buf1 and buf2 automatic storage.                 */
    /*       In the case where the value is needed ("return buf" below), */
    /*       I malloc storage as needed for the return string.           */
    /*       NOTE: we allocate only the amount needed (strdup) rather    */
    /*       than PATH_MAX, so if anyone is using the result incorrectly */
    /*       (depending on extra length in the path), this will break    */
    /*       their code now.                                       [AS]  */

    /* buf = buf1 = XtMalloc((unsigned)PATH_MAX);	/**  PERF: see above */
    /* buf2 = XtMalloc((unsigned)PATH_MAX);		/**  PERF: see above */

    char buf1[(unsigned)PATH_MAX];  /* PERF */
    char buf2[(unsigned)PATH_MAX];  /* PERF */
    buf = buf1;
	...
	if (Resolve(path, len, substitutions, num_substitutions,
		...
		if ((*predicate) (buf)) {
		    ...
		    /* if (buf == buf1) XtFree(buf2);   /** PERF: see above */
		    /* else XtFree(buf1);		/** PERF: see above */
		    /* return buf;			/** PERF: see above */

    		    return (strdup(buf));	/** PERF: see above */
    ...
    /* XtFree(buf1);		/** PERF: see above */
    /* XtFree(buf2);		/** PERF: see above */
    return NULL;
}
...
/* PERF: These macros (ALLOCATE_MAYBE and FREE_MAYBE) are substituted for */
...
/*       it's already in use, a real ALLOC takes place.  FREE_MAYBE       */    /*       unwinds this situation.   [AS]                                   */
...
#endif
{
    ...
    char localBuf[1000]; static char localBufInUse = 0;	/* PERF */
    ...
    /* massagedPath = ALLOCATE_LOCAL(bytesAllocd); 	/** PERF: see above */
    massagedPath = ALLOCATE_MAYBE(bytesAllocd,localBuf,localBufInUse);/* PERF */
	...
	if (bytesLeft < 8) {
		...
		/* DEALLOCATE_LOCAL(massagedPath); 	/** PERF: see above */
		FREE_MAYBE(massagedPath,localBuf,localBufInUse); /* PERF */
	    pathMallocd = True;
	    massagedPath = new;

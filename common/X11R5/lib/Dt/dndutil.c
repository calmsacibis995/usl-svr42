...
Dt__DnDInfoPtr	dip;
{
	...
	if (dip->send_done)
		OlDnDDragNDropDone(w, *selection, dip->timestamp, NULL, NULL);
...
Dt__DnDSendPtr sip;
{
	...
	OlDnDDisownSelection(sip->widget, sip->selection, CurrentTime);
	OlDnDFreeTransientAtom(sip->widget, sip->selection);
 ...
 */
Boolean
static ConvertSelectionProc( Widget		w,
			Atom *		selection,
			Atom *		target,
			Atom *		type_rtn,
			XtPointer *	val_rtn,
		    ...
		    XtPointer				client_data)
{
	...
	if ((sip->selection = OlDnDAllocTransientAtom(w)) == (Atom)None) {
	...
	if (OlDnDOwnSelection(w, sip->selection,
			      drop_timestamp,
			      ConvertSelectionProc,
	...
	if (attrs & DT_B_SEND_EVENT) {
		...
		if (OlDnDDeliverTriggerMessage(w,
			RootWindowOfScreen(XtScreen(w)),
			root_x,
			root_y,
			sip->selection,

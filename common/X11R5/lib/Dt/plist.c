...
/*
 * returns the string value associated with the property.
 */
char *
...
/*
 * if prop_value is NULL, delete named property.
 */
char *
...
int (*func)();
{
	...
	while (*p && (*p != '}')) {
		...
		if (namelen && vallen) {
			...
			value = Dt__strndup(value, vallen);
	...
	*buff++ = '{';
	for (; cnt; cnt--, pp++) {
		buff += sprintf(buff, "%s%s=\"%s\";",
		    		pp->name, Dt__AttrToStr(pp->attrs), pp->value);
	}

...
static void FBHandleButton OL_ARGS((Widget, Position, Position, 
...
static void
FBHandleButton OLARGLIST((w, x, y, virtual_name, new_current, is_motion))
  OLARG( Widget,        w)
  OLARG( Position,      x)
  OLARG( Position,      y)
      ...
      switch(ve->xevent->type)
            ...
            if (fexcp-> button_type == OL_OBLONG_BTN)
	    {
	       CALL_SELECTPROC(w, ve->virtual_name, ve->item_index)
            ...
            FBHandleButton(w, x, y, ve-> virtual_name, ve->item_index, True);
            break;
         case ButtonPress:
...
#undef NOT_SAME_ITEM
	       ...
	       CALL_SELECTPROC(w, ve->virtual_name, ve->item_index)
            ...
            FBHandleButton(w, x, y, ve-> virtual_name, ve->item_index, False);
            break;
         case EnterNotify:
	    if (ve->item_index == OL_NO_ITEM)
	    {
		break;
            ...
            if (_OlIsPendingStayupMode(shell))
               _OlResetStayupMode(shell);
            FBHandleButton(w, x, y, ve-> virtual_name, ve->item_index,

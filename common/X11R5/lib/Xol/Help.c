...
static void	GetDesktopHelp(Widget w, Window app_win, char *tag,
...
static void
GetDesktopHelp(w, app_win, tag, source_type, source, x, y)
Widget  w;
Window  app_win;
char    *tag;
int     source_type;
...
int     y;
{
	...
	if (source_type == OL_DISK_SOURCE) {
		...
		if (tag) {
			...
			sprintf(buf, "@OL_DISPLAY_HELP: SERIAL=-2 VERSION=0 "
			...
			sprintf(buf, "@OL_DISPLAY_HELP: SERIAL=-2 VERSION=0 "
				"CLIENT=%lu HELPTYPE=%d APPNAME=\"%s\" "
				"FILENAME=\"%s\" XPOS=%d YPOS=%d", app_win,
				source_type, name, source, x, y); 
		}
		...
		if (tag) {
			if (path && file && sect) {
				...
				sprintf(buf, "@OL_DISPLAY_HELP: SERIAL=-2 VERSION=0 "
					...
					app_win, source_type, name, app_title, tag,
					file, path, sect, x, y);
				...
				sprintf(buf, "@OL_DISPLAY_HELP: SERIAL=-2 VERSION=0 "
					"CLIENT=%lu HELPTYPE=%d APPNAME=\"%s\" "
					"APPTITLE=\"%s\" TITLE=\"%s\" FILENAME=\"%s\" "
					"HELPDIR=\"%s\" XPOS=%d YPOS=%d", app_win,
					source_type, name, app_title, tag, file, path, x, y);
				...
				sprintf(buf, "@OL_DISPLAY_HELP: SERIAL=-2 VERSION=0 "
					"CLIENT=%lu HELPTYPE=%d APPNAME=\"%s\" "
					"APPTITLE=\"%s\" TITLE=\"%s\" FILENAME=\"%s\" "
					"SECTTAG=\"%s\" XPOS=%d YPOS=%d", app_win,
					source_type, name, app_title, tag, file, sect, x, y);
				...
				sprintf(buf, "@OL_DISPLAY_HELP: SERIAL=-2 VERSION=0 "
					"CLIENT=%lu HELPTYPE=%d APPNAME=\"%s\" "
					"APPTITLE=\"%s\" TITLE=\"%s\" FILENAME=\"%s\" "
					"XPOS=%d YPOS=%d", app_win, source_type, name,
					app_title, tag, file, x, y);
			} else {
				/* should never get here since file must be specified */
			}
			...
			if (path && file && sect) {
				...
				sprintf(buf, "@OL_DISPLAY_HELP: SERIAL=-2 VERSION=0 "
					...
					"SECTTAG=\"%s\" XPOS=%d YPOS=%d", app_win,
					source_type, name, app_title, file,
					path, sect, x, y);
				...
				size = strlen(name) + strlen(file) + strlen(sect)
						+ 250;
				buf = (char *)malloc(size * sizeof(char));

				sprintf(buf, "@OL_DISPLAY_HELP: SERIAL=-2 VERSION=0 "
					"CLIENT=%lu HELPTYPE=%d APPNAME=\"%s\" "
					"APPTITLE=\"%s\" FILENAME=\"%s\" SECTTAG=\"%s\" "
					"XPOS=%d YPOS=%d", app_win, source_type, name,
				...
				sprintf(buf, "@OL_DISPLAY_HELP: SERIAL=-2 VERSION=0 "
					...
					app_win, source_type, name, app_title, file, x, y);
			} else {
				/* should never get here since file must be specified */
			}
		}

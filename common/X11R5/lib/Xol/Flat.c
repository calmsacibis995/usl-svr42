	...
	OLGRA( Time *,		time)
{
	     ...
	     OlCanAcceptFocus(w, *time) == True))
	{
		...
		old_last_focus_item		= FPART(w).last_focus_item;
		...
		else
		{
			...
			if (!can_take_focus)
			{
				FPART(w).focus_item = old_focus_item;
				FPART(w).last_focus_item = old_last_focus_item;
			}
		}
	}
	else

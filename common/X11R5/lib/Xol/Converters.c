...
#ifndef	NOIDENT
#ident	"@(#)olmisc:Converters.c	1.48.1.23"
#endif
...
static OLconst char *	MapFont OL_ARGS((
	Screen *		screen,
	OLconst char *		str,
	String		*	cset,
	Boolean			scalable
...
OLconst DefaultFonts default_fonts[] = {
	{ "xtDefaultFont", "XtDefaultFont", "-*-*-*-R-*-*-*-120-*-*-*-*-ISO8859-1", "-*-*-*-R-*-*-*-120-*-*-*-*-ISO8859-1" },
	{ olDefaultFont, OlDefaultFont, Nlucida, Nhelvetica },
	{ olDefaultBoldFont, OlDefaultBoldFont, NlucidaBold, NhelveticaBold },
	{ olDefaultFixedFont, OlDefaultFixedFont, NlucidaFixed, NcourierFixed },
...
#define RESOLVE(DPY,TYPE,FILE,SUFF) XtResolvePathname(DPY,TYPE,FILE,SUFF,NULL,NULL,0,NULL)
  ...
  OLGRA (char *,	buf)
{
  ...
  */
  ...
  if ((filename[0] == '/' || (filename[0] == '.')
    ...
    char *home = (char *) getenv((OLconst char *)"XWINHOME");
    static OLconst char *pixmaps = (OLconst char *)"pixmaps";
    static OLconst char *bitmaps = (OLconst char *)"bitmaps";
    static OLconst char *xpm = (OLconst char *)".xpm";
    static OLconst char *xbm = (OLconst char *)".xbm";

    if (DefaultDepthOfScreen(screen) > 1) {
	found = ResolveFile(dpy, buf, home, PIXMAPLOC, pixmaps, filename, xpm);
	if (!found)
	    found = ResolveFile(dpy, buf, home, BITMAPLOC, bitmaps, filename, xbm);
    } else {
	found = ResolveFile(dpy, buf, home, BITMAPLOC, bitmaps, filename, xbm);
	if (!found)
	    found = ResolveFile(dpy, buf, home, PIXMAPLOC, pixmaps, filename, xpm);
...
pass2:
	...
	/*  As a last resort, try the default font name.  This is stored
	    in a special type of string in the StringList. As usual, we
	    do the MapFont so that the wildcards are expanded to the
	    proper resolution and point size.  */
	...
	OLGRA (Font *,		p_font)
{
	...
	/*  First try to use the font mapping resolution table in MapFont. */
	try = MapFont(screen, str, cset, False);
	
	if ((*p_font = LoadFont(display, try)))  {
		return (True);
	}

	/*  Next try to map the font to a scalable font by setting
	    the x and y resolution to 0.  */
	try = MapFont(screen, str, cset, True);
	
	if ((*p_font = LoadFont(display, try)))  {
		return (True);
	}

	/*  Try using the font name as it was given.  */
	if (try != str && (*p_font = LoadFont(display, str)))  {
		return (True);
	}
...
static OLconst char *
MapFont OLARGLIST(( screen, str, cset, scalable ))
	OLARG (Screen *,	screen)
	OLARG (OLconst char *,		str)
	OLARG (String *,	cset)
	OLGRA (Boolean,		scalable)
{
	/*
	 * Note: We assume the point-size/resolution map is
	 * screen independent.
	 */

	typedef struct FontResolution {
		OLconst struct {
			OLconst Cardinal	ps;
			OLconst Dimension	H;
			OLconst Dimension	V;
	...
	static OLconst FontResolution	font_resolutions[] = {
		{ { 12,  75,  55 }, { 12,  76,  55 } },
		{ { 12,  75,  63 }, { 10,  75,  75 } },
		{ {  0,  75,  75 }, {  0,  75,  75 } },
		{ { 12,  95,  95 }, { 12, 100, 100 } },
...
#define XLFD_CHARSET_REGISTRY	13
...
#define QUESTION_MARK		'?'
	...
	OLconst char *		xlfd_parts[_XLFD_MAX_ELEMENT + 1];

	register Cardinal	i;

	OLconst FontResolution *best_pfr = 0;
	...
	/*
	 * If the resolution information is present,
	...
	/*
	 * Convert the string representation of the point size
	 * to an integer. Note that "xlfd_parts[]" contains
	 * pointers into the original string; each part pointed
	 * to is NOT null terminated. However, "atoi()" stops at
	 * the first non-digit after a leading minus sign, so all
	 * we have to do is make sure we're not starting on a
	 * minus sign (== XFNDelim).
	 *
	 * Note also that the XLFD value for the point size is
	 * in decipoints.
	 */
	...
	case ASTERISK:
	case QUESTION_MARK:
		ps = OL_DEFAULT_POINT_SIZE;
		break;
	default:
		ps = atoi(xlfd_parts[XLFD_POINT_SIZE]) / 10;
		break;
	}

	switch (*(xlfd_parts[XLFD_AVERAGE_WIDTH])) {
	case XFNDelim:
	case ASTERISK:
	case QUESTION_MARK:
		break;
	default:
		avg_width = atoi(xlfd_parts[XLFD_AVERAGE_WIDTH]) ;
		break;
	}
	...
	case QUESTION_MARK:
		break;
	default:
		pixel = atoi(xlfd_parts[XLFD_PIXEL_SIZE]) ;
		break;
	}
	/*
...
#define DPI(DOTS,MM)	(Cardinal)( ((DOTS) * 254) / ((MM) * 10) )
	...
	/*
	 * Search the font resolution table for an entry with the
	 * closest HxV resolution and a matching point size
	 * (a table point size of 0 is a matching wildcard).
	 */

	if (!scalable)  {
		for (i = 0; i < XtNumber(font_resolutions); i++) {
			register OLconst FontResolution *pfr = &font_resolutions[i];
			register long		best_dist;
	
			if (!pfr->from.ps || pfr->from.ps == ps) {
				register long	d_H  = (pfr->from.H - H);
				register long	d_V  = (pfr->from.V - V);
				register long	dist = d_H*d_H + d_V*d_V;
	
				if (!best_pfr || dist < best_dist) {
					best_pfr  = pfr;
					best_dist = dist;
				}
			}
	...
	/*
	 * If we found a mapping, copy the font name replacing
	 * the point size and resolution fields with the
	 * optimum values.
	 */
	if (!best_pfr && !scalable)
		{
		*cset = NULL;
		return (str);
		}
		...
		static char		cregistry[_XLFD_MAX_LENGTH + 1];
		register String		q	= cregistry;
		register String		p	= copy;
		register Cardinal	n	= _XLFD_MAX_ELEMENT + 1;
		register Cardinal	bytes;

#if	defined(USG) || defined(SYSV) || defined(SVR4_0) || defined(USL) || defined(SVR4)
# define SPRINTF(x) sprintf x
#else
# define SPRINTF(x) strlen(sprintf x)	/* e.g. sun */
#endif
		...
		*cset = q;
		for (i = 1; i < n; i++) {
			...
			case XLFD_POINT_SIZE:
				if (best_pfr && best_pfr->to.ps)
					ps = best_pfr->to.ps;
				p += SPRINTF((p, (OLconst char *)"%d0-", ps));
				break;
			case XLFD_RESOLUTION_X:
				if (scalable || pixel == 0 || avg_width == 0)
					p += SPRINTF((p, (OLconst char *)"%d-", H));
				else
					p += SPRINTF((p, (OLconst char *)"%d-", best_pfr->to.H));
				break;
			case XLFD_RESOLUTION_Y:
				if (scalable || pixel == 0 || avg_width == 0)
					p += SPRINTF((p, (OLconst char *)"%d-", V));
				else
					p += SPRINTF((p, (OLconst char *)"%d-", best_pfr->to.V));
				break;
			case XLFD_PIXEL_SIZE:
				if (scalable || pixel == 0 || avg_width == 0) {
					p += SPRINTF((p, (OLconst char *)"0-"));
					break;
				}
			...
			case XLFD_AVERAGE_WIDTH:
				...
				p += bytes;
				break;
			case XLFD_CHARSET_REGISTRY:
				strcpy (q, xlfd_parts[i]);
				/*FALLTHROUGH*/
			default:
				bytes = xlfd_parts[i+1] - xlfd_parts[i];
				strncpy (p, xlfd_parts[i], bytes);

...
#ifndef	NOIDENT
#ident	"@(#)virtual:Virtual.c	1.26.1.13"
#endif
 ...
 refused mapping because duplicate in \"%s\"",
	...
	if (vbk != (VBK *) NULL) {
		...
		if (self->num_mappings == OL_MAX_VIRTUAL_MAPPINGS)
		{
			OlVaDisplayWarningMsg((Display *)NULL, "", "",
				OleCOlToolkitWarning,
				"Attempt to add more than %d mappings to\
 virtual button \"%s\"", OL_MAX_VIRTUAL_MAPPINGS, XrmQuarkToString(qname));
			return;
		}
	}
	else {
	...
	if (vbk != (VBK *) NULL) {
		...
		if (self->num_mappings == OL_MAX_VIRTUAL_MAPPINGS)
		{
			OlVaDisplayWarningMsg((Display *)NULL, "", "",
				OleCOlToolkitWarning,
				"Attempt to add more than %d mappings to\
 virtual key \"%s\"", OL_MAX_VIRTUAL_MAPPINGS, XrmQuarkToString(qname));
			return;
		}
	}
	else {
...
/*		PrintInitError((Display *)NULL, func_name); */
	...
	else if (name == (char *) NULL || *name == '\0')
	{
		OlVaDisplayWarningMsg((Display *)NULL, "", "",
					OleCOlToolkitWarning,
					"%s: NULL or empty virtual button name",
					func_name);
		return(False);
	}
	else if (*name == '~')
	{
		OlVaDisplayWarningMsg((Display *)NULL, "", "",
					OleCOlToolkitWarning,
				"%s: virtual button name cannot begin with '~'",
				func_name);
		return(False);
	}
	...
	if (vbk == (VBK *) NULL)
	{
		OlVaDisplayWarningMsg((Display *)NULL, "", "",
					OleCOlToolkitWarning,
				"%s: Virtual Button \"%s\" does not exist",
				func_name, name);
		return(False);
	}
	...
	if (button_type == NULL) {
		...
		if (button_mask == 0)
		{
			OlVaDisplayWarningMsg((Display *)NULL, "", "",
					OleCOlToolkitWarning,
					"%s: invalid button number %d for\
 virtual button \"%s\"", func_name, button_type, name);
			return(False);
		}
...
/*		PrintInitError((Display *)NULL, func_name); */
	...
	else if (name == (char *) NULL || *name == '\0')
	{
		OlVaDisplayWarningMsg((Display *)NULL, "", "",
					OleCOlToolkitWarning,
			"%s: NULL or empty virtual key name", func_name);
		return(False);
	}
	...
	if (vbk == (VBK *) NULL)
	{
		OlVaDisplayWarningMsg((Display *)NULL, "", "",
					OleCOlToolkitWarning,
					"%s: Virtual Key \"%s\" does not exist",
					func_name, name);
		return(False);
	}
	...
	register OLconst char *	s;
{
	...
	while (*s != '\0')
	{
		...
		OlVaDisplayWarningMsg(XtDisplay(widget), "", "",
					OleCOlToolkitWarning,
					"ParseList: Invalid virtual button\
 or key name '%c': only 1 char long", *s);
		...
		if (*s != ':')
		{
			OlVaDisplayWarningMsg(XtDisplay(widget), "", "",
						OleCOlToolkitWarning,
						"ParseList: Missing ':' after\
 virtual button/key name \"%s\"", name_array);
	...
	register char *	s;		/* start address of mapping	*/
{
	...
	if (s == (char *) NULL || *s == '\0')
	{
		OlVaDisplayWarningMsg((Display *)NULL, "", "",
				OleCOlToolkitWarning,
				"Virtual Button/Key \"%s\" has no mappings",
				name);
		return(s);
	}
 ...
 mapping to virtual button \"%s\"", name);
		...
		OlVaDisplayWarningMsg((Display *)NULL, "", "",
					OleCOlToolkitWarning,
					"ParseMappings: virtual button/key\
					 ...
					 * production (in chars)	*/
{
	...
	*size = 0;
	...
	if (error != (char *)NULL)
	{
		OlVaDisplayWarningMsg((Display *)NULL, "", "",
			OleCOlToolkitWarning,
			"ParseProduction: virtual button/key error:\
 ==> %s\n==> found when parsing \"%.*s\"", error, (int) (p - *production),
 *production);
...
{
	OlVaDisplayErrorMsg(display, "", "", OleCOlToolkitError,
	"%s: virtual button/key code not initialized properly", func_name);
	...
	OLconst String	string;		/* mapping string to be parsed	*/
{
	...
	if (widget == (Widget)NULL)
	{
		...
		if (widget == (Widget)NULL)
		{
			OlVaDisplayWarningMsg((Display *)NULL, "", "",
				OleCOlToolkitWarning,
				"_OlAppAddVirtualMappings: NULL widget");
			return;
		}
	}
	...
	char *translation;		/* Unparsed translation		*/
{
	...
	if (translation == (char *)NULL)
	{
		OlVaDisplayWarningMsg((Display *)NULL, "", "",
			OleCOlToolkitWarning,
	   		"%s: NULL translation string", func_name);
	   return(translation);
	}
...
/*		PrintInitError((Display *)NULL, func_name); */
	...
	else if (list_length > (Cardinal)0 && list == (_OlVirtualMapping *)NULL)
	{
		OlVaDisplayErrorMsg((Display *)NULL, "", "",
			OleCOlToolkitError,
			"%s: NULL mapping list array for non-zero size",
			func_name);
	}
	else if (name == (String)NULL || *name == '\0')
	{
		OlVaDisplayErrorMsg((Display *)NULL, "", "",
			OleCOlToolkitError,
			"%s: NULL or empty name", func_name);
	}
...
#endif
{
	...
	default:
		...
		OlVaDisplayWarningMsg((Display *)NULL, "", "",
			OleCOlToolkitWarning,
			"_%s: called with event other than\
 ButtonPress, ButtonRelease, MotionNotify, KeyPress, KeyRelease", func_name);
		break;
	}
	return(status);

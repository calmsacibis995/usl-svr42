...
#else
__OlArrayFind (file, line, pVar, p_index, data_size, va_alist)
	String			file;
	int			line;
	CharArray *		pVar;
	int *			pIndx;
...
#endif
{
	...
	if (!pVar)
		...
		found = ___OlArrayFind(pVar, &indx, _OL_NULL_ARRAY_INDEX, data_size, ap);
		if (pIndx)
			*pIndx = indx;
	}
...
#else
____OlArrayInsert (file, line, pVar, flag, indx, data_size, va_alist)
	String			file;
	int			line;
	CharArray *		pVar;
	int			flag;
...
#endif
{
	...
	"Attempted ordered insert into unordered array: \"%s\", line %d.",
			...
			if (___OlArrayFind(pVar, &indx, indx, data_size, ap))
				return (indx);
		} else
			(void)___OlArrayFind(pVar, &indx, indx, data_size, ap);
		break;
...
#else
___OlArrayFind (pVar, pIndx, hint, data_size, ap)
	CharArray *		pVar;
	int *			pIndx;
	int			hint;
	Cardinal		data_size;

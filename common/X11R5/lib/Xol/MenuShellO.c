	...
	OLGRA( char,			size_or_draw)
{
  ...
  if (HAS_TITLE(nw)) {
	...
	if (size_or_draw == 'd') {
		...
		if (nPart->pin.width)
			x += nPart->pin.width + Width(nw, PUSHPIN_GAP);
	...
	if (HAS_TITLE(w) && w->shell.override_redirect)  {
		...
		if (HAS_PUSHPIN(w))  {
			...
			NPART(w, title.x) += Width(w, PUSHPIN_GAP) +
							NPART(w, pin.width);
			*width += Width(w, PUSHPIN_GAP) + NPART(w, pin.width);
		}
		...
		*height = SpecHeightOrBigger(
		...
		/*  All menus with titles also have a etched seperator line
		    ...
		    first menu item.  */
		*height += Height(w, SEPERATOR_HEIGHT) + Height(w, HEADER_SPACE);

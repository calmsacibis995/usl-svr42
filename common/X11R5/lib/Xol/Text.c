  ...
  TextWidget	sw;
{
  ...
  if (sw->text.align_horizontal == OL_BOTTOM)
    {
      bb_info.y = 0;
      hsb_info.y = bb_info.real_height + SW_HSB_GAP;
    }
  else
    {
      hsb_info.y = 0;
      bb_info.y = hsb_info.real_height + SW_HSB_GAP;
    }

  if (sw->text.align_vertical == OL_RIGHT)
    {
      bb_info.x = 0;
      vsb_info.x = bb_info.real_width + SW_VSB_GAP;
    }
  else
    {
      vsb_info.x = 0;
      bb_info.x = vsb_info.real_width + SW_VSB_GAP;
    }
  ...
  _SWGeometryInfo	*hsb_info;
{
  ...
  if (CheckWidth(sw, sw_info, bb_info, bbc_info, hsb, hsb_info))
    {
      extra_space = hsb_info->real_height + SW_HSB_GAP;
  ...
  _SWGeometryInfo	*vsb_info;
{
  ...
  if (CheckHeight(sw, sw_info, bb_info, bbc_info, vsb, vsb_info))
    {
      extra_space = vsb_info->real_width + SW_VSB_GAP;
  ...
  Cardinal	num_args;
{
  ...
  XtSetArg(bb_arg[n], XtNheight,
	   nsw->core.height - 2 * SW_BB_BORDER_WIDTH);			n++;
  XtSetArg(bb_arg[n], XtNlayout, OL_MINIMIZE);				n++;
  XtSetArg(bb_arg[n], XtNwidth,
	   nsw->core.width - 2 * SW_BB_BORDER_WIDTH);			n++;

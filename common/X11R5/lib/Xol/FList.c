    ...
    OLGRA( Dimension *,		return_height )
{
    ...
    *return_height += 2 * VERT_MARGIN;		/* Pad field height */
}
    ...
    OLGRA( FlatListWidget,	w )
{
    ...
    width += TotalPadding(w) + 2 * HORIZ_MARGIN + h_thickness;
...
#define OleMthisMsg1	(OLconst char *)"Updating item metrics for widget\
 \"%s\" (class \"%s\"): detect overflow because of a new item list,\
 preferred item list size is \"%d\" (current is \"%d\")"
#define OleMthisMsg2	(OLconst char *)"Updating item metrics for widget\
 ...
 (total number of items is \"%d\")"
	...
	OlVaDisplayWarningMsg(
		...
		new_list ? OleMthisMsg1 : OleMthisMsg2,
		XtName(w), OlWidgetToClassName(w), preferred, NumItems(w)
	);


#undef OleNbadNumItems
#undef OleToverflow
#undef OleMthisMsg1
#undef OleMthisMsg2
...
#define PDIFF(field)	(PW(new).field != PW(current).field)
	...
	ItemsTouched(current, new);
		 ...
		 * in this case...
		 */
	if (PDIFF(font) && !fp->items_touched)
	{
		Cardinal	i;

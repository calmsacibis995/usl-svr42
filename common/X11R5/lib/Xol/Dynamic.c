...
static int		CatchBadAccessError OL_ARGS((Display *,
						     XErrorEvent *));
static int		CompareFunc OL_ARGS((Token *, Token *));
	...
	OLGRA( OlVirtualEventTable,	db)
{
	...
	MemMove((XtPointer)&wc_list[i], (OLconst XtPointer)&wc_list[i+1],
		(wc_list_entries - i - 1) * sizeof(OlClassSearchRec));
	...
	OLGRA( OlVirtualEventTable,	db)
{
	...
	for (i = 0; i < avail_dbs_entries; i++)
		if (avail_dbs[i] == db)
		{
			...
			MemMove((XtPointer)&avail_dbs[i],
				(OLconst XtPointer)&avail_dbs[i+1],
				(avail_dbs_entries - i - 1) *
					sizeof(OlVirtualEventTable *)
			);
	...
	OLGRA( OlVirtualEventTable,	db)
{
	...
	MemMove((XtPointer)&wid_list[i], (OLconst XtPointer)&wid_list[i+1],
		(wid_list_entries - i - 1) * sizeof(OlClassSearchRec));
						   ...
						   OL_DEFAULT_IE, or db ptr */
{
	...
	static OlInputEvent	last_im_retval = OL_UNKNOWN_KEY_INPUT;
#endif
...
#define TABLE		SORTEDDB[NUMKEYS]
	...
	extern void	qsort();	/* see qsort(3C) */
	int		num_keys = 0;
	int		i, j;
	...
	OLGRA( OlVirtualEventTable,	db)
{
 ...
 fprintf (stderr, "UpdateIEDB: NULL db\n");
	...
	for (i = 0; i < db->num_key_bindings; i++)
	{
		r[i].resource_name   = db->key_bindings[i].name;
		r[i].resource_class  = db->key_bindings[i].name;
	...
	{
		r[i].resource_name   = db->btn_bindings[i].name;
		r[i].resource_class  = db->btn_bindings[i].name;

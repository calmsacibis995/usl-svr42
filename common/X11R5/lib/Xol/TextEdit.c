...
#ifndef NOIDENT
#ident	"@(#)textedit:TextEdit.c	1.144"
#endif
	...
	OLGRA(XtPointer,	     closure)
{
    ...
    XtGetSelectionValue(widget, selection, XA_STRING, Paste2,
...
#define NUM_WATT  9
    ...
    *valueMask |= CWBackPixel;
	...
	spot.x = ctx->textedit.cursor_x;
	spot.y = ctx->textedit.cursor_y;
	n++;
...
Cardinal * num_args;
{
	...
	if (oldtw-> primitive.font != newtw-> primitive.font && newtext->hsb != NULL){
    ...
    OLGRA(PasteSelectionRec, *paste_info)
{
    ...
    if (paste_info)
	XtGetSelectionValue((Widget)ctx, selection, XA_STRING, PrimaryPaste, 
	...
	OLGRA( int *,			format)
{
	...
	XtGetSelectionValue((Widget)ctx, *selection,
			    OL_XA_DELETE(XtDisplay((Widget)ctx)), PrimaryCut, 
			(XtPointer)paste_info, _XtLastTimestampProcessed(ctx));
    }
    else{
	/* 

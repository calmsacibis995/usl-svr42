...
void		_OlResolveGUISymbol OL_ARGS((...));
...
static void *
OpenGuiLib OLARGLIST((lib_path, lib_lib, lib_name))
	OLARG( OLconst char *,	lib_path)
	OLARG( OLconst char *,	lib_lib)
	OLGRA( OLconst char *,	lib_name)
{
#define BUF_SIZE	1024
	...
	total_size = strlen(lib_path) + strlen(lib_lib) + strlen(lib_name) + 3;

	if (total_size <= BUF_SIZE)			\
		fullpath = local_buf;
	else
		fullpath = XtMalloc(total_size);

	sprintf(fullpath, (OLconst char *)"%s/%s/%s", lib_path, lib_lib, lib_name);

	answer = dlopen(fullpath, RTLD_LAZY);

	if (fullpath != local_buf)
		XtFree(fullpath);
...
{
#define XWIN_HOME	(OLconst char *)"XWINHOME"
#define LD_LIB_PATH	(OLconst char *)"LD_LIBRARY_PATH"
	...
	if ((ld_library_path = getenv(LD_LIB_PATH)) != (char *)NULL &&
			strlen(ld_library_path))
	{
#define COMMA	(int)':'

		char *	val = strdup(ld_library_path);
		char *	where;
		char *	saved_val;

		saved_val = val;
		while ((where = strchr(val, COMMA)) != NULL || strlen(val))
		{
			if (where != (char *)NULL)
				*where = 0;

			if ((answer = OpenGuiLib(val, (OLconst char *)"",
					lib_name)) != (void *)NULL)
				break;
...
#undef COMMA
	...
	if ((xwin_home = getenv(XWIN_HOME)) != (char *)NULL &&
			strlen(xwin_home))
	{
		if ((answer = OpenGuiLib(xwin_home, (OLconst char *)"lib", lib_name))
				!= (void *)NULL)
			return(answer);
	}

	return(OpenGuiLib(DEFAULTPATH, (OLconst char *)"", lib_name));
...
extern void
_OlResolveGUISymbol OLARGLIST((OLVARGLIST))
	OLVARGS
{
	...
	if (uselib == (void *)NULL) {
		...
		if ((uselib = FindGuiLib()) == (void *)NULL)
		{
			OlVaDisplayWarningMsg(
				OlDefaultDisplay,

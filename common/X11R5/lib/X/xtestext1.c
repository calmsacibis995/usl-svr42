...
#ifndef NOIDENT
#ident	"@(#)R5Xlib:xtestext1.c	1.2"
#endif

#include "libXi.h"	/* Must be the first thing in this file */
...
{	
	...
	{
		/*
		** AT&T: Reinitialize global variables.
		*/
...
register Display	*dpy;
{
	 ...
	 */
        /* 
         * At this point XTestInputActionType and XTestFakeAckType
 ...
 *
 *	Inform server simulation is about to start
 *
 */
int
XTestStartSimulation(dpy)
/*
 * the connection to the X server
 */
register Display	*dpy;
{	
	...
	else
	{
		...
		req->reqType = XTestReqCode;
		/*
		 * set the minor request type code
		 */
		req->XTestReqType = X_TestStartSimulation;
 ...
 *	XTestStopSimulation
 *
 *	Inform server simulation has ended
 *
 */
int
XTestStopSimulation(dpy)
/*
 * the connection to the X server
 */
register Display	*dpy;
{	
	...
	else
	{
		 ...
		 * set the minor request type code
		 */
		req->XTestReqType = X_TestStopSimulation;

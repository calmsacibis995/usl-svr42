...
/*  Temporary fix for a USL compiler problem.  The cast of ch to
    unsigned char (towards the end of the expression) is a work around.  */
#define next_mbchar(ch,len,str) xrmtypes[(unsigned char)(ch = (*db->methods->mbchar)(db->mbstate, str, &len), str += len, (unsigned char) ch)]
...
#define NEWTABLE(q,i) \
    ...
    *prev = table
	...
	if (!quarks[1]) {
		...
		if ((type == XrmQString) && (entry->string == 1) &&
    ...
    Bool doall;
{
    ...
    Bool only_pcs;
    Bool dolines;
    char localBuf[1600]; char localBufInUse = 0; /* PERF */

    if (!db)
	return;

    /* if (!(value_str = Xmalloc(sizeof(char) * alloc_chars))) 	/** PERF */
    if (!(value_str = 	/* PERF */
	  ALLOCATE_MAYBE(sizeof(char) * alloc_chars, localBuf, localBufInUse)))
	return;
	...
	*t_quarks = NULLQUARK;
	...
	for(;;) {
		...
		needed = sizeof(char) * alloc_chars;	/* PERF */
		/* REALLOCATE_MAYBE: */
		if (localBufInUse && (value_str == localBuf))  /* PERF */
		{
			if (needed <= sizeof(localBuf)) temp_str = localBuf;
			else temp_str = Xmalloc (needed);
		}
		else temp_str = Xrealloc(value_str, needed);
		/*temp_str=Xrealloc(value_str,sizeof(char)*alloc_chars);/*PERF*/

		if (!temp_str) {
		    /* Xfree(value_str); 	/** PERF: see above */
		    FREE_MAYBE(value_str, localBuf, localBufInUse); /* PERF */
...
#endif
{
    ...
    /*
     * Ensure a newline has the effect of terminating the line by
     * replacing it with an ascii nul (which does terminate the line)
     */
    len = strlen(line)+1;
    newline = len < sizeof(buffer)? buffer : Xmalloc(len);
    if (!newline)
	return;
    for (s = newline; *line; s++)
    {
	if (*line == '\n')
		break;
	else
    ...
    *s = 0;
    ...
    GetDatabase(*pdb, newline, (char *)NULL, False);

...
/* PERF, Performance tuning, 4/92:  Quark buffer size set at page size for */
/*       larger machine.  Tune it to 386 page size of 4096 bytes, guessing */
/*       same 12 byte overhead for malloc (probably conservative).   [AS]  */
/*       Note that there's a little more room for improvement, since the   */
/*       method used for quark allocation is first fit.  If a large piece  */
/*       is needed, the last N bytes of the previous piece are never used. */
/*       It's a major code change for minor gain (no pages), so not done.  */
#define PAGESIZE 4096
#define MALLOCOVERHEAD 12
#define NEVERFREETABLESIZE ((PAGESIZE-MALLOCOVERHEAD) & ~(DALIGN-1)) /* PERF */
/* #define NEVERFREETABLESIZE ((8192-12) & ~(DALIGN-1)) /** PERF: see above */

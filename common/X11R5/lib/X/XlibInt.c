    ...
    Display *dpy;
{
	...
	do {
	    nfound = XSelect (dpy->fd + 1, r_mask, w_mask,
			     (char *)NULL, (char *)NULL);
  ...
  Display *dpy;
{
    ...
    do {
	BITSET(r_mask, dpy->fd);
	result = XSelect(dpy->fd + 1, r_mask,
    ...
    int mode;
{
	...
	if (!pend && !dpy->qlen && ++dpy->conn_checker >= XCONN_CHECK_FREQ)
	{
	    ...
	    CLEARBITS(r_mask);
	    BITSET(r_mask, dpy->fd);
	    if (pend = XSelect(dpy->fd + 1, (int *)r_mask, NULL, NULL,
			      &zero_time))
	    {
		if (pend > 0)
		{
...
#define MAX_WORKAREA 4800  /* up from 4096 to cover most cases - PERF 05/92 */
	/* should probably be an automatic work area; reexamine later */

...
#ident	"@(#)xcpxinstall:common/xcpcmd/xinstall/customR4.sh	1.1.1.7"
#ident  "$Header: customR4.sh 1.2 91/07/11 $"
...
perms="\*|inst" # known permlist names
perms_init=$perms
AWK=awk
TARCOMP=""
...
cleanup() {
	trap '' 1 2 3 15
	rm -f $init $prep $tmp* $ser $rmv_perm
	exit $1
}
...
chkswlabel() {
	...
	*)	error "incorrect product in drive"
	...
	then while true
		do
	           echo "Volume $_vol inserted as volume $vol";
...
extract() {
	...
	*)	flags=		;;
	...
	eval 'case $vol in
	'${mnt:-!}')	mntvol || return 1
		until	(cd /mnt; tar cf$flags - $filelist) | tar xf$TARCOMP -
		do	error "extraction of $filelist failed: try again? (y/n) \c"
			getyn && continue
			umount $dev
			return 1
		done
	...
	*)	until	tar xnpf$flags$TARCOMP $rdev $filelist
		do	error "extraction of $filelist failed: try again? (y/n) \c"
			getyn || return 1
		done
	esac'
...
# setselect(), setselname(), addproduct() are the only place sid can be reset.
setselect() {
	sid=$1
	case $sid in
	1)	# add a supported product
...
addproduct() {
	...
	saveprd=$prd
	execprep $prep && countperms ./tmp/perms/* || return 1
	i=`cd $perm; echo *`
	#  ./etc/perms may not exist.  Add it if necessary.
	[ -d ./etc/perms ] ||  {
		mkdir ./etc/perms
	...
	# chkfornewsets prints an error if perm is invalid
	chkfornewsets -f || return 1
	nametosid $saveprd && error "unknown set name: $saveprd" || sid=$?
	getsetvals $sid
	return 0
...
usage() {
echo "usage: custom -a product_to_add] [-m /dev/install | /dev/install1]"
echo "       custom [-s existing_product] [-i|l|r [pkgs]] [-m /dev/install | /dev/install1]"
echo "       custom [-s existing_product] [-f [files] ] [-m /dev/install | /dev/install1]"
}

# main()

# Initialize variables
perm=	arg=	vkey=	set=	cmdflg=	setchkdone=	tens=z
...
# evaluate arguments
[ $# != 0 ] && {
#	set -- `getopts i:r:l:f:m:s: $*` || {
#		echo "Usage: custom [-s set] [-ilr [pkgs]] [-f [file]] [-m /dev/install | /dev/install1]" >&2
#		exit 1
#	}
	...
	while case $1 in
		-s)	if [ -z "$addflg" ]
			then 
			    setflg=$2; shift; argflg="n"
			else
			    error "Only one selection of -s, -a"
			    usage
			    exit 1
			fi
			;;
		...
		-i)	if [ -z "$cmdname"  ] 
			then
			    cmdname="installpkgs";logflg="y"; argflg="y"
			else	
			    error "Only one selection of -i, -r, -l, -f"
			    usage
			    exit 1
			fi
			;;
		-r)	if [ -z "$cmdname"  ] 
			then
			    cmdname="removepkgs";logflg="y"; argflg="y"
			else	
			    error "Only one selection of -i, -r, -l, -f"
			    usage
			    exit 1
		...
		-l)	if [ -z "$cmdname"  ] 
			then
			    cmdname="listfiles";logflg="n"; argflg="y"
			else	
			    error "Only one selection of -i, -r, -l, -f"
			    usage
			    exit 1
		...
		-f)	if [ -z "$cmdname"  ] 
			then
			    cmdname="installfile";logflg="y"; argflg="y"
			else	
			    error "Only one selection of -i, -r, -l, -f"
			    usage
			    exit 1
		...
		-m)	dev=$2; rdev=`dirname $2`/r`basename $2`;
			if [ $dev != "/dev/install" -a $dev != "/dev/install1" ]
			then 
			    error "Device must be /dev/install or /dev/install1"
			    exit 1
			fi
			argflg="n"
			shift;
...
if [ "$cmdname" -a $logflg -eq "y" ]
then
	cmdflg='setlogtrap; $cmdname $arg; logit'
elif [ "$cmdname" -a $logflg -eq "n" ]
then
	cmdflg='"$cmdname" $arg'
fi

# If a package name is provided then you can add a new product 
# non-interactively.  Otherwise, add a product interactively.
...
# is everything here for select product? 
if [ "$setflg" -a "$cmdflg" ] 
then
	notty=y
	setselname $setflg || exit 1
	eval $cmdflg
	cleanup $?
elif [ "$addflg" = "y" ]	# is everything here for add product? 
then
	notty=y
	addproduct || exit 1
	cmdname="installpkgs";logflg="y"; argflg="y"

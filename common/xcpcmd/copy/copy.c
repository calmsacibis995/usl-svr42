...
#ident	"@(#)xcpcopy:copy.c	1.5.3.3"
#ident  "$Header: copy.c 1.2 91/07/11 $"
...
/* */
	...
	for(i=0; i<k; i++)	/* this code solves the multiple sources problem */ {
		...
		getstat();
                if ((k > 1 || type1 == S_IFDIR) && type2 != S_IFDIR && stat_flag                        != ENOENT) {
		...
		if (type1 == S_IFDIR && type2 != S_IFREG) {
			...
			char tmp_buf[sizeof(*buffer) + NAMSZ1];

			buffer = (struct dirent*)tmp_buf;
			s = buffer -> d_name;
			t = name1;

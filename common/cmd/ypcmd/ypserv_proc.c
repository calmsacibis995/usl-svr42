...
#ident	"@(#)ypcmd:ypserv_proc.c	1.6.9.2"
#ident  "$Header: ypserv_proc.c 1.3 91/09/20 $"
	...
	int prog;
{
	...
	if (prog == YPPROC_NEWXFR) {
		...
		if (!svc_getargs(transp, xdr_ypreq_xfr, &oldreq) ) {
			svcerr_decode(transp);
			return;
		}
	...
	if (prog == YPPROC_NEWXFR) {
		...
		if (!svc_freeargs(transp, xdr_ypreq_xfr, &oldreq) ) {
			FREE_ERR;
		}
	}
}

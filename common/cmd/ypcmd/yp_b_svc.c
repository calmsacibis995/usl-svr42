...
#ident	"@(#)ypcmd:yp_b_svc.c	1.6.8.2"
#ident  "$Header: yp_b_svc.c 1.3 91/09/20 $"
	...
	char **argv;
{
	...
	if (!svc_create(ypbindprog_3, YPBINDPROG, YPBINDVERS, "netpath")) {
 		msgout("unable to create (YPBINDPROG, YPBINDVERS) for netpath.");
		exit(1);
	}
...
signalHandler()
{
	(void) rpcb_unset(YPBINDPROG, YPBINDVERS, (struct netconfig *)NULL);
	exit(0);
}

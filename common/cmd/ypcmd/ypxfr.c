...
#ident	"@(#)ypcmd:ypxfr.c	1.4.6.3"
#ident  "$Header: ypxfr.c 1.3 91/09/20 $"
	...
	
{
	...
	if (strcmp(default_domain_name, domain) == 0) {
		...
		if (result) {
			if (gotkey == FALSE)
				logprintf("Update of system files %s (map = %s)\n",
					  result, map);
			else
				logprintf ("Update of system files %s (map = %s)\n",
					   result, map_alias);
	...
	int *status;
{
	...
	if((enum clnt_stat) clnt_call(domb.dom_client, YPPUSHPROC_XFRRESP,
			     xdr_yppushresp_xfr, &resp, xdr_void, 0, timeout)
	    != RPC_SUCCESS) {
		clnt_perror (domb.dom_client, "call YPPUSHPROC_XFRRESP");
		*status = YPPUSH_RPC;
		return;
	} 
}
      ...
      char *map;
{
	  ...
	  (void) strcat(cmd, " ");
	  (void) strcat(cmd, map);
	  if (debug) fprintf (stderr, "ypxfr: command: %s\n", cmd);
	  if ((ex = system(cmd)) != 0) {
	      logprintf("ypxfr: update of system file for map %s failed, exit code %d!\n",
			map, ex);
	      return (FALSE);
	  }
      }

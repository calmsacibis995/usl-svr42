...
#ident	"@(#)mail:common/cmd/mail/ckbinarsys.c	1.10.2.4"
#ident "@(#)ckbinarsys.c	2.20 'attmail mail(1) command'"
/*
    NAME
...
char	*argv[];
{
	...
	if (errflg) {
		...
		pfmt(stderr, MM_ACTION, ":164:Usage: %s [-S] -s system_name -t msg_type\n",
			argv[0]);
		exit (1);
	}
	...
	if (ckbinarsys (sysname, silent) == 0) {
		exit (0);
	} else {
		if (!silent) {
			pfmt(stderr, MM_INFO, 
				...
				sysname);
			pfmt(stderr, MM_INFO, ":166:Permission denied by '%s'\n",
				BINARSYS);
		}
		exit (1);
	}

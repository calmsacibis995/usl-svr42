...
#ident	"@(#)mail:common/cmd/mail/getsurr.c	1.5.2.3"
#ident "@(#)getsurr.c	1.9 'attmail mail(1) command'"
#include "mail.h"
/*
    NAME
	...
	for (;;) {
		...
		case 't': case 'T':
			...
			if (firstfield == TRUE) {
			    ...
			    s = s_copy(lbuf+len);
			    if (setsurg_rc(s, DEFAULT, (Ch == 'D' ? t_transport : t_translate),
			                   (int*)0, (int*)0, (int*)0, (int*)0) == (char *)NULL) {
			        s_free(s);
				return (-1);
			    }
			    s_free(s);

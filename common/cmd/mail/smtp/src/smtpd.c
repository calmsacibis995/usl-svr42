...
#ident	"@(#)mail:common/cmd/mail/smtp/src/smtpd.c	1.10.2.4"
#ident "@(#)smtpd.c	1.21 'attmail mail(1) command'"
...
int	debug;
#ifdef INETD
char	*progname = "in.smtpd";	/* Needed for logging */
#else
...
char *argv[];
{
	...
	while ((c = getopt(argc, argv, "rnH:h:dL:l:")) != EOF)
		switch (c) {
		case 'r':
			r_opt = 0;
			break;
	...
	if (errflg) {
		(void) syslog(LOG_CRIT, "Usage: %s [-L sysloadlimit] [-l smtpdlim] [-n] [-h thishost] [-H helohost] [-r]\n",
						progname);
#ifdef SVR4_1
		(void) pfmt(stderr, MM_ACTION, ":75:usage: %s [-L sysloadlimit] [-l smtpdlim] [-n] [-h thishost] [-H helohost] [-r]\n",
			progname);
#else
		(void) fprintf(stderr, "usage: %s [-L sysloadlimit] [-l smtpdlim] [-n] [-h thishost] [-H helohost] [-r]\n",
			progname);
#endif
		exit(2);
	}
...
#else
		(void) fprintf(stderr, "%s: too many args\n", progname);
#endif
		exit(2);
	}

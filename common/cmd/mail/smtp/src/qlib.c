...
#ident	"@(#)mail:common/cmd/mail/smtp/src/qlib.c	1.7.2.3"
#ident "@(#)qlib.c	1.10 'attmail mail(1) command'"
#include <sys/types.h>
#include <sys/stat.h>
#include <stdio.h>
#include <pwd.h>
	...
	if(stat(s_to_c(ln), &stbuf)==0) {
		...
		if (fp == 0 || fscanf(fp, "%d", &pid)!=1) {
			...
			pfmt(stderr, MM_INFO, ":8:can't read pid; breaking lock %s\n",
				s_to_c(ln));
#else
			fprintf(stderr, "%s: can't read pid: breaking lock %s\n",
				progname, s_to_c(ln));
#endif
			if(fp)
				fclose(fp);
...
#else
		fprintf(stderr, "%s: breaking stale lock %s\n", progname, s_to_c(ln));
#endif
		unlink(s_to_c(ln));
	}
	return 0;

...
#ident	"@(#)mail:common/cmd/mail/maillock.c	1.12.2.4"
#ident "@(#)maillock.c	2.24 'attmail mail(1) command'"
#include "maillock.h"
#include "stdc.h"
...
int	showerrs;
{
	...
	*/
	...
	sprintf(tmplock,"%s/%s", maildir, ":saved");
	if ((stat(tmplock, &statbuf) == 0) && ((statbuf.st_mode & S_IFMT) == S_IFDIR))
		sprintf(tmplock,"%s/%s/LCK..%ld", maildir, ":saved", (long)pid);
	else
		sprintf(tmplock,"%s/LCK..%ld", maildir, (long)pid);
...
char *user;
{
    ...
    sprintf(rdlk_curlock, "%s/%s.lock", RDLKDIR, user);
    ...
    memcpy(temp_curlock, curlock, sizeof(curlock));
    temp_locked = locked;
    temp_lffd = lffd;

    /* set current readlock's values */
    memcpy(curlock, rdlk_curlock, sizeof(curlock));
    locked = rdlk_locked;
    lffd = rdlk_lffd;
    ...
    memcpy(rdlk_curlock, curlock, sizeof(curlock));
    rdlk_locked = locked;
    rdlk_lffd = lffd;

    /* save the current lock's values */
    memcpy(curlock, temp_curlock, sizeof(curlock));
    locked = temp_locked;
    lffd = temp_lffd;

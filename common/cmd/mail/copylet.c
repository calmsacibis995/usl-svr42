...
#ident	"@(#)mail:common/cmd/mail/copylet.c	1.13.2.3"
#ident "@(#)copylet.c	2.31 'attmail mail(1) command'"
#include "mail.h"
/*
    NAME
...
int		Pflg;
{
	...
	while (k>0) {	/* process header */
		...
		htype = isheader (buf, &ctf, 0, pletinfo->hdrinfo.fnuhdrtype);
		Dout(pn, 5, "loop 1: buf = %s, htype= %d/%s\n", buf, htype, header[htype].tag);
		if (htype == H_CLEN) {
			save_a_txthdr(&pletinfo->hdrinfo, buf, htype);
			if ((hptr = pletinfo->hdrinfo.hdrs[H_CLEN]) != (Hdrs *)NULL) {
	...
	if ((type != ZAP) && rtrncont)
		while (k>0 || n>0) {
			if ((n > 0) && !suppress) {
				...
				if (htype == H_DATE) {
					dumprcv(&pletinfo->hdrinfo, type, htype,&didrcvlines,&suppress,f,Pflg);
					dumpaff(&pletinfo->hdrinfo, type, htype,&didafflines,&suppress,f,Pflg);
				}
			}
			if (k <= 0) {
				...
				dumprcv(&pletinfo->hdrinfo, type, -1,&didrcvlines,&suppress,f,Pflg);
				dumpaff(&pletinfo->hdrinfo, type, -1,&didafflines,&suppress,f,Pflg);
				break;
			}
			num = ((k < sizeof(buf)) ? k+1 : sizeof(buf));
			if (fgets (buf, num, pletinfo->tmpfile.tmpf) == NULL) {
			...
			case H_CLEN:
				save_a_txthdr(&pletinfo->hdrinfo, buf, htype);
				hptr = pletinfo->hdrinfo.hdrs[htype];
				if (htype == H_CLEN) {
					clen = atol (hptr->value);
				}
				/*

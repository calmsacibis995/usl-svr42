...
#ident	"@(#)cron:common/cmd/cron/funcs.c	1.8.1.4"
#ident  "$Header: funcs.c 1.2 91/06/26 $"
...
int size;
{
	...
	if((p=malloc(size)) == NULL) {
		pfmt(stdout, MM_ERROR, ":31:Out of memory: %s\n", strerror(errno));
		fflush(stdout);
		exit(55);
	}
	return p;
 ...
 *               write(2): None
 *
 * Notes: Sends a message to the cron daemon
*/
	...
	gid_t egid;
{
	...
	(void)setegid(ogid);
        ...
        pmsg->etype = etype;
        pmsg->action = action;
        (void) strncpy(pmsg->fname, fname, FILENAME_MAX);
        (void) strncpy(pmsg->logname, login, BUFSIZ);

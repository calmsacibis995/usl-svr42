...
#ident	"@(#)cron:common/cmd/cron/atq.c	1.7.9.3"
#ident "$Header: atq.c 1.2 91/07/29 $"
...
#define NOTALLOWED	"you are not authorized to use atq.  Sorry."
...
char **argv;
{
	...
	(void)setcat("uxcore");
	(void)setlabel("UX:atq");
	pp = getpwuid(getuid());
	pr.pw_uid = pp->pw_uid;
	if ((pr.pw_name = (char *)malloc(strlen(pp->pw_name) + 1)) == NULL) 
	...
	if (argc > 0) {
		...
		for (i = 0; i < argc; i++) {
			if (pr.pw_uid == 0 || 
				...
				if ((pp = getpwnam(argv[i])) == NULL) {
					pfmt(stderr,MM_ERROR,":0: ERROR: No such user %s\n",argv[i]);
					exit(1);
				}
		...
		if (badarg)
			if (argnum)
				...
				":0: ERROR: Printing queue information only for %s:\n", pr.pw_name);
			else {
				pfmt(stderr,MM_ERROR,
				":0: ERROR: Non-privileged user cannot request information regarding other users\n");
				exit(1);
			}
	}
	else if (pr.pw_uid != 0) {
	...
	if ((numentries == 0) && (!nflag)) {
				pfmt(stderr,MM_INFO,":0: no files in queue.\n");
				exit(0);
	}
	if (nflag) {
		pfmt(stderr,MM_INFO,":0: %d\n",(namewanted) ? countfiles(uidlist,argnum) : numentries);
		exit(0);
	}
	if ((namewanted) && (countfiles(uidlist,argnum) == 0)) {
		...
		pfmt(stderr,MM_INFO,":0: no files for %s.\n", (argnum == 1) ?
					c : "specified users");
		exit(0);
	}
...
char *file;
{
	...
	if (stat(file,&statb) < 0) {
		...
		pfmt(stderr,MM_ERROR,":0: ERROR: Couldn't stat spoolfile %s: %s\n",
					file,
					strerror(errno));
		return;
	}
...
char *file;
{
	...
	if ((filename = fopen(file,"r")) == NULL) {
		printf("%.27s\n", "???");
		pfmt(stderr,MM_ERROR,":0: Can't open job file %s: %s\n",
						file,
						strerror(errno));
		return;
	}
...
usage() 
{
	pfmt(stderr,MM_ERROR, ":0: usage:	atq [-c] [-n] [name ...]\n");
	exit(1);
}

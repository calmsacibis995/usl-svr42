...
#ident	"@(#)cr1:scheme.c	1.5.2.3"
#ident  "$Header: scheme.c 1.2 91/06/25 $"
...
do_protocol(int role)
{
	...
	case 'i':	/*  Act as imposer  */
		errflag = impose();
		break;

	case 'r':	/*  Act as responder  */
		errflag = respond();
		break;
...
main (int argc, char *argv[])
{
	...
	/*
	 * If authentication fails, remove the IAF module
	 * otherwise get current AVA list to modify.
	 */

	if ((errflag = do_protocol(role))) {
		(void) setava(FDOUT, NULL);
		failure(errflag, NULL);
	} else
		avas = retava(FDIN);

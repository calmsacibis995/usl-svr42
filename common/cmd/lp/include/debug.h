...
**
*/
#ident	"@(#)lp:include/debug.h	1.13.1.3"
#ident	"$Header: $"
...
#define DEBUGssd(s1, s2, i1)	\
	(void) _OPEN_DEBUG_FILE \
	(void)  fprintf (_DebugFilep, "%s/%s\t= 0d%d\n",	\
	((char *) s1 == NULL ? _Nullp : (char *) s1),	\
	((char *) s2 == NULL ? _Nullp : (char *) s2),	\
	(int) i1);					\
	(void)	fflush (_DebugFilep);

#define DEBUGsso(s1, s2, i1)	\
	(void) _OPEN_DEBUG_FILE \
	(void)  fprintf (_DebugFilep, "%s/%s\t= 0o%o\n",	\
	((char *) s1 == NULL ? _Nullp : (char *) s1),	\
	((char *) s2 == NULL ? _Nullp : (char *) s2),	\
	(int) i1);					\
	(void)	fflush (_DebugFilep);
...
#define	TRACEo(variable)	\
	DEBUGsso(FnName, #variable, variable)

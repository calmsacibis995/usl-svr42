...
#ident	"@(#)lp:include/lp.h	1.37.3.6"
#ident  "$Header: lp.h 1.2 91/06/27 $"
 ...
 * See comment above.
 */
#define BIGGEST_REQID		999
#define BIGGEST_REQID_S		"999"
...
int		_Chlvl ( char * , level_t );
int		_Close ( int );
int		_Creat ( char * , int );
int		_Fcntl ( int , int , struct flock * );
int		_Fstat ( int , struct stat * );
int		_Lchown ( char * , int , int );
int		_Link ( char * , char * );
...
void	CutAuditRec (int, int, int, char *);
int	TruncateLevel (char *, int);
char *	WrapLevel (char *, int);
int	SetProcLevel (char *);
char *	GetProcLevel (int);
int	SetFileLevel (char *, char *);

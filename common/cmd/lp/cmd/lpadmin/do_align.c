...
#ident	"@(#)lp:cmd/lpadmin/do_align.c	1.10.2.4"
#ident  "$Header: do_align.c 1.2 91/06/27 $"
...
/*
 * Variables to do resetting of printer BANNER values
 * *save_printer is a printer structure for manipulating printer settings.
 * banner_changed monitors if a printers banner setting has changed as a result
 * of this function and will require resetting after processing completes
 * Fixes MR ul91-03132
*/

PRINTER *save_printer = 0;
 ...
 *               outmsg: None
 *
 * notes - SET UP PRINTER TO PRINT ALIGNMENT PATTERNS
 */
				...
				*pwheel;
{
	...
	void			(*old_sighup)(),
     ...
     /* New code to reset banner setting of printer if necessary */
 	if (oldp -> banner == BAN_ALWAYS)
          {
           banner_changed =1 ;
           save_printer = oldp;
           save_printer->banner = BAN_OFF;
           if (putprinter(printer,save_printer) == -1 )
           LP_ERRMSG2(ERROR, E_LP_PUTPRINTER, printer, PERROR);
...
Done:		if (!(printer_status & PS_DISABLED))
	...
	fclose (fifo_fp);
        ...
        /*
         *      If banner condition of printer changes to print 
         *      alignment pattern, reset it to previous state
        */
        if (banner_changed == 1) {
        save_printer->banner = BAN_ALWAYS;
        banner_changed = 0;
        if (putprinter(printer,save_printer) == -1)
           LP_ERRMSG2(ERROR, E_LP_PUTPRINTER, printer, PERROR);

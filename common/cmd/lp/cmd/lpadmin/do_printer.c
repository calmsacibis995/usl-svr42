...
#ident	"@(#)lp:cmd/lpadmin/do_printer.c	1.22.2.3"
#ident  "$Header: do_printer.c 1.2 91/06/27 $"
 ...
 *
 * Restrictions:
 *               allow_form_printer: None
 *               deny_form_printer: None
 *               allow_user_printer: None
 *               deny_user_printer: None
 *               getclass: None
 *
 * Notes - CREATE OR CHANGE PRINTER
 */
 ...
 *               putprinter: None
 *
 * Notes - SET OR CHANGE CONFIGURATION OF PRINTER
 */
	...
	char			*list;
{
...
#endif
#ifdef	NETWORKING
			prbufp->hilevel = PR_DEFAULT_HILEVEL;
			prbufp->lolevel = PR_DEFAULT_LOLEVEL;
#endif
		/*
	...
	case 'e':
		if (!s) {
			...
			/* if the interface file for the printer specified
			 * is a symbolic link to a standard interface,
			 * then point to the standard program instead of 
			 * to the link.  This way putprinter() will create
			 * another link instead of a copy of the program.
			 * abs s19.3
			 */
			if (realpath(prbufp->interface, real_path) != NULL &&
			    strncmp(Lp_Model, real_path, strlen(Lp_Model)) == 0)
			{
			    Free(prbufp->interface);
			    prbufp->interface = Strdup(real_path);
			}
		}
		break;
...
#ifdef	NETWORKING
	case 'R':	/*  For '-R'  */
		prbufp->hilevel = R_hilevel;
		prbufp->lolevel = R_lolevel;
		break;
 ...
 *               getname: None
 *
 * Notes - ADD USER NAME TO COMMAND
 */

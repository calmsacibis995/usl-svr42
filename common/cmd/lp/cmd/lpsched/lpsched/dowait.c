...
#ident	"@(#)lp:cmd/lpsched/lpsched/dowait.c	1.6.10.3"
#ident  "$Header: dowait.c 1.2 91/06/27 $"
...
#endif
{
	...
	while (DoneChildren > 0) {
		...
		killed = KILLED(ep->status); TRACE (killed)
		exited = EXITED(ep->status); TRACE (exited)
		...
		case EX_INTERF:
			 ...
			 */
			if (exited >= 0 && exited < EXEC_EXIT_USER)
			{
			 ...
			 */
			errbuf = geterrbuf(prs); TRACEs (errbuf)
			if (errbuf ||
			    (exited > 0 && exited <= EXEC_EXIT_USER) ||
			    killed)
			{
			...
			case EXEC_EXIT_NEXEC:
				...
				printer_fault (pps, prs, EXEC_FAULT, 0);
				break;
			...
			/*
			**  If the child exited with an access error
			**  then the job is no longer suitable for
			**  the device.  Disable the device and then
			**  re-enable it to 1) repel all jobs from
			**  the device; and 2) pick up the new device
			**  level (probable cause) which will also
			**  repel jobs (reassign or even cancel) and
			**  then make the device usable again.
			*/
			case EXEC_EXIT_ACCESS:
				(void)	disable (pps,
					"Internal device check.",
					 DISABLE_STOP);
				(void)	enable (pps, NULL);
				break;
...
/*		pps->status &= ~PS_BUSY;   Already done in r_send_job */
		 ...
		 */
		prs->request->outcome |=
			(outcome & (RS_FILTERED | RS_PRINTED |
				    RS_CANCELLED | RS_FAILED));
 ...
 *               Open: None
 *               Read: None
 * Notes - READ NON-BLANK STANDARD ERROR OUTPUT
 */

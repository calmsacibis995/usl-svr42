...
#ident	"@(#)lp:cmd/lpsched/lpsched/daisyforms.c	1.5.7.3"
#ident	"$Header: $"
...
#endif
{
	DEFINE_FNNAME (check_form_alert)
...
#endif
{
	DEFINE_FNNAME (check_pwheel_alert)
...
#define NALERT	ppw->alert
	...
	requests_waiting =
		max_requests_needing_pwheel_mounted (ppws->pwheel->name);
...
#endif
{
	DEFINE_FNNAME (max_requests_needing_form_mounted)
...
#endif
{
	DEFINE_FNNAME (max_requests_needing_pwheel_mounted)
...
#endif
{
	DEFINE_FNNAME (one_printer_with_charsets)

...
#ident	"@(#)lp:cmd/lpsched/lpsched/init.c	1.10.8.2"
#ident  "$Header: init.c 1.2 91/06/27 $"
...
#endif
{
    ...
    (void) Loadfilters(Lp_A_Filters);

    Request_List = init_requests();
}

/*
 * Procedure:     init_printers
 *
 * Restrictions:
 *               Unlink: None
*/
...
/*
 * Procedure:     init_requests
 *
 * Restrictions:
 *               next_x: None (inside macro next_dir)
 *               Access: None
*/
#ifdef	__STDC__
...
#endif
{
	...
	while((name = next_file(sysdir, &sysaddr)) != NULL)
	{
	    ...
	    *(table[count]->secure) = *s;
	    if (s->status == (uint) -1 || ! (s->status & SC_STATUS_ACCEPTED))
	    {
		if (r = Getrequest (reqfile))
		{
			r->outcome &= ~RS_ACTIVE;
...
#endif
{
	DEFINE_FNNAME (init_remote_printer)

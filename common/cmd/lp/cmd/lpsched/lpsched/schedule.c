...
#ident	"@(#)lp:cmd/lpsched/lpsched/schedule.c	1.15.10.5"
#ident "$Header: 1.1 91/02/28 17:49:03 $"
...
static void	_ev_pollbsdsys (SSTATUS *);
...
static void	ev_pollbsdsys ();
static void	_ev_pollbsdsys ();
...
#endif
{
	...
	for (prs = Request_List; prs; prs = prs->next)
	{
		    ...
		    prs->request->outcome & RS_SENT)
		{
			...
			schedlog (
				"_ev_pollbsdsys: status for printers on %s\n",
				pss->system->name);

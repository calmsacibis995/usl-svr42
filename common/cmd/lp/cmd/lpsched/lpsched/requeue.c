...
#ident	"@(#)lp:cmd/lpsched/lpsched/requeue.c	1.6.9.3"
#ident	"$Header: $"
...
#else
queue_attract (pps, newRange, qchk_p, attract_just_one)
...
#endif
{
	DEFINE_FNNAME (queue_attract)

	int		called_schedule	= 0;
	RSTATUS *	prs;
	CSTATUS *	pcs;
#ifdef	CLASS_ACCEPT_PRINTERS_REJECT_SOWHAT
	int		inclass;
#endif
	/*
...
#define ISANY(PRS)	STREQU(PRS->request->destination, NAME_ANY)
	...
	for (prs = Request_List; prs; prs = prs->next)
	{
		...
		if (! SKIP_IT (prs) &&
			...
			/*
			**  If request is destined for this printer
			**  then OK.
			*/
			if (prs->printer == pps ||
#ifdef	CLASS_ACCEPT_PRINTERS_REJECT_SOWHAT
...
#endif
{
	DEFINE_FNNAME (qchk_waiting)

	return (!(prs->request->outcome &
		(RS_HELD|RS_DONE|RS_ACTIVE|RS_GONEREMOTE)) &&
		!NEEDS_FILTERING(prs));
}

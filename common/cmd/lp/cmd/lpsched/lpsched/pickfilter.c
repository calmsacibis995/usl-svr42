...
#ident	"@(#)lp:cmd/lpsched/lpsched/pickfilter.c	1.2.8.6"
#ident  "$Header: pickfilter.c 1.2 91/06/27 $"
...
/*
 * Procedure:     pickfilter
 *
 * Restrictions:
 *               insfilter: None
 *               chkprinter: None
 * Notes - SEE IF WE CAN FIND A FILTER FOR THIS REQUEST
 */
...
#endif
{
	...
	unload_str (&(prs->plen));
	unload_str (&(prs->pwid));
	unload_str (&(prs->slowparm->type));
	unload_str (&(prs->slowparm->modes));
	unload_str (&(prs->slowparm->pages));
	...
	if (prs->request->copies > 1) {
		*pp++ = PARM_COPIES;
		(void) sprintf ((*pp++ = BIGGEST_NUMBER_S), "%d", prs->request->copies);
	}
...
#define DROP(P,R)	if (P) {P[-1] = P[0] = hole; R = 0;} else
			if (chk & PCK_CPI)	DROP (p_cpi, prs->cpi) /* EMPTY */ ;
			if (chk & PCK_LPI)	DROP (p_lpi, prs->lpi) /* EMPTY */ ;
			if (chk & PCK_WIDTH)	DROP (p_pwid, prs->pwid) /* EMPTY */ ;
			if (chk & PCK_LENGTH)	DROP (p_plen, prs->plen) /* EMPTY */ ;
	...
	if (parms)
		Free ((char *)parms);
	if (modes) {
		prs->slowparm->modes = sprintlist (modes);
		freelist (modes);
	}

...
#ident	"@(#)lp:cmd/lpsched/lpNet/bsdChild/recvjob.c	1.7.2.4"
#ident "$Header: recvjob.c 1.2 91/07/29 $"
...
#endif
{
	...
	if (ns < 0) {		/* then we just created the Rhost dir */
	    	/* 
		 * Set level of the dir to SYS_PUBLIC abs s19
		 */
	    
	    lvl = PR_SYS_PUBLIC;
	    while (lvlfile (Rhost, MAC_SET, &lvl) < 0 &&
		   errno == EINTR)
		continue;
	}
	ACK();

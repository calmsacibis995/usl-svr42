	...
	char *			argv[];
{
	...
	while ((c = getopt(argc, argv, OPT_LIST)) != -1) {
		...
		case 'f':
			...
			form = optarg;
			if (!syn_name(form)) {
				LP_ERRMSG2 (ERROR, E_LP_NOTNAME, form, Namemax);
				exit (1);
			} else if (!*form)
				form = NAME_ALL;
			break;
...
#endif
{
	...
	if (STREQU(NAME_ALL, form)) {
		...
		nl = "";
		while ((ret = getform(form, &fbuf, &alert, &align_fp)) == 0
				|| errno != ENOENT) {
			if (ret == 0) {
...
#endif
{
	...
	while ((ret = getform(NAME_ALL, &fbuf, &alert, (FILE **)0)) == 0
		|| errno != ENOENT)
		if (ret == 0 && !alert.shcmd)
...
#endif
{
	...
	if (STREQU(NAME_ALL, form)) {
		...
		while ((ret = getform(form, &fbuf, (FALERT *)0, (FILE **)0)) == 0
			|| errno != ENOENT)
			if (ret == 0 && (*subaction)(fbuf.name) == 1)
				return (1);
 ...
 *               mrecv: None
 *
 * Notes - NOTIFY SPOOLER OF ACTION ON FORMS DB
*/
...
#endif
{
	...
	if (errno == EBADF) {
		...
		case LP_EBADINT:
			...
			break;
		case LP_EBADNAME:
			LP_ERRMSG2 (WARNING, E_FOR_NOTNAME, linenum, Namemax);
			break;
		case LP_EBADARG:

...
#ident	"@(#)lp:cmd/lpdata/lpdata.c	1.9.1.6"
...
#else
void	    list_class(), get_class(), list_filter(), get_filter(),
	    list_form(), get_form(), list_printer(), get_printer(),
	    list_default(), check(), fix_list(), chk_sched();
#endif	/* __STDC__ */
...
void
#ifdef __STDC__
dostty(FILE * File, PRINTER * prbuf)
#else
dostty(File, prbuf)
FILE *File;
PRINTER *prbuf;

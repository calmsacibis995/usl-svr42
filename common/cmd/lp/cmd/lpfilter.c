	...
	char			*argv[];
{
	...
	case 'f':
		...
		if (
			...
			LP_ERRMSG2 (ERROR, E_LP_NOTNAME, filter, Namemax);
			exit (1);
		} else if (!*filter)
			filter = NAME_ALL;
		break;
 ...
 *               mrecv: None
 *
 * Notes - TELL SPOOLER TO LOAD FILTER TABLE
*/
static void		alert_spooler ()
{
	char			msgbuf[MSGMAX];

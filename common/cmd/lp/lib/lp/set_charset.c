...
#ident	"@(#)lp:lib/lp/set_charset.c	1.17.2.3"
#ident	"$Header: $"
...
#endif
{
	...
	(void) tidbit ((char *)0, "daisy", &has_print_wheel);
	if (has_print_wheel)
		return (E_SUCCESS);

	(void) tidbit ((char *)0, "csnm", &char_set_names);
	if (
		(int) strlen(char_set) > 2
	     && char_set[0] == 'c'
	     && char_set[1] == 's'
	     && char_set[2]
	...
	(void) tidbit ((char *)0, "scs", &select_char_set);
	p = q = 0;
	if ((p = tparm(select_char_set, cs)) && *p && (p = Strdup(p))) {

		(void) tidbit ((char *)0, "scsd", &start_char_set_def);

...
#ident	"@(#)lp:lib/filters/insfilter.c	1.18.1.6"
#ident	"$Header: $"
...
#define	X_DONE	0x0400	/* PARM is handled in the pipeline */
...
#else
insfilter (pipes,input_type,output_type,printer_type,printer,parms,flagsp,slowparm,slowout_modes)
	char			**pipes,
				*input_type,
				*output_type,
				*printer_type,
...
#endif
{
	 ...
	 */
	{
		...
		for (ppt = parmtable; ppt->keyword; ppt++) {
			if (STREQU (ppt->keyword, PARM_MODES) ||
			    STREQU (ppt->keyword, PARM_PAGES) ||
			    STREQU (ppt->keyword, PARM_COPIES) )
				ppt->flags &= ~X_DONE;
	...
	if (!pipes) {
		...
		if (pslowf) {
			...
			ret = fl_slow;
			/*
			** Update the Job parameters that are effected by
			** slow filtering to prevent duplicate processing
			*/
			slowparm->type = Strdup (pslowf->outputp->name);
			for (pp = S.parms; pp->keyword; pp++) {
				if (pp->flags & X_DONE) {
					if (STREQU (pp->keyword, PARM_MODES))
						(void) dellist (slowout_modes, *(pp->pvalue));
					else
					if (STREQU (pp->keyword, PARM_PAGES))
						slowparm->pages = *(pp->pvalue);
					else	
					if (STREQU (pp->keyword, PARM_COPIES))
						slowparm->copies = atoi(*(pp->pvalue));
				}
			}
		} else
			pipes[0] = 0;
...
#define MARK(F,M)	(((F)->mark |= M), (F)->level = (unsigned char) our_tag)
	...
	for (pm = parms; pm->keyword; pm++) {
		...
		for (pt = pf->templates; pt->keyword; pt++) {
			...
			if (MATCH(pt, pm)) {
				...
				if (pp && (
					STREQU (pm->keyword, PARM_MODES) ||
					STREQU (pm->keyword, PARM_PAGES) ||
					STREQU (pm->keyword, PARM_COPIES) ))
						pm->flags |= X_DONE;

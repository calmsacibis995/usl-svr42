...
#ident	"@(#)lp:lib/printers/putprinter.c	1.26.2.3"
#ident  "$Header: putprinter.c 1.2 91/06/27 $"
...
#endif
{
	...
	if (prbufp->interface && (ignprinter & BAD_INTERFACE) == 0) {
		...
		if (
			...
			else if (!link_files || errno == ENOSYS) {
			...
			else {	/* symlink failed for other than ENOSYS */
			    Free (path);
			    return -1;
			}
		}

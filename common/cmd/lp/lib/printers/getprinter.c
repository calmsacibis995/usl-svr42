...
#ident	"@(#)lp:lib/printers/getprinter.c	1.24.2.3"
#ident  "$Header: getprinter.c 1.2 91/06/27 $"
...
CharStar:		if (*pp)
		...
		case	PR_RANGE:
			/*
			**  'lvlvalid()' returns EACCES if the level
			**  DB cannot be opened for any reason.  This
			**  could mean that we don't pass the access
			**  checks OR the DB doesn't exist.  The latter
			**  denotes ENOPKG.
			*/
			errno = 0;
			(void)	sscanf (p, "%ld,%ld",
				...
				(long *) &(prbuf.lolevel));
			errno = 0;
			if ((lvlvalid (&(prbuf.hilevel)) < 0 ||
			    lvlvalid (&(prbuf.lolevel)) < 0) &&
			    errno != EACCES)
			{
				freeprinter (&prbuf);
				close_lpfile (fp);
	 ...
	 */
	/* removed code that said: ``fail if we can't
	 * read it because of access permission AND we're "root"
	 * or "lp"'' abs s19.
	 */
	 ...
	 */
	if (lenlist (prbuf.printer_types) > 1 && prbuf.input_types &&
	 ...
	 * be ``unknown''.
	 */
	if (lenlist(prbuf.printer_types) > 1 &&

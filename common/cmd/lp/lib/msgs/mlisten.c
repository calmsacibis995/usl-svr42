...
#ident	"@(#)lp:lib/msgs/mlisten.c	1.14.1.3"
#ident	"$Header: $"
# include	<unistd.h>
# include	<stdlib.h>
# include	<string.h>
...
#define TURN_ON(X,F)	(void)Fcntl(X, F_SETFL, (struct flock *) (Fcntl(X, F_GETFL, (struct flock *) 0)|(F)))
...
#endif
{
    ...
    ConsSize = 20;
    Connections = (MESG **) Calloc(ConsSize, MDSIZE);
    PollFdList = (struct pollfd*) Calloc(ConsSize, sizeof(struct pollfd));
	...
	*/
	...
	for (x = 0; x < NumCons; x++)
	{
		...
		*/
		...
		recvbufp = (struct s_strrecvfd *)
                        Calloc(1, sizeof(struct s_strrecvfd)
                                + cred_size
                                - sizeof(struct sub_attr));
		if (!recvbufp)
		{
			errno = ENOMEM;
			return	NULL;
		}
		if (ioctl(mdp->readfd, I_S_RECVFD, recvbufp) != 0)
		{
		    if (errno == EINTR)
		    {
			Free (recvbufp);
		...
		*/
		...
		md->credp = (struct sub_attr *) Calloc (1, cred_size);
		...
		md->mque = NULL;
		md->on_discon = NULL;
		md->readfd = md->writefd = recvbufp->fd;
		md->state = MDS_IDLE;
		md->type = MD_UNKNOWN;
		md->uid = recvbufp->uid;
		if (flvlfile (md->readfd, MAC_GET, &(md->lid)) < 0)
		{
			Free (recvbufp);
			Free (md->credp);
			Free (md);
		...
		(void)	memcpy (md->credp, &(recvbufp->s_attrs),
			cred_size);
...
#endif
{
	...
	if ((md->on_discon =
	     (PTR_TO_VOID_FUNC_PTR) Calloc (size, sizeof(VOID_FUNC_PTR)))
	     == NULL)
	{
	    errno = ENOMEM;
	    return(-1);

...
#ident	"@(#)lp:lib/users/loadpri.c	1.15.1.4"
#ident	"$Header: $"
...
empty:
	    ...
	    (void)memset ((char *)pri_tbl.users, 0, sizeof(pri_tbl.users));
	    return (&pri_tbl);
	}
	return(0);
    }
	...
	if (!(user = next_user(f, line, &p)))
	{
	    ...
	    {
		pri_tbl.deflt_limit = (short) opri;
		if (pri == -1) break;
...
Error:
	...
	do
	{
	    (void)add_user (&pri_tbl, user, pri);
	}

...
#ident	"@(#)devmgmt:common/cmd/devmgmt/devreserv/main.c	1.7.11.2"
#ident  "$Header: main.c 1.3 91/06/26 $"

/***************************************************************************
 * Command: devreserv
 * Inheritable Privileges: P_MACWRITE
 *       Fixed Privileges: None
 * Notes: reserves devices for exclusive use.
 *
 ***************************************************************************/
	...
	char	       *argv[];		/* Arg vector */
{
	...
	if ((fp=fopen(DDB_TAB,"r"))==(FILE *)NULL) {
	    (void) sprintf(txt, M_DEVTAB, DDB_TAB);
	    stdmsg(MM_NRECOV, lbl, MM_ERROR, txt);
	    exit(EX_TABLES);
	}
	...
	if (reqlist = buildreqlist(argp)) {
	     ...
	     * P_MACWRITE privilege required in case the lock file
             * needs to be created under /etc which is SYS_PUBLIC.
	     */

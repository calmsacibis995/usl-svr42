...
#ident	"@(#)devmgmt:common/cmd/devmgmt/devattr/main.c	1.7.10.2"
#ident  "$Header: main.c 1.3 91/06/27 $"
...
#define M_USAGE		"syntax incorrect, invalid options\n\
USAGE: devattr [-v] device [attribute [...]]"
#define	M_CONSTY	"Device Database in inconsistent state - notify administrator"
#define	M_NODEV		"requested device not found in Device Database"
#define	M_DEVTAB	"Device Database could not be opened for reading"
#define	M_NOMEM 	"Insufficient memory"
	...
	char   *argv[];		/* Pointer to arguments */
{
	...
	if ((ddb_state=ddb_check()) < 0) {
	   /* Not all the DDB files present/accessible.    */
	    stdmsg(MM_NRECOV, lbl, MM_ERROR, M_DEVTAB);
	    exit(EX_DDBPROB);
	} else if(ddb_state==0) {
	   /* DDB files corrupted - magic-nos do not match */
	    stdmsg(MM_NRECOV, lbl, MM_ERROR, M_CONSTY);
	    exit(EX_DDBPROB);
	}
...
level_t level;
{
	...
	if (bufp=(char *)malloc(bufsz)) {
		/* convert level to requested format */
	   	lvlout(&level, bufp, bufsz, Lvlform);
	    	return(bufp);
	} else {
	    	/* out of memory */
	        stdmsg(MM_NRECOV, lbl, MM_ERROR, M_NOMEM);
	    	exit(EX_ERROR); 
	}
}

...
#ident	"@(#)passwd:passwd.c	1.4.8.8"
#ident  "$Header: passwd.c 1.2 91/06/24 $"
	...
	char **argv;
{
	 ...
	 *
	 * Called first because ck_xpass() clears the P_SYSOPS privilege
	 * from the maximum set if this is a graphical passwd command.
	 */
	graphics = ck_xpass();
	...
	char	*uname;
{
	 ...
	 */
	return update_sfile(SHADOW, OSHADOW, SHADTEMP, uname);
}
	...
	uid_t	real_uid;
{
	...
	if (!graphics) {
		 ...
		 *
		 * NOTE:	The IAF login scheme explicitily clears the
		 *		P_SYSOPS privilege in its maximum privilege
		 *		set before calling "passwd".  This means that
		 *		a user can change their own password at login
		 *		time and still not be considered a password
		 *		administrator.
		 *
	...
	if ((defltfp = defopen(PWADMIN)) != NULL) { /* M005  start */
		if ((char_p = defread(defltfp, "LOGIN_ONLY")) != NULL) {
			if (*char_p) {
				 ...
				 *
				 * NOTE: This default value is not valid with
				 *	 the graphical passwd command.
				 */
				if (!graphics && !strcmp(char_p, "Yes")) {
					login_only = 1;
				}
			}
		}
...
ck_xpass()
{
	...
	/*
	 * Special case for graphical passwd:
	 *
	 *	Clear P_SYSOPS from maximum set.
	 */
	(void) procprivl(CLRPRV, pm_max(P_SYSOPS), (priv_t *)0);

	...
	if (sflg)
		...
		case 1 :
			while (fread(&acct.oab, sizeof(acct.oab), 1, stdin) == 1) {
				...
				cmt.tcm_io = (ulong_t) expand(acct.oab.ac_io);
				cmt.tcm_rw = (ulong_t) expand(acct.oab.ac_rw);
				tenter(&cmt);
			}
			break;
		case 2 :
			...
			while (fread(&acct.ab, sizeof(acct.ab), 1, stdin) == 1) {
				...
				cmt.tcm_io = (ulong_t) expand(acct.ab.ac_io);
				cmt.tcm_rw = (ulong_t) expand(acct.ab.ac_rw);
				tenter(&cmt);
			}
			break;
		}

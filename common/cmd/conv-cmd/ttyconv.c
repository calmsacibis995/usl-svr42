...
#ident	"@(#)conv-cmd:ttyconv.c	1.5.3.3"
#ident "$Header: ttyconv.c 1.2 91/09/30 $"
 ...
 *		- it also does similar conversion for /etc/conf/init.d/kernel
 *
 *		- requirements:
 *			- /usr must be mounted
...
#define	GREP_INITTAB	"grep getty %s"
#define CHG_CONSOLE	"sed '/^co:/s/\\/.*$/\\/usr\\/lib\\/saf\\/ttymon -g -p \"%s\" -m ldterm -d \\/dev\\/console -l %s/' %s > _tmpinittab"
#define RMGETTY		"sed '/getty/d' %s > _tmpinittab" 
...
#define	SVCADD	\
"/usr/sbin/pmadm -a -p %s -s %s -i root -v `/usr/sbin/ttyadm -V` %s %s -m \"`/usr/sbin/ttyadm %s -p \\\"%s\\\" -d %s -s %s -l %s`\" 2>/dev/null"

#define	MVTMP		"mv _tmpinittab %s"

#define IA_SCHEME	"/usr/bin/shserv"
...
main()
{
	...
	/* get all getty entries */
	conv_getty("/etc/inittab");
	conv_getty("/etc/conf/init.d/kernel");
}
static void
conv_getty(s)
	char	*s;	/* filename */
{
	...
	char	grep_inittab_cmd[BUFSIZ];
	char	rmgetty_cmd[BUFSIZ];
	char	mvtmp_cmd[BUFSIZ];

	sprintf(grep_inittab_cmd, GREP_INITTAB, s);
	sprintf(rmgetty_cmd, RMGETTY, s);
	sprintf(mvtmp_cmd, MVTMP, s);

	if (( gfp = popen(grep_inittab_cmd, "r")) == NULL) {
		perror("popen failed");
		exit(1);
	}

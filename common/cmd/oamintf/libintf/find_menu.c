...
#ident	"@(#)oamintf:common/cmd/oamintf/libintf/find_menu.c	1.3.6.2"
#ident  "$Header: find_menu.c 2.0 91/07/12 $"
...
char *path;	/* logical path name to look for */
{
	...
	*(strrchr(cur_pdir, '/')) = NULL;
	...
	(void) strcpy(m_name, MAIN_MENU);
	(void) strcat(curmenu, MAIN_MENU);  /* add main menu name to curmenu */
		...
		while(itemptr != NULL) {
			if((strncmp(itemptr->mname, next, MAXITEMLEN) > 0) && 
			   (!last)) {
				...
				if(last) {
					m_item.exists = 1;
					if(( (int) strlen(itemptr->pkginsts) > 2)
					   /* pkginst present */
					   && isfmlobj) {
						(void) strcpy(pkginst, 
							(itemptr->pkginsts+1));

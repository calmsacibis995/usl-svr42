...
#ident	"@(#)filemgmt:common/cmd/oamintf/files/bin/rmdisk.sh	1.4.3.2"
#ident  "$Header: rmdisk.sh 2.1 91/09/12 $"

trap '/usr/bin/rm -f /tmp/$$.*' 0
...
/usr/bin/cp /etc/fstab /etc/Ofstab
/usr/bin/cp /tmp/$$.keep /etc/fstab

eval /usr/bin/rm /dev/dsk/${dskname}? /dev/SA/$1 \

...
#ident	"@(#)filemgmt:common/cmd/oamintf/files/bin/checkfs.sh	1.3.4.2"
#ident  "$Header: checkfs.sh 2.1 91/09/12 $"
...
then
	$TFADMIN /sbin/fsck -F ${fstype} -n ${ddrive} >>/tmp/check.err 2>>/tmp/check.err
else
	$TFADMIN /sbin/fsck -F ${fstype} -y ${ddrive} >>/tmp/check.err 2>>/tmp/check.err
fi
#exit=$?

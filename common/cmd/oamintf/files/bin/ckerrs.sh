...
#ident	"@(#)filemgmt:common/cmd/oamintf/files/bin/ckerrs.sh	1.6.4.2"
#ident  "$Header: ckerrs.sh 2.1 91/09/12 $"
...
majmin=`ls -l ${bddrive} 2>/dev/null  |
	/usr/bin/sed -n 's/.*\( [0-9]\{1,\}, *[0-9]\{1,\} \).*/\1/p'`

#It is possible there is no corresponding block device for a character dev

if [ -n "${majmin}" ]
then
	mounted=`$TFADMIN /sbin/mount  |  /usr/bin/cut -d' ' -f3  \
		|  xargs ls -l 2>/dev/null  \
		| /usr/bin/sed -n "/${majmin}/s/.* \\(.*\\)/\\1/p"`
fi
if [ -n "${mounted}" ]
then
	filesys=`$TFADMIN /sbin/mount \
		| /usr/bin/sed -n "\\; on ${mounted} ;s;^\\([^ ]*\\) .*;\\1;p"`
...
#if /usr/bin/dd count=1 if=${ddrive} of=/dev/null 2>/dev/null
$TFADMIN /sbin/mount -F $fstype $ddrive /mnt
if [ $? = 0 ]
then
	$TFADMIN /sbin/umount /mnt
else
	echo >>/tmp/check.err '

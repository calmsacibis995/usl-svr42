...
#ident	"@(#)oamintf:common/cmd/oamintf/edsysadm/mkchgs.sh	1.11.10.4"
...
do
	...
	if [ "${THIS}" = "TASK" ] 
	then
		BASENM=`basename $THAT`
		fmliobj=`$ECHO ${BASENM} | $CUT -f1 -d"."`
...
#	if $GREP "^${NAME}\^" $oampath/${NAME}.menu
	...
	if [ "${FLAG}" = "addmenu" ]
	then
		[ ! -d $newpath -a -n "$newpath" ] && $MKDIR  $newpath

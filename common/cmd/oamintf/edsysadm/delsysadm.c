...
#ident	"@(#)oamintf:common/cmd/oamintf/edsysadm/delsysadm.c	1.3.8.5"
#ident  "$Header: delsysadm.c 2.0 91/07/12 $"
...
struct search_menus {
	char mkywd[(NAMELEN+1)];	/* for mname ifit's a .menu */
	char mwhere[ACTLEN];		/* for dir part of maction */
	char mwhat[ACTLEN];		/* for .menu part of maction */
...
/*
 * GETFULL
 * takes one argument, menu_item->path - decodes it into an 
 * absolute path, then sets the global variables intfpath,
 * relpath, and exprpath.
 */
static void
getfull(location)
	char *location;
{
	...
	char *graboam;
	int start_rel;
	char tmplocn[PATH_MAX]; /* tmp buffer to hold arg */

	start_rel = 0;
	(void) strcpy(tmplocn, location); /* load tmp buffer */
	checkit = strtok(tmplocn,PATHDEL);
 ...
 * GETLOCN
 * 
 * Decodes a menu_item->path into an absolute path by decoding
 * OAMBASE.
 */

static void
getlocn(location, pathnm)
	char *location; /* location to be decoded. */
	char *pathnm; /* buffer to store results in. must be big */
		       /* enough */
{
	char *checkit;
	char *oambase;
	char menupath[MLNSZ]; 
	char tmplocn[PATH_MAX]; /* tmp buffer to save argument */

	(void) strcpy(tmplocn, location); /* load tmpbuffer */
	checkit = strtok(tmplocn ,PATHDEL);
	...
	if ((checkit=strtok(NULL,PATHDEL)) != NULL) {
		...
		while ((checkit=strtok(NULL,PATHDEL)) != NULL) {
			(void) sprintf(menupath,"%s%s%s",menupath,
			    PATHDEL,checkit);
		}
	} else {
		/*
		 * We only got oambase off of locn. We
		 * need to add '/menu' for pathname to be correct.
		 * There are no menus at OAMBASE! We are looking
		 * at the main menu.
		 */
		 (void) sprintf(menupath, "%s%s%s", oambase, PATHDEL,
		     EU_PATH);
	}

	(void) strcpy(pathnm, menupath);
	...
	while(xfer_item != NULL) {
		...
		if (sameitem) {	/* skip ifnot placeholder,
			...
			if ((xfer_item->pholder == P_INACTIVE) ||  
				...
				(void) strcat(outbuf, "\n");
				if (fputs(outbuf, file) == EOF)
					return(1); /* was -1*/
				vacant = 0;	/* parent menu has placeholder item */
			}
			/* else skip this entry - menu empty? */
...
int errcode;
{
	switch (errcode) {
		...
		case SUCCESS :
			/*
			 * let's remove the expr temp file. No need
...
char *newreal;			/* original tmp file, becomes new parent menu */
{
	...
	/* we were successful - remove temp */
	(void) unlink(old_real);
	return(0);
}	/* end of parnt_xfer */
/*
**	parnt_travel
...
parnt_travel()
{
	...
	if ((pitemz = find_menu(prog, findparent)) != NULL) {
		getlocn(pitemz->path, pathname); 
		(void) sprintf(parntmenu,"%s%s%s",pathname,PATHDEL,
		    pitemz->menu_name);
	...
	if (piptrz == NULL) {
		...
		while ((piptrz != NULL) && (strncmp(parntgot, piptrz->mname, strlen(parntgot)) != 0)) {
			piptrz = piptrz->next;
	...
	while(xfer_item != NULL) {
		...
		if ((sameitem) && (vacant)) {	
			...
			if ((xfer_item->pholder == P_INACTIVE) ||  
		     	     (xfer_item->pholder == P_ACTIVE)) {
				(void) sprintf(outbuf, "%s^%s^", 
			           xfer_item->mname,xfer_item->mdescr);
				(void) strcat(outbuf, "placeholder^");
				(void) strcat(outbuf, xfer_item->maction);
				(void) strcat(outbuf, "\n");
				if (fputs(outbuf, file) == EOF)
...
char *argv[];
{
	...
	if ((mitemz = find_menu(argv[0], argv[argitem])) != NULL) {
		getfull(mitemz->path);	/* to get full dir struct */
		getlocn(mitemz->path, pathname);
		(void) sprintf(menufile,"%s%s%s",pathname,PATHDEL,
		    mitemz->menu_name);
	...
	*/
	...
	if (iptrz == NULL) {
		...
		*/
		...
		/*
		 * item_gone() has the side effect of setting vacant.
		 */
		...
		/*
		 * If vacant is set, then empty menu? else, menu
		 * just has a placeholder??? What is the 
		 * difference between a empty menu and a full one
		 * with a placeholder????? Must be: vacant == empty
		 * menu, !vacant means nonempty, with possible
		 * placeholder.
		 */
		if (vacant) {

...
#ident	"@(#)oamintf:common/cmd/oamintf/intf_install/mod_menus.c	1.6.11.2"
#ident  "$Header: mod_menus.c 2.0 91/07/12 $"
...
char **argv;
{
	...
	struct stat buf;	/* used to check existence of menu file before sorting */
	...
	while(fgets(instr, sizeof(instr), miffile)) {
		...
		/*
		 * In case an existing menu is not in alphabetical order, sort it
		 * before inserting a new entry.  An out of order menu may exist 
		 * if a package developer appended an entry directly into a menu
		 * (an illegal but currently practiced method).  When inserting a
		 * new entry into a menu, find_mif() only looks up to the correct 
		 * alphabetical position at which the entry should be placed.  If
		 * the menu file is out of order to begin with and a duplicate
		 * entry of the new one being inserted exists past the point where
		 * the new one is to be inserted, then the collision would not be
		 * detected.  This would result in duplicate name entries in the 
		 * menu file.  Sorting it now not only prevents the collision
		 * but also cleans up the possible mess resulting from improper 
		 * insertion of menu items from postinstall scripts.
		 * 
		 * If menu exists (we're inserting an entry into an existing menu),
		 * set up pointer to head of sorted menu.  Otherwise, don't attempt
		 * to sort what doesn't exist yet.
		 */
		if (stat(cur_menu, &buf) < 0) {
			if (errno != ENOENT) {
				errors++;
				break;
			}
		...
		/*
		 * If there are some existing entries in 
		 * menu, go ahead and sort those entries.
		 */
		else if (file_str->entries != NULL)
			file_str->entries = menu_sort(file_str->entries);
...
char *name;		/* name of new menu item to write into menu file */
{
	...
	/*
	 * Create new directory via mk_dir() function in libintf.a.  This
	 * function creates the new directory and runs the installf command 
	 * to set the attributes for the new directory according to DIRINFO
	 * as defined in ~oamintf/int_include/intf.h.
	 */
	if(tflag) {
...
/**************************************************
 * menu_sort()
 * argument expected: head of menu item list
 * argument returned: head of sorted menu item list
 ***************************************************/
static struct item_def *
menu_sort(i_head)
struct item_def	*i_head;
{
	struct	item_def *i_curr;
	struct	item_def *i_next;
	struct	item_def *i_hold;
	struct	item_def *i_prev;
	int	flag;
	int	n;
	int	k;
	int	j;

	/* Determine number of entries in menu file */
	for (i_curr = i_head, n = 1; i_curr->next != NULL;
		++n, i_curr = i_curr->next)
			;
	k = n;
	flag = n;
	while (flag > 0) { 
		...
		i_prev = NULL;
		for (j = 1; j <= k; j++) {
			if(strncmp(i_curr->mname, i_next->mname, sizeof(i_curr->mname)) > 0) {
				flag = j;
				i_hold = i_curr;
				i_hold->next = i_next->next;
				i_next->next = i_hold;
				if (i_hold == i_head)
					i_head = i_next;
				if (i_prev != NULL)

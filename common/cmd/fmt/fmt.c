...
#ident	"@(#)fmt:fmt.c	1.2.5.1"
#ident "$Header: fmt.c 1.2 91/08/08 $"
...
char	outbuf[MAXBSIZE];		/* Sandbagged output line image */
...
fmt(fi)
	FILE *fi;
{
	char linebuf[MAXBSIZE], canonb[MAXBSIZE];
...
char *c;
{
	...
	if (c == NULL) {
		fprintf(stderr, "usage: fmt [-c] [-s] [-w width | -width] [inputfile...]\n");
		fprintf(stderr,"	width not specified following -w option\n");	
		exit(1);
	}
	while (*c >= '0' && *c <= '9')
		n *= 10, n += *c++ - '0';
	if (*c || n <= 0 || n >= MAXBSIZE -2) {
		fprintf(stderr, "usage: fmt [-c] [-s] [-w width | -width] [inputfile...]\n");

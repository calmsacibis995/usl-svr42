...
#ident	"@(#)face:src/filecab/fileb/chkperm.c	1.6.7.4"
#ident "$Header: chkperm.c 1.12 91/11/26 $"
/*                       chkperm.c
 *
...
#define e_val(A)	(((A) == YES) ? gettxt("uxface:61", "yes") : gettxt("uxface:56", "no"))
...
char **argv;
{
...
#ifndef NO_ES_VERSION
	level_t	set_val, test_lid;
	uid_t	privid;
	int	es_running=0, mac_absent=1;
#endif
	...
	if(opterr) 
	{
		fprintf(stderr, gettxt("uxface:38", "Usage: chkperm -l|-d|-v|-t cap|-e cap|-y cap|-n cap [-u user-name]\n"));
		exit(FAIL);
	}
	...
	if (privid < 0 )
		es_running = 1; /* LPM environment */
	else
		es_running = 0; /* SUM environment */
	if (es_running == 0)
#endif
	   if ( (dflg || yflg || nflg) && getenv("FACEPID") == NULL && getuid() != 0 ) {
		if ( dflg )
			fprintf(stderr,gettxt("uxface:42", "You must be super-user to undefine a FACE user.\n"));
		else
			fprintf(stderr,gettxt("uxface:40", "You must be super-user to set FACE permissions for a user.\n"));
		exit(FAIL);
	   }
...
char *capname;
{
	...
	for (index = 0; index < CAPS; index++)
		if ( strcoll(capname,caps[index].name) == 0 )
			return( index );

	fprintf(stderr,gettxt("uxface:24", "Invalid cap-name: %s\n"),capname);
	return(FAIL);
}
...
/*
		...
		if ( fwrite((char *)iobuf,sizeof(*iobuf),1,fp) != 1 ) {
			fprintf(stderr,gettxt ("uxface:22","Error writing permissions file.\n");
			return(FAIL);
		}
*/
	}
...
get_uname()
{
	...
	if ((pw = getpwuid(getuid())) == NULL) {
		fprintf(stderr,gettxt("uxface:11", "Can't read user name on system.\n"));
		return(NULL);
	}
...
open_file()
{
	...
	if ( (fpt = getenv(fbase)) == NULL ) {
		fprintf(stderr,gettxt("uxface:1", "$%s must be set in the environment.\n"),fbase);
		return(NULL);
	}
	...
	if ( access(fpath,00) ) {
		if ( (fp = fopen(fpath,"w+")) == NULL ) {
			fprintf(stderr,gettxt("uxface:20", "Error creating %s\n"),fpath);
			return(fp);
		}
#ifndef NO_ES_VERSION
		else {
			if ((lvlproc (MAC_GET, &test_lid) !=0 ) && (errno == ENOPKG))
				mac_absent = 1; /*MAC is not installed */
			else
				mac_absent = 0 ; /* MAC is installed */
			if (mac_absent == 0) {
			   (void)fclose(fp);
			   set_val = 1; /*SYS_PUBLIC =1 */
			   if ( lvlfile (fpath, MAC_SET, &set_val) != 0) {
				fprintf(stderr, gettxt("uxface:21", "Error in setting MAC Level\n"));
				return(NULL);
			   }	
			}
		...
		if ( (fp = fopen(fpath,"r+")) == NULL ) {
			fprintf(stderr,gettxt("uxface:19", "Error accessing %s\n"),fpath);
			return(fp);
		}
	}

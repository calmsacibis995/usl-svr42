...
#ident	"@(#)face:src/filecab/fileb/mnucheck.c	1.2.4.3"
#ident  "$Header: mnucheck.c 1.6 91/11/18 $"
...
char *argv[];
{
	...
	if(access(hpath, 00) == 0) {
		if ((fp=fopen(hpath, "r")) == NULL) {
			fprintf(stderr, gettxt("uxface:15", "Cannot open file %s"),hpath);
			return(FAILURE);
		}
	...
	if(access(path, 00) == 0) {
		if ((nfp=fopen(path,"r")) == NULL) {
			fprintf(stderr, gettxt("uxface:15", "Cannot open file %s"),path);
			return(FAILURE);
		}

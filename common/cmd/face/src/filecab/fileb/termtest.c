...
#ident	"@(#)face:src/filecab/fileb/termtest.c	1.6.4.3"
#ident  "$Header: termtest.c 1.6 91/11/18 $"
...
char	*argv[];
{
	...
	for (i = 0; i < 2; i++) {
		if (i && Debug)
			puts(gettxt("uxface:34", "Trying again..."));
	...
	ioctl(0, TCSETAF, &Restore);
	if (Retval)
		fputs(gettxt("uxface:10", "Can't find identify file\n"), stderr);
	exit(Retval);
}
...
char	*dirname;
{
	...
	debug_out(FALSE, gettxt("uxface:26", "Processing file:"), path);
	...
	if (Tok == END)
		write(2, gettxt("uxface:23", "Extra 'end' ignored\n"), 20);
	fclose(Fp);
}
...
char	arg[10][ARGSIZ];
{
	...
	case QUERY:
		...
		Level--;
		if (Tok == EOF) {
			write(2, gettxt("uxface:25", "Missing 'end'\n"), 14);
			return;
		}
	...
	case BREAK:
		...
		if (!skip) {
			if (s) {
				...
				fputs(gettxt("uxface:27", "Syntax error: 'break' outside of 'query'\n"), stderr);
		}
		break;
	case PRINT:
		s = get_string(arg);
	...
	case EXIT:
		...
		break;
	default:
		write(2, gettxt("uxface:29", "Syntax error\n"), 13);
		break;
	}
	get_token();
}
	...
	if (!skip) {
		if (recv) {
			...
			if ((ex = regcmp(s, NULL)) == NULL) {
				fputs(gettxt("uxface:5", "Bad regular expression\n"), stderr);
				cleanup(2);
			}
			...
			fputs(gettxt("uxface:28", "Syntax error: 'case' outside of 'query'\n"), stderr);
	}
...
char	*s;
{
	...
	if (recv = lookup(s))
		debug_out(FALSE, gettxt("uxface:53", "found previous response of"), recv);
	else {
		recv = recvbuf;
		if (Debug)
			fputs(gettxt("uxface:59", "sending... "), stdout);
		ioctl(0, TCFLSH, 2);
		ioctl(2, TCXONC, 1);
		write(2, s, strlen(s));
		ioctl(2, TCSBRK, 1);
		...
		*rcvptr = '\0';
		numread = Level;
		Level = 0;
		debug_out(FALSE, gettxt("uxface:58", "received"), recvbuf);
		Level = numread;
		recv = enter(s, recv);
	}
	return recv;

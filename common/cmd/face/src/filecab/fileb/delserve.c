...
#ident	"@(#)face:src/filecab/fileb/delserve.c	1.4.4.3"
#ident  "$Header: delserve.c 1.6 91/11/18 $"
...
char *argv[];
{
	...
	if ((fp=fopen(spath,"r")) == NULL) {
		fprintf(stderr, gettxt("uxface:15", "Cannot open file %s"),spath);
		return FAIL;
	}

	if ((nfp=fopen(tpath,"w+")) == NULL) {
		fprintf(stderr, gettxt("uxface:15", "Cannot open file %s"),tpath);
		return FAIL;
	}

...
#ident	"@(#)face:src/filecab/fileb/modserv.c	1.3.4.3"
#ident  "$Header: modserv.c 1.6 91/11/18 $"
...
char *argv[];
{
	...
	if(strcoll(term,oterm) == 0 && strcoll(appath,oapath) == 0 && strcoll(dir,odir) == 0 ){
		...
		if ((fp=fopen(hpath,"w+")) == NULL) {
			fprintf(stderr,gettxt("uxface:14", "Cannot open file"));
			return FAIL;
		}

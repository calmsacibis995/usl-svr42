...
#ident	"@(#)pg:pg.c	1.31.1.11"
#ident "$Header: pg.c 1.2 91/08/13 $"
...
char *file_name;
{
	...
	if (!firstf)
		...
		firstf = 0;
		if (fnum < (nfiles-1))	/* check if more filenames */
			cmd_ret = command(file_name);
		if (initopt) {
			initopt = 0;
...
help()
{
	...
	(void) pfmt(stdout, MM_NOSTD, ":767:+1<newline> (next page); -1<newline> (previous page); 1<newline> (page 1).\n");
	(void) putc('\n', stdout);
...
register int n;
{
	...
	/*
	 * Take care of case when we are searching for pattern

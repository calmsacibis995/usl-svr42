...
#ident	"@(#)mv:mv.c	1.30.9.3"
#ident  "$Header: mv.c 1.3 91/08/13 $"
 ...
 *	   otherwise, just set the mode bits via chmod(2).
 ...
 *	3. this macro will not exit nor return; this is consistent
 *	   with not checking error on chmod.
 *	4. don't take any action on failure from acl call, except if
 *	   ENOSPC; on ACL_GET source may be on remote system not
 *	   supporting acl call; on ACL_SET target may be on non SFS fs,
 *	   in which case we just set the mode bits via chmod(2).
 */

#define	TRANSFERDAC(source, target, modebits) { \
	int aclerr = 0; \
	if (aclpkg) { \
...
register char *argv[];
{
	 ...
	 * 	cp [-e ignore|warn|force] [-i] [-p] [-r] dir1 dir2
	...
	if (cpy) {
		while ((c = getopt(argc, argv,"e:ipr")) != EOF)
			switch (c) {
                                case 'e':
					earg = optarg;
					if (!EQ(earg, "warn") &&
	...
	if (argc > 2) {
		if (stat(argv[argc-1], &s2) < 0) {
			pfmt(stderr, MM_ERROR, badstat,	argv[argc-1], strerror(errno));
			exit(2);
		}
...
char *source, *target;		/* filenames */
{
	...
	if (cpy) {
		...
		if (fixmode) {
			TRANSFERDAC(source, target, fixmode & MODEBITS);
		}
		if (pflg) {
			/*
...
, ":1012:Access and modification times for \"%s\" couldn't be reset\n"
	...
	if (ISDIR(s1)) {
		pfmt(stderr, MM_ERROR
			,":340:Cannot move directories across file systems\n");
		return 1;
	}
	...
	if (ISDEV(s1)) {
		...
		if (mknod(target, s1.st_mode, s1.st_rdev) < 0) {
			pfmt(stderr, MM_ERROR, errmsg, target,strerror(errno));
			return 1;
		}

		TRANSFERDAC(source, target, FMODE(s1));
		setimes(target);
		goto cleanup;
	}
	 ...
	 */
	if ((spd.st_mode & S_ISVTX) && (s1.st_uid != uid)
...
noattr:
	 ...
	 */
	if ((n > 0) && ((addr1= (char *)sbrk(0)) != (char *)-1)) {
		/*
		...
		addr2 = mmap(addr1, (size_t) n, PROT_READ
			, MAP_SHARED|MAP_FIXED, fi, 0);
		if (addr2 != (char *)-1) {
			ret = write(fo, addr2, (size_t) n);
			(void) munmap(addr2, (size_t)n);
			/*
...
char *source, **to;
{
	...
	if ((*statf)(target, &s2) >= 0) {
		...
		if ((*statf)(target, &s2) >= 0) {
			...
			if (iflg && ((mve && !silent && isatty(fileno(stdin))) || 
				     (cpy))) {
				pfmt(stderr, MM_NOSTD, ":345:%s: overwrite %s? ",
					cmd, target);
				if (getresp()) {
					if (buf != NULL)
						free(buf);
			...
			if (!cpy && (access(target, 2) < 0) &&
				...
				pfmt(stderr, MM_NOSTD,
					":346:%s: Existing %s: %o mode. Remove (y/n)? ",
...
usage()
{
	 ...
	 */
 	...
 	opt = cpy ? " [-e ignore|warn|force] [-i] [-p]" : lnk ? " [-f] [-n] [-s]" : " [-e ignore|warn|force] [-f] [-i]";
	...
	if(mve)
		...
		pfmt(stderr, MM_NOSTD, ":350:                      ln [-f] [-n] [-s] d1 d2\n");
	else if (cpy)
		pfmt(stderr, MM_NOSTD, ":1086:                      cp [-e ignore|warn|force] [-i] [-p] [-r] d1 d2\n");
	exit(2);
}

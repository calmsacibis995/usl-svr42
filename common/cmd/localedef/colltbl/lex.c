...
#ident	"@(#)localedef:common/cmd/localedef/colltbl/lex.c	1.1.5.2"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/localedef/colltbl/lex.c,v 1.1 91/02/28 17:44:43 ccs Exp $"
#include <stdio.h>
#include <string.h>
#include <ctype.h>
#include "colltbl.h"
	...
	default:
		...
		do {
			...
			if (c == '\\' || c == '0') {
				...
				j = 2;
				if (c == 'x' || c == 'X') {
					while (isxdigit(c=getchar()) && j<4)
						ibuf[j++] = c;
				} else {
					c = getchar();
					while (isodigit(c) && j<4) {
						ibuf[j++] = c;
						c = getchar();
					}
				}

...
#ident	"@(#)diff:diff.c	1.25"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/diff/diff.c,v 1.1 91/02/28 16:54:57 ccs Exp $"
...
char *pfiletype();	/*return the name of the type of the file */
...
static const char useC[] = ":143:Use -C num\n";
static const char badstat[] = ":21:Cannot access %s: %s\n";
static const char badopen[] = ":92:Cannot open %s: %s\n";
static const char badread[] = ":144:Read error in %s: %s\n";
static const char nonl[] = ":145:Missing newline at end of file %s\n";
static const char onlyin[] = ":146:Only in %.*s";
static const char nomem[] = ":31:Out of memory: %s\n";
static const char bindiff[] = ":147:Binary files %s and %s differ\n";
static const char badexec[] = ":148:Cannot execute %s: %s\n";
static const char badfork[] = ":43:fork() failed: %s\n";
...
char **argv;
{
		...
		while (*argp) 
			...
			case 'D':
				...
				if (*argp == '\0' && **argv != '-') {
					...
					pfmt(stderr, MM_ERROR, ":149:Use -D string\n");
					done();
				}
				continue;
			case 'b':
			...
			case 'S':
				if ((*argp == '\0') && (**argv != '-')) {
					...
					*--argp = 0;
					...
					pfmt(stderr, MM_ERROR, ":150:Use -S name\n");
					done();
				}
				continue;
			case 's':
			...
			default:
				...
				pfmt(stderr, MM_ACTION, ":151:Usage:\n\tdiff [ -bcefhilnrstw -C num -D string -S name ] file1 file2\n");
				done();
			}
	}
	if(argc!=2) {
		pfmt(stderr, MM_ERROR, ":152:Two filename arguments required\n");
		done();
	}
	...
	if (hflag) {
		if (opt) {
			pfmt(stderr, MM_ERROR,
			    ":153:-h doesn't support -e, -f, -n, -c, or -I\n");
			done();
		}
		else {
			diffargv[0] = "diffh";
			execv(diffh, diffargv);
			pfmt(stderr, MM_ERROR, badexec, diffh, strerror(errno));
			status = 2;
			done();
		}
	...
	if (!strcmp(file1, "-"))
		...
		pfmt(stderr, MM_ERROR, badstat, file1, strerror(errno));
		done();
	}
	if (!strcmp(file2, "-")) {
		if (!strcmp(file1, "-")) {
			pfmt(stderr, MM_ERROR, ":154:Cannot specify - -\n");
			done();
		}
		stb2.st_mode = S_IFREG;
	}
	else if (stat(file2, &stb2) < 0) {
		pfmt(stderr, MM_ERROR, badstat, file2, strerror(errno));
		done();
	}
	...
	if ((input[0] = fopen(file1, "r")) == NULL) {
		pfmt(stderr, MM_ERROR, badopen, file1, strerror(errno));
		status = 2;
		done();
	}
	if ((input[1] = fopen(file2, "r")) == NULL) {
		pfmt(stderr, MM_ERROR, badopen, file2, strerror(errno));
		status = 2;
		done();
	}
	...
	for (;;) {
		...
		if (ferror(input[0]) || ferror(input[1])) {
			pfmt(stderr, MM_ERROR, badread, ferror(input[0]) ? file1 : file2,
				strerror(errno));
			fclose(input[0]);
			fclose(input[1]);
			status = 2;
...
notsame:
	...
	if (filebinary(input[0]) || filebinary(input[1])) {
		...
		pfmt(stdout, MM_NOSTD, bindiff, file1, file2);
		fclose(input[0]);
		fclose(input[1]);
		done();
	}
...
same:
	if (opt == D_CONTEXT && anychange == 0)
		pfmt(stdout, MM_NOSTD, ":155:No differences encountered\n");
	done();
	/*NOTREACHED*/
}
	...
	if(!bflag && !wflag) {
		if (iflag)
			for(shift=0;(t=getc(f))!='\n';shift+=7) {
				if (t==EOF) {
					if (shift) {
						pfmt(stderr, MM_WARNING, nonl, str);
						break;
					}
					else
						return(0);
	...
	char **argv;
{
	...
	if (opt == D_IFDEF) {
		pfmt(stderr, MM_ERROR, ":156:Cannot specify -D with directories\n");
		done();
	}
	if (opt == D_EDIT && (sflag || lflag))
		pfmt(stderr, MM_WARNING,
		    ":157:Shouldn't give -s or -l with -e\n");
	dirstatus = 0;
	title[0] = 0;
	strcpy(title, "diff ");
	...
	if (lflag) {
		scanpr(dir1, ONLY, onlyin, file1, efile1, 0, 0);
		scanpr(dir2, ONLY, onlyin, file2, efile2, 0, 0);
		scanpr(dir1, SAME, ":158:Common identical files in %.*s and %.*s",
		    file1, efile1, file2, efile2);
		scanpr(dir1, DIFFER, ":159:Binary files which differ in %.*s and %.*s",
		    file1, efile1, file2, efile2);
		scanpr(dir1, DIRECT, ":160:Common subdirectories of %.*s and %.*s",
		    file1, efile1, file2, efile2);
	}
	if (rflag) {
		if (header && lflag)
	...
	for (; dp->d_entry; dp++) {
		...
		if (titled == 0) {
			if (header == 0)
				...
				printf("\n");
			pfmt(stdout, MM_NOSTD, title,
			    efile1 - file1 - 1, file1,
			    efile2 - file2 - 1, file2);
			pfmt(stdout, MM_NOSTD, ":161::\n");
			titled = 1;
		}
		printf("\t%s\n", dp->d_entry);
	}
	...
	int which;
{
	...
	pfmt(stdout, MM_NOSTD, ":162:Only in %.*s: %s\n", efile - file - 1, file, dp->d_entry);
}
	...
	char *cp;
{
	...
	dirp = opendir(cp);
	if (dirp == NULL) {
		pfmt(stderr, MM_ERROR, badopen, cp, strerror(errno));
		done();
	}
	nitems = 0;
	...
	register struct dir *dp;
{
	...
	if (fmt1 != S_IFREG || fmt2 != S_IFREG) {
		if (fmt1 == fmt2) {
			...
			case S_IFDIR:
				...
				if (lflag || opt == D_EDIT)
					goto closem;
				pfmt(stdout, MM_NOSTD, ":163:Common subdirectories: %s and %s\n",
				    file1, file2);
				goto closem;
			...
			case S_IFBLK:
				if (stb1.st_rdev == stb2.st_rdev)
					goto same;
				pfmt(stdout, MM_NOSTD, ":164:Special files %s and %s differ\n",
				    file1, file2);
				break;
			...
			case S_IFIFO:
				if (stb1.st_ino == stb2.st_ino)
					goto same;
				pfmt(stdout, MM_NOSTD, ":165:Named pipes %s and %s differ\n",
				    file1, file2);
				break;
			}
		} else {
			if (lflag)
				...
				pfmt(stdout, MM_NOSTD,
					":166:File %s is %s while file %s is %s\n",
	...
	for (;;) {
		...
		if (i < 0 || j < 0) {
			pfmt(stderr, MM_ERROR, badread, i < 0 ? file1 : file2,
				strerror(errno));
			close(f1); close(f2);
			return(2);
		}
...
same:
	...
	if (lflag)
		...
		pfmt(stdout, MM_NOSTD, ":167:Files %s and %s are identical\n", file1, file2);
	...
	char *wantpr;
{
	...
	if (wantpr) {
		...
		pid = fork();
		if (pid == (pid_t)-1) {
			pfmt(stderr, MM_ERROR, badfork, strerror(errno));
			done();
		}
		if (pid == 0) {
			close(0);
			...
			execv(pr+5, prargs);
			execv(pr, prargs);
			pfmt(stderr, MM_ERROR, badexec, pr, strerror(errno));
			done();
		}
	}
	pid = fork();
	...
	if (pid == 0) {
		...
		execv(diff, diffargv);
		pfmt(stderr, MM_ERROR, badexec, diff, strerror(errno));
		done();
	}
	if (wantpr)
	{
	...
	mode_t fmt;
{
	...
	case S_IFDIR:
		return gettxt(":168", "a directory");

	case S_IFCHR:
		return gettxt(":169", "a character special file");

	case S_IFBLK:
		return gettxt(":170", "a block special file");

	case S_IFREG:
		return gettxt(":171", "a plain file");

	case S_IFIFO:
		return gettxt(":172", "a named pipe");

	default:
		return gettxt(":173", "a file of unknown type");
	}
}
	...
	if((st->st_mode&S_IFMT)==S_IFDIR) {
		...
		while(*a1++ = *a2++)
			if(*a2 && *a2!='/' && a2[-1]=='/')
				a1 = b1;
		if (stat(*pa1, st) < 0) {
			pfmt(stderr, MM_ERROR, badstat, *pa1, strerror(errno));
			done();
		}
	}
...
char *fn;
{
	...
	if ((ifd = (strcmp(fn, "-") ? open(fn, 0) : 0)) < 0) {
		pfmt(stderr, MM_ERROR, badopen, fn, strerror(errno));
		done();
	}
	signal(SIGHUP,done);
	signal(SIGINT,done);
	...
	if ((ofd=creat(temp, S_IRUSR|S_IWUSR)) < 0) {
		pfmt(stderr, MM_ERROR, ":174:Cannot create %s: %s\n", temp,
			strerror(errno));
		done();
	}
	while((i=read(ifd,buf,BUFSIZ))>0)
		if (write(ofd,buf,i) != i) {
			pfmt(stderr, MM_ERROR, ":175:Write error in %s: %s\n",
				temp, strerror(errno));
			done();
		}
	close(ifd); close(ofd);
...
noroom()
{
	pfmt(stderr, MM_ERROR, ":176:Files too big\n");
	pfmt(stderr, MM_ACTION, ":177:Try -h\n");
	done();
}

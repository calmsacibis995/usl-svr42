...
#ident	"@(#)bnu:permission.c	2.12.12.4"
#ident  "$Header: permission.c 1.2 91/06/26 $"
...
static char *_RPaths[MAXPATHS+1] = { PUBDIR, NULL };
static char *_WPaths[MAXPATHS+1] = { PUBDIR, NULL };
static char *_NoRPaths[MAXPATHS+1] = { NULL };
static char *_NoWPaths[MAXPATHS+1] = { NULL };
...
static int _Authenticate;	/* TRUE if authentication is required */
static int _Encryption;		/* X_DES by default or else X_ENIGMA */
extern int _AuthCfg;
extern int _CryptCfg;
static char _MyName[MAXBASENAME+1];	/* MYNAME from PERMISSIONS file */
static char _Keys[MAXBASENAME+1];	/* KEYS   from PERMISSIONS file */
extern char _KeysCfg[];
...
char *name, *rmtname;
{
	...
	DEBUG(7, "_NoSpool (%s), ",
	    noSpool() ? "TRUE" : "FALSE");
	DEBUG(7, "_Keys (%s), ", _Keys);
	DEBUG(7, "_Authenticate (%s), ",
	    AuthReq() ? "TRUE" : "FALSE");
	DEBUG(7, "_Encryption (%s), ",
	    CryptType()==X_DES ? "DES" : "ENIGMA");
	return(ret);
}
...
fillFlds()
{
	...
	if (_Flds[U_AUTH] == NULL)
		_Authenticate = _AuthCfg;
	else if (EQUALS(_Flds[U_AUTH], "yes") || EQUALS(_Flds[U_AUTH], "req"))
		_Authenticate = TRUE;
	else
		_Authenticate = FALSE;

	if (_Flds[U_CRYPT] == NULL)
		_Encryption = _CryptCfg;
	else if (EQUALS(_Flds[U_CRYPT], "enigma")) 
		_Encryption = X_ENIGMA;
	else
		_Encryption = X_DES;

	if (_Flds[U_KEYS] != NULL) {
		strncpy(_Keys, _Flds[U_KEYS], MAXBASENAME);
		_Keys[MAXBASENAME] = NULLCHAR;
	} else
		strcpy(_Keys, _KeysCfg);
...
char *fullname;
{
	...
	/* Supply the expanded name even on failure. uuxqt counts on this. */
	(void) strcpy(fullname, name);
	return(FALSE);
}
 ...
 * if opt != NULL and permissions are ok,
...
char *from, *to, *opt;
{
	...
	*lxp = NULLCHAR;
	...
	/* dir is now the full path name of the parent directory */

	if ( (chkpth(to, CK_WRITE) == 0) && (eaccess(to, W_OK) == 0) )
		return(0);
	...
	if (opt != NULL)
	    if (mkdirs(dir, PUBMASK) == FAIL)
		return(FAIL);

	/* the directory now exists--check for writability */
	if ( (eaccess(dir, W_OK) == 0)  &&
		((chkpth(dir, CK_WRITE) == 0) || EQUALS(RemSpool, dir)) )
	    return(0);
...
struct stat	*status;
{
	...
	p = strrchr(filename, '/');
	*p = '\0';
	dirname = (p == filename) ? "/" : filename;
	ret = stat(dirname, status); 
	*p = '/';
	...
	if((status->st_uid == UUCPUID) && (!(status->st_mode & S_IWOTH)))
		return(1);

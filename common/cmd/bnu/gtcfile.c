...
#ident	"@(#)bnu:gtcfile.c	2.5.6.3"
#ident "$Header: gtcfile.c 1.1 91/02/28 $"
 ...
 *	if jflag is non-zero, print the jobids of committed
 *	command file(s) on standard output.
 *
 *	returns
 *		nothing
 */
...
char	*wfile1, *wfile2, *sys;
{
	...
	ASSERT(access(cmitfile, F_OK) != 0, Fl_EXISTS, cmitfile, 0);

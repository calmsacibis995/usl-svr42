...
#ident	"@(#)bnu:uustat.c	2.16.11.3"
#ident "$Header: uustat.c 1.1 91/02/28 $"
#include "uucp.h"
#include <sys/stat.h>
...
char **envp;
{
	...
	if (Summary) {
		...
		(void) machine(vec[5]);	/* longer name? */
	    }
	    closedir(spooldir);
	}
...
complete()
{
	...
	/* This function will search through /var/uucp/.Admin/account
	 * file for all jobs submitted by User.  If User jobs are
	 * found the state of 'completed' will be printed on stdout.
	 * Module called by uustat main().
	 */
	...
	while (fgets(abuf, BUFSIZ, fp) != NULL)
	  {
		...
		*strptr = ' ';
	    ...
	    /* If it is *not* a transfer record, we don't want to
	     * scan with the sscanf below, because we'll cause the
	     * stack to break
	     */
	    for (strptr = abuf; *strptr == ' ' || *strptr == '\t'; strptr++) ;
	    if (strncmp(strptr, "xfer", 4) != 0)
		continue;

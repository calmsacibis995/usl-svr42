...
#ident	"@(#)bnu:grades.c	1.7.6.6"
#ident "$Header: grades.c 1.1 91/02/28 $"
 ...
 *
 *	returns 
 *		SUCCESS - if grade is valid for the user
 *
 *		5 - if user doesn't have permissions to use
 *				the grade.
 *		6 - if grade is invalid.
 *
 */
...
char *grade;
{
	...
	while (rdfulline(cfd, line, BUFSIZ) != 0) {
		...
		if (upermit(carray, na) != FAIL) {
			...
			(void) fprintf(stderr, "%s: %s <%s>.\n %s\n%s\n",
				Progname, grd_err[PERMP1], grade,
				grd_err[PERMP2], grd_err[PERMP3]);
			return(5);
	...
	(void) fprintf(stderr, "%s: %s <%s> %s\n%s\n", Progname, 
		grd_err[NEXTP1], grade, grd_err[NEXTP2], grd_err[NEXTP3]);
	return(6);
}

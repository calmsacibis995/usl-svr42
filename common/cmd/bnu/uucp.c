...
#ident	"@(#)bnu:uucp.c	2.16.10.9"
#ident "$Header: uucp.c 1.1 91/02/28 $"
...
char	**envp;
{
	...
	static char emsg1[] = "uucp: No administrator defined service grades available on this machine.";
	...
	*Sgrade = NULLCHAR;
	...
	DEBUG(4, "\n\n** %s **\n", "START");
	if (gwd(Wrkdir) == FAIL) {
		fprintf(stderr,"uucp: Can not determine current directory\n");
		exit(54);
	}
	if (fopt) {
		if (*fopt != '/')
	...
	ret = CHDIR(WORKSPACE);
	if (ret != 0) {
		(void) fprintf(stderr, "uucp: <%s>: no work directory - get help\n",
		    WORKSPACE);
		exit(7);
	}
	...
	if (*sys2 != NULLCHAR) {
		if (versys(sys2) != 0) {
			(void) fprintf(stderr, "uucp: <%s>: bad system name\n", sys2);
			exit(8);
		}
	}
	else
	...
	for ( ; optind < argc - 1; optind++) {
	    ...
	    if (*fwd1 != NULLCHAR) {
		/* syntax error */
	        (void) fprintf(stderr, "uucp: <%s>: cannot have more than one ! in source file\n", argv[optind]);
	        cleanup(11);
	    }
...
register int	code;
{
	...
	if (code == 22)
		(void) fprintf(stderr, 
		"uucp: failed with <%d> file(s) sent and <%d> error(s)\n",
...
char *s1, *f1, *s2, *f2;
{
	...
	case 0:
		...
		if (statret == 0
		  ...
		  && stbuf.st_dev == stbuf1.st_dev) {
			(void) fprintf(stderr,
			    "uucp: <%s> <%s>: same file; cannot copy\n", file1, file2);
			return;
		}

		if (chkperm(file1, file2, strchr(Optns, 'd')) ) {
			(void) fprintf(stderr, "uucp: <%s>,<%s>: permission denied \n",file1,file2);
			cleanup(15);
		}
		if ((stbuf.st_mode & S_IFMT) == S_IFDIR) {
			(void) fprintf(stderr, "uucp: <%s>: source file cannot be a directory\n", file1);
			return;
		}
		/* see if I can read this file as real uid, gid */
		if ( access(file1, R_OK) ) {
			(void) fprintf(stderr,
			    "uucp: <%s>: cannot read file\n", file1);
			return;
		}
		...
		if (statret == 0 && (stbuf1.st_mode & W_OK ) == 0) {
			(void) fprintf(stderr,
			    "uucp: <%s>: cannot write file with mode <%lo>\n",
		...
		if (uucp_private(file2, &stdir)) {
			fprintf(stderr, "uucp: <%s>: permission denied\n",file2);
			return;
		}
		...
		if (uidxcp(file1, file2) == FAIL) {
			(void) fprintf(stderr,
			    "uucp: <%s>: cannot copy file, errno=<%d>\n", file2, errno);
			return;
		}
		/*
	...
	case 2:
		...
		if ((stbuf.st_mode & S_IFMT) == S_IFDIR) {
			(void) fprintf(stderr,
				"uucp: <%s>: source file cannot be a directory\n", file1);
			return;
		}
...
usage()
{
	...
	(void) fprintf(stderr,
"uucp: usage:  %s [-c|-C] [-d|-f] [-gGRADE] [-j] [-m] [-nUSER]\\\n\

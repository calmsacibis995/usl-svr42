...
#ident	"@(#)bnu:uucheck.c	2.9.9.2"
#ident  "$Header: uucheck.c 1.2 91/06/26 $"
...
int type;
{
	...
	if (type == U_MACHINE) {
	    ...
	    keys(cmd);
	    if (verbose) {
		printf("Authentication is %s using the '%s' scheme.\n",
			AuthReq() ? "REQUIRED" : "OPTIONAL", cmd);
		printf("Encription/Decription uses %s.\n",
			CryptType()==X_DES ? "DES" : "ENIGMA");
		printf("CAN execute the following commands if unauthenticated:\n");
	    }

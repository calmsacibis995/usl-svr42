...
#ident	"@(#)bnu:sysfiles.c	1.19.13.2"
#ident  "$Header: sysfiles.c 1.2 91/06/26 $"
...
unsigned connecttime = CONNECTTIME,
 ...
 *
 * NOTE: If a "service" is not found in the Sysfiles file, the very
 *	basic defaults of Systems/Devices/Dialers is used. It would
 *	make more sense to recurse with a service value of "uucico",
 *	but that might cause problems with cu so it is being deferred.
 */
static void
scansys(service)
char *service;
 ...
 *
 * NOTE: These values are NOT set in the per-machine variables,
 *	thus it is necessary to call mchFind() in order to set
 *	them to the values found here instead of the default
 *	values.
 */
static void
setconfig()
{
	...
	while (getline(f, buf) > 0) {
		...
		} else if (strncmp("AUTH=", tok, strlen("AUTH=")) == SAME) {
		    tok += strlen("AUTH=");
		    if ( EQUALS(tok, "yes") || EQUALS(tok, "req") )
			_AuthCfg = TRUE;
		    else
			_AuthCfg = FALSE;
		    DEBUG(7, "Authentication set to %s.\n",
			_AuthCfg ? "REQUIRED" : "OPTIONAL");
		} else if (strncmp("CRYPT=", tok, strlen("CRYPT=")) == SAME) {
		    tok += strlen("CRYPT=");
		    if ( EQUALS(tok, "enigma") )
			_CryptCfg = X_ENIGMA;
		    else
			_CryptCfg = X_DES;
		    DEBUG(7, "Encryption set to %s.\n",
			_CryptCfg==X_DES ? "DES" : "ENIGMA");
		} else if (strncmp("KEYS=", tok, strlen("KEYS=")) == SAME) {
		    tok += strlen("KEYS=");
		    if ( *tok != '\0' ) {
			if ( _KeysCfg[0] != '\0' )
				DEBUG(7, "Old key management scheme %s.", tok);
			DEBUG(7, "Default key management scheme now %s.", tok);
		        strcpy(_KeysCfg, tok);
		    }
	        } else {

...
#ident	"@(#)bnu:uucico.c	2.37.10.6"
#ident "$Header: uucico.c 1.1 91/02/28 $"
...
extern int sysaccess(), guinfo(), countProcs(),
	savline(), omsg(), restline(), imsg(), callok(), gnxseq(),
	cmtseq(), startup(), cntrl();
...
char **envp;
{
	...
	Euid = geteuid();
	Gid = getgid();
	Egid = getegid();	/* this should be UUCPGID */
	...
	*xflag = NULLCHAR;
	...
	call.attr = NULL;	/* let dial() pick settings */
	call.baud = -1;		/* unused */
	...
	call_ext.service = "uucico";	/* service name, "uucico" by default */
	call_ext.class = NULL;
	...
	if (Role == SLAVE) {
		...
		checkrmt();	/* Deal with possibly unknown remotes. */
		
		(void) sprintf(lockname, "%ld", (long) getpid());
		if (mlock(LOCKPRE, lockname) != SUCCESS) {
			omsg('R', "LCK", Ofn);
			cleanup(101);
		}
 ...
 */
	...
	if (Role == MASTER) {
		...
		if (Ofn < 0) {
			...
			if (ttyn != NULL) {
				...
				if ( EQUALSN(ttyn,"/dev/",5) )
				    ...
				    strcpy(Dc, ttyn);
				fchmod(Ifn, M_DEVICEMODE);
			} else
				strcpy(Dc, "notty");
		}
	
		/* Setup determines the type of I/O to use */
		...
		/* fixline() was moved to the connection server */

		if (call_ext.protocol)
			DEBUG(5, "call_ext.protocol='%s'\n", call_ext.protocol);
 ...
 * Function:	checkrmt
 *
 * If NOSTRANGERS is executable, and if the remote system is not
 * in our Systems file, reject the call by executing NOSTRANGERS.
 */
...
checkrmt ()
{
	...
	if ( (access(NOSTRANGERS, X_OK) == 0) && versys(Rmtname)) {

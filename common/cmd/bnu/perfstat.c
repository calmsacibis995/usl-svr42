...
#ident	"@(#)bnu:perfstat.c	1.3.6.3"
#ident "$Header: perfstat.c 1.1 91/02/28 $"
/*
...
static char	Msg_badopen[] = "Open failed, errno=%d\n";
static char	Msg_nolog[] = "No log file. Logging disabled.\n";
static char	Msg_opening[] =	"attempting to open %s log.\n";
...
static void	grabTimes();
static void	pfloat();
static void	reportConn();
static void	reportFile();
static void	reportTimes();
static void	subTimes();
...
char	*logname;
{
	...
	if (fd < 0) {	/* Error on open. */
		/* If the file is not there it will usually mean
		 * that the SA doesn't want to collect statisitcs. */
		if (errno == ENOENT)
			DEBUG(DB_DETAIL, Msg_nolog, "");
		else
			DEBUG(DB_IMPORTANT, Msg_badopen, errno);
		return FAIL;
	} else {
		*logfile = fd;
		return SUCCESS;

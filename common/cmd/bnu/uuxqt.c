...
#ident        "@(#)bnu:uuxqt.c	2.12.17.2"
#ident  "$Header: uuxqt.c 1.2 91/06/26 $"
...
static void retosndr(), uucpst(), rmfiles();
static int chkFile();
...
#define FAIL_AUTH(a)	{ \
		logent(user, a); \
		if ( send_nonzero ) \
			retosndr(user, Rmtname, return_stdin ? fin : "", incmd, \
				"Authentication failed.", ""); \
		if ( store_status ) \
			uucpst(Rmtname, _Sfile, "", incmd, a); \
		rmfiles(); \
		}
...
char *envp[];
{
	...
	/* get the real credentials first */
	Uid = getuid();
	Euid = geteuid();
	Gid = getgid();
	Egid = getegid();
	/* set effective uid to UUCP to behave like before */
	(void)seteuid(UUCPUID);
	/* get rid of privileges if applicable */
...
chkpart(char *ptr)
{
	...
	while ((ptr = getprm(ptr, whitesp, prm)) != NULL) {
	    ...
	    case '\\':	/* not sure if this is how to really handle '\\' */
		ret |= BAD_COMMAND;
...
char *dirname;
{
    ...
    int		auth_flag = 0;	/* status of authentication */
    mode_t	mask;
    ...
    char	afile[MAXFULLNAME]; /* authentication file */
	...
	*incmd = *retaddr = *retuser = NULLCHAR;
	...
	while (fgets(buf, BUFSIZ, xfp) != NULL) {
		...
		case X_AUTH:	/* authentication file name */
			    auth_flag = -1;
			    (void) sscanf(&buf[2], "%s %s %s",
					afile, Rlogname, Rsysname);
			    break;
	...
	if ( auth_flag == -1 ) {	/* we got an authentication file */
		auth_flag = check_auth(afile, _Xfile, msgbuf);
		if (auth_flag == -1) {	/* we failed authentication */
			FAIL_AUTH(msgbuf);
			continue;
		}
	} else {
		if ( AuthReq() ) {
			FAIL_AUTH("Authentication required.");
			continue;
		}
		DEBUG(6, "Proceeding without authentication.%s\n", "");
	}
 ...
 */
	...
	/* If user was not authenticated, check return from chkpart() */
	if ( (auth_flag == 0)  && (badfiles || cmdok) ) {
	...
	/* we must make sure XQTDIR exists before trying to copy files */
	
	(void) mkdirs(XQTDIR, DIRMASK);
	mv_Xfiles();
	...
	/* make sure .Workspace exists for placement of stdio files */

	(void) mkdirs(WORKSPACE, DIRMASK);

	ret = shio(_Cmd, fin, dfile, errDfile, auth_flag==1 ? Llogname : NULL);
  	if (ret == 0) 
...
/*
 * check_auth:
 *		validates an authentication file
 */
static int
check_auth(afile, xfile, msg)
char *afile, *xfile, *msg;
{
	char *temp_key;
	int fd;
	struct stat ia_stat;
	...
	if( (fd = open(afile, O_RDONLY)) < 0 ) {
		sprintf(msg, "Cannot open authentication file %s\n", afile);
		return(FAIL);
	}

	if (fstat(fd, &ia_stat) != 0) {
		sprintf(msg, "Cannot stat authentication file %s\n", afile);
		return(FAIL);
	}

	if ((iabufp = malloc(ia_stat.st_size)) == NULL) {
		sprintf(msg, "Cannot malloc() for authentication file %s\n", afile);
		return(FAIL);
	}

	if (read(fd, iabufp, ia_stat.st_size) < 0) {
		sprintf(msg, "Cannot read authentication file %s\n", afile);
		return(FAIL);
	}

	(void)close(fd);

	/* determine key management scheme to use */

	keys(Keys);

	/*
	 * This SHOULD be a scheme-dependent translation. If
	 * a scheme which uses a name format other than
	 * "logname@sysname" is used for authentication, this
	 * will break it. There is no such facility at this
	 * time.
	 */

	sprintf(Rprincipal, "%s@%s", Rlogname, Rsysname);

	(void)seteuid(Euid);	/* this needs to be scheme-privileged */
	temp_key = getkey(Keys, "uucp", Rprincipal);
	(void)seteuid(UUCPUID);	/* back to normal */
	if ( !temp_key ) {
		sprintf(msg, "No key available.");
		return(FAIL);
	} else {

		memset(u_key, 0, KEYLEN);
		(void) strncpy(u_key, temp_key, KEYLEN);
		cryptbuf(iabufp,  ia_stat.st_size, u_key,
			NULL, CryptType()|X_DECRYPT|X_CBC);

		if ( ((xname = strtok(iabufp, " \t\n")) == NULL)
			|| !EQUALS(xname, _Xfile)) {
			sprintf(msg, "Decryption failed.");
			return(FAIL);
		}

		if ( ((xname = strtok(NULL, " \t\n")) == NULL)
			|| !EQUALS(xname, Rlogname)) {
			sprintf(msg, "Wrong username authenticated.");
			return(FAIL);
		}

		if ( ((xname = strtok(NULL, " \t\n")) == NULL)
			|| !EQUALS(xname, Rsysname)) {
			sprintf(msg, "Wrong system name authenticated.");
			return(FAIL);
		} 

		name_ret = namemap(Keys, Rprincipal, Llogname);
		if (name_ret != 0) {
			sprintf(msg, "No local user mapping for %s.\n", Rprincipal);
			return(FAIL);
		}
		DEBUG(5, "local user name '%s'\n", Llogname);

		if ( (xname = strtok(NULL, " \t\n")) == NULL) {
			sprintf(msg, "No checksum type.");
			return(FAIL);
		}
		...
		switch (atoi(xname)) {
			...
			default:
				sprintf(msg, "Unknown checksum type '%s'.\n",
					xname);
				return(FAIL);
				/* NOTREACHED */
				break;
		...
		if ( (xname = strtok(NULL, " \t\n")) == NULL) {
			sprintf(msg, "No checksum.");
			return(FAIL);
		}

		sum = strtoul(xname, (char **) NULL, 10);

		if (sum != tsum) {
			sprintf(msg,"'%s' checksum mismatch (%u vs %u)\n",
				xfile, sum, tsum);
			return(FAIL);
		}

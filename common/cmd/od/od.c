...
#ident	"@(#)od:od.c	1.11.4.1"
#ident "$Header: od.c 1.2 91/08/09 $"
/*
...
usage()
{
	pfmt(stderr, MM_ACTION,
		":69:Usage: od [-bcdDfFoOsSvxX] [file] [[+]offset][.|b|x]\n");
	exit(1);
}
...
struct dfmt *c;
{
	...
	case 'b':
		pre(c->df_field, c->df_size);
		putn((unsigned long)(*n)&0377, c->df_radix, c->df_field);
		ret = c->df_size;
		break;
	case 'D':
	case 'O':
...
int c;
{
	...
	if(c >= 0200 && MB_CUR_MAX > 1)
		putn((unsigned long)c,8,3); /* Multi Envir. with Multi byte char */	
	else{
		if(isprint(c)) {
			printf("  ");
			putchar(c);
...
register char *s;
{
	...
	/*
	 * If offset is appended by '.', then offset is interpreted as decimal.
	 * If offset is appended by 'x', then offset is interpreted as hexadecimal.
	 */
	p = s;
	while(*p) {
		if (*p == '.')

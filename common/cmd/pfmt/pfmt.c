...
#ident	"@(#)pfmt:pfmt.c	1.2.3.3"
#ident "$Header: pfmt.c 1.3 91/07/29 $"
/*
 * pfmt and lfmt
 *
 * Usage:
...
char *argv[];
{
	...
	if ((ret = FMT(stderr, severity | MM_NOGET, format, argv[optind + 1],
			...
			argv[optind + 17], argv[optind + 18], argv[optind + 19])) < 0){
		exit(-ret);
	}
	exit(0);
}

...
#ident	"@(#)wchrtbl:wchrtbl.c	1.1.2.3"				
#ident  "$Header: wchrtbl.c 1.2 91/06/25 $"
/*								*/
...
struct  classname	{
	...
	"ul",		UL,		NULL,
	"LC_CTYPE",	WLC_CTYPE,	NULL,
	"cswidth",	CSWIDTH,	NULL,
	"LC_NUMERIC",	WLC_NUMERIC,	NULL,
...
/*				mask = 0x8000;  */      /* 0x40000000 */
		...
		code_addr = (long *)(type_addr + cnt_type[s]);
		if (cnt_code[s] != 0){
			wctbl[s].code = code_addr;
...
create1w()
{
	...
	for (s = 0; s < 3; s++){
		...
		if (wctbl[s].code == 0)
			...
			(void)printf("(wchar_t *)0x%s",hextostr(wctbl[s].code));
		if (s < 2)
			(void)printf("},\n");
		else
			(void)printf("}\n");

...
#ident	"@(#)csh:common/cmd/csh/sh.c	1.5.7.5"
#ident  "$Header: sh.c 1.2 91/06/26 $"
...
notty:
  printf("Warning: no access to tty; thus no job control in this shell...\n");
	...
	if (!fast && reenter == 0) {
		...
		dosource(loadhist);
		if (loginsh) {
			srccat(value(S_home/*"home"*/), S_SLADOTlogin/*"/.login"*/);
		}
	}
	...
	bool hflg;
{
	...
	volatile int oSHIN = -1, oldintty = intty;
...
#endif
	child++;
	/* when exiting from csh,  restore tty. */
	untty();

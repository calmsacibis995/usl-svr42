...
#ident	"@(#)logins:logins.c	1.15.15.4"
#ident  "$Header: logins.c 1.2 91/06/27 $"
 ...
 *	pfmt.h		Definitions for using the standard message generator
 */
 ...
 *	pfmt		Interface to the standard message generation facility
...
#define	OPTSTR			"adg:l:mopstuxbhv"
#define	ROOT_ID			0
...
/* This structure describes security information: levels and audit mask */
struct	secinfo {
	long		lvlcnt;
	level_t		*lvlp;
	level_t		def_lvl;
...
struct	display {
	...
	struct secinfo *sec_info; 	/* Security information structure */
	...
	struct passwd  *pwent;
{
	...
	int		found = FALSE;	/* FLAG, insertion point found */
	...
	if (compare != 0) {
	    ...
	    new->sec_info = (struct secinfo *) NULL;
 ...
 * void applysecinfo()
 * 
 *	This functions applies security information to an item 
 *	to be displayed. It allocates space for a structure decribing
 *	the security info, then fills in that structure from the
 *	information in the /etc/security/ia/master file.
 *
 *  Arguments: bflg, hflg, vflg
 *		args from command line
 *
 *  Returns:  Void
 */
/*
 * Procedure:     applysecinfo
 *
 * Restrictions:
 *		ia_openinfo:	none
*/
...
int	vflg;
{
	...
	*/
	...
	struct	secinfo	*secp;
	struct	display	*dp;
	uinfo_t	uinfo;		/* struct ptr for reading master file */
	int	i;
	level_t	*levelp;
	adtemask_t mask;
	...
	for (dp = displayhead->nextuid ; dp ; dp = dp->nextuid) {
	    ...
	    /* Allocate structure space for the security description */
	    secp = (struct secinfo *) allocblk(sizeof(struct secinfo));
	    dp->sec_info = secp;
		
	    if (ia_openinfo(dp->loginID, &uinfo) || (uinfo == NULL)) {
		secp->lvlcnt = 0;
		secp->lvlp = NULL;
		...
		if(hflg) {
			if (ia_get_lvl(uinfo, &levelp, &secp->lvlcnt)) {
				secp->lvlcnt = 0;
				secp->lvlp = NULL;
				vflg = 0;
			}
			else {
				secp->lvlp = (level_t *) allocblk(((unsigned int)secp->lvlcnt
						* sizeof(level_t))); 
				(void) memcpy(secp->lvlp, levelp, (secp->lvlcnt * sizeof(level_t)));
			}
...
*/
/*
 *  void writeunformatted(current, xtndflag, expflag, bflag, secflag)
 ...
 *	bflag		TRUE if AUDIT information is to
 *			be written, FALSE otherwise
 *	seflag		TRUE if MAC security level information is to
 *			be written, FALSE otherwise
 *
 *  Returns:  void
 */
...
static void
writeunformatted(current, xtndflag, expflag, bflag, secflag)
	...
	int		bflag;		/* AUDIT info flag */
	int		secflag;	/* MAC level info flag */
{
	/* Automatic data */
	struct secgrp  *psecgrp;	/* Secondary group info */
	struct pwdinfo *pwdinfo;	/* Password aging info */
	struct secinfo *secp;		/* Security info(AUDIT and MAC) */
	int	i;
	int	size;
	char	*bufp;
	int	bufsz = 64;
	...
	secp = current->sec_info;

	if (bflag) {
		for (i=1; i<=ADT_NUMOFEVTS; i++)
			if (EVENTCHK(i, secp->emask))
				(void) fprintf(stdout,":%s",adtevt[i].a_evtnamp);
	}

	if (secflag) {
	    bufp = allocstr((unsigned int) bufsz);
	    for(i=0; i < secp->lvlcnt; i++, secp->lvlp++) {
	    	size = lvlout(secp->lvlp, bufp, 0, LVL_FULL);
		if ( size > bufsz) {
			bufp = allocstr((unsigned int) size);
			bufsz = size;
		}
		(void)lvlout(secp->lvlp, bufp, bufsz, LVL_FULL);
		(void) fprintf(stdout, ":%s", bufp);	
	    }
	}
...

/*
 *  void writeformatted(current, xtndflag, expflag, bflag, secflag)
 *	struct display *current
 *	int		xtndflag
 *	int		expflag
 *	int		bflag
 ...
 *	xtndflag	TRUE if extended information is to be written,
 *			FALSE otherwise
 *	expflag		TRUE if password expiration information is to be 
...
static void
writeformatted(current, xtndflag, expflag, bflag, secflag)
	struct display *current;	/* Struct with info to write */
	int		xtndflag;	/* Write extended output flag */
	int		expflag;	/* Write password expiration info flag */
	int		bflag;		/* AUDIT info flag */
	int		secflag;	/* MAC level info flag */
{
	...
	if (secflag) {
		...
		errno = 0;
		if (lvlout(secp->lvlp, bufp, bufsz, LVL_FULL))
			(void) printf("failed lvlout errno = %d\n", errno);
		else
		(void) fprintf(stdout, "%s\n", bufp);	
	    }
	}
...

/*
 *  void genuidreport(pipeflag, xtndflag, expflag, bflag, secflag)
 *	int	pipeflag
 *	int	xtndflag
 *	int	expflag
 *	int	bflag
 ...
 *	bflag		int
 *			TRUE if audit information is to be displayed
 *			FALSE otherwise
 *	secflag		int
 *			TRUE if MAC level information is to be displayed
 *			FALSE otherwise
 *
 *  Returns:  void
 */
...

/*
 *  void genlogreport(pipeflag, xtndflag, expflag, bflag, secflag)
 *	int	pipeflag
 *	int	xtndflag
 *	int	expflag
 *	int	bflag
...
static void
genlogreport(pipeflag, xtndflag, expflag, bflag, secflag)
	...
	int	secflag;	/* MAC security level info flag */
{
	...
	if (pipeflag)
	    for (p = displayhead->nextlogin ; p ; p = p->nextlogin)
	    	writeunformatted(p, xtndflag, expflag, bflag, secflag);
	else
	    for (p = displayhead->nextlogin ; p ; p = p->nextlogin)
	    	writeformatted(p, xtndflag, expflag, bflag, secflag);
}

/*
 * logins [-admopstuxbhv] [-l logins] [-g groups]
 *
 ...
 *			aging information
 *	-b		list user's audit events
 *	-h		list valid security levels for user
 *	-v		list default security level for user
 *
 * Exit Codes:
...
static	char
	...
	*USAGE_MSG	= ":0:usage: logins [-admopstux%s%s] [-g groups] [-l logins]\n",
	*NO_PERM	= ":0:Permission denied\n";
	...
	char   *argv[];	/* Pointers pointing to the arguments */
{
	...
	actl_t			actl;		/* audit struct for auditctl */
	level_t			level;		/* level for lvlin call */
	...
	long			search_for;	/* Passwd entry to search for */
	...
	int			b_seen;		/* Is -b requested? */
	int			h_seen;		/* Is -h requested? */
	int			v_seen;		/* Is -v requested? */
	...
	int			mac = FALSE;	/* MAC flag if true MAC installed */
	int			audit = FALSE;	/* AUDIT flag if true AUDIT installed */

	(void) setlocale(LC_ALL, "");
	(void) setcat("uxcore");
	(void) setlabel("UX:logins");
	...
	/* Initializations */
	a_seen = d_seen = g_seen = l_seen = m_seen = o_seen = FALSE;
	p_seen = s_seen = t_seen = u_seen = x_seen = b_seen = FALSE;
	h_seen = v_seen = errflg = FALSE;
	...
	*/
	...
	while (!errflg && ((c = getopt(argc, argv, OPTSTR)) != EOF)) {
	    ...
	    /*
	     * -b option:
	     * Display user's audit events
	     */
	    case 'b':
		if (audit) {
			if (b_seen) errflg = TRUE;
			else b_seen = TRUE;
			break;
		}
		/*
		 ...
		 * It's bad if it is included so print a diagnostic
		 * message indicating the problem.
		*/
		else if (auditctl(ASTATUS, &actl, sizeof(actl)) < 0) {
			if (!(errno == ENOSYS || errno == ENOPKG)) {
				(void) pfmt(stderr, MM_WARNING, ":0:``%s'' %s\n",
					AUDITMASK, strerror(sav_err));
			}
			else {
				no_service(mac, audit, ":0:invalid option -b\n");
			}
		}
		break;
	    /*
	     * -v option:
	     * Display user's default level
	     */
	    case 'v':
		if (mac) {
			if (v_seen) errflg = TRUE;
			else v_seen = TRUE;
			break;
		}
		else {
			no_service(mac, audit, ":0:invalid option -h, -v\n");
		}
		break;
	    /*
	     * -h option:
	     * Display user's valid levels
	     */
	    case 'h':
		if (mac) {
			if (h_seen) errflg = TRUE;
			else h_seen = TRUE;
			break;
		}
		else {
	...
	if (errflg || (optind != argc)) {
		...
		(audit ? audit_str : ""), (mac ? mac_str : ""));
	    exit(1);
	}
	/*
	 * before any real processing is done, check to see if
	 * the user has the necessary access to the system data
	 * files.  If not, print a diagnostic message and exit.
	*/
	if (access(SHADOW, EFF_ONLY_OK|R_OK) != 0) {
	    	wrtmsg(MM_ERROR, NO_PERM);
		exit(1);
	}
	/*
	...
	if (d_seen) {
	     ...
	     * slow...
	     */
	    if ((searchpwdfile = lookpwdfile = fopen(PF_PATH, "r")) == (FILE *) NULL) {
		wrtmsg(MM_ERROR, ":0:Unable to open system password file: %s\n", PF_PATH);
		exit(101);
	    }
		...
		(void) fseek(lookpwdfile, search_for, 0);
		}
	 ...
	 */
	if (b_seen || h_seen || v_seen) 
		applysecinfo(b_seen, h_seen, v_seen);
	...
	if (t_seen) 
		genlogreport(o_seen, x_seen, a_seen, b_seen, (h_seen||v_seen));
	else 
		genuidreport(o_seen, x_seen, a_seen, b_seen, (h_seen||v_seen));
	...
	char	*msg;
{
	...
	wrtmsg(MM_ACTION, USAGE_MSG, (audit ? audit_str : ""),
		(mac ? mac_str : ""));
	exit(3);
}

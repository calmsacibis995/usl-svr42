...
#ident	"@(#)sdiff:sdiff.c	1.4.2.4"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/sdiff/sdiff.c,v 1.1 91/02/28 19:48:21 ccs Exp $"
	...
	*/
	...
	for(cp=buffer; *cp != '\n'; cp++){
		if(*cp == '\t'){
			...
			notspace = 1;
			if (ISSET2(*cp)) {
				if(tlen + wp._scrw2 - 1 >= hlen) break;
				cp += wp._eucw2;
				tlen += wp._scrw2;
				nchars += wp._eucw2 + 1;
			} else if (ISSET3(*cp)) {
				if(tlen + wp._scrw3 - 1 >= hlen) break;
				cp += wp._eucw3;
				tlen += wp._scrw3;
				nchars += wp._eucw3 + 1;
			} else {
				if (tlen + wp._scrw1 - 1 >= hlen) break;
				cp += wp._eucw1 - 1;
				tlen += wp._scrw1;
				nchars += wp._eucw1;
			}

...
#ident	"@(#)bfs:bfs.c	1.22.1.5"
#ident  "$Header: bfs.c 1.2 91/06/26 $"
#include <stdio.h>
#include <setjmp.h>
#include <signal.h>
#include <stdlib.h>
...
char *argv[];
{
	...
	if(argc == 3) verbose = 0;
	setbuf(stdout, 0);
	if(bigopen(bigfile=argv[argc-1])) quitNG();
	tty = isatty(0);
...
struct Comd *p;
{
	...
	while(peekc() == ' ') getnextc();
	while((fle[i++] = getnextc()) != '\n');
	fle[--i] = '\0';
...
int prt;
{
	...
	r = 0;
	while(peekc() == ' ') getnextc();	/* Ignore leading spaces */
	switch(c = peekc()) {
		case '\n':
		case ',':
		case ';':	return(0);
...
getnextc()
{
	if(!peeked) {
		while((!(infildes == oldfd && flag)) && (!flag1) && (!readc(infildes,&charbuf))) {
			...
			if((infildes=pop(fstack)) == -1) quitNG();

...
#ident  "$Header: hashserv.c 1.2 91/06/27 $"
...
{	
	...
	else if (mode & EX_OK) 
	{
		 ...
		 * is a pre 4.1 machine, EX_OK will result EINVAL
		 * from 4.0, EACCES from 3.x, because EX_OK is 
		 * new in 4.1.
		 *
		...
		if (errno == EACCES)
		{
			...
			if (testflg==0 || dir==0)
			{
				...
				if ( strcmp(vfstatb.f_basetype, "unknown") != 0)

...
#ident	"@(#)sh:common/cmd/sh/sh.mk	1.18.18.7"
#ident "$Header: sh.mk 1.4 91/05/23 $"
...
sh.dy: $(OBJECTS)
	$(CC) -o $@ $(OBJECTS) $(LDFLAGS) $(LDLIBS) -dy -s
...
install: all
	...
	$(INS) -o -m 0555 -u $(OWN) -g $(GRP) -f $(SBIN) sh.dy
	-@rm -rf $(SBIN)/OLDsh
	$(INS) -o -m 0555 -u $(OWN) -g $(GRP) -f $(USRBIN) sh
	$(INS) -o -m 0555 -u $(OWN) -g $(GRP) -f $(USRBIN) sh.dy
	-@rm -rf $(USRBIN)/OLDsh
	rm -f $(USRBIN)/jsh
	ln -f $(USRBIN)/sh $(USRBIN)/jsh
	rm -f $(USRLIB)/rsh
	ln -f $(USRBIN)/sh $(USRLIB)/rsh
	if [ ! -d $(ETC)/default ] ; \
	then \
		mkdir $(ETC)/default ; \
	fi

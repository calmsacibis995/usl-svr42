...
#ident	"@(#)sh:common/cmd/sh/main.c	1.14.27.7"
...
alloc_fdtab()
{
	...
	if (!fdmap)
	{
		if (getrlimit(RLIMIT_NOFILE, &rlmtbuf) < 0)
			rlmtbuf.rlim_cur = 20;
		fdmap = (struct fdsave *)alloc(rlmtbuf.rlim_cur * sizeof(struct fdsave));
	}
}

...
#ident	"@(#)initpkg:common/cmd/initpkg/umountall.sh	1.4.18.4"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/initpkg/umountall.sh,v 1.1 91/02/28 17:38:04 ccs Exp $"
...
# make sure that /usr is umounted last

/sbin/mount -v  | /usr/bin/sort -r +2 >/tmp/umount$$
/usr/bin/egrep -v "[ 	]+/usr[ 	]+" /tmp/umount$$ >/tmp/umount_$$
/usr/bin/egrep "[ 	]+/usr[ 	]+" /tmp/umount$$ >>/tmp/umount_$$
cat /tmp/umount_$$ |

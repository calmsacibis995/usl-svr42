...
char *file;
{
	...
	if (!(Val_ptr = Sflags[VALFLAG - 'a']))
		Val_ptr = Blank;
	if(atoi(Sflags[ENCODEFLAG - 'a']) == 1) /* if file is encoded force -b option */
		fprintf(iop, "admin -iCOMB$$ -b -r%s -fv%s -m '-yThis was COMBined' s.COMB$$\n", 
					rarg,Val_ptr);
	else
	...
	for (i = 0; i < NFLAGS; i++)
		if (p = Sflags[i])
			if((char)(i + 'a') != 'e') /*  encode flag taken care of when creating
						   **  the s-file */
				fprintf(iop, " -f%c%s\\\n", i + 'a', p);
	fprintf(iop, "\n");
	fprintf(iop, "sed -n '/^%c%c$/,/^%c%c$/p' %s >comb$$\n",

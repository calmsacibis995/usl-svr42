...
char *file;
{
	...
	*/
	...
	if (!HADG || HADL) {
		...
		mode_t xMode;	 /* mode for x bits */

		/*
		** Find mode of sccs file (s.file)
		** The mode of the extracted file depends on
		** the mode of the sccs file.
		...
		**	  0400		0400		0600
		*/
		if (stat(file, &tstatbuf) < 0) {
			sprintf(Error,"could not stat `%s' (ge16)", file);
			fatal(Error);
		}
	 	FileMode = tstatbuf.st_mode & 0444;
		xMode    = FileMode >> 2;
		...
		*/
		...
		if (gpkt.p_gout == 0) {
			if (HADP)
				...
				if (HADK)  /* Turn w bit on */
					FileMode |= (mode_t)0200;

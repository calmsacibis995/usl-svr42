...
#ident	"@(#)oampkg:common/cmd/oampkg/pkgmk/splpkgmap.c	1.15.10.5"
#ident  "$Header: $"
...
#define	ERR_OVERFLOW	"pard %d: selected files do not fit on volume"
...
int	*pilimit;
{
	...
	btotal += nblk(eptnum * EFACTOR, bsize) + 2L;
	ftotal++;
...
struct data **sf;
{
	...
	if(volno) {
		(void) fprintf(stdout, "part %2d -- %ld blocks, %ld entries\n",
			volno, btotal, ftotal);
		if(btotal > bmax)
			bmax = btotal;
		if(ftotal > fmax)

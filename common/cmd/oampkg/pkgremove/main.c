...
#ident	"@(#)oampkg:common/cmd/oampkg/pkgremove/main.c	1.13.11.5"
#ident  "$Header: $"
...
char	*class;
{
	...
	while(--i >= 0) {
		...
		if(!ept->ftype)
			...
			/* do lvlfile just to see if this is h load system,
			 * which means 4.1 packaging used on 4.0 machine
			 * if ENOPKG is returned, it is h load system
			 */
			if(lvlfile(0, 0, 0) == -1) {
				if(errno == ENOPKG)
					hload++;
			...
			if(!hload) {
				/* remove any file privileges */
				if((strcmp(ept->ainfo.priv_fix, "NONE") && 
			    	strcmp(ept->ainfo.priv_fix, "NULL")) ||
			    	(strcmp(ept->ainfo.priv_inh, "NONE") &&
			     	strcmp(ept->ainfo.priv_inh, "NULL"))) {
					if(rmprivs(ept->path))
	...
	if(script[0]) {
		(void) fclose(fp);
		ckreturn(pkgexecl(tmpfile, NULL, NULL, SHELL, script, NULL),
			ERR_CASFAIL);
	}
}

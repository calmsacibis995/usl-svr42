...
#ident	"@(#)cs:cs/callers.c	1.6.2.6"
#ident  "$Header: callers.c 1.2 91/06/26 $"
...
#ifdef CS
	{"CS",		cscall},	/* Connection Server caller */
#endif /* CS */

#ifdef DATAKIT
	{"DK",		dkcall},	/* standard DATAKIT VCS caller */
#endif /* DATAKIT */
...
#ifdef TLI
	{"TLI",		tlicall},	/* Transport Layer Interface */
#ifdef TLIS
	{"TLIS",	tlicall},	/* Transport Layer Interface */
#endif /*  TLIS  */
#endif /* TLI */
...
register char *flds[], *dev[];
{
	...
	if (dcf == -1) {
		...
		if ( Modemctrl ) {
			...
			dcf = dev_open(dcname, (O_RDWR | O_NDELAY));
		} else
			dcf = dev_open(dcname, O_RDWR);
		(void) alarm(0);
		if (dcf < 0) {
	...
	fixline(dcf, atoi(fdig(sdev[D_CLASS])), !D_TRANSFER);
...
/*
 *	cscall(flds, dev)	make a Connection Server dial-out
				request
 */
...
char *flds[], *dev[];
{
	...
	EXTERN  int	netfd;
	extern  int	returnfd;
	char *mach_p = NULL, *svc_p = NULL;

	DEBUG(4, "cscall: calling tli_connect \n", "");

	/* we make a copy so we don't change the buffer for the next call */

	mach_p = strdup(flds[F_PHONE]);

	if ((svc_p = strchr(mach_p, ',')) != NULL)
		*svc_p++ = '\0';

	if (mach_p && *mach_p && strcmp(mach_p, "-"))
		Nd_hostserv.h_host=mach_p;
	else
		Nd_hostserv.h_host=Call.telno;

	if (svc_p && *svc_p && strcmp(svc_p, "-"))
		Nd_hostserv.h_serv=svc_p;
	else
		Nd_hostserv.h_serv=((CALL_EXT	*)Call.device)->service;
	...
	if (returnfd < 0) {
		DEBUG(4, "cscall: tli_connect did not return authenticated fd\n", "");
		Uerror = SS_DIAL_FAILED;
		return(FAIL);
	}
	else {
		DEBUG(4, "cscall: tli_connect returned authenticated fd\n", "");
		return(returnfd);
	}
}
...
char *flds[], *dev[];
{
	...
	(void) sprintf(dcname, "/dev/%s", dev[D_LINE]);
	DEBUG(4, "dc - %s, ", dcname);
	if ((dcr = dev_open(dcname, (O_WRONLY | O_NDELAY))) < 0){
		Uerror = SS_DIAL_FAILED;
...
char *dcname, *dnname, *phone;
{
	...
	if ((dnf = dev_open(dcname, O_WRONLY)) < 0){
	...
	if ((dcf = dev_open(dcname,(O_RDWR | O_NDELAY))) < 0){
		DEBUG(5, "can't open %s\n", dcname);
		Uerror = SS_CANT_ACCESS_DEVICE;
		return(FAIL);
	}

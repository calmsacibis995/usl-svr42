...
#ident	"@(#)cs:cs/sysfiles.c	1.1.2.6"
...
/* import these */
extern char *strcpy(), *strtok(), *strchr();
...
int fd;
{
    char	strmod[FMNAMESZ+1], onstream[FMNAMESZ+1];
    int		optional, i;
	...
	DEBUG(5, (optional ? "pop_push: optionally POPing %s\n"
			   : "pop_push: POPing %s\n" ), strmod);
	if (!isastream(fd)){
	    DEBUG(5, "pop_push: cannot pop or push modules on a non-STREAMS device\n","");
	    return(FALSE);
	}
    ...
    if ( ! getpush(strmod, sizeof(strmod)) )
	return(TRUE);
    ...
    /* Check if the new module(s) is already pushed. */
    /* First get the number of modules currently on the stream. */

    if ((mod_list.sl_nmods = ioctl(fd, I_LIST,(struct str_list *)NULL)) < 0) {
	DEBUG(5, "pop_push: I_LIST on fd %d failed ", fd);
	DEBUG(5, "errno %d\n", errno);
	return(FALSE);
    }

    /* Allocate space for all the module names. */
    if ((mod_list.sl_modlist = (struct str_mlist *) calloc((unsigned)          mod_list.sl_nmods, (unsigned) sizeof(struct str_mlist))) == (struct str_mlist *)NULL) {
	DEBUG(5, "pop_push: calloc failed %d", errno);
	return(FALSE);
    }

    /* Get the NAMES of modules currently on the stream. */
    if (ioctl(fd, I_LIST, &mod_list) < 0) {
	DEBUG(5, "pop_push: I_LIST on fd %d failed ", fd);
	DEBUG(5, "errno %d\n", errno);
	return(FALSE);
    }
    ...
    do {
    /*	check for streams modules to push	*/
	for (i=0; i < mod_list.sl_nmods; i++) {
		if (strncmp(strmod, mod_list.sl_modlist[i].l_name,FMNAMESZ) == 0) {
			DEBUG(5,"%s already pushed\n",strmod);
			break;
		}
	}

	if (i == mod_list.sl_nmods) {

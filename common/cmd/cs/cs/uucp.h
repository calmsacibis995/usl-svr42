...
#ident	"@(#)cs:cs/uucp.h	1.2.2.6"
#ident  "$Header: uucp.h 1.2 91/06/26 $"
...
#ifndef DIAL
#define CDEBUG(l, f, s) if (Debug >= l) { sprintf(Scratch, f, s); uudebug(Scratch);}  
#else
#define CDEBUG(l, f, s)
#define SMALL
#endif
...
#ifndef SMALL
#define DEBUG(l, f, s) if (Debug >= l) { sprintf(Scratch, f, s); uudebug(Scratch);}  
#else
#define DEBUG(l, f, s) 
#endif /* SMALL */
 ...
 */
#define VERBOSE(f, s)  if (Verbose > 0) { sprintf(Scratch,f,s); uudebug(Scratch);} 
...
extern		noSpool(), AuthReq();			/* permission.c */

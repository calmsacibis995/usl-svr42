...
int	type;
{
	...
	if ((*s == 0) || (end == s) || (!isspace(*end) && *end)) {
		if (Debugging)
			debug("dobase: Illegal trailer on address string");
		buf[0] = '\0';
		return 0;
	}
	--end;
	...
	if (bp == 0) {
		if (Debugging)
			debug("stoa: dobase: number to long");
		return 0;
	}
...
unsigned	len, max;
{
	if (max && dest && max < len) {		/* No room */
		if (Debugging)
			debug("xfer: destination not long enough");
		return NULL;
	}
	if (!dest)
		if ((dest = malloc(len)) == NULL) {
			if (Debugging)
				debug("xfer: malloc failed");
			return NULL;
		}

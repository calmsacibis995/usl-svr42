	...
	char *dest;
{
	...
	*/
	...
	if ((fd = dev_open(dial_dev, O_RDWR)) < 0){
		logent("tp_open", "CONNECT FAILED");
		setalarm(0);
		if (dk_verbose){
			(void) sprintf(Scratch, "dkdial: Can't open %s", dial_dev);
			debug(Scratch);
		}
	...
	*/
	...
	if (ioctl(fd, DKIODIAL, &ioreq) < 0) {
		...
		if (dk_verbose){
			if (TimeErr){
				(void) sprintf(Scratch, Conn_Msg, Resp_Msg, ioreq.dialstring);
				debug(Scratch);
			}
		} 
		else {
			(void) sprintf(Scratch, Conn_Msg, ioreq.dialstring, dkerr(ioreq.iocb.req_error));
			debug(Scratch);
		}

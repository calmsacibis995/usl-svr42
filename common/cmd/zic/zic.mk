...
#ident	"@(#)zic:zic.mk	1.1.6.3"
#ident "$Header: zic.mk 1.6 91/05/23 $"
...
install: all $(FILES)
	if [ ! -d $(USRLIB)/locale/TZ ] ; then mkdir -p $(USRLIB)/locale/TZ ; fi
	$(CH)-chmod 755 $(USRLIB)/locale
	$(CH)-chgrp bin $(USRLIB)/locale
	$(CH)-chown bin $(USRLIB)/locale
	$(CH)-chmod 755 $(USRLIB)/locale/TZ
	$(CH)-chgrp bin $(USRLIB)/locale/TZ
	$(CH)-chown bin $(USRLIB)/locale/TZ
	for i in $(FILES); do \
	($(INS) -f $(USRLIB)/locale/TZ -m 644 $$i); done

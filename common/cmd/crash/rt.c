...
#ident	"@(#)crash:common/cmd/crash/rt.c	1.1.8.3"
#ident	"$Header: rt.c 1.1 91/07/23 $"
	...
	if(args[optind]) {
		...
		do {
			...
			if(arg2 != -1)
				...
				if(arg1 >= 0 && arg1 < rtmaxpri + 1) {
					slot = arg1;
					prrtdptbl(slot);
				}
				else
					fprintf(fp,"invalid real-time scheduler parameter table slot: %d\n",arg1);
			}
			slot = arg1 = arg2 = -1;
		}while(args[++optind]);
	}

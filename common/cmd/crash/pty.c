...
#ident	"@(#)crash:common/cmd/crash/pty.c	1.2.7.3"
...
long addr;
{
	char *heading = "\t  RQADDR MODNAME   MODID DACK_PTR  RDQ_PTR STATE\n";
	...
	if ( print) {
		...
		fprintf( fp, "%s", heading);
		fprintf( fp, "\t%x %-9s%6d%9x%9x", addr, mname, minfo.mi_idnum, ptem.dack_ptr, ptem.q_ptr);
		fprintf( fp, "%s\n",

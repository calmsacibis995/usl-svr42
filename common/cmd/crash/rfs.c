...
#ident	"@(#)crash:common/cmd/crash/rfs.c	1.17.1.3"
#ident	"$Header: rfs.c 1.1 91/07/23 $"
...
char *heading;
{
	...
	if(addr == -1)
		addr = Gdp_addr + slot*sizeof(gdpbuf);
	else
...
char *heading;
{
	...
	if(addr == -1)
		addr = Rcvd_addr + slot *sizeof(rcvdbuf);
	else
...
char *heading;
{
	...
	if(addr == -1)
		addr = Sndd_addr + slot*sizeof(snddbuf);
	else
...
char *heading;
{
	...
	if(addr == -1)
		addr = Rduser_addr + slot *sizeof(rduser);
	else

...
#ident	"@(#)creatiadb:creatiadb.c	1.1.14.4"
#ident  "$Header: creatiadb.c 1.3 91/06/26 $"
	...
	*/
	while (!end_of_file) {
		...
		if (((pwd = getpwent()) != NULL) &&
			((sp = getspnam(pwd->pw_name)) != NULL)) {

			if (audit) {
				if (getadtnam(pwd->pw_name, adtptr) != 0) {
			...
			(void) strcpy(mast.ia_pwdp,sp->sp_pwdp);
			...
			*/
			...
			if (errno == 0)
				...
				error++;
				(void) pfmt(stderr, MM_WARNING, MSG_3);
				log(NULL, MSG_3);
				errno = 0;
			}
			else {
 ...
 * Notes:	writes a message out to the log file ``/var/adm/creatlog''
		...
		*message;
{
	...
	*/
	...
	if ((logf = fopen(CREATLOG, "a")) != NULL) {
		(void) fprintf(logf,"%.2d/%.2d %.2d:%.2d %s ",tmp->tm_mon+1,
			tmp->tm_mday, tmp->tm_hour, tmp->tm_min, who);
		(void) pfmt(logf, MM_NOSTD, message);
		(void) fclose(logf);
	}
	else {
		pfmt(stderr, MM_WARNING, ":1000:could not log entry to ``%s''\n",
			CREATLOG);
	}
}

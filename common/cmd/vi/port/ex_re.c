...
#ident	"@(#)vi:port/ex_re.c	1.21.1.5"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/vi/port/ex_re.c,v 1.1 91/02/28 20:18:38 ccs Exp $"
...
out:
	*gp++ = '\0';
	...
	if(regerrno)
		...
		case 42:
cerror(":154", "\\( \\) Imbalance");
		case 43:
cerror(":155", "Awash in \\('s!|Too many \\('d subexpressions in a regular expression");
		case 50:
			goto complex;
		case 67:
cerror(":156", "Illegal byte sequence|Regular expression has illegal byte sequence");
		}
	re.Nbra = nbra;
	return(eof);
}

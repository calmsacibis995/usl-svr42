...
#ident	"@(#)vi:port/ex_temp.c	1.19.1.6"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/vi/port/ex_temp.c,v 1.1 91/02/28 20:18:46 ccs Exp $"
#include "ex.h"
#include "ex_temp.h"
#include "ex_vis.h"
#include "ex_tty.h"
...
dumbness:
	...
	ichanged = 0;
	ichang2 = 0;
	(void)strcat((char *)tfname, "/ExXXXXXXXXXX");
	...
	for (addr = addr1; addr <= addr2; addr++) {
		...
		if (sp->rg_flags) {
			if (addr == addr2)
				*wcursor = 0;
			if (addr == addr1)
				strcpy((char *)linebuf, (const char *)cursor);
		}
		YANKline();
	}
	rbflush();

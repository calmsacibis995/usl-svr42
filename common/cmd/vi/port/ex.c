...
#ident	"@(#)vi:port/ex.c	1.28.1.11"
#ident  "$Header: ex.c 1.3 91/08/12 $"
#include <pfmt.h>
#include "ex.h"
#include "ex_argv.h"
#include "ex_temp.h"
	...
	if (setexit() == 0 && !fast) {
		if ((globp = (unsigned char *)getenv("EXINIT")) && *globp)
			...
			if ((cp = (unsigned char *)getenv("HOME")) != 0 && *cp)
				source(strcat(strcpy((char *)genbuf, (const char *)cp), "/.exrc"), 1);
		}
		/*
		...
		/* Attention: Do not permit sourcing arbitrary .exrc's
		 *		Serious security holes might arise

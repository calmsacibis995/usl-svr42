...
#ident	"@(#)vi:port/ex_subr.c	1.24.1.11"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/vi/port/ex_subr.c,v 1.1 91/02/28 20:18:44 ccs Exp $"
#include <pfmt.h>
#include "ex.h"
#include "ex_re.h"
#include "ex_tty.h"
	...
	register unsigned char *str;
{
	...
	str = (unsigned char *)strcpy((char *)genbuf, (const char *)str);
	for (cp = str; *cp; cp++)
		switch (*cp) {
...
/*VARARGS2*/
void
merror(unsigned char *id, unsigned char *seekpt, ...)
{
	va_list i;
...
void
vmerror(unsigned char *id, unsigned char *seekpt, va_list ap)
{
	register unsigned char *cp = linebuf;
...
/*VARARGS2*/
void
smerror(unsigned char *id, unsigned char *seekpt, ...)
{
	va_list cp;
...
void
vsmerror(unsigned char *id, unsigned char *seekpt, va_list ap)
{
	errcnt++;
	merror1(id, seekpt);
	if (inopen && clr_eol)

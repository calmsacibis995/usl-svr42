...
#ident	"@(#)vi:port/ex_set.c	1.14.1.10"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/vi/port/ex_set.c,v 1.1 91/02/28 20:18:42 ccs Exp $"

#include "ex.h"
#include "ex_temp.h"
#include "ex_tty.h"

extern void error(unsigned char *id, unsigned char *str, ...);
extern void serror(unsigned char *id, unsigned char *str, ...);
/*
 * Set command.
 */
...
dontset:
		...
		for (op = options; op < &options[vi_NOPTS]; op++)
			if (eq(op->oname, cp) || op->oabbrev && eq(op->oabbrev, cp))
				break;
		if (op->oname == 0)
serror((unsigned char *)":157", (unsigned char *)"%s: No such option|%s: No such option - 'set all' gives all option values", cp);
		c = skipwh();
		if (peekchar() == '?') {
			ignchar();
printone:
		...
		if (no)
			serror((unsigned char *)":158", (unsigned char *)"Option %s is not a toggle", op->oname);
		if (c != 0 || setend())
			goto printone;
		if (getchar() != '=')
			serror((unsigned char *)":159",
				(unsigned char *)"Missing =|Missing = in assignment to option %s",
				op->oname);
		switch (op->otype) {

		case NUMERIC:
			if (!isdigit(peekchar()))
				error((unsigned char *)":160",
					(unsigned char *)"Digits required|Digits required after =");
			op->ovalue = getnum();
			if (value(vi_TABSTOP) <= 0)
				value(vi_TABSTOP) = TABS;
			if (value(vi_HARDTABS) <= 0)
				value(vi_HARDTABS) = op->odefault;
		...
		case OTERM:
			...
			while (!setend()) {
				if (cp >= &optname[ONMSZ])
error((unsigned char *)":161", (unsigned char *)"String too long|String too long in option assignment");
 ...
 */
				if (inopen)
error((unsigned char *)":162", (unsigned char *)"Cannot change type of terminal from within open/visual");
				unterm();
				setterm(optname);
			} else {
...
unterm()
{
	...
	for (i=0; i < MAXNOMACS; i++) {
		...
		if (arrows[i].cap && arrows[i].descr && strcmp((const char *)arrows[i].cap, (const char *)arrows[i].descr)) 
		...
		if (immacs[i].cap && immacs[i].descr && strcmp((const char *)immacs[i].cap, (const char *)immacs[i].descr)) 

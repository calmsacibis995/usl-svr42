...
#ident	"@(#)vi:port/ex_vops3.c	1.9.1.4"
#ident  "$Header: ex_vops3.c 1.2 91/06/27 $"
	...
	line *limit;
{
	...
	wasend = 0;
	lf = f;
	strcpy((char *)save, (const char *)linebuf);
	if (limit == 0)
		limit = dir < 0 ? one : dol;
	llimit = limit;
	wdot = dot;
	...
	unsigned char *cp;
{
	...
	wcursor = cp;
	strcpy((char *)sp, (const char *)linebuf);
	*wcursor = 0;
	strcpy((char *)cursor, (const char *)genbuf);
	...
	if (lmatchp(dot - vcline)) {
		...
		strcLIN(sp);
		strcpy((char *)scurs, (const char *)genbuf);
		if (!lmatchp((line *) 0))
			beep();
	}
	strcLIN(sp);

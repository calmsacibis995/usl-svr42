...
#ident	"@(#)vi:port/ex_cmdsub.c	1.17.1.8"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/vi/port/ex_cmdsub.c,v 1.1 91/02/28 20:18:27 ccs Exp $"
	...
	bool kill;
{
	...
	strcpy((char *)genbuf, (const char *)linebuf);
	...
	strcat((char *)linebuf, (const char *)gp);
	/*
	...
	strcpy((char *)gp, (const char *)linebuf);
	...
	int cnt;
{
	...
	for (addr = addr1; addr <= addr2; addr++) {
		...
		if (cp + strlen((const char *)(dp = vpastwh(linebuf))) >= &genbuf[LBSIZE - 2])
...
badtag:
	...
	while (fne) {
		...
		while (getfile() == 0) {
			...
			if(fgets((char *)linebuf, sizeof linebuf, iof)==NULL)
				goto goleft;
			linebuf[strlen((const char *)linebuf)-1] = 0;	/* was '\n' */
#endif
			while (*cp && *lp == *cp)
				cp++, lp++;
			/*
...
badtags:
			...
			*lp++ = 0;
			...
			strcpy((char *)cmdbuf, (const char *)cp);
			if (strcmp((char *)filebuf, (const char *)savedfile) || !edited) {
...
error(":50", "No write|No write since last change (:tag! overrides)");
				...
				strcat((char *)cmdbuf2, (const char *)filebuf);
				globp = cmdbuf2;
				d = peekc; ungetchar(0);
				commands(1, 1);
				peekc = d;
	...
	int ab;	/* true if this is abbr command */
{
	...
	*p = 0;
	...
	if (lhs[0] == '#') {
		...
		if (fnkey)
			strcpy((char *)lhs, (const char *)fnkey);
		dname = funkey;
	} else {
		dname = lhs;
	}
	...
	register struct maps *mp;
{
	...
	if (dest && mp==arrows) {
		 ...
		 */
		if (src[1] == 0 && src[0] == dest[strlen((const char *)dest)-1])
		...
		if (eq(src, dest+strlen((const char *)dest)-strlen((const char *)src)))
			error(EXmnotailid, EXmnotail);
	}
	/*
	...
	if (msnext - mapspace + strlen((const char *)dest) + strlen((const char *)src) + strlen((const char *)dname) + 3 > MAXCHARMACS)
		error(":64", "Too much macro text");
	CP(msnext, src);
	mp[slot].cap = msnext;
	msnext += strlen((const char *)src) + 1;	/* plus 1 for null on the end */
	CP(msnext, dest);
	mp[slot].mapto = msnext;
	msnext += strlen((const char *)dest) + 1;
	if (dname) {
		CP(msnext, dname);
		mp[slot].descr = msnext;
		msnext += strlen((const char *)dname) + 1;
	} else {

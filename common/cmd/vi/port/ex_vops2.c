...
#ident	"@(#)vi:port/ex_vops2.c	1.21.2.6"
#ident  "$Header: ex_vops2.c 1.2 91/06/27 $"
	...
	int cnt, indent;
{
	...
	for (;;) {
		...
		if (value(vi_AUTOINDENT)) {
			...
			gcursor = genindent(indent);
			*gcursor = 0;
			if (gcursor + strlen((const char *)linebuf) > &genbuf[LBSIZE - 2])
				gcursor = genbuf;
			CP(gcursor, linebuf);
		} else {
			CP(genbuf, gcursor + 1);
 ...
 */
unsigned char *
vgetline(cnt, gcursor, aescaped, commch, inpmode)
	int cnt;
	register unsigned char *gcursor;
	bool *aescaped, inpmode;
...
vbackup:
		...
		if (c != NL && !splitw) {
			...
			if (value(vi_WRAPMARGIN) &&
				...
				int bytelength;
				wdkind = 1;
				strncpy((char *)gcursor, (const char *)multic, length);
				gcursor += length;
				if (backsl) {
					if((length = mbftowc(multic, &wchar, getkey, &Peekkey)) <= 0) {
						beep();
				...
				*gcursor = 0;
				...
				if (outcol+(backsl?OCOLUMNS:0) - width >= OCOLUMNS - value(vi_WRAPMARGIN)) {
					 ...
					 */
                                        ...
                                        {
                                                /* restore processed erase or kill */
                                                unsigned char *ocp, c[2];
                                                int length;

                                                ocp = cp;
                                                length = strlen((char *)ocp);
                                                ocp += length;
                                                while ( length-- > 0) {
                                                        c[0] = *--ocp;
                                                        c[1] = 0;
                                                        macpush(c, 0);
                                                        if (*ocp == tty.c_cc[VERASE]
                                                        || *ocp == tty.c_cc[VKILL])
                                                                macpush("\\", 0);
							if (*ocp == ESCAPE) {
								c[0] = CTRL('v');
		 ...
		 * Process Word abbreviations only in Input Mode
		 */
		if (inpmode && anyabbrs && gcursor > ogcursor && !wordch(multic) && wordch(lastchr(ogcursor, gcursor))) {
				int wdtype, abno;
...
def:
			...
			(void)strncpy((char *)gcursor, (const char *)multic, length);
			gcursor += length;
			vcsync();
	...
	register int cnt;
{
	...
	len = strlen((const char *)linebuf);
	replen = strlen((const char *)genbuf);
...
unsigned char *mapto;
{
	...
	cnt = 0;
	final = strlen((const char *)mapto) - strlen((const char *)cap);

	for (i=0; i <= final; i++) 
	  if ((strncmp((char *)cap, (const char *)(mapto+i), strlen((const char *)cap)) == 0)       /* match */
	  && (i == 0     || !wordch(&mapto[i-1]))	      /* prefix ok */
	  && (i == final || !wordch(&mapto[i+strlen((const char *)cap)])))  /* suffix ok */
		cnt++;
	return (cnt);
}

...
#ident	"@(#)vi:port/ex_vput.c	1.16.1.8"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/vi/port/ex_vput.c,v 1.1 91/02/28 20:19:17 ccs Exp $"
...
unsigned char *id, *msg;
{
	...
	if (!vsmdclr)			/* if macpush() did not fail */
		if (*vmacp || vglobp)
			return;
	vsmdclr = 0;
	...
	if (*msg) {
		if (id && *id)
			strncpy((char *)locmsg, gettxt((char *)id, (char *)msg), LOCMSIZ);
		else
			strncpy((char *)locmsg, (const char *)msg, LOCMSIZ);
		if (value(vi_TERSE))

...
#ident	"@(#)vi:port/ex_tty.c	1.12.1.8"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/vi/port/ex_tty.c,v 1.1 91/02/28 20:18:50 ccs Exp $"
	...
	unsigned char *type;
{
	...
	if (cur_term && exit_ca_mode)
		putpad(exit_ca_mode);
	cur_term = 0;
	strcpy((char *)termtype,(const char *) type);
	...
	if(exit_attribute_mode)
		putpad(tparm(exit_attribute_mode));
	exit_bold = (exit_standout_mode ? exit_standout_mode : (exit_attribute_mode ? tparm(exit_attribute_mode) : 0));
	i = lines;
	if (lines <= 1)
		lines = 24;
	if (lines > TUBELINES)
...
unsigned char *key, *mapto, *desc;
{
	...
	if (any(*key, "\b\n "))
		return;
	strcpy((char *)surmapto, "\33");
	strcat((char *)surmapto, (const char *)mapto);
	p = freespace;
	strcpy((char *)p, (const char *)surmapto);
	freespace += strlen((const char *)surmapto) + 1;

...
#ident	"@(#)vi:port/ex_vmain.c	1.22.1.10"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/vi/port/ex_vmain.c,v 1.1 91/02/28 20:19:06 ccs Exp $"
...
gogo:
		...
		case ':':
			 ...
			 */
			oglobp = globp; strcpy((char *)vutmp, (const char *)(genbuf+1)); globp = vutmp;
doinit:
			esave[0] = 0;
			fixech();
...
vsave()
{
	 ...
	 */
	getDOT();
	if (strcmp((char *)linebuf, (const char *)temp) == 0)
		return;
	strcLIN(temp);
	putmark(dot);
}

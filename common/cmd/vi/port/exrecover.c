...
#ident	"@(#)vi:port/exrecover.c	1.28.1.9"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/vi/port/exrecover.c,v 1.1 91/02/28 20:19:23 ccs Exp $"
...
static void error(unsigned char *id, unsigned char *str, ...);
	...
	unsigned char *argv[];
{
	...
	if (argc != 2)
		error((unsigned char *)":260", (unsigned char *)" Wrong number of arguments to exrecover", 0);
	...
	if ((int) sbrk((int) (H.Flines * sizeof (line))) == -1)
		error((unsigned char *)":265", (unsigned char *)" Not enough core for lines", 0);
#ifdef DEBUG
...
error(unsigned char *id, unsigned char *str, ...)
{
	...
	vpfmt(stderr, MM_ERROR|MM_NOGET, lookedup, inf);
	...
	unsigned char *dirname;
{
	...
	for (fp = &svbuf[0]; fp < &svbuf[ecount]; fp++) {
		...
		cp[10] = 0;
		pfmt(stderr, MM_INFO, ":266:On %s at ", cp);
	...
	struct svfile *p1, *p2;
{
	...
	if (t = strcmp((char *)p1->sf_name, (const char *)p2->sf_name))
		return(t);
	...
	unsigned char *dir;
{
	 ...
	 * Extreme lossage...
	 */
	error((unsigned char *)":268", (unsigned char *)" File not found", 0);
}
	...
	unsigned char *dirname;
{
	...
	while ((direntry = readdir(dir)) != NULL) 
	{
		 ...
		 * we are looking for.
		 */
		(void)strcat(strcat(strcpy((char *)nb, (const char *)dirname), "/"), (const char *)filname);
		if (yeah(nb)) {
			/*
	...
	line atl;
{
	...
	off = (atl << SHFT) & LBTMSK;
	if (bno >= NMBLKS)
		error((unsigned char *)":66", (unsigned char *)" Tmp file too large");
	nleft = BUFSIZ - off;
	if (bno == iblock) 
		return (ibuff + off);
	iblock = bno;
...
syserror()
{
	...
	if (e >= 0 && errno <= sys_nerr)
		error((unsigned char *)"", (unsigned char *)strerror(e));
	else
		error((unsigned char *)":172", (unsigned char *)"System error %d", e);
	exit(1);
}

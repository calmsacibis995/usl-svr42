...
#ident	"@(#)vi:port/ex_vget.c	1.20.1.13"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/vi/port/ex_vget.c,v 1.1 91/02/28 20:19:02 ccs Exp $"
	...
	unsigned char c;
{
	...
	(void)vgetline(0, genbuf + 1, &waste, c, 0);
	ixlatctl(0);
	if (Outchar == termchar)
		putchar('\n');
	vscrap();
	...
	register unsigned char *buf, *str;
{
	...
	if ((unsigned char)buf[128] == 0200)
		return;
	if (strlen((const char *)buf) + strlen((const char *)str) + 1 >= VBSIZE) {
		buf[128] = 0200;
		return;
	}
	...
	unsigned char commch;		/* indicate if in append/insert/replace mode */
{
	...
	for (d=0; d < MAXNOMACS && maps[d].mapto; d++) {
		...
		if (p = maps[d].cap) {
			 ...
			 */
			 if ((maps == immacs) 
			 && strcmp((char *)maps[d].descr, (const char *)maps[d].cap)) {
				switch (commch) {
				  case 'R':
					if (!strcmp((const char *)maps[d].descr, "home"))
						st = (unsigned char *)"R";
					else
							if (outcol == 0)
								st = (unsigned char *)"R";
				...
				if(strlen((const char *)vmacbuf)  + strlen((const char *)st) > BUFSIZ) 
					 ...
					 * currently in (commch).
					 */
					strcat((char *)vmacbuf, (const char *)st);
			}
			c = getkey();
#ifdef MDEBUG
			if (trace)
	...
	/*
	 *  The ESCAPE is not a char in a macro. It ends a command. So
	 *  set vsdmclr, this will result clear the showmode message.
	 */
	if (value(vi_SHOWMODE) && (c == ESCAPE) && (strlen((const char *)(&b[1])) == 1))
		vsmdclr = 1;
...
#endif
	if ((vmacp ? strlen((const char *)vmacp) : 0) + strlen((const char *)st) > BUFSIZ)
		error(EXmmactoolongid, EXmmactoolong);
	if (vmacp) {
		strcpy((char *)tmpbuf, (const char *)vmacp);
		if (!FIXUNDO)
			canundo = 0;	/* can't undo inside a macro anyway */
	}
	strcpy((char *)vmacbuf, (const char *)st);
	if (vmacp)
		strcat((char *)vmacbuf, (const char *)tmpbuf);
	vmacp = vmacbuf;

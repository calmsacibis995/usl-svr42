...
#ident	"@(#)vi:port/ex_vops.c	1.14.1.5"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/vi/port/ex_vops.c,v 1.1 91/02/28 20:19:10 ccs Exp $"
...
bool show;	/* if true update the screen */
{
	...
	case VCAPU:
		vundkind = VCHNG;
		strcpy((char *)temp, (const char *)vutmp);
		strcpy((char *)vutmp, (const char *)linebuf);
	...
	if (wdot) {
		...
		if (cursor && wcursor) {
			...
			*cursor = 0;
			...
			getline(*wdot);
			if (strlen((const char *)genbuf) + strlen((const char *)wcursor) > LBSIZE - 2) {
				getDOT();
				beep();
				return;
			}
			strcat((char *)genbuf, (const char *)wcursor);

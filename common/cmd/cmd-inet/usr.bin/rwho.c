...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.bin/rwho.c	1.2.11.1"
#ident  "$Header: rwho.c 1.3 91/09/19 $"
	...
	char **argv;
{
	...
	if (dirp == NULL) {
        ...
        if ((myutmp = (struct myutmp *)malloc(maxusers * sizeof(*myutmp))) == NULL) {
	...
	while (dp = readdir(dirp)) {
		...
		for (n = cc / sizeof (struct whoent); n > 0; n--) {
			...
			if (nusers >= maxusers) {
				maxusers *= 2;
                                if ((myutmp = (struct myutmp *) realloc(myutmp, maxusers * sizeof(*myutmp))) == NULL) {
                                        fprintf(stderr,"Can't get memory\n");
				        exit(1);
                                }
                        mp = &myutmp[nusers];

...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.bin/ftp/ftp.c	1.6.11.1"
#ident  "$Header: ftp.c 1.3 91/09/19 $"
...
# endif h_addr
	...
	if (getreply(0, 0) > 2) { 	/* read startup message from server */
		if (cin)
			(void) fclose(cin);
		if (cout)
			(void) fclose(cout);
	...
	char *host;
{
	...
	n = command(0, "USER %s", user);
	if (n == CONTINUE) {
		if (pass == NULL)
			pass = mygetpass("Password:");
		n = command(0, "PASS %s", pass);
	}
	if (n == CONTINUE) {
		aflag++;
		acct = mygetpass("Account:");
		n = command(0, "ACCT %s", acct);
	}
	if (n != COMPLETE) {
	...
	va_dcl
{
	...
	va_end(ap);
	cpend = 1;
	r = getreply(!strcmp(fmt, "QUIT"), ignorerr);
...
getreply(expecteof, ignorerr)
	int expecteof;
	int ignorerr;	/* don't print err msg if is 500 or 502 */
{
	register int c, n;
	register int dig;
	...
	for (;;) {
		...
		while ((c = getc(cin)) != '\n') {
			...
			if (c == EOF) {
				...
				(void) fflush(stdout);
				/* reset macros so get a new set with the */
				/* next connection */
				macnum = 0;
				code = 421;
				return(4);
			...
			if (c != '\r' && (verbose > 0 ||
			    (verbose > -1 && n == '5' && dig > 4))) {
				if (debug || !ignorerr || ( code != 500 && code != 502)) {
				    if (proxflag &&
		...
		if (can_print && (debug || !ignorerr || ( code != 500 && code != 502))
		   && (verbose > 0 || verbose > -1 && n == '5') ) {
			(void) putchar(c);
			(void) fflush (stdout);
		}
	...
	char *cmd, *local, *remote;
{
	...
	if (setjmp(sendabort))
		goto abort;
	if (remote) {
		if (command(0, "%s %s", cmd, remote) != PRELIM) {
			(void) signal(SIGINT, oldintr);
			if (oldintp)
				(void) signal(SIGPIPE, oldintp);
			return;
	...
	/*		}                          			*/	
		...
		if (ferror(fin))
			perror(local);
		/*
		 *	we must flush the outputbuffer (D.M.)
		 */
		fflush(dout);
	...
	(void) fclose(dout);
	/*
	 *	we expect message 226 (Transfer complete)
	 *	message 552 is an error. (D.M.)
	 */
	if (getreply(0, 0) != 2) {
		(void) signal(SIGINT, oldintr);
		return;
...
/* after urgent byte rather than before as now is protocol            */
	...
	if ((c = getreply(0, 0)) == ERROR && code == 552) { /* needed for nic style abort */
		if (data >= 0) {
			(void) close(data);
			data = -1;
		}
...
initconn()
{
	...
	/*
	 * Wait up to 8 minutes for slow SLIP connections
	 * With a send buffer of 24KB this is good for speeds down
	 * to 52 byte/s
	 */
	linger.l_onoff = 1;
	linger.l_linger = 480;
...
noport:
	...
	if (setsockopt(data, SOL_SOCKET, SO_LINGER, (char *)&linger, sizeof(linger)) < 0)
		perror("ftp: SO_LINGER (ignored)");
	...
	char *cmd, *local, *remote;
{
	...
	if (command(0, "PORT %s", pasv) != COMPLETE) {
		switch (oldtype) {
			case 0:
				break;
			case TYPE_A:
	...
	pswitch(1);
	secndflag++;
	if (command(0, "%s %s", cmd2, local) != PRELIM)
		goto abort;
	ptflag++;
	(void) getreply(0, 0);
	pswitch(0);

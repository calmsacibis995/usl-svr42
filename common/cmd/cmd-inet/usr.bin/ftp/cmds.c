...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.bin/ftp/cmds.c	1.7.12.1"
#ident  "$Header: cmds.c 1.3 91/09/19 $"
	...
	char *argv[];
{
	...
	if ((p->t_arg != NULL) && (*(p->t_arg) != '\0'))
		comret = command (0, "TYPE %s %s", p->t_mode, p->t_arg);
	else
		comret = command(0, "TYPE %s", p->t_mode);
	if (comret == COMPLETE) {
	...
	if (proxy) {
		...
		while ((cp = remglob(argv,0)) != NULL) {
			...
			if (mflag && confirm(argv[0], cp)) {
				tp = cp;
				if (mcase) {
					for (tp2 = tp; *tp2 && !islower(*tp2); tp2++)
						continue;
					if (!*tp2) {
						tp2 = tmpbuf;
						while ((*tp2 = tolower(*tp)) != NULL) {
						     tp++;
						     tp2++;
						}
						tp = tmpbuf;
	...
	char *argv[];
{
	...
	/* experimental command support a la rfc1123 */
	if (command(1, "CWD %s", argv[1]) == ERROR
	   && (code == 500 || code == 502) )
		(void) command(0, "XCWD %s", argv[1]);
}
...
usage:
	...
	if (argc < 3) 
		goto usage;
	if (command(0, "RNFR %s", argv[1]) == CONTINUE)
		(void) command(0, "RNTO %s", argv[2]);
}
...
pwd()
{
	...
	verbose = 1;
	/* experimental command support a la rfc1123 */
	if (command(1, "PWD") == ERROR && (code == 500 || code == 502) )
		(void) command(0, "XPWD");
	verbose = oldverbose;
}
	...
	char *argv[];
{
	...
	/* experimental command support a la rfc1123 */
	if (command(1, "MKD %s", argv[1]) == ERROR
	   && (code == 500 || code == 502) )
		(void) command(0, "XMKD %s", argv[1]);
}
	...
	char *argv[];
{
	...
	/* experimental command support a la rfc1123 */
	if (command(1, "RMD %s", argv[1]) == ERROR
	   && (code == 500 || code == 502) )
		(void) command(0, "XRMD %s", argv[1]);
}
	...
	char *argv[];
{
	...
	if (command(0, buf) == PRELIM) {
		while (getreply(0, 0) == PRELIM);
	}
}
	...
	char *argv[];
{
	...
	verbose = 1;
	(void) command(0, argc == 1 ? "HELP" : "HELP %s", argv[1]);
	verbose = oldverbose;
}
...
{
	/* experimental command support a la rfc1123 */
	if (command(1, "CDUP") == ERROR && (code == 500 || code == 502) )
		(void) command(0, "XCUP");
}
	...
	char *argv[];
{
	...
	for (c = 0; c < macnum; c++) {
		if (!strncmp(argv[1], macros[c].mac_name, 9)) {
			printf("Macro name %s already in use\n",argv[1]);
			code = -1;
			return;			
		}
	}

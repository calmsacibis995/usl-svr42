...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.bin/ruptime.c	1.3.10.2"
#ident  "$Header: ruptime.c 1.3 91/09/19 $"
	...
	char *argv[];
{
	...
	name = argv[0];
	while ( (i = getopt(argc, argv, "alutr")) != -1 ) {
		switch (i) {
		case 'a':
			aflg++;
			break;
	...
	if (dirp == NULL) {
        ...
        if ((hs = (struct hs *)malloc(maxhosts * sizeof(*hs))) == NULL) {
	...
	while (dp = readdir(dirp)) {
		...
		if (nhosts == maxhosts) {
                        maxhosts *= 2;
                        if ((hs = (struct hs *)realloc(hs, maxhosts*sizeof(*hs))) == NULL) {
                                fprintf(stderr,"Can't get memory\n");
                                exit(1);
                        }
                        hsp = &hs[nhosts];

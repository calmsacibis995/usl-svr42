...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.bin/telnet.c	1.7.13.3"
#ident  "$Header: telnet.c 1.3 91/09/19 $"
...
int     localflow = 1;          /* do xon/xoff flow control locally */
...
struct termios  ottyatt;
struct termios  nttyatt;
	...
	register int f;
{
	...
	case 2:		/* remote character processing, local echo */
                ...
                /***	mode 1: character-at-a-time mode		***/
                /***	mode 2: character-at-a-time mode (local echo) 	***/ 

		satt.c_lflag &= ~ICANON;
		if (f == 1) {
			satt.c_lflag &= ~ECHO;
			satt.c_oflag &= ~(OPOST | ONLCR);
			satt.c_iflag &= ~ICRNL;
                }
		else {
			satt.c_lflag |= ECHO;
			...
			satt.c_iflag |= ICRNL;
                ...
                /* clear tchars <=> VINTR,VQUIT,VSTART,VSTOP,VEOF ? */
		memset(satt.c_cc, _POSIX_VDISABLE, sizeof(satt.c_cc));
		...
		if (!donelclchars)
			localchars = 0;
		if (localchars) {
                        satt.c_cc[VINTR] = ottyatt.c_cc[VINTR];
                        satt.c_cc[VQUIT] = ottyatt.c_cc[VQUIT];
		}
 		if (localflow) {
 			satt.c_cc[VSTART] = ottyatt.c_cc[VSTART];
 			satt.c_cc[VSTOP]  = ottyatt.c_cc[VSTOP];
		}
                satt.c_cc[VMIN]  = '\01';
                satt.c_cc[VTIME] = '\0';
		flags |= O_NONBLOCK;
	...
	case 5:		/* local character processing, no echo */
                ...
                /***	mode 3: line-by-line mode (remote echo)		***/
                /***	mode 4: line-by-line mode			***/
                /***	mode 5: line-by-line mode (local echoing        ***/
                /***                               suppressed)          ***/
		...
		if (localchars) {
#ifdef VDISCARD
			satt.c_cc[VEOL]   = satt.c_cc[VDISCARD];
			satt.c_cc[VDISCARD] = _POSIX_VDISABLE;
#endif
		} else {
			satt.c_cc[VINTR] = _POSIX_VDISABLE;
			satt.c_cc[VQUIT] = _POSIX_VDISABLE;
		}
		satt.c_cc[VSUSP] = escape;
#ifdef VDSUSP
		satt.c_cc[VDSUSP] = _POSIX_VDISABLE;
#endif
                satt.c_cc[VMIN]  = '\01';
                satt.c_cc[VTIME] = '\0';
		flags |= O_NONBLOCK;
	...
	if (tcsetattr(0, TCSADRAIN, &satt) == -1)
                perror("tcsetattr");
	if (fcntl(fileno(stdin), F_SETFL, flags) == -1)
		perror ("fcntl O_NONBLOCK");
	if (fcntl(fileno(stdout), F_SETFL, flags) == -1)
		perror ("fcntl O_NONBLOCK");
	if (f >= 3)
		signal(SIGTSTP, (void (*)())doescape);
	else if (old >= 3) {
...
telnet()
{
	...
	for (;;) {
		...
		while (tcc > 0) {
			...
			if (localchars) {
				if (sc == nttyatt.c_cc[VINTR]) {
					intp();
					break;
				} else if (sc == nttyatt.c_cc[VQUIT]) {
					sendbrk();
					break;
#ifdef VDISCARD
				} else if (sc == nttyatt.c_cc[VDISCARD]) {
					NET2ADD(IAC, AO);
					if (autoflush) {
					    doflush();
					}
					...
					;
				} else if (sc == nttyatt.c_cc[VKILL]) {
					NET2ADD(IAC, EL);
					break;
				} else if (sc == nttyatt.c_cc[VERASE]) {
					NET2ADD(IAC, EC);
					break;
				}
			}
...
suboption()
{
	...
	if ((subbuffer[1]&0xff) != TELQUAL_SEND) {
		...
		strcpy(namebuf, name);
		upcase(namebuf);
		(void) sprintf(nfrontp, "%c%c%c%c%s%c%c", IAC, SB, TELOPT_TTYPE,
...
struct setlist Setlist[] = {
    ...
    { " ", 
      ...
      0 },
    { "erase",	
      "character to cause an Erase Character",
      (char *) &nttyatt.c_cc[VERASE] },
#ifdef VDISCARD
    { "flushoutput", 
      "character to cause an Abort Output", 
      (char *) &nttyatt.c_cc[VDISCARD] },
#endif /* VDISCARD */
    { "interrupt", 
      "character to cause an Interrupt Process", 
      (char *) &nttyatt.c_cc[VINTR] },
    { "kill",	
      "character to cause an Erase Line", 
      (char *) &nttyatt.c_cc[VKILL] },
    { "quit",	
      "character to cause a Break", 
      (char *) &nttyatt.c_cc[VQUIT] },
    { "eof",	
      "character to cause an EOF ", 
      (char *) &nttyatt.c_cc[VEOF] },
    { 0 }
};
...
char *argv[];
{
	...
	if (proc == 0) {
		...
		if (argc == 1)
		    ...
		    for (line[0] = '\0', cpp = &argv[1]; *cpp; ) {
			strcat(line, *cpp++);
			strcat(line, " ");
		    }
		    execl(shell, shell, "-c", line);
		}
		perror(shell);
		exit(-1);
	}
...
{
	setcommandmode();
	if (tcsetattr(0, TCSADRAIN, &ottyatt) == -1)
                perror("tcsetattr");
	kill(0, SIGTSTP);
	/* reget parameters in case they were changed */
	if (tcgetattr(0,&ottyatt) == -1)
		perror ("tcgetattr");
	return 1;
}
	...
	char *argv[];
{
	...
	if (setjmp(peerdied) == 0) {
		if (Aseco_send_ticket(host, net, 0) == -1) {
			extern char aseco_error_message[];
			fprintf(stderr, "%s\n", aseco_error_message);
		}
		else {
			extern int Aseco_rcmd_exit();
			atexit(Aseco_rcmd_exit);
			telnet();
		}
	}
#else
	...
	char *argv[];
{
	...
	opterr = 0;	/* don't mutter about errors in debug opts */
	while ( (c = getopt(argc, argv, "dn:")) != -1 ) {
		...
		case 'd':
			...
			break;
		case 'n':
			if ( (NetTrace = fopen(optarg, "w")) == (FILE *)NULL)
		    		NetTrace = stdout;
			break;
	    }
	}

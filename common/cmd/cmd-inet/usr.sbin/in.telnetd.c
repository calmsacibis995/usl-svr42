...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.sbin/in.telnetd.c	1.7.13.4"
#ident  "$Header: in.telnetd.c 1.3 91/09/20 $"
...
char	doopt[] = { (char) IAC, (char) DO, '%', 'c', 0 };
char	dont[] = { (char) IAC, (char) DONT, '%', 'c', 0 };
char	will[] = { (char) IAC, (char) WILL, '%', 'c', 0 };
char	wont[] = { (char) IAC, (char) WONT, '%', 'c', 0 };
...
#define READ_SECS 5
    ...
    signal(SIGALRM, (void (*)())interrupt_read);
    ...
    if ((ncc < 0) && (errno != EINTR)) {
	syslog(LOG_INFO, "ttloop:  read: %m\n");
	exit(1);
    } else if (ncc == 0) {
	syslog(LOG_INFO, "ttloop:  peer died: %m\n");
	exit(1);
    } else if ((ncc < 0) && (errno == EINTR)) {
	ncc = 0;
...
/* dummy routine so that actually interrupt the read */
void
interrupt_read()
{
...
getterminaltype()
{
    static char sbuf[] = { (char) IAC, (char) DO, TELOPT_TTYPE };
	...
	if (ttloop() == -1){
		remopts[TELOPT_TTYPE] = OPT_NO;
		return;		/* no reply do not bother with tty type */
	}
    }
    if (remopts[TELOPT_TTYPE] == OPT_YES) {
	static char sbbuf[] = { (char) IAC, (char) SB, TELOPT_TTYPE, TELQUAL_SEND, (char) IAC, (char) SE };

	bcopy(sbbuf, nfrontp, sizeof sbbuf);
	nfrontp += sizeof sbbuf;
	while (sequenceIs(ttypesubopt, getterminal)) {
	    if (ttloop() == -1)
		break;		/* no reply to subopt ??? */
	}
    }
}
...
#define	ARG_COUNT	256
		...
		strncat(buf_invoke,
			(char *) (terminaltype ? terminaltype : "-"),
...
int	s;		/* socket number */
{
	...
	FD_ZERO(&excepts);
	FD_SET(s, &excepts);
	value = select(s+1, (fd_set *)NULL, (fd_set *)NULL, &excepts, &timeout);

...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.sbin/htable/htable.c	1.1.8.1"
#ident  "$Header: htable.c 1.2 91/06/26 $"
	...
	char *argv[];
{
	...
	while ( (c = getopt(argc, argv, "c:l:")) != -1 ) {
		switch (c) {
		case 'c':
			nconnected = addlocal(optarg, connected_nets);
			break;
		case 'l':
			nlocal = addlocal(optarg, local_nets);
			break;
		default:
			usage();
			/*NOTREACHED*/

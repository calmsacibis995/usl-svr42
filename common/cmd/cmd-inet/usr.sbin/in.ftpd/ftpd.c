...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.sbin/in.ftpd/ftpd.c	1.12.13.2"
#ident  "$Header: ftpd.c 1.3 91/09/19 $"
	...
	char *argv[];
{
	...
	/* do not use nested asm routines - some C compiler does not like it */
	tmp = ntohs(ctrl_addr.sin_port) - 1;
	data_source.sin_port = htons( tmp );
	debug = 0;
	while ( (c = getopt(argc, argv, "dlt:v")) != -1 ) {
		switch (c) {
		case 'v':
			debug = 1;
			break;
		...
		case 't':
			...
			break;
		default:
			fprintf(stderr, "ftpd: Unknown flag ignored.\n");
			break;
		}
	}
	while ( optind < argc )
		fprintf(stderr, "ftpd: Unknown arg %s ignored\n",
			argv[optind++]);
	...
	char *name;
{
	...
	if (oldeuid && oldeuid != -1)
		CLR_WORKPRIVS_NON_ADMIN(seteuid (0));
	if ((sp = getspnam(name)) == NULL) {
	if (oldeuid && oldeuid != -1)
		CLR_WORKPRIVS_NON_ADMIN(seteuid (oldeuid));
		return (NULL);
	}
	if (oldeuid && oldeuid != -1)
		CLR_WORKPRIVS_NON_ADMIN(seteuid (oldeuid));
	...
	char *passwd;
{
	...
	CLR_WORKPRIVS_NON_ADMIN(seteuid(pw->pw_uid));
...
#define MAX_BIND_TRIES 5
	...
	for (tries = 0; tries < MAX_BIND_TRIES; tries++) {
		if (bind(s, &data_source, sizeof (data_source)) == -1) {
			if (errno == EADDRINUSE)
				sleep(1);
			else 
				goto bad;
...
{
	if (logged_in) {
		(void) CLR_WORKPRIVS_NON_ADMIN(seteuid(0));
		logwtmp(ttyline, "", "");
	}

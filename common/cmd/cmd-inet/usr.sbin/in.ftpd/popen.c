...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.sbin/in.ftpd/popen.c	1.6.8.2"
#ident  "$Header: popen.c 1.2 91/06/26 $"
	...
	char *program, *type;
{
	...
	if (!pids) {
		if (!(pids =
		    (pid_t *)malloc((u_int)(NPIDS * sizeof(pid_t)))))
			return(NULL);
		bzero(pids, NPIDS * sizeof(pid_t));
	}
	if (pipe(pdes) < 0)
		return(NULL);
	...
	case 0:				/* child */
		...
		CLR_MAXPRIVS_FOR_EXEC execv(gargv[0], gargv);
		_exit(1);
	}

...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.sbin/slink/exec.c	1.2.8.1"
#ident  "$Header: exec.c 1.2 91/06/26 $"
...
va_dcl
{
	...
	(void) sprintf(buf, "Function \"%s\", command %d: ", fi->func->name, c->cmdno);
	(void) vsprintf(&buf[strlen(buf)], fmt, args);
	va_end(args);

...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.sbin/route.c	1.3.9.2"
#ident  "$Header: route.c 1.2 91/06/26 $"
	...
	char *argv[];
{
	...
	if (optind < argc) {
		if (strcmp(argv[optind], "add") == 0 ||
			...
			++optind;
			changeroute((argc - optind), &argv[optind]);
		} else
			printf("%s: huh?\n", argv[optind]);
	}
	exit(0);
...
flushroutes()
{
	...
	rks.mirk_symname = RTHASHSIZE;
	rks.mirk_buf = &rthashsize;
	rks.mirk_buflen = sizeof(rthashsize);

	if (ioctl(kmem,MIOC_READKSYM,&rks) != 0) {
		printf("route: \"rthashsize\", symbol not in kernel\n");
		exit(1);
	}
	routehash = (mblk_t **) malloc(rthashsize * sizeof(mblk_t *));

	rks.mirk_symname = RTHOST;
	rks.mirk_buf = routehash;
	rks.mirk_buflen = rthashsize * sizeof(mblk_t *);

	if (ioctl(kmem,MIOC_READKSYM,&rks) != 0) {
		printf("route: \"rthost\", symbol not in kernel\n");
		exit(1);
	}
...
again:
	...
	if (doinghost) {
		rks.mirk_symname = RTNET;
		rks.mirk_buf = routehash;
		rks.mirk_buflen = rthashsize * sizeof(mblk_t *);
		if(ioctl(kmem,MIOC_READKSYM, &rks) != 0) {
			printf("route: \"rtnet\", symbol not in kernel\n");
			exit(1);
		}
		doinghost = 0;
		goto again;
...
# endif XNS
	...
	default:
		...
		(void) sprintf(line, "af %d: %x %x %x %x %x %x %x", sa->sa_family,

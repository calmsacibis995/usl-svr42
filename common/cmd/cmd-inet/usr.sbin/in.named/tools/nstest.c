...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.sbin/in.named/tools/nstest.c	1.1.10.2"
#ident  "$Header: nstest.c 1.2 91/06/26 $"
	...
	char *argv[];
{
	...
	while ( (n = getopt(argc, argv, "p:ivrd")) != -1 ) {
		switch (n) {
		case 'p':
	...
	_res.nsaddr.sin_port = port;
 	if (optind < argc)
 		_res.nsaddr.sin_addr.s_addr = inet_addr(argv[optind++]);
 	if (optind < argc) {
 		log = fopen(argv[optind],"w");
 		if (log == NULL) perror(argv[optind]);
 	}
	for (;;) {
		printf("> ");
		fflush(stdout);
		if ((cp = (char *)fgets(buf,sizeof(buf),stdin)) == NULL)
			break;
		cp[strlen(cp)-1] = '\0';
		switch (*cp++) {
		case 'a':
		...
		case '^':
			{
			    ...
			    fgets(IType,sizeof(IType),stdin);
			    if (IType[0] == 'u') {
			    	Type = T_UNSPEC;

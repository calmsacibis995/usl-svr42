...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.sbin/in.named/ns_main.c	1.4.9.2"
#ident  "$Header: ns_main.c 1.2 91/06/26 $"
	...
	char *argv[], *envp[];
{
	...
	*/
	...
	while ( (c = getopt(argc, argv, "b:d:p:")) != -1 ) {
		...
		case 'p':
			ns_port = htons((u_short)atoi(optarg));
			break;
	...
	if (optind < argc ) {
		if ( bflag )	/* already set bootfile above */
			usage();

...
#ident	"@(#)cmd-inet:common/cmd/cmd-inet/usr.sbin/in.tftpd.c	1.9.6.1"
#ident  "$Header: in.tftpd.c 1.3 91/09/20 $"
...
usage:
	...
	if (pwd == NULL) {
		...
		pass.pw_uid = UID_NOBODY;
		pass.pw_gid = GID_NOBODY;
		pwd = &pass;
	}
...
again:
	...
	/* 
	 * Validate must be done in child because it does chroot.
	 * Main loop has to access /dev/udp.
	 */
...
send_data:
		...
		for ( ; ; ) {
			...
			alarm(0);
			timeout = 0;	/* success reset timeout */
			if (n < 0) {
				if (errno == EINTR)
					continue;
				if ((errno == ENETUNREACH) ||

...
#ident	"@(#)compress:compress.c	1.4.5.12"
#ident "$Header: compress.c 1.2 91/09/09 $"
...
register int argc; char **argv;
{
    ...
    *filelist = NULL;
...
#else
	    fprintf(stderr,"Usage: compress [-cfv] [-b maxbits] [file ...]\n");
#endif /* DEBUG */
	    exit(1);
    	}
	...
	if (errflg) {
		fprintf(stderr,"Usage: zcat [file ...]\n");
		exit(1);
    	}
    }
...
char *ifname, *ofname;
{
    ...
    fclose(stdout);
    if (lstat(ifname, &statbuf)) {		/* Get stat on input file */
	perror(ifname);
	return;
    }
...
do_Pflag() {
	...
	/* A NULL, EOF or broken pipe will break loop. 	*/
	   ...
	   if( (link(lng_ctmp, namebuf)) < 0) {   
		perror(namebuf);
		fprintf(stderr, "Compress: Cannot link tmp to %s\n",namebuf);
	   }

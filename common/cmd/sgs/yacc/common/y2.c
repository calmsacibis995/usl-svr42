...
{	int ii,i,j,lev,t, ty; /*ty is the sequencial number of token name in tokset*/
	...
	while ((c = getopt(argc, argv, "wvVdltp:Q:")) != EOF)
		switch (c) {
			case 'v':
				foutput = fopen(FILEU, "w" );
				if( foutput == NULL ) error(badopenid,
					badopen, FILEU, strerror(errno));
				break;
			case 'V':
			...
			case '?':
			default:
				(void) pfmt(stderr,MM_ACTION, ":36:Usage: yacc [-wvVdlt] [-Q(y/n)] [-p driver_file] file\n");
				exit(1);
		}

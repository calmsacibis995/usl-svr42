...
#ident	"@(#)acpp:common/expand.c	1.99"
/* macro expansion */
...
/* warned_recursion non-zero if current macro produced warning for recursion */
static int warned_recursion;
...
*/
{
	...
	register Token* tp;	/* a Token in the input list	*/
	register Token * pretp;	/* Token before tp */
	...
	tp = (pretp = inputp)->next;
	for(state = S_defined; ; tp = (pretp = tp)->next)
	{
		switch (tp->code)
		{
		case C_WhiteSpace:
...
*/
{
	...
	(pretp = &head)->next = tp = inputp;
	do {
		if (TK_INEXPAND(tp) == 0 && (tp->next == 0 || tp->next->code != C_Marker || (pp_flags & F_Xt)))
		...
		else if ((mp = (tp->code == C_Identifier? (tp->aux.hid?0:st_ismacro(tp)) : 0)) != 0
			 && st_hidden(mp))
		{
			tk_hide(tp);
			continue;
...
*/
{
#define MAX_RECURSION 2000
	static int recursion_level = 0;
	long number = tp->number;
	COMMENT(tp->code == C_Identifier || tp->code == C_RParen);
	if (++recursion_level > MAX_RECURSION)
		FATAL("macro nesting too deep", (char *)0);
#ifdef TRANSITION
	if (st_recurse(mp) && !warned_recursion)
	{
	        WARN("macro recursion", (char *)0);
		warned_recursion = 1;
        }
#endif

...
#else
	(void) sprintf(linetk.ptr.string, "%ld", number);
	linetk.rlen = strlen(linetk.ptr.string);
#endif
...
** returns zero if the macro may be expanded.
*/
{
	if (pp_flags & F_Xt) return 0;
...
int st_recurse(mp)
	Macro * mp;
/* Given a pointer to an active Macro definition, this routine
** returns a non-zero number if the macro was previously
** expanded, thus indicating macro recursion.
*/
{
	if (!(pp_flags & F_Xt)) return 0;
        COMMENT(find(mp->name, mp->namelen) != 0);
...
*/
{
	COMMENT(find(mp->name, mp->namelen) != 0);
	COMMENT((*find(mp->name, mp->namelen))->nformals != UNDEFINED);
	COMMENT((mp->flags & M_HIDDEN) == 0 || (pp_flags & F_Xt));
	mp->flags |= M_HIDDEN;
#ifdef DEBUG
	if ( DEBUG('s') > 0 )

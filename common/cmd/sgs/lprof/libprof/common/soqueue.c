...
#ident	"@(#)lprof:libprof/common/soqueue.c	1.2"
...
TYPE_BASEAD	base;
{
	...
	if (!(qn_p = search_queue((void *) link_p->l_name, chk_path))) {
		...
		qn_p = (QUEUE_NODE *) _lprof_Malloc(1, sizeof(*qn_p));
...
TYPE_BASEAD	base;
{
	...
	while (lm_p) {
		...
		if (highbase <= lm_p->l_addr && lm_p->l_addr <= base) { /* 5099 */
			rvlm_p = lm_p;
			highbase = lm_p->l_addr;	/* 5099 */
		}
		lm_p = lm_p->l_next;
	}

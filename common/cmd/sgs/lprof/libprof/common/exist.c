...
#ident	"@(#)lprof:libprof/common/exist.c	1.2"
#include <stdio.h>
#include <string.h>
...
struct  caOBJ_ENTRY *obj_entry;
{
	...
	if (
		...
		if (
			...
			goto success;
		} else if (_CAobj_time(objectfile, obj_entry) == OK) {
...
short
_CAobj_time(objectfile,obj_entry)
char *objectfile;
struct caOBJ_ENTRY *obj_entry;
{
	short ret_code = OK;
...
struct caFILEDATA *filedata;
{
	...
	{
		if(MACH_TYPE == UNKNOWN || hdrbuf.mach_type != MACH_TYPE)

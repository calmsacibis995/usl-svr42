...
#ident	"@(#)lprof:libprof/common/symintLoad.c	1.4"
...
PROF_FILE	*proffilePtr;
{
	...
	while (sym_p < sym_lim_p) {
		...
		/* is there at least one coverage array between this and the
		   next file symbol?
		*/
		if (stscan(&tsym_p, next_p, stchk_cov)) {
			symcount += addcovset(sym_p, next_p);
		}
	...
	prsym_list_p = (PROF_SYMBOL *) _lprof_Malloc(st_size, 1);
...
*
*	Note: "k" is used to avoid having "sncov_p" increment beyond
*	the last allocated search node and thereby (possibly) cause
*	a segmentation violation.
*/
static int
addcovset(filsym_p, end_p)
Elf32_Sym *filsym_p;
Elf32_Sym *end_p;
{
	SEARCH_NODE	*stl_p;
	...
	sncov_p = stl_p = build_stlist(filsym_p, end_p, stchk_focov, &stlcount);

	for (k = stlcount; k > 0; k--)
	{
		if (stchk_cov((Elf32_Sym *) (sncov_p->sn_value_p)))
		{
			fname_p = (char *)
				&(sncov_p->sn_name_p[sizeof(COV_PREFIX)-1]);
			if ((snfunc_p = sn_search(fname_p, stl_p, stlcount))
				|| (snfunc_p = sn_search(fname_p, gstlist,
					gstlist_cnt)))
			{
...
int		*count_p;
{
	...
	list_p = (SEARCH_NODE *) _lprof_Malloc(count, sizeof(*list_p));
...
*		- _lprof_Malloc space to contain the pointers.
...
init_dblist()
{
	...
	dblist = (SEARCH_NODE *) _lprof_Malloc(dblist_cnt, sizeof(*dblist));
...
DB_TAG	*tag_p;
{
	...
	curf_lns_p = (PROF_LINE *) _lprof_Malloc(curf_lncnt, sizeof(*curf_lns_p));
	...
	if ( ++prsym_cnt > prsym_cap ) {
		if ( prsym_cap == 0 ) {
			...
			prsym_list_p = (PROF_SYMBOL *)
				_lprof_Malloc( prsym_size, prsym_cap );
		} else {
			prsym_cap += PS_BLKFACTOR;
			prsym_list_p = (PROF_SYMBOL *)
				_lprof_Realloc( (char *) prsym_list_p,

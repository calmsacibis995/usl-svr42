...
#ident	"@(#)lprof:cmd/main.c	1.16.1.5"
 ...
 *	lprof [-psPTx] [-I incdir] [-r srcfile] [-c covfile] [-f func] [-o prog]
 *
 ...
 *	-f	produce information for these functions only - NOT IMPLEMENTED
 *	-o	use this object file (if different path)
 *	-I	look in directory
 *	-l	lprof-style: display numbered lines with execution counts.
 *	-P	produce human-readable count file (not with -s, -x, -m)
...
#define F_RCNT		FBIT(8)		/* Set if "-P" seen */
...
extern int _rcnt_flag;	/* readable count file flag */
...
char **argv;
{
    ...
    while ((i = getopt(argc, argv, "spxlI:r:c:o:m:d:PTV")) != EOF) {
        switch (i) {
		...
		break;
	    case 'P':   /* human readable count file */
		flags |= F_RCNT;
		flags |= F_LIST;
		_rcnt_flag = 1;
		break;
	...
	*/
    ...
    if (flags & F_RCNT) {
 	if (flags & F_SUM) {
		fprintf(stderr, "lprof: Readable count file (-P) unavailable with summary format (-s).\n");
		exit(1);
	}
	if (lxprint == XOPT) {
		fprintf(stderr, "lprof: Readable count file (-P) unavailable with execution format (-x).\n");
		exit(1);
	}
    }
    if (flags & F_MERGE) {
	if (flags & F_RCNT){
		fprintf(stderr, "lprof: Readable count file (-P) unavailable with merge option (-m).\n");
		exit(1);
	}
	if (cmd.cov_next < 2) {
	...
	ret_code = CAmerge(&cmd);
    }

    else {
	if (flags & F_SUM) {
	    ret_code = CAreport(SUM,&cmd);
	}

	if (flags & F_LINE) {
	    ret_code = CAreport(LINE,&cmd);
	}

	if (flags & F_LIST) {
	    if (cmd.fnc_next == 0)
		ret_code = CAreport(LISTALL,&cmd);
	    else
		ret_code = CAreport(LIST,&cmd);
	}
    }
    exit(ret_code);
}
...
struct command *cmd;
{
    ...
    if (flags & F_RNCT) printf("-P option\n");

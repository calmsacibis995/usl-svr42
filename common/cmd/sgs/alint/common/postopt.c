...
#ident	"@(#)alint:common/postopt.c	1.22"
/*
...
int goal;
{
	...
	*/
	if (TY_ISNUMTYPE(SY_TYPE(sid)) || TY_ISPTR(SY_TYPE(sid))) {
	    if (max_sids == 0) {
		max_sids = LNAMES;
...
ND1 *tr;
{
    ...
    static const char expfalse[] ="logical expression always false: op \"%s\"";
    static const char exptrue[] ="logical expression always true: op \"%s\"";
    static const char context[] ="constant in conditional context: op \"%s\"";
    LNBUG( ln_dbflag > 1, ("ln_logical"));

    /* 
    ** Check cases of logical and/or involving constants.
...
ND1 *p;
{
	...
	if (TY_EQTYPE(tc,tl))
	    return;
	tc = TT(tc, ts_ptr)->ts_type;
	tl = TT(tl, ts_ptr)->ts_type;
	/* strip all the qualifiers under ts_ptr strucrure */
	if (TY_EQTYPE(TY_UNQUAL(tc),TY_UNQUAL(tl)))
	    return;

...
#ident	"@(#)alint:common/lint.c	1.16"
#include <stdio.h>
#include <ctype.h>
#include "p1.h"
#include "lnstuff.h"
...
ND1 *p;
{
    ...
    return ((p->rval == ND_NOSYMBOL) && 
	    (((p->op == ICON) && (p->lval < 0)) ||
	     ((p->op == FCON) && (FP_CMPX(p->xval, zero) < 0))) );
}
...
ND1 *l, *r;
{
    ...
    return (((l->op==ICON) && (r->op==ICON) &&
		...
		||
	   ((l->op==FCON) && (r->op==FCON) &&
	    ((FP_CMPX(l->xval,r->xval) != 0) || (l->lval!=r->lval))));
}
...
int goal, side;
{
    ...
    */
    if (op == UNARY AND) {
	if ((goal&LEFT_ASSG) && tr->left->op == NAME)
	    return ASSG|AVAL;
	return (goal&LEFT_ASSG)|AVAL;
    }
    ...
    **
    ** We need to know if we are on the left-hand side of an
    ** assignment to look for array assignments.  Do not pass
    ** this through a STAR node.
    */
    if (goal&(ASSG|LEFT_ASSG)) {
	if (op == STAR && (goal&(AVAL|LEFT_ASSG)) == LEFT_ASSG)
	    return goal & ~LEFT_ASSG;	
	else if (side == LN_RIGHT)
	    return VAL|(goal&LEFT_ASSG);
	else if (op == DOT || op == CONV)
	    return goal;
	else return VAL|LEFT_ASSG; 
    }

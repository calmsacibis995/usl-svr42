	...
	if ( (scn = elf_getscn(elf,symtab)) != NULL)
	{
		if ( (shdr = elf32_getshdr(scn)) != NULL)
		{
		   ...
		   {	
			...
			if (( sectp != NULL ) &&
			    (( ELF32_ST_TYPE(p->st_info) == STT_FUNC) ||
 			    ...
 			    ( ELF32_ST_TYPE(p->st_info) == STT_OBJECT)))
			{
				...
				func->faddr = p->st_value + ((ehdr->e_type == ET_REL)? sectp->shdr->sh_addr : 0 );
				func->fcnindex = counter;
				func->stt_type = ELF32_ST_TYPE(p->st_info);

...
#ident	"@(#)acomp:common/sym.h	55.2.6.7"
/* sym.h */
...
#define SY_BUILTIN	(1<<17)		/* symbol was redefined as builtin */
#define SY_ADDRTAKEN	(1<<18)		/* set if address assigned or passed
					   in function call. Only calculated
					   if ADDRTAKEN is defined in CG
					*/
#define SY_AMIGO_OBJECT	(1<<19)		/* set for NAMES for which AMIGO
					** has created an object_info
					*/
#define SY_ASM_FULL_OPT (1<<20)		/* fully optimizable asm */

#define SY_ASM_PARTIAL_OPT (1<<21)	/* partially optimizable asm */
...
#define	SY_AUX(sid)	(SY(sid).sy_aux)	/* union(int, struct *) SY_AUX(SX sid) */
...
#define SY_ISSYM(id)	((id) > SY_NOSYM)

...
#ident	"@(#)acomp:common/acomp.mk	55.2.6.12"
...
#Following default should only be set when make is run from machine
#dependent makefile with lintit target -- it's there to keep lint quiet.
COMPVERS=
...
NO_AMIGO_ACOMP_O=	$(OF2) $(CG_O) $(OF1) $(OF3) $(ACLEX_O) \
			$(MDOBJ) $(FPE_O)
ACOMP_O=	$(NO_AMIGO_ACOMP_O) $(AMIGO_O) expand.$o

ACCHDR=	$A/p1.h $A/aclex.h $A/ansisup.h $A/cgstuff.h \
	$A/debug.h $A/decl.h $A/err.h $A/host.h $A/init.h $A/inline.h \
...
clobber:	clean
		-rm -f acomp acgram.[ch] aclex.c acomp.o

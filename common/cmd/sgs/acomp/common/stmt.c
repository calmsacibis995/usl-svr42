...
#ident	"@(#)acomp:common/stmt.c	55.4.2.8"
/* stmt.c */
...
#define cur_iter_line	sm_curblock.sm_cur_iter_line
...
extern void tfreeall();
    ...
    */
    ...
    DB_E_FCODE();			/* generate debug info at end of
					...
					*/
#ifndef NO_AMIGO
    if (do_inline && al_optable())
	inline_endf(sm_curfunc);
    else 
#endif
	cg_endf(al_endf(),sm_curfunc);             /* generate epilogue code */
    return;
}
...
*/
{
    ...
    */
    ...
    if (cur_iter != ND1NIL) {
#ifdef LINT
	elineno = cur_iter_line;
	sm_expr(cur_iter);
	elineno = save_elineno;
#else
	sm_expr(cur_iter);
#endif
    }
...
{
#ifdef LINT
    /* Do not analyze iterator until after loop */
    cur_iter = p;
    cur_iter_line = elineno;
    return;
#endif

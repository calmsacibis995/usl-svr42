...
#ident	"@(#)acomp:common/sym.c	55.1.4.7"
/* sym.c */
...
T1WORD type;
{
    ...
    sp->sy_regno = SY_NOREG;
    sp->sy_flags = SY_INSCOPE;
    sp->sy_sameas = SY_NOSYM;
    sp->sy_class = SC_AUTO;
    sp->sy_type = type;
    sp->sy_level = SL_INFUNC;
    sy_outofscope = i;
    return i;
}
#endif
...
*/
{
    ...
    */
	...
	for (i = sy_labtab; i != SY_NOSYM; i = SY(i).sy_next) {
	    ...
	    /* Check for unused/undefined labels */
	    if (SY(i).sy_class == SC_LABEL) {
		if (!( SY(i).sy_flags & SY_DEFINED))
			UERROR("undefined label: %s", SY(i).sy_name);
		else if (verbose && !(SY(i).sy_flags & SY_REF))
			WERROR("unused label: %s", SY(i).sy_name); 
	    }
	    last = i;
	}
	if (sy_labtab) {

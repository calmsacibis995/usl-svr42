...
#ident	"@(#)acomp:common/trees.c	55.12.3.12"
/* trees.c */
...
*/
{
    ...
    */
    l = p->left;
    if (callop(p->op)) {
	if (l->op == ICON && l->rval != ND_NOSYMBOL &&
		(SY_FLAGS(l->rval) & SY_AMBIG))
	    return 1;
	else if (l->op == UNARY AND && l->left->op == NAME && 
		l->left->rval != ND_NOSYMBOL && 
		(SY_FLAGS(l->left->rval) & SY_AMBIG))
	    return 1;
    }
...
*/
{
	...
	new->xval = FP_LTOX(1L);		/* don't use 0.0 in case of
						** division
						*/
    }
    TPRINT(new);
...
/* Make a NAME leaf node from a symbol index. */
{
    ...
    SY_FLAGS(sid) |= SY_REF;
#endif
    if (SY_FLAGS(sid) & SY_BUILTIN) 
        new->flags |= FF_BUILTIN;
    return( new );
}
...
*/
{
	...
	if (p->op == NAME)
		...
		UERROR("cannot take sizeof function");
    }
...
*/
{
    ...
    */
	...
	case ASG MINUS:
		...
		div = tr_arith(DIV, new, tr_icon((CONVAL) BITOOR(lsize)));
		al_call(div);
		return div;
	    }
...
#ifdef	FAT_ACOMP
	if (name->rval != ND_NOSYMBOL)
	    SY_FLAGS(name->rval) |= SY_UAND; /* note address taken */
#endif
#ifdef  OPTIM_SUPPORT
        OS_UAND(name->rval);
#endif
...
char *s;
{
    ...
    if( ty == LTYPE && p->op == NAME && SY_ISSYM(p->rval))
	DPRINTF("%s=%d) %s=%s, ", s,
	    (int) node_no(p), opst[p->op], SY_NAME(p->rval));
    else

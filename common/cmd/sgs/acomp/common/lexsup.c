...
#ident	"@(#)acomp:common/lexsup.c	1.17"
/* lexsup.c */
...
isident:;
    ...
    if (	len >= sizeof(BUILTIN_PREFIX)
	&&	s[0] == BUILTIN_PREFIX[0] 
        && 	strncmp(s, BUILTIN_PREFIX, sizeof(BUILTIN_PREFIX)-1) == 0
    ) {
	s += sizeof(BUILTIN_PREFIX) - 1;
	len -= sizeof(BUILTIN_PREFIX) - 1;
	builtin = 1;
    }

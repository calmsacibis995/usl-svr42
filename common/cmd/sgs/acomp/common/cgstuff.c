...
#ident	"@(#)acomp:common/cgstuff.c	55.4.7.2"
/* cgstuff.c */
...
int const_strings = 0;	/* -1c sets this to non-zero */
...
** space if readonly is C_READONLY or const_strings is non-zero.
...
/* This stuff SHOULD call exname(); leave out call for now. */
		...
		case SC_MOU:
		    break;
		default:
		    cerror("strange storage class %d in cg_p2tree()",SY_CLASS(sid));
		    /*NOTREACHED*/
		}
	    }
#if	0
...
/* Add item to queue after place. */
{
	...
	qp->cgq_delete_flag = 0; /* HACK HACK */
	++TD_USED(td_cgq);
	TD_CHKMAX(td_cgq);
	return qp;
}
...
void (*func)();
{
	...
	else if (func == cg_setlocctr) return (detail > 1) ? ("cg_setlocctr") : 0;
...
int detail;
{
	...
	if (!name)
		return;
	fprintf(stderr,"flow=%d line=%d scope_marker=%d ignore_flag=%d ",
		CGQ_INDEX_NUM(index),
		flow->cgq_ln,

...
#ident	"@(#)acomp:common/p1allo.c	1.3.5.15"
/* p1allo.c */
...
int al_saw_asm;			/* saw asm in this fn: suppress call to AMIGO.
...
int
al_are_calls()
/* Are there calls in this function -- is it a leaf function */
{
	return BLK(1).blk_calls;
}
...
/* Return set of registers in scope in current block. */
{
	...
	*/
	return ( BLK(node->lval).blk_regs_inscope );
}
...
#ifndef NO_AMIGO
static int
al_find_scope(start, stop, parent)
int start, stop, parent;
{
    int child, prev_child=0;
    int contains_child = 0;
    int first_contained = 0;
#ifdef lint
    int last_contained = 0;
#else
    int last_contained = 0;
#endif
    blk_t *cur_blk;

    /* check intersection of new block [first,last] with parent's children */
    /* after iteration, new block is between prev_child and child */
    for (child=BLK(parent).blk_child; child && stop >= BLK(child).blk_start;
	prev_child = child, child=BLK(child).blk_sibling) {

	/* new scope is identical to child */
	if (stop == BLK(child).blk_stop && start == BLK(child).blk_start)
		return child;

	/* new scope is within child */
	else if (stop <= BLK(child).blk_stop && start >= BLK(child).blk_start) {
	    	return al_find_scope(start,stop, child);
	}

	/* new scope contains child */
	else if (stop >= BLK(child).blk_stop && start <= BLK(child).blk_start) {
		contains_child = 1;
	}

	/* for the next two checks know that stop >= BLK(child).blk_start
	   also know new is not contained in child nor contains child
	*/

	else if (start <= BLK(child).blk_start){/* new contains child's strt */
		stop = BLK(child).blk_stop;	/* extend to contain child */
		contains_child = 1;
	}

	else if (start <= BLK(child).blk_stop){ /* new contains child's stop */
		start = BLK(child).blk_start;	/* extend to contain child */
		contains_child = 1;
	}
    }

    /* after possibly extending start and stop above, the scope == parent */
    if (start == BLK(parent).blk_start && stop == BLK(parent).blk_stop)
	return parent;

    /* create new block */
    cur_blk = al_alloc_block();

    /* intialize new block */
    cur_blk->blk_start = start;
    cur_blk->blk_stop = stop;
    cur_blk->blk_first = 1;
    cur_blk->blk_parent = parent;

    if (contains_child) {
	prev_child = 0;

    	/* Transfer those children of new block's parent, that are subsumed by 
	   new block, to new block.
	   After iteration new block is between prev_child and child.
	   All the children between first_contained and last_contained
	   inclusive, are subsumed by new block
	*/
	for (child=BLK(parent).blk_child; child && stop >= BLK(child).blk_stop;
	    child=BLK(child).blk_sibling) {

	    if (start <= BLK(child).blk_start) {	/* contains child */
	        if (first_contained == 0)
		    first_contained = child;
	        else
		    BLK(last_contained).blk_sibling = child;

	        last_contained = child;
		BLK(child).blk_parent = CUR_BLK;
	    }
	    else					/* child precedes */
		prev_child = child;
	}

	cur_blk->blk_child = first_contained;
	cur_blk->blk_lastchild = last_contained;
	BLK(last_contained).blk_sibling = 0;
    }

    /* splice into sibling chains */
    cur_blk->blk_sibling = child;

    if (prev_child)
	BLK(prev_child).blk_sibling = CUR_BLK;
    else
	BLK(parent).blk_child = CUR_BLK;

    if (!child)	/* is the last child */
	BLK(parent).blk_lastchild = CUR_BLK;
...
*/
{
    BLK(al_curblock).blk_stop = CGQ_CUR_INDEX();
...
int
al_optable() {
    return (do_amigo && !nerrors && !al_saw_asm && 
...
*/
{
    ...
    al_prop_call(al_curblock,flags);

    if (flags == CONTAINS_CALL) {
...
static void 
al_prop_call(block,flags) 
int block; 
unsigned flags;
{
    for (; block; block = BLK(block).blk_parent)
        BLK(block).blk_calls |= flags;
}
...
*/
{
	...
	regno = cisreg(VAUTO, cgtype, (OFFSET) 0, 
			op->blk_allregs, BLK(OBJ(obj).obj_block).blk_calls);
	break;
    case SC_PARAM:
	regno = cisreg(VPARAM, cgtype, (OFFSET) SY_OFFSET(sid), 
			op->blk_allregs, BLK(OBJ(obj).obj_block).blk_calls);
	break;
    }
...
**		threshold.
**
** There's an assumption here that cisreg() can be called in
** an arbitrary order, and that the offset in the parameter's
** symbol table entry is enough information to cisreg() to
...
/* Print block list */
{
    ...
    printf("%s Blocks\n%s ======\n", COMMENTSTR, COMMENTSTR);
    for (i = 1; i <= CUR_BLK; ++i) {
	printf("%s [%3d] par=%3d, chd=%3d, sib=%3d, fst=%3d, lst=%3d st=%5d sp=%5d calls=%d\n",
		...
		CGQ_INDEX_NUM(BLK(i).blk_stop), BLK(i).blk_calls);
    }
    fflush(stdout);
    return;
}
...
/* Assume there is a call if we are compiling statement at a time */
    ...
    */
	...
	regno = cisreg(op, cgtype, off, AA(al_sp).al_regs, CONTAINS_CALL);

...
#ident	"@(#)cpp:common/cpp.c	1.45.2.2"
...
# endif
unsigned char macbit[ ALFSIZ + 11 ];
unsigned char toktyp[ ALFSIZ ];
#define BLANK 1
#define IDENT 2
#define NUMBR 3
...
#if scw2
unsigned char t21[ ALFSIZ ], t22[ ALFSIZ ], t23[ ALFSIZ + NCPS ];
#endif
...
#define QB 16
#define WB 32
unsigned char fastab[ ALFSIZ ];
unsigned char slotab[ ALFSIZ ];
...
unsigned char buffer[ NCPS + BUFSIZ + BUFSIZ + NCPS ];
...
*/
#	define SBSIZE (244000 + (MAXFRE+1)*BUFSIZ)
#endif

unsigned char	sbf[ SBSIZE ];
unsigned char	*savch	= sbf;

/*	start for MNLS4.0	*/
/*# define DROP '\376' /* special character not legal ASCII or EBCDIC */ 
# define DROP 0377 /* special character not legal ASCII or EBCDIC */
/*	end for MNLS4.0	*/
# define WARN DROP
# define SAME 0
...
static unsigned char warnc = WARN;

int mactop, fretop;
unsigned char *instack[ MAXFRE ], *bufstack[ MAXFRE ], *endbuf[ MAXFRE ];

int plvl;	/* parenthesis level during scan for macro actuals */
int maclin;	/* line number of macro call requiring actuals */
unsigned char *macfil;	/* file name of macro call requiring actuals */
unsigned char *macnam;	/* name of macro requiring actuals */
int maclvl;	/* # calls since last decrease in nesting level */
unsigned char *macforw;	/* pointer which must be exceeded to decrease nesting level */
...
STATIC	unsigned char	*fnames[ MAXNEST ];
STATIC	unsigned char	*dirnams[ MAXNEST ];	/* actual directory of #include files */
STATIC	FILE	*fins[ MAXNEST ];
STATIC	int	lineno[ MAXNEST ];

STATIC	unsigned char	*dirs[ MAXINC ];	/* -I and <> directories */
STATIC	unsigned char	*dfltdir = (unsigned char *) 0;	/* user-supplied default (like /usr/include) */
unsigned char *copy(), *subst(), *trmdir(), *strchr();
unsigned char *chkend();
struct symtab *stsym();
...
STATIC	int	ifno;
# define NPREDEF 20
STATIC	unsigned char *prespc[ NPREDEF ];
STATIC	unsigned char **predef = prespc;
STATIC	unsigned char *punspc[ NPREDEF ];
STATIC	unsigned char **prund = punspc;
STATIC	int	exfail;
struct symtab
{
	unsigned char	*name;
...
STATIC	unsigned char	ifelstk[MAX_DEPTH];
...
unsigned char *pdef_mach[MAX_PD_MACH+2] =	/* choose using PD_MACH */
{
	/*
	...
	*/
	...
	(unsigned char *) "os",
	(unsigned char *) "mert",
	(unsigned char *) "RT",	(unsigned char *) 	"RES",		
	/* These are the currently supported machines */
	(unsigned char *) "interdata",(unsigned char *)	"pdp11",
	(unsigned char *) "vax",
	(unsigned char *) "u370",
	(unsigned char *) "u3b",
	(unsigned char *) "u3b5",
	(unsigned char *) "u3b2",
	(unsigned char *) "u3b20d",
	(unsigned char *) "i386",
	(unsigned char *) PD_MY_MACH,	/* for adding "own" machine */
	(unsigned char *) 0
};
...
unsigned char *pdef_sys[MAX_PD_SYS+2] =	/* choose using PD_SYS */
{
	(unsigned char *) "unix",
	(unsigned char *) "gcos",
	(unsigned char *) "ibm",
	(unsigned char *) "DMERT",
	(unsigned char *) PD_MY_SYS,	/* for adding "own" system */
	0
};
...
#	define MAXOUT 80
	...
	{			/* chars between linefeeds */
		register unsigned char c, *pblank;
		unsigned char savc, stopc, brk;
...
# endif
	if ( p1 < inp )
		p1 += fwrite( p1, sizeof( unsigned char ), inp - p1, f );
	outp = p1;
}
	...
	register unsigned char *p;
{
	...
	*/
	...
	for ( ;; )
	{
		...
		{
			maclvl = 0;
			if ( 0 < ( ninbuf = fread( pbuf, sizeof( unsigned char ), BUFSIZ, fin ) ) )
			{
				pend = pbuf + ninbuf;
				*pend = '\0';
				return( p );
			...
			if ( ifno == 0 )	/* end of input */
			{
				if ( plvl != 0 )
				{
					...
					unsigned char *tfil = fnames[ifno];
		...
		case WARN:
			{
				...
				dump();
				speakup = 0;
				for ( ii = sizeof(int) / sizeof(unsigned char); --ii >= 0; )
				{
					if ( eob( p ) )
						p = refill( p );
					speakup |= ( *p++ & 0xFF ) << ( ii * 8 );
	...
	register unsigned char *p;
{
	...
	unsigned char **dirp, *nfil;
	unsigned char filname[BUFSIZ];
...
#	endif
		...
		if ( NULL != ( fins[ifno + 1] = fopen( (char *) nfil, READ ) ) )
		{
			filok = 1;
			fin = fins[++ifno];
			if ( print_incs )
...
equfrm( a, p1, p2 )
	register unsigned char *a, *p1, *p2;
{
...
dodef( p )		/* process '#define' */
	unsigned char *p;
{
	register unsigned char *pin, *psav, *cf;
	unsigned char **pf, **qf;
	int b, c, params;
	int ex_blank;	/* used to ignore extra blanks in token-string */
	int sav_passcom = passcom;	/* saved passcom, used to reset it */
	struct symtab *np;
	unsigned char *oldval, *oldsavch;
	unsigned char *formal[MAXFRM]; /* formal[n] is name of nth formal */
	unsigned char formtxt[BUFSIZ]; /* space for formal names */
			...
			do_line:
				...
				*/
				...
				if ( *s != '\n' )
				{
					...
					{
						register unsigned char *t = savch;
...
subst( p, sp )
	...
	struct symtab *sp;
{
	static unsigned char match[] = "%s: argument mismatch";
	register unsigned char *ca, *vp;
	int params;
	unsigned char *actual[MAXFRM]; /* actual[n] is text of nth actual */
	unsigned char acttxt[BUFSIZ]; /* space for actuals */
	...
	if ( 0 != ( params = *--vp & 0xFF ) )	/*definition calls for params */
	{
		...
		if ( maclin != lineno[ifno] )	/* embedded linefeeds in macro call */
		{
			...
			for ( i = sizeof( int ) / sizeof( unsigned char ); --i >= 0; )
			{
				if ( bob( p ) )
				{
					outp = inp = p;
		...
		if ( params < 0 || (params > 0 && dparams != 1) )
			ppwarn( match, sp->name );
		while ( --params >= 0 )
			*pa++ = (unsigned char *) "" + 1;	/* null string for missing actuals */
		--flslvl;
		fasscan();
	}
...
# endif
	p = (unsigned char *) "_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
	i = 0;
	while ( c = *p++ )
	{
...
# if unix || DMERT
	fnames[ifno = 0] = (unsigned char *) "";
	dirnams[0] = dirs[0] = (unsigned char *) ".";
# endif
# if ibm
	fnames[ifno = 0] = "";
# endif
...
# endif
# if gimpel || gcos
	fnames[ifno = 0] = (unsigned char *) inquire( stdin, _FILENAME );
...
# endif
		...
		default:
			if ( fin == stdin )
			{
				if ( NULL == ( fin = fopen((char *) argv[i], READ ) ) )
				{
...
*/
			...
			{
				static unsigned char _sobuf[BUFSIZ];

				if ( NULL == ( fout = fopen((char *) argv[i], WRITE ) ) )
				{
...
# endif
	...
	if (dfltdir != (unsigned char *) 0)
		dirs[nd++] = dfltdir;
	else {
# if unix || DMERT
#    if defined(SGSINC) || defined(DMERT)
		if ( strlen( SGSINC ) != 0 )
			dirs[nd++] = (unsigned char *) SGSINC;	/* For cross cpp's */
#    endif
#    ifdef USRINC
		if ( strlen( USRINC ) != 0 )
			dirs[nd++] = (unsigned char *) USRINC;	/* usually /usr/include */
#    endif
...
# endif
	...
	fnames[ifno] = (unsigned char *) "command line";
	lineno[ifno] = 1;
	cp2 = prespc;
	while ( cp2 < predef )
		stsym( *cp2++ );
...
{
	static unsigned char name[NCPS];
	unsigned char *saveptr, ch;
	register unsigned char *ptr3 = name;

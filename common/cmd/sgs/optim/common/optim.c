...
#ident	"@(#)optim:common/optim.c	1.41.1.8"
...
static void indegree();		/* compute indegree */
...
optim()  { /* control improvement sequencing */
...
#endif
#ifdef BBOPTIM
		bldgr(true,false);	/* build flow graph and call bboptim */
#else
		bldgr(false);
#endif
	...
	static void
#ifdef ISLABREF
rmlbls(first) int first; { /* remove unreferenced labels */
#else
rmlbls() { 
#endif
...
#ifdef ISLABREF
		if(first && ISLABREF(p) && (s=getp(p)) != NULL) {
			addlbl(s, &b0);
			addref(s,(unsigned)strlen(s)+1,NULL);
		}
#endif
	}
...
#ifdef BBOPTIM
/*	static*/ void
/* build flow graph of procedure */
bldgr(opt,call_terminates) boolean opt,call_terminates; {
#else
	/*static*/ void
			...
			!tmp->marked && tmp->length == 1 &&
				...
				ISREMBR(tmp->lastn)) {
	    		   ...
	    		   if(!(isuncbr(tmp->lastn) || 
		     	   tmp->nextr != NULL || same(tmp->lastn, bcur->lastn)))
				   break;
...
#ifdef BBOPTIM
    bldgr(false,false); /* update block structure but don't call bboptim */
#else
    bldgr(false);
#endif
...
*/
	...
	for (ALLB(b,b0.next))
	{
	    ...
	    */
			if (is_jmp_ind(b->lastn) && (sw = get_base_label(b->lastn->op1))) {
...
#ifdef BBOPTIM
    bldgr(false,false);			/* build flow graph */
#else
    bldgr(false);
#endif
...
b->indeg,b->length,b->marked);
	fprintf(stderr, "b: %x,b->next: %x,b->nextl: %x, b->nextr: %x\n",
b,b->next, b->nextl, b->nextr);
	fprintf(stderr,
"b->firstn: %x, b->lastn: %x\n", b->firstn, b->lastn);

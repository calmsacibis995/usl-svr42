...
char     *filename;
{
	...
	else
	{
		...
		switch (class)
		{
			...
			case ELFCLASS64:   Printf("%-13s", "62-bit"); break;
	...
	Printf("%-13#x%-11#x%-13#x%-12#x%#x\n",
		elf_head_p->e_entry,
		elf_head_p->e_phoff,
		elf_head_p->e_shoff,
		elf_head_p->e_flags,
		elf_head_p->e_ehsize);
	Printf("%-13#x%-11u%-13#x%-12u%u\n\n",
...
char *filename;
{
	...
	for(num=1; num<num_scns; num++, p++)
	{
		...
		Printf("%-13#x%-13#x%-13#lx\t%s\n",
			p->p_shdr->sh_addr,
			p->p_shdr->sh_offset,
			p->p_shdr->sh_size,
			p->scn_name);

		Printf("\t%lu\t%lu\t%-13#x%-13#lx\n\n",
			p->p_shdr->sh_link,
			p->p_shdr->sh_info,
...
SCNTAB *reloc_symtab;
{
	...
	while(no_entries--)
	{
		...
		retValue++;
		Printf("%-12#x", (unsigned long)rela->r_offset);
		if (!v_flag)
		{
...
SCNTAB *reloc_symtab;
{
	...
	while(no_entries--)
	{
		...
		entCnt++;
		Printf("%-12#x", (unsigned long)rel->r_offset);
		if (!v_flag)
		{
			Printf("%-23d%-18d\n", sym, type);
		}

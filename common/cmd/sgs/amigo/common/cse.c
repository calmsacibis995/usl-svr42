...
static TEMP_SCOPE_ID PROTO( get_global_temp_scope, (void));
...
extern dont_fix_bug_flag;

			/*
			** If set, dont try to fix a bug in which cse causes
			** overlapping live ranges for amigo generated temps
			** which end up on the stack.
			*/
...
struct Block *blocks;
{
	...
	count = order_blocks(blocks);

	cse_arena = arena_init();

	if(!dont_fix_bug_flag)
...
struct Block *blocks;
{
	/* The blocks may be processed in any order.  Lexical order
...
int id;
{
	...
	*/
	...
	case LTYPE:
		return tree->op == CSE && (!id || tree->lval == id);
	case BITYPE:
...
ND1 *root;
{
	...
	if (r->left && !cse_in_tree(r->left, root->lval)) {
	...
	else if (r->right && !asgop(r->op) && !cse_in_tree(r->right, root->lval)) {
...
occur_info *occurs;
{
	...
	DEBUG(aflags[cse].level&4,("lexical cse scope: %d --> %d\n", 
		...
		/*
		** Following code should be changed to be unconditionally
		** executed once it is verified that the fix is adequate.
		** It is supposed to prevent an amigo allocated temp
	...
	END_FOR_ALL_ND1_IN_BLOCK
	/* NOTREACHED */
	cerror("No executable code found in block %d", start->lex_block_num);
}
...
int count;
{
	...
	out = Arena_alloc(cse_arena, count, Expr_set);
	oldout = Expr_set_alloc(cse_arena);
	e_gen_kill(blocks);

	e_avail[0] = Expr_set_alloc(cse_arena);
	out[0] = Expr_set_alloc(cse_arena);
	
	bv_init(false, e_avail[0]);
	bv_assign(out[0], e_gen[0]);
	for (b=blocks->next; b; b=b->next) {
		int i = b->lex_block_num - 1;
		e_avail[i] = Expr_set_alloc(cse_arena);
		out[i] = Expr_set_alloc(cse_arena);
		bv_init(true, out[i]);
...
struct Block *blocks;
{
	...
	while (blocks) {
		...
		e_gen[b] = Expr_set_alloc(cse_arena);
		e_kill[b] = Expr_set_alloc(cse_arena);

		bv_init(false, e_gen[b]);
		bv_init(false, e_kill[b]);
		...
		FOR_ALL_ND1_IN_BLOCK(blocks, flow, node, index)
			...
			bv_or_eq(e_gen[b], gen);
			bv_or_eq(e_kill[b], kill);
			bv_minus_eq(e_gen[b], kill);
...
struct Block *block;
{
	...
	cerror("cse source not found in block");
}

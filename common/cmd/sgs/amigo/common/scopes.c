...
#ident	"@(#)amigo:common/scopes.c	1.19"
...
Block top_down_basic;
{
	...
	DEBUG(aflags[sc].level & 2, ("===> SCOPES BEFORE COELESCE <===\n"));
	DEBUG_COND(aflags[sc].level & 2, scope_debug(scope_root));
	scope_num = 0;
	coelesce_scopes(scope_root);
	DEBUG(aflags[sc].level & 1, ("===> SCOPES AFTER COELESCE <===\n"));
...
Scope scope;
{
	...
	CGQ_FOR_ALL_IN_BLOCK_REVERSE(scope->block_end, flow, index)
		ND1 *node;
...
Scope scope;
{
	...
	return(2 * LOAD_COST(TY_DOUBLE) * get_block_weight(block));
}
...
static Boolean
check_edges(bl, start, end, constraint)
Block_list bl;
int start, end;
enum Edge_constraint constraint;
{
	...
	while(bl) {
		if(bl->block->block_count>=start && bl->block->block_count<=end)
			inside_count++;
		else
			outside_count++;
		bl = bl->next;
	...
	DEBUG(aflags[sc].level & 256, ("true\n"));
	return true;
}

	/*
	** If the block numbers in a scope do not correspond to their
	** lexical ordering, or if entries/exits are other that the
...
Scope scope;
{
	...
	start = first_block->block_count;
	end = last_block->block_count;
	...
	for(block = first_block; block != last_block->next; block = block->next) {
			...
			*/
		if(! check_edges(block->pred, start, end,
			...
			*/
		if(! check_edges(block->succ, start, end,
...
Block blocklist;
{
	...
	case pop:
		...
		if(st != stack) {
			...
			** items and coelesce into a single scope.
			...
			*/
			...
			while(st_next->marked) {
				...
				ASSERT(st_next->first_block == stack->first_block,stack); 
			...
			/*
			** stack points to the coelesced new scope.

...
#ident	"@(#)amigo:common/amigo.h	1.44.1.31"
#include "p1.h"
#include "mfile2.h"
#ifdef NODBG
...
#define DEPTH_FIRST(block_ptr) {\
	Block * block_ptr; \
	for(block_ptr = get_dfn_ptr_first(); \
		block_ptr <= get_dfn_ptr_last(); block_ptr++) {

#define REVERSE_DEPTH_FIRST(block_ptr) {\
	Block * block_ptr; \
	for(block_ptr = get_dfn_ptr_last(); \
		block_ptr >= get_dfn_ptr_first(); block_ptr--) {
...
\
		...
		&& (SY_CLASS((sid)) == SC_AUTO ||\
			(SY_CLASS((sid)) == SC_PARAM \
				&& SY_NAME((sid)) != dots && SY_NAME((sid)) != varargs))\
\
			/* not volatile */\
...
#define Object_list_alloc(arena) (Arena_alloc(arena,1,struct Object_list))
	...
	*/
#define BL_HAS_OS_LOOP		1	/* os_loop(..) */
#define BL_HAS_EXPR		2
#define BL_CONTAINS_CALL	4
#define BL_CONTAINS_DIV		8
#define BL_CONTAINS_FP_DIV	16
#define BL_PREHEADER		32	/* Contains os_loop(start) call */

typedef struct Block *Block;
typedef struct Block_list *Block_list;
typedef struct Loop_info *Loop;
typedef Expr_set Assign_set; /* For now, PSP */
...
struct Block {
	...
	Block next;		/* for lexical order */
	int lex_block_num;
	Block_list succ, pred;		/* flow graph edges */
...
struct Loop_info {
	...
	Block_list blocks;	/* List of blocks properly belonging to
				...
				*/
	Loop enclose;		/* the enclosing loop */
	Loop prev, next;	/* Next for inner to outer, prev outer to inner
				*/
	Loop nest, sibling;	/* nest points to a list of nested
...
typedef struct Expr_info {
	...
	/* following two can be in union RMA */
	struct Object_info *addcon;	
	struct Occur_info *occurs;	/* list of live occurrences of this expr */
} *Expr;
...
extern void PROTO(	loop_unrolling,(Loop));
...
extern Block PROTO( 	basic_block,(int branch_count, int max_label, 
			    int min_label, Boolean *has_loop));
...
extern Block *PROTO(	get_dfn_ptr_first,(void));
extern Block *PROTO(	get_dfn_ptr_last,(void));
...
extern void PROTO(	get_use_def,(Arena));
extern void PROTO(	hash_exprs,(int *branch_count, int *max_label, int *min_label));
...
#endif
extern void PROTO(	set_defs_refs,(Block, Expr_set kills, ND1 *node, int context));

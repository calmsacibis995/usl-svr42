...
#ident	"@(#)amigo:common/str_red.c	1.25"
#include "amigo.h"
#include <memory.h>
#include "costing.h"

typedef struct cand_descr {
	int in_table;
	int cand_type;
...
Loop loop;
{
	...
	if(IS_INDVAR(node,loop)) {
		...
		switch(def_node->op) { /* figure out the update const for
			...
			rtrn->update = sign * def_node->right->lval;
			break;
		default:
...
int pattern[];
{
	...
	pattern[index(C_ICON,MUL,C_ICON)] = IV_MUL_ICON;
...
ND1 * node;
{
	...
	return COST(node) > sr_threshold /* + update_cost(type) +
		load_cost(type) */;
}

#define CANDIDATE_IS_COST_EFFECTIVE(node) (cand_cost++,(node)->op != LS &&\
	(node->opt)->flags & EXPENSIVE)
...
START_OPT
		...
		DEBUG(aflags[sr].level&8,
			("Node is candidate %d with update %ld, induction var %s(%d):\n",
			node->opt->setid,
			candidate->update,

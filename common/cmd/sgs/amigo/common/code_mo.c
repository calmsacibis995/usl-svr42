...
#ident	"@(#)amigo:common/code_mo.c	1.26"
#include "amigo.h"
#include "costing.h"
...
Loop first_loop;
{
	...
	*/
	...
	for(loop = first_loop; loop; loop = loop->next) {
		...
		if(! loop->enclose ) { /* top level loop */
...
Loop loop;
{
	Loop enclose = loop->enclose;
	if( !INVARIANT(node, enclose) ) {
		if(node->left) 
...
START_OPT
		...
		if(loop->enclose) { /* cost subexpressions */
		...
		changed=true;
      		DEBUG(aflags[cm].level&1,
      			("Replacing invariant(%d) above, cost %d, by temp(%d) in loop %d at line=%d\n",
      				node->opt->setid, COST(node), node->rval, loop->id, dlineno));        
STOP_OPT
	}
	else {
...
Loop first_loop, last_loop; /* An innermost loop in the cluster */
{
	...
	for(loop = first_loop; loop != last_loop; loop = loop->next) {
		...
		BV_FOR(loop->invariants,inv)
			if(enclose &&
				!bv_belongs(inv, enclose->expr_kills)
				&& bv_belongs(inv,enclose->header->antic)) {
				bv_set_bit(inv,enclose->invariants);
...
in header for loop %d\n", inv, loop->id));
		...
		/* Build a new ND1 node from the expression.  In the
		   ...
		   if enclose is not null, and if expr is invariant
		   in enclose enter expr in the invariants of enclose
...
Loop loop;
{
	...
	Loop enclose = loop->enclose;
	if(enclose && !bv_belongs(expr->setid,enclose->expr_kills) &&
		bv_belongs(expr->setid,enclose->header->next->antic) &&
...
in loop %d\n", expr->setid, loop->id));
		...
		DEBUG_COND(aflags[cm].level&2,
	 	...
	 	bv_set_bit(expr->setid,enclose->invariants);

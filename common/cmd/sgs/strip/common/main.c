...
#ident	"@(#)strip:common/main.c	1.28"
/**************************** CI5/elf-based STRIP *****************************/
...
#ifndef __STDC__
static PF sigsave[] = {SIG_IGN,SIG_IGN,SIG_IGN,0};
#endif
...
char *temp_file_name;
{
	...
	char *temp_fname;
#ifdef __STDC__
	PF sigsave[] = {SIG_IGN,SIG_IGN,SIG_IGN,0};
#endif
	
	for (i = 0; signum[i]; i++) /* started writing, cannot interrupt */
		sigsave[i] = signal(signum[i], SIG_IGN);
	...
	temp_fname = get_dir(fname);
      	...
      	if ((fd=open(fname, O_WRONLY | O_TRUNC | O_CREAT, (mode_t) 0644)) == -1)
	{
		(void) fprintf(stderr,
	...
	(void) unlink(temp_fname);
#endif
	for (i = 0; signum[i]; i++) /* finished writing, restore sig. funcs. */
		(void)signal(signum[i], sigsave[i]);
	return;
}
...
/* used to indicate that the symbol table 
 * should not be deleted from a relocatable file */

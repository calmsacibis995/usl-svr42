...
#ifndef	DEBUG
#define	OPTIONS	"abd:e:h:I:l:mno:rstu:xz:B:GL:M:Q:T:VY:"
#else
#define	OPTIONS	"abd:e:h:I:l:mno:rstu:xz:B:D:GL:M:Q:T:VY:"
#endif
...
static void
build_bsym_list(input)
char *input;
{
	int	count = 1;
	int	size = BLIST_SIZE;
	char	*ptr;
	bsym_list = mymalloc(BLIST_SIZE*sizeof(int));
	ptr = strtok(input, " ,\t");
	bsym_list[0] = ptr;
	while((ptr = strtok(NULL, " ,\t")) != NULL)
	{
		if (count >= (size - 2))
		{
			...
			size += BLIST_SIZE;
			bsym_list = myrealloc(bsym_list, size*sizeof(int));
		}
		bsym_list[count++] = ptr;
	}
	bsym_list[count] = NULL;
}
	...
	DPRINTF(DBG_ARGS, (MSG_DEBUG, "input argument to process_flags is: %#x",c));
		...
		case 'z':
			if (strncmp(optarg, "defs", 4) == SAME) {
				...
				lderror(MSG_FATAL, "illegal -z %s option", optarg);
			}
			break;
...
int mode;
{
	...
	fprintf(stderr,"\t[-B sortbss assign contiguous addresses to bss symbols in each input file\n");

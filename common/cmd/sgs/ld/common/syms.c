...
update_syms()
{
	...
	if(!rflag && !Gflag){
		if (tlast == NULL)
			lderror(MSG_WARNING, "No read-only segments found.  Setting `%s` to 0."
				,ETEXT_USYM );
		if (dlast == NULL)
			lderror(MSG_WARNING, "No read-write segments found.  Setting `%s` to 0."
				,EDATA_USYM);
	}
	...
	for(h = 0; h< NBKTS; h++){
		...
		case SHN_UNDEF:
			...
			if ((ELF_ST_BIND(sym->ls_syment->st_info) == STB_WEAK) && !rflag)
				sym->ls_syment->st_shndx = SHN_ABS;
			break;
		case SHN_COMMON:

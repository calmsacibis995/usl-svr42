	...
	char	*fname;
{
	...
	case ELF_K_ELF:
		...
		case ET_DYN:
			...
			break;
		default:
			lderror(MSG_FATAL, "is an illegal elf file type");
			break;
		}
		break;
	case ELF_K_NONE:
		lderror(MSG_FATAL, "file type neither object nor archive");
		break;
	default:
		break;
	}
...
process_relobj()
{
	...
	savesec:
		...
		case SHT_DYNAMIC:
			...
			/*NOTREACHED*/
		default:
			if(shp->sh_type > SHT_HIUSER)
				lderror(MSG_WARNING, "section type 0x%x may conflict with system-defined type", shp->sh_type);
			else if(shp->sh_type < SHT_LOUSER )

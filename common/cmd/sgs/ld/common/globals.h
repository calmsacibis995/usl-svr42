...
#ident	"@(#)ld:common/globals.h	1.34"
...
struct insect {			/* Input section descriptor */
	...
	Elf_Data *is_outdata;	/* Data buffer pointer in the output */
				/* section */
	int	is_initSize;	/* used only when Bsortbss is used. The initial
				** section size is stored here which is later
				** used for assigning addresses to common
				** symbols
				*/
};
...
struct ldsym {			/* Symbol table node */
	...
	Boolean ls_Bsymbolic;		/* references to this symbol should be handled like -Bsymbolic */
};
...
extern Boolean
	...
	Bsortbss;		/* true if bss symbols have to be sorted
				** by input file in the output sections */
...
extern char	**bsym_list;		/* list of symbols to be handled like -Bsymbolic */
...
PROTO(void mark_bysmbolic, (void));
...
PROTO(VOID *myrealloc, (VOID *, unsigned int));
...
PROTO(Insect *getIsectByName, (List *, char *));

...
#endif
{
	...
	{
		error(MSGxpg, MSGnam, curstr, num);
		olstfree(op);
		return 0;
	}
	operfree(op);
...
#endif
{
	...
	*/
	...
	{
		error(MSGxpg, MSGstr, curstr, num);
		olstfree(op);
		return 0;
	}
	ep = setlessexpr(ep);
...
#endif
{
	...
	err:;
		...
		{
		default:
			fatal("expr_check():unknown expr type bit-list: %u",
				tybits);
			/*NOTREACHED*/
...
#endif
{
	...
	{
		error(MSGrng, MSGaln, curstr, num_tohex(vp->ev_int));
		return 0;
	}
...
#endif
{
	...
	*/
	...
	{
		error(MSGxpg, MSGaln, curstr, num);
		return 0;
	}
...
#   define ALLOC_ALGN_CHUNK 50
	...
	*/
	...
	do
	{
		...
		{
			error(MSGrng, MSGmfill, curstr, num_tohex(vp->ev_int));
			olstfree(op);
			return 0;
		}
		if (ulp > listbuf && (align >= ulp[-2] || max > ulp[-1]))
		{
			error(MSGpairs, curstr, align, max, ulp[-2], ulp[-1]);
			olstfree(op);
			return 0;
		}
		ulp[0] = align;
...
#endif
{
	...
	if (vp->ev_flags & (EV_OFLOW | EV_TRUNC))
	{
		error(MSGrng, "size", curstr, num_tohex(vp->ev_int));
		olstfree(op);
		return;
	}
	size = vp->ev_ulong;
...
#endif
{
	...
	{
		error(MSGnon, "same section label", curstr, 1);
		return;
	}
...
#endif
{
	...
	do
	{
		...
		*/
		...
		{
			error(MSGxpg, MSGr_i, curstr, num);
			olstfree(op);
			return;
		}
		switch (ep->ex_type)
		...
		case ExpTy_Relocatable:
			...
			break;
		default:
			error(MSGnon, MSGr_i, curstr, num);
			olstfree(op);
			return;
		}
		fixexpr(ep);
	...
	case Dir_Float:
		...
		if (FLOAT_ALIGN > 1)
		{
			if (sectoptalign(secp, flt_align) == 0)
				warn(MSGpad, secp->sec_sym->sym_name, curstr);
		}
		break;
	case Dir_Double:
		/*CONSTANTCONDITION*/
...
#endif
{
	if (sectoptalign((Section *)0, al) == 0)
		warn(MSGpad, cursect()->sec_sym->sym_name, dir);
	curstr = dir;
	dot_bytes(op, sz, form);
}

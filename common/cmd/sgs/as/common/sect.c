	...
	*/
	for (;;)
	{
		...
		*/
		...
		{
			(void)fprintf(stderr,
				"VarSize:Address changes (start %lu):\n", sz);
		}
#endif
...
#endif
{
	...
	if ((cp->code_size = npair) != npair)
		fatal("sectalign():too many align/max pairs: %u", npair);
...
#endif
{
	...
	{
		fatal("sectbackalign():bogus label: %s",
			(const char *)sp->sym_name);
	}
	if (secp->sec_lastpad->code_addr > cp->code_addr
		|| secp->sec_lastpad == cp)
	{
		error("Padding is not permitted between .backalign and %s",
			(const char *)sp->sym_name);
		return;
	}
	...
	*secp->sec_last = *cp;
	...
	cp->data.code_align = pair;
	if ((cp->code_size = npair) != npair)
		fatal("sectbackalign():too many align/max pairs: %u", npair);

...
#endif
{
	...
	*/
	errno = 0; /* Different versions of stdio may leave errno set;	 */ 
		   /* fp_atox can set errno, so ensure it's clear first. */
	flp->flt = fp_atox((const char *)str);
	if (errno != 0)
	{
		errno = 0;	/* ERANGE can be misleading */
		error("floating constant out of range: %.*s", len, str);
	}
	ep->ex_type = ExpTy_Floating;
	return ep;
#else
	fatal("fltexpr():creation of floating expression attempted");
#endif	/*FLOATEXPRS*/
}
...
#endif
{
	...
	*/
	...
	for (;;)	/* only loop for Cont_Expr */
	{
		...
		{
		default:
			fatal("exprfrom():unknown context: %u",
				(Uint)ep->ex_cont);
			/*NOTREACHED*/
...
#endif
{
	...
	*/
	for (;;)
	{
		...
		{
			exprerror(ep, MSGexp, tokstr((int)ep->ex_op));
			complain = 0;
		}
		...
		case Cont_Expr:
			...
			break;
		case Cont_Oplist:
			fatal("exprtype():at oplist context");
			/*NOTREACHED*/
		case Cont_Unknown:
...
#endif
{
	...
	if (op == ExpOp_Subtract)
		...
		fatal("unaryexpr():non-unary operator %s", tokstr(op));
	if ((ep = expr_avail) == 0)
		ep = alloexpr();
	expr_avail = ep->parent.ex_ptr;
	ep->ex_op = op;
...
#endif
{
	...
	if (op < ExpOp_OPER_BinaryFirst || op >= ExpOp_OPER_BinaryLast)
		fatal("binaryexpr():non-binary operator %s", tokstr(op));
	if ((ep = expr_avail) == 0)
		ep = alloexpr();
	expr_avail = ep->parent.ex_ptr;
	ep->ex_op = op;
...
#endif
{
	...
	if (op->oper_flags != 0)
		fatal("cutoper():cannot bypass amoded operand");
#ifdef DEBUG
	if (DEBUG('a') > 2)
		(void)fprintf(stderr, "Returning Operand @%#lx\n", (Ulong)op);
#endif
...
#endif
{
	...
	case ExpOp_LeafFloat:
#ifdef FLOATEXPRS
		(void)fputs(fp_xtoa(*ep->right.ex_flt), stderr);
#else

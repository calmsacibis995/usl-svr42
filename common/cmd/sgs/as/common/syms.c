...
#endif
{
	if (sp->sym_defn != 0)
	{
		error(MSGdef, (const char *)sp->sym_name);
		return 0;
	}
	sp->sym_defn = secp;
...
#endif
{
	...
	*/
	...
	if (CHAR_BIT * sizeof(Ulong) < 16 + CHAR_BIT + 5 + 1)
		fatal("lookup():hash calculation unroll invalid");
	switch (n = len)
	{
	case 0:
		fatal("lookup():zero length name");
		/*NOTREACHED*/
	default:
		hval = s[0] << 20;
		hval += s[1] << 16;
...
#endif
{
	...
	if (sp->sym_kind == SymKind_Common)
		warn("label was common symbol: %s", (const char *)sp->sym_name);
		...
		*/
		...
		case Bind_Temporary:
			if (stp->sym.sym_defn == 0
				&& stp->sym.sym_kind != SymKind_Common)
			{
				error(MSGundf, (char *)stp->sym.sym_name);
			}
			break;
		case Bind_Global:
		case Bind_Weak:
...
#endif
{
	...
	{
	default:
		fatal("set_valid():unknown exty: %u", (Uint)sp->sym_exty);
		/*NOTREACHED*/
	case ExpTy_Unknown:
	...
	case ExpTy_Relocatable:
		...
		*/
		...
		{
			fatal("set_valid():already have size: %s",
				(const char *)sp->sym_name);
		}
		if ((sp->sym_flags & (SYM_TYPED | SYM_VALTYPE)) == SYM_VALTYPE)
		{
			fatal("set_valid():already have type: %s",
				(const char *)sp->sym_name);
		}
		...
		*/
		if (!(vp->ev_flags & EV_RELOC))
			fatal("set_valid():no sect after reloc expr eval");
		...
		{
			if (vp->ev_sym == 0)
				fatal("set_valid():no sym/sect for reloc");
			backerror((Ulong)sp->sym_file, sp->sym_line,
...
#endif
{
	...
	if ((ep = sp->size.sym_expr) != 0)
	{
		...
		{
			fatal("size_valid():non-int symbol size: %s",
				(const char *)sp->sym_name);
		}
		vp = evalexpr(ep);
		/*
		...
		*/
		if (vp->ev_flags & (EV_OFLOW | EV_TRUNC))
		{
			exprerror(ep, "size for %s out of range: %s",
				(const char *)sp->sym_name,
				num_tohex(vp->ev_int));
		}
		size = vp->ev_ulong;
...
#endif
{
	...
	if ((ep = sp->type.sym_expr) != 0)
	{
		...
		if (ep->ex_type != ExpTy_Integer)
		{
			fatal("type_valid():non-str/int symbol type: %s",
				(const char *)sp->sym_name);
		}
		vp = evalexpr(ep);
		/*
		...
		*/
		...
		{
			if (ep == expr_object)
				fatal("type_valid():bad object type expr");
			exprerror(ep, "symbol type for %s out of range: %s",
				(const char *)sp->sym_name,
				num_tohex(vp->ev_int));
		}
		type = vp->ev_ulong;
...
#endif
{
	...
	*/
	...
	{
		fatal("align_valid():non-int symbol alignment: %s",
			(const char *)sp->sym_name);
	}
...
#endif
{
	...
	if (sp->sym_flags & SYM_BOUND)
	{
		...
		{
		default:
			fatal("bindsym():unknown binding: %d\n", bind);
			/*NOTREACHED*/
		case Bind_Local:
			bstr = "local";
			break;
		...
		if (sp->sym_bind == bind)
		{
			warn(MSGbind, "already", bstr,
				(const char *)sp->sym_name);
		}
		else
		{
			error(MSGbind, "previously", bstr,
				(const char *)sp->sym_name);
		}
	}
	sp->sym_bind = bind;
...
#endif
{
	if (sp->sym_flags & SYM_SIZED)
	{
		error(MSGmlt, "size", (const char *)sp->sym_name);
		return;
	}
	fixexpr(ep);
	sp->size.sym_expr = ep;
...
#endif
{
	if (sp->sym_flags & SYM_TYPED)
	{
		error(MSGmlt, "type", (const char *)sp->sym_name);
		return;
	}
	sp->type.sym_expr = ep;
...
#endif
{
	...
	{
		error("cannot define . (dot) as common");
		return;
	}
	if (sp->sym_kind == SymKind_Set)
	{
		error(".set name cannot also be common: %s",
			(const char *)sp->sym_name);
		return;
	}
	...
	if (sp->sym_defn != 0)
	{
		warn("common symbol already defined: %s",
			(const char *)sp->sym_name);
		return;
	}
	sp->sym_exty = ExpTy_Relocatable;
...
#endif
{
	...
	if (sp->sym_kind == SymKind_Common)
	{
		error("common name cannot also be .set: %s",
			(const char *)sp->sym_name);
		olstfree(op);
		return;
	}
	if (sp->sym_kind == SymKind_Dot)	/* .set .,.+intexpr */
	{
		...
		*/
		...
		secp = cursect();
		if (!(vp->ev_flags & EV_RELOC))
			fatal("opersym():no sect after reloc expr eval");
		if (vp->ev_sec != secp

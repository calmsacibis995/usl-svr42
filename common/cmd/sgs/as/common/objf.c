...
#endif
{
	...
	{
		fatal("addstr():strtab too small, need %lu, max %lu",
...
#endif
{
	...
	{
		fatal("startsyms():wrong number of symbols");
	}
...
#endif
{
	...
	if (num_of.sections != 2 * num_of.user_sect + 2)
		fatal("openobjf():wrong number of sections");
	if ((fd = open(name, O_WRONLY | O_CREAT | O_TRUNC, 0666)) < 0)
	{
		error("cannot open output file: %s", name);
		return 0;
	}
	/*
	...
	*/
	...
	{
		error("object file library/assembler version mismatch");
		return 1;
	}
	if ((elf = elf_begin(fd, ELF_C_WRITE, (Elf *)0)) == 0)
	{
		error("cannot begin output file: %s", name);
		return 1;
	}
...
#endif
{
	...
	{
		error("cannot create section: %s",
			name + (char *)strtab.base);
		return 0;
	}
	if ((osp->data = elf_newdata(osp->scn)) == 0)
	{
		error("cannot create data block for section: %s",
			name + (char *)strtab.base);
		return 0;
	}
	if ((osp->shdr = EFUNC_getshdr(osp->scn)) == 0)
	{
		error("cannot create section header for section: %s",
			name + (char *)strtab.base);
		return 0;
	}
...
#endif
{
	...
	*/
	...
	for (; cp->code_kind != CodeKind_Unset; cp = cp->code_next)
	{
		...
		{
			error("address for relocation out of range: %#lx",
				cp->code_addr);
		}
		...
		*/
		...
		{
			fatal("genrel():relocation type out of range: %#lx",
				(Ulong)cp->code_size);
		}
		if (symndx & ~MASK(TARGET_RELS_BIT))
		{
			fatal("genrel():reloc symbol index out of range: %#lx",
				symndx);
		}
	...
	*/
	...
	for (; cp->code_kind != CodeKind_Unset; cp = cp->code_next)
	{
		...
		*/
		...
		if (cp->code_size & ~MASK(TARGET_RELT_BIT))
		{
			fatal("genrela():relocation type out of range: %#lx",
				(Ulong)cp->code_size);
		}
		if (symndx & ~MASK(TARGET_RELS_BIT))
		{
			fatal("genrela():reloc symbol index out of range: %#lx",
				symndx);
		}
		rp->r_info = EMACR_R_INFO(symndx, cp->code_size);
...
#endif
{
	...
	{
		error("too many total sections, max %u",
	...
	*/
	...
	for (ns = 0; ++ns <= num_of.user_sect;)
	{
		...
		if (secp == 0)
			fatal("closeobjf():missing section: %lu", (Ulong)ns);
...
#endif
{
	...
	*/
	...
	{
		error("type for section \"%s\" out of range: %#lx",
	...
	{
		error("attribute for section \"%s\" out of range: %#lx",
			(const char *)secp->sec_sym->sym_name,
			secp->attr.sec_value);
	}
	shp->sh_flags = secp->attr.sec_value;
	...
	case SHT_SYMTAB:
	case SHT_DYNSYM:
		warn("link and info unset in header for section: %s",
...
#endif
{
	if (ep->ex_op != ExpOp_LeafString)
		fatal("objfsrcstr():not passed string expr");
	if (srcf_ptr != 0)
		return 0;
...
{
	if (ns >= SHN_LORESERVE)
		error("too many sections, max %u", (Uint)SHN_LORESERVE - 1);
	/*
...
#endif
{
	...
	*/
	...
	case SymKind_Set:
		...
		{
			fatal("objfsymbol():operand-typed .set: %s",
				(const char *)sp->sym_name);
		}
	...
	default:
		...
		{
			fatal("objfsymbol():non-label regular name: %s",
				(const char *)sp->sym_name);
		}
	...
	{
		fatal("objfsymbol():out of range binding: %u",
			(Uint)sp->sym_bind);
	}
	...
	if (sp->sym_exty == ExpTy_Integer)
		...
		fatal("objfsymbol():section number too big: %lu", val);
	else
		symp->st_shndx = val;
#ifdef DEBUG
	if (DEBUG('o') > 1)

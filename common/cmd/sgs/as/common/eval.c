...
#endif
{
	...
	if ((1 + ((Uchar)~(Uint)0)) != BIT(CHAR_BIT))
		fatal("initeval():bad CHAR_BIT value: %d", CHAR_BIT);
...
#endif
{
	...
	errno = 0; /* Different versions of stdio may leave errno set;  */ 
		   /* fp_ultox, fp_ltox, and fp_atox can set errno, so  */
		   /* ensure it's clear first. 				*/    
	...
	if (errno != 0)
	{
		...
		exprerror(vp->ev_expr, MSGrng, sval);
	}
}
#endif	/*FLOATEXPRS*/
...
#endif
{
	...
	*/
	...
	{
	default:
		fatal("eval():unknown operator: %s", tokstr((int)ep->ex_op));
		/*NOTREACHED*/
	...
	case ExpOp_LeafString:
		fatal("eval():inappropriate leaf: %s", tokstr(ep->ex_op));
		/*NOTREACHED*/
	...
	case ExpOp_OffsetPart:
		fatal("eval():<s> and <o> are not implemented");
		/*NOTREACHED*/
	case ExpOp_Pic_PC:
	case ExpOp_Pic_GOT:
	case ExpOp_Pic_PLT:
	...
	case ExpOp_Multiply:
		...
		if (ep->ex_type == ExpTy_Floating)
		{
			...
			*vp->ev_flt = fp_mul(*vp->ev_flt, rhs_flt);
			if (errno != 0)
			{
				errno = 0;
				vp->ev_flags |= EV_OFLOW;
	...
	case ExpOp_Divide:
		...
		if (ep->ex_type == ExpTy_Floating)
		{
			...
			*vp->ev_flt = fp_div(*vp->ev_flt, rhs_flt);
			if (errno != 0 || fp_iszero(rhs_flt))
			{
				errno = 0;
				vp->ev_flags |= EV_OFLOW;
			}
	...
	case ExpOp_Subtract:
		...
		if (ep->ex_type == ExpTy_Floating)
		{
			...
			rhs_flt = fp_neg(rhs_flt);
			*vp->ev_flt = fp_add(*vp->ev_flt, rhs_flt);
			if (errno != 0)
			{
				errno = 0;
				vp->ev_flags |= EV_OFLOW;
		...
		if (num_subtract(vp->ev_int, &rhs_int) != 0)
			vp->ev_flags |= EV_OFLOW;
		break;
	case ExpOp_Add:
#ifdef FLOATEXPRS
...
#endif
{
	...
	case ExpTy_Relocatable:
		...
		break;
	default:
		fatal("evalexpr():inappropriate expr type: %u",
			(Uint)ep->ex_type);
		/*NOTREACHED*/
	}
...
#endif
{
	...
	if (errno != 0)
	{
		...
		exprerror(vp->ev_expr, MSGtoobig,
			cp->info.code_form == CodeForm_Float ? "float"
			: cp->info.code_form == CodeForm_Double ? "double"
			: "-unknown-", fp_xtoa(*vp->ev_flt));
	}
	if (sz != cp->code_size)
	{
		fatal("flt_todata():data size (%u) != target size (%u)",
...
#else
	fatal("flt_todata():initialization of floating object attempted");
#endif	/*FLOATEXPRS*/
}
...
#endif
{
	...
	if (num_subtract(vp->ev_int, &num) != 0)
		vp->ev_flags |= EV_OFLOW;
...
#endif
{
	...
	if (vp->ev_expr == 0)
		fatal("delayexpr():Eval not derived from expr");
	else if (vp->ev_flags & EV_FLOAT)
		fatal("delayexpr():floating expression not reevaluatable");
	...
	for (rp = reeval_list; rp != 0; rp = rp->next)
	{
		...
		*/
		if (rp->flags & EV_RELOC)
		{
			...
			{
				if (vp->ev_dot == 0)
					msg = "%s based on defined name";
				else
					subeval(vp, vp->ev_dot->code_addr);
			}
			else if (vp->ev_dot != 0)
				msg = "%s based on undefined name";
			if (msg != 0)
			{
		...
		*/
		if (num_ucompare(&rp->num, vp->ev_int) != 0)
		{
			...
			exprerror(rp->expr, "%s %s, now %s", MSGdiff,

...
#ident	"@(#)cof2elf:common/cof2elf.c	1.13"
	...
	char	*argv[];
{
	...
	(void)sprintf(tmpdir, "%s/a%s", systmp, XX);
	(void)sprintf(tmpf, "%s/b%s", systmp, XX);
	if (*mktemp(tmpdir) == '\0'
	|| *mktemp(tmpf) == '\0'
	|| siginit() != 0
	...
	(void)strcpy(arcmd, SGS);
	(void)strcat(arcmd, "ar");
...
die(j)
{
	...
	if (fork() == 0)
	{
		...
		(void)execlp("rm", "rm", "-fr", tmpdir, tmpf, temp.t_tmp, (char *)0);
	}
	exit(j);
}
	...
	char		*fullnm;
{
	...
	case ELF_K_AR:
		...
		(void)sprintf(cmdbuf, "%s -t %s >%s", arcmd, fullnm, tmpf);
		if (system(cmdbuf) != 0)
		{
			error(1, "%s failed (table of contents)\n", arcmd);
			goto ar_bad;
		}
		(void)sprintf(cmdbuf, "cd %s; %s -x %s", tmpdir, arcmd, fullnm);
		if (system(cmdbuf) != 0)
		{
		...
		while (fgets(rdbuf, sizeof(rdbuf), ar_names) != NULL)
		{
			...
			*p = '\0';
			...
			printnm.file = rdbuf;
			(void)sprintf(cmdbuf, "%s/%s", tmpdir, rdbuf);
		...
		(void)fclose (ar_names);
	ar_bad:;
		if (work == 0) /* do nothing */
			;
		else if (work > 0 && cksave((char *)0) == 0)
		{
			(void)sprintf(cmdbuf, "cd %s; %s -rc %s `cat %s`",
			...
			work = -1;
		(void)sprintf(cmdbuf, "cd %s; rm -fr *", tmpdir);
		if (system(cmdbuf) != 0)
		{
	...
	char			*save;
{
	...
	if (save != 0)
	{
		...
		else
		{
			...
			(void)sprintf(svnm, "%s/%s", cwd, save);
		}
	...
	(void)sprintf(buf, fmt, printnm.file, svnm);
	if (system(buf) != 0)
	{
...
error(int prnm, const char *format, ...)
{
	...
	{
		if (printnm.lib)
			(void)fprintf(stderr, "%s(%s): ", printnm.lib, printnm.file);
		else
			(void)fprintf(stderr, "%s: ", printnm.file);
	}
...
badmsg:
	...
	while ((scn = elf_nextscn(elf, scn)) != 0)
	{
		...
		if ((data = elf_newdata(scn)) == 0)
			goto bad;
		data->d_buf = buf;
		(void)sprintf(data->d_buf, format, argv0, CPL_PKG, CPL_REL);
		data->d_size = sz;
		return 0;
	}
	if (elf_errmsg(0) != 0

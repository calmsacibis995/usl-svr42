...
STYSHP	shp[NSTYSHPS];
int	pshp[NSTYPSHPS]; /* lists of shapes corresponding to pshapes */
...

/* Check whether type RST is big enough to
** hold all the bits for the registers specified.
*/
	...
	*/
	...
	for( s=string; *s; ++s )
	{
		...
		case 'U':
			while (*s == 'U')
				++s;
			--s;	/* Because we are going to do ++s next */
			/* FALLTHRU */
		case 'A':
		case 'C':
		case 'I':
...
outoptb()
{
	...
	printf( "OPTAB * const ophead[] = {\n" );

	for( i=0; i<DSIZE; ++i )
	{
		if( ophd[i] < 0 ) printf( "	0,\n" );
		else printf( "	(OPTAB *)&table[%d],\n", ophd[i] );
	}
	printf( "};\n" );
}
/* output statistics */
...
** Check whether left and right shapes are just REG shapes,
...
int lval;				/* non-zero if left shape used in asgop */
{
	...
	*/
	...
	if ((lreg & (TINT|TUNSIGNED)) != 0 && (rreg & (TINT|TUNSIGNED|TFLOAT|TDOUBLE|TLDOUBLE)) == 0)
	    retval = RSUBOFL;		/* call right < left */
	else if ((lreg & (TINT|TUNSIGNED|TFLOAT|TDOUBLE|TLDOUBLE)) == 0 && (rreg & (TINT|TUNSIGNED)) != 0)
...
outshphd() {
    ...
    printf( "unsigned long const shphd[] = {\n" );
...
register sha,ty;
{
	...
	else printf( "\t(SH_PTR_PTR)P(%d),\t", shp[sha].ssh );
	prnam( ty, Tnam );
}
...
#define	TALL (TCHAR|TSHORT|TINT|TLONG|TUCHAR|TUSHORT|TUNSIGNED|TULONG|\
		TFLOAT|TDOUBLE|TLDOUBLE|TPOINT|TPOINT2|TSTRUCT|TFPTR)

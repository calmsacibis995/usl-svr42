...
#ident	"@(#)cg:common/reader.c	1.25.2.13"
...
/* CG:  don't do this if exceptions are to be ignored */
	if(   o
	   ...
	   && (pop != OROR)) 
	{
	    if( (   (p->in.left->tn.type == TLDOUBLE)
		 || (p->in.left->tn.type == TDOUBLE)
		 || (p->in.left->tn.type == TFLOAT)
		 || (p->in.right->tn.type == TLDOUBLE)
...
*/
{
	...
	*/
	...
	retval = !branches && count_refs(p, top, 0, 1) > ref;
#ifndef NODBG
...
NODE *p, *t;
{
	...
	{
	case NAME: 
		return (p->tn.name == t->tn.name && p->tn.lval == t->tn.lval);
	case CSE:
		return (p->csn.id == t->csn.id); 
	case REG:
		return (p->tn.rval == t->tn.rval);
	case VAUTO:
	case VPARAM:
	case TEMP:
	case ICON:
		return (p->tn.lval == t->tn.lval);
	case FCON:
		return (!FP_CMPX(p->tn.xval, t->tn.xval));
	default:
		cerror("count_refs: unexpected LTYPE %o", p->tn.op);
		/* NOTREACHED */
	}
	/* NOTREACHED */
}

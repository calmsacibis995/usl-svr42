...
#ident	"@(#)cg:common/nail.c	1.75.1.1"
...
NODE *p;
{
	...
	switch (tptr) {
#ifdef	ALDOUBLE2
	case TDOUBLE:   size = SZDOUBLE; align = ALDOUBLE2; break;
#else
	case TDOUBLE:   size = SZDOUBLE; align = ALDOUBLE; break;
#endif
	case TFLOAT:    size = SZFLOAT;  align = ALFLOAT;  break;
	case TLDOUBLE:  size = SZLDOUBLE;align = ALLDOUBLE;break;
	}

	defalign( align );
	deflab( i = getlab() );
	fincode( p->fpn.xval, size );
	p->tn.lval = 0;
	p->tn.rval = NI_FLSTAT;
...
NODE *p;
{
	...
	if (flags & COMMON) {
		...
		cerror("defnam(): ICOMMON not yet implemented");

...
#ident	"@(#)more:more.c	1.6.8.6"
#ident "$Header: more.c 1.2 91/08/09 $"
...
#endif
{
	...
	va_list argp;
	    ...
	    va_start(argp);
#endif
	    /* We know that the last argument is always a NULL */
	    while ((arg_list[argno++] = va_arg(argp, char*)) != (char *)0)
		;
	    va_end(argp);
            execvp(cmd, arg_list);
            write (2, "exec failed\n", 12);
...
#endif
	while ((cid = (int)wait((pid_t)0)) != (int)id)
		if (cid == -1 && errno == ECHILD)
			break;
        signal (SIGINT, end_it);
        signal (SIGQUIT, onquit);
#ifdef SIGTSTP
...
set_tty ()
{
        ioctl(1, TCGETA, &otty);     /* save old tty modes */
        ioctl(1, TCGETA, &ntty);
        ntty.c_lflag &= ~ECHO & ~ICANON;
        ntty.c_cc[VMIN] = (char)1;
        ntty.c_cc[VTIME] = (char)0;
        ioctl (1, TCSETAF, &ntty);        /* set new tty modes */
}

reset_tty ()
{
        ioctl (1, TCSETAF, &otty);        /* reset tty modes */
}
...
usage()
{
        ...
        fputs(" [-cdflrsuw] [-lines] [+linenumber] [+/pattern] [filename...]\n",stderr);
        exit(1);
}

#ident	"@(#)sdb:libexp/common/Expr.C	1.34"
...
//DBG						reg, stype, frame);
	...
	if ( stype == SINVALID || a->stype == Sxfloat )
		stype = a->stype;
...
//DBG			printe("buf = '%s'\n", buf);
	...
	// if we get here, *format is a "normal" mode  [cduoxfgL] or 0
...
//DBG				a->name, a->size);
		...
		if (a->size == size || (fund == ft_lfloat && a->stype != Sxfloat)) {
			memcpy(itype.rawbytes, rval.raw(), size);
		} else if ( a->flags & FPREG ) {

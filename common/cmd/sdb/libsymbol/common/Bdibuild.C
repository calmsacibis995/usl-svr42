#ident	"@(#)sdb:libsymbol/common/Bdibuild.C	1.19"
...
Bdibuild::get_location( Attr_form & form, Attr_value & value )
{
	...
	while ( len2 > 0 )
	{
		...
		{
			case OP_REG:
				locdesc.reg( reg_no(get_long()) );
				len2 -= 5;
				break;
			case OP_BASEREG:
				locdesc.basereg( reg_no(get_long()) );
				len2 -= 5;
				break;
			case OP_ADDR:
...
void
Bdibuild::element_list(short attr)
{
	Attribute	*prev;
	long		len2;
	long		word;
	...
	prev = 0;
	if ( (attr & FORM_MASK) == FORM_BLOCK4 )
		len2 = get_long();
	else if ( (attr & FORM_MASK) == FORM_BLOCK2 )
		len2 = get_short();
	while ( len2 > 0 )
	{

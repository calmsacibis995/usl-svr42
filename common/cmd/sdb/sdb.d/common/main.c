...
#ident	"@(#)sdb:sdb.d/common/main.c	1.24"
/*
 *	UNIX debugger
 */
...
*/
	...
	if( sdbpath[ 0 ] ) {
		printf( "Source path: " );
		for(p=sdbpath;*p;p++) {
			printchar((unsigned char)*p);
			/* user will see if junk is in file */
		}
		printf("\n");
	}
	...
	if ( (sigint =  (SIG_PF) signal( SIGINT, SIG_IGN ) ) !=  SIG_IGN) {
		sigint = (SIG_PF) fault;
		signal( SIGINT, fault );
	}
	sigqit =  (SIG_PF) signal( SIGQUIT, SIG_IGN ); /* ignore "quit" signal */

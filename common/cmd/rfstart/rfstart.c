...
#ident	"@(#)rfstart:rfstart.c	1.25.24.4"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/rfstart/rfstart.c,v 1.1 91/02/28 19:28:41 ccs Exp $"
#include <stdio.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
...
char *argv[];
{
	...
	cmd_name = argv[0];
 	...
 	if(rfsys(RF_RUNSTATE) == -1 && errno == ENOPKG){
 		ERROR("RFS not configured in kernel");
 		exit(2);
 	}
	...
	/*	Download lid and privilege information per
	 *	/etc/lid_and_priv for Enhanced Security
	 */
        if ((lvlproc(MAC_GET, &tmp_lid) == 0) || (errno!=ENOPKG)){
		system("/usr/sbin/lidload");
	}

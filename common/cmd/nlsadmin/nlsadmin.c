...
#ident	"@(#)nlsadmin:nlsadmin.c	1.6.8.2"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/nlsadmin/nlsadmin.c,v 1.1 91/02/28 18:41:27 ccs Exp $"
...
no_permission()
{
	nlsmesg(MM_ERROR, "User not privileged for operation");
	exit(NLS_PERM);
}
...
char	*netspec;
{
	...
	while (fgets(buf, BUFSIZ, fp) != NULL) {
		...
		if (!Quietflag) {
			...
			printf("%s\t%s\t%s\t%s\t%s\t# %s",
				(*entry.scheme)?entry.scheme:"NOSCHEME",
				(*entry.rpc)?entry.rpc:"NORPC",
				(*entry.id)?entry.id:"NO_ID",
...
struct	svcfields *entry;
{
	...
	*ptr++ = NULL;
	entry->scheme = ptr;
	if ((ptr = strchr(entry->scheme, ':')) == NULL)
		return(BADPMFMT);
	*ptr++ = NULL;
	entry->addr = ptr;

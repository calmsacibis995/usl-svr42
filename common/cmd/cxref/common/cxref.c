...
#ident	"@(#)cxref:common/cxref.c	1.1"
#include <stdio.h>
#include <sgs.h>
#include "cxref.h"
#include "string.h"
...
/**	end for mnls	**/
    ...
    **
    **     f		- (f)unction decl. at line, level, and name
    **     R		- (R)eference of variable name at line, level
    **     A		- (A)ssignment of variable name at line, level
    **     D		- (D)efinition of variable name at line, level
    **     T		- (T)entative def. of variable name at line, level
    **     P		- (P)reprocessor def. of name at line, level
    **     d		- (d)eclaration of variable name at line, level
    **     p		- (p)reprocessor reference of name at line, level
    **     c		- fun(c)tion reference of name at line, level
    **
    ** Many of these above types are new for the kernel modularity study.
    ** In order to keep cxref working with these new codes, they are
    ** mapped into the old codes immediately.
    **
    ...
    */
	...
	if (code == EOF)
	    break;
	/* Map code as described above.  This switch could be combined
	** with the code below to be more efficient, but at a loss of
	** clarity.
	*/
	switch (code) {
	    case 'P':
		...
		while (uptr != NULL) {
			...
			*/
			...
			/*  printleft(fptr->fname, w_file+1); */
			    printleft(fptr->fname, w_file+1+setcount(fptr->fname));
			    if (!lflag)

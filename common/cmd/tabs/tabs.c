...
#ident	"@(#)tabs:tabs.c	1.10.1.3"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/tabs/tabs.c,v 1.1 91/02/28 20:11:42 ccs Exp $"
...
const char illtabs[] = ":53:Illegal tabs\n";
...
int tabvect[NTABS];
{
	...
	for (i = 0; i<NTABS-1;) {
		if (*scan == '+') {
			...
			if (t = getnum(&scan))
				tabvect[i++] = last += t;
			else error(":55:Illegal increment\n");
		}
		else {
...
int tabvect[NTABS];
{
	...
	char *temp;
	if (level)
		error(":56:File indirection\n");
	if ((fildes = open(scan,O_RDONLY)) < 0)
		error(":3:Cannot open %s: %s\n", scan, strerror(errno));
...
{
	endup();
	pfmt(stderr, MM_ERROR, arg1, arg2, arg3);
	exit(1);
}

...
#ident	"@(#)tfm:tfmlib.c	1.11.2.3"
#ident  "$Header: tfmlib.c 1.2 91/06/27 $"
#include <ctype.h>
#include <sys/types.h>
#include <priv.h>
...
extern	int	privadd(),strcmp(),lockf(),rmdir();
extern	int	rmdir(),access(),write(),priv_err();
extern	int	read(),mkdir(),unlink(),close();
extern	void	free(),tfm_report();
...
**
** Since TFM Commands have no business reading or executing files to
** which they do not have mandatory read access, tfm_setup() clears the
** MACREAD privilege from the working privilege vector.  No other
** privileges are cleared by tfm_setup().
*/

...
#ident	"@(#)tfm:tfadmin.c	1.13.2.2"
#ident  "$Header: tfadmin.c 1.2 91/06/27 $"
/***************************************************************************
 * Command: tfadmin
...
#include "err.h"
#include "tfm.h"
#include <sys/param.h>		/* TEMPORARILY REQUIRED - wm4 */
#include <audit.h>
#include <libgen.h>
#include <string.h>
...
*/
		if(procpriv(PUTPRV,opt.inv.inv_priv,opt.inv.inv_count) < 0){
			pfmt(stderr,MM_ERROR,NOSETP);
			adumprec(ADT_TFADMIN,1,strlen(cmdline),cmdline);
			exit(1);
		}

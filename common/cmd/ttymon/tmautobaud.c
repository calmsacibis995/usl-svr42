...
#ident	"@(#)ttymon:common/cmd/ttymon/tmautobaud.c	1.6.11.2"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/ttymon/tmautobaud.c,v 1.1 91/02/28 20:16:12 ccs Exp $"
...
extern	const	char badioctlfd[];

/*
 * Procedure:     auto_termio
 *
 * Restrictions:
                 ioctl(2): none
                 strerror: none
...
int	fd;
{
	...
	static const char name[] = "auto_termio";
	...
	if (ioctl(fd, TCSETAF, &termio) == -1) {
		log(MM_ERROR, badioctlfd, name, "TCSETAF", fd, strerror(errno));
		return(-1);
	}
	return(0);
}
...
int	timeout;
{
	...
	while (count--) {
		...
		cp = &buf[1];
		if ((k=read(fd, cp, 5)) < 0) {
			log(MM_HALT, ":604:autobaud: Read error: %s", strerror(errno));
			exit(1);
		}
		if (timeout)

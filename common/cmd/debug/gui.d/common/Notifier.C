#ident	"@(#)debugger:gui.d/common/Notifier.C	1.1"

#include "Notifier.h"

struct Client
{
	void		*object;	// registered object
	Notify_func	func;		// member function of object
	void		*client_data;

			Client(void *o, Notify_func f, void *d)
...
void
Notifier::add(void *client, Notify_func func, void *client_data)
{
...
int
Notifier::remove(void *old_client, Notify_func old_func, void *old_cdata)
{
	Client	*item;
...
Notifier::notify(int reason_code, void *call_data)
{
	...
	for (item = (Client *)clients.first(); item;
			item = (Client *)clients.next())
		(*item->func)(item->object, server, reason_code,

#ident	"@(#)debugger:gui.d/common/Eventlist.C	1.9"
...
// event if the event applies to more than one process, so if p == 0,
// call change_event for each event window
static void
updateAllEventWindow(Event *e, Process *p, Event_action a)
{
	Window_set	*ws = (Window_set *)windows.first();

	for (; ws; ws = (Window_set *)windows.next())
	{
		Event_window	*ew = ws->get_event_window();

		// Event window needs to be updated only if the event applies to
		// the window set's current process
		if (!ew || !ew->is_open() || !ws->current_process())
			continue;

		switch(a)
		{
			case Event_add:
				if (ws->current_process() != p)
					continue;
				ew->add_event(e);
				break;
...
Event_list::enable_event(Message *m)
{
	...
	if (e != 0)
	{
		...
		updateAllEventWindow(e, proclist.find_process(m), Event_change);
	}
}
...
void
Event_list::delete_event(Message *m, Process *p)
{
	Word eId, pId;
	m->unbundle(eId, pId);
	...
	if (e != 0)
	{
		// notify must be done before delete_process since Source and Dis
		// windows check event's list of processes against their current process
		if (e->get_type() == ET_stop)
			event_list.change_list.notify(BK_delete, e);
		...
		//if this process is the only one using this event, delete the event 
		//else only delete this process from process list of this event 
		if (e->has_no_process())
		{
			events.remove(e);
...
void
Event_list::change_event(Message *m, Process *proc)
{
	if (!proc)
	{
		display_msg(E_ERROR, GE_internal, __FILE__, __LINE__);
	...
	while ((msg = dispatcher.get_response()) != 0)
	{
		...
		case MSG_stop_event_f:
			m->unbundle(id, qflag, state, count, condition, plist, cmd);
			((Stop_event *)event)->set_count((int)count);
			break;

		case MSG_syscall_event_f:
			m->unbundle(id, qflag, state, systype, count, condition,
				plist, cmd);
			((Syscall_event *)event)->set_count((int)count);
			break;

		case MSG_signal_event_f:
			m->unbundle(id, qflag, state, condition, plist, cmd);
			break;

		case MSG_onstop_event_f:
			m->unbundle(id, state, plist, cmd);
			break;
...
// that change the stop expression or the process list - essentially, debug
// debug deletes the old event and creates a new one with the same number
void
Event_list::new_event(Message *m, Process *proc)
{
	if (!proc)
	{
		display_msg(E_ERROR, GE_internal, __FILE__, __LINE__);
...
void
Event::delete_process(Process *p)
{
	if (p == 0)
	{
		display_msg(E_ERROR, GE_internal, __FILE__, __LINE__);
...
Event::set_plist(char *pIn)
{
	...
	//get the list of process applicable
	char	*p = strtok(pIn, " ,");
	Process	*pProc = 0;
...
Event::get_plist()
{
	...
	for (Plink *plink = head; plink; plink = plink->next())
	{
		...
		buffer1.add(plink->process()->get_name());
	}
	...
	: EVENT(eid, s, stop, p, cmds)
{
	...
	{
		updateAllEventWindow(this, procP->process(), Event_add);
	}
}
...
int *
File_list::get_break_list(Process *p)
{
	Flink	*ptr;
	int	line = 0;
	...
	for (ptr = head; ptr; ptr = ptr->next())
	{
		if (! ptr->get_event()->has_process(p))
			continue;

#ident	"@(#)debugger:libint/common/Transport.C	1.5"
	...
	UIcontext ui_ctxt)
{
	...
	{
		if ((tt < TT_DE_notify) || (tt > TT_DE_response))
			interface_error(fname, __LINE__, 1);
	}
#endif // NOCHECKS
...
Transport::read_message(Message *m)
{
	...
	int bytes_read = (*readfunc)(input, m, sizeof(Message) - sizeof(void *));
	if (!bytes_read)	// other side exited without sending quit message
		(*exitfunc)(0);
	...
	if (m->msg_length)
	{
		...
		char	*ptr = (char *)m->msg_data;
		int	bytes_needed = m->msg_length;

		bytes_read = (*readfunc)(input, m->msg_data, bytes_needed);
		while (bytes_read < bytes_needed && (errno == EAGAIN || errno == 0))
		{
			bytes_needed -= bytes_read;
			ptr += bytes_read;
			bytes_read = (*readfunc)(input, ptr, bytes_needed);
		}

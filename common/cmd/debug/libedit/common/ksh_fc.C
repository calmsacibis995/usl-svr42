#ident	"@(#)debugger:libedit/common/ksh_fc.C	1.3"
...
static char		*sh_substitute(const char *, const char *, char *);
...
hist_subst(int fd, char *replace)
{
	...
	io_fclose(fd);
	*newstr++ =  0;
	if ((sp = sh_substitute(string, replace, newstr)) == 0)
	{
		printe(ERR_fc_found, E_ERROR);
		return 0;
	}
...
static char *
sh_substitute(const char *string, const char *old, char *newstr)
{

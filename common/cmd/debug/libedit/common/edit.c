...
#ident	"@(#)debugger:libedit/common/edit.c	1.1"
...
int ed_window()
{
	...
	register char *cp = getenv("COLUMNS");
	if(cp)
	{
		n = atoi(cp)-1;
		if(n > MAXWINDOW)
...
register int i;
{
	...
	if(i=='_')
		...
		macro[2] = 0;
	if (isalnum(i)&&(np=nam_search(macro,sh.alias_tree,N_NOSCOPE))&&(out=getenv(np)))
	{
#ifdef MULTIBYTE
...
#else
	i = debug_read(fildes, readin, nchar);
#endif
#ifdef MULTIBYTE
	lookahead = maxtry = i;
	i = 0;

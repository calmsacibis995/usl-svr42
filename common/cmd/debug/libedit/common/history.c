...
#ident	"@(#)debugger:libedit/common/history.c	1.1"
...
retry:
	...
	if((fd=debug_open(histname,O_APPEND|O_RDWR|O_CREAT,S_IREAD|S_IWRITE))>=0)
	{
...
register char *fname;
{
	...
	if((fd=debug_open(mktemp(fname),O_CREAT|O_RDWR,RW_ALL)) < 0)
...
{
	register int	fd;
	if((fd=debug_open(name,O_RDONLY))<0)
		sh_fail(name, e_open);
	return(fd);
}

...
#ifndef NOIDENT
#ident	"@(#)debugger:libol/common/xpm.c	1.2"
#endif
...
Pixmap XCreatePixmapFromData(display, d, colormap, width, height, depth, 
			     ncolors, chars_per_pixel, colors, pixels, bg_pixel)
     Display *display;
     Drawable d;
     Colormap colormap ;
...
#define RETURN(code, flag) \
  ...
  if (ncolors > (MAXPRINTABLE*MAXPRINTABLE)) 
    RETURN (fatal(display, OleNbadColors,OleTtooMany,
		  OleMbadColors_tooMany), 0);

  if ((chars_per_pixel < 1) || (chars_per_pixel > 2))
    RETURN (fatal(display, OleNbadPixels,OleTcharsPerPixel,
		  OleMbadPixels_charsPerPixel), 0);
  ...
  if (!colors) RETURN (fatal(display, OleNbadColors,OleTnotDefined,
			     OleMbadColors_notDefined), 0);
  ...
  switch (chars_per_pixel)
      ...
      for (i=0; i<2*ncolors ; i+=2)
	{
	  if (sscanf(colors[i],"%c", &c1) != 1)
	    RETURN
	     (fatal(display, OleNbadColormap,OleTentry1,
		    OleMbadColormap_entry1), 0);
	  if (index(printable,c1))
	    {
	      cmap[i/2].cixel.c1 = c1 ;
	      if(strcmp(colors[i+1],"$background")==0)
		  Tpixel[i/2] = bg_pixel;
	      else {
	        if (!XParseColor(display,colormap,colors[i+1],&xcolor))
		  RETURN
		    (fatal(display, OleNbadColors,OleTcolorDefSpec,
			 OleMbadColors_colorDefSpec), 0);
	        XAllocColor(display,colormap,&xcolor);
	        Tpixel[i/2] = xcolor.pixel ;
	      }
	    } else
	      RETURN (fatal(display, OleNbadCixels,OleTnotPrintable,
			    OleMbadCixels_notPrintable), 0);
	}
      break;
    case 2:
      for (i=0; i<2*ncolors ; i+=2)
	{
	  if (sscanf(colors[i],"%c%c", &c1,&c2) != 2)
	    RETURN
	    (fatal(display, OleNbadColormap,OleTentry2,
		   OleMbadColormap_entry2), 0);
  ...
  if (!pixels) 
	RETURN (fatal(display, OleNbadPixels,OleTnotDefined,
		      OleMbadPixels_notDefined), 1);
  j = 0 ;
  while (j < height)
    {  
      if (strlen(pixels[j]) != (chars_per_pixel*width))
	RETURN (fatal(display, OleNbadPixmap,OleTlineLength2,
		      OleMbadPixmap_lineLength2), 1);
      switch (chars_per_pixel)
	{
	case 1:
	  for (i=0; i< (width) ; i++)
		...
		RETURN (fatal(display, OleNbadColormap,OleTbadCixel1,
			      OleMbadColormap_badCixel1), 1);
	...
	case 2:
	  for (i=0; i< (2*width) ; i+=2)
		...
		RETURN (fatal(display, OleNbadColormap,OleTbadCixel2,
			      OleMbadColormap_badCixel2), 1);
...
#define RETURN(code, flag) \
  ...
  if ((sscanf(linebuf, "#define %[^_]%s %d", namebuf, type, width) != 3)
      || (strcmp(name,namebuf)) 
      || (strcmp("_width",type))) 
	RETURN (fatal(display, OleNbadDefine,OleTnameWidth,
		      OleMbadDefine_nameWidth), 0);

  getline(linebuf,fstream);
  if ((sscanf(linebuf, "#define %[^_]%s %d", namebuf, type, height) != 3)
      || (strcmp(name,namebuf)) 
      || (strcmp("_height",type))) 
	RETURN (fatal(display, OleNbadDefine,OleTnameHeight,
		      OleMbadDefine_nameHeight), 0);

  getline(linebuf,fstream);
  if ((sscanf(linebuf, "#define %[^_]%s %d", namebuf, type, &ncolors) != 3)
      || (strcmp(name,namebuf)) 
      || (strcmp("_ncolors",type))) 
	RETURN (fatal(display, OleNbadDefine,OleTnameNcolors,
		      OleMbadDefine_nameNcolors), 0);
  ...
  if ((sscanf(linebuf, "#define %[^_]%s %d", namebuf, type, &cpp) != 3)
	...
	RETURN (fatal(display, OleNbadDefine,OleTnameCharsPerPixel,
		      OleMbadDefine_nameCharsPerPixel), 0);
  ...
  for (n=0; n<2 ; n++) {
	...
	if (strncmp(linebuf, "} ;",3))
	  RETURN (fatal(display, OleNbadSyntax,OleTnameMono,
			OleMbadSyntax_nameMono), 0);
	getline(linebuf,fstream);
	continue;
      }
      mono = 1;
	  ...
	  || (strcmp(name,namebuf))
	  || (strcmp("_colors[]",type))) 
	RETURN (fatal(display, OleNbadSyntax,OleTnameColors,
		      OleMbadSyntax_nameColors), 0);
      if (mono) {
	for (i=0; i<=ncolors ; i++) {
	  getline(linebuf,fstream);
	}
	if (strncmp(linebuf, "} ;",3))
	  RETURN (fatal(display, OleNbadSyntax,OleTcloseBracket,
			OleMbadSyntax_closeBracket), 0);
	continue;
      }
    }
    cmap = (CmapEntry *) malloc(ncolors*sizeof(CmapEntry)) ;
	...
	RETURN (fatal(display, OleNbadColors,OleTcolorDefSpec,
		      OleMbadColors_colorDefSpec), 0);
      XAllocColor(display,colormap,&xcolor);
  ...
  if ((sscanf(linebuf, "static char * %[^_]%s = {",namebuf,type) != 2)
      || (strcmp(name,namebuf))
      || (strcmp("_pixels[]",type))) 
	RETURN (fatal(display, OleNbadSyntax,OleTnamePixels,
		      OleMbadSyntax_namePixels), 0);
  ...
  *pixmap = XCreatePixmap(display,d,*width,*height,depth);
  ...
  while((j < *height) && strncmp(linebuf, "} ;",3))
	...
	RETURN (fatal(display, OleNbadPixmap,OleTlineLength1,
		      OleMbadPixmap_lineLength1,
	...
	case 1:
	  for (i=1; i<=(*width) ; i++)
		...
		RETURN (fatal(display, OleNbadColormap,OleTbadCixel1,
			      OleMbadColormap_badCixel1, c1), 1);
	...
	case 2:
	  for (i=1; i<(2*(*width)) ; i+=2)
		...
		RETURN (fatal(display, OleNbadColormap,OleTbadCixel2,
			      OleMbadColormap_badCixel2, c1, c2), 1);
	      XDrawPoint(display,*pixmap,Gc,i/2,j) ;
	    }
	  break;
	}
  ...
  if (strncmp(linebuf, "} ;",3))
    RETURN (fatal(display, OleNbadSyntax,OleTcloseBracket,
		  OleMbadSyntax_closeBracket), 1);

  if (j != *height)
    RETURN (fatal(display, OleNbadPixmap,OleTtooFewLines,
		  OleMbadPixmap_tooFewLines, *height - j), 1);

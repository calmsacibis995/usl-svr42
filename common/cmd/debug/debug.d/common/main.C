#ident	"@(#)debugger:debug.d/common/main.C	1.13"
/*
 *	UNIX debugger
 */
...
#define OPTS		"Vrs:d:c:f:i:l:X:Y:?"  // getopt option string

#if FOLLOWER_PROC
#ifndef FOLLOWER
#define FOLLOWER	"/usr/ccs/lib/follow"
#endif
...
// typedef needed to get around cfront problem passing
...
main(int argc, char *argv[], char **envp)
{
	...
	if (loadfile && rflag)
	{
		printe(ERR_opt_combo, E_FATAL, prog_name, "-l", "-r");
	}
#if FOLLOWER_PROC
	// for now, set up follower path
	if (foll_path)
	{
		follow_path = new(char[strlen(foll_path)+sizeof("follow")+1]);
		sprintf((char *)follow_path, "%s/%s", foll_path, "follow");
	}
	else
		follow_path = FOLLOWER;
#endif
	...
	else if (optind < argc)
	{
		...
		{
			if (loadfile)
				printe(ERR_create_dashl, E_FATAL,
					prog_name);
			if (rflag || (get_ui_type() == ui_gui))
				(void)sprintf(cmd_line, "create -r -f %s %s",
					follow_opt, argv[optind++]);
			else
				(void)sprintf(cmd_line, "create -f %s %s",
					follow_opt, argv[optind++]);
		}
		else 
		{
			if (rflag)
				printe(ERR_grab_dashr, E_FATAL,
					prog_name);
			if (loadfile)
				(void)sprintf(cmd_line,"grab -f %s -l %s %s\n",
					follow_opt, loadfile, argv[optind++]);
			else
				(void)sprintf(cmd_line, "grab -f %s %s",
					follow_opt, argv[optind++]);
		}
		while (argc > optind)
			(void)sprintf(cmd_line,"%s %s", cmd_line,
				argv[optind++]);

#ident	"@(#)debugger:libsymbol/common/Elfbuild.C	1.6"
#include	"Interface.h"
#include	"Elfbuild.h"
#include	"Locdesc.h"
#include	"Syminfo.h"
...
Elfbuild::first_symbol()
{
	...
	{
		long	symoff, stroff;
		Iaddr	symaddr, straddr;
		// C++ 2.0 workaround
		void* tempsymptr = symptr;
		void* tempstrptr = strptr;
		if ((object->getsect(s_symtab, tempsymptr, symaddr, hisym, symoff) == 0) ||
		  (object->getsect(s_strtab, tempstrptr, straddr, histr, stroff) == 0))
			return 0;
		symptr = (char*)tempsymptr;
		strptr = (char*)tempstrptr;
...
Attribute *
Elfbuild::make_record( long offset, int )
{
	Attribute 	*attribute;
	Syminfo		syminfo;
	Locdesc		locdesc;
	...
	if ( syminfo.type == st_object )
	{
		if (syminfo.bind == sb_global)
			protorec.add_attr( an_tag, af_tag, t_global_variable );
		else
		{
	...
	else if ( syminfo.type == st_func )
	{
		...
		if (syminfo.bind == sb_global)
		{
			if (!syminfo.resolved && syminfo.lo != 0)
				...
				plt = 1;
			protorec.add_attr( an_tag, af_tag, t_global_sub );
		}
		else
		{

...
#ident	"@(#)nserve:nsdb.c	1.9.12.3"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/nserve/nsdb.c,v 1.1 91/02/28 18:42:13 ccs Exp $"
#include <stdio.h>
#include <string.h>
#include <ctype.h>
#include "nslog.h"
...
int	size;
{
	...
	strcpy(dbuf,dname);
	p=namepart(dbuf);
	if (((size == SZ_RES)?v_resname(p):v_uname(p)) != 0
		|| (int)strlen(p) > size)
		return(FALSE);
	...
	for (p=rtoken(dbuf); p != NULL && *p != '\0'; p=rtoken(NULL)) {
		...
		if (v_dname(p) != 0)
			sflag = TRUE;
		else if ((int)strlen(p) > SZ_DELEMENT)
			sflag = TRUE;
	}
	return(TRUE);
}

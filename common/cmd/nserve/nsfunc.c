...
#ident	"@(#)nserve:nsfunc.c	1.14.12.4"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/nserve/nsfunc.c,v 1.1 91/02/28 18:42:22 ccs Exp $"
#include <stdio.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
int		pd;
{
	...
	case NS_QUERY:
		...
		if (rlist) {
			...
			nhp->h_ancnt = copyrlist(&(nreq->rq_an),rlist, 0);
			nhp->h_rcode = R_NOERR;
	...
	case NS_FINDP:
		...
		if (!Primary || !domauth(qp->q_name)) {
			...
			recl[0] = recp;
			recl[1] = NULL;
			nhp->h_ancnt = copyrlist(&(nreq->rq_an),recl,0);
			nhp->h_rcode = R_NOERR;
			nhp->h_arcnt = getars(nreq->rq_an,&(nreq->rq_ar),
				nhp->h_ancnt);
		}
	...
	case NS_INIT:
		...
		*p_key = '\0';
		 ...
		 *   it...
		 */
		(void)copyrlist(srp,rlist,MAXREC);
...
int nelem;
{
	...
	if(	*target != NULL	/* caller is providing space */
	&&	nelem > 0	/* bounds checking requested */
	&&	i+1 > nelem	/* the space is insufficient */
	){
		/* bail out */
		(*target)[0] = NULL;
		return 0;
...
struct address	*addr;
{
	...
	if ((rlist=findrr(rec->rr_owner,A)) != NULL) {
		if (addr && (addrstr = aatos((char *)NULL, addr, HEX)) != NULL) {
			if (strcmp(rlist[0]->rr_a, addrstr) == 0) {
				freelist(rlist);
				return(ret);
			} else {
				ret = remrr(rec->rr_owner,rlist[0]);
	   			freelist(rlist);
			}
		}
	}
	...
	if (addr && (arec->rr_a=aatos((char *)NULL,addr,HEX)) != NULL) {
		arec->rr_type = A;

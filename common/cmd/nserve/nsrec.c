...
#ident	"@(#)nserve:nsrec.c	1.21.13.5"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/nserve/nsrec.c,v 1.1 91/02/28 18:42:30 ccs Exp $"
#include <stdio.h>
#include <string.h>
#include <signal.h>
...
#define NLS_Q		"/usr/sbin/nlsadmin -q"
#define NLS_ADDR 	"/usr/sbin/nlsadmin -l -"
...
char	*dom;
{
	...
	if ((rlist = findrr(abuf,NSTYPE)) == NULL ||
	    (count = copyrlist(&srp,rlist, MAXREC)) == 0 ||
...
getnsinfo()
{
	...
	for (i=0; i < Mylastdom; i++) {
		...
		count = copyrlist(&srp,rlist, MAXREC);

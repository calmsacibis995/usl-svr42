...
#ident	"@(#)oamuser:user/uidage.c	1.1.12.3"
#ident  "$Header: uidage.c 2.0 91/07/13 $"
...
static	const	char
	*miss	= ":0:Unexpected failure.  Uid aging file(s) missing\n",
	*nochg	= ":0:Unexpected failure.  Uid aging files unchanged\n",
	*badent	= ":0:bad entry found in /etc/security/ia/ageduid.\n";
	...
	int	i_uidage;
{
	...
	if (error >= 1) {
		(void) pfmt(stderr, MM_ERROR, badent);
	}

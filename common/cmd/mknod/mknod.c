...
#ident	"@(#)mknod:mknod.c	1.9.1.11"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/mknod/mknod.c,v 1.1 91/02/28 18:33:28 ccs Exp $"
	...
	if(argc == 3) {
		...
		case 'm':
			...
			if (sysfs(GETFSIND, "xnamfs") == -1) {
				fprintf(stderr,"mknod: invalid option - Xenix package not installed\n");
				exit(2);
			}
			mode = S_IFNAM;
			arg = S_INSHD;
...
usage()
{
	...
	fprintf(stderr,"usage: mknod name p\n");
	exit(2);
}

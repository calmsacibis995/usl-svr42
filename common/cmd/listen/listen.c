...
#ident	"@(#)listen:listen.c	1.19.21.4"
#ident "$Header: listen.c 1.2 91/06/13 $"
...
char **adrp;
{
	...
	/*
	 * Make sure the transport provider allows reuse of the same
	 * address on which existing connections came in.
	 */
	ret = netdir_options(Netconf, ND_SET_REUSEADDR, fd, NULL);
	DEBUG((9, "netdir_options returns %d with t_errno %d", ret, t_errno));

...
#ident	"@(#)pcintf:pkg_rlock/rlock_init.c	1.1.1.3"
#include <sccs.h>

SCCSID(@(#)rlock_init.c	3.4);	/* 8/21/91 21:42:42 */
...
typedef struct {
	...
	PAD			/* structure padding (see internal.h) */
} basicDataT;
 ...
 *
 *	output:	(int) - RL_SUCCESS (0) or RL_FAIL
 *
...
{	r0 openFileT FAR *openFileP;
	...
	lockSet = lsNew(_rlCfgData.cfgLsetKey, LS_ALL_WRITE | LS_ALL_READ,
			NLOCKS, NLOCKS + (int)_rlCfgData.cfgRecLocks);
	...
	if ((lockCount == RL_FAIL) || !_rlSetLocks(ALL_LOCK, UNUSED_LOCK))
	{
...
{	int shmDesc, saveErrno;
	...
	if ((_rlInitLocks(_rlCfgData.cfgLsetKey) == RL_FAIL) ||
	...
	else if (shmctl(shmDesc, IPC_RMID, NIL_SHMID_DS) == RL_FAIL)
	{
		saveErrno = errno;
		shmError = TRUE;
	}
	...
	_rlUnlock();
	lsError = lsRmv(_rlCfgData.cfgLsetKey) != RL_SUCCESS;
...
{	int shmDesc, getFlags, saveErrno;
	...
	shmDesc = shmget(_rlCfgData.cfgShmKey,
				(int)_rlockShm.segSize, getFlags);
	if (shmDesc == RL_FAIL)
	{
		switch (errno)
		{
	...
	if (_rlockShm.segmentP == (char FAR *)RL_FAIL)
	{
		switch (errno)	
		{
	...
	if (!create)
	{
		if (shmctl(shmDesc, IPC_STAT, &sysShmInfo) == RL_FAIL)
		{
			saveErrno = errno;
...
#ifdef PAD_SHMSTRUCTS
	rlockOffset	= (off_t)(((ulong)rlockOffset + (ulong)3) & (ulong)~3);
#endif

...
#ident	"@(#)pcintf:pkg_rlock/multi_init.c	1.1.1.2"
#include <sccs.h>

SCCSID(@(#)multi_init.c	3.3);  /* 8/21/91 21:41:23 */
...
{	r0 oldOpenFile FAR *ofFree;
	...
	if ((lockCount == RL_FAIL) ||
		_rlSetLocks(ALL_LOCK, UNUSED_LOCK) == RL_FAIL)
	{
		(void)lsRmv(LS_KEY);
		rlockErr = RLERR_LOCKSET;
		return RL_FAIL;
...
{	int lockCount;
	...
	return doAttach(FALSE, lockCount) ? RL_SUCCESS : RL_FAIL;
}
...
{	int ofDesc, fhDesc, ltDesc, ofErr, fhErr, ltErr, saveErrno;
	...
	if ((_rlInitLocks(LS_KEY) == RL_FAIL) ||
	    (_rlSetLocks(ALL_LOCK, UNUSED_LOCK) == RL_FAIL))
	{
		rlockErr = RLERR_LOCKSET;
		return RL_FAIL;
	...
	if ((ofDesc == RL_FAIL) || (fhDesc == RL_FAIL) || (ltDesc == RL_FAIL))
	{
		saveErrno = errno;
		shmError = TRUE;
	}
	...
	if (!shmError &&
	    ((ofErr == RL_FAIL) || (fhErr == RL_FAIL) || (ltErr == RL_FAIL)))
	{
		saveErrno = errno;
		shmError = TRUE;
	}
	...
	_rlUnlock();
	lsError = lsRmv(LS_KEY) != RL_SUCCESS;
...
{	int ofDesc, fhDesc, ltDesc, getFlags, saveErrno;
	...
	if ((ofShmP == (ofShmT FAR *)RL_FAIL) ||
	    (fhShmP == (fhShmT FAR *)RL_FAIL) ||
	    (ltShmP == (ltShmT FAR *)RL_FAIL))
	{
		switch (errno)
		{

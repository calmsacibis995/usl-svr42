...
#ident	"@(#)pcintf:pkg_lockset/lsetuse.c	1.1.1.2"
#include <sccs.h>

SCCSID(@(#)lsetuse.c	3.2);	/* 8/20/91 14:38:31 */
...
key_t			semKey;		/* Semaphore set key (name) */
	...
	if ((semDesc = semget(semKey, 0, UP_ORW)) == LS_FAIL)
		if ((semDesc = semget(semKey, 0, UP_WRW)) == LS_FAIL)
			if ((semDesc = semget(semKey, 0, UP_GRW)) == LS_FAIL) {
				lsetErr = LSERR_SYSTEM;
				return INVALID_LOCKSET;
			}

	/* Fetch semaphore set description information */
	if (semctl(semDesc, 0, IPC_STAT, &semData) == LS_FAIL)
	{
		lsetErr = LSERR_SYSTEM;
		return INVALID_LOCKSET;
	}

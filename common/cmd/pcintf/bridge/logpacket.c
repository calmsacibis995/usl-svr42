...
#ident	"@(#)pcintf:bridge/logpacket.c	1.1.1.3"
#include	"sccs.h"
SCCSID(@(#)logpacket.c	6.7	LCC);	/* Modified: 10:55:17 12/3/91 */
...
long                    how;
{
	...
	|| (dbgCheck(DBG_XPLOG) == 0)) {
	if (how & LOGNETHEADER)
	{
#ifdef  ETHNETPCI
#ifndef BERKELEY42
	    fprintf(logFile, ethHdrFmt, nAddrFmt((unsigned char *)pk->net.dst),
		nAddrFmt((unsigned char *)pk->net.src), pk->net.type[0],
		pk->net.type[1]);
#else   /* BERKELEY42 */
	...
	{
#define bhdr    pk->hdr
	    fprintf(logFile, hdr1Fmt, bhdr.versNum,bhdr.res,bhdr.req,bhdr.stat,
...
#undef  bhdr
	...
	if ((how & LOGASCII) && (pk->hdr.t_cnt > 0))
	{
	    ...
	    /* do not log find output packets */
/*
 *	370 compiler seems a tad bogus so we have this bizarre illegal
 *	statement here for LOCUS and IX370 defined.
 */
	    if (rq != EST_BRIDGE && (dbgCheck(DBG_YPLOG) == 0 || (
		rq != READ_SEQ && rq != READ_RAN && rq != WRITE_SEQ &&
		rq != WRITE_RAN && (kind == PLOG_RCV || (rq != FIND_FIRST &&
		rq != NEXT_FIND))))) {
		fputs("ascii: ", logFile);
    ...
    if (kind != PLOG_RCV)
	fprintf(logFile,
	    "\nSending >>\t\t\t>>>> %d %s Response %d %s <<<<\n",
	    rq, reqname(rq), pk->hdr.res, resname(pk->hdr.res));
    fflush(logFile);
}
    ...
    int rq;
{
    ...
    case GET_SITE_ATTR:	rr = "GET_SITE_ATTR";	break;
    case DOS_NLS_INFO:	rr = "DOS_NLS_INFO";	break;
    case S_LOGIN:	rr = "S_LOGIN";		break;
    case K_LOGIN:	rr = "K_LOGIN";		break;
    default: rr = "???"; break;
    }
return rr;
}

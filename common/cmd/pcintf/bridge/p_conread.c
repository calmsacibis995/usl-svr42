...
#ident	"@(#)pcintf:bridge/p_conread.c	1.1.1.2"
#include	"sccs.h"
SCCSID(@(#)p_conread.c	6.3	LCC);	/* Modified: 23:22:17 7/12/91 */
...
/* --- local routines --- */
LOCAL int	lineread	PROTO((int, int, struct output *));

/* --- globals --- */
extern  char err_class, err_action, err_locus;


void pci_conread(vdescriptor, dosfunc, flsh, bytes, addr, request)
    ...
    int		request;		/* Contains DOS request number */
{
    ...
    {
	addr->hdr.res = (unsigned char)((adesc == NO_FDESC)
	...
	{
	    do
		addr->hdr.b_cnt = (unsigned short)read(adesc, addr->text, 1);
	    while ((int)addr->hdr.b_cnt == -1 && errno == EINTR);
	    if ((int)addr->hdr.b_cnt < 0)
	    {
		err_handler(&addr->hdr.res, request, NULL);
		return;
	    }
...
struct  output  *addr;  /* Pointer to response buffer */
{
    ...
    *bufp = 0x0d;
    addr->hdr.b_cnt = (unsigned short)numb;
    addr->hdr.t_cnt = (unsigned short)numb + 1;
    return 0;
}

...
#ident	"@(#)iconv:tree.c	1.1.6.2"
#ident  "$Header: tree.c 1.3 91/07/02 $"
...
static const char badnode[] = ":97:Internal error - unknown node type(%d) %d\n";
	...
	register unsigned char *result;
{
	...
	if (! *input) {
		pfmt(stderr, MM_ERROR, ":98:Internal error in 'tree' (no input).\n");
		return(E_ERROR);
	}
 ...
 */
	...
	if (p->n_val != *input) {
		pfmt(stderr, MM_ERROR, ":99:Internal error in 'tree' (n_val != *input)\n");
		return(E_ERROR);
	}
	...
	if (p->n_flag == N_RESULT) {
		pfmt(stderr, MM_ERROR, badnode, 3, p->n_flag);
		return(E_ERROR);
	}
	if (p->n_what.n_child) {	/* has a child??? */
		...
		p->n_val = *input++;
		if (! *input) {
			pfmt(stderr, MM_ERROR, ":100:Internal error in tree walk (unexpected end of input).\n");
			return(E_ERROR);
		}
	}
 ...
 */
		case N_EMPTY:
		case N_RESULT:	break;
		default:	pfmt(stderr, MM_ERROR, badnode, 1, q->n_flag);
				exit(1);
				break;
		}
		q = q->n_next;
	...
	while (q) {
		...
		case N_RESULT:	q->n_node = 0;
				break;
		default:	pfmt(stderr, MM_ERROR, badnode, 2, q->n_flag);
				exit(1);
		}
		q = q->n_next;
	}

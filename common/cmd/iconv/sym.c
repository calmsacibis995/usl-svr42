...
#ident	"@(#)iconv:sym.c	1.1.6.1"
#ident  "$Header: sym.c 1.2 91/07/01 $"
...
const char nomem[] = ":12:Out of memory: %s\n";
	...
	if (lexfp = fopen(PREDEF, "r")) {	/* see if any pre-defs */
		while ((left = yylex()) != ACCEPT) {
			...
			if ((right = yylex()) == ACCEPT) {
				pfmt(stderr, MM_WARNING, ":95:Pre-definition entry mismatch; ignoring \"%s\"\n", lstr);
				return;
			}
	...
	char *name;
{
	...
	for (ptr = s_head->s_next; ptr; ptr = ptr->s_next) {
		if (! ptr->s_name)
			pfmt(stderr, MM_ERROR, ":96:Internal error in s_find.\n");
		else {
			/* return if match */

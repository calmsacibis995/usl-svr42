...
%{
#ident	"@(#)egrep:egrep.y	1.10.2.7"
#ident "$Header: egrep.y 1.2 91/08/12 $"
%}
...
#ifndef SS2             /* may be defined already in sys/euc.h */
#define SS2 0216
#define SS3 0217
#endif
#define MASK EUCMASK    /* mask bits for 32-bits code */
#define MASK1 P11
#define MASK2 P01
#define MASK3 P10
...
int range_flag = 0, opt_flag = 0, opt_sign;
int min_range, max_range, ptr_range, token_range;
wchar_t last_c;
...
yylex() {
	...
	if (range_flag == 1) {
		if (ptr_range == max_range)	/* we've done it already */
			range_flag = 0;
		else {
			if (ptr_range < min_range) { 	/* process mandatory */
				ptr_range++;
				c = token_range;
				goto defchar;
			} else {			/* process optionals */
				if (opt_flag == 0) {	/* first : token */
					opt_flag = 1;
					c = token_range;
					goto defchar;
				} else {	/* next time : optional sign */
					ptr_range++;
					opt_flag = 0;
					return opt_sign;
				}
		...
		case '\\':
			if ((c = nextch()) == '\0') synerror();
				...
				if(nextch() != '}')
					synerror();
				/*
				 * We've recognised a range.
				 * The last token found (until now in 'last_c')
				 * has to be repeated :
				 * - at least 'min_range' times
				 *   (min_range mandatory occurences of it) and
				 * - at most 'max_range' times
				 *   (max_range - min_range optional occurences)
				 * We will express this while returning :
				 * - 'min_range' - 1 times the same token (it
				 *   has already been done once, before reading
				 *   \{..\})
				 * - if 'max_range' was given :
				 *   + 'max_range' - min_range' times : first
				 *     the token, the next time a question mark.
				 *   if 'max_range' wasn't given (infinite
				 *   range) :
				 *   + once first the token, the next time a
				 *     star.
				 */
				range_flag = 1;
				token_range = last_c;
				if (min_range > 1) {	/* process mandatory */
					ptr_range = 2;
					opt_flag = 0;
					c = token_range;
					goto defchar;
...
char *file;
{
	...
	lnum = 1;
	tln = 0;
	if((count = bread(fileno(f), buf, BUFSIZ)) <= 0) {
		fclose(f);
	...
	for(;;) {
		if((ptrend = memchr(ptr, '\n', buf + count - ptr)) == NULL) {
			...
			if((count1 = bread(fileno(f), p, BUFSIZ)) > 0) {
				count += count1;
				blkno += count1;
				continue;
			}
...
/* BUG FIX : PM # A0070471 - function bread() added to mask read()

    bread(fildes, buf, n) loops until n bytes have been read or the end of
			  file has been reached. This is necessary because
...
mdotenter()
{
	...
	x1 = node(OR, x1, x2);
	if(eucw2) {
		x2 = enter((unsigned char)'\216');
		for(i = 1; i <= eucw2; i++) 
			x2 = node(CAT, x2, enter(MDOT));
		x1 = node(OR, x1, x2);
	}
	if(eucw3) {
		x2 = enter((unsigned char)'\217');
		for(i = 1; i <= eucw3; i++) 
			x2 = node(CAT, x2, enter(MDOT));
		x1 = node(OR, x1, x2);
	}

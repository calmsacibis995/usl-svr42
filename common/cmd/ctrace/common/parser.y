...
%{
#ident	"@(#)ctrace:common/parser.y	1.3"
...
enum	bool asm_flag = no;		/* asm function indicator */
enum	bool stmt_part = no;
enum	bool para_type = no;
...
static  enum    bool save_tmp = yes; /* keep save_header temporarily */
...
declaration:
	...
	|	TYPEDEF opt_decl_specifiers {
			/* put flag on to look for names */
			typename = yes;
		}
		opt_init_decl_list decl_semi {
			typename = no;
		}
	|	error semi_or_curly {
			err_flag = yes;
...
enumerator:
		id_or_function 
	|	id_or_function L_EQ constant_expression 
	;
...
type_specifier_list:
		type_or_class
	|	type_specifier_list type_or_class
	;
...
parameter_list:
		parameter_declaration { para_type = yes; }
...
oldasm:		L_ASM L_LP L_STRING L_RP L_SEMI
	|	L_ASM {
			asm_flag = yes;
			save_tmp = save_header;
			save_header = no;
			}
		opt_decl_specifiers declarator opt_asm_body
			{
			asm_flag = no;
			puttext(yyleng);
			save_header = save_tmp;
			}
	;

opt_asm_body:
		L_LC opt_id_list L_RC
	|	L_SEMI
	;
...
postfix_e:
	...
	|	s_postfix_e L_STROP id_or_function
			{

...
#ident	"@(#)truss:common/cmd/truss/expound.c	1.10.6.4"
#ident  "$Header: expound.c 1.2 91/06/27 $"
#include <stdio.h>
#include <sys/types.h>
...
static 	void	show_modadm();
static	void	show_modstatus();
...
show_modadm(process_t *Pr)
{
	...
	if(sys_args[1] != MOD_C_MREG)
		return;
	if(Pread(Pr,sys_args[2],(char *) &mreg, sizeof(struct mod_mreg)) !=
	...
	switch(type) {
		...
		case MOD_TY_FS:
			printf("modname=%s\tfile system name=%s\n",
				mreg.md_modname,
				fetchstring((long) mreg.md_typedata,FSTYPSZ));
			break;
		case MOD_TY_STR:
			printf("modname=%s\tstream name=%s\n",
				mreg.md_modname,fetchstring((long) mreg.md_typedata,FMNAMESZ));
			break;
		case MOD_TY_MISC:
		case MOD_TY_NONE:
		default:
...
show_modstatus(process_t *Pr, long status)
{
	...
	if(Pread(Pr,status,(char *) &mstat, sizeof(struct modstatus)) !=

...
#ident	"@(#)truss:common/cmd/truss/main.c	1.5.7.4"
#ident  "$Header: main.c 1.3 91/07/09 $"
#include <stdio.h>
#include <fcntl.h>
#include <ctype.h>
#include <string.h>
...
fetchval(addr)
{
	process_t *Pr = &Proc;
	long retval;
	Pread(Pr,addr,(char *) &retval,sizeof(long));
	return(retval);
}
char *
...
{
	/*
	 * This is based on the fact that, e.g., "/proc/214" and
	 * "/proc/0214" are the same file, namely process 214, and
	 * are not linked to each other.
	 */

	struct stat stat1;	/* dir/pid  */
	struct stat stat2;	/* dir/pid with "0" prepended to pid */
	char path[200];
	register char * p;
	...
	/* append "pid" to the directory path and stat() the file */
	p = path + strlen(path);
	*p++ = '/';
	sprintf(p, "%d", getpid());
	...
	/* append "0pid" to the directory path and stat() the file */
	*p++ = '0';
	sprintf(p, "%d", getpid());

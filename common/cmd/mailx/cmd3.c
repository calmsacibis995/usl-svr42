...
#ident	"@(#)mailx:cmd3.c	1.13.2.8"
#ident "@(#)cmd3.c	1.17 'attmail mail(1) command'"
...
static char	*getfilename ARGS((char *name, int *aedit));
	...
	int *aedit;
{
	...
	*aedit = 1;
	...
	case '&':
		...
		strcpy(origprevfile, origname);
		if (fname[1] == 0) {
			char *tmpmbox = Getf("MBOX");
			strcpy(origname, tmpmbox);
			*aedit = !ismbox(tmpmbox);
			return(tmpmbox);
		}
		/* Fall into . . . */

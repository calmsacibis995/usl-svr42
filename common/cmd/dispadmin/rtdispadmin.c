...
#ident	"@(#)dispadmin:common/cmd/dispadmin/rtdispadmin.c	1.5.7.3"
#ident  "$Header: rtdispadmin.c 1.3 91/08/13 $"

/***************************************************************************
 * Command: RTdispadmin
 *
 * Inheritable Privileges: P_RTIME,P_SYSOPS
...
/*
 * Procedure: get_rtdptbl
 *
 * Notes: Retrieve the current rt_dptbl from memory, convert the time
...
/*
 * Procedure: set_rtdptbl
 *
 * Notes: Read the rt_dptbl values from infile, convert the time
...
line %d of input file\n", basenm, linenum);
	...
	for (i = 0; i < rtadmin.rt_ndpents; i++) {
		...
		if ( strcmp(tokp,RT_TQINF_STRING) == MATCH) {
			(void)printf("Strings match on line %d\n",i);
			rt_dptbl[i].rt_quantum=RT_TQINF;
		} else {
			if (res != HZ) {
				hrtime.hrt_secs = 0;
	...
	valid range; line %d of input,\ntable not overwritten\n", basenm, linenum);
		...
		if (rt_dptbl[i].rt_quantum <= 0 && rt_dptbl[i].rt_quantum != RT_TQINF)

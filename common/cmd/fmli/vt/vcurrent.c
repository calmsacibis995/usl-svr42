 ...
 *	All Rights Reserved
 */
#ident	"@(#)fmli:vt/vcurrent.c	1.11.3.3"
...
vt_id	VT_curid;
/* an array containing the definition of every VT in the FMLI session */
struct vt	*VT_array;

extern bool CheckingWorld;      /* abs W2, s18 */
 ...
 */
	if (VT_curid >= 0) {
		...
		curvt->flags |= VT_TDIRTY;
	        curvt->flags |= VT_PADIRTY;  /* update scrollbar. abs s17 */
	...
	if (CheckingWorld != TRUE)   /* don't restack frames during
				      * a checkworld.
				      * (not the same as == FALSE) abs W2,s18 */
	{
...
/* debug stuff
	...
	fprintf( stderr, gettxt(":0","\t\t\tAfter change\n") );
	fprintf( stderr, "VT_front = %d\n", VT_front );
	fprintf( stderr, "VT_back =  %d\n", VT_back );
	fprintf( stderr, "VT_curid = %d\n\n", VT_curid );

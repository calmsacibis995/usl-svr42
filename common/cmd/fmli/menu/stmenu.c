...
#ident	"@(#)fmli:menu/stmenu.c	1.16.3.5"
...
/* #define myisprint(C)	((C) >= ' ' && (C) <= '~')  8 bit dirty.  abs s17 */

#define myisprint(C) ((C) < 0400 && isprint((C)))   /* abs s17 */
...
register token	t;
{
	...
	if (curstring) {
		...
		if (myisprint((int) t) && t != ' ') {
			sprintf(badmatch, gettxt(":42","Can't match item starting with \"%.*s%c\""),
				 m->hcols, curstring, (char) t);
			(void)mess_err(badmatch); 	/* abs s15 */
			return(TOK_BADCHAR);
		}
	}
				...
				while (men.flags & MENU_INACT) {
					if ((newindex = m->index + row) < m->number) {
						...
						men = (*m->disp)(m->index, m->arg);
		    				...
		    				sprintf(badmatch, gettxt(":43","Can't match inactive menu item.") );
		    				(void)mess_err(badmatch); /* abs s15 */
	...
	case TOK_SHOME:		/* move to last item on cur. page */
		{
		    ...
		    menu_ctl(MNU_curid, CTGETPARMS, &topline, &index); /* abs */
                    if ( m->ncols > 1 )
                        num_row = m->number; /* multi-column menu -ck */
                    else
                        num_row = row;
                    menu_index(m, num_row + topline - 1, MENU_ALL);
		    t = TOK_NEXT;
		    men = (*m->disp)(m->index, m->arg);
		    while (men.flags & MENU_INACT) {
			menu_index(m, m->index - 1, MENU_ALL);
	...
	default:
		if (myisprint((int) t)) {
			if ((newindex = menu_match(m, nil, 0, t)) != FAIL) {
				...
				sprintf(badmatch, gettxt(":44","Can't match item starting with \"%c\""), (char) t);
				(void)mess_err(badmatch); 	/* abs s15 */
				return(TOK_BADCHAR);
			}
		}

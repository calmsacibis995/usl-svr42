...
#ident	"@(#)fmli:menu/mdefault.c	1.25.3.3"
...
char	*arg;
{
     ...
     */
    if (vid == VT_UNDEFINED) {
	(void)mess_err( gettxt(":1","Frame can not be displayed, it may be too large for the screen") ); /* abs s15 */
	return((menu_id) FAIL);
    }
...
char	*arg;
{
	...
	vt_replace(MNU_array[mid].vid, MNU_array[newmid].vid); /* abs W2,s18 */
	menu_close(mid);

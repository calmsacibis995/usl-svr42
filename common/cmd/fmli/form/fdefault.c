 ...
 *	All Rights Reserved
 */
#ident	"@(#)fmli:form/fdefault.c	1.14.3.3"
...

{
	...
	if (vid == VT_UNDEFINED) {
                i18n_msg=gettxt(":1","Frame can not be displayed, it may be too large for the screen");
		(void)mess_err(i18n_msg);   /* abs s14, */
		return((form_id) FAIL);
	}
...
char	*arg;
{
	...
	if ((newvid = vt_create(s,
		/* abs s16 */	((flags & ~(unsigned)VT_COSTS) | VT_COVERCUR),
				VT_UNDEFINED,
				VT_UNDEFINED,
				formrows,
				formcols)) == VT_UNDEFINED)
	{
	...
	if (newvid != VT_UNDEFINED) {
	        vt_replace(formvid, newvid);   /* abs W2, s18 */
		vt_close(formvid);
...

{
	...
	*****/
	...
	if (rows < 4 /* f->rows */ || cols < 5 /* f->cols */) {
                i18n_msg=gettxt(":2","Too small, try again");
		(void)mess_err(i18n_msg);	/* abs s15 */
		return FAIL;
	}

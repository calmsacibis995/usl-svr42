...
#include	<sys/wait.h>	/* abs s14 */
#include	<stdlib.h>	/* abs k18 */
#endif				/* abs s14 */
#ifndef DO_NOT_DELAY
#include 	<sys/time.h>	/* abs k18 */
#endif
#include	<errno.h>	/* abs s14 */
#include	<ctype.h>	/* abs s14 */
#include 	<pwd.h>		/* abs s14 */
#include	"sizes.h"	/* abs s14 */
#include	"wish.h"
#include	"token.h"
#include	"fm_mn_par.h"
#include	"slk.h"
...
/***********************************************************************
* the following 5 character pointers are for comparison with label
* strings and will be translated in init_I18n_slk()
*
* The variable i18n_local_strings is set to True when the application
* uses localized strings. (I set to True in eval_nls_msg().
*
************************************************************************
*/

#define SLK_NUM_CHAR	8

char *prevpage   = "PREVPAGE";
char *prevpageid = ":178";
char *nextpage   = "NEXTPAGE";
char *nextpageid = ":179";
char *prevfrm    = "PREV-FRM";
char *prevfrmid  = ":173";
char *nextfrm    = "NEXT-FRM";
char *nextfrmid  = ":174";
char *enter      = "ENTER";
char *enterid    = ":177";
...
/***********************************************************************
* added to all the initialized slk structures the msgid field.
************************************************************************
*/

/* dmd TCB */
static struct slk Moreslk = {
	"CHG-KEYS",	":168",	TOK_TOGSLK,	NULL,	NULL,	NULL
};

/* dmd TCB */
static struct slk Markslk = {
	"MARK",		":169",	TOK_MARK,	NULL,	NULL,	NULL
};

/* dmd TCB */
static struct slk Blankslk = {
	"",		NULL,	TOK_NOP,	NULL,	NULL,	NULL
};

/* the default set of SLKs for forms */
struct slk Formslk[] = {
	{"HELP",	":170",	TOK_HELP,	NULL,	NULL,	NULL},
	{"CHOICES",	":171",	TOK_OPTIONS,	NULL,	NULL,	NULL},
	{"SAVE",	":172",	TOK_DONE,	NULL,	NULL,	NULL},
	{"PREV-FRM",	":173",	TOK_PREV_WDW,	NULL,	NULL,	NULL},
	{"NEXT-FRM",	":174",	TOK_NEXT_WDW,	NULL,	NULL,	NULL},
	{"CANCEL",	":175",	TOK_CLOSE,	NULL,	NULL,	NULL},
	{"CMD-MENU",	":176",	TOK_CMD,	NULL,	NULL,	NULL},
	{"",		NULL,	TOK_NOP,	NULL,	NULL,	NULL},
	{NULL, 		NULL,	TOK_NOP, 	NULL,	NULL,	NULL}
};

#define MARK 1

/* the default set of SLKs for menus */
struct slk Menuslk[] = {
	{"HELP",	":170",	TOK_HELP,	NULL,	NULL,	NULL},
	{"",		NULL,	TOK_NOP,	NULL,	NULL,	NULL},/* MARK for mult. selections  */
	{"ENTER",	":177",	TOK_RETURN,	NULL,	NULL,	NULL},
	{"PREV-FRM",	":173",	TOK_PREV_WDW,	NULL,	NULL,	NULL},
	{"NEXT-FRM",	":174",	TOK_NEXT_WDW,	NULL,	NULL,	NULL},
	{"CANCEL",	":175",	TOK_CLOSE,	NULL,	NULL,	NULL},
	{"CMD-MENU",	":176",	TOK_CMD,	NULL,	NULL,	NULL},
...
/* the default set of SLKs for Text objects */
struct slk Textslk[] = {
	{"HELP",	":170",	TOK_HELP,	NULL,	NULL,	NULL},
	{"PREVPAGE",	":178",	TOK_PPAGE,	NULL,	NULL,	NULL},
	{"NEXTPAGE",	":179",	TOK_NPAGE,	NULL,	NULL,	NULL},
	{"PREV-FRM",	":173",	TOK_PREV_WDW,	NULL,	NULL,	NULL},
	{"NEXT-FRM",	":174",	TOK_NEXT_WDW,	NULL,	NULL,	NULL},
	{"CANCEL",	":175",	TOK_CLOSE,	NULL,	NULL,	NULL},
	{"CMD-MENU",	":176",	TOK_CMD,	NULL,	NULL,	NULL},
...
struct slk Echslk[] = {		/* Enter/Cancel/Help slks */
	{"HELP",	":170",	TOK_HELP,	NULL,	NULL,	NULL},
	{"",		NULL,	TOK_BADCHAR,	NULL,	NULL,	NULL},
	{"",		NULL,	TOK_BADCHAR,	NULL,	NULL,	NULL},
	{"",		NULL,	TOK_BADCHAR,	NULL,	NULL,	NULL},
	{"",		NULL,	TOK_BADCHAR,	NULL,	NULL,	NULL},
	{"CANCEL",	":175",	TOK_CANCEL,	NULL,	NULL,	NULL},
	{"",		NULL,	TOK_BADCHAR,	NULL,	NULL,	NULL},
	{"",		NULL,	TOK_BADCHAR,	NULL,	NULL,	NULL},
	{"",		NULL,	TOK_BADCHAR,	NULL,	NULL,	NULL},
	{"",		NULL,	TOK_BADCHAR,	NULL,	NULL,	NULL},
...
static struct slk Vmslk[] = {
	...
	{"",		NULL,	TOK_NOP,	NULL,	NULL,	NULL},
	{"HELP",	":170",	TOK_HELP,	NULL,	NULL,	NULL},
	{"COPY",	":180",	TOK_COPY,	NULL,	NULL,	NULL},
	{"MOVE",	":181",	TOK_MOVE,	NULL,	NULL,	NULL},
	{"DELETE",	":182",	TOK_DELETE,	NULL,	NULL,	NULL},
	{"RENAME",	":183",	TOK_REPLACE,	NULL,	NULL,	NULL},
	{"CREATE",	":184",	TOK_CREATE,	NULL,	NULL,	NULL},
	{"SECURITY",	":185",	TOK_SECURITY,	NULL,	NULL,	NULL},
	{"",		NULL,	TOK_NOP,	NULL,	NULL,	NULL},
	{NULL,		NULL,	TOK_NOP,	NULL,	NULL,	NULL}
};
 ...
 */
/* dmd TCB */
static struct slk Browslk[] = {	/* slks when browsing */
	{"HELP",	":170",	TOK_HELP,	NULL,	NULL,	NULL},
	{"",		NULL,	TOK_NOP,	NULL,	NULL,	NULL},
	{"",		NULL,	TOK_NOP,	NULL,	NULL,	NULL},
	{"PREV-FRM",	":173",	TOK_PREV_WDW,	NULL,	NULL,	NULL},
	...
	{"CANCEL",	":175",	TOK_CANCEL,	NULL,	NULL,	NULL},
	{"CMD-MENU",	":176",	TOK_CMD,	NULL,	NULL,	NULL},
	{"SELECT",	":186",	TOK_SELECT,	NULL,	NULL,	NULL},
	{NULL,		NULL,	TOK_NOP,	NULL,	NULL,	NULL}
};
...
/* the SLK set for an object without SLKs */
struct slk No_slks[1];
/* the current set of SLKs */
struct slk SLK_array[MAX_SLKS];
/* the default set of Screen Labeled Keys ( SLKS ) */
struct slk Defslk[MAX_SLKS];
/***** struct slk *Curslk = &Defslk[0];	   abs s14 */
static int SLK_level = 0;
static bool ModifiedPFKs = FALSE;	/* abs s14 */

static void showslks();
static bool sfk_prompt();

extern struct passwd *getpwnam(); 	/* abs s19 */
extern char *getenv();			/* abs s14 */
extern char *Progname;			/* abs s14 */
extern int Vflag;
...
*/
	...
	/* remember we downloaded sequences so 
	 * we can restore them on exit.  abs s14
	 */
	ModifiedPFKs = TRUE;	/* abs s14 */
...
sfk_prompt()
{
    ...
    char *i18n_response;
    char *i18n_first_letter;
    char i18n_s1[BUFSIZ]; /* -tf- */
    ...
    *word_end = '\0';					/* abs k18.2 */
    ...
    i18n_response=gettxt(":189","yes");
    i18n_first_letter=strncpy(i18n_s1,i18n_response,1);

    if (*response && strnCcmp(response, i18n_first_letter, 1) && strCcmp(response, i18n_response))
    {
...
/* restore_pfk is called after fmli exits curses.  
 * If FMLI has modified the terminals Programable Function Keys,
 * it looks for a file called .restorePFKs in the current directory 
 * then in the user's home directory.  If it finds this file, and can 
 * read it, it calls $SHELL, if defined, else /bin/sh to execute it.  
 *
 * The intent is that the file will contain a script that 
 * uses tput(1) to load the terminal's function keys with the user's 
 * prefered values since fmli may have over-wrote them, however the file 
 * may contain any script needed to do whatever post processing is needed.
 * abs s14
 */

#define BASENAME ".restorePFKs"
#define SLASH_BASENAME "/.restorePFKs"
#define LEN_SLASH_BASENAME 14	/* length of above + 1 for EOS */
...
int signum;
{
    ...
    file = BASENAME;
    if (access(file, R_OK) == 0)   /* if file is in current dir..  abs s16 */
    {
	if (getcwd(path, PATHSIZ - LEN_SLASH_BASENAME) != NULL)	/* abs s16 */
	    file = strcat(path, SLASH_BASENAME); 		/* abs s16 */
    }
    else  /* ..check home dir */
    {
	if ((pw = getpwnam(cuserid(NULL))) == NULL) 
	    return;
	strncpy(path, pw->pw_dir, PATHSIZ - LEN_SLASH_BASENAME);
	file = strcat(path, SLASH_BASENAME);
	if (access(file, R_OK) != 0 )
	    return;
    }

    printf("Invoking %s to run the shell script in \n%s to restore function key settings...\n", shell, file);
    fflush(stdout);
...
/* a flag denoting whether or not the SLKs should be refreshed */
int Refresh_slks = 0;
	...
	if (label && (*label == '\0')) {     /* disable any SLK */
		...
		if (slknum >= CHG_KEYS) {    /* redefine certain SLKS */ 
			...
			if (strCcmp(label, "prevpage") == 0 ||
			    (i18n_local_strings && strnCcmp(label, prevpage, SLK_NUM_CHAR) == 0))
				slk->tok = TOK_PPAGE;
			else if (strCcmp(label, "nextpage") == 0 ||
				 (i18n_local_strings && strnCcmp(label, nextpage, SLK_NUM_CHAR) == 0))
				slk->tok = TOK_NPAGE;
			else if (strCcmp(label, "prev-frm") == 0 ||
				 (i18n_local_strings && strnCcmp(label, prevfrm, SLK_NUM_CHAR) == 0))
				slk->tok = TOK_PREV_WDW;
			else if (strCcmp(label, "next-frm") == 0 ||
				 (i18n_local_strings && strnCcmp(label, nextfrm, SLK_NUM_CHAR) == 0))
				slk->tok = TOK_NEXT_WDW;
			else if (strCcmp(label, "enter") == 0 ||
				 (i18n_local_strings && strnCcmp(label, enter, SLK_NUM_CHAR) == 0))
				slk->tok = TOK_RETURN;
			else
				slk->tok = slktok;
			slk->tokstr = NULL;
...
* _init_I18n_slk (slk)
* struct slk *slk;
* initializes the label field of the referenced slk array; i. e.
* if the msgid field of an element is not NULL, it tries to retrieve
* a translated label text. The original label text is used as default.
*
************************************************************************
*/
...
static void _init_I18n_slk (slk)
struct slk *slk;
{
register struct slk *p;
register char *l, *s;


for ( p = slk; p->label != NULL ; ++p )
  {
    if ( p->msgid != NULL )
      {
	if ( (l = gettxt(p->msgid, p->label))  && l != p->label )
	  {
	    if ( (s = malloc ((SLK_NUM_CHAR+1)*sizeof(char))) != NULL )
	      {
	        strncpy (s, l, SLK_NUM_CHAR);
	        s[SLK_NUM_CHAR] ='\0';
	        p->label = s;
	      }
	  }
...
if ( p->msgid != NULL )
  {
    if ( (l = gettxt(p->msgid, p->label))  && l != p->label )
      {
        if ( (s = malloc ((SLK_NUM_CHAR+1)*sizeof(char))) )
	  {
            strncpy (s, l, SLK_NUM_CHAR);
            s[SLK_NUM_CHAR] ='\0';
            p->label = s;
...
_init_I18n_slk (Formslk);
_init_I18n_slk (Menuslk);
_init_I18n_slk (Textslk);
_init_I18n_slk (Echslk);
_init_I18n_slk (Vmslk);
_init_I18n_slk (Browslk);

prevpage = gettxt(prevpageid, prevpage);
nextpage = gettxt(nextpageid, nextpage);
prevfrm  = gettxt(prevfrmid, prevfrm);
nextfrm  = gettxt(nextfrmid, nextfrm);
enter    = gettxt(enterid, enter);

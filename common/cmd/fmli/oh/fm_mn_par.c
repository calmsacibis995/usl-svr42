...
#ident	"@(#)fmli:oh/fm_mn_par.c	1.19.3.4"
...
unsigned int Fm_mn_seq = 1;				/* abs s18 */
...
register struct fm_mn *fm_mn;
{
    ...
    if (fm_mn == NULL || fm_mn->single.attrs == NULL)
	return;
    fm_mn->in_use_flag = (unsigned int) 0;	         	/* abs s18 */
...
int no;
{
	...
	if (defaults[i].flags & MAKE_COPY)
	{
	    ...
	    *p = defaults[i];
            if (defaults[i].def != NULL)
	    {
		if (defaults[i].i18n_msgid != NULL)
                	i18n_msg = gettxt(defaults[i].i18n_msgid,defaults[i].def);
		else
			i18n_msg = defaults[i].def;
		p->def = malloc (strlen(i18n_msg) + 1);
		if(p->def == NULL)
		{
		    fatal(NOMEM, nil);
		}
		strcpy(p->def, i18n_msg);
	    }
	    p->flags |= FREEIT;
        }
	else

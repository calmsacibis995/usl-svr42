...
#ident	"@(#)fmli:oh/namecheck.c	1.19.3.3"
...
*/
	...
	if (*p == ' ' || *p == '\t') {
	    ...
	    sprintf(error, gettxt(":137","Object name cannot contain space or tab characters\n") );
	    *errstr = error;
	    return(FALSE);
        }
 ...
 */
	...
	sprintf(error, gettxt(":138","Object name cannot have more than %d characters\n"), maxchar);
	*errstr = error;
	return(FALSE);
    }
    /* check if total pathsize too big */
    if (opt)
	maxpath = len + 4;	/* 4: "/" + prefix(2) + NULL */
    else
	maxpath = len + 10; /* name + /.pref + 3 is biggest defined */
/* dmd s15	maxpath = FILE_NAME_SIZ + 10; /* name + /.pref + 3 is biggest defined */

    if ((int)(path?strlen(path):0) + maxpath > PATHSIZ) {

	sprintf(error,
		gettxt(":139","Can't create object: folder path length exceeds %d characters\n")
		, PATHSIZ - maxpath);
	*errstr = error;
	return(FALSE);
    }
...
miked */
	...
	sprintf(error,
	gettxt(":140","Object name cannot contain the special character '%c'"), name[span]);
	*errstr = error;
	return(FALSE);
    }
	...
	if ( iscntrl( *p ) ) {
	    ...
	    sprintf(error,
		    gettxt(":141","Object name cannot contain the special character ^%c"),
		    (*p + '@'));
	    *errstr = error;
	    return(FALSE);
	}
	...
	sprintf(error, gettxt(":142","Object name cannot start with the character '%c'"), *name);
	*errstr = error;
	return(FALSE);
    }
	...
	if (ott->objmask & M_WB)
	    sprintf(error, gettxt(":143","object exists in wastebasket") );
	else
	    sprintf(error, gettxt(":144","An object with that name already exists") );
	...
	*errstr = "An object with that name already exists";
    ...
    if (access(pathbuf, 0) != -1) {
       ...
       i18n_string=gettxt(":145","An object with that name already exists in %s");
       i18n_length=strlen(i18n_string) - SUBSTLEN;

       sprintf(dbuf, i18n_string,
		bsd_path_to_title(path?path:&Opwd[5], MESS_COLS-i18n_length));
       *errstr = dbuf;
       retval = FALSE;
    } else
	retval = TRUE;

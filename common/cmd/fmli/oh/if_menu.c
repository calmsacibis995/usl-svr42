 ...
 *	All Rights Reserved
 */
#ident	"@(#)fmli:oh/if_menu.c	1.48.3.5"
...
static struct attribute Mn_tab[MN_KEYS] = {
        ...
        /*** 
             fourth argument for msgid since default name for menu
             must be retrieved from catalogue
        ***/
	...
	{ "menu",	RET_STR|EVAL_ONCE, ":117",        "Menu", NULL, 0 },
	...
	{ "rows",	RET_INT|EVAL_ONCE, NULL,        "0", NULL, 0 },
	{ "columns",	RET_INT|EVAL_ONCE, NULL,        "0", NULL, 0 },
	{ "close",	RET_BOOL|EVAL_ONCE, NULL,	NULL, NULL, 0 },
	{ "reread",	RET_BOOL|EVAL_ALWAYS, NULL,	NULL, NULL, 0 },
	{ "multicolumn",RET_BOOL|EVAL_ONCE, NULL,	"", NULL, 0 },
	{ "multiselect",RET_BOOL|EVAL_ONCE, NULL,	NULL, NULL, 0 },
...
static struct attribute Mn_fld_tab[MN_FLD_KEYS] = {
	...
	{ "description",RET_STR|EVAL_ONCE,	NULL, 	NULL, NULL, 0 },
	...
	{ "arg",	RET_STR|EVAL_SOMETIMES,	NULL, 	NULL, NULL, 0 },
	{ "selected",   MAKE_COPY|RET_BOOL|EVAL_SOMETIMES, NULL, 	NULL, NULL, 0 },
	{ "itemmsg",    RET_STR|EVAL_ALWAYS,	NULL, 	NULL, NULL, 0 },
...
FILE *fp;
{
    ...
    /* Test of init-file Autosort	*/
    if ((get_sing_def(&fm_mn, MN_AUTOSORT) == NULL && Autosort == TRUE) || 
	sing_eval(&fm_mn, MN_AUTOSORT))
	   autosort (&fm_mn);
...
register struct actrec *a;
{
	...
	if ((fp = fopen(a->path, "r")) == NULL)
	{
		...
		if (errno == EACCES) /*file exists, but permissions changed*/
                        warn(UPDAT_NOPERM, a->path);
                else if (errno == ENOENT) /* file removed after posting */
...
bool all;
{
	...
	if (p = strchr(ARGS()[0], '='))    /* 0 since delAltenv.. abs s15 */
	    *p = '\0';
	strcpy(namebuf, ARGS()[0]);        /* ..shifts the array. abs s15 */
	if (p)
	    *p = '=';
	delAltenv(namebuf);
    }
...
* menu_item_cmp (p1, p2)
* struct fld *p1, *p2;
* compares two menu items by their name using strcoll()
*
* autosort(fm_mn)
* struct fm_mn *fm_mn;
* sorts the item entries in a menu frame corresponding to their name
*
************************************************************************
*/
...
{
   int i;
   i=strcoll ((p1->attrs[MN_NAME])->cur, (p2->attrs[MN_NAME])->cur);
   if (i == 0) {
      i=strcmp ((p1->attrs[MN_NAME])->cur, (p2->attrs[MN_NAME])->cur);
      return(i);
   }
   else return (i);
}
...
struct fm_mn *fm_mn;
{
    ...
    if ((num_fields = array_len(fm_mn->multi)) <= 0) 	
	return;
    
    for ( i = 0 ; i < num_fields ; ++i)
	multi_eval(fm_mn, i, MN_NAME);
    qsort ((char*)(fm_mn->multi), num_fields, 
	   sizeof (struct attribute *), menu_item_cmp);

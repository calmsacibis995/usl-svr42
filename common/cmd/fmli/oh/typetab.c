...
#ident	"@(#)fmli:oh/typetab.c	1.17.3.3"
...
#ifdef PRE_CI5_COMPILE		/* abs s14 */
#include "eft.types.h"		/* abs s14 */
#endif				/* abs s14 */
...
/* dmd TCB */
static struct ott_tab Otts[MAX_OTT];
...
/*	(void) fclose(dirfp); */
	...
	if ((ottfp = fopen(ottname, "r")) != NULL &&
	    ...
	    if (fgets(buf, BUFSIZ, ottfp) && strcoll(buf, Ott_version) != 0) {
		...
		entry->hname = strsave(entry->dname);	/* dmd s15 */
...
*/
	...
	while ((dir_entry = readdir(dirfp)) != NULL) /* abs */
	{
		...
		for (; i < recnum; i++)
            /*** strcoll instead of strcmp ***/
		    if (strcoll(fname, Cur_entry[i].name) == 0 && strcmp(fname, Cur_entry[i].name) == 0) {
			   entry = Cur_entry + i;
			   break;
		    }
	    }
 ...
 */
            /*** strcoll instead of strcmp ***/
		if (strcoll(ent->objtype, "MAIL_OUT") == 0)
...
int *el1, *el2;
{
	...
	/* strcoll() instead of strcmp() */
        i=strcoll(Cur_entry[*el1].dname, Cur_entry[*el2].dname);
        if (i == 0) {
           i=strcmp(Cur_entry[*el1].dname, Cur_entry[*el2].dname);
           return(reverse * i);
        }
        else return(reverse * i);
}
...
int *el1, *el2;
{
	...
	/* strcoll() instead of strcmp() */
	if ((comp1 = strcoll(Cur_entry[*el1].display, Cur_entry[*el2].display)) == 0 && (comp1 = strcmp(Cur_entry[*el1].display, Cur_entry[*el2].display)) == 0)
...
char *path;
{
	...
	if (!(Otts[i].path))
	    continue;
            /*** strcoll instead of strcmp ***/
	if ((Otts[i].modes & OTT_ACTIVE) && (strcoll(Otts[i].path, path) == 0) && (strcmp(Otts[i].path, path) == 0)) {
#ifdef _DEBUG
...
char *name;
{
	...
	for (i = 0; i < size; i++)
		if (!(Cur_entry[i].objmask & M_DL) && 
            /*** strcoll instead of strcmp ***/
				strcoll(Cur_entry[i].name, name) == 0  &&
...
char *name;
{
	...
	for (i = 0; i < size; i++)
		if (!(Cur_entry[i].objmask & M_DL) &&
            /*** strcoll instead of strcmp ***/
				strcoll(Cur_entry[i].dname, name) == 0 &&
...
ott_write()
{
	...
	if (!(Cur_entry[i].objmask & M_DL)) {
	    if (Cur_entry[i].dname && Cur_entry[i].dname[0]) {
            /*** strcoll instead of strcmp ***/
		if (strcoll(Cur_entry[i].dname, Cur_entry[i].name) == 0 &&
		    strcmp(Cur_entry[i].dname, Cur_entry[i].name) == 0)
		    dname = ".";
		else
		...
		Cur_entry[i].objtype) {
            /*** 301091 strcoll instead of strcmp ***/
		if (strcoll(Cur_entry[i].display, def_display(Cur_entry[i].objtype)) == 0 &&
		strcoll(Cur_entry[i].display, def_display(Cur_entry[i].objtype)) == 0)
		    display = ".";
		else

...
#ident	"@(#)fmli:oh/evstr.c	1.13.3.4"
...
int *pflags;
{
	...
	if (firstime)
	{
	    ...
	    str = io_string(out);
	if ((strncmp(str, "vary", 4) == 0) && (!isalnum((int)str[4])) && (!(strncmp(&str[4],"_",1) == 0))) /* p7 ck */
	    {
/*		if (*pflags & EVAL_ONCE)
...
** abs k15 */
	...
	else if ((strncmp(str, "const", 5) == 0) && (!isalnum((int)str[5])) && (!(strncmp(&str[5],"_",1) == 0)))
	    {

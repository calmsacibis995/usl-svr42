...
#ident	"@(#)fmli:oh/ootread.c	1.10.3.4"
...
/* dmd TCB */
static struct operation	Obj_sp;
/* the OOT entry for the unremove operation for all objects */
struct operation	Obj_unrm;
/* dmd TCB */
static struct operation	Obj_ml;
/* dmd TCB */
static struct operation	Obj_viewfull;
/* dmd TCB */
static struct operation	Unknown_ex;
/* the OOT entry for the convert operation for unknown objects */
struct operation	Unknown_cv;

/* dmd TCB */
static struct oper_trans {
	char	*name;
	struct operation	*op;
} Optrans[] = {
	{"SH",	&Obj_sh},
...
char *objtype, *file;
{
    ...
    fprintf(fp, gettxt(":149","#\n# Object Definition Dump of object type %s\n"), objtype);
    fprintf(fp, gettxt(":150","# Dump date: %s#\n"), ictime(&t));

 ...
 *	All Rights Reserved
 */
#ident	"@(#)fmli:oh/if_init.c	1.36.3.3"
...
#define INIT_AUTOLAYOUT		28 	/* abs s17 */
#define INIT_AUTOSORT		29
...
#define WORKINGMSGID            ":114"
#define FOURFOUR		"4-4"
#define DEFTOGGLE		3

/*
** inserted i18n_msgid field in the array initialisation
*/
static struct attribute Init_single_tab[INIT_SING_KEYS] = {
	{ "interrupt",	RET_STR|EVAL_ONCE, NULL,	"false", "false", 0 },
	{ "oninterrupt",RET_STR|EVAL_ONCE,  NULL,	DEF_ONINTR, DEF_ONINTR, 0 },
	{ "title",		RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },
	{ "text",		RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },
	{ "rows",		RET_STR|EVAL_ONCE,	NULL,	"10", NULL, 0 },
	{ "columns",		RET_STR|EVAL_ONCE,	NULL,	"50", NULL, 0 },
	{ "banner",		RET_STR|EVAL_ONCE,	NULL,	NULL, NULL, 0 },
	{ "bancol",		RET_INT|EVAL_ONCE,	NULL,	"", NULL, 0 },
	{ "working",		RET_STR|EVAL_ONCE,	":114",	"Working", NULL, 0 },
	{ "screen",		RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },	
	{ "banner_text",	RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },	
	{ "window_text",	RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },	
	{ "active_border",	RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },
	{ "inactive_border",	RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },
	{ "active_title_text",	RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },
	{ "active_title_bar",	RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },
	{ "inactive_title_text",RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },
	{ "inactive_title_bar",	RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },
	{ "highlight_bar",	RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },	
	{ "highlight_bar_text",	RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },
	{ "slk_bar",		RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },	
	{ "slk_text",		RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },
	{ "field_bar",		RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },
	{ "field_text",		RET_STR|EVAL_ONCE,	NULL,	"", NULL, 0 },
	{ "toggle",		RET_STR|EVAL_ONCE,      NULL,	"3", NULL, 0 },
	{ "nobang",		RET_BOOL|EVAL_ONCE,	NULL,	FALSE, FALSE, 0 },
	{ "use_incorrect_pre4.0_behavior",
				RET_BOOL|EVAL_ONCE,	NULL,	FALSE, FALSE, 0 },
	{ "permanentmsg",	RET_STR|EVAL_ONCE,	NULL,	"", FALSE, 0 },
        { "autolayout",		RET_BOOL|EVAL_ONCE,	NULL,	NULL, NULL, 0 },	/* abs s17 */
        { "autosort",		RET_BOOL|EVAL_ONCE,	NULL,	NULL, NULL, 0 },
};
...
static struct attribute Init_multi_tab[INIT_MULT_KEYS] = {
	...
	{ "name",	RET_STR|EVAL_ALWAYS,	NULL,	NULL, NULL, 0 },
	{ "show",	RET_BOOL|EVAL_ALWAYS,	NULL,	"", NULL, 0 },
	{ "button",	RET_INT|EVAL_ALWAYS,	NULL,	"0", NULL, 0 }
};
 ...
 * extern globals
 */
/* the current working message */
char	*Work_msg = NULL;

/* the screen column to display the working message in */
int	Work_col;

/* the column to display the Mail indicator in */
int	Mail_col;

/* the flag that indicates whether choices should be toggled or displayed as a menu */
int	Toggle = DEFTOGGLE;

/* Nobang is set to TRUE if the ! command is disabled from the command line */
bool    Nobang = FALSE;		/* defaults to bang enabled */

/* Autolayout is set to the result of evaluating the autolayout init file 
 * descriptor.  It is inherited by all Form frames but can be over-ridden
 * per frame basis by coding the frame level autolayout descriptor.
 */
bool Autolayout = FALSE;	/* abs s17 */

/* 
 * Autosort is set to the result of evaluating the autosort init file 
 * descriptor.  It is inherited by all Form frames but can be over-ridden
 * per frame basis by coding the frame level autosort descriptor.
 */
bool Autosort = FALSE;	/* abs s17 */
...
**		return;                 abs k15 */
	...
	if ((fp = fopen(initfile, "r")) == NULL)
	{
	    ...
	    (void)mess_err( gettxt(":115","error: initialization file missing or not readable") ); /* abs s15 */
	    mess_flash(gettxt(":115","error: initialization file missing or not readable") );
	    doupdate();
	    return;
	}
	...
	set_default(INIT_AUTOLAYOUT); 				  /* abs s17 */
	Autolayout = sing_eval(&Inits, INIT_AUTOLAYOUT) ? TRUE: FALSE; /*s17 */
	Autosort = sing_eval(&Inits, INIT_AUTOSORT) ? TRUE : FALSE;
...
**		return;              abs k15 */
	...
	{
	    (void)mess_err( gettxt(":116","error: commands file missing or not readable") ); /* abs s15 */
	    mess_flash( gettxt(":116","error: commands file missing or not readable") );
	    doupdate();
	    return;
	}
...
set_def_status()
{
	...
	if (!Work_msg)
		Work_msg = gettxt (WORKINGMSGID, WORKINGMSG);

 ...
 *	All Rights Reserved
 */
#ident	"@(#)fmli:sys/genfind.c	1.10.3.4"
...
IOSTRUCT *errstr;
{
    ...
    char *i18n_default,idx[10];
    char i18n_label[BUFSIZ]; 
    char *i18n_msg;
    char *i18n_labeled_msg;
    extern char *gettxt();

    if (argc < 6 && argc > 8) {
        strcpy(i18n_label,"genfind: ");
        i18n_msg=gettxt(":246","Arguments invalid");
        i18n_labeled_msg=strcat(i18n_label,i18n_msg);
	putastr(i18n_labeled_msg, errstr);   /* abs s14 */
	return FAIL;
    }
    i = 1;
    origin = 0;
    ...
    if ((int)strlen(argv[i]) > PATHSIZ - 2) { 			/* EFT k16 */
        strcpy(i18n_label,"genfind: ");
        i18n_msg=gettxt(":247","Path too long");
        i18n_labeled_msg=strcat(i18n_label,i18n_msg);
	putastr(i18n_labeled_msg, errstr);   /* abs s14 */
	return FAIL;
    }
    ...
    if ((pattern = myregcmp(argv[++i])) == NULL) {
           strcpy(i18n_label,"genfind: ");
           i18n_msg=gettxt(":248","Name invalid");
           i18n_labeled_msg=strcat(i18n_label,i18n_msg);
	   putastr(i18n_labeled_msg, errstr);   /* abs s14 */
	   return FAIL;
    }
	...
	if ((passwdptr = getpwnam(argv[i])) == NULL) {
           strcpy(i18n_label,"genfind: ");
           i18n_msg=gettxt(":249","Owner invalid");
           i18n_labeled_msg=strcat(i18n_label,i18n_msg);
           putastr(i18n_labeled_msg, errstr);   /* abs s14 */
	   return FAIL;
	}
	...
	if (range(argv[i], origin, &min_days, &max_days) < 0) {
           strcpy(i18n_label,"genfind: ");
           i18n_msg=gettxt(":250","Age invalid");
           i18n_labeled_msg=strcat(i18n_label,i18n_msg);
	   putastr(i18n_labeled_msg, errstr);   /* abs s14 */
	   return FAIL;
	}
	...
	while(fgets(buf,BUFSIZ,fp)) {
	    type=strtok(buf,"\t");
	    dtype=strtok(NULL,"\n");
	    if ((i18n_default = strchr(dtype,':')) != NULL) { /* sni-tf: translate
							  dtype if necessary */
		strcpy(idx,":");
		strncat(idx, dtype, (int)(i18n_default - dtype) );
		dtype = gettxt(idx, ++i18n_default);
	    }

...
#ident	"@(#)fmli:sys/eval.c	1.26.3.7"
...
int	EV_retcode;
/* a flag indicating whether or not the current set of backquotes has been evaluated or not */
int	EV_backquotes;
/* the last token processed by eval() */
int	Lasttok;

static void err_mess();	        /* abs s14 */

extern char *io_string();	/* abs s14 */
extern int in_an_if;	/* (in an if statement) see evfuncs.c */
extern char status_of_if[MAX_IF_DEPTH];   /* status and pos. of each if */
...
abs */
		...
		case ET_DOLLAR:
			/* 
			** Call eval_dollar() also in EV_GROUP case.
			** Therefor eval_dollar needs to return 
			** immediately if not a $$ message and not
			** EV_TOKEN.
			*/
			if (flags & EV_TOKEN ||
			    flags & EV_GROUP )
...
int	flags;
{
	...
	/* Following lines to handle "$$" syntax */

	if ( (c = getac(instr)) == '$' )
	  {
	    return (eval_nls_message (instr, outstr, flags));
	  }
	else
	    ungetac(c, instr);
	if ( !(flags & EV_TOKEN) )
...
int	flags;
{
    ...
    mystderr = io_open(EV_USE_STRING, NULL); /* abs s14 */
    altstdout = NULL;
    altstderr = NULL;
	...
	if (argc == 1) {
		...
		for (a = 0; a < instr->mu.str.count - start_look + 1; a++) {
			...
			if (nonwhite) {
			    ...
			    sprintf(errbuf, gettxt(":219","Syntax error - \";\" found after \"%s\""), argv[0]);
			    (void)mess_err(errbuf);
			    Lasttok = ET_EOF;
			    in_an_if = 0;
			}
			break;
	...
	case ET_GTHAN + DOUBLE:	/* append symbol */
	{
	    ...
	    register char	*p;
	    int savetok=Lasttok;
            int cn2, cn1, ca=0;  /* ck p7 */

            for (cn2=1; cn2 <= in_an_if; cn2++) {
              if (status_of_if[cn2] & IF_IS_TRUE) {
                 if ((status_of_if[cn2] & IN_AN_ELSE) ||
                     (status_of_if[cn2] & IN_AN_ELIF_SKIP))
                        ca = 1;
              } else {
                     if (status_of_if[cn2] & IN_A_THEN)
                        ca = 1;
                }
            }
            if (ca == 1)
	...
	if (doit || argc >= MAXARGS - 1) {
	    ...
	    {
		argv[argc] = NULL;
		if (!((argc == 1) && (argv[0][0] == 0))) /* Not good fix; but avoids core dump */
			EV_retcode = evalargv(argc, argv, mystdin,
				      altstdout ? altstdout : mystdout,
		/* abs s14 */         altstderr ? altstderr : mystderr);

		/* if stderr not redirected then copy it to the message line
                 * abs s14
		 */
		if (altstderr == NULL) 		/* abs s14 */
		    err_mess(mystderr);		/* abs s14 */
		/*
...
/* err_mess
 *          Copy the text from the given I/O structure to the message line.
 *	    abs s14
 */
static void
err_mess(iop)
IOSTRUCT iop;
{
    char *errstr, *errstart, *newline;

    errstr = errstart = io_string(iop);
    ...
    if (*errstr != EOS)
    {
	/* skip white space, blank lines */
	while (isspace(*errstart)) 
	    errstart++;

	/* collapse multiple lines int one line */
	while ((newline = strchr(errstart, '\n')) != NULL)
	    *newline = ';';
	
	/* write it to the message line if there's anything to write */
	if (isprint(*errstart))
	{
	    (void)mess_err(errstart);
	    doupdate();
	}
    }
    free(errstr);
...
/***********************************************************************
* new function eval_nls_message
* handles $$ syntax for nls message strings in description files
* Syntax:
* $$<message_catalogue>:<msg_number>:<default_message>
* where <default_message> can be one word or a string enclosed in
* double quotes
************************************************************************
*/


int i18n_local_strings = 0;

extern char *expand ();


static
eval_nls_message (instr, outstr, flags)
IOSTRUCT	*instr;
IOSTRUCT	*outstr;
int 		flags;
{
...
#ifdef DEBUG
    fprintf (stderr, "eval_nls_message: flags=0x%x ", flags);
    if ( flags & IN_DQ )
	fprintf (stderr, "IN_DQ ");
#endif

    /* open an IO struct for temporary strings */
    iop = io_open(EV_USE_STRING, NULL);

    /* get the name of the message catalogue and the message number 
    ** syntax: $$<message_catalogue>:<message_number>:<message>
    */
    while ( (c = getac(instr)) )
      {
	if ( c == ':' )
    ...
    msg_id = expand (io_ret_string(iop));
    /* clear iop to store the default message */
    io_clear (iop);
    /* now get the default message.
    ** We support two cases for strings:
    **  1) An abitrary string encloses in double quotes or single quotes
    **     "......." or '........"
    **     the delimiter character (" or ') must be escaped (\" or \')
    **     if it should be in the string.
    **  2) A string of words
    */
    if ( (c = getac(instr)) == '"'  || c == '\'' )
      {
	delim = c;
/*  	putac (c, iop);	  write the delimiter to the stream, ar010492 */
	while ( (c = getac(instr)) )
	  {
	    if ( c != delim ) /* ar010492 */
	    	putac (c, iop);
	    if ( c == '\\'  &&  (c = getac(instr)) )
	        ...
	        break;
	  } /* END OF WHILE (c = getac(instr))) */
	if ( !c )
	  {
	    /* unexpected end of stream, the problem may be a
	    * missing delimiter.
	    */
	    putac (delim, iop);
	  }
      ...
      {
	register int string = 1;
        register IOSTRUCT   *tmp;
	...
	while (string)
	  {
		...
		string = 0;
		io_seek(tmp, 0);
		io_push(instr, tmp);	/* push it back in instr */
		break;
	      }
	    putac (c, tmp);
	    switch (c)
		...
		io_push(instr, tmp);	/* push it back in instr */
	      ...
	      {
		/* add the string in tmp to the deafult message in iop */
		putastr (io_ret_string (tmp), iop);
#ifdef DEBUG
		fprintf (stderr, "\nadding \"%s\"\n", 
				io_ret_string (tmp));
#endif
    ...
    def_msg = io_ret_string (iop);

    if ( (flags & EV_TOKEN) )
      {
	/* make a possible expansion of the translated string */
        msg = strsave (gettxt (msg_id, def_msg));
        msge = expand (msg);
      }
    else
      {
	msge = gettxt (msg_id, def_msg);
	msg = NULL;
      }
#ifdef DEBUG
    fprintf (stderr, "msg_id=\"%s\"  def_msg=\"%s\"  msge=\"%s\"\n",
		msg_id, def_msg, msge);
#endif


    if ( delim && (msge[0] != '"' && msge[0] != '\'') && !(EV_TOKEN & flags) )
	putac (delim, outstr);
    else
	delim = 0;

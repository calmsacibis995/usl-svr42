 ...
 *	All Rights Reserved
 */
#ident	"@(#)fmli:sys/coproc.c	1.23.3.3"
...
/* a flag indicating whether or not a co-process is active */
int Coproc_active = 0;
...
IOSTRUCT *errstr;
{
	...
	if ((i = findtab(args[1])) == -1)	/* look-up coproc in proc tab */
		return(NULL);
	make_cur(i, errstr);			/* make the coproc current */
...
IOSTRUCT *errstr;		/* abs s14 */
{
    ...
    {
        i18n_msg=gettxt(":209","Can't open the write pipe: ");
	strcpy(i18n_labelled_msg,"coproc: "); /* -tf- */
	strcat(i18n_labelled_msg,i18n_msg);

	putastr(i18n_labelled_msg,errstr); /* abs s14 */

#ifndef PRE_CI5_COMPILE                                       	/* abs s14 */
	putastr(strerror(errno), errstr); 			/* abs s14 */
#endif								/* abs s14 */
	/*****
    ...
    {
        i18n_msg=gettxt(":210","Can't open the read pipe: ");
	strcpy(i18n_labelled_msg,"coproc: "); /* -tf- */
	strcat(i18n_labelled_msg,i18n_msg);
...
IOSTRUCT *errstr;
{
    ...
    make_cur(array_len(Cotable) - 1, errstr); 		/* abs s14 */
    if (gobble) {
	char buf[BUFSIZ];

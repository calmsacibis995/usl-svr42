...
#ident	"@(#)syslogd:syslogd.c	1.7.5.10"
	...
	while ((ncp = getnetconfig(handle)) != NULL) {
		if (ncp->nc_semantics == NC_TPI_CLTS) {
			if (netdir_getbyaddr(ncp, &hsp, nbp) == 0) {
				...
				if (hsp->h_cnt > 0) {
					/* ignore cnt > 1 */
					(void) strcpy(h->name, hsp->h_hostservs->h_host);
					break;
				}
			}
		}
...
nofile:
	...
	signal(SIGHUP, (void (*)())init);	/* enable signal handler */
}
	...
	register struct filed *f;
{
	...
	case '/':
		...
		if ((f->f_file = open(p, O_WRONLY|O_APPEND|O_CREAT|O_NOCTTY)) < 0) {
			logerror(p);
			break;
		}
		if (isatty(f->f_file)) {

...
pc_l, pc_h, pc_m, pc_m, head.nfns, n_cc, n_pc));
	...
	/* Shifting right by 2 is the quickest way to lose accuracy */
	/* Solve overflow problem by casting to double later */
	bias = BIAS;
...
if (debug_value & 04) {
	...
	*/
	...
	for (n = 0; n < n_syms; n++) {		/* Loop on symbols. */
		...
		i0 = (((unsigned) pc0 - (unsigned) pc_l) * (double)sf)/bias;

			/* Upper bound (least or least + 1) of indices. */
		i1 = (((unsigned) pc1 - (unsigned) pc_l) * (double)sf)/bias;

		if (i1 >= n_pc)				/* If past top, */
			i1 = n_pc - 1;				/* adjust. */

			/* Lowest addr for which count maps to pcounts[i0]; */
		pc00 =  pc_l + (unsigned long)(((double)bias * i0)/sf);

			/* Lowest addr for which count maps to pcounts[i1]. */
		pc10 =  pc_l + (unsigned long)(((double)bias * i1)/sf);
...
"Time unaccounted for: %.7G\n", t_tot - t0));
	...
	_symintClose(ldptr);
		...
		{
			Fprint(stderr, "%s: no call counts captured\n", cmdname);
		}
	exit(0);
}

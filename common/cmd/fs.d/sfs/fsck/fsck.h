...
#ident	"@(#)sfs.cmds:sfs/fsck/fsck.h	1.3.2.4"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/fs.d/sfs/fsck/fsck.h,v 1.1 91/02/28 17:26:47 ccs Exp $"
...
struct inodesc {
	...
	int (*id_secfunc)();    /* security function to be applied */
...
char  	*secstatemap;           /* ptr to secure inode state table */
...
/* definitions for the secure inode state table */
#define       SEC_BADACL      01      /* invalid ACL for inode */
#define       SEC_DUPACL      02      /* duplicate ACL block for inode */
#define       SEC_INOVIS      04      /* inode visited (for pass 2) */

/* block size for x ACL entries */
#define       ACLBLKSIZ(x) \
	fragroundup(&sblock, sizeof(struct aclhdr) + ((x) * sizeof(struct acl)))

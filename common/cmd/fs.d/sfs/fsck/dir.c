...
#ident	"@(#)sfs.cmds:sfs/fsck/dir.c	1.3.4.5"
#ident "$Header: dir.c 1.2 91/09/30 $"
	...
	ino_t inumber;
{
	...
	curino.id_secfunc = parentino->id_secfunc;
	...
	ino_t pdir;
{
	...
	*pathp = '\0';
	...
	if (!DIRCT(dp)) {
		...
		idesc.id_name = lfname;
		if ((ckinode(ginode(SFSROOTINO), &idesc) & ALTERED) == 0) {
			pfatal("SORRY. CANNOT CREATE lost+found DIRECTORY\n\n");
			return (0);
		}
		inodirty();

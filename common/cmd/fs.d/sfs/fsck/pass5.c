...
#ident	"@(#)sfs.cmds:sfs/fsck/pass5.c	1.2.3.2"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/fs.d/sfs/fsck/pass5.c,v 1.1 91/02/28 17:27:06 ccs Exp $"
...
pass5()
{
	...
	for (c = 0; c < sblock.fs_ncg; c++) {
		...
		/*
                 * free files doesn't include alternate inodes
                 */
                newcg->cg_cs.cs_nifree = sblock.fs_ipg / NIPFILE;
		...
		/*
                 ...
                 */
                for (i = 0; i < sblock.fs_ipg; j += NIPFILE, i += NIPFILE) {
                        /*
                         * allocate alternate inode
                         */                        
			setbit(newcg->cg_iused, i + 1);
			switch (statemap[j]) {
		...
		for (i = 0, d = dbase;
			...
			if (frags == sblock.fs_frag) {
				...
				sfs_fragacct(&sblock, blk, newcg->cg_frsum, 1);
			}
		}

...
#ident	"@(#)sfs.cmds:sfs/fsck/setup.c	1.4.6.4"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/fs.d/sfs/fsck/setup.c,v 1.1 91/02/28 17:27:08 ccs Exp $"
...
sbok:
	...
	if (lncntp == NULL) {
        ...
        secstatemap = calloc((unsigned)(imax + 1), sizeof(char));
        if (secstatemap == NULL) {
                printf("cannot alloc %d bytes for secstatemap\n", imax + 1);
                goto badsb;
        }
        bzero(secstatemap, imax+1);

...
#ident	"@(#)sfs.cmds:sfs/labelit/labelit.c	1.3.6.6"
#ident "$Header: labelit.c 1.3 91/06/28 $"
...
/* Volcopy security header for tapes - see sfs volcopy.c */
	...
	if (nflag) {
		...
		if (!nflag) {
			...
			if((argc==2 && Tape_hdr.v_reel>1) || equal(Tape_hdr.v_magic,"Finc",4))
				exit(0);
			if(read(fso, &in_sec_hdr, sizeof(in_sec_hdr)) != sizeof(in_sec_hdr)) {
				fprintf(stderr, "labelit: cannot read sec_hdr\n");
				exit(31+1);
			}
		...
		memset(&in_sec_hdr, '\0', sizeof(in_sec_hdr));
		...
		if(write(fso, &in_sec_hdr, sizeof(in_sec_hdr)) != sizeof(in_sec_hdr)) {
			fprintf(stderr, "labelit: cannot write sec_hdr\n");
			exit(31+1);
		}
...
usage ()
{
	...
	(void) fprintf (stderr, "sfs usage: labelit [-F sfs] [generic options] [-n] special [fsname volume]\n");
	exit (31+1);
}
	...
	char		*volume;
{
	...
	if (fsname != NULL) {
		...
		if (altsblock.fs_magic != SFS_MAGIC) {
			(void) fprintf (stderr, "labelit: ");

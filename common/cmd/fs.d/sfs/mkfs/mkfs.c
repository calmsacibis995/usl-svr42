...
#ident	"@(#)sfs.cmds:sfs/mkfs/mkfs.c	1.3.10.12"
	...
	char *argv[];
{
	...
	if (Cflag && Lflag) {
		fprintf(stderr, "sfs mkfs: cannot specify both -o C and -o L\n");
		usage();
	}
	argc -= optind;
	argv = &argv[optind];
	...
	fsi = open(fsys, 0);
	if(fsi < 0) {
		fprintf(stderr, "sfs mkfs: %s: cannot open\n", fsys);
		exit(RET_FSYS_OPEN);
	}
	if (mflag) {
		dump_fs (fsys, fsi);
	...
	if (!Nflag) {
		fso = creat(fsys, 0666);
		if(fso < 0) {
			fprintf(stderr, "sfs mkfs: %s: cannot create\n", fsys);
			exit(RET_FSYS_CREATE);
		}
		if(stat(fsys, &statarea) < 0) {
			fprintf(stderr, "sfs mkfs: %s: %s: cannot stat\n",
				argv[0], fsys);
			exit(RET_FSYS_STAT);
		}
		if ((statarea.st_mode & S_IFMT) != S_IFBLK &&
		    (statarea.st_mode & S_IFMT) != S_IFCHR) {
			fprintf(stderr, "sfs mkfs: %s is not special device\n", fsys);
			exit(RET_FSYS_DEV);
		}
		if (statarea.st_flags & _S_ISMOUNTED) {
			fprintf(stderr, "sfs mkfs: %s is mounted, can't mkfs\n", fsys);
			exit(RET_FSYS_MOUNT);
		}
	...
	fssize = atoi(argv[1]);
	if (fssize <= 0) {
		fprintf(stderr, "sfs mkfs: invalid size %d\n", fssize);
		exit(RET_BAD_FSSIZE);
	}
	...
	if (sblock.fs_ntrak <= 0) {
		fprintf(stderr, "sfs mkfs: invalid ntrak %d\n", sblock.fs_ntrak);
		exit(RET_BAD_NTRAK);
	}
	if (sblock.fs_nsect <= 0) {
		fprintf(stderr, "sfs mkfs: invalid nsect %d\n", sblock.fs_nsect);
		exit(RET_BAD_NSECT);
	}
	...
	if (argc > 8)
		rps = atoi(argv[8]);
	if (rps <= 0) {
		fprintf(stderr, "sfs mkfs: invalid rotations per second %d\n", rps);
		exit(RET_BAD_RPS);
	}
	...
	if (rot <= -1) {	/* default by newfs and mkfs */
		...
		if (rot >= (1000/rps)) {
			fprintf(stderr, "sfs mkfs: invalid rotational delay %d\n", rot);
			exit(RET_BAD_ROT);
		}
	}
	...
	if (!POWEROF2(sblock.fs_bsize)) {
		fprintf(stderr, "sfs mkfs: block size must be a power of 2, not %d\n",
			sblock.fs_bsize);
		exit(RET_BSIZE_POW2);
	}
	if (!POWEROF2(sblock.fs_fsize)) {
		fprintf(stderr, 
			"sfs mkfs: fragment size must be a power of 2, not %d\n",
			sblock.fs_fsize);
		exit(RET_FSIZE_POW2);
	}
	if (sblock.fs_fsize < DEV_BSIZE) {
		fprintf(stderr, 
			"sfs mkfs: fragment size %d is too small, minimum is %d\n",
			sblock.fs_fsize, DEV_BSIZE);
		exit(RET_FSIZE_SMALL);
	}
	if (sblock.fs_bsize < MINBSIZE) {
		fprintf(stderr, 
			"sfs mkfs: block size %d is too small, minimum is %d\n",
			sblock.fs_bsize, MINBSIZE);
		exit(RET_BSIZE_SMALL);
	}
	if (sblock.fs_bsize > MAXBSIZE) {
		fprintf(stderr, "sfs mkfs: block size %d is too big, maximum is %d\n",
			sblock.fs_bsize, MAXBSIZE);
		exit(RET_BSIZE_BIG);
	}		
	if (sblock.fs_bsize < sblock.fs_fsize) {
		fprintf(stderr, 
	        "sfs mkfs: block size (%d) cannot be smaller than fragment size (%d)\n",
	...
	if (sblock.fs_frag > MAXFRAG) {
		fprintf(stderr, 
		"sfs mkfs: fragment size %d is too small, minimum with block size %d is %d\n",
	...
	if (sblock.fs_cpc > MAXCPG) {
		fprintf(stderr,
	            "sfs mkfs: maximum block size with nsect %d and ntrak %d is %d\n",
	...
	if (sblock.fs_cpg < 1) {
		fprintf(stderr, 
			"sfs mkfs: cylinder groups must have at least 1 cylinder\n");
		exit(RET_CPG_ONE);
	}
	if (sblock.fs_cpg > MAXCPG) {
		fprintf(stderr,
		   "sfs mkfs: cylinder groups are limited to %d cylinders\n", MAXCPG);
		exit(RET_CPG_MAX);
	}
	if (sblock.fs_cpg % sblock.fs_cpc != 0) {
		fprintf(stderr,
		   "sfs mkfs: cylinder groups must have a multiple of %d cylinders\n",
		    sblock.fs_cpc);
		exit(RET_CPG_MULT);
	}
	/*
	...
	if (sblock.fs_ncyl < 1) {
		fprintf(stderr, 
			"sfs mkfs: file systems must have at least one cylinder\n");
		exit(RET_FS_ONE);
	}
	/*
...
next:
	...
	if (sblock.fs_spc > MAXBPG(&sblock) * NSPB(&sblock)) {
		fprintf(stderr,
			"sfs mkfs: too many sectors per cylinder (%d sectors)\n",
			sblock.fs_spc);
	...
	if (sblock.fs_fpg > MAXBPG(&sblock) * sblock.fs_frag) {
		fprintf(stderr, 
			"sfs mkfs: cylinder group too large (%d cylinders); ",
			sblock.fs_cpg);
	...
	if (cgdmin(&sblock, i) - cgbase(&sblock, i) >= sblock.fs_fpg) {
		fprintf(stderr,
		    "sfs mkfs: inode blocks/cyl group (%d) >= data blocks (%d)\n",
	...
	register struct inode *ip;
{
	...
	rdfs(fsbtodb(&sblock, cgtod(&sblock, 0)), sblock.fs_cgsize,
	    (char *)&acg);
		if (acg.cg_magic != CG_MAGIC) {
			fprintf(stderr, "sfs mkfs: cg 0: bad magic number\n");
			exit(RET_BAD_MAGIC);
		}
		acg.cg_cs.cs_nifree--;
		...
		if((int)ip->i_number >= sblock.fs_ipg * sblock.fs_ncg) {
			fprintf(stderr, 
				"sfs mkfs: fsinit: inode value out of range (%d).\n",
			    ip->i_number);
		...
		char *bf;
	{
		...
		if (lseek(fsi, bno * DEV_BSIZE, 0) < 0) {
			fprintf(stderr, "sfs mkfs: seek error: %ld\n", bno);
			perror("rdfs");
			exit(RET_ERR_SEEK);
		}
		n = read(fsi, bf, size);
		if(n != size) {
			fprintf(stderr, "sfs mkfs: read error: %ld\n", bno);
			perror("rdfs");
			exit(RET_ERR_READ);
		}
	}
...
void wtfs(bno, size, bf)
		...
		char *bf;
	{
		...
		n = write(fso, bf, size);
		if(n != size) {
			fprintf(stderr, "sfs mkfs: write error: %ld\n", bno);
			perror("wtfs");
			exit(RET_ERR_WRITE);
		}
	}
...
{
	(void) fprintf (stderr,
	    "sfs usage: mkfs [-F FSType] [-V] [-m] [-o options] special size\n");
	(void) fprintf (stderr,
...
long big;
{
		...
		case ',':
                case '\0':
                        if ((n > big) || (n < 0))
                        {
                                fprintf(stderr, "sfs mkfs: argument out of range: \"%lu\"\n", n);
                                exit(RET_ARG_RANGE);
                        }
			cs--;
			string = cs;
                ...
                default:
                        fprintf(stderr, "sfs mkfs: bad numeric arg: \"%s\"\n", string);
                        exit(RET_ARG_BAD);
...
int ins;
{
	...
	if (Lflag)	/* if -o L option avoid prompting user */
		...
		fprintf(stderr, "sfs mkfs: cannot open /dev/tty\n");
                exit(RET_TTY_OPEN);
	}
        fprintf (ttyp, "WARNING: This file system is able to support more\n");
        fprintf (ttyp, "than 32,768 files. Some older applications (written for UNIX System V\n");
        fprintf (ttyp, "Release 3.2 or before) may not work correctly on such a file system,\n");
        fprintf (ttyp, "even if fewer than 32,768 files are actually present. If you wish to run such\n");
        fprintf (ttyp, "applications (without recompiling them), you should restrict the maximum\n");
        fprintf (ttyp, "number of files that may be created to fewer than 32,768.\n\n");

        while (inos == 0) {
		fprintf(ttyp, "Your choices are:\n\n");
		fprintf(ttyp, "1. Restrict this file system to fewer than 32,768 files.\n");	
		fprintf(ttyp, "2. Allow this file system to contain more than 32,768 files\n");

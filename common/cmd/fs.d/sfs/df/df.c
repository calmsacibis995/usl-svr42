...
#ident	"@(#)sfs.cmds:sfs/df/df.c	1.3.5.4"
	...
	char *file;
{
	...
	if (sblock.fs_magic != SFS_MAGIC) {
		(void) fprintf(stderr, "df: %s: not a sfs file system\n", 
		    file);
		(void) close(fi);
		return;
	}
	...
	if (iflag) {
		...
		reserved = (double) totalblks * sblock.fs_minfree / 100 ;
		avail = free - reserved;
	...
	unsigned int size;
{
	...
	if ((ret = (char *)malloc(size)) == NULL) {
		(void) fprintf(stderr, "df: ran out of memory!\n");
		exit(32);
	}
	return (ret);
}
...
usage()
{
	...
	(void) fprintf(stderr, "sfs usage: df [-F sfs] [generic options] [-o i] [directory | special]\n");
	exit(32);
}

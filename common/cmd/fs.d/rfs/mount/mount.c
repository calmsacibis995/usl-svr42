...
#ident	"@(#)rfs.cmds:rfs/mount/mount.c	1.23.8.6"
#ident	"$Header: $"
...
/* libns */
extern char		*dompart();
extern struct address	*astoa();
extern int		uidmap();
extern int		negotiate();
extern int		att_connect();
extern int		rf_request();
extern void		nserror();
...
extern int		rfsys(int, ...); 	/* undocumented I/F */
...
static char	typename[NAME_MAX];
static char	*myname;
static char	fstype[] = FSTYPE;
static char	mnttab[] = MNTTAB;
static struct 	netconfig *np;
static char	*myopts[] = {
	"ro",
	"rw",
	"nocaching",
	...
	int	rflag;
{
	...
	if (n_name == NULL) {
		...
		Sprintf (netpath, "NETPATH=%s", npath);
		(void)putenv(netpath);
	...
	register char	*bs, *mp;
{
	switch (errno) {
	case EPERM:
		Fprintf(stderr, "%s: not privileged\n", myname);
		break;
	case ENXIO:
	...
	case ENOENT:
		...
		break;
	case EINVAL:
		Fprintf(stderr, "%s:\t%s is an invalid resource,\n",myname,bs);
		Fprintf(stderr, "\tor invalid input,\n");
		Fprintf(stderr, "\tor trying loopback without loopback mode.\n");
		break;
	case EBUSY:
		Fprintf(stderr, "%s:\t%s is already mounted,\n", myname, bs);
		Fprintf(stderr, "\tor %s is busy,\n", mp);
		Fprintf(stderr, "\tor remote machine at NSRMOUNT limit.\n");
		break;
	case ENOTBLK:
	...
	struct rf_token	*token;
{
	...
	if (rf_request(fd, RF_RF) == -1) {
 	...
 	if ((gdpmisc.version =
 	    negotiate(fd, &mypasswd[0], CLIENT, &gdpmisc.ngroups_max,
	...
	char	*mach_name;
{
	...
	Sprintf(netmaster, TPNETMASTER, netspec);
	Sprintf(dommaster, TPDOMMASTER, netspec);
	file[0] = netmaster;
	file[1] = dommaster;
...
{
	Fprintf(stderr,
	"%s Usage:\n%s [-rc] [-o specific_options] {special | mount_point}\n",
		fstype, myname);
	Fprintf(stderr,
		   "%s [-rc] [-o specific_options] special mount_point\n",
			myname);

...
#ident	"@(#)s5.cmds:s5/ff.c	1.24.1.9"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/fs.d/s5/ff.c,v 1.1 91/02/28 17:26:08 ccs Exp $"
/*
...
char *dev;
{
	...
	if (runopt&F_REQUEST)	/* If -i used, don't display total number of links detected */
		;
	else if (runopt&F_LINKSTOO)
...
int super()
{
	...
	retry = 0;
again:	
	have = ulimit(3,0) - sbrk(0) - 16000;	/* get memory available */
...
monikers()
{
	...
	if (runopt&F_LINKSTOO) {
		...
		for (i=0; i<nLink; i++) {
			...
			if (runopt&F_REQUEST)
				for (j=0; inode_list[j] != -1; j++)
					if (Linklist[i].inum == inode_list[j])

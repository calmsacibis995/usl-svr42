...
#ident	"@(#)s5.cmds:s5/fsdb.c	1.27.1.9"
#ident "$Header: fsdb.c 1.2 91/09/30 $"
		...
		for(i=0; count--; ip++) {
			...
			case S_IFNAM:
				...
				if((ip->di_mode & S_IFMT) == S_IFCHR ||
				    (ip->di_mode & S_IFMT) == S_IFBLK) {
					l3tol(&tmp,&ip->di_addr[3*3],1);
					printf("maj:%3.3o  min:%3.3o  ",
					    ip->di_addr[2*3] & 0377, tmp & 0177777);
				}
				else {
					for(i = 0; i < NADDR; i++) {
 ...
 */
	    ...
	    char	*fs;
	{
		...
		fgets(blocksize,sizeof(blocksize),stdin);

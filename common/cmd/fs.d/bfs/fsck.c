...
#ident	"@(#)bfs.cmds:bfs/fsck.c	1.9.7.4"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/fs.d/bfs/fsck.c,v 1.1 91/02/28 17:20:19 ccs Exp $"
...
int argc;
{
	...
	if((statarea.st_mode & S_IFMT) == S_IFBLK) {
		...
		if ((statarea.st_flags & _S_ISMOUNTED) && !nflag) {
			fprintf(stderr, "bfs %s: %s file system is mounted as a block device, ignored\n", argv[0], special);

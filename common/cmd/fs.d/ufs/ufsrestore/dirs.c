...
#ident	"@(#)ufs.cmds:ufs/ufsrestore/dirs.c	1.4.6.3"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/fs.d/ufs/ufsrestore/dirs.c,v 1.1 91/02/28 17:30:26 ccs Exp $"
	...
	int size;
{
	...
	if (cvtflag) {
		...
		for (loc = 0; loc < size; ) {
			...
			if (dp->d_reclen == 0 || (int) dp->d_reclen > i) {
				loc += i;
				continue;
			}
			loc += dp->d_reclen;
	...
	struct direct *dp;
{
	...
	if (dirloc + (int) dp->d_reclen > DIRBLKSIZ) {
	...
	register RST_DIR *dirp;
{
	...
	for (;;) {
		...
		if (dp->d_reclen == 0 ||
		    (int) dp->d_reclen > DIRBLKSIZ + 1 - dirp->dd_loc) {

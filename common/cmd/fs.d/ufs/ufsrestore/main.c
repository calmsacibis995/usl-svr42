...
#ident	"@(#)ufs.cmds:ufs/ufsrestore/main.c	1.4.6.2"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/fs.d/ufs/ufsrestore/main.c,v 1.1 91/02/28 17:30:34 ccs Exp $"
...
usage:
	...
	case 'r':
		...
		if (dumptime > 0) {
			...
			treescan(".", SFSROOTINO, nodeupdates);
			findunreflinks();
			removeoldnodes();
		} else {
			/*
		...
		if (dflag) {
			...
			treescan(".", SFSROOTINO, verifyfile);
		}

...
#ident	"@(#)ufs.cmds:ufs/ufsrestore/symtab.c	1.2.6.3"
#ident "$Header: symtab.c 1.2 91/09/30 $"
/*
	...
	ino_t inum;
{
	...
	if (inum < SFSROOTINO || inum >= maxino)
		return (NIL);
	...
	char *name;
{
	...
	cp = name;
	for (ep = lookupino(SFSROOTINO); ep != NIL; ep = ep->e_entries) {
		for (np = buf; *cp != '/' && *cp != '\0'; )
			*np++ = *cp++;
		*np = '\0';
	...
	int type;
{
	...
	if (ep == NIL) {
		if (inum != SFSROOTINO || lookupino(SFSROOTINO) != NIL)
			panic("bad name to addentry %s\n", name);
		np->e_name = savename(name);
		np->e_namlen = strlen(name);
		np->e_parent = np;
		addino(SFSROOTINO, np);
		return (np);
	}
...
static struct strhdr strtblhdr[allocsize(SFS_MAXNAMLEN) / STRTBLINCR];

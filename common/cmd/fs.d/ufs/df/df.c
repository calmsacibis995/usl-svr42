...
#ident	"@(#)ufs.cmds:ufs/df/df.c	1.11.8.4"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/fs.d/ufs/df/df.c,v 1.1 91/02/28 17:28:03 ccs Exp $"
		...
		for (mntl = mkmntlist(); mntl != NULL && num ; 
		   ...
		   for (i = optind; i < argc; i++) {
			...
			if (stat(argv[i], &filestat) < 0) {
				pfmt(stderr, MM_ERROR, badstat, argv[i],
					strerror(errno));
				argv[i] = NULL ;
				--num;
			} else {
...
{
	if (nflag)
		(void) pfmt(stdout, MM_NOSTD, ":303:VFStype name - ufs\n");
	if (iflag)
		if (eflag)
...
dfreedev(file)
	char *file;
{
	long totalblks, availblks, avail, free, used, reserved;
	int fi;
	...
	if (sblock.fs_magic != UFS_MAGIC) {
		(void) pfmt(stderr, MM_ERROR, ":310:%s: not a ufs file system\n", 
		    file);
		(void) close(fi);
		return;
	}
	...
	if (iflag) {
		...
		reserved = totalblks * sblock.fs_minfree / 100 ;
		avail = free - reserved;
		...
		if (bflag) {
			...
			(void) printf("%8d%8d%8d",
			    ...
			    availblks * sblock.fs_fsize / 1024);
			(void) printf("%6.0f%%",
			    totalblks ? (double)used/(double)totalblks * 100.0 : 0 );
			(void) printf("  ");
		}
		if (tflag) {
	...
	struct mnttab *mnt;
{
	...
	if ((int)strlen(mnt->mnt_special) > 20) {
		(void) printf("%s\n", mnt->mnt_special);
		(void) printf("                    ");
	} else {

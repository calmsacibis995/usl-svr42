 ...
 *  
 */
#ident	"@(#)ufs.cmds:ufs/ufsdump/dumptape.c	1.8.5.4"
#ident "$Header: dumptape.c 1.2 91/09/30 $"
...
ufsrewind()
{
	...
	if (pipeout)
		return;
	/*
	 * msg("Tape rewinding\n");  * this message makes no sense for 
	 *	no rewinding tapes, so we removed it.
	 */
	close(to);
...
/* Little wrapper for dumpabort to shut up some compiler warnings;
   This wrapper just makes it look like what 'signal' 
   wants as an argument 
*/
void dumpabort_wrapper(dummy)
	/* ARGSUSED */
	int	dummy;
{
	dumpabort();
...
enslave()
{
	...
	master = getpid();
	signal(SIGTERM, dumpabort_wrapper); /* Slave sends SIGTERM on dumpabort() */

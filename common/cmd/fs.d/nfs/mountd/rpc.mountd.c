...
#ident	"@(#)nfs.cmds:nfs/mountd/rpc.mountd.c	1.11.8.3"
#ident	"$Header: $"
...
#define	MOUNTPROG program
	...
	while ((c = getopt(argc, argv, "np:")) != (char)EOF)
		...
		case 'p':
			program = atoi(optarg);
			if (program <= 0) {
				fprintf(stderr, "mountd: bad program number\n");
				exit(1);
			}
			break;
		case 'n':
	...
	SVCXPRT *transp;
{
	...
	static struct nd_hostserv     local_hs;
	static char local_hname[] = "localhost";
	static char local_sname[] = "";
	int	x;
	char	myname[256];
	...
	local_hs.h_host = local_hname;
	local_hs.h_serv = local_sname;
	local_hsl.h_cnt = 1;
	local_hsl.h_hostservs = &local_hs;
	...
	/* return that its the localhost. Later we can then check */
	/* to see if its localhost and use the authentication */
	/* information to obtain the real name of the remote */
	/* machine. This is as swtchd will mount on behalf of */
	/* a remote machine and will thus appear to be local */

	for(x=0; x < serv->h_cnt; x++) {

		if (!strcmp(serv->h_hostservs[x].h_host,"localhost"))
			return(&local_hsl);
	...
	struct svc_req *rqstp;
{
	...
	if (!strcmp(name,"localhost"))
	{
		...
		name = ((struct authunix_parms *)rqstp->rq_clntcred)->aup_machname; 
	} 
	...
	struct svc_req *rqstp;
{
	...
	if (!strcmp(name,"localhost"))
	{
		/* possibly come through the swtchd which will */
		/* be doing work on behalf of a remote machine */
		/* thus use the name used in the authentication */
		/* hope they always use auth unix */
...
usage()
{
	(void) fprintf(stderr, "Usage: mountd [-n] [ -p rpc-prog]\n");
	exit(1);
}

...
#ident	"@(#)nfs.cmds:nfs/pcnfsd/pcnfsd.c	1.1.9.5"
#ident	"$Header: $"
	...
	char           *Argv[];
{
	...
	if (!ioctl(0, I_LOOK, mname) &&
		...
		assert(bindresvport(nconf, fd, baddr) >= 0);
	...
	struct t_bind *baddr;
{
	...
	(void) memset((char *) baddr->addr.buf, 0, baddr->addr.len);
	sin = (struct sockaddr_in *) baddr->addr.buf;
	sin->sin_family = AF_INET;

	baddr->qlen = 0;
	baddr->addr.len = baddr->addr.maxlen;
	...
	for (i = 0; i < NPORTS && errno == EADDRINUSE; i++) {
		...
		if (port > ENDPORT)
			port = STARTPORT;
		res = t_bind(fd, baddr, tres);
		if ((res == 0) && (memcmp(baddr->addr.buf, tres->addr.buf,
					(int) tres->addr.len) == 0))
			break;
	}

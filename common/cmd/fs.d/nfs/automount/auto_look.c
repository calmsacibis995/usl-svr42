...
#ident	"@(#)nfs.cmds:nfs/automount/auto_look.c	1.6.5.3"
#ident "$Header: $"
	...
	struct authunix_parms *cred;
{
	...
	if (me == NULL) {
		/*
		 * Yes, this assumes that uid 0 is privileged. However, there
		 * is no other solution in this case, since nothing in the
		 * contents of an authunix_parms structure gives any hint of
		 * what privileges the user has. (maybe we are checking for the
		 * privilege to set one's uid to 0)
		 */

...
#ident	"@(#)nfs.cmds:nfs/mount/mount.c	1.18.13.9"
#ident	"$Header: mount.c 1.3 91/09/20 $"
	...
	char **argv;
{
	...
	if ((ret = check_nfs())) {
		if (ret == 2) {
			fprintf(stderr, "nfs_mount: nfs is not running, cannot mount %s\n", argv[optind+1]);
			exit(RET_NFSDOWN);
		} else if (ret == 3) {
			fprintf(stderr, "nfs_mount: rpcbind is not running, cannot mount %s\n", argv[optind+1]);
			exit(RET_NFSDOWN);
		}
		/*
	...
	char *fshost, *fspath;
{
	...
	cl = clnt_create(fshost, MOUNTPROG, MOUNTVERS, "datagram_v");
	if (cl == NULL) {
		pr_err("%s:%s: server not responding for umount %s\n",
			fshost, fspath, clnt_spcreateerror(""));
		return (RET_RETRY);
	}
	if (bindresvport(cl) < 0) {
	...
	cl->cl_auth = authsys_create_default();
	rpc_stat = clnt_call(cl, MOUNTPROC_UMNT, xdr_path, &fspath,
...
check_nfs()
{
	...
	if ((stat = rpcb_rmtcall(nconf_udp, "localhost", RPCBPROG,
		RPCBVERS, NULLPROC, xdr_void, &argp, xdr_void, &res, timeout,

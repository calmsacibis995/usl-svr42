...
#ident	"@(#)nfs.cmds:nfs/lockd/flk_reclox.c	1.2.5.6"
#ident	"$Header: $"
	...
	struct	reclock	*fl;
{
	...
	new->lck.lox.lld.l_pad[0] = lckdat->lck.lox.lld.l_pad[0];
	new->w_flag = 0;
	new->sleep_id = locksleep_id++;
	if (fl == NULL) {
		if (lck_list != NULL)
			new->next = *lck_list;
		else
	...
	while (flp = nflp) {
		...
		if( SAMEOWNER(&(flp->lck.lox.lld), &(ld->lck.lox.lld)) ) {
			...
			case S_BEFORE|E_MIDDLE:
				...
				if (ld->lck.lox.lld.l_type == flp->lck.lox.lld.l_type)
					if (flp->lck.lox.lld.l_len &&
							flp->lck.lox.lld.l_len != MAXEND)
						ld->lck.lox.lld.l_end = END(&flp->lck.lox.lld)
								- ld->lck.lox.lld.l_start + 1;
					else
						ld->lck.lox.lld.l_end = MAXEND - 
					...
					if (ld->lck.lox.lld.l_len &&
							ld->lck.lox.lld.l_len != MAXEND)
						td.lck.lox.lld.l_start =
							END(&ld->lck.lox.lld) + 1;
					else 
						td.lck.lox.lld.l_start = MAXEND;
					td.block = flp->block;
        				td.exclusive = flp->exclusive;
        				td.reclaim = flp->reclaim;
        				td.state = flp->state;
        				td.rel = flp->rel;
        				if (debug)
        				...
        				td.lck.op = flp->lck.op;
        				td.lck.svid = flp->lck.svid;
        				if (debug)
                				printf("Before copying td.lck.oh...\n");
        				obj_copy(&td.lck.oh, &ld->lck.oh);
        				td.lck.clnt = flp->lck.clnt;
        				td.lck.caller_name = flp->lck.caller_name;
        				td.lck.lox.granted = flp->lck.lox.granted;
        				td.lck.lox.color = flp->lck.lox.color;
        				td.lck.lox.LockID = flp->lck.lox.LockID;
        				td.lck.lox.class = flp->lck.lox.class;
        				if (debug)
                				printf("Before copying ld->lck.lox.lld...\n");
        				td.lck.lox.lld.l_sysid = flp->lck.lox.lld.l_sysid;
        				td.lck.lox.lld.l_len = END(&flp->lck.lox.lld) -
							END(&ld->lck.lox.lld);
        				td.lck.lox.lld.l_whence =
							flp->lck.lox.lld.l_whence;
        				td.lck.lox.lld.l_pid = flp->lck.lox.lld.l_pid;
        				td.lck.lox.lld.l_pad[0] = flp->lck.lox.lld.l_pad[0];
        				td.w_flag = 0;
					tdp = tdi = flp;
					do {
						if (debug)
					...
					if (flp->lck.lox.lld.l_len &&
							flp->lck.lox.lld.l_len != MAXEND)
                                        	td.lck.lox.lld.l_len =
							END(&flp->lck.lox.lld) - 
							td.lck.lox.lld.l_start + 1;
					else
						td.lck.lox.lld.l_len = MAXEND - 
                                                        td.lck.lox.lld.l_start + 1;
					if (debug)
        ...
        struct reclock **insrt;
{
        ...
        *insrt = NULL;
		...
		/*
                 * Get ride of the following cases:
                 *
                 *      (1) f      [-----]
                 *                         [-----] lckdat
                 *
                 ...
                 *
                 */
                if (END(&f->lck.lox.lld) < lckdat->l_start ||
                        f->lck.lox.lld.l_start > END(lckdat))
                        continue;
                	...
                	if( SAMEOWNER(&(f->lck.lox.lld), lckdat) ) {
                        	if ((lckdat->l_start-1) <= END(&f->lck.lox.lld))
                                ...
                                || (f->lck.lox.lld.l_type == F_RDLCK && lckdat->l_type == F_WRLCK))) {
                                        return f;
                	} else if (f->lck.lox.lld.l_type == F_WRLCK || 
                    		(f->lck.lox.lld.l_type == F_RDLCK && lckdat->l_type == F_WRLCK))
                        	return f;
		} else {
			if( SAMEOWNER(&(f->lck.lox.lld), lckdat) ) {
...
int
deadflck_reclox(flp, lckdat)
	struct reclock *flp;
	struct flock *lckdat;
{
	register struct reclock *blck, *sr;
	short blckpid, blcksysid;

	if (debug) {
		printf("enter deadflck_reclox(): start %d len %d type %d\n",
			lckdat->l_start, lckdat->l_len, lckdat->l_type);
		printf("enter deadflck_reclox(): pid %d, sysid %d\n",
			lckdat->l_pid, lckdat->l_sysid);
	}

	/*
	 * current blocking lock pointer
	 */
	blck = flp;
	blckpid = blck->lck.lox.lld.l_pid;
	blcksysid = blck->lck.lox.lld.l_sysid;
	if (debug)
		printf("pid is %d, sysid is %d\n", blckpid, blcksysid);
	...
	do {
		...
		for (sr = sleep_q; sr != NULL; sr = sr->next) {
			if (blckpid == sr->lck.lox.lld.l_pid
			  && blcksysid == sr->lck.lox.lld.l_sysid) {
				blckpid = sr->lck.lox.lls.blk.pid;
				blcksysid = sr->lck.lox.lls.blk.sysid;
				if (debug)
					printf("found!: pid is %d, sysid is %d\n",
					blckpid, blcksysid);
				break;
			}
		}

...
#ident	"@(#)nfs.cmds:nfs/lockd/prot_pklm.c	1.8.7.4"
#ident	"$Header: $"
...
extern remote_result 	*remote_test();
extern remote_result 	*remote_lock();
extern remote_result 	*remote_unlock();
extern remote_result 	*remote_cancel();
	...
	remote_result *(*remote)();
{
	...
	if (debug) {
		printf("entered klm_msg_routine(proc =%d): op=%d, (%d, %d) by ",
	...
	if ((msgp = retransmitted(a, proc)) != NULL) {
		if (debug)
			printf("klm_msg_routine(): retransmitted msg\n");
		/*
		...
		if (msgp->reply == NULL) {
			/*
			 * record last received klm msg and do not reply
			 */
			if (debug)
			  printf("klm_msg_routine(): retrans msg, reply null\n");
			klm_msg = msgp;
			return;
		} else {
			if (debug)
			  printf("klm_msg_routine(): retrans msg, reply present\n");
			if (msgp->reply->lstat != nlm_blocked) {
				if (debug)
				  printf("klm_msg_routine(): retrans msg, replied\n");
				klm_reply(proc, msgp->reply);
				dequeue(msgp);
			} else {
				/*
				 * lock blocked, do not reply
				 */
				if (debug)
				  printf("klm_msg_routine(): retrans msg, not replied\n");
				klm_msg = msgp;
			}
			return;
		}
		...
		if (proc == KLM_LOCK && !remote_data(a) &&
			 ...
			 */
			if (debug)
				printf("klm_msg_routine(): retransmitted, lbl (%x)\n, nl");
			a->rel = 1;
	...
	/*
	 * reset reply sent as add_reply() called from remote()
	 * may set it.
	 */
	reply_sent = 0;

	result = remote(a, proc);
	...
	/*
	 * reply only if result is present and
	 * if add_reply() has not called klm_reply()
	 * already and if status is non blocked
	 */
	if ((result != NULL) && !reply_sent && result->lstat != nlm_blocked)  {
	...
	SVCXPRT *Transp;
{
	...
	if ((req = get_reclock()) != NULL) {
		if (!svc_getargs(Transp, xdr_Argument, klm_req)) {
			...
			req->rel = 1;
			release_reclock(req);
			release_klmargs(Rqstp->rq_proc, klm_req);
			(void) sigsetmask(oldmask);
			return;
		}
		switch (Rqstp->rq_proc) {
...
release_klmargs(proc, req)
int proc;
caddr_t req;
{
	switch (proc) {
	case KLM_TEST:
		release_klm_testargs(req);
		break;
	case KLM_LOCK:
	case KLM_CANCEL:
		release_klm_lockargs(req);
		break;
	case KLM_UNLOCK:
		release_klm_unlockargs(req);
		break;
	}
}

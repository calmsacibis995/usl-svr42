...
#ident	"@(#)nfs.cmds:nfs/lockd/prot_priv.c	1.5.6.3"
#ident	"$Header: $"
...
#define	F_UNLKSYS	04	/*
				 * should really be removed once sys/fcntl.h
				 * gets updated
				 */
	...
	struct status *statp;
{
	...
	if ((hp = gethostbyname(statp->mon_name)) == NULL) {
        ...
        ld.l_pid = getpid();
        if (debug) {
                printf("ld.l_start=%d ld.l_len=%d ld.l_pid=%d ld.l_sysid=%x\n",
                        ld.l_start, ld.l_len, ld.l_pid, ld.l_sysid);
	}
        if ((err = fcntl(fd, cmd, &ld)) == -1) {
                perror("fcntl");
                printf("lockd: unable to clear a lock. \n");
        }
        close(fd);

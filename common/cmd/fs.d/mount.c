...
#ident	"@(#)fs.cmds:common/cmd/fs.d/mount.c	1.82.1.31"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/fs.d/mount.c,v 1.1 91/02/28 17:20:54 ccs Exp $"
 ...
 */
#define	FORMAT	"%a %b %e %H:%M:%S %Y"	/* date time format */
	...
	char	*argv[];
{
	...
	if (oflg != 0 && oflg != 1) {
		fprintf(stderr, "UX:%s:ERROR: more than one 'o' flag was specified\n", myname);
		usage();
		exit(RET_OPTIONS);
	}
	...
	if (pflg && vflg) {
		fprintf(stderr, "UX:%s:ERROR: invalid combination of options -p & -v\n", myname);
		usage();
		exit(RET_OPTIONS);
	}
	...
	if (zflg && Zflg) {
		fprintf(stderr, "UX:%s:ERROR: invalid combination of options -z & -Z\n", myname);
		usage();
		exit(RET_OPTIONS);
	}
	...
	if (dflg + fflg + Fflg > 1) {
		fprintf(stderr, "UX:%s:ERROR: more than one FSType specified\n", myname);
		usage();
		exit(RET_FSTYPE_ONE);
	}
	...
	if (optind != argc) {
		if (pflg || vflg) {
			fprintf(stderr, "UX:%s:ERROR: cannot use -p or -v with arguments\n", myname);
			usage();
			exit(RET_ARGS);
		}
		else if (zflg || Zflg) {

			fprintf(stderr, "UX:%s:ERROR: cannot use -z or -Z with arguments\n", myname);
			usage();
			exit(RET_ARGS);
		}
	}
	 ...
	 */
		...
		if (ret == 0) {
			...
			if ((ij =stat(mountp, &stbuf)) == -1) {
				fprintf(stderr, "%s: cannot stat mount point %s\n", 
					myname, mountp);
				exit(RET_MP_STAT);
			}
	...
	if (mac_install) {
		if (lflg)
			...
			if (devalloc(special, DEV_GET, &devddb) < 0) {  
				switch(errno) {
				case ENODEV:
					fprintf(stderr, "UX:%s:ERROR:%s Device not found in Device Database\n", myname, special);
					exit(RET_DDB_NOENT);
					break;
				case EINVAL:
					fprintf(stderr, "UX:%s:ERROR: security attributes for %s missing or invalid in Device Database\n", myname, special);
					exit(RET_DDB_INVAL);
					break;
				default:
					fprintf(stderr, "UX:%s:ERROR: Device Database is inaccessible\n", myname);
					exit(RET_DDB_ACCESS);
					break;
				}
			}
			...
			if ((lflg || vfsflag) &&
				...
				fprintf(stderr, "UX:%s:ERROR: level ceiling not within device range\n", myname);
				exit(RET_LVL_RANGE);
			}
		}
	}
	if ((pid = fork()) == -1) {
		fprintf(stderr, "UX:%s:ERROR: cannot fork\n", myname);
		exit(RET_FORK_ERR);
	}
	else if (pid == 0) {
	...
	/* parent   -- wait for completion of child */
		...
		if (mac_install && (!chexitval)) {
			...
			if ((lflg || vfsflag) && 
				...
				fprintf(stderr, "UX:%s:WARNING: could not set new ceiling on mounted file system\n", myname);
			}
		}
		else exit(chexitval); 
	}
	...
	char	*fstype, *newargv[];
{
	...
	if ((sem_file = creat(SEM_FILE,0600)) == -1 || lockf(sem_file,F_LOCK, 0L) <0 ) {
		if ((errno == EACCES) || (errno == EPERM)) {
			...
			fprintf(stderr,
				"mount: cannot lock temp file <%s>: %s\n",
				SEM_FILE, strerror(errno));
			exit(RET_MNT_LOCK);
		}
	}
		...
		while(lvlout(&tmp_lid, lvl_name, lvl_namez, lvlformat) == -1) {
			if ((lvlformat == LVL_FULL) && (errno == ENOSPC)) {
				...
				if ((tmp_name = malloc(lvl_namez*2)) == (char *)NULL) {
					fprintf(stderr,
	"UX:%s:ERROR: no memory to print level ceiling of file system %s\n", myname, name);
					exit(RET_MALLOC_ERR);
				}
			lvl_namez *= 2;
			if (lvl_name !=lvl_buf)
			...
			lvl_name = tmp_name;
				...
				fprintf(stderr,
			"UX:%s:ERROR: cannot translate level to text format %s \n", myname, name);
				return(1);
			}
		}
		printf(" %s", lvl_name);
		...
		fprintf(stderr, "UX:%s:ERROR: permission denied\n", myname);
		return(1);
	}
	return(0);
}
...

{
	...
	if (lvlin(ceilingp, levelp) == -1) {
		...
		case EINVAL:	/* invalid security level specified */
			fprintf(stderr, "UX:%s:ERROR: invalid security level specified\n", myname);
			exit(RET_LVL_INVAL);
			break;
		case EACCES:	/* no access to LTDB */
			fprintf(stderr, "UX:%s:ERROR: LTDB is inaccessible\n", myname);
			exit(RET_LTDB_ACCESS);
			break;
		default:
			fprintf(stderr, "UX:%s:ERROR: error in validating level ceiling,  errno =%d\n", myname, errno);
			exit(RET_CEILING);
			break;
		}
	}
...

{
	...
	if (lvlfile(mountp, MAC_GET, &mountplid) == -1) {
		fprintf(stderr, "UX:%s:ERROR: cannot get level of mount point\n", myname);
		exit(RET_MP_UNKLVL);
	}
	/* 
	...
	if (!((lvldom(&(devddbp->hilevel), &mountplid) > 0) &&
		...
		fprintf(stderr, "UX:%s:ERROR: mount point level not within device range\n", myname);
		exit(RET_MP_RANGE);
	}	

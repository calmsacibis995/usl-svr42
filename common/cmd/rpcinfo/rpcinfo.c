...
#ident	"@(#)rpcinfo:rpcinfo.c	1.7.11.3"
#ident  "$Header: rpcinfo.c 1.2 91/06/27 $"
	...
	char **argv;
{
	...
	if (argc == 1) {
		...
		uname(&utsname);
		get_inet_address(&server_addr, utsname.nodename);
	}
	...
	char **argv;
{
	...
	minutetimeout.tv_sec = 60;
	minutetimeout.tv_usec = 0;
	if ( (rpc_stat = CLNT_CALL(client, RPCBPROC_DUMP, xdr_void, NULL,
		xdr_rpcblist, &head, minutetimeout)) != RPC_SUCCESS) {

		if (rpc_stat == RPC_PROGVERSMISMATCH) {
			printf("rpcinfo: can't contact rpcbind, trying portmapper\n");
			pmapdump(argc, argv);
		} else
	...
	"   program version netid address	  service    owner\n");
		for (; head != NULL; head = head->rpcb_next) {
			...
			printf("%-16s", head->rpcb_map.r_addr);

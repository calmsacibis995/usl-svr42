...
#ident	"@(#)ksh:sh/expand.c	1.4.6.7"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/ksh/sh/expand.c,v 1.1 91/02/28 17:40:57 ccs Exp $"
...
register char *rescan;
{
	...
	{
		int offset;
		if(!S_ISDIR(test_mode(ap->argval)))
			return;
		stakputc('/');
		offset = staktell();
	...
	{
#ifdef KSHELL
		if(!endslash && is_option(MARKDIR) && S_ISDIR(test_mode(ap->argval)))
			stakputc('/');
#endif /* KSHELL */

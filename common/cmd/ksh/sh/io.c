...
#ident	"@(#)ksh:sh/io.c	1.5.6.4"
#ident "$Header: /sms/sinixV5.4es/rcs/s19-full/usr/src/cmd/ksh/sh/io.c,v 1.1 91/02/28 17:41:04 ccs Exp $"
		...
		if(*ion)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						struct stat st;
						if(stat(ion,&st)>=0 && S_ISREG(st.st_mode) && st.st_rdev==0)
							sh_fail(ion,e_fexists);
					}
#else
					else if(is_option(NOCLOB) && S_ISREG(test_mode(ion)))
						sh_fail(ion,e_fexists);
#endif /* FS_3D */
				}
			openit:

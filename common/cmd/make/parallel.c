...
DEPBLOCK depp;
	...
	if(!shenvp)
		fatal1("No shenv to restore tar=%s, done=%d\n",tar->namep, tar->done);
...
try_again:
		...
		if(p){
			...
			if(!nproc && not_done)
				fatal("Internal ERROR: Some dep. didn't built");
		}
...
NAMEBLOCK tarp;
{
	...
	if ( status >> 8 )
		fprintf(fp, "*** Error code %d", status >> 8 );
	else
		fprintf(fp, "*** Termination code %d", status );
	...
	if(IS_ON(BLOCK)){
		if(!tarp->tbfp){
			...
			if((fp = tarp->tbfp = fopen(tmp_block, "w+")) == NULL)
				fatal1("Fail to open <%s>", tmp_block);

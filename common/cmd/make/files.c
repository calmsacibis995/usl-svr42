...
char	*an, *name;
{
	...
	fatal("Cannot do global variable search in old object file.");
	if (len > 8L)
		len = 8L;
	/*
...
seek_error:	fatal1("seek error on archive %s", an);
	...
	while (fread((char *) & ar_old, sizeof(ar_old), 1, arfd) == 1) {
		...
		if (ar_old.ar_size < sizeof(arobj_old) || 
			...
			fatal1("%s is not an object module (bu42)", ar_old.ar_name);
...
register CHARSTAR f;
{
	...
	if ( !(arfd = fopen(f, "r")) ){
		if(errno == ENOENT )
			...
			fatal1("cannot open %s for reading\n", f);
	}
	/*

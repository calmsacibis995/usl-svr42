...
time_t	*tval;
{
	...
	*	modification times of dependents	*/
	...
	if (IS_ON(DBUG)) {
		blprt(reclevel);
		printf("doname(%s,%d) done=%d\n", p->namep, reclevel, p->done);
		(void)fflush(stdout);
	}
#endif
...
*/
			...
			if (lp->shp) { /* shell line specified? */
				if (explcom){	/*  specified before? */
					if(IS_OFF(WARN))
						fprintf(stderr, "Warning: Too many command lines for `%s' (bu10)\n",
					    p->namep);
				}else
					explcom = lp->shp;
			}
		...
		if (explcom)
			...
			if (p1 = SRCHNAME(".DEFAULT")) {
				...
				for (lp2 = p1->linep; lp2; lp2 = lp2->nextline)
					if (implcom = lp2->shp)
						errstat += docom(p, implcom, p->linep?p->linep->depp:NULL);
			} else if ( !(IS_ON(GET) && get(p->namep, 0)) )
...
p_error:	if(IS_OFF(WARN))
						fprintf(stderr, "Warning: $! nulled, predecessor cycle\n");
				ballb[0] = CNULL;
				return;
			}
		circles[npreds++] = npp;
...
register NAMEBLOCK p;
	...
	if(p){
				if(IS_OFF(WARN))
						fprintf(stderr, "Warning: predecessor cycle (%s)\n", dep->namep);
	}else
		dep->backname = tar;
}

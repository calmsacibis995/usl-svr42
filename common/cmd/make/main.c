...
CHARSTAR argv[];
{
	...
	*/
	...
	if ( !sufflist ) 
		if(IS_OFF(WARN))
			fprintf(stderr, "Warning: No suffix list.\n");
...
char c;
{
	...
	fprintf(stderr, "[-d] ");
#endif
	fprintf(stderr, "[-w] [-P] [-e] [-t] [-q] [names]\n");
	mkexit(1);
}

...
#ident	"@(#)deroff:deroff.c	1.9.1.5"
#ident "$Header: deroff.c 1.2 91/04/04 $"
...
char **av;
{
	...
	if ((line = (char *)malloc(linesize)) == (char *)NULL)
		fatal("out of memory\n", (char *) NULL); /* exits */
...
int cnst;
{
	...
	*line = c;
	lp = line;
	for( ; ; ){
		if ((unsigned)(lp - line) >= linesize-1){
			/* processing last character of current line buffer.
			 * Expand size of buffer
			 */
			if (linesize <= MAXLINELEN){
				linesize += SIZEDELTA;
				save_p = line;
				if ((line = realloc(line, linesize)) == 0)
					fatal("out of memory\n", (char *)NULL);
				lp = line + (lp - save_p);
			} else
...
comx:
	...
	} else if (msflag && (c1 == 'D' || c1 == 'N' || c1 == 'K'|| c1 == 'P')
		   && c2 == 'S'){

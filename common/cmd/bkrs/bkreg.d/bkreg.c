...
#ident	"@(#)bkrs:common/cmd/bkrs/bkreg.d/bkreg.c	1.15.11.4"
#ident  "$Header: bkreg.c 1.2 91/06/21 $"
...
synopsis()
{
	...
	(void)fprintf( stderr,
		...
		brcmdname );
	(void)fprintf( stderr,
		"%s -e tag [-o orig] [-c weeks:days|demand] [-m method] [-d ddev] [-t table] [-b moptions] [-D depend] [-P prio]\n",
		brcmdname );
	(void)fprintf( stderr,
		"%s -r tag [-t table]\n", brcmdname );
	(void)fprintf( stderr,
		"%s [-A | -O | -R] [-hsv] [-t table] [-c weeks[:days]|demand]\n",
		brcmdname );
	(void)fprintf( stderr,
		"%s -C fields [-hv] [-t table] [-c weeks[:days]|demand] [-f c]\n",
		brcmdname );
}
...
hor_orig_print()
{
	...
	while( selectline( entry ) != FALSE ) {
		...
		if (odevice == NULL)
			dev_change = 0;
		else if (prev_odevice == NULL)
			dev_change = 1;
		else if( strcmp( (char *)odevice, (char *)prev_odevice ) != 0 ) 
			dev_change = 1;
...
overt_print()
{
	...
	while ( !done ) {
		while ( (colcnt < ncol) &&
			...
			if (odevice == NULL)
				dev_change = 0;
			else if (prev_odev == NULL)
				dev_change = 1;
			else if (strcmp((char *)odevice, (char *)prev_odev) != 0)
				dev_change = 1;

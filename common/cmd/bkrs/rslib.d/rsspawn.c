...
#ident	"@(#)bkrs:common/cmd/bkrs/rslib.d/rsspawn.c	1.6.5.4"
#ident  "$Header: rsspawn.c 1.2 91/06/21 $"
...
int flags;
{
	...
	char *argv[ MAXARGS ], *freelist[ MAXARGS ], *ddevice, name[ 30 ], *dchar1, *dchar2;
	int status;
	...
	/* Pre-pend escape char '\\' to any ':' in dchar */
	if( !(dchar1 = dchar2 = (char *)malloc( (unsigned int)  strlen( dchar ) 
		+ 30 /* spare to carry ':' escape char '\\' */ ) ) ) {
		free( dchar );
		return( BKNOMEMORY );
	} else {
		/* escape */
		for ( ddevice = dchar; *ddevice != '\0';  ) {
			if( *ddevice == ':' )  *dchar2++ = '\\';
			*dchar2++ = *ddevice++;
		}
		*dchar2 = '\0';
	...
	if( !(ddevice = (char *)malloc( (unsigned int) strlen( ddev )
		+ (unsigned int) strlen( dchar1 ) + (unsigned int) strlen( dmnames )
		+ 4 ) ) ) {
		free( dchar );
		return( BKNOMEMORY );
	}
	(void) sprintf( ddevice, ":%s:%s:%s", ddev, dchar1, dmnames );

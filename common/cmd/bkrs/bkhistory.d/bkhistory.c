...
#ident	"@(#)bkrs:common/cmd/bkrs/bkhistory.d/bkhistory.c	1.8.8.4"
#ident  "$Header: bkhistory.c 1.2 91/06/21 $"
...
/*  malloc space for the first 100 entryq ptrs  */
	entryqptr = (ENTRY *) malloc(newsize);
/*  malloc space for the first date pointers  */
	idates = (time_t *) malloc(idatesize);
...
nextentry()
{
	...
	if ( (entrycnt > 0) && (entrycnt % EMALLOCSZ == 0) ) {
		newsize += (EMALLOCSZ * sizeof(ENTRY));
		entryqptr = (ENTRY *)realloc( (char *)entryqptr, (unsigned)newsize );

	}

	if (entryqptr == NULL || (neptr = TLgetentry( htid )) == NULL) {
...
prt_lvalues()
{
	...
	for ( i = 0; i < entrycnt; i++ ) {
		...
		/* if old entry for H_DMNAME, then do not print this TOC */
		prvalues[1] = TLgetfield( htid, eptr, H_DMNAME );
		if ( (prvalues[1] == NULL) || (*prvalues[1] == '!') ) {
			continue;
		}
...
prt_dvalues()
{
	...
	for ( i = 0; i < entrycnt; i++ ) {
		...
		else /* assumed: if ( strcmp( (char *)arch, "N" ) == 0 ) */
			if ( (*tocname == NULL) || (access( tocname, F_OK) != 0) )

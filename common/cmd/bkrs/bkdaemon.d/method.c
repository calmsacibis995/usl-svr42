...
#ident	"@(#)bkrs:common/cmd/bkrs/bkdaemon.d/method.c	1.14.11.4"
#ident  "$Header: method.c 1.5 91/06/24 $"
...
int	m_slot;
{
	...
	if( !method->entry.ddevice && !(*method->entry.ddevice) ) {
...
int *argc, *options;
{
	...
	*options = 0;
	...
	ptr = flags;
	SKIPWS( ptr );
	for( tmpptr = ptr; (ptr != NULL && *ptr != NULL); tmpptr = ptr ) {
		if( *argc > MD_MAXARGS ) {
			brlog(
...
method_t *method;
{
	...
	/* make sure these arg pointers aren't null */

	for(i=3; i<8; i++)
		if (method->argv[i] == NULL)
			method->argv[i] = (char *)"";
...
method_t *method;
{
	...
	method->argv[ argc++ ] = strdup( (char *)bk_get_method_path(entry->method));
	...
	method->argv[ argc++ ] = (buffer == NULL) ? (char *)"" :
		(char *)strdup((char *)buffer);
	method->argv[ argc++ ] = (entry->oname == NULL) ? (char *)"" :
		 (char *)strdup((char *)entry->oname);
	method->argv[ argc++ ] = (entry->odevice == NULL) ? (char *)"" :
		(char *)strdup( (char *)entry->odevice );
	method->argv[ argc++ ] = (entry->olabel == NULL) ? (char *)"" :
		(char *)strdup( (char *)entry->olabel );
...
bkrotate_t date;
{
	...
	for( entryno = *dot; ; entryno++ ) {
		...
		case TLOK:
			...
			if ((ret = en_parse( tid, eptr, &entry )) == -1)
				continue;
			else if (ret == -2)
				return ((brentry_t *)NULL);

...
#ident	"@(#)bkrs:common/cmd/bkrs/bkdaemon.d/eventq.c	1.4.6.2"
#ident  "$Header: eventq.c 1.2 91/06/21 $"
...
/*	for( eventp = &eventq; eventp; eventp = eventp->forward ) {*/
	for( eventp = &eventq; eventp && eventp->forward; eventp = eventp->forward ) {
...
/*	for( eventp = &sleepq; eventp; eventp = eventp->forward ) { */
	for( eventp = &sleepq; eventp && eventp->forward; eventp = eventp->forward ) {

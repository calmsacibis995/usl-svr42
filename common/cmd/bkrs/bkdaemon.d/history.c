...
#ident	"@(#)bkrs:common/cmd/bkrs/bkdaemon.d/history.c	1.9.5.3"
#ident  "$Header: history.c 1.2 91/06/21 $"
...
history_m *msg;
{
	...
	if( msg->flags & HST_IS_TOC ) {
		if( msg->flags & HST_CONTINUE ) {
			...
			(void) bkTLassign( h_tid, eptr, H_TMNAME, ptr, h );
			free( ptr );

		} else {
			(void) bkTLassign( h_tid, eptr, H_TMNAME, msg->labels, h );
			(void) bkTLassign( h_tid, eptr, H_ARCHTOC, "Y", h );
		}
		...
		(void) bkTLassign( h_tid, eptr, H_DMNAME, ptr, h );
		free( ptr );

	} else {
		(void) bkTLassign( h_tid, eptr, H_DMNAME, msg->labels, h );

		(void) bkTLassign( h_tid, eptr, H_DATE, buffer, h );
		
		if( !(msg->flags & HST_MODIFY) ) {
			(void) bkTLassign( h_tid, eptr, H_TAG, method->entry.tag, h );
			(void) bkTLassign( h_tid, eptr, H_METHOD, method->entry.method, h );
			(void) bkTLassign( h_tid, eptr, H_OPTIONS, method->entry.options, h );
		}

		(void) bkTLassign( h_tid, eptr, H_ONAME, msg->oname, h );
		(void) bkTLassign( h_tid, eptr, H_ODEVICE, msg->odevice, h );

		if( method->entry.dgroup )
			(void) bkTLassign( h_tid, eptr, H_DGROUP, method->entry.dgroup, h );

		(void) bkTLassign( h_tid, eptr, H_DDEVICE, method->entry.ddevice, h );
		(void) bkTLassign( h_tid, eptr, H_DCHAR, method->dchar, h );

		(void) sprintf( buffer, "%d", msg->nvolumes );
		(void) bkTLassign( h_tid, eptr, H_DNVOL, buffer, h );

		(void) sprintf( buffer, "%d", msg->size );
		(void) bkTLassign( h_tid, eptr, H_SIZE, buffer, h );
	}

	(void) bkTLassign( h_tid, eptr, H_TOCNAME, msg->tocname, h );
...
argv_t *argv;
{
	...
	for( i = 0; (*argv)[i]; i++ ) {
		...
		if( some ) {
			...
			(void) bkTLassign( tid, eptr, fieldname, ptr, h );
			free( ptr );
		}
	}

...
#ident	"@(#)bkrs:common/cmd/bkrs/bkexcept.d/bkexcept.c	1.6.9.4"
#ident  "$Header: bkexcept.c 1.2 91/06/21 $"
...
add()
{
	...
	while ( (pat = (*p_argv)[i++]) != NULL ) {
		if ( strcmp( pat, "-" ) == 0 ) {
			pat = inpbuf;
			while ( fgets( pat, BUFSIZ, stdin ) != NULL ) {
				pat[strlen(pat)-1]='\0';	
...
tblopen( path, oflag )
{
	...
	{                                                       /* RDS */
		if (oflag == O_RDONLY)                          /* RDS */
			bkerror(stderr, ERROR17, table);        /* RDS */
		else                                            /* RDS */
		{                                               /* RDS */
			bkerror(stderr, ERROR18, table);        /* RDS */
			insert_format(tid, EX_EXCEPT_F);        /* RDS */
		}                                               /* RDS */
	}                                                       /* RDS */

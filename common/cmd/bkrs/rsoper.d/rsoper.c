...
#ident	"@(#)bkrs:common/cmd/bkrs/rsoper.d/rsoper.c	1.6.6.2"
#ident  "$Header: rsoper.c 1.2 91/06/21 $"
	...
	while( (c = getopt( argc, argv, "c:d:j:m:no:r:stu:v?" )) != -1 )
		...
		case 'm':	/* method name */
			...
			if ((method != NULL) && (*method != NULL)) {
				char *methpath;
				methpath = bk_get_method_path(method);
				if (access(methpath, F_OK) == -1) {
					bkerror( stderr, ERROR22, method, "method not found" );
					exit(2);
				}
			} else {
				bkerror( stderr, ERROR26);
				exit(2);
			}
	...
	if( flags & rFLAG ) {
		...
		*/
		...
		if( rc = rsspawn( list, oname, odevice, ddevice, dchar, dmnames, method,
			...
			switch( rc ) {
			case BKNOMEMORY:
				bkerror( stderr, ERROR22, method, "no memory available" );
				break;
			default:

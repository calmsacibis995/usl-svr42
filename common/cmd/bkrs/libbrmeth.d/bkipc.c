...
#ident	"@(#)bkrs:common/cmd/bkrs/libbrmeth.d/bkipc.c	1.11.6.3"
#ident  "$Header: bkipc.c 1.2 91/06/21 $"
...
#ifdef TRACE
		brlog( "bkm_send(): write() returns %d (wanted %d)",
		 rc,  sizeof(bkmessage_t) );
#endif
...
#endif
#ifdef DEBUG
if ((oambase = (char *)getenv("OAMBASE")) != NULL)
	(void) sprintf(filename, "/tmp/%.10s", oambase, name);
else
#endif

...
#ident	"@(#)bkrs:common/cmd/bkrs/libbrmeth.d/bkgetvol.c	1.2.8.2"
#ident  "$Header: bkgetvol.c 1.2 91/06/21 $"
	...
	if( type && !(strncmp(type, "dir", 3)) ) {
		...
		if( IS_ALIAS( device ) ) {
			...
			if( ptr = getattr( device, "pathname" ) )
				if ((int)strlen( ptr )+(int)strlen(volname) < 
					(PATH_MAX - 1))

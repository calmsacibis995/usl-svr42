...
#ident	"@(#)bkrs:common/cmd/bkrs/restore.d/restore.c	1.5.6.5"
#ident  "$Header: restore.c 1.2 91/06/21 $"
...
#ifdef TRACE
		brlog("restore: restore_request for \"%s\"", rqst.object);
#endif
		if( flags & nFLAG ) {
			...
			/*
			With the fdisk method restore checks the return code of the
			generated restore requests. Any value != 0 is interpreted as
			error. Restore returns 2 if it cannot restore, and it cannot
			satisfy the request because there is another tape (fdisk) in
			the tape drive.
			We simply ignore this - might be better to check for rc == 2 ???
			*/
				/* some++; */

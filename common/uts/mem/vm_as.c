...
#ident	"@(#)uts-comm:mem/vm_as.c	1.1.3.4"
#ident	"$Header: $"
 ...
 * Returns a pointer to the new address space data structure.
 */
struct as *
as_dup(as)
	register struct as *as;
...
/*
 * Set the virtual mapping for the interval [addr, addr + size)
...
/*
 * Unmap the range of addresses [addr, addr + size)
...
/*
 * Add the range of addresses [addr, addr + size)
 * to the specified address space.  crfp is a pointer
 * to the desired segment creation routine, and argsp
 * is the argument pointer to be passed to that routine.
 * We return 0 on success, or a non-zero errno on failure.
 */
int
	...
	caddr_t argsp;
{
	...
	if (as != &kas && as->a_size + rsize > u.u_rlimit[RLIMIT_VMEM].rlim_cur)
		return ENOMEM;
...
/*
 * Returns the protection value for the given address in the
 * specified address space.  Also returns via naddr the next
 * address at which the protection value changes (or the end
 * of the associated segment if that is reached first).
 */
u_int
 ...
 * when we call segmap_pagecreate.
 ...
 * The pages created by segmap_pagecreate()
 ...
 * It relies on the fact that segmap_pagecreate()
 ...
 * segmap_pagecreate will find and use them.
	...
	*(ppp = pl) = NULL;
	...
	for (; raddr < eaddr; sz += cpsz, cpsz = PAGESIZE, raddr += PAGESIZE) {
		...
		error = VOP_GETPAGE(svp, foff, PAGESIZE, NULL, ppp,
			PAGESIZE, seg, raddr, S_READ, sys_cred);
		if (error)
			goto err;
		ppp++;
	}

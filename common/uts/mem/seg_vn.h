...
#ident	"@(#)uts-comm:mem/seg_vn.h	1.3.4.3"
#ident	"$Header: $"
 ...
 * structure so that we can free everything when the reference
 * count goes to zero.
 */
struct anon_map {
...
/*
 * It is sometimes necessary to know if a given segment is of
 * seg_vn type.  We provide a macro to be used for this purpose.
 * (Unfortunately this requires us to make our ops vector global.)
 */
#define IS_SEGVN(seg)	((seg)->s_ops == &segvn_ops)
extern	struct seg_ops	segvn_ops;

/*
 * Provided as shorthand for creating zero-fill-on-demand segments.
 */
extern	caddr_t zfod_argsp;	/* args for creation of user zfod segments */
extern	caddr_t kzfod_argsp;	/* args for creation of kernel zfod segments */

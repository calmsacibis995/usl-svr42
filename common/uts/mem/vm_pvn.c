...
#ident	"@(#)uts-comm:mem/vm_pvn.c	1.9.4.10"
#ident	"$Header: $"
 ...
 *
 * Note that with isra == 0, NULL is only returned if a page was
 * previously entered by someone else.
 */
page_t *
...
/*
 * Entry point to be used by page r/w subr's and other such routines that
 * want to report an error and abort a list of pages setup for pageio
 * which do not go through the normal pvn_done processing.
 */
void
	...
	int flags;
{
	...
	bp->b_pages = NULL;		/* release the buffer */
	...
	for (bytes = 0; bytes < bp->b_bcount; bytes += PAGESIZE) {
		...
		if (bp->b_flags & B_FREE) {
			...
			if (pp->p_keepcnt == 0
				 ...
				 */
				if (pp->p_mod == 0 && PAGE_HAS_MAPPINGS(pp))
					hat_pagesync(pp);
	...
	int flags;
{
	 ...
	 */
	if (PAGE_HAS_MAPPINGS(pp) && !(pp->p_mod && (flags & (B_FREE | B_INVAL)) != 0))
		hat_pagesync(pp);
	 ...
	 */
	if (PAGE_HAS_MAPPINGS(pp) && (flags & (B_FREE | B_INVAL)) != 0)
		hat_pageunload(pp);
	pp->p_mod = 0;
	pp->p_ref = 0;
	ASSERT(pp->p_free == 0);
 ...
 * has marker pairs running around the list.
	...
	register int flags;
{
	...
	/* Process only clean pages on this vnode that fit in the range */
	...
	while (pp != spp) {
		 ...
		 * If trying to invalidate a logically locked page, forget it
		 * Skip the klustering case that would cause us to block
		 * Don't bother waiting for an intrans page or locked page
		 */
		if ((pp->p_vnode != vp) ||
		    (pp->p_offset < off) || pp->p_gone || 
		    (pp->p_lckcnt != 0 || pp->p_cowcnt != 0) || 
		    ((flags & B_DELWRI) != 0 && (pp->p_keepcnt != 0 || pp->p_lock)) ||
		...
		/* If the page is not marked modified -- go check it the pte */
		if (PAGE_HAS_MAPPINGS(pp) && !pp->p_mod)
	...
	/* Process all pages on this vnode that fit in the range */
	pp = spp->p_vpnext;
	...
	struct cred *cred;
{
	...
	ASSERT(plsz >= len);		/* ensure that we have enough space */
...
/*
 * This routine returns "true" if there are no
 * non-marker pages on the given vnode's v_pages list.
 */
int
pvn_vpempty(vp)
	register struct vnode *vp;

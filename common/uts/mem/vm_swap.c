...
#ident	"@(#)uts-comm:mem/vm_swap.c	1.10.3.9"
#ident	"$Header: $"
 ...
 * associated vnode.
 *
 * When we delete a swap device, since it is too complicated to find
...
extern struct bootobj swapfile;		/* boot time config information */
...
struct swapinfo		*swapinfo;	/* heads list of swap areas */
STATIC struct swapinfo	*silast;	/* the current one to allocate from */
STATIC int		nswapfiles;	/* number of swap files in use */
...
STATIC struct ianon	*ianon_lookup();
STATIC struct ianon	*ianon_alloc();
	...
	register struct anon *ap;
{
	 ...
	 */
	if (ap->an_flag & AINDIR) {
		struct ianon *iap, *ibap;
		...
		bap = iap->an_bap;
		ASSERT(bap != NULL);
		ASSERT(bap->an_flag & AINDIR);
		ibap = ianon_lookup(bap);
		ASSERT(ibap != NULL);
		ASSERT(ibap->an_bap == ap);
	...
	register uint *offsetp;
{
	...
	if (ap->an_flag & AINDIR) {
		...
		iap = ianon_lookup(ap);
		ASSERT(iap != NULL);
		ASSERT((bap = iap->an_bap) != NULL);
		ASSERT(ap->an_refcnt > 0);
		ASSERT(bap->an_flag & AINDIR);
		ASSERT((ibap = ianon_lookup(bap)) != NULL);
		ASSERT(ibap->an_bap == ap);
		ASSERT(bap->an_refcnt == 0);
		ap = iap->an_bap;
	}
		...
		do {
			if (vp == sip->si_vp && offset >= sip->si_soff 
			  && offset < sip->si_eoff) {
				...
				if (ap->an_refcnt) {
					ASSERT(!(ap->an_flag & AINDIR) ||
					(((iap = ianon_lookup(ap)) != NULL) &&
					 ((bap = iap->an_bap) != NULL) &&
					 ((ibap = ianon_lookup(bap)) != NULL) &&
					 (ibap->an_bap == ap) &&
					 (bap->an_refcnt == 0)));
					return (ap);
				} else if (ap->an_flag & AINDIR) {
					iap = ianon_lookup(ap);
					ASSERT((iap != NULL) &&
						(iap->an_bap != NULL));
					bap = iap->an_bap;
	...
	rval_t *rvp;
{
	...
	case SC_LIST:
		 ...
		 */
		if (mac_installed && pm_denied(u.u_cred, P_SYSOPS))
			return(EPERM);

		if ((error = ucopyin((caddr_t)uap->sc_arg, 
		  (caddr_t)&length, sizeof(int), 0)) != 0)
			return(error);
		...
		do {
			...
			if (length-- == 0)	/* no place for more entries */
				break;
			if ((error = ucopyin((caddr_t)ust, (caddr_t)&st,
						sizeof(swapent_t), 0)) != 0)
				return(error);
			...
			if ((error = ucopyout((caddr_t)&st, (caddr_t)ust,
						sizeof(swapent_t), 0)) != 0) {
				rvp->r_val1 = cnt;
				return (error);
			}
			if (sip->si_pname)
				if ((error = ucopyout(sip->si_pname,
				 st.ste_path, strlen(sip->si_pname), 0)) != 0) {
					rvp->r_val1 = cnt;
					return (error);
				}
			ust++;
	...
	char	*swapname;
{
	...
	/*
	 * For compatibility reasons, we allow block devices with unknown
	 * sizes (which show up as zero size) to bypass the range checks.
	 * In effect, we assume they are infinite in size.
	 */
	if (vattr.va_size == 0 && vp->v_type == VBLK)
		vattr.va_size = eoff;
	...
	for (sipp = &swapinfo; (nsip = *sipp) != NULL; sipp = &nsip->si_next) {
		...
		if (nsip->si_flags & ST_DELETED) {
			if (nsip->si_soff == soff && nsip->si_npgs == npage
	...
	if (error == ENOSYS) {
		for (off = soff; off < eoff; off += PAGESIZE) {
			error = vn_rdwr(UIO_WRITE, vp, (char *)&dummy, 1, off,
				UIO_SYSSPACE, 0, RLIM_INFINITY, u.u_cred, &dummy);
			if (error)
				break;
		}
	}
	...
	*sipp = nsip;
	...
	nsip->si_soff = soff;
	nsip->si_eoff = eoff;
	nsip->si_anon = (struct anon *)kmem_zalloc(npage*sizeof (struct anon), KM_SLEEP);
	nsip->si_eanon = nsip->si_anon + (npage - 1);
	...
	anoninfo.ani_free += npage;
	anoninfo.ani_max += npage;
	availsmem += npage;
	nswapfiles++;
	nsip->si_flags &= ~ST_INDEL;
 ...
 * can now delete all the data structures.
 */
STATIC void
swapinfo_free(sip)
	register struct swapinfo *sip;
{
	...
	*sipp = sip->si_next;
	...
	/* si_npgs should be the true number of anon entries allocated */
	ASSERT(sip->si_npgs == btoc(sip->si_eoff - sip->si_soff));
	kmem_free(sip->si_anon, sip->si_npgs * sizeof(struct anon));
	kmem_free(sip->si_ianon, sip->si_npgs * sizeof(struct ianon));
...
lookagain:
		if (cur->an_flag & AINDIR) {
			...
			if (cur->an_refcnt) {
				 ...
				 */
				ASSERT((icur = ianon_lookup(cur)) != NULL);
				ASSERT((bap = icur->an_bap) != NULL);
				ASSERT(bap->an_flag & AINDIR);
				ASSERT((ibap = ianon_lookup(bap)) != NULL);
				ASSERT(ibap->an_bap == cur);
				ASSERT(bap->an_refcnt == 0);
				continue;
			} else {
				/*
...
delagain:
		...
		while (opp == NULL) {
			/*
			 * use "sys_cred" so the following vn_rdwr()
			 * call doesn't fail for privilege reasons.
			*/
			error = vn_rdwr(UIO_READ, ovp, &dummy, 1, ooff,
				UIO_SYSSPACE, 0, (long)0, sys_cred, &resid);
			if (error)
			    cmn_err(CE_PANIC, "swap space inaccessible\n");
			opp = page_lookup(ovp, ooff);
		}
		...
		/* set up indirection */
		old->an_flag |= AINDIR;
		iold = ianon_alloc(old);
		iold->an_bap = new;
		new->an_flag |= AINDIR;
		inew = ianon_alloc(new);
		inew->an_bap = old;
		if (cur != old) {
			/*
			 ...
			 * For this code to be executed, we
			 * must have cur not equal to old, in
			 * which case icur is pointing to the
			 * ianon structure corresponding to cur.
			 */
			ASSERT(icur->an_bap == old);
	...
	struct swapinfo *sip;
{
	...
	register struct ianon *idir, *iind;
	register page_t *opp;
	register struct anon *eanon;
	struct vnode *nvp, *ovp;
	u_int noff, ooff;
...
lookagain:
		if ((dir->an_refcnt == 0) || !(dir->an_flag & AINDIR))
			continue;

		idir = ianon_lookup(dir);
		ASSERT((idir != NULL) && (idir->an_bap != NULL));
		ind = idir->an_bap;
		ASSERT(ind->an_flag & AINDIR);
		ASSERT((iind = ianon_lookup(ind)) != NULL);
		ASSERT(iind->an_bap == dir);
	...
	register swpi_t	*si;
{
	...
	case SI_LIST:
		...
		do {
			...
			if (ucopyout((caddr_t)&st, ustp, sizeof(swpt_t), 0)) {
				error = EFAULT;
				break;
			}
			ustp += sizeof(swpt_t);
	...
	register struct anon *ap;
{
	...
	if (ap->an_flag & AINDIR) {
		...
		iap = ianon_lookup(ap);
		ASSERT((iap != NULL) && (iap->an_bap != NULL));
		ap = iap->an_bap;
	}
	do {
		if (sip->si_anon <= ap && ap <= sip->si_eanon) {
...
swapconf()
{
	...
	if (swapdev != NODEV)
		swapfile.bo_vp = makespecvp(swapdev, VBLK);
	else if (swapfile.bo_name[0] == '\0')
		return;
	else {
		error = lookupname(swapfile.bo_name, UIO_SYSSPACE,
				   FOLLOW, NULLVPP, &swapfile.bo_vp);
		if (error) {
			cmn_err(CE_PANIC,
				"swapconf lookupname %s failed - error %d\n",
				swapfile.bo_name, error);
		}
	}

	if (swapfile.bo_size == 0) {
		/* 
		 * Get partition size and use the whole thing.
		 */
		vnode_t *vp = swapfile.bo_vp;
		struct vattr vattr;

		swapfile.bo_offset = 0;

		if (error = VOP_OPEN(&vp, FREAD, u.u_cred))
			goto no_size;

		vattr.va_mask = AT_SIZE;
		error = VOP_GETATTR(vp, &vattr, ATTR_COMM, u.u_cred);
		(void) VOP_CLOSE(vp, FREAD, 1, (off_t) 0, u.u_cred);
		if (error)
			goto no_size;

		swapfile.bo_size = vattr.va_size >> SCTRSHFT; 
no_size: ;
	}
...
/*
 * Return the indirect anon pointer corresponding to the specified anon
 *	pointer.
 */
STATIC struct ianon *
ianon_lookup(ap)
	struct anon *ap;
{
	...
	do {
		if (sip->si_anon <= ap && ap <= sip->si_eanon) {
			if (sip->si_ianon == NULL)
				return ((struct ianon *)NULL);
			return (sip->si_ianon + (ap - sip->si_anon));
		}
	} while ((sip = sip->si_next) != NULL);
	cmn_err(CE_PANIC, "ianon_lookup");
}

/*
 * Return the indirect anon pointer corresponding to the specified anon
 *	pointer, allocating an indirect anon array if there isn't one
 *	already.
 */
STATIC struct ianon *
ianon_alloc(ap)
	struct anon *ap;
{
	...
	do {
		if (sip->si_anon <= ap && ap <= sip->si_eanon) {
			if (sip->si_ianon == NULL)
				sip->si_ianon = (struct ianon *)
					kmem_zalloc(sip->si_npgs *
					sizeof(struct ianon), KM_SLEEP);
			return (sip->si_ianon + (ap - sip->si_anon));
		}
	} while ((sip = sip->si_next) != NULL);
	cmn_err(CE_PANIC, "ianon_alloc");
}

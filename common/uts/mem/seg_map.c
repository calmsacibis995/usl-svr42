...
#ident	"@(#)uts-comm:mem/seg_map.c	1.1.3.3"
#ident	"$Header: $"
...
/*
 * Add smap "smp" to the freelist headed by smd->smd_free.
 * If anybody was awaiting a free smap for this segment,
 * notify them of its availability.
 */
#define segmap_smapadd(smd, smp) {					\
	if (smd->smd_free == (struct smap *)NULL) {			\
		...
		if (smd->smd_want)					\
			cmn_err(CE_PANIC,"segmap_smapadd: unexpected want"); \
	}								\
}

/*
 * Remove smap "smp" from the freelist headed by smd->smd_free,
 * and initialize its reference count.
 */
#define segmap_smapsub(smd, smp) {					\
	if (smd->smd_free == smp) {					\
		if ((smd->smd_free = smp->sm_next) == smp)		\
			smd->smd_free = NULL;				\
		else {							\
...
/*
 * Some statistics for segmap operations.
 * These can be looked at via a kernel debugger.
 */
STATIC struct segmapcnt {
...
/*
 * Create a seg_map segment.
 * This is called once, at startup time, to create the segkmap segment.
 */
int
...
/*
 * Do an F_SOFTUNLOCK call over the range requested.
	...
	enum seg_rw rw;
{
	...
	u_int prot;
	u_int pg;
	u_int npgs; 		/* number of pages locked down if F_SOFTLOCK */
	int err;
	...
	err = VOP_GETPAGE(vp, off, len, &prot, pl, MAXBSIZE, seg, addr,
			  (rw == S_WRITE ? S_OTHER : rw), sys_cred);
	if (err) {
		if (type == F_SOFTLOCK) {
                        availrmem += npgs;
                        pages_pp_kernel -= npgs;
	...
	addr_t addr;
{
	...
	BUMPSMAPCNT(segmapcnt.smc_faulta);
	...
	addr_t addr;
{
	...
	return (smd->smd_sm->sm_off + ((u_int)addr & MAXBOFFSET));
}
	...
	u_int off;
{
	 ...
	 * where the vp has been freed and the smap structure is
	...
	int softlock;
{
	...
	BUMPSMAPCNT(segmapcnt.smc_pagecreate);
...
/*
 * Allocate an smap for the specified vnode and offset (if
 * necessary) and return the corresponding virtual address.
 */
addr_t
...
/*
 * Give up the smap associated with the specified virtual address
 * (as returned by segmap_getmap()).  Flags specify what to do with
 * the pages on release (write them back, put them on free list, etc.).
 */
int
	...
	u_int flags;
{
	...
	if (flags != SM_DONTNEED && flags != 0) {
		...
		if (smp->sm_refcnt == 1) {
			...
			if (flags & SM_FREE) {
				bflags |= B_FREE;
				BUMPSMAPCNT(segmapcnt.smc_rel_free);
			}
...
/*
 * Flush any smap associated with the given vnode.
 */
void
segmap_flush(seg, vp)
	struct seg *seg;

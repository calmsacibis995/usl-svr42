...
#ident	"@(#)uts-comm:mem/seg_vn.c	1.7.3.5"
#ident	"$Header: $"
...
/*
 * Private seg_ops routines.
 */
STATIC int		segvn_dup();
STATIC int		segvn_unmap();
STATIC void		segvn_free();
STATIC faultcode_t	segvn_fault();
STATIC faultcode_t	segvn_faulta();
STATIC void		segvn_unload();
STATIC int		segvn_setprot();
STATIC int		segvn_checkprot();
STATIC int		segvn_getprot();
STATIC off_t		segvn_getoffset();
STATIC int		segvn_gettype();
STATIC int		segvn_getvp();
STATIC int		segvn_kluster();
STATIC u_int		segvn_swapout();
STATIC int		segvn_sync();
STATIC int		segvn_incore();
STATIC int		segvn_lockop();
...
STATIC int	segvn_concat();
STATIC int	segvn_extend_prev();
STATIC int	segvn_extend_next();
STATIC void	segvn_vpage();
STATIC void	anonmap_alloc();

extern void	free_vp_pages();

/*
 * Create a segment for shared or copy-on-write mapping
 * from a vnode/anonymous memory.
 */
int
	...
	enum seg_rw rw;
{
	...
	if (stealCOW && app != NULL && *app == NULL &&
	    ...
	    opp->p_mod == 0 && !PAGE_HAS_MAPPINGS(opp) &&
	...
	enum seg_rw rw;
{
	...
	if (svd->amp == NULL && (svd->vp == NULL ||
		...
		/* ok, we need to do it */
		/* allocate array of anon pointers up front, since we can't sleep
                   later. we can be swapped out when sleeping. */
                npp = (struct anon **)kmem_zalloc((u_int)
	...
	register u_int len, prot;
{
	...
	if (addr == seg->s_base && len == seg->s_size && svd->pageprot == 0) {
		if (svd->vp && svd->type == MAP_PRIVATE && 
			(prot & PROT_WRITE) == 0 && svd->swresv > 0 &&
		  		svd->amp == NULL) {
			anon_unresv(svd->swresv);
			svd->swresv = 0;
		}
	...
	struct seg *seg;
{
	...
	for (page = 0; page < npages; page++) {
		...
		if (pp->p_keepcnt != 0) {
			 ...
			 */
			if (pp->p_intrans && !pp->p_pagein && !PAGE_HAS_MAPPINGS(pp))
				pgcnt++;
			continue;
		}

		if (PAGE_HAS_MAPPINGS(pp) != NULL)
			continue;

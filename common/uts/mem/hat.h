...
#ident	"@(#)uts-comm:mem/hat.h	1.1.2.4"
#ident	"$Header: $"
 ...
 * in vm_hat.h.  The hat layer manages the address
...
#include <mem/vm_hat.h>	/* SVR4.0COMPAT */
#endif
 ...
 *	- initialize any needed hat structures when the process is swapped in.
...
/*
 * Operations that work on all active translations for a given page:
...
/*
 * Operations that return physical page IDs (e.g. - used by mapin):
 *	- return the ppid for kernel virtual address.
 *	- return the ppid for arbitrary virtual address
 *	  (hat_getpfnum, not yet implemented since not needed).
 ...
 * 	HAT_LOCK      Lock down mapping resources;
 *		      hat_memload() and hat_devload().

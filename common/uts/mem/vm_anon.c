...
#ident	"@(#)uts-comm:mem/vm_anon.c	1.4.4.7"
#ident	"$Header: $"
...
struct anoninfo	anoninfo;		/* tracks availability of anon pages */
...
anon_resv(size)
	u_int size;
{
	register int npage = btopr(size);

	if (availsmem - npage < tune.t_minasmem) {
		nomemmsg("anon_resv", npage, 0, 0);
		return 0;
	}

	if (anoninfo.ani_resv + npage > anoninfo.ani_max) {
		return 0;
	} 
	anoninfo.ani_resv += npage;
	availsmem -= npage;
	return 1;
}
...
anon_unresv(size)
	u_int size;
{
	register u_int npage = btopr(size);
...
/*
 * Special version of anon_alloc for seg_u debugging.
 */
struct anon *
anon_upalloc()
{
...
/*
 * Special version of anon_decref for seg_u debugging.
 */
void
anon_updecref(ap)
	register struct anon *ap;
...
/*
 * Special version of anon_free for seg_u debugging.
 */
void

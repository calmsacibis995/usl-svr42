...
#ident	"@(#)uts-comm:proc/slp.c	1.8.3.6"
#ident	"$Header: $"
	...
	int			preemptflg;
{
	...
	for (plinkp = &sqp->sq_first; (pp = *plinkp) != NULL; ) {
		...
		if (pp->p_wchan == chan) {
			...
			/*
			 * If the target process is not at the level of
			 * the caller, we have a covert channel event.
			 * (Note that the event is not meaningful,
			 * and will not be counted, if we were called
		 	 * from interrupt level.)  We don't want to call
			 * cc_limiter() directly here, since it might sleep.
			 * Therefore we simply count the events, and call the
			 * limiter later just prior to returning to user level,
			 * in systrap().
			 */
			if (MAC_ACCESS(MACEQUAL, u.u_cred->cr_lid,
						pp->p_cred->cr_lid)
			&&  !servicing_interrupt()) {
				u.u_systrap |= P_CC;	/* slow path */
				u.u_covchan++;
			}
...
/*
 * Wake up one process sleeping on chan.  Note that a process on a
 * sleep queue may be either in state SSLEEP or in state SSTOP; if it
 * was stopped we remove it from its sleep queue but we don't set it
 * running here.  The preemptflg indicates whether the caller is
 * requesting a preemptive or non-preemptive wakeup.  Preemptive
 ...
 */
void
wake_oneproc(chan,preemptflg)
register caddr_t chan; int preemptflg;
{
	register proc_t		*pp,
				*rp;
	register proc_t		**plinkp;
	...
	for (plinkp = &sqp->sq_first; (pp = *plinkp) != NULL; ) {
		...
		if (pp->p_wchan == chan) {
			...
			pp->p_wchan = 0;
			/*
 			 * REMEMBER: we are only waking up one so leave
 			 * sqp->sq_last alone unless we have the last process
 			 * waiting on the channel. PULS: AA37083 - dbg.
 			 */
			*plinkp = pp->p_link;	/* remove from list */
			if (*plinkp == NULL) {	/* and set sq_last to last */
				sqp->sq_last = rp;
			}
			...
			if (pp->p_stat == SSLEEP) {
				...
				/*
 				 * Awakened one proc; thats enough, don't worry
 				 * about changing plink, we don't need it now.
 				 */
				break;
			}
		} else {
			/* 
 			 * Remember last process and point plink to last process
 			 * structure not waiting on our channel.
 			 */
			rp = pp;
			plinkp = &pp->p_link;
		}

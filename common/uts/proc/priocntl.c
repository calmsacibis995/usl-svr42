...
#ident	"@(#)uts-comm:proc/priocntl.c	1.6.2.2"
#ident	"$Header: $"
...
struct pcmpargs	*argp;
{
	...
	/*
	 * Check MAC access.  If caller doesn't dominate the specified
	 * process, return ESRCH which will cause dotoprocs() to pretend
	 * it didn't see the process at all.
	 */
	if (MAC_ACCESS(MACDOM, u.u_cred->cr_lid, pp->p_cred->cr_lid)

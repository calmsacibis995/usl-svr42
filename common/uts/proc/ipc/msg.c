...
#ident	"@(#)uts-comm:proc/ipc/msg.c	1.8.3.4"
#ident	"$Header: $"
/*
...
STATIC ccevent_t cc_re_msg = { CC_RE_MSG, CCBITS_RE_MSG };

/* Convert bytes to msg segments. */
#define	btoq(X)	(((long)(X) + msginfo.msgssz - 1) / msginfo.msgssz)

/*
 * Macro to free msg headers.
 */
#define	msgfp_free(mp, wflag) { \
	if (((mp)->msg_next = msgfp) == NULL) \
	...
	rval_t *rvp;
{
	...
	if (error = msgconv(uap->msgid, &qp)) /* get msqid_ds for this msgid */
         ...
         */
	(void)adt_auditme(A_IPCCHK,ADT_MSG_CTL,qp->msg_perm.ipc_secp->ipc_lid);
	...
	case IPC_RMID:	/* for use with expanded msqid_ds struct - 
				...
				*/
		if (error =
		    ipcaccess(&qp->msg_perm, MSG_R|MSG_W, IPC_MAC, u.u_cred))
			break;
		...
		if (((uap->cmd == IPC_O_RMID) &&
			(uap->msgid + msginfo.msgmni > SHRT_MAX)) ||
			(uap->msgid + msginfo.msgmni) < 0)
	...
	case IPC_O_SET:
		...
		if (ods.msg_qbytes > qp->msg_qbytes
	...
	case IPC_O_STAT:
		if (error =
		    ipcaccess(&qp->msg_perm, MSG_R, IPC_MAC|IPC_DAC, u.u_cred))
			break;
	...
	case IPC_STAT:
		...
		bcopy((caddr_t)qp, (caddr_t)&ds, sizeof(ds));
		ds.msg_first = (struct msg *)NULL;	/* kernel addr */
		ds.msg_last = (struct msg *)NULL;
		ds.msg_perm.ipc_secp = (struct ipc_sec *)NULL;

		if (copyout((caddr_t)&ds, (caddr_t)uap->buf, sizeof(ds))) {
			error = EFAULT;
			break;
		}
		break;
	...
	rval_t *rvp;
{
	...
	if (error = ipcget(uap->key, uap->msgflg, (struct ipc_perm *)msgque,
	  msginfo.msgmni, sizeof(*qp), &s, (struct ipc_perm **)&qp))
         ...
         * pertains to this event and lid.
         */
	(void)adt_auditme(A_IPCCHK,ADT_MSG_GET,qp->msg_perm.ipc_secp->ipc_lid);
...
msginit()
{
	...
	register struct msqid_ds        *qp;    /* ptr to msgque */
	register struct ipc_sec         *sp;    /* ptr to sec structs */

	/* Allocate physical memory for message buffer. */
	if ((msg = (paddr_t)kmem_zalloc((int)(msginfo.msgseg * msginfo.msgssz),
		KM_NOSLEEP)) == NULL) {
	...
	/* initialize DAC entries for message queues */
	if ((sp = (struct ipc_sec *)
		kmem_zalloc(msginfo.msgmni*sizeof(struct ipc_sec), KM_NOSLEEP))
	==  (struct ipc_sec *)NULL)
		cmn_err(CE_PANIC, "Can't allocate msg security struct.\n");

	for (i = msginfo.msgmni, qp = msgque; i > 0; i--, qp++, sp++)
		qp->msg_perm.ipc_secp = sp;
}
	...
	rval_t *rvp;
{
	...
	if (error = msgconv(uap->msqid, &qp))
         ...
         * pertains to this event and lid.
         */
	(void)adt_auditme(A_IPCCHK,ADT_MSG_OP,qp->msg_perm.ipc_secp->ipc_lid);

	/* write MAC access is needed since this operation modifies the object */
	if ((error = ipcaccess(&qp->msg_perm, MSG_R|MSG_W, IPC_MAC, u.u_cred))
	||  (error = ipcaccess(&qp->msg_perm, MSG_R, IPC_DAC, u.u_cred)))
		goto msgrcv_out;
        MAC_ASSERT(qp,MAC_SAME);
	if (uap->msgsz < 0) {
		error = EINVAL;
	...
	rval_t *rvp;
{
	...
	if (error = ipcaccess(&qp->msg_perm, MSG_W, IPC_MAC|IPC_DAC, u.u_cred))
		goto msgsnd_out;
        MAC_ASSERT(qp,MAC_SAME);

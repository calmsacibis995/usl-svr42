...
#ident	"@(#)uts-comm:proc/class/rt.c	1.9.3.6"
#ident	"$Header: $"
...
#include <proc/class/rtpriocntl.h>
#include <mem/kmem.h>
#include <svc/systm.h>
...
KSTATIC rtproc_t	rt_plisthead;	/* dummy rtproc at head of rtproc list */
...
cred_t	*reqpcredp;
{
	...
	if (copyin(uaddr, (caddr_t)&rtadmin, sizeof(rtadmin_t))) {
		ADT_ADMIN(ADT_SCHED_RT,EFAULT,ADT_RT_SETDPTBL,0,0,0,0,0,0);
		return(EFAULT);
	}
	...
	case RT_SETDPTBL:
		 ...
		 * exactly match the current rt_dptbl in size.
		 */
		if (pm_denied(reqpcredp, P_SYSOPS)
		    && pm_denied(reqpcredp, P_RTIME)) {
			ADT_ADMIN(ADT_SCHED_RT,EPERM,ADT_RT_SETDPTBL,0,0,0,0,0,0);
			return(EPERM);
		}
		if (rtadmin.rt_ndpents * sizeof(rtdpent_t) != rtdpsz) {
			ADT_ADMIN(ADT_SCHED_RT,EINVAL,ADT_RT_SETDPTBL,0,0,0,0,0,0);
			return(EINVAL);
		}
		...
		for (i = 0; i < rtadmin.rt_ndpents; i++) {
			...
			if (tmpdpp[i].rt_quantum <= 0 &&
				...
				ADT_ADMIN(ADT_SCHED_RT,EINVAL,ADT_RT_SETDPTBL,rt_dptbl[i].rt_globpri,tmpdpp[i].rt_quantum,0,0,0,0);
				kmem_free(tmpdpp, rtdpsz);
				return(EINVAL);
			}
		}
		...
		for (i = 0; i < rtadmin.rt_ndpents; i++) {
			...
			ADT_ADMIN(ADT_SCHED_RT,0,ADT_RT_SETDPTBL,rt_dptbl[i].rt_globpri,rt_dptbl[i].rt_quantum,0,0,0,0);
		}
		splx(oldlvl);
...
cred_t			*reqpcredp;
{
	 ...
	 * which initiates the request must be the privileged.
	 ...
	 */
	if (reqpcredp != NULL && pm_denied(reqpcredp, P_RTIME)) {
		ADT_PARMSSET(ADT_SCHED_RT,EPERM,ADT_RT_NEW,pp->p_pid,0,0,0);
		return EPERM;
	}
	...
	if ((*pflagp & SLOAD) == 0) {
		if (memlow()) {
			ADT_PARMSSET(ADT_SCHED_RT,ENOMEM,ADT_RT_NEW,pp->p_pid,0,0,0);
			return(ENOMEM);
		}
		if (*pflagp & SUSWAP) {
			ADT_PARMSSET(ADT_SCHED_RT,EAGAIN,ADT_RT_NEW,pp->p_pid,0,0,0);
			return(EAGAIN);
		}
		ub_lock(pp);
		if (swapinub(pp) == 0) {
	 ...
	 * Write audit record
	 */
	ADT_PARMSSET(ADT_SCHED_RT,0,ADT_RT_NEW,pp->p_pid,rtpp->rt_pri,0,0);
...
rtproc_t		*rtpp;
{
	 ...
	 * privileged.  If the target process is currently in
	 * a class other than real-time the requesting process
	 * must be privileged.
	 */
	if (reqpcredp != NULL) {
		if (targpcid == rt_cid) {
			if (reqpcid != rt_cid && pm_denied(reqpcredp, P_RTIME))
				return(EPERM);
		} else {  /* target process is not real-time */
			if (pm_denied(reqpcredp, P_RTIME))
				return(EPERM);
		}
	}
...
cred_t			*reqpcredp;
{
	 ...
	 * that the requesting process be real-time unless it is privileged.
	 * This may also have been checked previously but if our caller
	 * passes us a credential structure we assume it hasn't and
	 * we check it here.
	 */
	if (reqpcredp != NULL && reqpcid != rt_cid && pm_denied(reqpcredp, P_RTIME)) {
		ADT_PARMSSET(ADT_SCHED_RT,EPERM,ADT_RT_PARMSSET, rtpp->rt_procp->p_pid,rtkprmsp->rt_pri,0,0);
		return(EPERM);
	}
	 ...
	 * Write audit record
	 */
	ADT_PARMSSET(ADT_SCHED_RT,0,ADT_RT_PARMSSET,rtpp->rt_procp->p_pid,rtkprmsp->rt_pri,0,0);
	splx(oldlvl);
	return(0);
}

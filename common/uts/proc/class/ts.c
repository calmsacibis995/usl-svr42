...
#ident	"@(#)uts-comm:proc/class/ts.c	1.10.3.11"
#ident	"$Header: $"
...
#define	TS_MAXKMDPRI ts_kmdpris[ts_maxkmdpri]

#define SLPSCORE(tspp)  ((tspp)->ts_procp->p_slptime)
#define PRISCORE(tspp)  (TS_MAXKMDPRI - *((tspp)->ts_pprip))
#define	AGESCORE(tspp)  ((tspp)->ts_procp->p_inoutage)
#define MEMSCORE(tspp)  ((tspp)->ts_procp->p_as == 0 ? 0 : \
			(TS_MAXKMDPRI * (tspp)->ts_procp->p_as->a_rss) / availrmem)

/*
 * Very large processes (with RSS claims over 1.6M) are likely X
 * executables with large shared libraries.
 * Do not penalize them as much on the swapin decision, this gives more
 * weight to the priority of the process, making the system more
 * responsive.
 */

#define SWPSCORE(tspp) (tspp)->ts_procp->p_swrss > 400 \
 ? ((TS_MAXKMDPRI * (tspp)->ts_procp->p_swrss) / (2 * availrmem)) \
 : ((TS_MAXKMDPRI * (tspp)->ts_procp->p_swrss) / availrmem)
...
KSTATIC tsproc_t	ts_plisthead;/* dummy tsproc at head of tsproc list */
...
cred_t	*reqpcredp;
{
	...
	if (copyin(uaddr, (caddr_t)&tsadmin, sizeof(tsadmin_t))) {
		ADT_ADMIN(ADT_SCHED_TS,EFAULT,ADT_TS_SETDPTBL,0,0,0,0,0,0);
		return(EFAULT);
	}
	...
	case TS_SETDPTBL:
		...
		if (pm_denied(reqpcredp, P_SYSOPS)
			...
			ADT_ADMIN(ADT_SCHED_TS,EPERM,ADT_TS_SETDPTBL,0,0,0,0,0,0);
			return(EPERM);
		}
		if (tsadmin.ts_ndpents * sizeof(tsdpent_t) != tsdpsz) {
			ADT_ADMIN(ADT_SCHED_TS,EINVAL,ADT_TS_SETDPTBL,0,0,0,0,0,0);
			return(EINVAL);
		}
		...
		for (i = 0; i < tsadmin.ts_ndpents; i++) {
			...
			* Write audit record
			*/
			ADT_ADMIN(ADT_SCHED_TS, 0, ADT_TS_SETDPTBL,
					tmpdpp[i].ts_globpri, 
					tmpdpp[i].ts_quantum,
					tmpdpp[i].ts_tqexp,
					tmpdpp[i].ts_slpret,
...
cred_t		*reqpcredp;
{
	...
	if ((tspp = (tsproc_t *)kmem_fast_alloc(&ts_pmembase, sizeof(tsproc_t),
		...
		ADT_PARMSSET(ADT_SCHED_TS,ENOMEM,ADT_TS_NEW,pp->p_pid,0,0,0);
		return(ENOMEM);
	}
	...
	if (tsparmsp == NULL) {
		...
		if (reqpcredp != NULL && reqtsuprilim > 0 &&
			...
			ADT_PARMSSET(ADT_SCHED_TS,EPERM,ADT_TS_NEW,pp->p_pid,reqtsupri,reqtsuprilim,0);
			return(EPERM);
		}
	...
	/*
         * Write audit record
	 */
	ADT_PARMSSET(ADT_SCHED_TS,0,ADT_TS_NEW,pp->p_pid,tspp->ts_upri,tspp->ts_uprilim,0);
...
tsproc_t		*tspp;
{
	...
	if (targpcid == ts_cid) {
		...
		if (tsparmsp->ts_uprilim > 0 && pm_denied(reqpcredp, P_TSHAR))
			return(EPERM);
	}
	return(0);
}
...
cred_t			*reqpcredp;
{
	 ...
	 */
	if (reqpcredp != NULL) {
		if (reqtsuprilim > tspp->ts_uprilim && pm_denied(reqpcredp, P_TSHAR)) {
			ADT_PARMSSET(ADT_SCHED_TS,EPERM,ADT_TS_PARMSSET,tspp->ts_procp->p_pid,tsparmsp->ts_upri,tsparmsp->ts_uprilim,0);
			return(EPERM);
		}
	}
	...
	if ((tspp->ts_flags & TSKPRI) != 0) {
		/*
         	* Write audit record
	 	*/
		ADT_PARMSSET(ADT_SCHED_TS,0,ADT_TS_PARMSSET,tspp->ts_procp->p_pid,tsparmsp->ts_upri,tsparmsp->ts_uprilim,0);
...
/* Swap out priority...
 *
 * 1. How long the process has been sleeping.
 *    - the longer it is sleeping, the higher its score.
 * 2. The global priority of the process.
 *    - the lower its priority, the higher its score
 * 3. How much memory it is using.
 *    - the larger its resident set, the higher its score.
 * 4. The time that the process has been in core.
 *    - the longer it has been in core, the higher its score.
 *
 ****
 *
 * We calculate the desirability of swapping a process back in as
 * a combination of (swap in the lowest score):
 *
 * 1. The global priority of the process.
 *    - the lower its priority, the higher its score
 * 2. How much memory it is using.
 *    - the larger its resident set (when it was last swapped out),
 *	the higher its score.
 * 3. The time that the process has been on swap.
 *    - the longer the process has been out, the lower its score.
 */
...
int	*runflagp;
{
	...
	*runflagp = 0;
	for (tspp = ts_plisthead.ts_next; tspp != &ts_plisthead;
		...
		if (pstat == SRUN) {
			if ((pflag & SLOAD) == 0) { 
				if ((score=((SWPSCORE(tspp)) + PRISCORE(tspp)
				    -AGESCORE(tspp))) < minscore) {
					retpp = tspp;
					minscore = score;
				}
			} else {
...
boolean_t	*unloadokp;
{
	register tsproc_t	*tspp;
	register tsproc_t	*retpp;
	int	maxsscore;		/* max score for sleeping process */
	int	maxrscore;		/* max score for runable process */
	int	oldlvl;
	int	runable_score, sleep_score;
	int	sleeper = 0;

	retpp = NULL;
	maxsscore = maxrscore = 0;
	oldlvl = splhi();
	...
	for (tspp = ts_plisthead.ts_next; tspp != &ts_plisthead;
		...
		if (tspp->ts_procp == jl)
			continue;
		if (pstat == SSLEEP || pstat == SSTOP) {
			if ((SLPSCORE(tspp) >= MAXSLEEPTIME) &&
			   (AGESCORE(tspp) >= MAXINOUTAGE)) {
				retpp = tspp;
				break;
			} else {
				if ((sleep_score=(MEMSCORE(tspp)+SLPSCORE(tspp)
				   +AGESCORE(tspp))) > maxsscore){
					retpp = tspp;
					maxsscore = sleep_score;
					sleeper = 1;
				}
			}
		} else {
			if (sleeper == 0 && pstat == SRUN && 
			(runable_score=(PRISCORE(tspp)+MEMSCORE(tspp)
			+AGESCORE(tspp))) > maxrscore) {
				retpp = tspp;
				maxrscore = runable_score;
			}
		}
...
tsproc_t	*tspp;
{
	if ((tspp->ts_flags & TSKPRI) == 0)
        ...
        if (tspp->ts_sleepwait > TS_SLEEPWAIT) {
                tspp->ts_cpupri = ts_dptbl[tspp->ts_cpupri].ts_slpret;
                tspp->ts_timeleft = ts_dptbl[tsumdpri].ts_quantum;

                /* Whenever we bump the quantuum, decay the sleepwait. */
                if (tspp->ts_sleepwait > TS_SLEEPMAX)
                       tspp->ts_sleepwait = TS_SLEEPMAX / 2;
                else
                       tspp->ts_sleepwait = tspp->ts_sleepwait / 2;
        }
	...
	tspp->ts_dispwait = 0;
        ...
        if (npwakecnt == 0) {
                if (curpri < maxrunpri) {
                        runrun++;
                } else if (curpri == maxrunpri && tsumdpri == ts_maxumdpri) {
                        tspp->ts_flags |= TSBACKQ;
                        runrun++;
                }
        }
...
ts_update()
{
	...
	for (tspp = ts_plisthead.ts_next; tspp != &ts_plisthead;
		...
		if ((tspp->ts_flags & TSKPRI) != 0) {
			/*
                         * If the process is sleeping -- bump it's sleepwait.
                         */
                        if (pstat == SSLEEP)
                                tspp->ts_sleepwait += 8;

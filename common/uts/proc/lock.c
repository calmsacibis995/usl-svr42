...
#ident	"@(#)uts-comm:proc/lock.c	1.7.4.2"
#ident	"$Header: $"
...
/*
 * Lock specified portion of process memory on request.
 */
...
/*
 * Unlock the current process and its u-block.
 */
STATIC void
ubunlock()
{
...
/*
 * Unlock pages of memory.  Called by memcntl() when as_ctl()
 * fails during a request to lock or unlock memory.
 */
STATIC void
...
size_t	nbits;
{
	...
	/*
	 * Loop through the address range and unlock pages
	 * via as_ctl().
	 */

...
#ident	"@(#)uts-comm:util/bitmap.c	1.2.3.2"
#ident	"$Header: $"
	...
	size_t			nbits;		/* number of bits in bitmap */
{
	...
	/* number of words to be searched in bitmap */
	...
	if (wx <= maxword) {
		...
		/* determine how many bits will be searched in the word */
	...
	size_t	nbits;			/* number of bits in bitmap */
{
	...
	/* starting at pos1 in bitmap search for first bit that is on */
	...
	/* set pos1 to index of first bit that is on */
	*pos1 = inx;

	/* search for first bit that is off */
	for (; inx < nbits; inx++)
		if (!BT_TEST(bitmap, inx))
			break;	

	/* set pos2 to index of the last bit that is on */
	*pos2 = inx - 1;

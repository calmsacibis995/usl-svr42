...
#ident	"@(#)uts-comm:util/list.c	1.2.3.2"
#ident	"$Header: $"
 ...
 * Reset result's list pointers to empty list state.
 * Argument p is the header of the list.
 */
struct ls_elt *
ls_remque(p)
	struct ls_elt *p;
...
/* 
 * Unlink denoted element from list.
 * Reset element's pointers to empty list state.
 */
void
ls_remove(p)
	struct ls_elt *p;

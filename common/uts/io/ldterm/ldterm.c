...
#ident	"@(#)uts-comm:io/ldterm/ldterm.c	1.14.4.9"
#ident	"$Header: $"
/*
 * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
STATIC	void ldtermopen_wakeup(/*caddr_t addr;*/);
STATIC	mblk_t *ldterm_docanon(/*unsigned char c, mblk_t *bpt, int ebsize,
    ldtermstd_state_t *tp*/);
STATIC	int ldterm_unget(/*ldtermstd_state_t *tp*/);
STATIC	void ldterm_trim(/*ldtermstd_state_t *tp*/);
STATIC	void ldterm_rubout(/*unsigned char c, queue_t *q, int ebsize,
    ldtermstd_state_t *tp*/);
STATIC	int ldterm_tabcols(/*ldtermstd_state_t *tp*/);
STATIC	void ldterm_erase(/*queue_t *q, int ebsize, ldtermstd_state_t *tp*/);
STATIC	void ldterm_werase(/*queue_t *q, int ebsize, ldtermstd_state_t *tp*/);
STATIC	void ldterm_tokerase(/*queue_t *q, int ebsize, ldtermstd_state_t *tp*/);
STATIC	void ldterm_kill(/*queue_t *q, int ebsize, ldtermstd_state_t *tp*/);
STATIC	void ldterm_reprint(/*queue_t *q, int ebsize, ldtermstd_state_t *tp*/);
STATIC	mblk_t *ldterm_dononcanon(/*mblk_t *bp, mblk_t *bpt, int ebsize,
    queue_t *q, ldtermstd_state_t *tp*/);
STATIC	int ldterm_echo(/*unsigned char c, queue_t *q, int ebsize, ldtermstd_state_t *tp*/);
STATIC	void ldterm_outchar(/*unsigned char c, queue_t *q, int bsize, ldtermstd_state_t *tp*/);
STATIC	void ldterm_outstring(/*unsigned char *cp, int len, queue_t *q,
    int ebsize, ldtermstd_state_t *tp*/);
STATIC	mblk_t *newmsg(/*ldtermstd_state_t *tp*/);
STATIC	void ldterm_msg_upstream(/*queue_t *q, ldtermstd_state_t *tp*/);
STATIC	mblk_t *ldterm_output_msg(/*queue_t *q, mblk_t *imp, ldtermstd_state_t *tp,
    int echoing*/);
STATIC	void ldterm_flush_output(/*unsigned char c, queue_t *q, ldtermstd_state_t *tp*/);
STATIC	void ldterm_dosig(/*queue_t *q, int sig*/);
STATIC	void ldterm_do_ioctl(/*queue_t *q, mblk_t *mp*/);
STATIC	int chgstropts(/*struct termios *oldmodep, struct ldtermstd_state_t *tp,
    queue_t *q*/);
STATIC	void ldterm_ioctl_reply(/*queue_t *q, mblk_t *mp*/);
STATIC	void ldterm_vmin_timeout(/*queue_t *q*/);
STATIC	void ldterm_adjust_modes(/* struct ldterm_state_d *tp*/);
STATIC	void ldterm_euc_erase(/* q, ebsize, tp*/);
STATIC	void ldterm_eucwarn(/* tp */);
STATIC	void cp_eucwioc(/* from, to, dir*/);
STATIC	int ldterm_memwidth(/*c, w*/);
STATIC	int ldterm_dispwidth(/*c, w, mode*/);
STATIC	int ldterm_codeset(/*c*/);
	...
	cred_t *crp;
{
	...
	bzero((caddr_t)&tp->eucwioc, EUCSIZE);	/* zero out eucioc structure */
 	/* zero out emap_info structure */
	bzero((caddr_t)&tp->emap_i, sizeof(struct emap_info));
 ...
 */
	 ...
	 */
	if(tp->emap_i.emap_mp);
		freemsg(tp->emap_i.emap_mp);
	tp->emap_i.emap_mp = tp->emap_i.emap_lmp =  NULL;
	str_emunmap(q,&tp->t_emap,0); /* free up buffers holding codeset maps */
	str_nmunmap(q,&tp->t_emap,0); /* free up buffers holding codeset maps */
...
#ifdef DEBUG
		   cmn_err(CE_NOTE,
		      "LDTERM: LDSMAP pullupmsg failed:state %d \n",
					tp->emap_i.emap_reqstate);
#endif
	 	   ldterm_iocnack(q,mp,iocp,EINVAL,-1);
		   return;
			
		}
		switch(tp->emap_i.emap_reqstate){
			default: 
#ifdef DEBUG
			cmn_err(CE_NOTE, 
			     "LDTERM: LDSMAP: bad emap state %d\n",
					tp->emap_i.emap_reqstate);
#endif
			ldterm_iocnack(q,mp,iocp,EINVAL,-1);
			return;
			case READ_EMAP_BUFS: 
				if ( tp->emap_i.emap_nbuf == -1 ) {
				   ep = (struct emtab *)mp->b_cont->b_rptr;
				   nbuf =  (int) (ep->e_stab >> EMBSHIFT ) ;
				   if ( nbuf < 0 || nbuf >= NEMBUFS ) {
#ifdef DEBUG
			cmn_err(CE_NOTE,
				"LDTERM: LDSMAP bad nbuf %d \n", nbuf);
#endif
				      ldterm_iocnack(q,mp,iocp,EINVAL,-1);
				      return;
				   }
				   tp->emap_i.emap_nbuf =   nbuf;
				   tp->emap_i.emap_lmp = 
				       tp->emap_i.emap_mp = mp->b_cont;
				   tp->emap_i.emap_mp->b_cont = NULL;
				   mp->b_cont = NULL;
				}
				else {
				   tp->emap_i.emap_lmp->b_cont = mp->b_cont;
				   tp->emap_i.emap_lmp = mp->b_cont;
				   tp->emap_i.emap_lmp->b_cont =  NULL;
				   mp->b_cont = NULL;
				}
#ifdef DEBUG
				cmn_err(CE_NOTE,
				   "LDTERM: LDSMAP uaddr=%x nbuf=%d\n",
				tp->emap_i.emap_uaddr,tp->emap_i.emap_nbuf);
#endif
				if ( tp->emap_i.emap_nbuf == 0) goto alldone;
		   		tp->emap_i.emap_nbuf--; 
		   		tp->emap_i.emap_uaddr +=  E_TABSZ;
		   		tp->emap_i.emap_reqstate =  READ_EMAP_BUFS;
				crqp = (struct copyreq *)mp->b_rptr;
				crqp->cq_addr =  tp->emap_i.emap_uaddr;
...
alldone:
				tp->emap_i.emap_reqstate = 99; /* unknown state */
				tp->emap_i.emap_nbuf =  -1;
		   		tp->emap_i.emap_uaddr =  (caddr_t) NULL;
				error = str_emsetmap(q,tp->emap_i.emap_mp,&tp->t_emap);
				freemsg(tp->emap_i.emap_mp);
				tp->emap_i.emap_mp = 
					tp->emap_i.emap_lmp =  NULL;
				if (error) 
				{
#ifdef DEBUG
				   cmn_err(CE_NOTE,
				   "LDTERM: str_emsetmap error=%d\n", error);
#endif
	...
	case NMSMAP: {
		...
		if (pullupmsg(mp->b_cont,E_TABSZ) == 0) {
#ifdef DEBUG
			I18N_DEBUG( "LDTERM: NMSMAP pullupmsg failed\n");
#endif
			ldterm_iocnack(q,mp,iocp,EINVAL,-1);
			return;
		}
	...
	register mblk_t *mp;
{
	...
	case LDSMAP:  {
		...
		if (iocp->ioc_cmd == LDSMAP) {
		   tp->emap_i.emap_uaddr = crqp->cq_addr; 
		   tp->emap_i.emap_reqstate =  READ_EMAP_BUFS;
		   tp->emap_i.emap_nbuf =  (int) -1;
		   tp->emap_i.emap_mp =  (mblk_t *) NULL;
		   tp->emap_i.emap_lmp =  (mblk_t *) NULL;
		}
		qreply(q,mp);
		return;
	}
...
*/
...
#ifdef DEBUG
			I18N_DEBUG( "LDTERM: LDNMAP\n");
#endif
			str_emunmap(q,&tp->t_emap,1);
		}
		else {
#ifdef DEBUG
			I18N_DEBUG( "LDTERM: NMNMAP\n");
#endif
	...
	case TCSETSF: {
		...
		if((tp->t_modes.c_cflag&CBAUD) == B0)

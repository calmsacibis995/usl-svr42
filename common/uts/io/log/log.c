...
#ident	"@(#)uts-comm:io/log/log.c	1.9.3.5"
#ident	"$Header: $"
...
STATIC int logopen(), logclose();
STATIC int logwput(), logrsrv();
...
extern struct log log_log[];	/* log device state table, defined in log.cf */
extern int log_cnt;		/* # of minor devices, defined in log.cf */
extern ulong_t loghiwat;	/* tunable high water mark */
extern ulong_t loglowat;	/* tunable low water mark */

/*
 * Defining the variable logdevflag indicates that this driver is written
 * to use the new style driver open/close interface.
 * The flags set security attributes to have device state as public and no
 * MAC access checks for data transfer.
 */
int logdevflag = (D_INITPUB |D_NOSPECMACDATA);		

/* control structure for covert channel limiter */
STATIC ccevent_t cc_re_log = { CC_RE_LOG, CCBITS_RE_LOG };
 ...
 *
 * The VA_ macros are defined in util/cmn_err.h and should follow
 * the definitions the CCS folk made for the user header stdarg.h
 */

...
#ident	"@(#)uts-comm:io/stream.c	1.13.3.8"
#ident	"$Header: $"
...
extern char strbcwait;		/* bufcall functions are waiting */
struct mdbblock *xmdballoc();
struct msgb	*xmsgalloc();
extern struct striopst	*strioclist;

#ifdef	DEBUG
KSTATIC int	_streams_crash = 1; /* to signal the crash routines */   
#else
KSTATIC int	_streams_crash = 0;
#endif
...
#define	 _FREEB(msgp) {							\
	...
	if ( --(dbp->db_ref) == 0 ) { 					\
		if ((char *)dbp + sizeof(struct dbinfo) != 		\
		 (char *)(dbp->db_base)) { 				\
...
/*
 * Allocate message and data block.
 *
 ...
 * the given priority.
 *
 * Note: this function will be phased out in the future.  It is intended to
 * provide a hint as to whether an allocb is likely to succeed.  Since
 * intervening interrupts can change the memory configuration, the hint
 * from testb is not guaranteed to be correct and the caller of allocb must
 * be prepared for failure anyhow.  Because of this, just return true instead
 * of spending a lot of time figuring out the "right" answer.
 */
/* ARGSUSED */
int
testb(size, pri)
...
/*
 * Register function for ioctl post-processing.  Return 0 if the function
 * is successfully registered, 1 otherwise.
 */
strioccall(func, arg, iocid, q)
	int (*func)();
	caddr_t arg;
	long iocid;
	queue_t *q;
 ...
 */
	s = splstr();
	sp = (struct striopst *)findioc(iocid);
	splx(s);
	if (sp == NULL) {

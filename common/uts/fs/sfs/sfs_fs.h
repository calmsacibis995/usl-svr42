...
#ifndef _FS_SFS_SFS_FS_H	/* wrapper symbol for kernel use */
#define _FS_SFS_SFS_FS_H	/* subject to change without notice */
#ident	"@(#)uts-comm:fs/sfs/sfs_fs.h	1.4.2.3"
#ident	"$Header: $"
 ...
 */
#define SFSROOTINO      ((ino_t)2)      /* i number of all roots */
#define LOSTFOUNDINO    (SFSROOTINO + 2)
 ...
 * MINBSIZE was originally set to 4096 to avoid ever hitting the
 * triple indirection code.  However, there was a 20% gaede throughput
 * difference between systems with ufs 4k filesystems and ufs 2k
 * filesystems on a 3b2,  (and similarly for sfs 2K and 4K filesystems,)
 * so now we're allowing ufs and sfs 2K filesystems.  We suspect the
 * reason for the slowdown when using 4k blocks is caused by excess I/O
 * when blocksize exceeds pagesize.  On 2k filesystems, files larger 
 * than half a gigabyte will use the triple indirection blocks.
 *
 * MINBSIZE must be big enough to hold a cylinder group block,
 * thus changes to (struct cg) must keep its size within MINBSIZE.
 * Currently struct cg is under 1k on a 3b2.
...
/*
 * Values for fs_clean flag
 */
#define FsOKAY		0x4f4b4159	/* file system unmounted cleanly */
#define FsACTIVE	0xac544956	/* file system active */
					/* or unmounted in invalid state */
/*
...
#define insque(q,p)     sfs_insque((caddr_t)q,(caddr_t)p)
#define remque(q)       sfs_remque((caddr_t)q)
...
#define getfs(vfsp) ((struct fs *)((struct sfs_vfs *)vfsp->vfs_data)->vfs_bufp->b_un.b_addr)


#endif /* _FS_SFS_SFS_FS_H */

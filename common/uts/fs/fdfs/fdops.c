...
#ident	"@(#)uts-comm:fs/fdfs/fdops.c	1.7.3.3"
#ident	"$Header: $"
	...
	struct cred *cr;
{
	...
	if (uiop->uio_offset < 0
	  || uiop->uio_offset >= (nentries + 2) * FDFSSDSIZE
	  || uiop->uio_resid <= 0)
		return 0;
	if (uiop->uio_offset < 2*FDFSSDSIZE) {
		error = uiomove((caddr_t)dotbuf + uiop->uio_offset,
		  min(uiop->uio_resid, 2*FDFSSDSIZE - uiop->uio_offset),
		  UIO_READ, uiop);
		if (uiop->uio_resid <= 0 || error)
			return error;
	}
	minfd = (uiop->uio_offset - 2*FDFSSDSIZE)/FDFSSDSIZE;
	...
	cred_t *cr;
{
	if (vp->v_type == VDIR) {
		vap->va_nlink = 2;
		vap->va_size = (u.u_nofiles + 2) * FDFSSDSIZE;
		vap->va_mode = 0555;
...
STATIC struct vfsops fdfsvfsops = {
	fdfsmount,
	fdfsunmount,
	fdfsroot,
	fdfsstatvfs,

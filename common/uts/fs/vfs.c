...
#ident	"@(#)uts-comm:fs/vfs.c	1.18.2.9"
#ident	"$Header: $"
...
STATIC ccevent_t cc_spec_sync = { CC_SPEC_SYNC, CCBITS_SPEC_SYNC };
...
STATIC int sync_active;		/* sync currently in progress, fix for
	...
	rval_t *rvp;
{
	...
	if (uap->flags & MS_REMOUNT) {
		...
		VFS_INIT(vfsp, fstype, (caddr_t) NULL);
	}
...
mod_mount_again:
	/* if the fs is loadable and is loaded, increase the reference count */
	if((mcp = mod_shadowvfssw[fstype]) && !remount)
		MOD_HOLD(mcp);

	if (error = VFS_MOUNT(vfsp, vp, uap, u.u_cred)) {
		/* ENOLOAD indicates that fs is loadable, but not loaded */
		if(!remount && error == ENOLOAD) {
			/* try to load the fs */
			if(modld(mod_fsname(fstype),sys_cred, &mcp) == 0) {
				/*
				 * The previous vfsops value was invalid,
				 * now the module is loaded, set the vfsops.
				 */
		...
		if (remount) {
			 ...
			 * Also set the ceiling level and the floor level
...
/*
 * Unmount file system given its vfs entry pointer (vfsp).
 */
int
dounmount(vfsp, cr)
	register struct vfs *vfsp;
	register cred_t *cr;
{
	...
	extern	void	pm_clrdev();		/* for security */
	int error;
	...
	if (error = VFS_UNMOUNT(vfsp, cr))
		...
		if(mod_shadowvfssw[vfsp->vfs_fstype])
			MOD_RELE(mod_shadowvfssw[vfsp->vfs_fstype]);
		vfs_remove(vfsp);
...
sync()
{
	...
	cc_limiter(&cc_spec_sync, u.u_cred);

	(void) random(0L);		/* reseed random number generator */
...
/*
 * System call to update every mounted file system.
 */
...
/*
 * vfs_mountroot is called at startup time to mount the root filesystem.
 */
void
vfs_mountroot()
{
	...
	if (vsw = vfs_getvfssw(rootfstype)) {
		VFS_INIT(rootvfs, vsw - vfssw, (caddr_t)0);
...
/*
 * Search the vfs list for a specified file system id.  Returns a pointer
 * to it or NULL if no suitable entry is found.
 */
struct vfs *
getvfs(fsid)
	fsid_t *fsid;
...
/* Find an index into vfssw given a name- used by loadable file system types */
int 
vfs_getvfsswind(const char *fsname)
{
	vfssw_t *vswp;
	if((vswp = vfs_getvfssw(fsname)) == NULL)
		cmn_err(CE_PANIC,"MOD: Trying to get index of unknown FS name %s.\n", fsname);
	return(vswp - vfssw);
}
...
/*
 * Calls on all configured file system types to do appropriate
 * initialization.  Called at system startup time.
 */
void
vfsinit()
{

...
#ident	"@(#)uts-comm:fs/s5fs/s5alloc.c	1.5.3.1"
#ident	"$Header: $"
...
void	blkfree(), ifree(), s5indirtrunc();
	...
	int flag;
{
	 ...
	 */
	offset = lp->l_start &
		(bsize < PAGESIZE ? PAGEOFFSET : s5vfsp->vfs_bmask);
	if (offset == 0)
		pvn_vptrunc(vp, lp->l_start, (u_int)0);
	else {
		/*
		 * Make sure the last block is properly allocated.
		 */
		lastblock = (lp->l_start - 1) >> s5vfsp->vfs_bshift;
		if (error = bmapalloc(ip, lastblock, lastblock, 0, 0)) {
			IUNLOCK(ip);
		...
		pvn_vptrunc(vp, lp->l_start, MAX(PAGESIZE, bsize) - offset);
	}

...
#ident	"@(#)uts-comm:fs/s5fs/s5dir.c	1.4.2.2"
#ident	"$Header: $"
/*
	...
	*ipp = NULL;
	...
	while (count) {
		...
		if (error = fbread(dvp, offset & bmask, bsize, S_OTHER, &fbp))
			goto out;
		/*
		 * Search directory block.  s5searchdir() returns the offset
		 * of a matching entry, or the offset of an empty entry,
		 * or -1.
		 */
		n = MIN(bsize, count);
		cp = fbp->fb_addr;
		if ((off = s5searchdir(cp, n, dir.d_name)) != -1) {
			cp += off;
	...
	struct cred *cr;
{
	 ...
	 * be deleted (except by a privileged process).
	...
	if (op == DR_RMDIR) {
		 ...
		 * a privileged process to unlink a directory.
		 */

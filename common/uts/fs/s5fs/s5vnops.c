...
#ident	"@(#)uts-comm:fs/s5fs/s5vnops.c	1.13.3.9"
#ident	"$Header: $"
...
#define	ISVDEV(t) ((t) != VREG && (t) == VCHR || (t) == VBLK ||  \
		(t) == VFIFO || (t) == VXNAM)
...
/* ARGSUSED */
STATIC int
s5lookup(dvp, name, vpp, pnp, flags, rdir, cr)
	struct vnode *dvp;
	char *name;
	struct vnode **vpp;
	struct pathname *pnp;
...
/*
 * Link a file. Links to directories are no longer allowed.
 * 
 */
STATIC int
 ...
 */
STATIC int
s5rename(sdvp, sname, tdvp, tname, cr)
	...
	struct cred *cr;
{
	...
	*tnm = '\0';
	...
	if (((error = iaccess(sdp, IWRITE, cr)) != 0)
	  || ((sdp->i_mode & ISVTX) && pm_denied(cr, P_OWNER)
	...
	struct cred *cr;
{
	...
	*dirnm = '\0';
	...
	ip = VTOI(dvp);
	error = direnter(ip, dirnm, DE_MKDIR, (struct inode *) 0,
	  (struct inode *) 0, vap, &xip, cr);
	ITIMES(ip);
	if (error == 0) {
		ip = xip;
...
out:
	...
	ACC_TIMES(ip); /* need to only update access times in inode */
	return error;
}
	...
	struct cred *cr;		/* user credentials */
{
	...
	*linknm = '\0';
	...
	if ((error = direnter(dip, linknm, DE_CREATE, (struct inode *) 0,
	...
	struct cred *cr;
{
	...
	/*
	 * only check if access time is modified after a read 
	 */
	ACC_TIMES(ip); 	
	IUNLOCK(ip);
	return error;
	...
	struct cred *cr;
{
	...
	IRWLOCK(ip);
	ILOCK(ip);
	error = syncip(ip, 0, 0);		/* Do synchronous writes */
	ITIMES(ip);
	IUNLOCK(ip);
	IRWUNLOCK(ip);
	return error;
		...
		if (do_bmap) {
			...
			for (i = 0, bnp = bn; i < multi_io; i += 2) {
				if (ip->i_size <= curoff)
					break;
				bnp2 = bnp + 1;
				err = bmap(ip, lbn+i, bnp, bnp2, S_OTHER, 0);
				if (err)
					break;
...
/* ARGSUSED */
STATIC int
s5map(vp, off, as, addrp, len, prot, maxprot, flags, fcr)
	struct vnode *vp;
	u_int off;
	struct as *as;
	caddr_t *addrp;
	...
	struct cred	*cred;
{
	...
	return bmapalloc(VTOI(vp), firstlbn, lastlbn, 1, NULL);
}

...
#ident	"@(#)uts-comm:fs/s5fs/s5rdwri.c	1.6.3.2"
#ident	"$Header: $"
	...
	int ioflag;
{
	...
	register off_t isize = ip->i_size;
	int flags, error = 0;
	off_t off;
	caddr_t base;
	...
	do {
		...
		n = (isize  < uiop->uio_offset) ? 0 : MIN(n, isize - uiop->uio_offset);
		if (n == 0)
			break;
		...
		if ((error = uiomove(base+on, n, UIO_READ, uiop)) == 0) {
			 ...
			 */
			if (n + on == MAXBSIZE || uiop->uio_offset == isize)
				flags = SM_DONTNEED;
	...
	int ioflag;
{
	...
	int part_write = 0;
	int sects;
	daddr_t dblist[MAXBSIZE >> SCTRSHFT];
	...
	while (error == 0 && uiop->uio_resid > 0) {
		...
		for (i = 0; i < (MAXBSIZE/NBPSCTR); i++)
			dblist[i] = 0;
		error = bmapalloc(ip, firstlbn, lastlbn, pagecreate, &dblist[0]);
		if (error) {

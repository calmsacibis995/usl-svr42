...
#ident	"@(#)uts-comm:fs/bfs/bfs_vnops.c	1.14.3.4"
#ident	"$Header: $"
	...
	struct cred *cr;
{
	...
	if (vp->v_type != VDIR) {			/* Vnode is not root */
	...
	register struct cred *cr;
{
	...
	BFS_INOLOCK(bs, BFS_OFF2INO((off_t)vp->v_data));
	error =BFS_GETINODE(BFS_DEVNODE(vp->v_vfsp), (off_t)vp->v_data, dir,cr);
	if (error) {
		kmem_free((caddr_t)dir, sizeof(struct bfs_dirent));
		BFS_IOEND(bs);
		BFS_INOUNLOCK(bs, BFS_OFF2INO((off_t)vp->v_data));
	...
	if (mask & AT_SIZE) {
		...
		if (bs->bsup_lastfile == (off_t)vp->v_data) {
			...
			for (l = BFS_DIRSTART; l < bs->bsup_start;
				...
				if (error) {
					kmem_free((caddr_t)dir,
						sizeof(struct bfs_dirent));
					kmem_free((caddr_t)tdir,
						sizeof(struct bfs_dirent));
					BFS_IOEND(bs);
					BFS_INOUNLOCK(bs,
						BFS_OFF2INO((off_t)vp->v_data));
					return error;
				}
...
out:
	new_error = BFS_PUTINODE(BFS_DEVNODE(vp->v_vfsp), vp->v_data, dir, cr);
	...
	struct cred *cr;
{
	...
	if (dir->d_eblock - dir->d_sblock > BIGFILE) {
		...
		error =BFS_GETINODE(bs->bsup_devnode,bs->bsup_lastfile,ldir,cr);
		if (error) {
			BFS_INOUNLOCK(bs, BFS_OFF2INO((off_t)rvp->v_data));
			kmem_free(dir, sizeof(struct bfs_dirent));
	...
	struct cred *cr;
{
	...
	if (error = bfs_iaccess(tdvp, VWRITE, cr))
		return error;
	...
	struct cred *cr;
{
		...
		if (excl == NONEXCL) {
			...
			if (mode &&
				...
				if ((vap->va_mask & AT_SIZE) &&
				    vap->va_size == 0) {
					error = bfs_truncate(bs, dir_off, 0,cr);
					if (error) {
						VN_RELE(avp);
						return error;
					}
	 ...
	 */
	error = bfs_nextfile(bs, &dir_off, &block_no, cr);
	if (error) {
		BFS_IOEND(bs);
		kmem_free(dir, sizeof(struct bfs_dirent));
		return error;

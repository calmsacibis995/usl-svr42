...
#ident	"@(#)uts-comm:fs/procfs/prsubr.c	1.8.3.3"
#ident	"$Header: $"
...
/*
 * Copyout a mapped u-block to an RFS server.
 * Since rcopyout() doesn't protect against SEGU fault errors,
 * we have to copy it to a local buffer first.
 * Since the buffer is big, and this should get infrequent use,
 * we dynamically allocate and free the buffer on each use.
 */
int
prrgetu(up, addr, len)
	caddr_t	up;
	...
	u_int	len;
{
	...
	error = rcopyout(buf, addr, len, NULL);
	kmem_free(buf, len);
	return error;
}

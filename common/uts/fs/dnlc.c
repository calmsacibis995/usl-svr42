...
#ident	"@(#)uts-comm:fs/dnlc.c	1.6.3.2"
#ident	"$Header: $"
...
extern	int	nchash_size;
extern	struct nc_hash	nc_hash[];
#define	NC_HASH(namep, namelen, vp)	\
	((namep[0] + namep[namelen-1] + namelen + (((int) vp)>>8)) & (nchash_size-1))
...
STATIC ccevent_t cc_cache_dnlc = { CC_CACHE_DNLC, CCBITS_CACHE_DNLC };

STATIC struct ncstats ncstats;		/* cache effectiveness statistics */
...
dnlc_purge()
{
	...
	ncstats.purges++;
start:
	for (nch = nc_hash; nch < &nc_hash[nchash_size]; nch++) {
		ncp = nch->hash_next;
	  ...
	  ncp = ncp->hash_next) {
		if (ncp->dp == dp && ncp->namlen == namlen
		  ...
		  && bcmp(ncp->name, name, namlen) == 0) {
			 ...
			 */
			if (ncp->lid
			&&  MAC_ACCESS(MACEQUAL, ncp->lid, u.u_cred->cr_lid)) {
				ncp->lid = (lid_t)0;
				cc_limiter(&cc_cache_dnlc, u.u_cred);
				goto retry;
			}
			return ncp;
		}

...
#ident	"@(#)uts-comm:fs/xnamfs/xnamsubr.c	1.3.2.4"
#ident	"$Header: $"
...
MOD_FS_WRAPPER(xnam, xnamload, NULL, "XNAMFS: XENIX Sem/Sh Mem");
	...
	struct cred *cr;
{
	...
	if (vp == NULL || (dev-1) >= XNAMTABLESIZE)
		return NULL;
...
/*
 * Pre-validate attributes that will be passed to a subsequent xnamvp() call.
 */
/*ARGSUSED*/
int
xnampreval(type, dev, cr)
	vtype_t type;
	dev_t dev;
	struct cred *cr;
{
	if ((dev-1) >= XNAMTABLESIZE)
		return EINVAL;
	else
		return 0;
}

...
#ident	"@(#)uts-comm:fs/rfs/du.c	1.15.3.6"
#ident	"$Header: $"
...
extern int	acl_valid();	/*for Enhanced Security */
...
STATIC void	du_evn_mk();
STATIC void	du_evn_stat();
STATIC void	du_evn_check();
STATIC int	dusys_acl();
STATIC int	du_acl_set_mac();
STATIC int	du_namesetattr_mac();
STATIC int	du_vn_remove_mac();
STATIC int	du_vn_link_mac();
STATIC int	du_vn_create_mac();
STATIC int	du_vn_open_mac();
STATIC int	du_vn_rename_mac();
STATIC int	du_file_exists();
STATIC int	dusys_chmod_base();
...
/* removed struct exec_ids declaration to du.h */

int du_evn_cnt = 0; /* for ersatz vnodes */
...
STATIC queue_t	fakequeue;
STATIC sndd_t	fakesndd;

STATIC gdp_t fakegdp =
{
	&fakequeue,	/* queue */
	NULL,		/* file */
	0,		/* mntcnt */
...
STATIC vnode_t *fakevn = &(fakesndd.sd_vn);
	...
	cred_t 		*crp;		/* credentials structure */
{
	...
	case DULXSTAT:
	case DUXSTAT:
	case DUDEVSTAT:	/* Enhanced Security */
	case DUMKMLD:	/* Enhanced Security */
	case DULVLFILE:	/* Enhanced Security */
		return dusys_stat(   dvp, comp, vpp, pnp, flags, rdirvp, crp);
	case DUACL:	/* Enhanced Security */
		return dusys_acl(    dvp, comp, vpp, pnp, flags, rdirvp, crp);
	case DUUMOUNT:
	...
	cred_t			*crp;		/* credentials structure */
{
	...
	union rq_arg		rqarg;
	vnode_t			*evp 	= NULLVP;/* will be ersatz */

	if( (error = du_file_exists(dvp, comp, &evp, pnp, flags, rdirvp, crp))
		!= 0){
		if( error != ENOENT ){
			return error;
		}
	}
	error = du_vn_create_mac(dvp, &evp);
	if (evp != NULLVP) {
		VN_RELE(evp);
	}
	if (error != 0) {
	...
	cred_t 		*crp;		/* credentials structure */
{
	...
	/* if	no problem on acesss
	 * and	no problem fetching next component
	 * and	no problem with content of next component (is "..")
	 */
		...
		*/
		if (gp->hetero != NO_CONV) {
			if (!rf_fcanon(STAT_FMT, rpdata, rpdata +
			  datasz, (caddr_t)&stat_info)) {
				gdp_j_accuse("dusys_stat bad data", gp);
				error = EPROTO;
				goto out;
			}
		} else {
			stat_info = *(struct stat *)rpdata;
		}
		hibyte(stat_info.st_dev) = ~(gp - gdp);

		/* create and return an ersatz vnode */
		du_evn_mk(dvp, vpp);
		du_evn_stat(*vpp, &stat_info);
		/*
		VN_HOLD(*vpp);	ersatz vnode is held on creation.
		*/
	...
	cred_t 		*crp;		/* credentials structure */
{
	...
	if( (error =
		du_vn_open_mac(dvp, comp, vpp, pnp, flags, rdirvp, crp)) != 0) {
		return error;
	}
	rqarg = init_rq_arg;
	...
	cred_t 		*crp;		/* credentials structure */
{
	...
	if( (error = du_vn_remove_mac(dvp)) != 0) {
		return error;
	}
	...
	cred_t			*crp;		/* credentials structure */
{
	...
	if( (error = du_vn_rename_mac(dvp)) != 0) {
		return error;
	}
	if (*pnp->pn_path == '\0') {
	...
	cred_t 		*crp;		/* credentials structure */
{
	...
	if( (error = du_namesetattr_mac(dvp)) != 0) {
		return error;
	}
	rqarg = init_rq_arg;
	...
	cred_t 			*crp;		/* credentials structure */
{
	...
	return dusys_chmod_base(dvp, comp, vpp, pnp, flags, rdirvp, crp,
		(mode_t)((struct a *)u.u_ap)->fmode);
}
	...
	cred_t 			*crp;		/* credentials structure */
{
	...
	/*
	 * Obtain time values (if any) from user space.  Canonize (if needed).
	 * Transmit in dusys_utime_pass() below.
	 */
	if( ((struct a *)u.u_ap)->times ){
		if(copyin((caddr_t)((struct a *)u.u_ap)->times,
			  (caddr_t)&utimbuf, sizeof(utimbuf_t))){
			error = EFAULT;
			goto out;
		}
		if(canon != NO_CONV){
			(void)rf_tcanon(UTIME_FMT,
				(caddr_t)&utimbuf, (caddr_t)&utimbuf);
		}
	}
 ...
 * is no new reference, and for creat() of our mount point, in which case
 * we fail with EISDIR.
 * Return 0 for success, nonzero errno for failure.
 * NOTE:
	...
	cred_t		*crp;
{
	...
	if (ISERSATZ_SD(sdp)){
		du_evn_check(vp,  EVN_CONT_VATTR);/*stashed earlier*/
		*vap = VTOST(vp)->EVN_VATTR;
		return 0;
	}
	if (	u.u_syscall == RFOPEN
	||	u.u_syscall == RFCREATE) {
		/*
	...
	/* We have a ``real'' vnode from a prior lookup, either
	 * earlier in this system call or in
	 * a prior system call that produced a file descriptor
	 * Use the send descriptor to get the information now.
	 */
	...
	if ((error = rfcl_op(sdp, crp, DUFSTAT, &rqarg, &bp, TRUE)) == 0 &&
	  (error = RF_RESP(bp)->rp_errno) == 0) {
		...
		if (flags & ATTR_EXEC) {
			...
			/* retrieve exec_ids saved in dusys_exec() */

			if ((dstp = dst_unlink(sdp, u.u_procp->p_pid))
				== NULL){
				ASSERT(vp->v_flag & VROOT);
				/* No prior call to du_lookup() because
				 * pathname to mountpoint resolved entirely
				 * on client.
				 */
				return EACCES; /* [sic] */
			}
	...
	cred_t		*crp;
{
	...
	/*
	 * namefs sets this flag so we can detect its opens of mounted
	 * files.  We can't allow them because, since they are not
	 * preceeded by lookups, old servers haven't opened their files,
	 * and the corresponding closes will put the servers' reference
	 * counts in an undefined state.
	 */
	...
	case AT_MODE:			/* chmod */
		if (u.u_syscall == DUCHMOD) {
			...
			error = dusys_chmod(vp, ".", &resultvp, &pn, 0,
			  NULLVP, crp);
		} else if (u.u_syscall == DUACL ) {	/* acl */
			ASSERT(((struct acla *)u.u_ap)->cmd == ACL_SET);
			ASSERT(vp->v_type == VDIR);
			error = dusys_chmod_base(vp, ".", &resultvp, &pn, 0,
			  NULLVP, crp, vap->va_mode);
		} else {
			error = ENOSYS;
		}
		break;
	...
	register rfsr_ctrl_t	*ctrlp;
{
	...
	if (fmode) {
		 ...
		 */
		/*
		 * Note:  3.x clients don't know about the "exec" check.
		 */

		if (vp->v_type == VFIFO)
			error = MAC_VACCESS(vp, VWRITE, stp->sr_cred);
		else {
			int	exok	= 0; /* for 3.x client */

			error = MAC_VACCESS(vp,
				(exok ? fmode | R_OK : fmode), stp->sr_cred);
		}
		if(!error){
	...
	register rfsr_ctrl_t *ctrlp;
{
	...
	if (vp->v_type != VDIR) {
		 ...
		 */
		if (!(error = MAC_VACCESS(vp, VEXEC, stp->sr_cred))){
	
		        MAC_ASSERT (vp, MAC_DOMINATES);
				/* search access implies
						process must dominate */
	...
	register rfsr_ctrl_t	*ctrlp;
{
	 ...
	 */
	/* ala execpermissions() in proc/exec.c */
	if ( (error = MAC_VACCESS(vp, VEXEC, stp->sr_cred)) != 0){
		VN_RELE(vp);
		return error;
	}
	...
	register rfsr_ctrl_t	*ctrlp;
{
	...
	case F_SETLKW:
		...
		MAC_ASSERT(rvp, MAC_SAME); /* MAC write implies same labels */
		/* FALLTHRU */
	...
	case F_FREESP:
		if (rvp->v_type != VREG) {
			error = EINVAL;
                         ...
                         * question.
                         */
                        if ( (error = MAC_VACCESS(rvp, VWRITE, stp->sr_cred))
					!= 0)
                                break;
                        if (!error)
			{
                                MAC_ASSERT(rvp, MAC_SAME); /* MAC write implies
                                                   same labels */
	...
	register rfsr_ctrl_t	*ctrlp;
{
	 ...
	 */
	if ((fmode & S_IFMT) != S_IFIFO && pm_denied(stp->sr_cred, P_FILESYS)) {
		return EPERM;
	}
	/*
	...
	rfsr_ctrl_t	*sr_ctrlp;
{
	...
	if (filemode & FCREAT) {
		 ...
		 * may have changed after early MAC check in
		 * dusr_vn_create().
		 */
		if (!(tvp->v_macflag & VMAC_DOPEN)
		&&   (error = MAC_VACCESS(tvp, mode, stp->sr_cred)) != 0)
			goto out;
	} else {
		/*
	 ...
	 */
	if ((error = VOP_OPEN(&vp, filemode, stp->sr_cred)) != 0) {
		if(tvp){
			VN_MACCHGOK(tvp);
			VN_RELE(tvp);
		}
		...
		if (filemode & FTRUNC) {
			...
			vattr.va_size = 0;
			if((error
			  = VOP_SETATTR(vp, &vattr, 0, stp->sr_cred)) != 0){
				(void)VOP_CLOSE(vp, filemode, 1, 0,
					stp->sr_cred);
			}
		}
	}
	...
	rfsr_ctrl_t	*sr_ctrlp;
{
	...
	switch (vap->va_type){
		...
		case VCHR:
		case VBLK:
			if (  pm_denied(stp->sr_cred, P_FILESYS)) {
				return EPERM;
			}
			break;
	}
	...
	*vpp = NULL;
	...
	if (*vpp != NULL) {
		...
		if ((*vpp)->v_type != VCHR && (*vpp)->v_type != VBLK) {
			if ((error = MAC_VACCESS(*vpp, mode, stp->sr_cred))
									!= 0) {
				VN_RELE(*vpp);
				goto out;
			}
	...
	if (error == 0) {
		...
		/* In upper level, if no MAC support from fs,
		 * a LID is provided identical to mount point directory.
		 * No point in sending a LID to a 3.x client so we don't.
		 */
	}
out:
	/* in upper level, auditing here as well  */
	pn_free(&pn);
	VN_RELE(dvp);
	return error;
}
	...
	rfsr_ctrl_t	*sr_ctrlp;
{
	...
	if ((error = MAC_VACCESS(tdvp, VWRITE, stp->sr_cred)) != 0){
		goto out;	/* target directory */
	}
	if ((error = MAC_VACCESS( fvp, VWRITE, stp->sr_cred)) != 0){
		goto out;	/* source file */
	}
	...
	register struct statfs	*csbp;	/* pass client pointer back */
{
	...
	if (!(error = VFS_STATVFS(vfsp, &statvfs))) {
		 ...
		 */
		if (MAC_ACCESS(MACDOM, stp->sr_cred->cr_lid,
						vfsp->vfs_macceiling)
...
/* for Enhanced Security  */
STATIC void
du_evn_mk(dvp, vpp)	/* make empty ersatz vnode */
	vnode_t		*dvp;	/* current directory */
	vnode_t		**vpp;	/* returned vnode */
{
	evn_t		*evnp;
	vnode_t		*vp;
	sndd_t		*sdp;
	...
	/* allocate space, include vnode, sndd, and stash contiguously */
	evnp = (evn_t *)kmem_zalloc(sizeof(evn_t), KM_SLEEP);
	...
	/* initialize intra-structure pointers */
	vp->v_data	= (caddr_t)sdp;
	sdp->sd_stashp	= (struct dustash *)stashp;
	sdp->sd_queue	= queuep;
	queuep->q_ptr	= (caddr_t)gdpp;

	sdp->sd_stat	= SDERSATZ;
	stashp->evn_type= EVN_CONT_EMPTY;
	gdpp->version	= QPTOGP(VTOSD(dvp)->sd_queue)->version;

	VN_INIT(vp, dvp->v_vfsp, VNON, (dev_t)0);  /* <= implicit hold */
	vp->v_vfsmountedhere	= NULL;
	vp->v_op		= &rf_vnodeops;
	vp->v_stream		= NULL;
	vp->v_filocks		= NULL;
	vp->v_lid		= (lid_t)0; /* upper level assigns per MP */
	vp->v_macflag		= (u_long)0;/* since VMAC_SUPPORT is clear */
...
STATIC void
du_evn_stat(vp, statp)	/* put stat (and vattr) info into ersatz vnode */
	vnode_t		*vp;	/* really an ersatz vnode */
	struct stat	*statp;	/* stat info */

{
	evn_st_t	*stashp	= VTOST(vp);

	ASSERT(ISERSATZ_VN(vp));

	stashp->EVN_STAT = *statp;
	du_stat_to_vattr(&stashp->EVN_STAT, &stashp->EVN_VATTR);
	vp->v_type = stashp->EVN_VATTR.va_type;
	stashp->evn_type |= EVN_CONT_STAT;
	stashp->evn_type |= EVN_CONT_VATTR;
}

/* ARGSUSED */
STATIC void
du_evn_check(vp, type)
	vnode_t		*vp;	/* hopefully an ersatz vnode with stash */
	evn_contype_t	type;	/* type   of contents to be stashed */
{
	ASSERT(ISERSATZ_VN(vp));
	ASSERT(EVNTOST(VTOEVN(vp))->evn_type & type);
}

/* ARGSUSED */
STATIC int
dusys_acl(dvp, comp, vpp, pnp, flags, rdirvp, crp)
	vnode_t			*dvp;		/* current directory */
	char 			*comp;		/* current component of name */
	vnode_t 		**vpp;		/* return vnode pointer */
	pathname_t 		*pnp;		/* pathname structure */
	...
	cred_t 			*crp;		/* credentials structure */
{
	struct acla		*uap	= (struct acla *)u.u_ap;
	int			error	= 0;
	long			nbytes;
	struct acl		*tmpaclp= (struct acl *)0;
	struct acl		*baseaclp;
	struct vattr		vattr;
	long			dentries = 0;	/* number of default entries */
	long			defaults = 0;	/* indicates defaults allowed */
	struct vnode		*vp	=NULLVP;

	switch (uap->cmd) {
	case ACL_SET:
		if( (error =
		du_file_exists(dvp, comp, &vp, pnp, flags, rdirvp, crp))
			!= 0){
			break;
		}  
		if( (error = du_acl_set_mac(dvp)) != 0){
			break;
		}
		...
		if ((tmpaclp = (struct acl *)kmem_alloc(nbytes, KM_SLEEP))
			== (struct acl *)0) {
			error = ENOMEM;
			break;
		}
		if (copyin((caddr_t)(uap->aclbufp), (caddr_t)tmpaclp, nbytes)
			!= 0) {
			kmem_free((caddr_t)tmpaclp, nbytes);
			error = EFAULT;
			break;
		}
		if (vp->v_type == VDIR)
			defaults++;
		if ((error = acl_valid(tmpaclp, uap->nentries, defaults,
			&dentries)) != 0) {
			kmem_free((caddr_t)tmpaclp, nbytes);
			break;
		}
		error = ENOSYS; /* <-- the VOP that fails */
		if ((error == ENOSYS) && (uap->nentries == NACLBASE)) {
			...
			vattr.va_mask = AT_MODE;
			error = dusys_chmod_base(dvp, comp, vpp, pnp, flags,
					rdirvp, crp, vattr.va_mode);
		}	/* end if error*/
		kmem_free((caddr_t)tmpaclp, nbytes);
		break;
	case ACL_GET:
...
/* ARGSUSED */
STATIC int
dusys_chmod_base(dvp, comp, vpp, pnp, flags, rdirvp, crp, mode)
	vnode_t 		*dvp;		/* current directory */
	char 			*comp;		/* current component of name */
	vnode_t 		**vpp;		/* return vnode pointer */
	pathname_t 		*pnp;		/* pathname structure */
	...
	vnode_t	*dvp;
{
	...
	/* used for covert channel analysis */
...
STATIC int
du_vn_remove_mac(dvp)
	vnode_t	*dvp;
{
	int	error	= 0;
	lid_t	vlid	= dvp->v_lid;
        /*
         * Must have MAC write access to the vnode's parent directory
	 * and be dominated by level on file (to prevent covert channel).
	 */
	...
	vnode_t	*dvp;
{
	...
	if ((error = MAC_VACCESS(fvp, VWRITE, u.u_cred)) == 0)
		error = MAC_VACCESS(tdvp, VWRITE, u.u_cred);
	return error;
}
STATIC int
du_vn_create_mac(dvp, vpp)
	vnode_t	*dvp;
	vnode_t	**vpp;	/* pointer to an ersatz vnode, if file exists */
{
	int	error = 0;
	int	mode;
	int	filemode;
	...
	case DUXMKNOD:
		mode = 0;
		break;
	case RFCREATE:		/* vn_create called from vn_open */
	case DUCOREDUMP:	/* vn_create called from vn_open */
		mode = VWRITE;
		break;
	case RFOPEN:		/* vn_create called from vn_open */
		/* from open */
		filemode = (int)(uap->fmode-FOPEN);
	...
	default:
		cmn_err(CE_PANIC,
			"du_vn_create_mac: unexpected system call, no.=%d\n",
			u.u_syscall);
		break;
	}
	...
	if (*vpp != NULL) {
		...
		if ((*vpp)->v_type != VCHR && (*vpp)->v_type != VBLK) {
			if ((error = MAC_VACCESS(*vpp, mode, u.u_cred)) != 0) {
				/* caller will release ersatz  vnode
                                VN_RELE(*vpp);
				*/
                                goto out;
			}
...
/* ARGSUSED */
STATIC int
du_vn_open_mac(dvp, comp, vpp, pnp, flags, rdirvp, crp)
	vnode_t		*dvp;		/* current directory */
	char		*comp;		/* current component of name */
	vnode_t		**vpp;		/* return vnode pointer */
	pathname_t	*pnp;		/* pathname structure */
	...
	cred_t		*crp;		/* credentials structure */
{
	...
	default:
		cmn_err(CE_PANIC,
			"du_vn_open_mac: unexpected system call, no.=%d\n",
                        u.u_syscall);
		break;
	}
	...
	if (filemode & FCREAT) { 
		...
		if( (error = du_file_exists(dvp, comp, &evp, pnp,
				flags, rdirvp, crp)) != 0){
			if( error != ENOENT ){
				return error;
			}
...
STATIC int
du_vn_rename_mac(dvp)
	vnode_t	*dvp;
{
	int	error	= 0;
	vnode_t	*fdvp	= dvp;
	/*
	vnode_t	*tdvp	= dvp;  not user per below
	vnode_t	*fvp	= dvp;  not used per below
	*/
	...
	/*
        && (error = MAC_VACCESS(tdvp, VWRITE, u.u_cred)) == 0 same lid value
	*/
	){
		/*  fvp has same lid as fdvp and tdvp
		 *  no point in doing another MAC_VACCESS 
		if (fvp->v_type == VDIR)
...
/* ARGSUSED */
STATIC int
du_file_exists(dvp, comp, vpp, pnp, flags, rdirvp, crp)
	vnode_t		*dvp;		/* current directory */
	char		*comp;		/* current component of name */
	vnode_t		**vpp;		/* return vnode pointer */
	pathname_t	*pnp;		/* pathname structure */
...
{
	int		error = 0;
	pathname_t	pn_copy;	/* pathname structure */
	char		buf[MAXPATHLEN];/* duplicate storage */

	/* dusys_stat will update pnp.
	 * duplicate so original can be reused
	 */
	pn_copy = *pnp;
	bcopy((caddr_t)pn_copy.pn_buf, (caddr_t)buf, MAXPATHLEN);
	pn_copy.pn_buf = buf;

	/* create an ersatz vnode for the file, if it exists */
	if((error = dusys_stat(dvp,comp,vpp,&pn_copy,flags,rdirvp,crp)) != 0){
		return error;
	}
	
	/* give the ersatz vnode the lid that would have been assigned
	 * by the upper level
	 */
	(*vpp)->v_lid = dvp->v_lid;

...
#ident	"@(#)uts-comm:fs/rfs/rf_auth.c	1.7.2.4"
#ident	"$Header: $"
...
char     	*Global[2] = {0, 0};    /* global default table ref */
...
void
rf_acl_idmap(direction, gp, aclbufp, nentries)
	enum acl_dir	direction;
	gdp_t		*gp;
	struct acl	*aclbufp;
	int		nentries;
{
	int		i;
	uid_t		(*idmapper)() =
				direction == CL2SR ? glid : rglid;

	for(i = 0; i < nentries; i++, aclbufp++){
		switch (aclbufp->a_type){
		case	    USER:
		case	DEF_USER:
			aclbufp->a_id = (*idmapper)(UID_DEV, gp, aclbufp->a_id);
			break;
		case	    GROUP:
		case	DEF_GROUP:
			aclbufp->a_id = (*idmapper)(GID_DEV, gp, aclbufp->a_id);
			break;
		default:
			break;
		}
...
/*
 *	Note:  the lid value is irrelevant for any client
 *	claiming to run Mandatory Access Control (MAC)
 */
void
rf_setgdplp(gdpp)
struct gdp	*gdpp;
{
	...
	if (rfs_lid_and_priv_p == NULL) { /* hardcoded defaults */
		gdpp->lid  =  (lid_t)0;
		gdpp->priv = (pvec_t)0;
		return;
	}

	/* initialize with adminstrator defined  defaults */
	gdpp->lid  = rfs_lid_and_priv_p->rfloadlp_deflid;
	gdpp->priv = rfs_lid_and_priv_p->rfloadlp_defpriv;

	/* check for explicit entry for client in rfs_lid_and_priv
	 * data structures
	 */
	for(	i = 0,
		n = rfs_lid_and_priv_p->rfloadlp_size/sizeof(rfslp_t),
		p = rfs_lid_and_priv_p->rfloadlp_buf;
		i < n;
		i++, p++
	){
		if(strcmp(gdpp->token.t_uname, p->rflp_hostname) == 0){
			gdpp->lid  = p->rflp_lid;
			gdpp->priv = p->rflp_priv;
			break;
		}
	}
	return; 

...
#ident	"@(#)uts-comm:fs/rfs/rf_cache.c	1.7.2.2"
#ident	"$Header: $"
	...
	while (!error && nbytes) {
		...
		if (pp) {
			...
			if (newp) {
				...
				/*
				 * Another page may have been faulted in since
				 * we did the page_lookup.  Abort it.
				 */
	...
	register ulong		vcode;
{
	...
	if ((rdup = vrdp->rd_user_list) != NULL) {
		RDU_HOLD(rdup);
	}
	while (rdup) {

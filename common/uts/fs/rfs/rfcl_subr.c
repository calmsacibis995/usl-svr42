...
#ident	"@(#)uts-comm:fs/rfs/rfcl_subr.c	1.9.3.2"
#ident	"$Header: $"
	...
	ulong		ulim;
{
	...
	/* Initialize lid and priv fields of request
	 *  Only do so if server is 4.0 or later
	 *  Priviledge goes in a message extension added 4.0
	 *  Nasty things happen if one overwrites a 3.x message
	 */
	if(gdp->version > RFS1DOT0) { 
		/* We are 4.1 or later (or we wouldn't be excuting this code) */
		/* Thus we have some value for privileges, send them */
		reqp->rq_priv     = crp->cr_wkgpriv;
	
	        /*
	         * If MAC is running here send LID
		 * If server is not running MAC (a pre-ES server or an ES server
		 * without MAC on) the server will ignore these values.
		 *
		 */
		if (mac_installed ) {
			cop->co_lid       = crp->cr_lid;
			reqp->rq_cr_flags = crp->cr_flags;
		} else {
			/* if server is running MAC it will assign a value
			 * there according to its data base
			 * if server not running MAC it ignores these fields.
			 */
			cop->co_lid       = (lid_t)0;
			reqp->rq_cr_flags = (long) 0;
		}
	}
	...
	register vfs_t		*vfsp;
{
	...
	lid_t			lid;
	u_long			macflag;
	gdp_t			*gp = QPTOGP(giftsdp->sd_queue);
	...
	/* for Enhanced Security */
	if (gp->mac == RFS_MAC){	/* server has MAC */
		/* use the LID it sent us */
		lid	= cop->co_lid;
		macflag = (u_long)rp->rp_macflag;
	} else {
		/* if we have MAC upper level will set per mount point
		 * else we will ignore
		 */
		lid	= (lid_t)0; 
		macflag	= (u_long)0;
	...
	vp = SDTOV(giftsdp);
/* --- */
	/* finish initializaton */
	/* Enhanced Security */
	/* Initialize its lid and macflag with values returned in the most
	 * recent response.
	 */
	vp->v_lid	= lid;
	vp->v_macflag	= macflag;
/* --- */
	sdp = sndd;
	while (sdp != endsndd) {
	...
	register vfs_t	*vfsp;
{
	...
	register gdp_t		*gp = QPTOGP(giftsdp->sd_queue);

	if (vp->v_type <= VNON || vp->v_type >= VBAD) {
		gdp_j_accuse("rfcl_vn_init unexpected vtype",gp);
		return EPROTO;
	}
	...
	register int		*uio_errorp;
{
	...
	*in_bpp = NULL;
	...
	/*
	 * TO DO:  ASSERT(uiop->uio_offset + count > cwruiop->uio_offset);
	 */
	...
	/*
	 * This loop takes the data to be written in response to the
	 * server's RFCOPYIN message, and chops it into datasz chunks,
	 * where datasz is the maximum amount of data that can be in
	 * an RFS message for this virtual circuit.
	 *

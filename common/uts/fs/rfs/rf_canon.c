...
#ident	"@(#)uts-comm:fs/rfs/rf_canon.c	1.5.2.2"
#ident	"$Header: $"
	...
	register int version;	/* version of RFS protocol */
{
	...
	case RF_NACK_MSG:
		...
		break;
	default:
		cmn_err(CE_WARN, "rf_hdrtcanon: bad type\n");
		return 0;
	}

	/* rf_message_t and rf_common_t header */
	if(rf_tcanon(MSGCOM_FMT, (caddr_t)bp->b_rptr, (caddr_t)bp->b_rptr)==0){
		return 0;
	}
	...
	case RFS2DOT1:
		...
		break;
	default:
		cmn_err(CE_WARN, "rf_hdrtcanon: bad version\n");
	}
	return rf_tcanon(fmt, head, head);
}
...
int
rf_acltcanon(sour, dest, n)
	struct acl	*sour;
	caddr_t		dest;
	int		n;
{
 	int		i, tally = 0;
	size_t		dataun = sizeof(struct acl) + ACL_XP;

        for(i = 0; i < n; i++, sour++, dest+=dataun){
		tally += rf_tcanon(ACL_FMT, (caddr_t)sour, dest);
	}
        return tally;
}
int
rf_aclfcanon(sour, dest, n)
	caddr_t		sour;
	struct acl	*dest;
	int		n;
{
	...
	for(i = 0; i < n; i++, sour+=dataun, dest++){
		if(!rf_fcanon(ACL_FMT, sour, sour+dataun, (caddr_t)dest)){
			return EPROTO;
		}
	}
	return 0;

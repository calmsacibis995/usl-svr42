...
#ident	"@(#)uts-comm:fs/rfs/rf_admin.c	1.12.2.7"
#ident	"$Header: $"
...
rf_daemon())()
{
	...
	/*
	 * Release all resources. On some implementations, relvm()
	 * may not actually release the address space. Hence the
	 * flag ISADAEMON.
	 */
	relvm(rf_daemon_procp, ISADAEMON);
	...
	SDTOV(&reply_port)->v_lid	= (lid_t)0;	/* clear MAC level */
	SDTOV(&reply_port)->v_macflag	= (u_long)0;	/* and macflag */
 ...
 * For now, RFTMO_TIME is tuneable, but this should be specifiable
 * for each virtual circuit, e.g., as a mount option.
 */
extern int rftmo_time;
...
rf_tmo()
{
	...
	/*
	 * Don't timeout rfs circuits.
	 */
	if (rftmo_time == -1) {
		exit(CLD_EXITED, 0);
	}

	u.u_procp->p_cstime = u.u_procp->p_stime = u.u_procp->p_cutime = 
	  u.u_procp->p_utime = 0;

	rftmo_clicks = rftmo_time * HZ;

	/* TO DO:  allocate a real sndd so recovery can find it. */

	out_port.sd_stat = SDUSED;
	SDTOV(&out_port)->v_count	= 1;
	SDTOV(&out_port)->v_lid		= (lid_t)0;	/* clear MAC level */
	SDTOV(&out_port)->v_macflag	= (u_long)0;	/* and macflag */

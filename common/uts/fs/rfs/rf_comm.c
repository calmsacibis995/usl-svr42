...
#ident	"@(#)uts-comm:fs/rfs/rf_comm.c	1.6.2.5"
#ident	"$Header: $"
/*
 ...
 * Also, returns EPROTO on canonization problems.
 */
int
	...
	int			retrans;
{
	...
	if (retrans) {
		if (tgdp->hetero == ALL_CONV) {
			...
			if(rf_mcfcanon(bp) == 0){
				cmn_err(CE_WARN,
			"rf_sndmsg: decanonization error: common header\n");
				return EPROTO;
			}
 			if(rf_rhfcanon(bp, tgdp) == 0){
 				cmn_err(CE_WARN,
 			"rf_sndmsg: decanonization error: request header\n");
 				return EPROTO;
 			}
#endif
		}
	...
	if (tgdp->hetero == ALL_CONV) {
 		if (rf_hdrtcanon(bp, tgdp->version) == 0){
 			rf_freemsg(bp);
 			return EPROTO;
 		}
	}
...
rfesb_pvn_done(wp)
	rfa_work_t	*wp;
{
	buf_t		*bufp = (buf_t *)wp->rfaw_farg;

	/*
	 * We assume that, if we failed to send the message,
	 * we're in recovery, so the page contents are no
	 * longer of interest.  We B_WRITE on and B_READ
	 * off to discourage pageout from trying to write
	 * forever.
	 *
	...
	register cred_t		*crp;
{
	...
	if (mntid >= 0) {
		...
		/*
		 * Caller holds a reference to the rd_user structure, too,
		 * so we can't possibly NULL *rdpp.  If the reference count
		 * on *rdpp goes to zero here, *rdpp will be freed when
		 * the working reference count on the rd_user structure
		 * goes to 0.
		 */

		rdu_del(rdp, sysid, mntid, nrefs, crp);
	} else if (!rdp->rd_refcnt) {

		/* No rd_user structure associated with the reference. */
...
/*
 * Return receive descriptor to the freelist.  If an RDGENERAL receive
 * descriptor, give up vnode reference.  NULL *rdpp.
 */
void
rcvd_free(rdpp)
	rcvd_t		**rdpp;
	...
	register queue_t *qp;
{
	 ...
	 */
	if ((rduptr = rdu_find(rd, sysid, mntid)) != NULL) {
		/* got one in use */
		rduptr->ru_vcount++;
		...
		rduptr->ru_refcnt = rduptr->ru_vcount = 1;
		rduptr->ru_queue = qp;
...
/*
 * Free rd_user structure, NULL *rdupp.  If no more rd_users on *rdpp,
 * free *rdpp and NULL it.
 */
void
rdu_free(rdpp, rdupp)
	rcvd_t			**rdpp;
	rd_user_t		**rdupp;
{
	...
	ASSERT(!rdup->ru_refcnt);

	if (rdup == rdp->rd_user_list) {
		rdp->rd_user_list = rdup->ru_next;
		if (!rdp->rd_user_list && !rdp->rd_refcnt) {
			rcvd_free(rdpp);
		}
	} else {
		register rd_user_t	*prev = rdp->rd_user_list;

		while (prev->ru_next != rdup) {
			prev = prev->ru_next;
		}
		prev->ru_next = rdup->ru_next;
	}
...
/*
 * Return a held reference to the rduser structure corresponding to the given
 * rd, sysid and mntid, or NULL if no such structure was found.
 */
rd_user_t *
rdu_find(rd, sysid, mntid)
	register rcvd_t		*rd;
	register sysid_t	sysid;
	register long		mntid;
{
	...
	register int		fmode;
{
	...
	if (!(rdup = rdu_find(rd, sysid, mntid))) {
		/* error? */
		return;
	}
	...
	register cred_t		*crp;
{
	...
	if (!(rdup = rdu_find(rdp, sysid, mntid))) {
		return;
	}

	/* rd_user structure will be freed when ru_refcnt goes to 0. */
	...
	/* Last reference.  First undo any outstanding opens. */

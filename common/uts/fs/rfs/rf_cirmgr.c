...
#ident	"@(#)uts-comm:fs/rfs/rf_cirmgr.c	1.9.2.4"
#ident	"$Header: $"
...
loop:
		/*
		 * Resetting gp->timeout here could postpone detecting some
		 * circuit timeouts, but only when there is a flurry of
		 * arriving packets just before the timeout.  In that event,
		 * the next timeout will be detected.
		 */
		gp->timeout = 0;
	...
	register gdpmisc_t	*gmp;
{
	...
	int			s;
	register gdp_t		*tmpgp;
	register gdp_t		*endgdp = gdp + maxgdp;

	ASSERT(gdpp->constate == GDPFREE);
	gdpp->constate = GDPDISCONN;

        if ((getf(fd, &fp) != 0) || !(stp = fp->f_vnode->v_stream)) {
		*qpp = NULL;
	...
	for (tmpgp = gdp; tmpgp < endgdp; tmpgp++) {
		if (tmpgp != GDPFREE && tmpgp->queue == qp) {
			*qpp = NULL;
			freeb(bp);
			gdpp->constate = GDPFREE;
			splx(s);
	...
	register queue_t *qp;
{
	...
	/* before returning Q's to stream head, flush them - just in case */
	flushq(qp, FLUSHALL);

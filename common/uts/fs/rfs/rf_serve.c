...
#ident	"@(#)uts-comm:fs/rfs/rf_serve.c	1.7.3.4"
#ident	"$Header: $"
...
extern rcvd_t	*sigrd;
extern int	rf_filtpriv;
extern uid_t	rf_privid;
...
rf_serve()
{
	...
	bzero((caddr_t)&state, sizeof(rfsr_state_t));
	++rfsr_nservers;
...
newmblk:
		...
		/*
		 * gag - u_syscall still needed, but only for sanity
		...
		state.sr_cred->cr_uid  =
		state.sr_cred->cr_ruid = gluid(state.sr_gdpp, cop->co_uid);
		state.sr_cred->cr_gid  =
		state.sr_cred->cr_rgid = glgid(state.sr_gdpp, cop->co_gid);
		...
		/* for Enhanced Security -- define privileges */
		if (state.sr_vcver < RFS2DOT1){	/* pre-ES client */
			/* define privileges according to mapped uid */
			state.sr_cred->cr_wkgpriv =
			state.sr_cred->cr_maxpriv =
				state.sr_cred->cr_uid == rf_privid
					? P_ALLPRIVS  /* keep root status */
					: (pvec_t)0;
		} else { /* ES client, it has sent a privilege */ 
			state.sr_cred->cr_wkgpriv =
			state.sr_cred->cr_maxpriv = req->rq_priv;
		}
		/* filter privileges per lidload command, default no privs */
		/* SUM systems do not; LPM systems do */
		if(rf_filtpriv){
			state.sr_cred->cr_wkgpriv &= state.sr_gdpp->priv;
			state.sr_cred->cr_maxpriv &= state.sr_gdpp->priv;
		}

		/* for Enhanced Security -- define level id's (lids) */
		/* note: currently only flag defines in cr_flags
		 * is VMAC_ISMLD (multi-level directory, a lid feature
		 */ 
		if(mac_installed){ 	/* here  ( on server) */
			if ( state.sr_gdpp->mac	== RFS_MAC ) {	/* MAC client */
				/* initialize from request message*/
				state.sr_cred->cr_lid     = cop->co_lid;
				state.sr_cred->cr_flags   = req->rq_cr_flags;
			} else {
				/* Client does not have MAC.
				 * Client is SVRES (RFS2DOT1) without MAC 
				 * or client is older version (4.0, 3.x).
				 */
				/* use value set via lidload, default 0 */
				state.sr_cred->cr_lid     = state.sr_gdpp->lid;
				state.sr_cred->cr_flags   = (ulong)0;
			}
		} else	{
			/* we will be ignoring lid's, set to zero to be sure */
			state.sr_cred->cr_lid     = (lid_t)0;
			state.sr_cred->cr_flags   = (ulong)0;
		}
		...
		if (state.sr_rdp->rd_vp) {
			state.sr_rdup = rdu_find(state.sr_rdp,
				state.sr_gdpp->sysid, srchan->sd_mntid);
			if (!state.sr_rdup) {
				error = rfsr_discon("No file reference",
				  &state);
				rfsr_rmactive(p);
				SR_FREEMSG(&state);
				continue;
		...
		if (msg->m_stat & RF_SIGNAL) {
			/*
			 * Let remote signals occur in operation context
		...
		if (state.sr_opcode < 0 || state.sr_opcode > RF_MAX_OPCODE) {
			if (state.sr_rdup) {
				RDU_RELE(&state.sr_rdp, &state.sr_rdup);
				state.sr_rdp = NULL;
				state.sr_rdup = NULL;
			}

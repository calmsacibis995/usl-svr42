...
#ident	"@(#)uts-comm:fs/rfs/rf_vnops.c	1.12.3.5"
#ident	"$Header: $"
...
int		rf_nosys();
STATIC int	rf_getacl();
STATIC int	rf_getaclcnt();
STATIC int	rf_setacl();
STATIC int	rf_setlevel();
STATIC int	rf_getdvstat();
STATIC int	rf_setdvstat();
STATIC int	rf_makemld();
...
struct vnodeops rf_vnodeops = {
	...
	rf_setdvstat,
	rf_makemld,
	rf_nosys,	/* filler, to balance rf_getlevel */
	rf_nosys,	/* filler, to balance rf_testmld  */
	rf_nosys,	/* filler */
	rf_nosys,	/* filler */
	rf_nosys,	/* filler */
	rf_nosys,	/* filler */
	...
	cred_t		*crp;
{
	...
	if ((error = rfcl_op(sdp, crp, RFFSYNC, &init_rq_arg, &bp,
				/* style? damage to global resource? */
	  TRUE)) == 0) {
...
rf_mkdir(dvp, nm, vap, vpp, crp)
	...
	cred_t			*crp;
{
	return rf_mkdir_common(dvp, nm, vap, vpp, crp, RFMKDIR,
		"rf_mkdir_pass no file reference");
}
STATIC int
rf_mkdir_common(dvp, nm, vap, vpp, crp, opcode, jmsg)
	vnode_t			*dvp;		/* parent directory */
	char			*nm;		/* name of new dir */
	vattr_t			*vap;		/* attributes of new dir */
	vnode_t			**vpp;		/* out param:  new dir's */
	cred_t			*crp;
	int			opcode;		/* RFMKDIR or RFMAKEMLD */
	char			*jmsg;		/* message for j_accuse() */
{
	register int		error;
	int			nacked;
	int			ntries;
	...
	*vpp = NULLVP;
	...
	for (ntries = nacked = 1; ntries < RFCL_MAXTRIES && nacked; ntries++) {
		...
		rfcl_reqsetup(bp, chansdp, crp, opcode, ULIMIT);
		data = rf_msgdata(bp, headsz);
	...
	if (!error && (error = RF_RESP(bp)->rp_errno) == 0) {
		...
		if (!(msg->m_stat & RF_GIFT)) {
			gdp_j_accuse(jmsg, QPTOGP((queue_t *)msg->m_queue));
			error = EPROTO;
		} else {
			sndd_set(giftsdp, msg->m_queue, &msg->m_gift);
			if ((error = rfcl_findsndd(&giftsdp, crp, bp,
	...
	cred_t		*crp;
{
	...
	/*
	 * namefs sets this flag so du_open can detect its opens of mounted
	 * files.  du  can't allow them because, since they are not
	 ...
	 *
	 * We turn the flag off here to avoid passing it to the innocent.
	 */
...
/* ARGSUSED */
STATIC int
rf_readdir(vp, uiop, fcrp, eofp)
	vnode_t			*vp;
	uio_t			*uiop;
	cred_t			*fcrp;	/* file credential */
	int			*eofp;
{
	register int		error;
	int			nacked;
	 ...
	 * process cred.	[See ul90-26201]
	 */
	crp = crdup(fcrp);
	crp->cr_lid	= u.u_procp->p_cred->cr_lid;
	crp->cr_wkgpriv	= u.u_procp->p_cred->cr_wkgpriv;
	crp->cr_maxpriv	= u.u_procp->p_cred->cr_maxpriv;
...
/* ARGSUSED */
STATIC int
rf_read(vp, uiop, f, fcrp)
	vnode_t		*vp;
	uio_t		*uiop;
	int		f;
	cred_t		*fcrp; /* file credentials */
{
	register sndd_t	*sdp = VTOSD(vp);
 ...
 */
STATIC int
rf_map(vp, off, as, addrp, len, prot, maxprot, flags, fcrp)
	vnode_t			*vp;
	uint			off;
	struct as		*as;
	caddr_t			*addrp;
	...
	cred_t			*fcrp; /* file credentials */
{
	...
	error = as_map(as, *addrp, len, segvn_create, (caddr_t)&crargs);
	crfree(crp);
	return error;
	...
	cred_t			*cred;
{
	...
	if (vp->v_vfsp->vfs_flag & VFS_RDONLY && dirty) {
		...
		/*
		 * We're not really doing async I/O, but we set the
		 * flag for the side-effects; B_ASYNC provokes pvn_done
		 * to call pageio_done.
		 */
...
/* ARGSUSED */
STATIC int
rf_getacl(vp, nentries, dentriesp, aclbufp, cr, rvalp)
	vnode_t		*vp;
	int		nentries;
	int		*dentriesp;
	struct acl	*aclbufp;
	...
	int		*rvalp;
{
	...
	int		error = 0;
	mblk_t		*bp   = NULL;
	union rq_arg	rq_arg = init_rq_arg;
	struct gdp	*gp   = QPTOGP(sdp->sd_queue);
	size_t		hdrsz = RF_MIN_RESP(gp->version);
	int		n;
	size_t		dataun, datasz;
	caddr_t		rpdata;
	char		*jmsg = "rf_getacl bad data";
	int		canon = gp->hetero != NO_CONV;

	rfcl_fsinfo.fsivop_other++;

	if (gp->version < RFS2DOT1 || gp->aclmax == 0) {
		return ENOSYS;
	}
	rq_getacl.nentries = (long)nentries;
	if ((error = rfcl_op(sdp, cr, RFGETACL, &rq_arg, &bp, TRUE))==0){ 
		if ((error = RF_RESP(bp)->rp_errno) == 0) {
			*rvalp = n = (int)RF_RESP(bp)->rp_rval;
			*dentriesp = (int)RF_RESP(bp)->rp_getacl.dentries;
			dataun = canon
				? sizeof(struct acl) + ACL_XP
				: sizeof(struct acl);
			datasz = dataun * n;
			if (RF_PULLUP(bp, hdrsz, datasz)) {
				gdp_j_accuse(jmsg, gp);
				error = EPROTO;
			} else {
				rpdata = rf_msgdata(bp, hdrsz);
				if (canon){
					if ((error = rf_aclfcanon(
							rpdata, aclbufp,
							n )) != 0 ){
							gdp_j_accuse(jmsg, gp);
							goto cleanup;
					...
					bcopy(rpdata, (caddr_t)aclbufp, datasz);
				}
			}
		}
cleanup:
	...
	int		*rvalp;
{
	...
	if ((error = rfcl_op(sdp, cred, RFGETACLCNT, &rq_arg, &bp, TRUE)) == 0){
		if ((error = RF_RESP(bp)->rp_errno) == 0){
			*rvalp = (int)RF_RESP(bp)->rp_rval;
		}
		rf_freemsg(bp);
		bp = NULL;
	}
...
/* ARGSUSED */
STATIC int
rf_setacl(vp, nentries, dentries, aclbufp, crp)
	vnode_t		*vp;
	int		nentries;
	int		dentries;
	struct acl	*aclbufp;
	cred_t		*crp;
{
	...
	/*
	 * validate nentries relative to aclmax on server
	 *	if too large, fail it here and save the trip
	 */
	if (nentries > gp->aclmax) {
		return ENOSPC;
	}
	rq_setacl.nentries = (long)nentries;
	rq_setacl.dentries = (long)dentries;
	...
	for (ntries = nacked = 1; ntries < RFCL_MAXTRIES && nacked; ntries++) {
		...
		rfcl_reqsetup(bp, chansdp, crp, RFSETACL, ULIMIT);
		RF_REQ(bp)->rq_arg = rq_arg;
		data = rf_msgdata(bp, headsz);
		if (canon) {
			(void)rf_acltcanon(aclbufp, data, nentries);
		} else {
			bcopy((caddr_t)aclbufp, data, datasz);
		}
		error = rfcl_xac(&bp, totalsz, rdp, gp->version,FALSE, &nacked);
	}
out:
	rf_freemsg(bp);
	bp = NULL;
	...
	cred_t		*cred;
{
	...
	if (gp->version < RFS2DOT1 || gp->mac == RFS_NO_MAC ) {
		return ENOSYS;
	}

	rq_setlevel.lid = (long)lid;
	if ((error = rfcl_op(sdp, cred, RFSETLEVEL, &rq_arg, &bp, TRUE)) == 0) {
		error = RF_RESP(bp)->rp_errno;
		rf_freemsg(bp);
		bp = NULL;
	}
...
/* ARGSUSED */
STATIC int
rf_getdvstat(vp, dvstatp, cred)
	vnode_t		*vp;
	struct devstat	*dvstatp;
	cred_t		*cred;
{
	register sndd_t	*sdp	= VTOSD(vp);
	int		error	= 0;
	...
	size_t		hdrsz	= RF_MIN_RESP(gp->version);
	size_t		datasz;
	char		*jmsg	= "rf_getdvstat bad data";
	int		canon	= gp->hetero != NO_CONV;
	...
	if ((error = rfcl_op(sdp, cred, RFGETDVSTAT, &rq_arg, &bp, TRUE))==0){ 
		if((error = RF_RESP(bp)->rp_errno) == 0) {
			datasz = canon
			...
			if (RF_PULLUP(bp, hdrsz, datasz)) {
				...
				rpdata = rf_msgdata(bp, hdrsz);
				if (canon){
					if(!rf_fcanon(	DVSTAT_FMT,
							rpdata, rpdata+datasz,
							(caddr_t)dvstatp)) {
	...
	cred_t		*crp;
{
	...
	for (ntries = nacked = 1; ntries < RFCL_MAXTRIES && nacked; ntries++) {
		...
		rfcl_reqsetup(bp, chansdp, crp, RFSETDVSTAT, ULIMIT);
		data = rf_msgdata(bp, headsz);
		if (canon) {
			(void)rf_tcanon(DVSTAT_FMT, (caddr_t)dvstatp, data);
		} else {
			*(struct devstat *)data = *dvstatp;
		}
		error = rfcl_xac(&bp, totalsz, rdp, gp->version,FALSE, &nacked);
	}
out:
	...
	cred_t			*crp;
{
	...
	return rf_mkdir_common(dvp, nm, vap, vpp, crp, RFMAKEMLD,
		"rf_makemld pass no file reference");
}

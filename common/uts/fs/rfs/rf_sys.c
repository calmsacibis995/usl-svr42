...
#ident	"@(#)uts-comm:fs/rfs/rf_sys.c	1.10.2.5"
#ident	"$Header: $"
/*
...
STATIC int	rf_vflg = 0;	/* require (1) host verification or not (0) */
STATIC int	rfud_lastumsg;

STATIC int	rfs_vhigh = RFS2DOT1;
...
STATIC int	rfsys_loadlp();
STATIC void	rfsys_loadlp_recover();
STATIC int	rfsys_loadlp_hostnames();
STATIC void	rfs_gdp_lpupdate();

rfsloadlp_t *rfs_lid_and_priv_p = NULL;
	...
	rval_t			*rvp;
{
	...
	return rf_setidmap(sap->name, sap->flag, sap->map);
}
	...
	rval_t		*rvp;
{
	...
	/* shall privileges be filtered from client requests ?*/
	/* in SUM there is a privileged ID (no error) so don't filter */
	/*		note:  third argument unused for this opcode */
	if( pm_secsys(ES_PRVID, &pm_response, (caddr_t)0) == 0){
		rf_filtpriv	= FALSE;
		rf_privid 	= (uid_t)pm_response.r_val1;
	} else {
		rf_filtpriv	= TRUE;
		rf_privid	= 0; /* used for pre-4.1 clients */ 
	}
...
typedef struct rfsys_loadlpa {
	int		opcode;
	rfsloadlp_t	*rfsloadlp_argp;
} rfsys_loadlpa_t ;

/* ARGSUSED */
STATIC int
rfsys_loadlp(uap, rvp)
	caddr_t		uap;
	rval_t		*rvp;
{
	rfsys_loadlpa_t	*lpp = (rfsys_loadlpa_t *)uap;
	rfsloadlp_t	*tp;
	rfsloadlp_t	*tx;
	rfslp_t		*tq;
	int		error = 0;
	...
	tp = (rfsloadlp_t *)kmem_zalloc(sizeof(rfsloadlp_t), KM_SLEEP);

	/* bring in user's control structure */
	if (copyin((caddr_t)lpp->rfsloadlp_argp, (caddr_t)tp, sizeof(*tp))){
		rfsys_loadlp_recover(tp);
		return EFAULT;
	}

	/* save pointer to rfslpbuf structs in user space */
	tq = tp->rfloadlp_buf;

	/* allocate space for user's client specific data */ 
	tp->rfloadlp_buf = (rfslp_t *)kmem_zalloc(tp->rfloadlp_size,
				KM_SLEEP);

	/* bring in user's client specific data */ 
	if (copyin((caddr_t)tq, (caddr_t)tp->rfloadlp_buf, tp->rfloadlp_size)){
		rfsys_loadlp_recover(tp);
		return EFAULT;
	}

	/* bring in client names from user space */
	if( (error = rfsys_loadlp_hostnames(
			tp->rfloadlp_buf,
			tp->rfloadlp_size)) != 0){
		rfsys_loadlp_recover(tp);
		return error;
	...
	tx = rfs_lid_and_priv_p;	/* save location of old info */
	rfs_lid_and_priv_p = tp;	/* switch in new info (atomically) */
	rfs_gdp_lpupdate();		/* apply new info to current clients */
	rfsys_loadlp_recover(tx);	/* recover space of former info */

	return error;
}

/* Following the practice in rfs/rf_auth.c */
#ifndef BUFSIZ
#define BUFSIZ 1024		/* for client list allocation */
#endif

STATIC int
rfsys_loadlp_hostnames(p, size)
	rfslp_t		*p;
	unsigned	size;
{
	int		i;
	...
	/*
	 * On entry rfslp_t structures have user addresses for hostnames.
	 * Those names will be read in, they will bemeasured,
	   sufficient kernel space allocated will be allocated, and
	   the names will be copied in.
	 * The internal addresses will be set to kernel space.
	 */

	for( i = 0; i < n; i++, p++){
		if ((error = copyinstr(p->rflp_hostname, hostname,
				sizeof(hostname), &len))){
			return error;
		}
		p->rflp_hostname = kmem_zalloc(len, KM_SLEEP );
		bcopy(hostname, p->rflp_hostname, len);
	}
	return error;
}

STATIC void
rfsys_loadlp_recover(tx)
	rfsloadlp_t	*tx;
{
	rfslp_t		*p;
	int		i, n;
	...
	if ( (p = tx->rfloadlp_buf) != NULL ){

		n = tx->rfloadlp_size/sizeof(rfslp_t);

		/* free the spaces for hostnames */
		for( i = 0; i < n; i++, p++){
			if(p->rflp_hostname != NULL){
				kmem_free(p->rflp_hostname,
					strlen(p->rflp_hostname)+1);
			}
		}

		/* free the space for array itself */
		kmem_free(tx->rfloadlp_buf, tx->rfloadlp_size);
	} 

	/* free space for old control structure */
	kmem_free(tx, sizeof(*tx));

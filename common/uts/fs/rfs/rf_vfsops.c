...
#ident	"@(#)uts-comm:fs/rfs/rf_vfsops.c	1.11.3.3"
#ident	"$Header: $"
	...
	for ( ; gp < endgdp; gp++) {
		...
		if (qp && (gp->constate == GDPCONNECT) &&
		  gp->version < RFS2DOT0) {
			...
			if (rfcl_op(sdp, crp, RFSYNCTIME,
			  &rqarg, &bp, FALSE) == 0) {
				if (RF_RESP(bp)->rp_errno == 0) {
					gp->timeskew_sec =
	  ...
	  TRUE)) == 0 && (error = RF_RESP(bp)->rp_errno) == 0) {
		...
		if (RF_PULLUP(bp, rpsz, datasz)) {
			...
			if (gp->hetero != NO_CONV) {
				if (!rf_fcanon(STATVFS_FMT, rpdata,
				  rpdata + datasz, (caddr_t)stvfsp)) {
					gdp_j_accuse("rf_statvfs bad data", gp);
					error = EPROTO;
				}
			} else {
...
STATIC int
rf_setceiling(vfsp, lid)
	register vfs_t	*vfsp;
	lid_t		lid;
{
	vfsp->vfs_macceiling = lid; /* does NOT go remote */
	return 0;
}

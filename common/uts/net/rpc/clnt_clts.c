...
#ident	"@(#)uts-comm:net/rpc/clnt_clts.c	1.9.3.4"
#ident	"$Header: $"
/*
...
STATIC	enum clnt_stat	clnt_clts_kcallit();
STATIC	void		clnt_clts_kabort();
STATIC	void		clnt_clts_kerror();
STATIC	bool_t		clnt_clts_kfreeres();
STATIC	bool_t		clnt_clts_kcontrol();
STATIC	void		clnt_clts_kdestroy();
...
STATIC long	clnt_clts_xid = 0;	/* transaction id used by all clients */
 ...
 * should then call clnt_clts_setxid() to save that transaction ID in the client
...
call_again:
	...
	if (rempos != 0) {
		...
		if ((! XDR_PUTLONG(xdrs, (long *)&procnum)) ||
		    (! AUTH_MARSHALL(h->cl_auth, xdrs, &p->cku_addr)) ||
		    (! (*xdr_args)(xdrs, argsp))) {
 ...
 */
void
clnt_clts_reopen(h, pgm, vers, kncp)
	register CLIENT			*h;
	...
	u_long				vers;
{
	...
	RPCLOG(2, "clnt_clts_reopen: outbuflen %d ", p->cku_outbuflen);
	RPCLOG(2, "sendsz %d, ", sendsz);
	RPCLOG(2, "cku_prog %d ", cku_prog);
	RPCLOG(2, "pgm %d, ", pgm);
	RPCLOG(2, "cku_vers %d ", cku_vers);
	RPCLOG(2, "vers %d\n", vers);

...
#ident	"@(#)uts-comm:net/rpc/auth.h	1.3.2.4"
#ident	"$Header: $"
...
#ifdef _KERNEL
#define AUTH_MARSHALL(auth, xdrs, haddr)	\
		((*((auth)->ah_ops->ah_marshal))(auth, xdrs, haddr))
#define auth_marshall(auth, xdrs, haddr)	\
		((*((auth)->ah_ops->ah_marshal))(auth, xdrs, haddr))
#else
...
/*
 * ESV style authentication
 * AUTH *authesv_create(machname, uid, gid, len, aup_gids, auditid, privs, sens, info, integ, ncs)
 *	char *machname;
 *	uid_t uid;
 *	gid_t gid;
 *	int len;
 ...
 *	s_token ncs;
 */
extern AUTH *authesv_create();
extern AUTH *authesv_create_default();	/* takes no parameters */

...
#ident	"@(#)uts-comm:net/tcpip/tcp_output.c	1.4.3.5"
#ident	"$Header: $"
...
send:
	...
	if (len) {
		...
		ASSERT(q);
		bp->b_cont = (q ? tcp_dupblks(WR(q), off, len) : 0);
		if (bp->b_cont == 0) {
			len = 0;
		}
	 ...
	 * delaying messages about window sizes.
	 *
	 * If resending a FIN, be sure not to use a new sequence number. 
	 * If resending a SYN, use sequence number from snd_una.
	 */

...
#ident	"@(#)uts-comm:net/tcpip/in.c	1.6.4.2"
#ident	"$Header: $"
...
struct ip_provider *loopprov;	/* loopback provider */
	...
	mblk_t         *bp;
{
	...
	if (prov->if_flags & IFF_LOOPBACK)
		...
		in_rtinit(prov->if_dstaddr, prov, RTF_HOST | RTF_UP);
	else {
		netaddr = in_makeaddr(prov->ia.ia_subnet, INADDR_ANY);

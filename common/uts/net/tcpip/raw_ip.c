...
#ident	"@(#)uts-comm:net/tcpip/raw_ip.c	1.4.3.2"
#ident	"$Header: $"
	...
	mblk_t         *bp0;
{
	...
	/*
	 * van jacobson's mods for traceroute:
	 * if the protocol is IPPROTO_RAW, assume the user handed us a
	 * complete IP packet.  Otherwise, allocate an mblk for
	 * a header and fill it in as needed.
	 */
	if (inp->inp_proto == IPPROTO_RAW) {
		 ...
		 */
		ip = (struct ip *) (bp0->b_rptr =
			(u_char *) (bp0->b_datap->db_lim - sizeof(struct ip)));

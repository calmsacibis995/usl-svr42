...
#ident	"@(#)uts-comm:net/tcpip/tcp_input.c	1.4.4.4"
#ident	"$Header: $"
	...
	register mblk_t *bp;
{
	...
	if (bp0 = dupb(tcp_dihdr))
		bp0->b_cont = bp;
	else if (bp0 = copyb(tcp_dihdr))
		bp0->b_cont = bp;
	else
#ifdef SYSV
		cmn_err(CE_WARN, "headerize: dupb/copyb failed");
#else
		printf( "headerize: dupb/copyb failed");
#endif
	return (bp0);
}
	...
	mblk_t         *bp;
{
	...
	if (tcpcksum) {
		...
		if (ti->ti_sum = in_cksum(bp, len)) {
			if (tcpprintfs)
#ifdef SYSV
				cmn_err(CE_NOTE,"tcp sum: src %x, sum %x", ntohl(ti->ti_src), ti->ti_sum);
#else
...
moreinput:
	...
	if (off < sizeof(struct tcphdr) || off > ti->ti_len) {
		if (tcpprintfs)
#ifdef SYSV
			cmn_err(CE_NOTE,"tcp off: src %x off %d", ntohl(ti->ti_src), off);
#else
	...
	case TCPS_SYN_SENT:
		...
		tp->t_flags |= TF_ACKNOW;
			...
			inpisconnected(inp);
			tp->t_state = TCPS_ESTABLISHED;
			tp->t_maxseg = MIN(tp->t_maxseg, (ushort_t)tcp_mss(tp));

...
#ident	"@(#)uts-comm:net/transport/sockmod.c	1.12.3.5"
#ident	"$Header: $"
...
socklog(so, "sockmodrsrv: Got T_CONN_IND\n", 0);
			...
			if ((nbp = dupmsg(mp)) == (mblk_t *)NULL) {
				SKMOD_RECOVER(q, mp, sizeof (mblk_t));
				return (0);
			}
	...
	register struct so_so	*so;
{
	...
	case SO_SNDBUF:
		so->sndbuf = *(int *)OPTVAL(opt);
		if (strqset(WR(so->rdq), QHIWAT, 0, so->sndbuf))
			cmn_err(CE_WARN, "sockmod: SO_SNDBUF: couldn't set q_hiwat\n") ;
		break;

	case SO_RCVBUF:
		so->rcvbuf = *(int *)OPTVAL(opt);
		if (strqset(so->rdq, QHIWAT, 0, so->rcvbuf))
			cmn_err(CE_WARN, "sockmod: SO_RCVBUF: couldn't set q_hiwat\n");
	...
	register int		size;
{
	...
	if ((urg_ptr = (struct free_ptr *)kmem_alloc(sizeof (*urg_ptr),
				KM_NOSLEEP)) == NULL) {
		SKMOD_RECOVER(q, mp, sizeof (*urg_ptr));
		kmem_free(buf, size);
		freeb(bp);
		return ((mblk_t *)NULL);
	}
...
socklog(so, "do_esbzero: Entered\n", 0);
	...
	if ((zero_ptr = (struct free_ptr *)kmem_alloc(sizeof (*zero_ptr),
				KM_NOSLEEP)) == NULL) {
		SKMOD_RECOVER(q, mp, sizeof (*zero_ptr));
		kmem_free(buf, size);
		freeb(bp);
		return ((mblk_t *)NULL);
	}
	...
	int		arg;
{
	if (so->udata.so_options & SO_DEBUG || dosocklog & 1) {
		...
		else	(void)strlog(SIMOD_ID, so-so_so, 0, SL_TRACE, str, arg);
	}
}

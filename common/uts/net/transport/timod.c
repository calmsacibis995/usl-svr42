...
#ident	"@(#)uts-comm:net/transport/timod.c	1.8.2.2"
#ident	"$Header: $"
/*
...
STATIC struct tim_tim *tim_tim = NULL;
	...
	cred_t *crp;
{
	...
	if (tim_tim == NULL) {
		/* LINTED kmem_zalloc returns word aligned pointer */
		if ((tim_tim =
		(struct tim_tim *)kmem_zalloc((sizeof(struct tim_tim) * tim_cnt),
			KM_NOSLEEP)) == NULL) {
			return (ENOMEM);
		}
	}

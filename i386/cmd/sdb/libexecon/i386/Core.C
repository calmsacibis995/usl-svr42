#ident	"@(#)sdb:libexecon/i386/Core.C	1.3"
...
void
Core::update_reg( RegRef ref, register long * word, int )
{
	if ( ref <= FP_INDEX ) {	// General Registers
		data->status->pr_reg[ ref ] = word[0];
		return;
	}

	// FP Register Stack or Special Registers
	::memcpy( (char*)data->fpregs, (char*)&sdbfpstate, sizeof(sdbfpstate) );
	return;
}

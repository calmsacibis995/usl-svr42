#ident	"@(#)sdb:libexp/i386/Call2.C	1.9"
...
static double result( Process *, Iaddr );
...
static double result( Process *process, Iaddr top )
{
	double		dbl;
	unsigned long	*p = (unsigned long *)&dbl;
...
Rvalue *get_ret( Process *process, TYPE *ret_type, Iaddr top )
{
	Rvalue *ret_val = 0;
	Fund_type ft;
	Symbol ut;
	...
	if ( ret_type->fund_type(ft) ) {
		...
		default:
			...
			break;
		case ft_sfloat:
			itype.isfloat = (float)result( process, top );
			ret_val = package_itype( Ssfloat, &itype );
			break;
		case ft_lfloat:
			itype.idfloat = result( process, top );
			ret_val = package_itype( Sdfloat, &itype );
			break;
		case ft_xfloat:
			xdbl = result( process, top );
			convert_to_fp((unsigned char *)&xdbl, 8, 0, itype.rawbytes);
			ret_val = package_itype( Sxfloat, &itype );
			break;
		case ft_none:
		case ft_scomplex:
		case ft_lcomplex:

#ident	"@(#)sdb:libexp/i386/Call.C	1.12"
		...
		if ((arg_tmp = arg_flag) != 0 && narg <= fun_narg) {
			...
			case ft_ulong:
				...
				case ft_xfloat:
					...
					size = 12;
					convert_to_fp((unsigned char *)(&arg_single),
							4, 0, arg_string);
					break;
				case ft_lfloat:
			...
			case ft_lfloat:
				...
				case ft_xfloat:
					size = 12;
					convert_to_fp((unsigned char *)(&arg_double),
							8, 0, arg_string);
					break;
...
// call the function
	...
	process->getreg(FP_STACK);
	m = STACK_TOP;

// if requested, fetch and print the result

	if ( fmt ) {
		Rvalue *retval = get_ret ( process, &ret_type, m );
		if ( retval ) {
...
#ifndef PTRACE
	// Release FP registers which be used to store function return values
	n = (m+7) % 8;
	if ( TAG_OF_(n) != 0x3 )	// non-EMPTY tag, clean it up
	{
		n = 0xffff - (0x3 << 2*m);
		n |= (TAG_OF_(m) << (2*m));
		writereg( process, FP_TW, n);
	}
#endif

	...
	for (loc = shdr->sh_addr; ((loc-shdr->sh_addr) < shdr->sh_size) && (errlev < MAXERRS); printline()){
		...
		*/
		...
		if ( dp->adr_mode != CBW  && dp->adr_mode != CWD ) {
			...
			(void) sprintf(mneu, (strlen(mnemonic)<(unsigned int)7 ? "%-7s" : "%-9s "),
			               mnemonic);
		}
		...
		case RM16:
			...
			if (!got_modrm_byte)
				get_modrm_byte(&mode, &reg, &r_m);
			get_operand(mode, r_m, 1, 0);
			reg_name = REG16[reg][1];
...
bad_opcode()
{
	...
	printerrline();	/* to print the error message	*/

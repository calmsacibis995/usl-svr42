...
picode(p) NODE *p;
{
	...
	case UTYPE:
		if (o == STAR && p->in.left->in.op == PLUS) {
			...
			if (q->in.op == ICON && q->in.name != (char *)0) {
			    ...
			    if (q->in.lval == 0 || stat) {
			        DEBUG("Before array PIC rewrite 1", p);
				p->in.left->in.right = picode(p->in.left->in.left);
				p->in.left->in.left = picode(q);
			        DEBUG("After array PIC rewrite 1", p);
			    }
			    else {
				NODE * l;
			        DEBUG("Before array PIC rewrite 2", p);
				l = build_in(PLUS, TPOINT,
				...
				p->in.left->in.left->in.right = picode(p->in.left->in.left->in.right);
			        DEBUG("After array PIC rewrite 2", p);
			    }
			    if (stat) {
	...
	case LTYPE:
		...
		case ICON:
			...
			DEBUG("Before NAME/ICON PIC rewrite", p);
			...
			if (p->tn.strat & VOLATILE)
				r->in.strat |= VOLATILE;
			nfree(p);
			DEBUG("After NAME/ICON PIC rewrite", r);
			return r;
		}
		return p;
	}

...
#ident	"@(#)optim:i386/inline.c	1.10.1.4"
#include <sys/types.h>
#include "optim.h"
#include "optutil.h"
#include "paths.h"
...
int numauto;	/* number of words of autos */
{
	...
	for( ALLN( pn ) ) {
		if (pn->uses & EBI)  /* can't inline function with %ebi */
		  	return;
		if( isprof( pn ) ) 
			totalni -= 2;
		if( pn->op == CALL ) {
...
ilfile()
{
	...
	if( 0 ) {
		int size, sumsize;
		sumsize = 0;
		if( 0 ) fprintf(stderr, "in-line expansion limit = %d\n",
			intpc );
		for( ALLPN( prn ) ) {
			size = prn->pncalls * 
	...
	curr=0;
	linptr = fgets(linebuf, LINELEN, stdin);
	bytes_read = strlen(linebuf);
	for( ; linptr; curr += bytes_read, bytes_read = 0,
		linptr=fgets( linebuf, LINELEN, stdin ) ){

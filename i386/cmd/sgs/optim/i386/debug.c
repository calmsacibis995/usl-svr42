...
#ident	"@(#)optim:i386/debug.c	1.6.1.4"
...
**
**	>>>NOTE<<< TAG_inline_subroutine has been removed from dwarf.h
**	as of load d9.  All references are now commented out here, and
**	this code no longer supports inlining.
**
...
static int line_id = 0;
    ...
    if(init_debug_flag) {
    	...
    	printf(" .4byte ..LN%d_%d-%s\n",UniqueId,line_id,text_begin); /* address delta */
#ifdef IMPL_PUSHSECTION
    	printf("\t.popsection\n");
#else
    	printf("\t.previous\n");
...
char *fname;
{
    ...
    printf("\t.section\t.line\n"); 
    printf("\t.4byte %d; .2byte 0xffff; .4byte %s-%s\n",

	...
	*/
	if (!name())
	{
		if (chtab[*p] & CH_DIG)
		{
			...
			{
				error("invalid old-style @ token: @%s",
					prtstr(p, token_len));
			}
	...
	else
	{
		...
		*/
		...
		error("invalid operator: @%s", prtstr(p, token_len));
	}
	return ExpOp_Pic_GOTOFF;
}
...
#endif
{
	...
	*/
	...
	error("invalid register: %%%s", prtstr(p, len));
	return Reg_eax;
}
...
#endif
{
	...
	for (;;) /* only loop for white space, new-line, and garbage chars */
	{
		...
		*/
		...
		default:
		invalid:;
			error("invalid input character: %s",
				prtstr(cur, (size_t)1));
			current++;
			cur++;
			continue;
		...
		case '>':
			if (*++cur != '>')
				error("invalid right shift operator token");
			else
				current++;
		...
		case '<':
			...
			{
			default:
				error("invalid operator token: <%s>",
					prtstr(cur, (size_t)1));
				continue;
			case 's':
				if (*++cur != '>')
					error("missing > for <s> operator");
				else
					current++;
				return ExpOp_SegmentPart;
			case 'o':
				if (*++cur != '>')
					error("missing > for <o> operator");
				else
					current++;

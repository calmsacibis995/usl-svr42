...
#endif
{
	...
	*--p = 0;
	if (str[0] == '-')
	{
		...
		{
		toobig:;
			exprerror(vp->ev_expr, "BCD number too big: %s", str);
			return;
		}
		len--;
		str++;
...
#endif
{
	...
	{
		if (n != BCD_BYTES)
			fatal("int_todata():bad BCD length: %lu", (Ulong)n);
	...
	if (cp->info.code_form != CodeForm_Integer)
		fatal("int_todata():bad form: %u", (Uint)cp->info.code_form);
	...
	{
		exprwarn(vp->ev_expr, "value does not fit in %lu bytes: %s",

...
#endif
{
	...
	while (--num >= 0)
	{
		...
		if ((n = strlen((const char *)s)) == 0)
			fatal("addinst():zero length mnemonic");
		itp->len = n;
		hval = *s;
		while (--n > 0)
		{
		...
		while ((p = *prev) != 0)
		{
			...
			if (i == 0 && n == p->len)
			{
				fatal("addinst():identical mnemonics: %s",
					(const char *)s);
			}
			if (i > 0)
				prev = &p->right;
...
#endif
{
	...
	*/
	...
	if (CHAR_BIT * sizeof(Ulong) < 16 + CHAR_BIT + 5 + 1)
		fatal("findinst():hash calculation unroll invalid");
	switch (n = len)
	{
	case 0:
		fatal("findinst():zero length mnemonic");
		/*NOTREACHED*/
	default:
		hval = s[0] << 20;
		hval += s[1] << 16;
	...
	if ((itp = addinst(s, n)) != 0)
		return (Inst *) &itp->inst;
err:;
	error("unknown instruction: %s", prtstr(s, n));
	return 0;
}

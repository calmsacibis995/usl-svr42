...
#define ALWAYS_FITS_IN(vp, n) \
	(vp->ev_nsbit <= n && (vp->ev_flags & (EV_LDIFF|EV_RELOC|EV_OFLOW)) == 0)
...
*/
#define FITS_IN(vp, n) \
	(vp->ev_nsbit <= n && (vp->ev_flags & (EV_RELOC|EV_OFLOW)) == 0)
...
#endif
{
    ...
    */
	...
	if ((op->oper_flags & Amode_FPreg) == 0)
	    opererror("operand must be integer or relocatable", op);
	break;
    case ExpTy_Register:
	if (op->oper_flags & Amode_BIS)
	    regopererror("invalid use of register in operand: %s",
		ep->right.ex_reg, op);
	/*FALLTHROUGH*/
    case ExpTy_Integer:
...
#endif
{
	...
	default:
	    opererror("literal must be integer or relocatable", op);
	}
	return;
    }
	...
	if (   ep->ex_op != ExpOp_LeafRegister
	    ...
	    regopererror("invalid segment register: %s", ep->right.ex_reg, op);
	}
	...
	*/
	if ((ep = op->oper_amode.base) != 0) {
		...
		*/
		if (   ep->right.ex_reg != Reg_dx
			...
			regopererror("base must be 32-bit register: %s",
			    ep->right.ex_reg, op);
		}
	    }
	}
	if ((ep = op->oper_amode.index) != 0) {
		...
		regopererror("index must be 32-bit register: %s",
		    ep->right.ex_reg, op);
	    }
...
#endif
{
	secp->sec_impdep = cp->code_impdep;
	return 0;
}
...
#endif
{
	...
	{
		error("invalid .nopsets operand: \"%s\"", prtstr(str, len));
		return;
	}
	secp = cursect();
	cp = secp->sec_last;
	sectfinst(secp, &inst_nopsets, (Oplist *)0);
	cp->code_impdep = ccval;
}
...
#endif
{
	...
	for (;;)
	{
		...
		{
		case 1:
			if (proc_type == ProcType_386 && secp->sec_impdep != 0)
				p[0] = 0xf8;	/* clc */
			else
				p[0] = 0x90;	/* nop */
			break;
		case 2:
			if (secp->sec_impdep != 0)	/* cmpl %edi,%edi */
			{
				p[0] = 0x3b;
				p[1] = 0xff;
			}
		...
		case 4:
			...
			goto copy4;
		case 5:
			if (secp->sec_impdep != 0)	/* cmpl $0,%eax */
			{
			modcc5:;
				p[0] = 0x3d;
				p[1] = 0;
			...
			goto fill3;
		case 6:
			if (secp->sec_impdep != 0)	/* cmpl $0,%edi */
			{
			modcc6:;
				p[0] = 0x81;
				p[1] = 0xff;
...
#endif
{
    ...
    if ((vp->ev_flags & EV_OFLOW) != 0 || (vp->ev_minbit > size * CHAR_BIT))
...
#endif
{
	...
	*/
    ...
    cp->code_varsize = size;	/* remember size of variable portion */
    return size;
}

...
#endif
{
    ...
    */
    if (cp->code_size < 4) {
	exprerror(cp->data.code_expr, MSGsmall);
    }
    else if (vp->ev_nsbit > cp->code_size * CHAR_BIT) {
	exprwarn(cp->data.code_expr, MSGrange, num_tohex(vp->ev_int));
    }
    else {
...
#endif
{
    ...
    */
	...
	break;
    default:
	fatal("relocaddr():bad PIC modifier %#x", vp->ev_pic);
    }
...
#endif
{
    ...
    */
    ...
    default:
	fatal("relocpcrel():bad PIC modifier %#x", vp->ev_pic);
    }
    dorelo(vp, p, secp, type);
    return;
}

...
#ident	"@(#)tar:i386/cmd/tar/tar.c	1.63"
#ident  "$Header: tar.c 1.2 91/08/13 $"
...
int complain;
{
	...
	(void) pfmt(stderr, MM_ACTION, 
		":753:Usage: tar -{txruc}[0-9vfbk[FX]hLielmnopwAC] [tapefile] [blocksize] [tapesize] [argfile] [exclude-file] [-I include-file] files ...\n");
	done(1);
}
...
int ofd;
{
	...
	if (extno != 1) {  /* starting in middle of file? */
		...
		(void) pfmt(vfile, MM_ACTION, 
			":760:OK to read file beginning with volume #%d (y/n) ? ",
			extno);
		if (response() != 'y') {
canit:
			passtape();
...
asknicely:
		...
		if (i != extno) {
			(void) pfmt(stderr, MM_WARNING, 
				...
				extno, i);
			(void) pfmt(stderr, MM_ACTION, 
				":501:Ignore error, Abort this file, or load New volume (i/a/n) ? ");
			c = response();
			if (c == 'a')
				goto canit;
...
char *s;
{
	...
	if ((i = fork()) == 0) {
		...
		(void) pfmt(stderr, MM_ERROR, ":518:pwd failed!: %s\n",
			strerror(errno));
		(void) fprintf(vfile, "/\n");
		exit(1);
	}
	if (i == -1) {
		(void) pfmt(stderr, MM_ERROR, 
			":519:No process to get directory name!: %s\n",
			strerror(errno));
		done(7);
	}

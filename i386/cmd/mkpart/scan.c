...
#ident	"@(#)mkpart:i386/cmd/mkpart/scan.c	1.1.3.2"
#ident	"$Header: scan.c 1.1 91/05/29 $"
...
static	symbol	*hashbuckets[NHASH];	/* for hashing into the symbol table */
...
int len;
	...
	if ( !(space = (char *)malloc((len = buffer_pos - start) + 1)) ) {

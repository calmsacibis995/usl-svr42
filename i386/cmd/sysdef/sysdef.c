...
#ident	"@(#)sysdef:i386/cmd/sysdef/sysdef.c	1.37.11.4"
#ident  "$Header: sysdef.c 1.1 91/06/12 $"
	...
	char	**argv;
{
	...
	if (incore) {
		if((memfile = open(mem,O_RDONLY)) == NULL) {
		...
		*/
		if((sysfile = open(os,O_RDONLY)) == NULL) {
			fprintf(stderr,"cannot open %s\n",os);
			exit(1);
		}

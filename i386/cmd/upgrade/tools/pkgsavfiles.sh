...
#ident	"@(#)upgrade:i386/cmd/upgrade/tools/pkgsavfiles.sh	1.28"
#ident	"$Header: $"
...
#  It saves the 'v' files in /var/sadm/upgrade
...
[ ! -d ${UPGRADE_STORE}/SVR4.2 ] && mkdir -p ${UPGRADE_STORE}/SVR4.2
...
[ "$PKGINSTALL_TYPE" ] || { 
	echo "Error: $0:   PKGINSTALL_TYPE must be set by now" 
...
[ "$PKGINSTALL_TYPE" = OVERLAY ] && {
	...
	grep -v "^/etc/conf" $sav >$olist 2>>$UPERR

...
#ident	"@(#)initpkg:i386/cmd/initpkg/mountall.sh	1.18.23.6"
#ident	"$Header: mountall.sh 1.4 91/08/13 $"
...
do
	...
	F)	FSType=$OPTARG;
		if [ "$TYPES" = "one" ]
		then
			echo "mountall: more than one FSType specified" 1>&2
			exit 2
		else
			TYPES="one"
		fi;
		case $FSType in
		?????????*) 
			echo "mountall: FSType $FSType exceeds 8 characters" 1>&2
			exit 2
		esac
		;;
...
if [ "$FSTAB" != ""  -a  ! -s "$FSTAB" ]
then
	echo "mountall: file system table ($FSTAB) not found" 1>&2
	exit 1
fi
...
#	a '-' in any field is a no-op.
  ...
  This will take a few minutes.\n"
		...
		39)	echo "$special could not be read"
			;;

		1)	echo "fsck failed for $special"
			;;
		esac

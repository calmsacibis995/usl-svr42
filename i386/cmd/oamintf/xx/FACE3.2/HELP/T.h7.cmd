...
Executing commands from the Commands Menu:
     ...
     1.  Strike the function key labeled CMD-MENU (F7) or type
     ...
     2.	 Navigate in the Commands Menu with the arrow keys. If you
         ...
         arguments press RETURN.  The command selected will be issued
     ...
     3.  If you would like to issue a command from the  Commands  Menu,
         ...
         strike ctrl-z (depress the CTRL key and strike z simultaneously).
 	 The highlighted command will appear on the command
         line.  Type  additional arguments and press RETURN to perform the 
         action.

     4.  If you do not wish to issue a command from the Commands Menu,
         strike CANCEL to close the Commands Menu Frame.

Executing commands from the command line:

    1. Strike ctrl-z to see the -> prompt on the command line.
...
Executing commands from the function keys F1 - F8:
    ...
    2. To issue a command strike the function key that is labeled with

...
struct typ_info *info;
{
	...
	omftypes[tindex] = (char *)0;	/* prohibit recursion on this type */
	if(ctyp == T_ENUM){
...
long size;
{
	...
	if(!otypbufsize){
		if((otypbuf = (char *)malloc(expand)) == NULL)
			fatal("Not enough memory to process types data");
	}
	else{
		if((otypbuf = (char *)realloc(otypbuf,otypbufsize+expand)) == NULL)
			fatal("Not enough memory to process types data");
	}
	otypbufsize += expand;

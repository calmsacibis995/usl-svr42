...
#ident	"@(#)crash:i386/cmd/crash/tty.c	1.9.1.3"
...
gettty()
{
	...
	if ((strcmp(type, "") == 0) && !args[optind]) {
		...
		if(line_disp) {
			GETDSYM(wsbase,B_FALSE);
			if(S_wsbase->n_value == 0) {
				line_disp = 0;
				goto _prstty;
			}
			readmem(S_wsbase->n_value,1,-1,&wsbase,
					sizeof(wsbase),"cmux_ws_t *");
			if(wsbase == 0) {
				line_disp = 0;
				goto _prstty;
			}
			readmem(wsbase,1,-1,&wsbase,
					sizeof(wsbase),"cmux_ws_t *");
			if(wsbase == 0) {
				line_disp = 0;
				goto _prstty;
			}
			readmem(wsbase,1,-1,&cmux_ws,sizeof(cmux_ws_t),
					"cmux_ws_t");
			if(cmux_ws.w_cmuxpp == NULL) {
				line_disp == 0;
				goto _prstty;
			}
			cmux_array = 
			     (cmux_t **) calloc(1,count * sizeof(cmux_t *));
			if(cmux_array == NULL) {
				line_disp = 0;
				goto _prstty;
			}
			readmem(cmux_ws.w_cmuxpp,1,-1,cmux_array, 
				count * sizeof(cmux_t *),"w_cmuxpp");
		}
...
_prstty:
		for ( slot = 0; slot < count; slot++) {
			prstty( all, full, slot, phys, addr, "", 0, sheading);
			if(line_disp && *(cmux_array+slot)) {
				readmem(*(cmux_array+slot),1,-1,&cmux,
					sizeof(cmux_t),"cmux_t");
				prsldterm(full,cmux.cmux_rqp);
			}
		}
...
char *heading;
{
	...
	if (is_workstation) {
		...
		idx = slot;
	if (idx == -1)
		readmem((long)addr,!phys,-1,(char *)&tbuf,
...
long addr;
{
	char *heading = "\t  RQADDR MODNAME   MODID  MESSAGE   ENDMSG   MSGLEN STATE\n";

	struct queue q;
	struct module_info minfo;
	struct ldterm_mod ldterm;

	if(findqmod(addr,"ldterm",&q,&minfo) == 0)
		return(0);

	readmem( (long)(q.q_ptr), 1, -1, (char *)&ldterm, sizeof( struct ldterm_mod), "");

	fprintf( fp, "%s", heading);
	fprintf( fp, "\t%x %-9s%6d%9x%9x%9x ", addr, "ldterm", minfo.mi_idnum,
...
struct module_info *minfo;
{
	...
	mname = (char *) calloc(1,namelen);
	if(mname == NULL)
		return(0);

	/*
	 * Wade through the link-lists to extract the desired module
	 */
	while(addr != 0) {
		...
		readmem( (long)(minfo->mi_idname), 1, -1, mname, namelen, "");
		if(strcmp(mname,name) == 0) {
			break;
		}
		addr = (unsigned long) q->q_next;
	}
	free(mname);
	return(addr);
}

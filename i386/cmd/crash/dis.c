...
#ident	"@(#)crash:i386/cmd/crash/dis.c	1.6"
#ident "$Header: dis.c 1.1 91/07/23 $"
...
/*  [30]  */	{{"wrmsr",TERM,GO_ON,0},{"rdtsc",TERM,GO_ON,0},	{"rdmsr",TERM,GO_ON,0},	INVALID,
	...
	while(count--){
		...
		if(absflg){
			...
			if(!sp)
				...
				sprintf(addrbuf,"%-12s",sp->n_offset);

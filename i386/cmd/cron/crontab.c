...
#ident	"@(#)cron:i386/cmd/cron/crontab.c	1.3"
#ident  "$Header: $"
/***************************************************************************
 * Command: crontab
...
char **argv;
{
	...
	if (eflag) {
		...
		if (pid == 0) {		/* child process */
			...
			if (strncmp(editor, NULL, 1) == 0)
				editor = getenv("EDITOR");
			if (strncmp(editor, NULL, 1) == 0)
				editor = "ed";
			(void)sprintf(buf, "%s %s", editor, edtemp);
			(void)printf("%s\n", buf);
			/* there is no MACREAD so don't worry about trojen horse */
			(void) sleep(1);	/* make sure modtime changes */
			if (system(buf) == 0) {
				/* sanity checks */

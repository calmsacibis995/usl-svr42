...
#ident	"@(#)eac:i386/eaccmd/dosutil/dosformat/dosformat.c	1.3.1.5"
#ident  "$Header: dosformat.c 1.2 91/09/30 $"
...
char	**argv;
{
	...
	*/
	...
	/*
		...
		then error;  if the device name does not end in :
		tack one on (as dosformat cannot be passed a filename)
	*/

	(void) strcpy(device, argv[optind]);
	if ((strlen(device) < 6) || (strncmp(device, "/dev/", 5) != 0)) {
		fprintf(stderr, "dosformat: Device must be a special file name such as /dev/rdsk/f03ht\n");
		exit(1);
	}
	...
	*/
	for (trk = 0; trk < parms_struct.dp_heads * (unsigned int)parms_struct.dp_cyls; trk++) {
	...
	*/
	if (MAX_TRACK < parms_struct.dp_secsiz * (unsigned int)parms_struct.dp_sectors) {
	...
	for (trk = 0; trk < parms_struct.dp_heads * (unsigned int)parms_struct.dp_cyls; trk++) {
		...
		*/
		if (read_failed) {
			...
			for (sector = trk * parms_struct.dp_sectors; sector < (long)(trk + 1) * (unsigned int)parms_struct.dp_sectors; ) {
#ifdef DEBUG

...
extern clock_t lbolt;                    /* Ticks from power up */
...
*/
	...
	error = CL_ENTERCLASS(&class[vpixclass], NULL, pp, &pp->p_stat,
		&pp->p_pri, &pp->p_flag, &pp->p_cred,
		&clprocp, NULL, NULL);
	...
	i = splhi();
	CL_EXITCLASS(pp, pp->p_clproc);
	pp->p_cid = vpixclass;
	pp->p_clfuncs = class[vpixclass].cl_funcs;
	pp->p_clproc = clprocp;
	splx(i);
...
long    arg;
{
	...
	if (((u.u_ar0[EFL] & PS_IOPL) < (arg & PS_IOPL)) 
...
*/
	...
	/* If the slice shift changed, set the appropriate nice value. */
		...
		vc_busywait(p, xtp->xt_tslice_shft);

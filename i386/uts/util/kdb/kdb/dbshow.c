...
#ident	"@(#)uts-x86:util/kdb/kdb/dbshow.c	1.6"
#ident	"$Header: $"
...
db_uprocp()
{
	...
	addr.a_as = AS_KVIRT;
	addr.a_addr = (u_long)&u.u_procp;
	if (db_read(addr, (char *)&uprocp, sizeof(proc_t *)) == -1)
		uprocp = (proc_t *)-1;
	return uprocp;
}
	...
	proc_t	*p;
{
	...
	if ((u_int)p < v.v_proc) {
		p = dbg_pid_entry(prslot = (u_int)p);
	} else {
		if ((int)p == -1) {
			if (show)
				dbprintf( "(current) " );
	...
	if (prslot >= v.v_proc)
		p = NULL;
	if (p != db_uprocp() && dbg_pid_entry(prslot) != p)
		p = NULL;
	if (show && p == NULL)
	...
	void		(*prf)();
{
	...
	if (p == db_uprocp()) {
		db_stacktrace(prf, (ulong_t)_debugger, 1);
		return;
	}

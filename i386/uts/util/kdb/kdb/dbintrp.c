...
#ident	"@(#)uts-x86:util/kdb/kdb/dbintrp.c	1.6"
#ident	"$Header: $"
...
{
	if (cpu_family >= 5)
		dbstack[dbtos].value.number = real_cr4();
	else
...
c_brk(arg)
{
	...
	if (debugger_early) {
		dberror("Too early to set breakpoints");
		return;
	}
	...
	if ((cur_brk_type == BRK_IO) && (cpu_family < 5)) {
...
c_single(arg)
{
	...
	if (arg) {
		if (debugger_early) {
			dberror("Too early to single-step");
			return;
		} if (ex_frame == NULL) {
...
"Variables:     = :: vars",
"Breakpoints:   B b brkoff brkon brksoff brkson clrbrk clrbrks curbrk trace ?brk",
"Single Step:   S s SS ss",
"Kernel Data:   pinode pprnode ps psnode puinode pvfs pvnode sleeping",
"Kernel Stack:  pstack stack stackargs",
...
"			for P5 use suffix /io for indicating i/o breakpoint",

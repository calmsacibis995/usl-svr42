...
#ident	"@(#)uts-x86:io/strsubr.c	1.18.4.9"
#ident	"$Header: $"
...
extern	struct	modctl	*mod_shadowcsw[], *mod_shadowfsw[];
extern	int	fmodswsz;

static void freeband();
static void strgiveback();
static void strqbuf();
static void strunbcall();
	...
	cred_t *crp;
{
	...
	/*
	 * If the module is loadable, increment its reference count.
	 */
	if(QU_MODCTLP(rq) = modctlp)	{
		MOD_HOLD(modctlp);
	}
	else	{
		QU_MODIDX(rq) = idx;
	...
	if (rq->q_flag & QOLD) {
		...
		if ((cmpdev(odev) == NODEV) || ((*rq->q_qinfo->qi_qopen)(rq,
		    cmpdev(odev), flag, sflg) == OPENFAIL)) {
			qdetach(rq, 0, 0, crp);
			splx(s);
		...
		/*
		 * This is duplicate functionality of the CLONE driver but is
		 * included here to handle cases when a clone is not opened via
		 * the CLONE driver.
		 */
		idx = getmajor(*devp);
	...
	cred_t *crp;
{
	...
	/*
	 * If the module is loadable, decrement its reference count.
	 */
	if(modctlp = QU_MODCTLP(qp))	{
		MOD_RELE(modctlp);
	}
	...
	file_t *fpdown;
{
	 ...
	 */
	if (mac_installed && (rand = random((ulong)INT_MAX)) != 0)
		lnk_id = (int)rand;
	...
	do {
		...
		for (lp = (struct linkinfo *)Strinfo[DYN_LINKBLK].sd_head; lp; lp = lp->li_next)
	...
	*mpp = NULL;
	...
	if ((mctl != NULL) && (mctl->len >= 0)) {
		...
		if (uiop->uio_segflg == UIO_SYSSPACE) {
			bcopy((caddr_t)base, (caddr_t) bp->b_wptr, ctlcount);
		} else {
...
/* Enhanced Application Compatibilit Support */
int
modispushed(stp, name)
	struct stdata *stp;
	char* name;
{
	int i = findmod(name);
	register queue_t *q = 0;

	if (i >= 0) {
		register struct qinit* wrinit = fmodsw[i].f_str->st_wrinit;
...
/* End Enhanced Application Compatibilit Support */
...
/*
 * Send signal, sig, if Stream is a controlling tty.  For the signal, SIGPOLL,
 * send it to all processes listed on Stream's siglist if the Stream is
 * registered to receive SIGPOLL when a message is received by the Stream Head.
 */
void
...
/*
 * Send SIGPOLL to processes listed on Stream's siglist if Stream is registered
 * for Hangup.  Wakeup any processes polling Stream for Hangup.
 */
strhup(stp)
	struct stdata *stp;
{
...
/*
 * Mark/Allocate Stream as a controlling tty.
 */
stralloctty(sp, stp)
	register sess_t *sp;
	register struct stdata *stp;
...
/*
 * Unmark/Free Stream as a controlling tty.
 */
strfreectty(stp)
	register struct stdata *stp;
{
...
/*
 * Allocate Stream as a controlling tty iff
 *	-Stream is not in an error, or hangup state or is not linked under a
 *	 multiplexor.
 *	-Stream is not already allocated as a controlling tty.
 *	-the calling process is a session leader.
 *	-the calling process does not already have a controlling tty.
 */
int
strctty(pp, stp)
	register proc_t *pp;
...
findioc(iocid)
	long iocid;
{
	register struct striopst *sp;
	register struct striopst *prevsp;

...
#ident	"@(#)uts-x86:io/ttcompat/ttcompat.c	1.7.3.6"
#ident	"$Header: $"
	...
	int dev, oflag, sflag;
{
	...
	if (q->q_ptr != NULL)  {
		...
		/* fail open if TIOCEXCL was done and not privileged */
		if ((tp->t_new_lflags & XCLUDE) && (pm_denied(u.u_cred, P_SYSOPS))) {
			u.u_error = EBUSY;
			return(OPENFAIL);
		}
		else
	...
	bzero((caddr_t)tp,sizeof(ttcompat_state_t));
	tp->t_savbp = bp;
	...
	/* process them right away */
		...
		case TIOCLSET:
			...
			switch (iocbp->ioc_cmd) {
				...
				case SCO_XCSETAW:
				case SCO_XCSETAF:
					cqp->cq_size=sizeof(struct sco_termios);
					break;
	...
	mblk_t *mp;
{
	...
	case SCO_XCGETA:
		...
		/* Update the current state -- May not be needed */
		...
		scop->c_line = 0;
		scop->c_ispeed = tp->t_curstate.t_ispeed;
		scop->c_ospeed = tp->t_curstate.t_ospeed;
	...
	case SCO_XCSETAF:
	    {
		register struct sco_termios *scop;

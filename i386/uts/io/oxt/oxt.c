...
#ident	"@(#)uts-x86:io/oxt/oxt.c	1.7"
#ident	"$Header: $"
...
#endif
#include <io/ioctl.h>  /* for union ioctl_arg */
#include <io/xt/jioctl.h>
#include <io/oxt/xtproto.h>
#include <io/oxt/xt.h>
...
int n;
{
	...
	for (fp = from, tp = to, i = 0 ; i < n ; i+=3) {
		...
		*tp++ = 0x40 | (unsigned char)(c1&0xc0)>>(unsigned char) 2
			| (unsigned char) (c2&0xc0)>>(unsigned char) 4
			| (unsigned char) (c3&0xc0)>> (unsigned char) 6;
		*tp++ = 0x40 | (c1&0x3f);
		if (i+1 < n)
			*tp++ = 0x40 | (c2&0x3f);
		if (i+2 < n)
		...
		case PR_DATA:
			...
			*/
			...
			if (crc((Pbyte *) & Pkt, (int)(pkp->pkt.HEADER_DSIZE + 2)))
				...
				if ((unsigned char) GET_CHAN(pkp->pkt) >= 
					(unsigned char) lp->l.nchans) {

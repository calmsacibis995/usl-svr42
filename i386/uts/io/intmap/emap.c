...
#ident	"@(#)uts-x86:io/intmap/emap.c	1.8"
#ident	"$Header: $"
...
**	removed for 4.1dt.  Thus calls from ldterm and tt1.c for emenmap() and 
...
register struct tty *tp;
{
	...
	struct buf *bp;
	if (tp->t_mstate == ES_NULL)        /* there never was mapping */
		 u.u_error = ENAVAIL;
	else
	tp->t_mstate = ES_START;                /* emapping enabled */
}

str_emenmap(q, emp_tp)
register queue_t *q;
...
caddr_t arg;
{
	...
	if (tp->t_mstate == ES_NULL) {
		...
		for(;;) {					       /*L003*/
			if (copyout((caddr_t)paddr(bp), arg, E_TABSZ)) {	       /*L003*/
...
**	SCO events LDEV_SETTYPE, LDEV_GETEV put in i386/Version4
**	The reference in ldterm.c and tty1.c for LDEMAP and LDDMAP is being
**	removed for 4.1dt.  Thus calls from ldterm and tt1.c for emdsmap() and 
**	str_emdsmap() will not be done.
*/
/*
 * Disable emapping on a line temporarily.
 * Called from ttioctl().
 */
emdsmap(tp)
register struct tty *tp;
{
	...
	tp->t_mstate = ES_OFF;         /* mapping turned off for a while */
}

str_emdsmap(q, emp_tp)
register queue_t *q;
register struct emp_tty *emp_tp;
{
	register struct xmap *xmp;
	...
	emp_tp->t_mstate = ES_OFF;         /* mapping turned off for a while */

#ifdef SCO_DEBUG
	I18N_DEBUG("str_emdsmap: returning ");
#endif
}
...
struct buf *bp;
{
	...
	if (fmp) {
		...
		cmn_err(CE_NOTE, "emaddmap: indices=%d stab=%d sind=%d \n",
...
#ifdef SCO_DEBUG
	cmn_err(CE_NOTE, "emchkmap:nsind=%d nschar=%d ",
				nsind, nschar);
#endif
	return(0);
...
#ifdef SCO_DEBUG
	cmn_err(CE_NOTE, "str_emgetmap: stab=%d sind=%d \n",
		ep->e_stab,ep->e_sind);
#endif
inval:
	return(error);
...
register mblk_t *str_bp;
{
	...
	if (nbufs < 1 || nbufs > NEMBUFS) {
#ifdef SCO_DEBUG
		cmn_err(CE_NOTE, "str_emchkmap invalid bufs %d: ",nbufs);
#endif
		goto inval;
	}

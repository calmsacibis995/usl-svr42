...
int	n;
{
	...
	for (fp = tmpbuf, tp = to, i = 0 ; i < n ; i += 3) {
		...
		*tp++ = (unsigned char)(0x40 | ((int)c1 & 0xc0) >> 2 | ((int)c2 & 0xc0) >> 4 | ((int)c3 & 0xc0) >> 6);
		*tp++ = 0x40 | (c1 & 0x3f);
		if (i + 1 < n)
			*tp++ = 0x40 | (c2 & 0x3f);
		if (i + 2 < n)
 ...
 *
 * +) Change xt.cf/Space.c to reflect the new size of the nxtctl
 *    structure.
 *

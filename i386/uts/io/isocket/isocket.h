...
#ifndef _IO_ISOCKET_ISOCKET_H	/* wrapper symbol for kernel use */
#define _IO_ISOCKET_ISOCKET_H	/* subject to change without notice */

#ident	"@(#)uts-x86:io/isocket/isocket.h	1.3"
#ident	"$Header: $"
...
struct isocdev {
	...
	ushort isoc_state;		/* state of isocket module */
	ushort isoc_domain;		/* network family (domain) name */
	ushort isoc_type;		/* network type */
	ushort isoc_proto;		/* network protocol */
	struct isockaddr isoc_saddr;	/* local network address */
	struct isockaddr isoc_daddr;	/* remote network address */
	ushort isoc_slen;		/* local network length */
	ushort isoc_dlen;		/* remote network length */
	ushort isoc_pend;	/* number of pending connections allowed */
	ushort isoc_backlog;	/* number of pending connections requested */
	ushort isoc_npend;		/* length of isoc_pendlist queue */
	...
	uint   isoc_ioctl;		/* id of ioctl pending */
	int    isoc_ioctlcmd;		/* cmd of ioctl pending */
	ushort isoc_prisig;		/* priority data signal to send */
	ushort isoc_flags;		/* isocket flags */
...
/* wait for locally generated comand to clear the SO_DONTROUTE option */ 

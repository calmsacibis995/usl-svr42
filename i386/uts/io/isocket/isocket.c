...
#ident	"@(#)uts-x86:io/isocket/isocket.c	1.6"
...
#include "net/tcpip/if_arp.h"
#include "net/tcpip/route.h"
#include "io/isocket/isocket.h"
...
#define LOG5(type, str, a1, a2, a3, a4, a5)  \
		{ \
			if (isocdebug & type) \
...
int	isocopen(), isocclose(), isocrput(), isocwput(), isocrsrv(), isocwsrv();
void isocioctl(), isocack(), isocaccack(), isocaddopts();
...
STATIC long ISC_to_SVR4_sioctl[N_ISC_sioctl] = {
...
STATIC int	SVR4_to_ISC_error[N_SVR4NET_ERRS] = {
	...
	/* SVR4 error number */
	/*  95 */		ISC_ENOTSOCK, 	/* Socket operation on non-isocket */
	/*  96 */		ISC_EDESTADDRREQ,
	/*  97 */		ISC_EMSGSIZE,
	/*  98 */		ISC_EPROTOTYPE,
	/*  99 */		ISC_ENOPROTOOPT,
...
int	sflag;	/* stream open flags */
{
	...
	LOG4 (SOCTRACE, "isocopen(%x, %x, %x, %x)\n", q, dev, flag, sflag );


	if (sflag != MODOPEN)
		return (OPENFAIL);	/* ENXIO */
...
#ifdef CLDEBUG
		printf("isocclose: cleanup #3\n");
		printf("isocclose: sd->isoc_head = %x \n", sd->isoc_head);
#endif
 ...
 */
void
isocack (q, mp, prim, type)
queue_t *q;			/* queue pointer */
mblk_t  *mp;			/* message pointer */
int	prim;			/* correct primitive */
int	type;			/* correct message type, M_PROTO or M_PCPROTO */
{
	mblk_t * nmp;
...
int	err, syserr, prim;
{
	...
	LOG5 (SOCTRACE, "isocnak(%x, %x, %x, %x, %x)\n", q, mp, err, syserr, prim);
	...
	if (SO_ERROR_ACK_SIZE <= (mp->b_datap->db_lim - mp->b_datap->db_base)) {
		...
		putbq (q, mp);
		if (bufcall (SO_ERROR_ACK_SIZE, BPRI_MED, qenable, q))
			return (1);
		else {
...
int	number;
{
	...
	if ((bmp = allocb(len, BPRI_MED)) == NULL) {
		putbq (q, mp);
		(void) bufcall (len, BPRI_MED, qenable, q);
		return(1);
	}
	...
	if ((int)tprim->ADDR_length > 0) {
		rval = isoc_aligned_copy(tprim, tprim->ADDR_length, size,
			    &sd->isoc_saddr, &tprim->ADDR_offset);
...
struct isocdev *sd;
{
	...
	if (len <= (mp->b_datap->db_lim - mp->b_datap->db_base)) {
		...
		putbq (q, mp);
		(void) bufcall(sizeof(struct iocblk), BPRI_MED, qenable, q);
		return (1);
	} else
		freemsg (mp);
	...
	if (tprim->DEST_length > 0) {
		isoc_aligned_copy(tprim, sd->isoc_dlen, size,
				  &sd->isoc_daddr, &tprim->DEST_offset); 
...
struct isocdev *sd;
{
	...
	if (tprim->OPT_length > 0) {
		...
		LOG2 (SOCCMDS, "isocconncon: options len = %x, %s\n", 
				tprim->OPT_length, 
				hex_sprintf (mp->b_rptr + tprim->OPT_offset, 
					     tprim->OPT_length));
	}
...
struct isocdev *sd;
{
	...
	if (sd->isoc_pendlist == NULL)
		...
		register mblk_t *omp = sd->isoc_pendlist;
...
queue_t *q;	/* read queue */
{
	...
	if ((mp = allocb (len, BPRI_HI)) == NULL) {
		if (bufcall (len, BPRI_HI, isocaccack, q) == 0) {
			LOG0 (SOCERRS,  "ioscaccack: Losing an accept ack\n" );
		}
		return;
	}
...
struct isocdev *sd;
{
	...
	if (sd->isoc_template == NULL)
		if (isoctemplate (sd) > 0) {
			putbq (q, mp);
			(void) bufcall (64, BPRI_MED, qenable, q);
			return (1);
		}
	...
	if (sd->isoc_type == ISC_SOCK_STREAM) {
		...
		if ((int)tdr->DEST_length > 0) {
			...
			rval = isoc_aligned_copy(tdr, tdr->DEST_length, size,
				    dest, &tdr->DEST_offset);
		...
		if ((sprim->DATA_flags & MSG_DONTROUTE) &&
			...
			if (rval) {
				...
				freemsg(nmp);
				putbq (q, mp);
				(void) bufcall(rval, BPRI_MED, qenable, q);
				return(1);
			}
			freemsg(mp);
			return(0);
 ...
 */
int
isocdataind (q, mp, sd, priority)
queue_t *q;	/* read queue */
mblk_t *mp;	/* message pointer */
struct isocdev *sd;
int	priority;
{
	struct T_data_ind *tprim;
	struct SO_unitdata_ind *sprim;
	mblk_t * nmp;

	LOG4 (SOCTRACE, "isocdataind(%x, %x, %x, %x)\n", q, mp, sd, priority);

	tprim = (struct T_data_ind *) mp->b_rptr;
...
struct isocdev *sd;
{
	...
	LOG3 (SOCTRACE, "isocudataind(%x, %x, %x)\n", q, mp, sd);
 ...
 */
int
isocordrelind (q, mp, sd)
queue_t *q;	/* read queue */
mblk_t *mp;	/* message pointer */
struct isocdev *sd;
{
	struct T_ordrel_ind *tprim = (struct T_ordrel_ind *) mp->b_rptr;
...
struct isocdev *sd;
{
	...
	if ((ump = allocb (size, BPRI_MED)) == NULL) {
		putbq (q, mp);
		(void) bufcall (size, BPRI_MED, qenable, q);
		return (1);
	}
...
int	optval, type;
{
	...
	if (type == 1) {		/* results are in a struct opthdr */
		...
		register struct SO_getopt *sprim;
 ...
 */
int
isoc_shutdown (q, mp, sd)
queue_t *q;	/* write queue */
mblk_t *mp;	/* message pointer to ioctl */
struct isocdev *sd;
{
	int	type;  /* type of shutdown request */
	struct T_ordrel_req *tprim;  /* send orderly release request to tcp */
	register struct iocblk *iocp;
	...
	rval = isoc_sendioctl(q, mp, sd, ump, len, SI_SHUTDOWN, sd->isoc_state);
	return(0);
}

isoc_shdownack(q, mp, sd)
queue_t *q;	/* read queue */
mblk_t *mp;	/* message pointer */
struct isocdev *sd;
{
 ...
 */
int
isocshutdown (q, mp, sd)
queue_t *q;	/* write queue */
mblk_t *mp;	/* message pointer */
struct isocdev *sd;
{
	struct SO_shutdown *sprim;
	struct T_ordrel_req *oprim;
	mblk_t * nmp;
...
struct isocdev *sd;
{
	...
	if (sd->isoc_type == ISC_SOCK_STREAM) {
		register struct T_data_req *tdr;
		...
		register struct T_unitdata_req *tdr;
		int	len;
		int	size;
		int		rval;
		...
		if (sd->isoc_template == NULL) {
			if ((nmp = allocb (len, BPRI_MED)) == NULL) {
				(void) bufcall (len, BPRI_MED, isoctemplate, 
						sd);
				return (1);
			}
		} else
 ...
 */
STATIC int	
isoc_optionlength (level, optname)
int	level, optname;
{
	register int	len = 0;
...
isoc_route(q, mp, sd, val) 
queue_t *q;	/* This is the write queue */
mblk_t *mp;	/* message pointer */
struct isocdev *sd;
int	val;
{ 
...
struct isocdev *sd;
{
	...
	if ((ump = allocb(sizeof(struct si_udata), BPRI_MED)) == NULL) {
		putbq (q, mp);
		(void) bufcall(sizeof(struct si_udata), BPRI_MED, qenable, q);
		return (1);
	}
...
isoc_sendioctl(q, mp, sd, ump, len, smodcmd, state)
queue_t *q;	/* write queue */
mblk_t *mp;	/* message pointer */
struct isocdev *sd;
mblk_t *ump;	/* additional message pointer */
int len;	/* length of the additional message */
int smodcmd;	/* Sockmod ioctl command to send down */
int state;	/* New state for this module */
{
	struct iocblk *iocp;
	mblk_t	*nmp;
	uint	id;
...
isoc_sendproto(q, mp, sd, smodcmd, state)
queue_t *q;	/* write queue */
mblk_t *mp;	/* message pointer */
struct isocdev *sd;
int smodcmd;	/* Sockmod ioctl command to send down */
...
isoc_aligned_copy(buf, len, init_offset, datap, rtn_offset)
char	*buf;
int	len;
int	init_offset;
char	*datap;

...
#ident	"@(#)uts-x86:io/strsubr.h	1.9"
#ident	"$Header: $"
...
#include <util/mod/mod_k.h>	/* REQUIRED */
#endif
...
#include <sys/poll.h>	/* REQUIRED */
#include <sys/stream.h>	/* REQUIRED */
#include <sys/mod_k.h>	/* REQUIRED */
...
struct queinfo {
	...
	struct modctl	*qu_modctlp;	/* for loadable modules */
	int		qu_modidx;	/* for loadable modules */
};
...
#define		QU_MODCTLP(q)	((struct queinfo *)(q))->qu_modctlp
...
extern struct queue	*qtail;		/*  last queue */
extern struct queue	*scanqhead;	/* list for msg consolidation */
extern struct queue	*scanqtail;	/*  last queue */
extern struct bclist	strbcalls;	/* list of waiting bufcalls */
extern int		strscanflag;	/* strscan timeout pending */
extern struct msgb	*msgfreelist;	/* list of free message blocks */
extern struct mdbblock	*mdbfreelist;	/* list of free mdb blocks */
extern struct strinfo	Strinfo[];	/* dynamic resource info */
extern long		Strcount;	/* count of streams resources */
extern unsigned char	qbf[];		/* band flushing backenable flags */

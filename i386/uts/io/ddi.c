...
register struct uio *uiop;
{
	...
	if (upper > limit) {
		if (uap = (struct a *)u.u_ap) { /* check for NULL */
...
cred_t *cr;
{
	return(!pm_privon(cr, pm_privbit(P_DRIVER))?EPERM:0);
}
	...
	uint_t		flags;
{
	 ...
	 */
	if (physaddr + nbytes - 1 < physaddr)
		cmn_err(CE_PANIC, "bad address range in physmap");

	flags &= KM_NOSLEEP;

	base = btop(physaddr);
	size = btopr(PAGOFF(physaddr) + nbytes);
	 ...
	 */
	if (base == 0)
		return((addr_t)phystokv(physaddr));

	while ((addr = (addr_t)sptalloc((int)size, PG_V, (caddr_t)base,
	...
	uint_t		flags;
{
	...
	if ((vaddr -= offset) == (addr_t)phystokv(0))
		return;

...
#ident	"@(#)uts-x86:svc/systm.h	1.10"
#ident	"$Header: $"
...
extern int	Dstflag;	/* configurable daylight savings time flag */
extern int	Timezone;	/* configurable time zone */
extern int	Hz;		/* ticks per second of the clock */
...
extern daddr_t	nswap;		/* size of swap space in blocks */
...
extern caddr_t  waitloc;        /* value of the PC when system is idle */
#define SYSIDLE(pc) ((pc) == waitloc)
...
#define KPT_TIMEIN	20	/* callout table calls			*/

	/*
	**      The following defines are used in ``common'' source
	**      thus, to keep the common portable, use macros
	*/


#define PC_TO_KPC     asm("leal 0(%esp), %eax");

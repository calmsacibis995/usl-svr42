...
/ All Rights Reserved
	...
	.ident	"@(#)uts-x86:svc/intr.s	1.9"
	.ident	"$Header: $"
	.file	"svc/intr.s"

include(../util/symbols.m4)

	.set	RPL_SEL,	0x03	/ Mask for priority levels
ifdef(`MERGE386',`
	.set	VMFLAG, 0x20000
')
ifdef(`VPIX',`
...
ifdef(`MERGE386',`
	...
	jnz	nobrkmon
')
	testw	$RPL_SEL, 4(%esp)	/ Test priority level
	jnz	nobrkmon
...
ifdef(`VPIX',`
	...
	jne	procndp			/ No, theres a chip - usual process
...
setemandemul:
	...
	pushl	$F_ON		/ Clear appropriate flags
	popfl
	pushl	%ebx
	pushl	%ecx
	pushl	%edx
...
/	Setup the ds and es segment registers.
	xorl	%eax, %eax
	movw	$KDSSEL, %ax
	movw	%ax, %ds
	movw	%ax, %es

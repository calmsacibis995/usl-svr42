...
#ident	"@(#)uts-x86:mem/vm_hat.h	1.4"
#ident	"$Header: $"
...
#include <sys/immu.h>	/* COMPATIBILITY */
...
#define PAGE_HAS_MAPPINGS(pp)	((int)(pp)->p_hat.mappings)
...
/*
 * download the ref and mod bits from pte list into pp structure.
 * also, flush the pte from MMU whenever the bits are reset, and
 * the pte is in kernel as. we don't care to flush for non-kernel
 * as since this macro is meant to be used in kernel context.
 *
 * Not worth special-casing on 80x86.
 */
#define hat_getprefmod(pp)	hat_pagesync(pp)


/*
 * download the "mod" bit if it's set, from the pte list into the pp->p_mod.
 */
#define hat_getpmod(pp) { \
	register pte_t *ptep; \
	register hatmap_t *mapp; \
	if ((!(pp->p_mod)) && ((mapp = (hatmap_t *)(pp)->p_hat.mappings) != NULL)) { \
		while ( !(pp->p_mod = mapptoptep(mapp)->pgm.pg_mod) && \
			((mapp = mapp->hat_mnext) != NULL) ); \
	} \
}

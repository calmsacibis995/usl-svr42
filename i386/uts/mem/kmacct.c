...
#ident	"@(#)uts-x86:mem/kmacct.c	1.7"
#ident	"$Header: $"
...
int kmacdevflag = D_OLD;

/*
 * Variables supplied by kmacct.cf
 */
	...
	caddr_t	pcstack[];
{
	...
	for (i = 0; i < num; i++)
	{
		...
		*pc = (caddr_t) *(++fp); 
		...
		if ((oldpcptr > (unsigned int *)0) &&
		((unsigned int)oldpcptr > (unsigned int)&u.u_stack) &&
		((unsigned int)oldpcptr < (unsigned int)&u.u_stack[KSTKSZ]) &&
		(*oldpcptr > (unsigned int)&u.u_stack) &&
		(*oldpcptr < (unsigned int)&u.u_stack[KSTKSZ]))
			fp = oldpcptr;
		else
			break;
	} 
...
int	all;
{
	...
	for ( size = MINASMALL, i = SMINOFFSET;
		 ...
		 * also have to clear Km_Paranoid so we don't recurse if
		 * we call cmn_err().  (cmn_err -> strlog -> allocb
		 * -> kmem_alloc -> kmem_worry -> kmem_check -> cmn_err
		 * ... stack overflow!)
		 */
		oldpri = splhi();
		Km_Paranoid = 0;

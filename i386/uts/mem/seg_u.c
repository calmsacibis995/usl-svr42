...
#ident	"@(#)uts-x86:mem/seg_u.c	1.7"
#ident	"$Header: seg_u.c 1.2 91/06/27 $"
...
pte_t *usertable;	/* pte's for global 'u.' */
 ...
 */
STATIC faultcode_t	segu_fault();
STATIC int		segu_checkprot();
STATIC int		segu_getprot();
STATIC off_t		segu_getoffset();
STATIC int		segu_gettype();
STATIC int		segu_getvp();
STATIC int		segu_kluster();
...
#ifdef __STDC__
STATIC int segu_softload(struct seg *, addr_t, u_int, int, int);
	...
	caddr_t		argsp;
{
	 ...
	 * hat_map(). Seg_u does most of its own dirty work,
	 * setting up its own mappings.
	 */
	...
	register proc_t	*p;
{
	...
	/*
	 * 80x86 ublock can constitute a variable number of pages.
	 */
	...
	register proc_t *p;
{
	...
	/*
	 * 80x86 has variable sized ublocks.
	 */
	ASSERT((MINUSIZE <= usize) && (usize <= MAXUSIZE));
	...
	if (fc != 0) {
		cmn_err(CE_PANIC,
			"swapoutub: segu_fault() failed to swap u area out");
	}
	...
	int	no_swap;	/* flag: don't allocate swap space */
{
	...
	/*
	 * 80x86 ublock can constitute of a variable number of pages.
	 */
	ASSERT((MINUSIZE <= usize) && (usize <= MAXUSIZE));
	 ...
	 * Nor do we need swap space for processes with SLOCK.
	 */
	if (no_swap) {
	...
	struct proc *p;
{
	 ...
	 * Instead we delay it and remove it here. This is 80x86 specific
	...
	int		locked;
{
	...
	ASSERT(!(sup->su_proc->p_flag & SLOCK));
	...
	int		lock;
{
	...
	for ( ; va < vlim; va += ptob(1), i++) {
		...
		err = VOP_GETPAGE(vp, off, ptob(1), (u_int *)NULL,
			pl, ptob(1), seg, va, S_WRITE, sys_cred);
		AUNLOCK(ap);
		if (err) {
			/*
 ...
 * This is needed for i286 emulation and for direct-mapped video
 * which may need to expand the I/O bitmap.
 */
segu_expand(newsz)
register unsigned int newsz;
{
	...
	if (newsz > SEGU_PAGES) {	/* and not... MAXUSIZE - since this driver */
		return 0;		/* can handle only SEGU_PAGES "slots" */
	}
	...
	if (anon_resv(ptob(npages)) == 0) {
		if (segu_debug)
			cmn_err(CE_CONT,
				"segu_expand: no swap space available\n");
		return 0;
	}

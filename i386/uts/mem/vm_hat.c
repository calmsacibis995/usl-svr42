...
#ident	"@(#)uts-x86:mem/vm_hat.c	1.22"
#ident	"$Header: $"
...
#include <proc/disp.h>	/* added for preemption point PREEMPT() */
...
void	hat_pteload(struct seg *, addr_t, page_t *, u_int, u_int, u_int);
#else
paddr_t svirtophys();
...
STATIC ccevent_t cc_cache_page = { CC_CACHE_PAGE, CCBITS_CACHE_PAGE };

/*
 * General debugging.
 */
STATIC u_int hat_debug = 0x4;
STATIC u_int hat_duppts = 1;
STATIC u_int hat_mappts = 1;
STATIC u_int hat_pgfail = 0;
STATIC u_int hat_ptstolen = 0;
STATIC u_int hat_mcflpcnt = 0;
#endif /* DEBUG */
...
register pte_t *ptep;
{
	...
	if ((pp = (page_t *) page_numtopp(ptep->pgm.pg_pfn)) != (page_t *) NULL)
	{
		...
		nextmap = (hatmap_t *)(&(pp->p_hat.mappings));
				...
				/* 0 = not invoked in context of current process */
{
	...
	do {
		 ...
		 * As soon as a page-table is locked use it to
		 * as a terminating condition.  This is needed to
		 * prevent an infinite loop when
		 * more than one page-table is locked.
		 */
		...
		for (mcnum = 0; mcnum < HAT_MCPP && ptap->hatpt_aec > 0; mcp++, mcnum++) {
			...
			if (ec == 0)
			    cmn_err(CE_NOTE,
			    "mcp = %x, ptcp = %x, mcnum = %x, ptap = %x, mapp = %x",
			    mcp, ptcp, mcnum, ptap, mapp);
#endif
			ASSERT(ec);
	...
	register page_t *pp;
{
	...
	mapp = (hatmap_t *)(pp->p_hat.mappings);
	pp->p_hat.mappings = (caddr_t)NULL;
	...
	u_int		flags;
{
	...
	if (ptcp->ptp.hat_mcaec == 0) {
		...
		mcndx = 0;
		for (; mcndx < HAT_EPMC; ptep++, mcndx++, mapp++) {
			if (!mapp || (mapp->hat_mapv != 0)) 
				cmn_err(CE_NOTE, "mcp = %x, ptcp = %x",
					mcp, ptcp);
			ASSERT(mapp);
			ASSERT(mapp->hat_mapv == 0);
			if (!ptep || (ptep->pg_pte != 0))
				cmn_err(CE_NOTE, "mcp = %x, ptcp = %x",
					mcp, ptcp);
			ASSERT(ptep);
			ASSERT(ptep->pg_pte == 0);
...
u_int  lock;
{
	 ...
	 * When a page is physically (i/o) read in (sfs_getapage(),
	 * spec_getapage()), the level of the calling process
	 ...
	 */
	if (pp->p_lid
	&&  MAC_ACCESS(MACEQUAL, pp->p_lid, u.u_cred->cr_lid)) {
		pp->p_lid = (lid_t)0;
#ifdef DEBUG
		cc_limiter(&cc_cache_page, u.u_cred);
#endif
	}
}
...
found_ptap:
	...
	if (pp) {
		mapp->hat_mnext = (hatmap_t *)pp->p_hat.mappings;
		pp->p_hat.mappings = (caddr_t)mapp;
	}
 ...
 *
 * The code has an "#ifdef" on PRECOW.  If PRECOW is defined,
 * then hat_dup will use a heuristic to prevent copy-on-write
 * faults in the child.   The heuristic is to make copies of
 * pages which are mapped copy-on-write and which are dirty,
 * and to map those copies into the child's address space.
 * The intent is to avoid copy-on-write faults in the child
 * following a fork, by pre-copying the C-O-W pages which the
 * child may be likely to touch.  The heuristic is based on
 * an expectation that if the parent has touched a C-O-W page,
 * then the child is likely to as well.  The default is for
 * PRECOW not to be defined, and then the parent and child will
 * share all C-O-W pages initially.
 */
...
struct as *new_as;
{
	...
	do {
		...
		while (mcndx < HAT_EPMC) {
			 ...
			 * there exists at least one valid pte.
			 */
...
printf("hat_dup:page_lookup slept and parent's page is gone %d\n",
			 ...
			 */
			new_mapp->hat_mnext = (hatmap_t *)pp->p_hat.mappings;
			pp->p_hat.mappings = (caddr_t)new_mapp;
...
/*
** hat_asunload --
** Unload the page directory entries of an address space.
** Used to simulate restorpd() for an exiting vfork'd process.
*/

/* (This routine is used to do cleanup when exiting a VM86 process). */
...
{
	register struct hat	*hatp;
	register struct hatpt	*ptap, *eptap;

 ...
 *
 *	internal routines:
 *		kmem_allocspool()	allocate small memory ("buffer") pool
 *		kmem_allocbpool()	allocate  big  memory ("buffer") pool
 *		kmem_freepool()		free memory pool
 *		kmem_coalesce()		coalesce freed memory with "buddy"
 *
 *	special-purpose routines:
 *		kmem_avail()		returns estimate of available memory
 *					(for STREAMS bufcall() compatibility)
 *		kmem_prod()		wake sleepers if memory is available
 *					(called periodically by fsflush daemon)
 *
 *	Routines in the kmem_alloc family take a flag argument (KM_SLEEP or
 *	KM_NOSLEEP) that indicates whether or not the caller is willing to
 *	sleep until the requested memory becomes available.  If the memory
 *	is not immediately available and KM_NOSLEEP was specified, the
 *	allocator will simply return NULL, and the caller may take whatever
 *	action it deems appropriate in the circumstances.  If KM_SLEEP was
 *	specified, the allocator will sleep until memory becomes available;
 *	it will never return NULL.  (There is one special case:  NULL will
 *	always be returned for any request of zero size.)
 */
...
#if defined(DEBUG) || defined(KMACCT)
#include <mem/kmacct.h>
#endif /* DEBUG */
...
#ifndef KM_NOSLEEP
#define	KM_NOSLEEP	1	/* must agree with NOSLEEP in immu.h	*/
#endif /* !KM_NOSLEEP */
...
KSTATIC struct kmeminfo kmeminfo;		/* usage statistics */
...
KSTATIC int	Km_pools[KMEM_NCLASS];	/* # of pools in each class	*/
STATIC int	Km_allocspool;		/* # of calls to kmem_allocspool */
STATIC int	Km_allocbpool;		/* # of calls to kmem_allocbpool */
STATIC int	Km_freepool;		/* # of calls to kmem_freepool */
...
int	flags;		/* flags				*/
{
  ...
  allocMaxagain:
	 ...
	 *	fb_poolp will have been init'd in kmem_alloc[sb]pool() or
 ...
 * It is invoked from main(), running as the kmdaemon process.
 *
 * To free pool:
...
/*
 * kmem_prod		wake up sleepers if memory is available
 *
 * This routine is provided as a hook to be called periodically
 * from a daemon process, to see if somebody is sleeping despite
 * memory being available.  This can happen if some other
 * subsystem returns memory via sptfree (i.e. without telling us
 * about it) and could cause deadlock if we didn't check for it
 * periodically.
 */
void
kmem_prod()

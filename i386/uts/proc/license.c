...
#ident	"@(#)uts-x86:proc/license.c	1.2"
#ident	"$Header: $"
	...
	register int c;
{
	...
	case EUA_UUCP:		/* Enable uucp special */
		u.u_procp->p_user_license |= PU_LOGIN_PROC|PU_LIM_OK|PU_UUCP;
		return 0;
	...
	case EUA_ADD_USER:	/* Add a User */
		...
		cur_lim_user++;
		/*
		 * EUA_ADD_USER comes from login(1M).  So if we're going in
		 * via login and we're not the process group leader, then
		 * we're probably running under a server.  With starlan
		 * it's ttysrv.  Therefore, mark the parent process.
		 *
		 * When this kid dies, exit won't call de-alloc because we're
		 * not the process group leader.  ttysrv is running as root.
		 * It has to be marked so that it's decremented when it dies.
		 */

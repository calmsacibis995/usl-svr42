...
struct sigaltstack {
	char	*ss_sp;		/* pointer to alternate signal stack */
	int	ss_size;	/* size of signal stack */
	int	ss_flags;	/* disposition of signal stack */
};
...
#define sigutok(us,ks)		(*(ks) = (us)->sa_sigbits[0])
#define sigktou(ks,us)		((us)->sa_sigbits[0] = *(ks),	\
				 (us)->sa_sigbits[1] = 0,	\
				 (us)->sa_sigbits[2] = 0,	\
				 (us)->sa_sigbits[3] = 0)
...
typedef struct {
	...
	int	perm;		/* used to denote if signal can be sent */
	int	checkperm;	/* used to determine if permission required */
} sigsend_t;

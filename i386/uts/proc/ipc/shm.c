...
#ident	"@(#)uts-x86:proc/ipc/shm.c	1.3"
#ident	"$Header: $"
	...
	rval_t *rvp;
{
	 ...
	 */
	(void)adt_auditme(A_IPCCHK,ADT_SHM_OP,sp->shm_perm.ipc_secp->ipc_lid);

	/*
	 * Write MAC access is needed since both read and write operations
	 * modify the object.
	 */
	if (error = ipcaccess(&sp->shm_perm, SHM_R|SHM_W, IPC_MAC, u.u_cred))
		goto errret;
        MAC_ASSERT(sp,MAC_SAME);
	if (error = ipcaccess(&sp->shm_perm, SHM_R, IPC_DAC, u.u_cred))
		goto errret;
	if ((uap->flag & SHM_RDONLY) == 0
	  && (error = ipcaccess(&sp->shm_perm, SHM_W, IPC_DAC, u.u_cred)))
		goto errret;
	...
	rval_t *rvp;
{
	...
	if (error = shmconv(uap->shmid, &sp))
         ...
         * pertains to this event and lid.
         */
	(void)adt_auditme(A_IPCCHK,ADT_SHM_CTL,sp->shm_perm.ipc_secp->ipc_lid);
	...
	case IPC_RMID:
		if (error =
		    ipcaccess(&sp->shm_perm, SHM_R|SHM_W, IPC_MAC, u.u_cred))
			break;
                MAC_ASSERT(sp,MAC_SAME);
		...
		indx = (int) (++sp->shm_perm.seq*shminfo.shmmni+(sp - shmem));
		/* SVR3 - seq number remains within SHRT_MAX range */
		if ((uap->cmd == IPC_O_RMID && indx > SHRT_MAX) || (indx < 0))
				sp->shm_perm.seq = 0;
		/*
	...
	case IPC_O_STAT:
		if (error =
		    ipcaccess(&sp->shm_perm, SHM_R, IPC_MAC|IPC_DAC, u.u_cred))
			break;
	...
	/* XENIX Support */
		if (VIRTUAL_XOUT) {
			 ...
			 */
			xds.shm_nattch = (sp->shm_amp->refcnt >> 1) - 1;
	...
	*/
	...
	case IPC_STAT:
		...
		ds.shm_amp = (struct anon_map *)NULL; /* kernel addr */
...
/*
 * shminit - Called by main (main.c) to initialize shared memory stuff.
 */
void
shminit()
{
	register int	i;
	register struct shmid_ds *mp;
	register struct ipc_sec *sp;	/* ptr to sec structs */

	if ((sp = (struct ipc_sec *)
		kmem_zalloc(shminfo.shmmni*sizeof(struct ipc_sec), KM_NOSLEEP))
	==  (struct ipc_sec *)NULL)
		cmn_err(CE_PANIC, "Can't allocate shm security struct.\n");
	
	/* initialize DAC entries in shared memory segments */
	for (i = shminfo.shmmni, mp = shmem; i > 0; i--, mp++, sp++)
		mp->shm_perm.ipc_secp = sp;
}

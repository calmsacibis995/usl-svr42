...
#ident	"@(#)uts-x86:proc/disp.c	1.12"
#ident	"$Header: $"
...
#include <mem/immu.h>
#include <mem/kmem.h>
#include <mem/vm_hat.h>	/* XXX - ublock kludge */
...
extern int	do386b1;	/* enable B1 stepping workaround */
...
proc_t		*old_curproc;	/* previously running process */
proc_t		*oldproc;	/* previous process which zombied and
				   needs its ublock released */
...
KSTATIC dispq_t	*dispq;		/* ptr to array of disp queues indexed by pri */
STATIC int	srunprocs;	/* total number of loaded, runnable procs */
KSTATIC int	nglobpris;	/* number of global sched prios configured */
...
dispinit()
{
	 ...
	 */
	if ((dispq = (dispq_t *)kmem_zalloc(nglobpris * sizeof(dispq_t),
	    KM_NOSLEEP)) == NULL)
		cmn_err(CE_PANIC,
		    "Can't allocate memory for dispatcher queues.");

	if ((dqactmap = (ulong *)kmem_zalloc(((nglobpris / BT_NBIPUL) + 1) *
...
pswtch()
{
	...
	if (pp != old_curproc) {
		...
		/*
		 * Call context-switch notification functions registered
		 * through swtch_notify().
		 */
		for (swp = pp->p_swtch_in; swp; swp = swp->swn_next)
			(*swp->swn_func) (swp);
	} else
		(void) spl0();
...
/*
 * int
 * swtch_notify(swnotify_t *swp, swnotify_cmd_t swcmd)
 *	Register for or cancel notification of when the current execution
 *	context is switched in.
 */
int
swtch_notify(swp, swcmd)
swnotify_t *swp;
swnotify_cmd_t swcmd;
{
	swnotify_t **swpp;

	switch (swcmd) {
	case SWN_REGISTER:	/* register for notification */
		swp->swn_next = u.u_procp->p_swtch_in;
		u.u_procp->p_swtch_in = swp;
		swp->swn_contextp = u.u_procp;
		return 0;
	case SWN_CANCEL:	/* cancel notification */
		if (swp->swn_contextp == NULL)
			return 0;	/* context already exited */
		swpp = &((proc_t *)swp->swn_contextp)->p_swtch_in;
		while (*swpp != swp) {
			if (*swpp == NULL)
				return ESRCH;
			swpp = &(*swpp)->swn_next;
 ...
 * pp into the buffer pointed to by parmsp.  Note that this function
 * does not do a MAC access check.  Callers of this function are
 * responsible for doing the appropriate MAC checks.
 */
void
parmsget(pp, parmsp)
proc_t		*pp;
...
register proc_t		*targpp;
{
	...
	if (reqpp != NULL) {
		 ...
		 */
		if (MAC_ACCESS(MACEQUAL, reqpcredp->cr_lid,
		  targpp->p_cred->cr_lid) && pm_denied(reqpcredp, P_MACWRITE)) {
			 ...
			 */
			if (MAC_ACCESS(MACDOM, reqpcredp->cr_lid,
			    targpp->p_cred->cr_lid))
				return(ESRCH);
			else
				return(EPERM);
		}

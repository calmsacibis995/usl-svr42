...
#ident	"@(#)uts-x86:proc/exec.c	1.17"
#ident	"$Header: $"
...
extern int exec_ncargs;		/* defined in kernel.cf */
...
/*
 * Overlays the current process with another executable.
 */
	...
	rval_t *rvp;
{
	...
	if (uap->argp) {
		/*
		 * Determine the validity of any command arguments
		 * supplied.
		 */
	...
	if (uap->envp) {
		/*
		 * Determine the validity of any environ variables
		 * supplied.
		 */
	...
	PREEMPT();
	/*
	 * The integral argument 0 indicates that this is the first
	 * call to gexec().
	 */
	...
	/* 
	 * Call setregs() to perform the necessary initializations
	 * of the u-area, stack pointer, frame pointer, argument
	 * pointer, and program counter in the u-area.
	 */
...
STATIC exhdmap_t *exhd_freelist;
STATIC int exhd_freeincr = 8;		/* used by kmem_fast_alloc routine */

/*
 * Get a file, allocate a map, and read in the specified
 * portion of the file.
 */
STATIC int
	...
	exhdmap_t **mappp;
{
	...
	/*
	 * Find the map whose current base is equal to the given offset.
	 * When located, fault in the appropriate pages.
	 */
	...
	/* map didn't exist, need a new fbuf */
	...
	mapp = (exhdmap_t *) kmem_fast_alloc(
		...
		exhd_freeincr,
		KM_SLEEP);
	struct_zero((caddr_t)mapp, sizeof(*mapp));
...
/*
 * Handle the case where the file can not be mapped or faulted.
 */
STATIC int
	...
	caddr_t cpp;
{
	...
	/*
	 * Do some housekeeping if there are any maps on the maplist.
	 */
	for (mpp = &ehdap->maplist; (mapp = *mpp) != NULL; ) {
		if (mapp->keepcnt
	...
	if (flags & EXHD_COPY)
		tcp = (caddr_t) cpp;	/* used later to copy file segments */
	else {
		/*
		 * Allocate a map list.
		 */
	...
	if (cnt > size)
		cnt = size;
	for (;;) {
		bcopy(fcp, tcp, cnt);	/* copy the file segments */
		if ((size -= cnt) <= 0)		/* loop terminator */
			return 0;
		tcp += cnt;
		off += cnt;
		error = exhd_getfbuf(ehdap, off, size, flags & EXHD_KEEPMAP,
	...
	register exhda_t *hdp;
{
	...
	if (hdp == NULL)
		return;
	/*
	 * Loop through the map list and place the maps on the
	 * list in the structure specified by hdp on the exhd_freelist.
	 */
	for (mapp = hdp->maplist; mapp != NULL; mapp = nmapp) {
		if (mapp->bndrycasep) {
			/* free the memory for the boundary case */
			kmem_free(mapp->bndrycasep, mapp->bndrycasesz);
		}
		if (mapp->fbufp) {
			/* if the map has a file buffer, release it */
			fbrelse(mapp->fbufp, S_READ);
		}
		nmapp = mapp->nextmap;
		kmem_fast_free((caddr_t *) &exhd_freelist, (caddr_t)mapp);
...
/*
 * Verify the access permissions of the file to be executed.
 */
int
	...
	struct uarg *args;
{
	...
	struct_zero((caddr_t)ehdp, sizeof(*ehdp));
...
/*
 * Determine the effective user and group IDs for the process.
 */
STATIC int
	...
	uid_t *gidp;
{
	...
	/* 
	 * The setuid/setgid-on-exec functionality must be
	 * supported by the file system that contains this file.
	 */
	if ((vp->v_vfsp->vfs_flag & VFS_NOSUID) == 0) {
		if (vattrp->va_mode & VSUID)
			uid = vattrp->va_uid;
		if ((vattrp->va_mode & (VSGID|(VEXEC>>3))) == (VSGID|(VEXEC>>3)))
			gid = vattrp->va_gid;
	}
	 ...
	 * the presence of tracing.
 	 */
	if (((pp->p_flag & STRC) == 0 || !pm_denied(pp->p_cred, P_SYSOPS))
	...
	long *execsz;
{
	...
	int resid;
	cred_t  *oldcrp, *tmpcrp;		
	extern  cred_t  *pm_calcpriv();	
	uid_t uid, gid;
	struct vattr vattr;
	short magic;
	char *mcp;
	...
	if ((error = VOP_OPEN(vpp, FREAD, pp->p_cred)) != 0)
		return error;

	vp = *vpp;
	if ((error = exhd_getmap(&ehda, 0, 2, EXHD_NOALIGN, 
		(caddr_t)&mcp)) != 0) {
		/*
		 * Free the map list.
		 */
	...
	/*
	 * Read the magic number in the file to determine its type
	 * (e.g. ELF, COFF, INTP, etc.).
	 */
	magic = getexmag(mcp);

	/*
	 * See if the setuid/setgid-on-exec bits are set on the file.
	 */
	setid = execsetid(vp, &vattr, &uid, &gid);

	/*
	 * Call the privilege calculation routine to determine
	 * the process privileges. It is essential that the
	 * privileges are calculated BEFORE the call to the
	 * execsw.exec_func so that executable files with the
	 * magic number "#!" can use privilege correctly when
	 * "read" by an interpreter trusted to "pass" privilege
	 * properly. Note, also, that the credentials of the
	 * current process must be modified to take advantage of
	 * this situation.
	 *
	 * NOTE: pm_calcpriv() may call crdup() which sleeps on
	 * memory allocation and does not recognize signals.
	 * This could led to a hung process that can't be killed.
	*/

	oldcrp = pp->p_cred;	/* save current cred struture */


	pp->p_cred = pm_calcpriv(vp, &vattr, pp->p_cred, level);
...
/* End Enhanced Application Compatibility Support */
	...
	/*
	 * Loop through the different exec types and call the
	 * correct exec function based on the magic number.
	 * First, set the extra dynamic memory range to the
	 * default of 0.  Save the current value so that they
	 * can be restored in case of an exec error.
	 */
	oshmbase = u.u_shmbase;
	oshmend = u.u_shmend;
	u.u_shmbase = u.u_shmend = 0;
	...
	if (i >= nexectype && !error)   /* didn't find a good exec type */
		error = ENOEXEC;

	/*
	 * If the current credential structure isn't equal to the
	 * original credential structure (as a result of the call
	 * to pm_calcpriv()) and there was an error, reset the
	 * credentials back to the original credentials and exit.
	 */
	if (pp->p_cred != oldcrp) {
		/* error of -1 is a flag from the eumlators */
		if (error > 0) {
			/*
			 * Save the current credentials temporarily
			 * so we don't lose the information we need.
			 */
			tmpcrp = pp->p_cred;
			/*
			 * Reset the credentials back to the original.
			 */
			pp->p_cred = oldcrp;
			/*
			 * Free the cred structure created by crdup()
			 * in the pm_calcpriv() routine.
			 */
			crfree(tmpcrp);
			u.u_execsw = save_execsw;
			u.u_shmbase = oshmbase;
			u.u_shmend = oshmend;
			goto closevp;
		}
		else {
			/*
			 * There is now a new set of credentials so
			 * decrement the reference count in the original
	...
	/*
	 * The credentials didn't change. If there was an error, get out!
	 */
	else if (error > 0) {
		u.u_execsw = save_execsw;
		u.u_shmbase = oshmbase;
	...
	/*
	 * Special case for the emulators.
	 */
	if (error == -1) {	
		error = 0;
		goto closevp;
	...
	if (level == 0) {		/* only do first time through gexec */
		if (setid) {
			/*
			...
			if (oldcrp == pp->p_cred) /* not copied in pm_calcpriv */
				pp->p_cred = crcopy(pp->p_cred);
			pp->p_cred->cr_uid = uid;
			pp->p_cred->cr_gid = gid;
			pp->p_cred->cr_suid = uid;
			pp->p_cred->cr_sgid = gid;
			if (uid < USHRT_MAX)	/* non-EFT compatibility */
				u.u_uid = (o_uid_t) uid;
			else {
				/*
				 * Set the "old" style UID in the proc
				 * structure to something meaningful.
				 */
				u.u_uid = (o_uid_t) UID_NOBODY;
			}

			if (gid < USHRT_MAX)	/* non-EFT compatibility */
				u.u_gid = (o_gid_t) gid;
			else {
				/*
				 * Set the "old" style GID in the proc
				 * structure to something meaningful.
				 */
...
closevp:
	closerr = VOP_CLOSE(vp, FREAD, 1, 0, pp->p_cred);

	return error ? error : closerr;
}

/*
 * Map in a file segment.
 */
int
...
/*
 * Remove the current process image and allocate a new address space.
 * Called from the object file specific code so errors can't be returned
 * to the process doing the exec.  Therefore, the calling routine has
 * the responsibility to send a SIGKILL signal to the new process in the
 * event of any subsequent errors.
 */
int
remove_proc(args)
	struct uarg	*args;
...
/* End XENIX Support */
	...
	nas = as_alloc();	/* allocate a new address space */
	/*
	 * Move the stack to the address just allocated.
	 */
	...
	/*
	 * Update the process's address space pointer
	 * to point to the new address space.
	 */
	p->p_as = nas;
	hat_asload();
...
/*
 * Open the specified file and return a file descriptor in fdp.
 */
int
execopen(vpp, fdp)
	struct vnode **vpp;
...
/*
 * Close the file specified by the supplied file descriptor.
 */
int
execclose(fd)
	int fd;
 ...
 * supplied as the default execsw[].exec_func entry for
 * any object file format that does not define its own.
 *
 * Issue a warning message for missing exec capability
 * and return an error.
 */
	...
	int setid;
{
	...
	if (pn_get(args->fname, args->fnameseg, &pn) == 0) {
		cmn_err(CE_WARN, "missing exec capability for %s",
		  pn.pn_path);
		pn_free(&pn);
	} else
		cmn_err(CE_WARN, "missing exec capability");
	return ENOEXEC;
}
	...
	long		*execsz;
{
	...
	if ((vp->v_vfsp->vfs_flag & VFS_NOSUID) == 0) {
		if (vattr.va_mode & VSUID)
			uid = vattr.va_uid;
		if ((vattr.va_mode & (VSGID|(VEXEC>>3))) == (VSGID|(VEXEC>>3)))
			gid = vattr.va_gid;
	}

	if (((u.u_procp->p_flag & STRC) == 0 || !pm_denied(u.u_cred, P_SYSOPS))

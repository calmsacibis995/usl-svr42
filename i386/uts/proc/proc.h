...
#define p_arg	p_unw.p_int		/* Map MP name to old UNIX name */
	...
	char	p_filler2[52];		/* unused */

	int	p_alarmid;		/* alarm clock timeout id */
	clock_t	p_alarmtime;		/* alarm clock signal time */
	uint	p_inoutage;		/* time on swap device or in memory */
	uint	p_slptime;		/* time process current sleep */
	...
	struct proc *p_nextorph;
 	struct aproc *p_aprocp;		/* audit structure pointer	 */
} proc_t;
...
extern proc_t *proc_adtflush;		/* audit daemon */
 ...
 * flags for relvm()
 */
#define ISADAEMON	0x01
#define ISNOTADAEMON	0x02

/*
 * Macro to clean up parent-child-sibling pointers.
 * 
 * It locates the correct p_orphan/p_nextorphan and
 * p_child/p_sibling links and unlinks the process "p"
 *
 * This macro is used when removing zombie processes,
 * and to clean up the pointers when fork fails.
 */
#define	CLEAN_PCS(p)    { \
	register proc_t	*pcs_q; \
	if ((pcs_q = (p)->p_nextofkin)->p_orphan == (p))	{ \
		pcs_q->p_orphan = (p)->p_nextorph; \
	} else	{ \
		for (pcs_q = pcs_q->p_orphan; pcs_q; pcs_q = pcs_q->p_nextorph)	{ \
			if (pcs_q->p_nextorph == (p))	{ \
				break; \
			} \
		} \
		ASSERT(pcs_q && pcs_q->p_nextorph == (p)); \
		pcs_q->p_nextorph = (p)->p_nextorph; \
	} \
	if ((pcs_q = (p)->p_parent)->p_child == (p))	{ \
		pcs_q->p_child = (p)->p_sibling; \
	} else { \
		for (pcs_q = pcs_q->p_child; pcs_q; pcs_q = pcs_q->p_sibling)	{ \
			if (pcs_q->p_sibling == (p))	{ \
				break; \
			} \
		} \
		ASSERT(pcs_q && pcs_q->p_sibling == (p)); \
		pcs_q->p_sibling = (p)->p_sibling; \
	} \
}

/*
 * constants for the maximum time we will keep track on how long
 * a process has been sleeping or has been in memory or on the
 * swap device. These numbers are guesses.
 */

#define MAXINOUTAGE	60
#define MAXSLEEPTIME	30
...
void pgexit(proc_t *);
void unorphan(struct pid *);
void pgorphan(struct pid *, proc_t *, int);

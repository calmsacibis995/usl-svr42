...
#ident	"@(#)uts-x86:proc/obj/coff.c	1.9"
#ident	"$Header: $"

/*
 * This file contains COFF object specific routines.
 */
...
/*
 * COFF specific exec routine, called through
 * the execsw table.
 */
/* ARGSUSED */
int
...
int setid;
{
	...
	if (error == 0)
		u.u_exdata = edp; /* XXXX dependency on object file */
		u.u_renv = u.u_exdata.ux_renv;	/* 386 specific */
...
exhda_t *ehdp;
{
	...
	while (bp < libend) {
		...
		if (MAC_VACCESS(nvp, VEXEC, u.u_cred)) {
			error = ELIBACC;
			VN_RELE(nvp);
			goto bad;
		}
	...
	/* End Enhanced Application Compatibility Support */
		...
		struct_zero((caddr_t)&lhda, sizeof(lhda));
		lhda.vp = nvp;
...
/*
 * COFF specific core routine, called through
 * the execsw table.
 */

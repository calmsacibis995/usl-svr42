...
#ident	"@(#)uts-x86:fs/procfs/prioctl.c	1.6.3.3"
#ident	"$Header: $"
	...
	int *rvalp;
{
	...
	case PIOCGETU:		/* read struct user */
		up = prumap(p);
		if (RF_SERVER())
			error = prrgetu((caddr_t)up, cmaddr, ctob(USIZE));
		else
	...
	case PIOCSTRACE:	/* set signal trace mask */
	{
		...
		if (copyin(cmaddr, (caddr_t) &smask, sizeof(smask)))
			...
			if (!sigisempty(&sigmask))
				...
				up = prumap(p);
				/*
				 * Check that stop-on-entry/exit
				 * is not set; undo profiling if set.
				 */
				CATCH_FAULTS(CATCH_SEGU_FAULT) {
					if ((up->u_systrap & ~P_PROF) == 0)
						p->p_flag &= ~SPROCTR;
				}
				prunmap(p);
	...
	case PIOCSSIG:		/* set current signal */
	{
		...
		if (cmaddr && copyin(cmaddr, (caddr_t) &info, sizeof(info)))
			...
			/*
			 * Set signaled sleeping process running.
			 */
			if (p->p_stat == SSLEEP
			  && (p->p_flag & (SNWAKE|SNOSTOP)) == 0)
				setrun(p);
...
/*
 * Distinguish "writable" ioctl requests from others.
 */
STATIC int
isprwrioctl(cmd)
	int cmd;

...
#ident	"@(#)uts-x86:fs/flock.c	1.9.3.7"
#ident	"$Header: $"
...
#define	RM_SLEEPLCKS(tmp, lck)						\
	for ((tmp) = sleeplcks; (tmp) != NULL;) {			\
		if ((tmp)->set.l_whence == (lck)->l_whence &&		\
		    ...
		    (tmp)->set.l_sysid == (lck)->l_sysid &&		\
		    (tmp)->set.l_pid == (lck)->l_pid) {			\
...
extern	struct	flckinfo flckinfo;	/* acct info, defined in kernel.cf */
STATIC	struct	filock	*sleeplcks;	/* head of chain of sleeping locks */

/* control structure for covert channel limiter */
STATIC ccevent_t cc_re_flock = { CC_RE_FLOCK, CCBITS_RE_FLOCK };
	...
	struct	flock	*lckdat;
{
	...
	if (flckinfo.reccnt >= tune.t_flckrec) {
		cc_limiter(&cc_re_flock, u.u_cred);
		return NULL;
	}
			...
			if ((found = blocked(*lock_list, lckdat, &insrt))
			  == NULL) {
				...
				if (deadflck(found, lckdat))
				    ...
				    /*
				     * If request is not already on sleeplcks,

...
#ident	"@(#)uts-x86:fs/xx/xxvfsops.c	1.5"
#ident	"$Header: $"
	...
	enum whymountroot why;
{
	...
	case ROOT_INIT:
		 ...
		 */
		vp = (struct vnode *) makespecvp(rootdev, VBLK);
	...
	if (why == ROOT_INIT) {
		...
		if ((error = xxiget(vfsp, S5ROOTINO, &mip)) == 0) {
			ITOVADDR(mip)->v_flag |= VROOT;
			IUNLOCK(mip);
			s5vfsp->vfs_root = ITOVADDR(mip);

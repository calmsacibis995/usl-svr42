...
#ident	"@(#)uts-x86:fs/xx/xxinode.c	1.6"
#ident	"$Header: $"
...
struct	hinode	xxhinode[NHINO];	/* xx hash table */
...
void xxiupdat(), xxiunhash(), xxtloop(), xxinoinit();
int  xxitrunc();
extern struct vnodeops xxvnodeops;
extern struct vfsops xx_vfsops;
...
xx_unload(void)
{
	kmem_free(xxinode, xxninode*sizeof(inode_t));
	return(0);
}
	...
	register struct inode *ip;
{
	...
	if ((ip->i_mode == 0 || ITOVADDR(ip)->v_pages == NULL)
...
{
	register char *p1, *p2;
	register struct vnode *vp = ITOVADDR(ip);

...
#ident	"@(#)uts-x86:fs/xx/xxvnops.c	1.5"
#ident	"$Header: $"
	...
	if (error == 0) {
		...
		*vpp = ITOVADDR(ip);
		...
		if (ISVDEV((*vpp)->v_type)) {
			...
			newvp = (struct vnode *)specvp(*vpp, (*vpp)->v_rdev, (*vpp)->v_type, cr);
			VN_RELE(*vpp);
			if (newvp == NULL)
				error = ENOSYS;
			else
	...
	struct cred *cr;
{
	...
	*vpp = ITOVADDR(ip);
	...
	if (((ip->i_mode & IFMT) == IFREG) && (vap->va_mask & AT_SIZE))
		error = fs_vcode(ITOVADDR(ip), &ip->i_vcode);
	IUNLOCK(ip);
	/*
	 * If vnode is a device return special vnode instead.
	 */
	...
	struct cred *cr;
{
	...
	if (VOP_REALVP(svp, &realvp) == 0)
		svp = realvp;
	sip = VTOI(svp);
	if (svp->v_type == VDIR && pm_denied(cr, P_FILESYS))
		return EPERM;
	tdp = VTOI(tdvp);
	...
	int flags;
{
	...
	bsize = VBSIZE(ITOVADDR(ip));
	devvp = S5VFS(ITOVADDR(ip)->v_vfsp)->vfs_devvp;

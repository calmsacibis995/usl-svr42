...
#ident	"@(#)uts-x86:fs/xx/xxblklist.c	1.2"
#ident	"$Header: $"
...
STATIC int	xxbldblklst();
STATIC int	xxbldindr();
	...
	register struct inode *ip;
{
	 ...
	 */
	if ((err = xxbldblklst(bnptr, ip, nblks)) == 0) {
		while (nblks < npblks)
			bnptr[nblks++] = 0;
	...
	register int		nblks;
{
	...
	while (lp < eptr) {
		err = xxbldindr(ip, &lp, eptr, dev, ip->i_addr[i], i-(NADDR-3));
		if (err)
			return err;
		i++;
	}
...
STATIC
int 
xxbldindr(ip, lp, eptr, dev, blknbr, indlvl)
	struct inode 		*ip;
	register int		**lp;
	register int		*eptr;
	register dev_t	dev;
	...
	while (cnt-- && *lp < eptr) {
		if (indlvl == 0)
			*(*lp)++ = *bnptr++;
			...
			err = xxbldindr(ip, lp, eptr, dev, *bnptr++, indlvl-1);
			if (err) 
				break;
		}
	}

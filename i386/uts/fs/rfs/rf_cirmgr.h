...
#ifndef _FS_RFS_RF_CIRMGR_H	/* wrapper symbol for kernel use */
#define _FS_RFS_RF_CIRMGR_H	/* subject to change without notice */

#ident	"@(#)uts-x86:fs/rfs/rf_cirmgr.h	1.4"
#ident	"$Header: $"
...
struct gdp {
	...
	ulong		inseq;
	int		aclmax;		/* on other */
	int		mac;		/* is other side running MAC? */
	lid_t		lid;		/* used if other side non-MAC */
	lid_t		cmwlid;		/* used if other side non-MAC - CMW */
	pvec_t		priv;		/* allowed priviliges for client */
};
...
/* RFS Mandatory Access Control Indicator for Remote Machine */
#define RFS_NO_MAC	0
#define RFS_MAC		1

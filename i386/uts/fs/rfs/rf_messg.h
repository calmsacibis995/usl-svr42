...
#ifndef _FS_RFS_RF_MESSG_H	/* wrapper symbol for kernel use */
#define _FS_RFS_RF_MESSG_H	/* subject to change without notice */

#ident	"@(#)uts-x86:fs/rfs/rf_messg.h	1.5"
#ident	"$Header: $"
...
/*
 * maximum # of ACLs that can be passed between RFS
 * systems.
 */
#define RF_MAXACL	100
...
#define RFGETACL	135 	/* get ACL list */
#define RFGETACLCNT	136	/* get number of ACLs for file */
#define RFSETACL	137	/* set ACL list for file */
#define RFSETLEVEL	138	/* set LID of file */
#define RFGETDVSTAT	139	/* get devstat of file */
#define RFSETDVSTAT	140	/* set devstat of file */
#define RFMAKEMLD	141	/* make multilevel directory */

#define RF_MAX_OPCODE RFMAKEMLD	/* <- keep this current! */

/* System call numbers for ES system calls that have pathname arguments
 * Used in du_lookup (fake lookup for 3.x compatability)
 */ 
#define DUDEVSTAT	147
#define DULVLFILE	151
#define DUACL		157
#define DUMKMLD		165
...
typedef struct rf_common {
	...
	union {
		...
		lid_t	lid;	/* LID of client in request;
                                 * LID of file in response containing gift;
                                 * RFS2DOT1 and later only
                                 */
		lid_t	cmwlid;	/* CMW - LID of client in request;
                                 * LID of file in response containing gift;
                                 * RFS2DOT1 and later only
                                 */
	} co_un;
...
union rq_arg {
	...
	struct rqsetlevel	rqsetlevel;
	struct rqgetacl		rqgetacl;
	struct rqsetacl		rqsetacl;
};
...
struct rqv2 {
	...
	long	rqv2_priv;
	long	rqv2_cr_flags;
				/* with RF_MAXGROUPS at 32, takes the
...
/*
 * VOP_SYMLINK,VOP_CREATE,VOP_MKDIR,and VOP_MAKEMLD create new directory entry
 */
struct rqmkdent {
	rf_attr_t	attr;
	char		nm[MAXNAMELEN];
...
#define rq_getacl	rq_arg.rqgetacl
#define rq_setacl	rq_arg.rqsetacl
#define rq_setlevel	rq_arg.rqsetlevel
#define rq_ngroups	rqv2.rqv2_ngroups
#define rq_groups	rqv2.rqv2_groups
#define rq_rrdir_gen	rqv2.rqv2_rrdir_gen
#define rq_priv		rqv2.rqv2_priv
#define rq_cr_flags	rqv2.rqv2_cr_flags
...
struct rpgetacl {
	long	dentries;
	long	svr4pad0; /*keep all union members iso-sized */
};
...
struct rpv2 {
	...
	long		macflag;
	long		pad[1];	/* takes the messsage + response less data
				 * to 128 bytes */
				/* changed from 2->1 when macflag added */
};

...
#ident	"@(#)uts-x86:fs/rfs/rfsr_subr.c	1.8"
#ident	"$Header: $"
	...
	rfsr_ctrl_t		*sr_ctrlp;
{
	...
	rf_request_t		*req;
	char			eff_dirname[MLD_SZ];	/* MLD eff dir */
	char			*tcomp = NULL;		/* component when MLD */
	...
	/*
	 * When we begin a name interpretation, consume any leading '/'
	 * characters.  (Presence of these could be considered grounds
	 * for a disconnect.)
	 */
...
next:
	 ...
	 */
	if ((error = pn_stripcomponent(pnp, component)) != 0) {
		goto bad;
	}
...
checkforroot:
	...
	&&  mac_installed) {
		...
		if (!(tvp->v_macflag & VMAC_SUPPORT)) {
			...
			/*
			 * Still relevant, there might be a non-secure
			 * file system mounted within the resource.
			 */
			if (tvp->v_vfsp)  
		...
		&&  !(stp->sr_cred->cr_flags & CR_MLDREAL)
		&&  vp != tvp) {
			if (strcmp(component, "..") == 0) {
				tcomp = component;
			} else {
				fs_itoh(stp->sr_cred->cr_lid, eff_dirname);
				tcomp = eff_dirname;
			}
			/* omit auditing */
			if ((error = MAC_VACCESS(tvp, VEXEC, stp->sr_cred))
		  	  != 0) {
				VN_RELE(tvp);
			} else {
				VN_RELE(vp);
	...
	while ((vfsp = cvp->v_vfsmountedhere) != NULL) {
		...
		&&  !(stp->sr_cred->cr_flags & CR_MLDREAL)) {
			...
			if ((error = mld_deflect(vp, eff_dirname, &tvp, pnp,
			  lookup_flags, rdir, stp->sr_cred)) != 0) {
				cvp = NULL;
				goto bad;
			}
		}
...
skip:
	...
	if (pn_pathleft(pnp) == 0) {
		...
		if (mac_installed && (vp->v_macflag & VMAC_ISMLD) &&
		  !(stp->sr_cred->cr_flags & CR_MLDREAL) &&  tcomp) {
			pn_set(pnp, tcomp);
		} else {
			pn_setlast(pnp);
		}
	...
	register sndd_t		*out_port;	/* channel back to client */
{
	...
	if (!rdup) {
		rcvd_delete(&giftrd, (sysid_t)0, -1L, 1, stp->sr_cred);
		return ENOSPC;
	}
	vattr.va_mask =
	...
	if (stp->sr_vcver >= RFS2DOT0) {
		...
		/* Always LID value to client running this protocol,
		 * if client does not have MAC (e.g., 4.0 or ES sans MAC
		 * these values ignored.
		 * Machines on old protocol (i.e., 3.x) handled below.
		 */
		if ( mac_installed ) {	/* here  on server */
			cop->co_lid	= vp->v_lid;
			resp->rp_macflag= vp->v_macflag;
		} else {
			cop->co_lid	= (lid_t)0;
			resp->rp_macflag= (long)0;
		}
	} else {
	...
	int			isastream;
{
	 ...
	 * memory for the i386, but that is obviously machine-dependent.
	 **************************************************************
	 */
	register unsigned	readsize = MIN(stp->sr_ret_val, ctob(MAXKSEG));
	...
	int			isastream;
{
	...
	register size_t		bufsize = MIN(stp->sr_ret_val, ctob(MAXKSEG));
	...
	case RFWRITE:
		{
			...
			if (!rdu_modecheck(stp->sr_rdup, mode)) {
				return EPROTO;
			}
		}
		break;
		...
		vattr.va_mask = AT_MODE | AT_SIZE;
		  ...
		  stp->sr_cred)) == 0) {
			if (op == RFPUTPAGE) {
				...
				if (uiop->uio_offset > vattr.va_size) {
					error = EFAULT;
				} else if (stp->sr_ret_val + uiop->uio_offset
				  > vattr.va_size) {
					stp->sr_ret_val =
					  vattr.va_size - uiop->uio_offset;
...
rfubyte(from)
	caddr_t from;
{
	unsigned char to[sizeof(char)+1];
	...
	register long		mntid;
{
	...
	if ((rdup = rdu_find(rcvdp, u.u_procp->p_sysid, mntid)) != NULL) {

...
#ifndef _FS_RFS_RF_COMM_H	/* wrapper symbol for kernel use */
#define _FS_RFS_RF_COMM_H	/* subject to change without notice */

#ident	"@(#)uts-x86:fs/rfs/rf_comm.h	1.5"
#ident	"$Header: $"
...
#ifndef _FS_RFS_RF_MESSG_H
#include <fs/rfs/rf_messg.h>	/* REQUIRED */
#endif
...
#include <sys/rf_messg.h>	/* REQUIRED */
...
extern int rf_state;		/* current state of RFS */
...
#define SDMNDLCK	0x40	/* remote file mandatory lock set
				 ...
				 */
#define SDINTER		0x80
#define SDERSATZ	(SDUSED\
			|SDLOCKED\
			|SDLINKDOWN\
			|SDSERVE\
			|SDWANT\
			|SDCACHE\
			|SDMNDLCK) /* hopefully, an impossible case
					except for ersatz */
				/*is this overloading? */
...
#define	ISERSATZ_SD(sdp)	((sdp)->sd_stat == SDERSATZ)
#define	ISERSATZ_VN(vp)		ISERSATZ_SD(VTOSD(vp))
 ...
 * The aggregate of all RFS references to a server file contributes
 * 1 to the v_count for that file.
 */
...
typedef struct rd_user {
	...
	ushort		ru_vcount;	/* client references to denoted vnode */
	ushort		ru_refcnt;	/* working references to rd_user */
...
extern void		rdu_free();
#define	RDU_HOLD(rdup)	((void)((rdup)->ru_refcnt++))
/*
 * Give up held reference to rd_user structure.  May, in general deallocate
 * that structure and associated receive descriptor, thereby releasing
 * the vnode referred to by the receive descriptor.
 */
#define RDU_RELE(rdpp, rdupp)	\
	((void)(--(*(rdupp))->ru_refcnt ? 0 : \
		(*(rdupp))->ru_vcount ? 0 : (rdu_free((rdpp), ((rdupp))), 0)))

...
#ident	"@(#)idtools:i386/ktool/idtools/idspace.c	1.3"
#ident	"$Header:"
...
/* error messages */
#define USAGE		"Usage:  idspace [-i inodes] [-r blocks | -u blocks | -t blocks]"
...
char *argv[];
{
	...
	if (!onlyflag){
		if(stat("/stand/unix", &pstat) && 
			...
			fprintf(stderr, 
	"WARNING: Cannot obtain size of unix, assume %d blocks.\n", UNIXBLK);

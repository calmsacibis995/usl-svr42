...
extern struct ip_provider *yhprov_withdstaddr();
extern struct ip_provider *yhin_onnetof();
extern struct ip_provider yhprovider[], *yhlastprov;
mblk_t         *yhdup_range();
mblk_t         *yhip_insertoptions();

#define satosin(sa)	((struct sockaddr_in *) (sa))

/*
 * YHNP output.This is the upper write service routine for clnp. The packets in
 * the queue contain a skeletal YHNP header (as ipovly).  The queue elements
	...
	queue_t        *q;
{
	...
	while (bp = getq(q)) {
		...
		(void) yhip_output(q, bp);

	}
}

yhip_snduderr(q, addr, error)
queue_t *q;
struct in_addr addr;
int error;
{
	...
	mblk_t         *bp;
{
	...
	if (ip_req->options)
		bp->b_cont = yhip_insertoptions(bp->b_cont, ip_req->options, &hlen);
	...
	if (prov == NULL) {
		...
		if (flags & IP_ROUTETOIF) {
			prov = yhprov_withdstaddr(*dst);
			if (prov == 0){
				prov = yhin_onnetof(yhin_netof(ip->ip_dst));
			}
			if (prov == 0) {
				error = ENETUNREACH;
				goto bad;
			...
			if (ro->ro_rt == 0
			    && yhrtalloc(ro, rtflags) == RT_DEFER) {
				RTFREE(ro->ro_rt);
				ro->ro_rt = 0;
#ifdef SLIP
				/*
				 * might be able to ifdef-out the whole test??
				 */
				yhsldefer(yhip_output, q, bp);
#endif /* SLIP */
				return (0);
			}
			if (ro->ro_rt == 0
				...
				if (yhin_localaddr(ip->ip_dst)){
					...
					if ((prov->if_flags & IFF_UP) &&
					    !(prov->if_flags & (IFF_POINTOPOINT | IFF_LOOPBACK)))  {
							break;
						}
				   }
				   *dst = ip->ip_dst;
...
out:	qp = prov->qbot;
	 ...
	 */
	if (ip->ip_len <= prov->if_maxtu) {
		yhpadpckt(bp->b_cont, prov->if_mintu - ip->ip_len);
		ip->ip_len = htons((u_short) ip->ip_len);
		ip->ip_off = htons((u_short) ip->ip_off);
		ip->ip_sum = 0;
		ip->ip_sum = yhin_cksum(bp->b_cont, hlen);
		yhiplwput(qp, bp);
		goto done;
	}
	/*
	...
	/*
	 * Discard DL header from logical message for yhdup_range's sake. Loop
	...
	for (off = 0; off < ip->ip_len - hlen; off += len) {
		...
		*mhip = *ip;
		if (hlen > sizeof(struct ip)) {
			fhlen = sizeof(struct ip) + yhip_optcopy(ip, mhip, off);
			mhip->ip_hl = fhlen >> 2;
		}
		mh->b_wptr += fhlen;
		...
		mhip->ip_sum = 0;
		mhip->ip_sum = yhin_cksum(mh, fhlen);
		mh->b_cont  = yhdup_range(bp1, off, len);
		if ( mh->b_cont == NULL ) {
			(void) freemsg(mh);
			error = ENOSR;	/* ??? */
			goto bad;
		}
		yhpadpckt(mh, prov->if_mintu - ntohs((u_short) mhip->ip_len));
...
bad:
	if (!(flags & IP_FORWARDING) && error)
		yhip_snduderr(q, ip->ip_dst, error);
	freemsg(bp);
done:
...
/*
 * Insert YHNP options into preformed packet.Adjust YHNP destination as required
 * for YHNP source routing, as indicated by a non-zero in_addr at the start of
 * the options. 
 */
mblk_t         *
yhip_insertoptions(bp, opt, phlen)
	register mblk_t *bp;
	mblk_t         *opt;
	int            *phlen;
{
	...
	register int    off, len;
{
	...
	while (len) {
		...
		if (len != 0 && bp == 0) {
#ifdef SYSV
			cmn_err(CE_PANIC, "yhdup_range 2");
#else
			panic ("yhdup_range 2");
#endif SYSV
		}

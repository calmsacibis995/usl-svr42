...
#define strcmp	yhin_strcmp
#define strlen	yhin_strlen
#define strncmp yhin_strncmp
#define strncpy	yhin_strncpy
#define strcpy	yhin_strcpy
...
extern struct ifstats *yhifstats;
...
yhinet_netmatch(in1, in2)
	struct in_addr  in1, in2;
{

	return (yhin_netof(in1) == yhin_netof(in2));
}

struct in_addr
yhin_makeaddr(net, host)
	u_long          net, host;
{
...
#endif
int             yhsubnetsarelocal = SUBNETSARELOCAL;
...
int             yhin_interfaces;	/* number of external internet interfaces */
struct ip_provider *yhloopprov;	/* loopback provider */
	...
	mblk_t         *bp;
{
	...
	size = (unsigned int)(iocbp->ioc_cmd & ~(IOC_INOUT|IOC_VOID)) >> 16;
	...
	case SIOCSIFDSTADDR:
		...
		if (prov->ia.ia_flags & IFA_ROUTE) {
			yhrtinit(prov->if_dstaddr, *PROV_INADDR(prov),
			       (int) SIOCDELRT, RTF_HOST);
			yhrtinit(*SOCK_INADDR(&ifr->ifr_addr), *PROV_INADDR(prov),
	...
	mblk_t         *bp;
{
	...
	if (prov->ia.ia_flags & IFA_ROUTE) {
		if (prov->if_flags & IFF_LOOPBACK) {
			yhrtinit(*PROV_INADDR(prov), *PROV_INADDR(prov),
			       (int) SIOCDELRT, RTF_HOST);
		} else if (prov->if_flags & IFF_POINTOPOINT) {
			yhrtinit(prov->if_dstaddr, *PROV_INADDR(prov),
			       (int) SIOCDELRT, RTF_HOST);
		} else {
			netaddr = yhin_makeaddr(prov->ia.ia_subnet, INADDR_ANY);
			yhrtinit(netaddr, *PROV_INADDR(prov), (int) SIOCDELRT, 0);
		}
	...
	if (prov->if_flags & IFF_BROADCAST) {
		*SOCK_INADDR(&prov->if_broadaddr) = 
			yhin_makeaddr(prov->ia.ia_subnet, INADDR_BROADCAST);
	...
	if (prov->if_flags & IFF_LOOPBACK)
		yhin_rtinit(*PROV_INADDR(prov), prov, RTF_HOST | RTF_UP);
	else if (prov->if_flags & IFF_POINTOPOINT)
		yhin_rtinit(prov->if_dstaddr, prov, RTF_HOST | RTF_UP);
	else {
		netaddr = yhin_makeaddr(prov->ia.ia_subnet, INADDR_ANY);
		yhin_rtinit(netaddr, prov, RTF_UP);
	}
	prov->ia.ia_flags |= IFA_ROUTE;
	return;
}
	...
	int			flags;
{
	...
	for (mfirst = mprev; m = *mprev; mprev = &m->b_cont) {
		...
		if (flags & RTF_HOST) {
			...
			if (yhinet_netmatch(route->rt_dst, dst) == 0)
				continue;
		}
...
/*
 * ioctls sent downstream by yhin_control come back upstream through
 * yhin_upstream. 
 */
	...
	mblk_t         *bp;
{
	...
	case SIOCSIFNAME:{
			...
			for (ifs = yhifstats; ifs; ifs = ifs->ifs_next) {
				strncpy(name, ifs->ifs_name, IFNAMSIZ);
				yhitox(ifs->ifs_unit, unit);
	...
	struct in_addr  addr;
{
	...
	for (prov = yhprovider; prov <= yhlastprov; prov++) {
		if (prov->qbot
		    && (prov->if_flags & IFF_UP)
		    && yhinet_netmatch(*PROV_INADDR(prov), addr)) {
			return (prov);
		}
	}
	return ((struct ip_provider *) 0);

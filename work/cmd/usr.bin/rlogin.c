	...
	char **argv;
{
	...
	while ( (c = getopt(ac, av, "sdl:e:78L")) != -1 ) {
		switch(c ) {
	...
	ENABLE_WORK_PRIVS;
	if (isosi)
        	rem = rcmd_osi(&host, htons(port_number), pwd->pw_name,
	...
	if (options & SO_DEBUG &&
		...
		perror("rlogin: SO_DEBUG (ignored)");
	if (setsockopt(rem, SOL_SOCKET, SO_LINGER, (char *)&linger,
		sizeof(linger)) < 0)
			perror("rlogin: SO_LINGER (ignored)");
	DISABLE_WORK_PRIVS;
#ifndef SYSV
...
usage:
	fprintf(stderr,
	  "usage: rlogin [ -osi ][ -ex ] [ -l username ] [ -7 ] [ -8 ] [ -L ] host\n");
pop:
	exit(1);
}

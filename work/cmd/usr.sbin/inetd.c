...
#define	TOOMANY		50		/* don't start more than TOOMANY */
#define	CNT_INTVL	20		/* servers in CNT_INTVL sec. */
...
int	dgram_valid_cache = FALSE;
ushort	dgram_state_cache;
level_t	inetd_level,		/* level of current process */
	dgram_hilevel_cache,	/* stored hi-level of datagram device */
	dgram_lolevel_cache,	/* stored low-level of datagram device */
	dgram_level_cache;	/* cached values above are valid */
#endif SYSV
	...
	char *argv[], *envp[];
{
	...
	pid_t pid;
#ifndef SYSV
#define CMD_OPTIONS "dst"
	struct sigvec sv;
#else
	/* The -l option sets the MAC level of inetd.
	** default is USER_LOGIN, as defined in include/mac.h
	*/
#define CMD_OPTIONS "dstl:"
	char *level_opt = USER_LOGIN;
	extern char *optarg;
	char *istate;
	int pidfd;
	int ret;
	...
	while ( (i = getopt(argc, argv, CMD_OPTIONS)) != -1 ) {
		switch (i) {
		...
		*/
		case 'l':
			level_opt = optarg;
			/* MAC string -> level structure*/
			if ((0 != lvlin(level_opt, &inetd_level))
			   && is_enhancedsecurity()) {
				if ( standalone )
					fprintf(stderr,
					"lvlin conversion rejected -l param\n");
				else
					syslog(LOG_WARNING,
					"lvlin conversion rejected -l param\n");
			}
	...
	if (!standalone) {
		...
		if ( istate && ( !strcmp(istate, "enabled")))
			State = PM_ENABLED;
		else if ( istate && (!strcmp(istate, "disabled")))
			State = PM_DISABLED;
		else {
		...
		pid = getpid();
		(void) sprintf(pidstring, "%ld", pid);
		i = strlen(pidstring) + 1;	/* +1 for null */
		ftruncate(pidfd, 0);
	...
	/* Set MAC level for inetd process */
	if (is_enhancedsecurity() && (NULL != level_opt)) {
		/* MAC string -> level structure*/
		if (0 != lvlin(level_opt, &inetd_level)) {
			syslog(LOG_WARNING, "lvlin conversion rejected -l param\n");
		} else {
			/*change process level*/
			if (0 != lvlproc(MAC_SET, &inetd_level)) {
				syslog(LOG_WARNING, "Warning:lvlproc unsuccessfull\n");
			}
		}
	}
	...
	for (;;) {
		...
		if (!sep->se_wait && sep->se_socktype == SOCK_STREAM) {
			...
			ctrl = sep->se_fd;
#ifdef SYSV
		if (is_enhancedsecurity() && (ctrl >= 0)) {
			int ret_val;

			ret_val = set_fd_level(ctrl, DEV_PUBLIC, inetd_level,
						inetd_level, inetd_level);
			if (debug)
				fprintf(debugfp,
					"%s(%d,%d,%d,%d,%d) ret %d\n",
					"DEV_PUBLIC set_fd_level",
					ctrl,
					(unsigned short)DEV_PUBLIC,
					(unsigned long)inetd_level,
					(unsigned long)inetd_level,
					(unsigned long)inetd_level,
					ret_val);
		}
		...
		if (pid == 0) {
			...
			(void) sprintf(addrbuf, "%x.%d", ntohl(rem_addr.sin_addr.s_addr),
			...
			if (sep->se_bi)
				...
				if (sep->se_argv[0] != NULL) {
					...
					CLR_MAXPRIVS_FOR_EXEC execl(sep->se_server,
						rindex(sep->se_server, '/')+1,
						sep->se_socktype == SOCK_DGRAM
						? (char *)0 : addrbuf, (char *)0);
				    } else {
					CLR_MAXPRIVS_FOR_EXEC execv(sep->se_server, sep->se_argv);
				    }
			        } else {
					CLR_MAXPRIVS_FOR_EXEC execv(sep->se_server, sep->se_argv);
				}
	...
	for (;;) {
		...
		for (sep = servtab; sep; sep = sep->se_next)
			...
			if ((is_enhancedsecurity())
				&& (dgram_valid_cache)
				&& (SOCK_DGRAM == sep->se_socktype)
				&& (-1 != sep->se_fd) ) {
					int ret_val;
		
					ret_val = set_fd_level(sep->se_fd,
						dgram_state_cache,
						dgram_level_cache,
						dgram_hilevel_cache,
						dgram_lolevel_cache);
					if (debug)
					     fprintf(debugfp,
						"%s(%d,%d,%d,%d,%d) ret %d\n",
						"UDP reset set_fd_level",
						sep->se_fd,
						(ushort)dgram_state_cache,
						(ulong)dgram_level_cache,
						(ulong)dgram_hilevel_cache,
						(ulong)dgram_lolevel_cache,
						ret_val);
			}
#else
	for (;;) {
...
#define SWAP(a, b) { char *c = a; a = b; b = c; }
		...
		if (sep->se_isrpc) {
			...
			if(strcmp(sep->se_proto, "yhtp") == 0)
				sep->se_ctrladdr.sin_family = AF_OSI;
			else
			...
			if ((is_enhancedsecurity())
				...
				&& (SOCK_DGRAM == sep->se_socktype)
				&& (-1 != sep->se_fd)
				&& (0==fdevstat(sep->se_fd,DEV_GET,&Pdvstat))
				&& (0==flvlfile(sep->se_fd, MAC_GET, &dgram_level_cache))) {
					dgram_valid_cache = TRUE;
					dgram_state_cache = Pdvstat.dev_state;
					dgram_hilevel_cache = Pdvstat.dev_hilevel;
					dgram_lolevel_cache = Pdvstat.dev_lolevel;
	...
	register struct servtab *sep;
{
	...
	if (sep->se_socktype == SOCK_TLI) {
		...
		if ((sep->se_fd = socket(strcmp(sep->se_proto,"yhtp") ? 
			AF_INET : AF_OSI, sep->se_socktype, 0)) < 0) {
...
setsockopt(fd, SOL_SOCKET, opt, (char *)&on, sizeof (on))
		...
		if (turnon(sep->se_fd, SO_KEEPALIVE) < 0)
	...
	if (sep->se_socktype == SOCK_TLI) {
		if (sep->se_proto[0] != '/') {
			...
			if (strncmp(sep->se_proto, "rpc/", 4) == 0) {
				(void) sprintf(buf, "/dev/%s", sep->se_proto + 
					strlen("rpc/"));
			} else
				(void) sprintf(buf, "/dev/%s", sep->se_proto);
...
if((fp=fopen("/tmp/inetd.log","wb")) == NULL)
  	exit(2);
lt=time(NULL);
ptr=localtime(&lt);
fprintf(fp,"******************************\n");
fprintf(fp,"      %s",asctime(ptr));
fprintf(fp,"******************************\n");
fprintf( fp,"Start Inetd echo !\n");
	setproctitle("echo", s);
	read(s, (char *)&len, sizeof(len));
	fprintf(fp,"Length of data will coming is %u\n",len);

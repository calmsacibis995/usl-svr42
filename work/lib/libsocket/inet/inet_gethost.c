...
#ident	"@(#)libsocket:common/lib/libsocket/inet/inet_gethost.c	1.1.3.2"
#ident  "$Header: inet_gethost.c 1.2 91/06/26 $"
	...
	register char *nam;
{
	 ...
	 */
	sin.sin_addr.s_addr = inet_addr(nam);
	if ( (long)sin.sin_addr.s_addr != -1 )
		return(gethostbyaddr(&(sin.sin_addr), sizeof(sin.sin_addr),
								AF_INET));
